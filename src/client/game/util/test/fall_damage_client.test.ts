...
import {
  clientFallDamageTick,
  clientFallDamageTickWithGrace,
} from "@/client/game/util/fall_damage_client";
import { initFallTracker } from "@/shared/game/fall_damage";
...
function simulate(
  ...
  for (const t of ticks) {
    const r = clientFallDamageTick(state, {
      onGround: t.onGround,
      y: t.y,
...
describe("client fall-damage tick", () => {
  ...
  it("suppresses a protected landing without erasing the next real fall", () => {
    let state = initFallTracker(100);

    let tick = clientFallDamageTickWithGrace(state, {
      onGround: false,
      y: 90,
    ...
    tick = clientFallDamageTickWithGrace(state, {
      onGround: true,
      y: 80,
      canTakeFallDamage: true,
      canApplyFallDamage: false,
  ...
  it("uses hard landing impact as a fallback when cached ground state missed the fall", () => {
    const tick = clientFallDamageTickWithGrace(initFallTracker(90), {
      onGround: true,
      y: 70,
      canTakeFallDamage: true,
      canApplyFallDamage: true,
