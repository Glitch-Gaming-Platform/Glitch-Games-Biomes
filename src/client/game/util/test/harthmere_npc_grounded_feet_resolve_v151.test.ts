...
import { resolveHarthmereNpcGroundedFeetYV1 } from "@/client/game/util/harthmere_entity_grounding";

describe("resolveHarthmereNpcGroundedFeetYV1 (kill-target grounding stability)", () => {
  it("grounds to the real surface and caches it", () => {
    const out = resolveHarthmereNpcGroundedFeetYV1(
  ...
  it("re-grounds to a new surface after the column changes height", () => {
    ...
    cache = resolveHarthmereNpcGroundedFeetYV1({ status: "grounded", feetY: 14 }, cache).cache;
    const out = resolveHarthmereNpcGroundedFeetYV1({ status: "grounded", feetY: 48 }, cache);
