...
// HARTHMERE_GROUNDING_SINGLE_SOURCE_V151:
...
const WORLD_PLACEMENT_FILES = [
  ...
  "src/client/game/renderers/local_dev/harthmere_quest_object_markers_v145.ts",
  "src/client/game/renderers/local_dev/harthmere_gathering_node_markers_v1.ts",
];
...
describe("HARTHMERE_GROUNDING_SINGLE_SOURCE_V151", () => {
  ...
  it("the shared module exposes the single 'with memory' entrypoint", () => {
    ...
    assert.ok(src.includes("export function harthmereGroundedFeetYWithMemoryV1"));
    assert.ok(src.includes("export const resolveHarthmereGroundedFeetYV1"));
  });
});
