...
export const HARTHMERE_BUSINESS_BOARD_PROCEDURAL_MARKER_VERSION_V1 =
  "harthmere-business-board-procedural-marker-v1" as const;
export const HARTHMERE_BUSINESS_BOARD_PROCEDURAL_POLISH_VERSION_V2 =
  "harthmere-business-board-compact-service-list-v2" as const;

const BUSINESS_BOARD_ACCENTS_V1 = [
  0x4cc9ff,
  0xffd54f,
  0x8ce99a,
  0xff8fab,
...
export interface HarthmereBusinessBoardMarkerLocationV1
{
  id: string;
  label: string;
  x: number;
...
export const HARTHMERE_BUSINESS_BOARD_INTERACTION_RADIUS_V1 = 9;

export const HARTHMERE_BUSINESS_BOARD_MARKER_LOCATIONS_V1: readonly HarthmereBusinessBoardMarkerLocationV1[] =
  HARTHMERE_BUSINESS_OUTPOSTS_V1.map((outpost, index) => {
    ...
    return {
      ...
      accentColor:
        BUSINESS_BOARD_ACCENTS_V1[index % BUSINESS_BOARD_ACCENTS_V1.length],
    };
  });

export function nearestHarthmereBusinessBoardPhysicalPromptV1(
  playerPosition:
    | {
        x: number;
        y?: number;
  ...
  let best: HarthmereBusinessBoardMarkerLocationV1 | undefined;
  let bestDistance = Infinity;
  for (const location of HARTHMERE_BUSINESS_BOARD_MARKER_LOCATIONS_V1) {
    const distance = Math.hypot(
      location.x - playerPosition.x,
      location.z - playerPosition.z,
    );
    if (
      distance > HARTHMERE_BUSINESS_BOARD_INTERACTION_RADIUS_V1 ||
  ...
  return best
    ? {
        ...
        radius: HARTHMERE_BUSINESS_BOARD_INTERACTION_RADIUS_V1,
        distance: bestDistance,
      }
    : undefined;
}
...
function box(
  ...
  mesh.userData.harthmereBusinessBoardMarkerVersion =
    HARTHMERE_BUSINESS_BOARD_PROCEDURAL_MARKER_VERSION_V1;
  mesh.userData.harthmereBusinessBoardPolishVersion =
    HARTHMERE_BUSINESS_BOARD_PROCEDURAL_POLISH_VERSION_V2;
  group.add(mesh);
  return mesh;
}

export function createHarthmereBusinessBoardMarkerMeshV1(
  location: HarthmereBusinessBoardMarkerLocationV1,
): THREE.Group {
  const offset = harthmereBusinessOutpostRuntimeOffsetForTestV1();
  const group = new THREE.Group();
  group.name = `${location.label} ${HARTHMERE_BUSINESS_BOARD_PROCEDURAL_MARKER_VERSION_V1}`;
...
export class HarthmereBusinessBoardMarkerRendererV1 implements Renderer {
  public readonly name = HARTHMERE_BUSINESS_BOARD_PROCEDURAL_MARKER_VERSION_V1;
  private readonly root = new THREE.Group();

  constructor() {
    this.root.name = `harthmere-business-board-markers root ${HARTHMERE_BUSINESS_BOARD_PROCEDURAL_MARKER_VERSION_V1}`;
    for (const location of HARTHMERE_BUSINESS_BOARD_MARKER_LOCATIONS_V1) {
      const marker = createHarthmereBusinessBoardMarkerMeshV1(location);
  ...
  draw(scenes: Scenes, _dt: number): void {
    ...
    if (typeof window !== "undefined") {
      (window as any).__harthmereBusinessBoardMarkerDebugV1 = {
        version: HARTHMERE_BUSINESS_BOARD_PROCEDURAL_MARKER_VERSION_V1,
        boards: () =>
          HARTHMERE_BUSINESS_BOARD_MARKER_LOCATIONS_V1.map((location) => ({
            id: location.id,
...
export function makeHarthmereBusinessBoardMarkerRendererV1() {
  return new HarthmereBusinessBoardMarkerRendererV1();
}
