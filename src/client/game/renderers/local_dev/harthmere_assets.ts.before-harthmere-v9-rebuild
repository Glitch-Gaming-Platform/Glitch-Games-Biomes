...
const PLACEMENTS: RuntimePlacement[] = [
  // HARTHMERE_V8_TOWN_DESIGN_REBUILD_START
  // Town/city design pass. This replaces the old town decoration list with
  // district-driven exterior readability, landmarks, clear roads, visible
  // storefront identities, public spaces, hidden edges, and ambient life.
  // Rule: no loose tabletop props. Small items appear only as combined assets,
  // shelves, racks, counters, crates, barrels, chests, mounted signs, or floor
  // objects so the town never regresses into floating cups/books/coins.

  // North Gate / arrival road: wide gate lane, authority, stables, travel logic.
  P("banner_red", 480, -282, 0, 1.2, "North Gate watch banner", "North Gate"),
  P("banner_red", 494, -282, 0, 1.2, "North Gate watch banner", "North Gate"),
  P("lantern", 479, -270, 0, 0.85, "Gate lantern on post", "North Gate"),
  P("lantern", 496, -270, 0, 0.85, "Gate lantern on post", "North Gate"),
  P(
    "cart_high",
    470,
    -261,
  ...
  P("crate_a", 468, -256, 0, 1.15, "Traveler luggage by stable", "North Gate"),
  P("barrel_fp", 472, -273, 0, 0.82, "Stable water barrel", "North Gate"),
  ...
  A("townsperson_guard", 486, -266, 0, 1.28, "Gate patrol", "North Gate", {
    radius: 4.4,
    speed: 0.25,
    phase: 0.8,
  }),

  // Main road wayfinding markers. These sit outside the walk lane and act like
  // map symbols in world-space: food, law, services, temple, docks, danger.
  P(
    "banner_yellow",
    446,
    -204,
  ...
  P("banner_red", 510, -256, Math.PI, 0.86, "Guard Yard marker", "Guard Yard"),
  P("banner_white", 486, -154, 0, 0.86, "Temple marker", "Temple Green"),
  P("banner_blue", 584, -198, -Math.PI / 2, 0.86, "Dock marker", "River Docks"),
  P(
    "banner_brown",
    404,
    -176,
    Math.PI / 2,
    0.86,
    "Mudden Ward warning cloth",
    "Mudden Ward",
  ),

  // Market Square: open social/event hub. Stalls live on the edges so mounts,
  // NPCs, and players can pass through the fountain loop.
  P(
    "fountain_round",
    486,
    -209,
  ...
  // Market District: bakery and general goods storefronts. Exterior identity is
  // strong; walk lanes remain clear around both doors.
  P(
    "table_plate_fp",
    419.8,
    -192,
  ...
  // Residential district: private home and small lived-in yard, no false shop markers.
  P(
    "bed_twin1",
    451,
    -262,
  ...
  P(
    ...
    Math.PI,
    0.78,
    "Clothes cabinet against north wall",
    "Residential District",
  ),
  P(
    "bookcase_2",
  ...
  // Player Services Plaza: bank, courier, auction, storage. Formal and readable.
  P(
    "table_long_decorated",
    556,
    -222,
  ...
  // Craftsman Row: two shops, an open work lane, and visible economic logic.
  ...
  // Apothecary and Magic edge: distinct storefront silhouettes near Temple route.
  ...
  // Copper Kettle / entertainment district: social space with clear traffic flow.
  P(
    "tavern_bar_corner",
    561,
    -202,
  ...
  P(
    ...
    Math.PI / 2,
    0.8,
    "Wall-mounted tavern lantern",
    "Copper Kettle",
  ),
  A("animal_cat", 536, -203, Math.PI / 2, 0.95, "Tavern cat", "Copper Kettle", {
    radius: 0.9,
    speed: 0.35,
    phase: 0.9,
  }),
  ...
  // Temple Green: quiet landmark district, public courtyard, graveyard, healing path.
  ...
  P(
    ...
    Math.PI,
    0.82,
    "Mounted Missing Bell vigil cloth",
    "Temple Green",
  ),
  ...
  // Noble Rise: clean roads, gardens, guarded restriction. More space, less clutter.
  P(
    "banner_red",
    552,
    -251,
  ...
  P("hedge", 574, -248, 0, 0.85, "Noble hedge", "Noble Rise"),
  A(
    "townsperson_market",
    566,
    -253,
  ...
  // Guard / military district: training yard and legal identity.
  ...
  // River Docks: trade, fishing, warehouse, smuggling edges, clear water boundary.
  ...
  P("barrel_large", 580, -158, 0, 0.78, "Warehouse barrel", "River Docks"),
  P("box_stacked", 598, -154, 0, 0.76, "Warehouse crate stack", "River Docks"),
  ...
  // Mudden Ward: poor district, tight-but-walkable alleys, hidden route hints.
  ...
  // Farm, mill, and orchard: food/water source for the settlement.
  ...
  // Gathering/resource node readability. These are visible markers only; the
  // interactive gathering state lives in LocalDevHarthmereGatheringSystem. Each
  // marker sits on believable ground/water/edge context and avoids door lanes.
  P(
    "rock_wide",
    503,
    -270,
  ...
  // Building/property plot readability. These are preview/worksite markers only;
  // the interactive ownership, construction, repair, permission, and tax state
  // lives in LocalDevHarthmereBuildingSystem. They sit beside roads and service
  // zones without blocking doors, quest NPCs, resource nodes, or main lanes.
  ...
  P("banner_yellow", 456, -198, Math.PI / 2, 0.62, "Market stall property marker", "Building"),
  ...
  // HARTHMERE_V8_TOWN_DESIGN_REBUILD_END
];
...
function createProceduralTownsperson(
  ...
  const palette = townspersonPalette(placement.asset);
  const root = new THREE.Group();
  root.name = placement.name ?? placement.asset;
  root.position.set(...placement.at);
  root.rotation.y = placement.rot ?? 0;
  ...
  // Minecraft-style block proportions: feet start on the authored ground and
  // every body part is a rectangular voxel. These are ambient walkers only,
  // not quest givers, so they cannot break dialogue interactions.
  root.add(
    boxMesh(
    ...
    boxMesh("townsperson-body", [0.38, 0.52, 0.2], [0, 0.68, 0], palette.tunic),
    boxMesh(
  ...
  if (placement.asset === "townsperson_guard") {
    root.add(
      boxMesh("guard-tabard", [0.4, 0.12, 0.22], [0, 0.9, -0.02], 0x141414),
    );
  } else if (placement.asset === "townsperson_courier") {
    root.add(
      boxMesh(
        "courier-satchel",
        [0.18, 0.18, 0.08],
  ...
  } else if (placement.asset === "townsperson_farmer") {
    root.add(
      boxMesh("farmer-hat", [0.42, 0.06, 0.36], [0, 1.33, -0.01], 0xb89652),
    );
  } else if (placement.asset === "townsperson_clergy") {
    root.add(
      boxMesh(
        "clergy-sash",
        [0.08, 0.54, 0.22],
