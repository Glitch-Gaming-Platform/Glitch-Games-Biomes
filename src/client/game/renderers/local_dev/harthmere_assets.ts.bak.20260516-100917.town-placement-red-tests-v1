...
private harthmerePlayerSword?: THREE.Group;
  ...
  private resolveHarthmereRuntimePlacement(placement: RuntimePlacement): RuntimePlacement {
    ...
    const crowded = this.spawnedLifePositions.some(([x, z]) => distanceSq2d(x, z, startX, startZ) < 0.9 * 0.9);
    ...
    for (const radius of rings) {
      for (let i = 0; i < 32; i += 1) {
        ...
        if (this.spawnedLifePositions.some(([sx, sz]) => distanceSq2d(sx, sz, x, z) < 0.9 * 0.9)) {
          continue;
        }
