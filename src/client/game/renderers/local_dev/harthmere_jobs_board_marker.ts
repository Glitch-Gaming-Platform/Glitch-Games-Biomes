// HARTHMERE_JOBS_BOARD_PROCEDURAL_MARKER
//
...
export const HARTHMERE_JOBS_BOARD_PROCEDURAL_MARKER_VERSION =
  "harthmere-jobs-board-procedural-marker" as const;
export const HARTHMERE_JOBS_BOARD_PROCEDURAL_POLISH_VERSION =
  "harthmere-jobs-board-procedural-polish" as const;
export const HARTHMERE_JOBS_BOARD_WANTED_NOTICE_GRAPHIC_VERSION =
  "harthmere-jobs-board-wanted-notice-graphic" as const;
...
function createHarthmereJobsBoardMaterial(color: THREE.Color, opacity = 1) {
...
function addHarthmereJobsBoardBox(
  group: THREE.Group,
  name: string,
...
function addHarthmereJobsBoardLetter(
  group: THREE.Group,
  letter: "J" | "O" | "B" | "S",
  x: number,
  y: number,
...
export function createHarthmereJobsBoardKioskMesh(
  ...
  group.name = `${location.label} kiosk ${HARTHMERE_JOBS_BOARD_PROCEDURAL_MARKER_VERSION}`;
  ...
  const mats = {
    accent: createHarthmereJobsBoardMaterial(accent),
    deepAccent: createHarthmereJobsBoardMaterial(deepAccent),
    darkWood: createHarthmereJobsBoardMaterial(darkWood),
    warmWood: createHarthmereJobsBoardMaterial(warmWood),
    stone: createHarthmereJobsBoardMaterial(stone),
    darkStone: createHarthmereJobsBoardMaterial(darkStone),
    parchment: createHarthmereJobsBoardMaterial(parchment),
    parchmentAlt: createHarthmereJobsBoardMaterial(parchmentAlt),
    warrantRed: createHarthmereJobsBoardMaterial(warrantRed),
    noticeBlue: createHarthmereJobsBoardMaterial(noticeBlue),
    ink: createHarthmereJobsBoardMaterial(ink),
    gold: createHarthmereJobsBoardMaterial(gold),
    moss: createHarthmereJobsBoardMaterial(moss),
    glow: createHarthmereJobsBoardMaterial(glow),
  };
  ...
  addHarthmereJobsBoardLetter(group, "J", -1.15, 4.42, mats.ink);
  addHarthmereJobsBoardLetter(group, "O", -0.38, 4.42, mats.ink);
  addHarthmereJobsBoardLetter(group, "B", 0.39, 4.42, mats.ink);
  addHarthmereJobsBoardLetter(group, "S", 1.16, 4.42, mats.ink);
  ...
  for (let index = 0; index < notices.length; index += 1) {
    ...
    const note = addHarthmereJobsBoardBox(
      group,
    ...
    for (let line = 0; line < 3; line += 1) {
      ...
      const stripe = addHarthmereJobsBoardBox(
        group,
    ...
    for (const pinX of [-width * 0.36, width * 0.36]) {
      const pin = addHarthmereJobsBoardBox(
        group,
  ...
  const banner = addHarthmereJobsBoardBox(
    group,
...
export class HarthmereJobsBoardMarkerRenderer implements Renderer {
  ...
  constructor() {
    this.root.name = `harthmere-jobs-board-markers root ${HARTHMERE_JOBS_BOARD_PROCEDURAL_MARKER_VERSION}`;
    for (const location of HARTHMERE_JOBS_BOARD_MARKER_LOCATIONS) {
      const kiosk = createHarthmereJobsBoardKioskMesh(location);
      this.root.add(kiosk);
  ...
  private publishDebugBridge(): void {
    if (typeof window !== "undefined") {
      (window as any).__harthmereJobsBoardMarkerDebug = {
        ...
        boards: () =>
          HARTHMERE_JOBS_BOARD_MARKER_LOCATIONS.map((location) => ({
            id: location.id,
...
export function makeHarthmereJobsBoardMarkerRenderer() {
  return new HarthmereJobsBoardMarkerRenderer();
}
