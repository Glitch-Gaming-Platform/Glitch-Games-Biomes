...
describe("Harthmere business board procedural markers current", () => {
  it("creates one visible procedural business-board marker for every production business outpost", () => {
    ...
    for (const outpost of HARTHMERE_BUSINESS_OUTPOSTS) {
      ...
      assert.ok(
        location,
        `${outpost.outpostId} should have a procedural business board`
      );
      const dashboard =
  ...
  it("draws the Greenlamp clinic board at the exact dashboard access point", () => {
    ...
    assert.equal(mesh.position.z, -178);
  ...
  it("uses a compact visible service-list board instead of the giant jobs-board graphic", () => {
    ...
    bounds.getSize(size);
    assert.ok(
      size.x >= 3.5 && size.x <= 4.5,
      `business board width should be compact, got ${size.x}`
    );
    assert.ok(
      size.y >= 3 && size.y <= 3.6,
      `business board height should be compact, got ${size.y}`
    );
    assert.ok(
      size.z >= 1 && size.z <= 1.9,
      `business board depth should be compact, got ${size.z}`
    );
    ...
    mesh.traverse((child) => {
      ...
      assert.equal(
        child.frustumCulled,
        false,
        `${child.name} should not be frustum culled near the player`
      );
      assert.equal(
    ...
    assert.ok(
      meshCount >= 20,
      `business board should be made of many visible pieces, got ${meshCount}`
    );
    assert.equal(
      partCounts.get("service_row"),
      5,
      "business board should show a readable list of services/jobs"
    );
    assert.equal(
      partCounts.get("service_checkbox"),
      5,
      "each listed service should have a checkbox/status marker"
    );
    assert.equal(
      partCounts.get("status_chip"),
      5,
      "each listed service should have a status/payout chip"
    );
    assert.equal(
      partCounts.get("use_f_tile"),
      1,
      "business board should visibly advertise the F interaction key"
    );
  });
  ...
  it("uses the physical board itself to drive the F prompt radius", () => {
    ...
    assert.equal(
      prompt?.displayName,
      "Greenlamp Walk-In Clinic Business Board"
    );
  ...
  it("reattaches the procedural business boards after scene recreation", () => {
    ...
    assert.ok(
      secondRoot,
      "business board root must attach to a recreated scene"
    );
    assert.equal(secondRoot, firstRoot);
    assert.equal(firstScenes.three.children.includes(firstRoot!), false);
  });

  it("does not construct invisible audit-board geometry on mobile", () => {
    const renderer = makeHarthmereBusinessBoardMarkerRenderer(true);
  ...
  it("registers the business-board renderer so runtime placement filters cannot remove it", () => {
    ...
    assert.equal(
      assetsSource.includes(
        "BIG BUSINESS BOARD open menu shift terminal hard to miss"
      ),
      false,
