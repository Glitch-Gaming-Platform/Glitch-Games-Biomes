...
import {
  HARTHMERE_BUSINESS_BOARD_INTERACTION_RADIUS_V1,
  HARTHMERE_BUSINESS_BOARD_MARKER_LOCATIONS_V1,
  HARTHMERE_BUSINESS_BOARD_PROCEDURAL_POLISH_VERSION_V2,
  HARTHMERE_BUSINESS_BOARD_PROCEDURAL_MARKER_VERSION_V1,
  createHarthmereBusinessBoardMarkerMeshV1,
  makeHarthmereBusinessBoardMarkerRendererV1,
  nearestHarthmereBusinessBoardPhysicalPromptV1,
} from "@/client/game/renderers/local_dev/harthmere_business_board_marker_v1";
...
describe("Harthmere business board procedural markers V1", () => {
  it("creates one visible procedural business-board marker for every production business outpost", () => {
    assert.equal(
      HARTHMERE_BUSINESS_BOARD_MARKER_LOCATIONS_V1.length,
      HARTHMERE_BUSINESS_OUTPOSTS_V1.length,
    );
    assert.equal(HARTHMERE_BUSINESS_BOARD_MARKER_LOCATIONS_V1.length, 19);

    for (const outpost of HARTHMERE_BUSINESS_OUTPOSTS_V1) {
      const location = HARTHMERE_BUSINESS_BOARD_MARKER_LOCATIONS_V1.find(
  ...
  it("draws the Greenlamp clinic board at the exact dashboard access point", () => {
    ...
    const mesh = createHarthmereBusinessBoardMarkerMeshV1(location!);
  ...
  it("uses a compact visible service-list board instead of the giant jobs-board graphic", () => {
    const location = HARTHMERE_BUSINESS_BOARD_MARKER_LOCATIONS_V1[0];
    const mesh = createHarthmereBusinessBoardMarkerMeshV1(location);
  ...
  it("uses the physical board itself to drive the F prompt radius", () => {
    ...
    assert.ok(location, "Greenlamp clinic board location must exist");
    const prompt = nearestHarthmereBusinessBoardPhysicalPromptV1({
      x: location!.x,
      y: location!.y,
      z: location!.z + HARTHMERE_BUSINESS_BOARD_INTERACTION_RADIUS_V1 - 0.25,
    });
    ...
    const tooFar = nearestHarthmereBusinessBoardPhysicalPromptV1({
      x: location!.x,
      y: location!.y,
      z: location!.z + HARTHMERE_BUSINESS_BOARD_INTERACTION_RADIUS_V1 + 0.25,
    });
  ...
  it("reattaches the procedural business boards after scene recreation", () => {
    const renderer = makeHarthmereBusinessBoardMarkerRendererV1();
    const firstScenes = createNewScenes();
    renderer.draw(firstScenes, 0.016);
    const firstRoot = firstScenes.three.children.find((child) =>
      child.name.includes(HARTHMERE_BUSINESS_BOARD_PROCEDURAL_MARKER_VERSION_V1),
    );
  ...
  it("registers the business-board renderer so runtime placement filters cannot remove it", () => {
    ...
    assert.ok(
      renderersSource.includes("makeHarthmereBusinessBoardMarkerRendererV1"),
    ...
    assert.ok(
      hudSource.includes("nearestHarthmereBusinessBoardPhysicalPromptV1"),
