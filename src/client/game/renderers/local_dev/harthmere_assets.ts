...
import {
  ...
  type HarthmereCreatureSoundProfile,
...
import {
  ...
  type HarthmereCinematicExpressionPlaybackCursor,
...
// Synthetic cutscene actors use this bridge too, so it is no longer optional:
// disabling it would remove flashback cast while the native NPC renderer keeps
// real ECS actors.
...
function isHarthmereEcsDrivenCreatureAsset(asset: string): boolean {
  ...
  return (
    ...
    asset.startsWith("boss_")
  );
}

const HARTHMERE_NO_SPARK_BASIC_ACTOR_MATCH_VERSION =
  "harthmere-no-spark-basic-actor-match";
const HARTHMERE_FIX_DEBUG_RENDERER_CALL_VERSION =
  "harthmere-fix-debug-renderer-call";
const HARTHMERE_ROBUST_PHYSICAL_COMBAT_SANITIZE_VERSION =
  "harthmere-robust-physical-combat-sanitize";
const HARTHMERE_PRODUCTION_POLISH_RUNTIME_VERSION =
  HARTHMERE_PRODUCTION_POLISH_VERSION;
const HARTHMERE_PRODUCTION_VOXEL_SELF_EDIT_RUNTIME_VERSION =
  HARTHMERE_PRODUCTION_VOXEL_SELF_EDIT_VERSION;
const HARTHMERE_FLOATING_BLOCK_RUNTIME_VERSION =
  HARTHMERE_FLOATING_BLOCK_INTEGRITY_VERSION;
const HARTHMERE_RUNTIME_PERFORMANCE_PROFILE_RUNTIME_VERSION =
  HARTHMERE_RUNTIME_PERFORMANCE_PROFILE_VERSION;
const HARTHMERE_SURVEY_PERFORMANCE_RESPONSE_VERSION =
  "harthmere-survey-performance-response";
...
const HARTHMERE_NPC_NAVIGATION_FIX_VERSION =
  "harthmere-npc-grounded-swept-route-navigation";
const HARTHMERE_MARKET_SQUARE_IDENTITY_VERSION =
  "harthmere-market-square-identity";
const HARTHMERE_PLAYER_SERVICES_PLAZA_VERSION =
  "harthmere-player-services-plaza";
const HARTHMERE_COPPER_KETTLE_INN_VERSION = "harthmere-copper-kettle-inn";
const HARTHMERE_CRAFTSMAN_ROW_BLACK_ANVIL_VERSION =
  "harthmere-craftsman-row-black-anvil";
...
const HARTHMERE_TOWN_DEBUG_RUNTIME_FIXES_VERSION =
  "harthmere-town-debug-runtime-fixes";
...
const HARTHMERE_TOWN_AUDIT_PATTERN_FIXES_VERSION =
  "harthmere-town-audit-pattern-fixes";
const HARTHMERE_TOWN_WALK_DEBUG_VERSION = "harthmere-town-walk-debug";
const HARTHMERE_TOWN_SYSTEMS_VERSION =
  "harthmere-town-registry-metadata-collision-lod";
const HARTHMERE_ASSET_SIZE_COLLISION_RUNTIME_VERSION =
  HARTHMERE_UPLOADED_ASSET_DIMENSIONS_VERSION;
const HARTHMERE_SOLID_UPLOADED_ASSET_PLAYER_COLLISION =
  "harthmere-solid-uploaded-asset-player-collision";
const HARTHMERE_TOWN_SPACING_COLLISION_FIX_VERSION =
  "harthmere-town-spacing-collision-solid-fixture";
const HARTHMERE_INTERIOR_ENTERABILITY_FIX_VERSION =
  "harthmere-interior-enterability-blocker-fixes";
const HARTHMERE_SERVICE_BUILDING_BLOCK_REBUILD_VERSION =
  "harthmere-service-building-block-rebuild";
const HARTHMERE_RESIDENT_HOUSING_RENDERER_VERSION =
  HARTHMERE_RESIDENT_HOUSING_STONE_SHELL_VERSION;
...
const HARTHMERE_BODY_WEAPON_VISUAL_COHESION_VERSION =
...
const HARTHMERE_COMBAT_POLISH_THEME_SEQUENCE =
  HARTHMERE_ATTACK_VISUAL_THEMES.map(
    (theme) => theme.id
...
type CombatLifeInstance = {
  ...
  combatPulse?: {
    ...
    evadeFamily?: HarthmereLiveCreatureEvadeVisual["family"];
  };
  harthmerePolishHasDrawn?: boolean;
  soundProfile?: HarthmereCreatureSoundProfile | null;
  bossVisual?: HarthmereBossVisualAsset;
  snapshotStatic: {
    radius: number;
    species: string;
    behavior: string;
...
function harthmereBossVisualForRuntimeActor(actor: CombatLifeInstance) {
  return (
    harthmereBossVisualForLabel(actor.label) ??
    HARTHMERE_BOSS_VISUAL_ASSETS.find(
      (visual) => actor.asset === `boss_${visual.id}`
    )
  );
}
...
function harthmereRuntimeCombatFamily(actor: CombatLifeInstance) {
  ...
  if (
    /animal_|wolf|bear|boar|deer|stag|doe|buck|fox|dog|hound|cat|rat|pig|cow|sheep|goat|horse|chicken|pigeon|crow|rabbit|bunny|snake|fish|turtle/.test(
      text
    )
  ) {
    return "animal";
...
const HARTHMERE_ALL_WEAPON_EQUIPMENT_IDS = [
  ...
  ...HARTHMERE_PREMIUM_WEAPONS.map((weapon) => weapon.id),
] as const;

type HarthmereWeaponVisualProfileName =
  | "melee"
  | "ranged"
  | "projectile"
  | "quiver"
...
> = {
  ...
  projectile: {
    draw: "Nock_24",
    sheathe: "ImpactTwitch_24",
    basic: "Nock_24",
    heavy: "ProjectileSpin_24",
    idle: "ProjectileSpin_24",
  },
  quiver: {
    draw: "EquipBack_24",
    sheathe: "IdleBack_24",
  ...
  magicBook: {
    draw: "OpenRead_24",
    sheathe: "Close_24",
    basic: "CastFromBook_24",
    heavy: "CastFromBook_24",
    idle: "OpenRead_24",
  },
  thrown: {
    draw: "Ready_24",
...
function harthmereWeaponVisualProfileForEquipmentId(
  equipmentId: string
): HarthmereWeaponVisualProfileName {
  ...
  if (
    (HARTHMERE_RANGED_WEAPON_EQUIPMENT_IDS as readonly string[]).includes(
      equipmentId
    )
  ) {
    return "ranged";
  ...
  if (
    (HARTHMERE_MAGIC_WEAPON_EQUIPMENT_IDS as readonly string[]).includes(
      equipmentId
    )
  ) {
    return "magic";
  ...
  if (
    (HARTHMERE_SHIELD_WEAPON_EQUIPMENT_IDS as readonly string[]).includes(
      equipmentId
    )
  ) {
    return "shield";
...
function resolveHarthmerePlayerWeaponEquipmentEntry(
  itemId: string | undefined
) {
  const premium = getHarthmerePremiumWeapon(itemId);
  if (premium) {
    return {
      entry: {
        id: premium.id,
        assetUrl: premium.assetUrl,
        animations: [premium.idleClip],
        targetLength: premium.targetLength,
      },
      equipmentId: premium.id,
      profile: premium.profile,
      clips: HARTHMERE_WEAPON_VISUAL_CLIP_PROFILES[premium.profile],
    };
  }
  const requested =
    itemId && HARTHMERE_PLAYER_WEAPON_ITEM_TO_EQUIPMENT_ID[itemId]
  ...
  for (const equipmentId of fallbacks) {
    ...
    if (
      entry &&
      (HARTHMERE_ALL_WEAPON_EQUIPMENT_IDS as readonly string[]).includes(
        entry.id
      )
    ) {
      ...
      return {
        ...
        profile,
        clips: HARTHMERE_WEAPON_VISUAL_CLIP_PROFILES[profile],
      };
    }
  }
  return undefined;
...
function resolveHarthmerePlayerWeaponVisualClips(itemId: string | undefined) {
  return (
    resolveHarthmerePlayerWeaponEquipmentEntry(itemId)?.clips ??
    HARTHMERE_PLAYER_SWORD_CLIPS
  );
}

const HARTHMERE_UNIQUE_NPC_COSMETICS_VERSION =
  "harthmere-unique-npc-cosmetics-body-fit-clothing";
const HARTHMERE_FACE_BODY_VISUAL_REFINEMENT_VERSION =
...
function harthmereUniqueNpcSeed(placement: RuntimePlacement) {
  return harthmereStableCombatHash(
    `${placement.asset}|${placement.name ?? ""}|${placement.district ?? ""}|${
      placement.combatOffset ?? ""
    }`
  );
}
...
function tintColor(color: number, amount: number) {
  ...
  return c
    .lerp(
      new THREE.Color(amount >= 0 ? 0xffffff : 0x000000),
...
function uniqueTownspersonPalette(
  placement: RuntimePlacement
): HarthmereUniqueNpcPalette {
  const cosmeticId =
    placement.combatOffset ?? harthmereUniqueNpcSeed(placement);
  ...
  const trim = pickSeeded(
    [tintColor(tunic, 0.45), tintColor(tunic, -0.18), accent],
    seed,
    2
  );
  ...
  const detail = pickSeeded(
    [0x1f1f22, 0xf0e6c8, 0xcaa169, 0x8b2f2d, 0x355b8f],
    seed,
    5
  );
  return {
...
function addUniqueNpcGear(
  ...
  const leftHandAnchor = findHumanoidAnchor(root, [
    /lefthand/i,
    /left_hand/i,
    /hand_l/i,
  ]);
  const rightHandAnchor = findHumanoidAnchor(root, [
    /righthand/i,
    /right_hand/i,
    /hand_r/i,
  ]);
  ...
  if (face.accessory === "cap") {
    headGroup.add(
      cosmeticBox("npc-cap", [0.34, 0.08, 0.3], [0, 0.16, 0], palette.accent)
    );
  ...
  chestGroup.add(
    cosmeticBox(
      ...
      pickSeeded(
        [palette.trim, palette.detail, tintColor(palette.tunic, -0.34)],
        seed,
        11
      )
    ),
  ...
  if (/guard/.test(role)) {
    chestGroup.add(
      ...
      cosmeticBox(
        "npc-guard-pauldron-left",
        [0.12, 0.08, 0.12],
        [-0.2, 0.16, 0],
        palette.metal
      ),
      cosmeticBox(
        "npc-guard-pauldron-right",
        [0.12, 0.08, 0.12],
        [0.2, 0.16, 0],
        palette.metal
      )
    ...
    headGroup.add(
      cosmeticBox("npc-farmer-hat", [0.42, 0.05, 0.36], [0, 0.18, 0], 0xb99655)
    );
    chestGroup.add(
      cosmeticBox(
        "npc-farmer-kerchief",
    ...
    chestGroup.add(
      cosmeticBox(
        "npc-clergy-stole-left",
        [0.06, 0.44, 0.04],
        [-0.06, -0.02, -0.12],
        palette.accent
      ),
      cosmeticBox(
        "npc-clergy-stole-right",
        [0.06, 0.44, 0.04],
        [0.06, -0.02, -0.12],
        palette.accent
      ),
    ...
    chestGroup.add(
      cosmeticBox(
        "npc-undead-tatter-left",
        [0.08, 0.22, 0.04],
        [-0.12, -0.14, -0.12],
        tintColor(palette.tunic, -0.1)
      ),
      cosmeticBox(
        "npc-undead-tatter-right",
        [0.08, 0.22, 0.04],
        [0.12, -0.16, -0.1],
        tintColor(palette.tunic, -0.14)
      )
    );
  } else {
    if ((seed & 1) === 0) {
...
function applyUniqueNpcVisualDecorations(
  ...
  setMeshColorByName(
    root,
    ["skin", "face", "head", "hand", "arm"],
    palette.skin
  );
  setMeshColorByName(
    root,
    ["shirt", "body", "torso", "tunic", "robe", "coat"],
    palette.tunic
  );
...
type HarthmereRendererDebugWindow = typeof window & {
  ...
  __harthmereNpcCollisionSummary?: Record<
    string,
    {
      actor: string;
      asset: string;
...
function harthmereVerboseRendererDebugEnabled() {
  return (
    typeof window !== "undefined" &&
    window.localStorage?.getItem(
      "biomes.localDev.harthmere.rendererVerbose"
    ) === "1"
  );
}

function snapshotHarthmereRegisterActorSummary() {
  return harthmereRendererDebugWindow()
    ?.__harthmereRendererRegisterActorSummary;
}
...
function recordHarthmereRegisterActorDebug(payload: Record<string, unknown>) {
  ...
  const role = String(
    payload.appearanceRole ?? payload.socialRole ?? "unknown_role"
  );
  const district = String(payload.district ?? "unknown_district");
  const key = `${asset}|${role}|${district}`;

  const summary = win.__harthmereRendererRegisterActorSummary ?? {
    at: now,
    total: 0,
    byAsset: {},
    byRole: {},
  ...
  const bucket = summary.byAsset[key] ?? {
    count: 0,
    firstAt: now,
    lastAt: now,
    asset: payload.asset,
...
function debugHarthmereRenderer(
  stage: string,
...
function harthmereAutoCombatOffset(
  ...
  const basis = `${asset}|${name ?? ""}|${district ?? ""}|${Math.round(
    x * 10
  )}|${Math.round(z * 10)}`;
...
function harthmereCombatActorBehavior(
  asset: string,
  label?: string,
  district?: string
) {
  ...
  if (
    /bandit|outlaw|thief|ambusher|trapper|smuggler|undead|zombie|corpse|drowned|gravewood|wolf|bear|boar|snake|rat/.test(
      text
    )
  ) {
    return "hostile";
...
function harthmereCombatActorSocialRole(
  asset: string,
  label?: string,
  district?: string
) {
  ...
  if (
    /bandit|outlaw|thief|ambusher|trapper|smuggler|undead|zombie|corpse|drowned/.test(
      text
    )
  ) {
    return "hostile";
...
const ASSETS: RuntimeAsset[] = [
  ...
  ...HARTHMERE_BOSS_VISUAL_ASSETS.map((visual) => ({
    key: `boss_${visual.id}`,
    format: "gltf" as const,
    path: `${ROOT}/glb/bosses/${visual.id}_world.glb`,
    defaultScale: 1,
  })),
  ...
  gltf(
    ...
    "glb/props/market/fountain-round-detail.glb",
    1.0
  ),
  gltf(
    "fountain_square_detail",
    "glb/props/market/fountain-square-detail.glb",
    1.0
  ),
  ...
  gltf(
    "forest_tree_bare_1a",
    "gltf/kaykit/forest_nature/Tree_Bare_1_A_Color1.gltf",
    1.0
  ),
  gltf(
    "forest_tree_bare_2b",
    "gltf/kaykit/forest_nature/Tree_Bare_2_B_Color1.gltf",
    1.0
  ),
  ...
  gltf(
    "forest_grass_1a",
    "gltf/kaykit/forest_nature/Grass_1_A_Color1.gltf",
    1.0
  ),
  gltf(
    "forest_grass_1c",
    "gltf/kaykit/forest_nature/Grass_1_C_Color1.gltf",
    1.0
  ),
  gltf(
    "forest_grass_2b",
    "gltf/kaykit/forest_nature/Grass_2_B_Color1.gltf",
    1.0
  ),
  gltf(
    "forest_grass_2d",
    "gltf/kaykit/forest_nature/Grass_2_D_Color1.gltf",
    1.0
  ),
  ...
  gltf(
    "mine_stone_01",
    "glb/environment/mines/kyrises_voxel_mines/stone-01.glb",
    0.9
  ),
  gltf(
    "mine_stone_02",
    "glb/environment/mines/kyrises_voxel_mines/stone-02.glb",
    0.9
  ),
  gltf(
    "mine_coal_block",
    "glb/environment/mines/kyrises_voxel_mines/coal-block.glb",
    0.9
  ),
  gltf(
    "mine_coal_piece",
    "glb/environment/mines/kyrises_voxel_mines/coal-piece.glb",
    0.9
  ),
  gltf(
    "mine_silver_stone",
    "glb/environment/mines/kyrises_voxel_mines/silver-stone.glb",
    0.9
  ),
  gltf(
    "mine_gold_fragment",
    "glb/environment/mines/kyrises_voxel_mines/gold-fragment.glb",
    0.9
  ),
  gltf(
    "mine_pickaxe",
    "glb/environment/mines/kyrises_voxel_mines/pickaxe.glb",
    0.8
  ),
  gltf(
    "minecart",
    "glb/environment/mines/kyrises_voxel_mines/minecart.glb",
    0.8
  ),
  ...
  fbx(
    "food_green_apple",
    "fbx/props/food/quaternius_ultimate_food/Apple_Green.fbx",
    0.006
  ),
  fbx(
    "food_carrot",
    "fbx/props/food/quaternius_ultimate_food/Carrot.fbx",
    0.0035
  ),
  fbx("food_fish", "fbx/props/food/quaternius_ultimate_food/Fish.fbx", 0.004),
  fbx(
    "food_fishbone",
    "fbx/props/food/quaternius_ultimate_food/FishBone.fbx",
    0.004
  ),
  fbx(
    "food_mushroom",
    "fbx/props/food/quaternius_ultimate_food/Mushroom.fbx",
    0.018
  ),
  gltf(
    "mine_gold_block",
    "glb/environment/mines/kyrises_voxel_mines/gold-block.glb",
    0.8
  ),
  gltf(
    "mine_silver_block",
    "glb/environment/mines/kyrises_voxel_mines/silver-block.glb",
    0.8
  ),
  gltf(
    "mine_diamond_fragment",
    "glb/environment/mines/kyrises_voxel_mines/diamond-fragment.glb",
    0.72
  ),
  ...
  gltf(
    "townsperson_courier",
    "gltf/characters/player_body_variants/harthmere_player_slim_river.gltf",
    0.88
  ),
  gltf(
    "townsperson_dockhand",
    "gltf/characters/player_body_variants/harthmere_player_stocky_river.gltf",
    0.9
  ),
  gltf(
    "townsperson_mudden",
    "gltf/characters/player_body_variants/harthmere_player_average_earth.gltf",
    0.9
  ),
  gltf(
    "townsperson_farmer",
    "gltf/characters/player_body_variants/harthmere_player_average_forest.gltf",
    0.9
  ),
  gltf(
    "townsperson_clergy",
    "gltf/characters/player_body_variants/harthmere_player_soft_royal.gltf",
    0.88
  ),
  gltf(
    "townsperson_hunter",
    "gltf/characters/player_body_variants/harthmere_player_athletic_forest.gltf",
    0.9
  ),
  gltf(
    "townsperson_bandit",
    "gltf/characters/player_body_variants/harthmere_player_athletic_ash.gltf",
    0.9
  ),
  gltf(
    "townsperson_smuggler",
    "gltf/characters/player_body_variants/harthmere_player_slim_ash.gltf",
    0.88
  ),
  gltf(
    "townsperson_undead",
    "gltf/characters/player_body_variants/harthmere_player_soft_ash.gltf",
    0.88
  ),
  gltf(
    "townsperson_charcoal",
    "gltf/characters/player_body_variants/harthmere_player_stocky_earth.gltf",
    0.9
  ),
  gltf(
    "townsperson_market",
    "gltf/characters/player_body_variants/harthmere_player_average_ember.gltf",
    0.9
  ),
  ...
  gltf(
    ...
    "glb/buildings/fantasy_town/wall-window-glass.glb",
    1.0
  ),
  gltf(
    "arch_wall_window_round",
    "glb/buildings/fantasy_town/wall-window-round.glb",
    1.0
  ),
  gltf(
    "arch_wall_doorway_round",
    "glb/buildings/fantasy_town/wall-doorway-round.glb",
    1.0
  ),
  ...
  gltf(
    "arch_wall_wood_door",
    "glb/buildings/fantasy_town/wall-wood-door.glb",
    1.0
  ),
  gltf(
    "arch_wall_wood_broken",
    "glb/buildings/fantasy_town/wall-wood-broken.glb",
    1.0
  ),
  gltf(
    "arch_wall_wood_corner",
    "glb/buildings/fantasy_town/wall-wood-corner.glb",
    1.0
  ),
  gltf("arch_roof_gable", "glb/buildings/fantasy_town/roof-gable.glb", 1.0),
  gltf(
    "arch_roof_high_gable",
    "glb/buildings/fantasy_town/roof-high-gable.glb",
    1.0
  ),
  gltf(
    "arch_roof_high_point",
    "glb/buildings/fantasy_town/roof-high-point.glb",
    1.0
  ),
  gltf(
    "arch_roof_high_window",
    "glb/buildings/fantasy_town/roof-high-window.glb",
    1.0
  ),
  ...
  gltf(
    ...
    "glb/buildings/fantasy_town/stairs-wide-stone.glb",
    1.0
  ),
  ...
  gltf(
    "arch_balcony_fence",
    "glb/buildings/fantasy_town/balcony-wall-fence.glb",
    1.0
  ),
  ...
  obj(
    ...
    "church_cemetery",
    "church-81-ch_windowlower",
    1.0
  ),
  obj(
    "obj_church_window_upper",
    "church_cemetery",
    "church-75-ch_windowupper",
    1.0
  ),
  obj(
    "obj_church_base_lower",
    "church_cemetery",
    "church-83-ch_baselower",
    1.0
  ),
  obj(
    "obj_church_base_upper",
    "church_cemetery",
    "church-82-ch_baseupper",
    1.0
  ),
  obj(
    "obj_church_trapdoor_metal",
    "church_cemetery",
    "church-62-ch_trapdoormetal",
    1.0
  ),
  ...
  gltf(
    "sword_2h_color",
    "glb/equipment/weapons/sword_2handed_color.gltf",
    0.72
  ),
...
function shouldRenderHarthmereCombatDiagnostics() {
  ...
  try {
    return (
      window.localStorage?.getItem(
        "biomes.localDev.harthmere.combatDiagnostics"
      ) === "1" ||
      window.localStorage?.getItem("biomes.localDev.harthmere.combatDebug") ===
        "1"
    );
...
function isSnapshotRawFloatingNpcRuntimePlacement(placement: RuntimePlacement) {
  ...
  return (
    ...
    /jackie|ranger[_ ]?jane|luis|taye|alexis|dimmi|oldcoop|old coop|buddy|mucked[_ ]?robot|rosalyn|guild[_ ]?clerk[_ ]?nia|nia|nina/.test(
      label
    )
  );
}
...
function isHarthmereRuntimeObjMapStructure(placement: RuntimePlacement) {
  ...
  return /house|shop|kiosk|tower|wall|bridge|church|chapel|crypt|grave|building|roof|stair|gate|fence/.test(
    label
  );
}
...
function isHarthmereSnapshotBuiltRuntimeOwnedPlacement(
  ...
  return /tree|forest_|bush|grass|road|fountain|hedge|fence|wall|roof|stairs|bridge|tower|house|cottage|dungeon|underways|crypt|watermill|windmill|grave fence|landmark block|mine_stone|mine_coal|mine_silver|mine_gold|mine_diamond/i.test(
    label
  );
}
...
function snapHarthmereRuntimePlacementToGround(
  placement: RuntimePlacement
): RuntimePlacement {
...
const HARTHMERE_ROUTE_POSITION_SAFE_VERSION =
  "harthmere-route-position-safe-negative-progress";
const HARTHMERE_NPC_ROUTE_DISTRIBUTION_VERSION =
  "harthmere-npc-route-dispersal-density";
const HARTHMERE_NPC_LOCAL_DENSITY_MAX_WITHIN_12M = 7;
const HARTHMERE_NPC_LOCAL_DENSITY_MAX_WITHIN_20M = 16;

const HARTHMERE_NPC_ROUTE_NAVIGATION_VERSION =
  "harthmere-npc-road-safe-route-anchors";
...
function harthmereTownRouteKeyForActor(
  asset: string,
  name?: string,
  district?: string
  ...
  if (
    /wilds|forest|briarfen|gravewood|bandit ridge|greenmere|watchtower ridge|charcoal|orchard lane|gate fields|mill road|old hunter/.test(
      label
    )
  )
    return undefined;
  ...
  if (/craftsman|apothecary|magic shop/.test(rawDistrict))
  ...
  if (
    /residential|cottage|house|farmer|farmhand|chicken|pig|cow|sheep/.test(
      label
    )
  )
    return "residential";
  ...
  if (
    /player services|bank|auction|storage|courier anwen|mail|guild|wardrobe/.test(
      label
    )
  )
    return "player_services";
  if (
    /craftsman|black anvil|smith|forge|carpentry|tailor|leather|ore delivery/.test(
      label
    )
  )
    return "craftsman_row";
  if (/copper kettle|inn|tavern|gambler|bard|patron/.test(label))
    return "copper_kettle";
  if (
    /temple|chapel|clergy|father|sister|pilgrim|apothecary|ysabet|healer/.test(
      label
    )
  )
    return "temple_green";
  if (/noble|reeve|edrik|tax|legal|clerk/.test(label)) return "noble_rise";
  if (/river docks|dock|ferry|fish|smuggl|warehouse|tovin/.test(label))
    return "river_docks";
  if (
    /guard|sergeant|bounty|duel|sparring|drill|prisoner|quartermaster/.test(
      label
    )
  )
    return "guard_yard";
  if (
    /market|mara|vendor|produce|crier|performer|customer|pigeon|livestock|pickpocket/.test(
      label
    )
  )
...
function makeHarthmereTownActorRouteWander(
  ...
  const hash = harthmereNpcStableHash(
    `${asset}|${name ?? ""}|${district ?? ""}|${x.toFixed(1)}|${z.toFixed(1)}`
  );
...
function isHarthmereValidRoutePoint(
...
function firstHarthmereValidRoutePoint(
  route: readonly [number, number][]
): [number, number] {
...
function harthmereRoutePosition(
  route: readonly [number, number][],
  progress: number
): [number, number] {
  ...
  const segment =
    ((floorProgress % route.length) + route.length) % route.length;
...
function normalizeHarthmereActorWander(
  ...
  const routedTownWander = makeHarthmereTownActorRouteWander(
    asset,
    name,
    district,
    x,
...
function registerHarthmereProceduralSolidObstacle(
  ...
  harthmereDynamicProceduralCollisionObstacles = [
    ...harthmereDynamicProceduralCollisionObstacles.filter(
      (existing) => existing.name !== obstacle.name
    ),
    obstacle,
  ];
...
function isHarthmereBuildingNavigationOpeningPlacement(
  asset: string,
  name: string
): boolean {
  return (
    ...
    (/^obj_wall_entrance_door/i.test(asset) &&
      /ironbound door|gate passage|road exit|town exit/.test(name))
  );
}
...
function isHarthmereSolidBannerOrFlagFixture(
  asset: string,
  name: string
): boolean {
  return (
...
function isHarthmereBuildingBodyAsset(asset: string, name: string): boolean {
  ...
  return (
    /^obj_(church|chapel|temple|cathedral|town_hall|shop|smithy|inn|tavern|cottage|hut|barracks|tower_body|gate_house)_/i.test(
      asset
    ) ||
    /^obj_house_\d+$/i.test(asset) ||
    /^obj_shop_(simple|closed)$/i.test(asset) ||
    /^obj_tower_(complex|simple|door)$/i.test(asset) ||
    /^obj_wall_(simple|simple_windows|entrance|entrance_door|stairs)$/i.test(
      asset
    ) ||
    asset === "obj_kiosk" ||
    /church|chapel|temple|cathedral|building body|shop body|smithy body|inn body|tavern body|whole voxel building mesh/i.test(
      name
    )
  );
}
...
function harthmerePlayerCollisionObstacleShapeForPlacement(
  ...
  const isVisualOnly =
    ...
    (name.includes("window") &&
      !isHarthmereExteriorWindowCollisionAsset(asset)) ||
    (name.includes("flag") &&
      !isHarthmereSolidBannerOrFlagFixture(asset, name)) ||
  ...
  if (isAuthoredSolidLandmarkFixture) {
    ...
    jumpable = true;
    collisionProfile = isWalkableGateOrStair
      ? "pass_through_navigation"
      : "visual_only";
    ...
    asset === "fountain_round" ||
    name.includes("fountain") ||
    name.includes("old well")
  ) {
    playerHalfX = Math.max(0.34, halfX * 0.34);
    playerHalfZ = Math.max(0.34, halfZ * 0.34);
    height = name.includes("center stone")
      ? Math.max(0.36, 0.48 * scale)
      : Math.max(0.62, 0.72 * scale);
    jumpable = true;
    ...
    isHarthmereBuildingBodyAsset(asset, name) ||
    asset === "arch_windmill" ||
...
function harthmereNpcStaticObstacleForPlacement(
  ...
  if (asset.startsWith("arch_wall_")) {
    ...
    if (
      /partition|stair support|balcony railing|support layer|support block|row [2-9]|row 1[0-9]|upper room partition/.test(
        label
      )
    ) {
      return undefined;
  ...
  if (
    asset === "obj_church_iso" ||
    isHarthmereBuildingBodyAsset(
      asset,
      String(placement.name ?? asset).toLowerCase()
    )
  ) {
    return makeHarthmereNpcCollisionObstacle(
      placement,
...
function harthmereNpcCollisionObstacles(): HarthmereNpcCollisionObstacle[] {
  if (!harthmereNpcCollisionObstacleCache) {
    const serverVoxelCollisionPlacements =
      filterHarthmereServerVoxelOwnedStructuralPlacements(
        PLACEMENTS
      ).placements;
    harthmereNpcCollisionObstacleCache = serverVoxelCollisionPlacements
      .map(harthmereNpcStaticObstacleForPlacement)
      .filter((obstacle): obstacle is HarthmereNpcCollisionObstacle =>
        Boolean(obstacle)
      );
  }
...
function sweepHarthmereNpcCollisionObstacle(
  ...
  const steps = Math.max(
    1,
    Math.ceil(distance / HARTHMERE_NPC_BODY_SWEEP_STEP_METERS)
  );
...
function harthmereNpcGroundedY(
  ...
  const baseY = Number.isFinite(instance.base[1])
    ? instance.base[1]
    : GROUND_Y + 0.1;
...
function harthmereTownDebugVec(
  values: readonly number[] | THREE.Vector3
): number[] {
  const array: readonly number[] =
    values instanceof THREE.Vector3 ? values.toArray() : values;
...
function harthmereTownDebugIsActorAsset(asset?: string): boolean {
  ...
  return (
    ...
    /guard|bandit|farmer|merchant|dockhand|priest|clergy|courier|banker|zombie|wolf|boar|bear|deer|cat|dog|crow|rat|sheep|chicken/i.test(
      asset
    )
  );
}

function harthmereTownDebugExpectedCollisionReason(
  placement: RuntimePlacement
): string | undefined {
  const asset = placement.asset;
  const name = placement.name ?? "";
  if (
    /^obj_flag_large_|^obj_lamp_ground_|^fountain_(round_detail|center|square_detail|square)$/i.test(
      asset
    )
  ) {
  ...
  if (
    /wall|tower|gate|fence|hedge|building|chapel|barracks|warehouse|hall|smithy|inn|counter|vault|well|cart|stall|bridge/i.test(
      `${asset} ${name}`
    )
  ) {
...
function harthmereTownDebugBoxXZOverlap(
  a: THREE.Box3,
  b: THREE.Box3,
  padding = 0
): boolean {
...
function harthmereStructuralGroupKey(
  placement: RuntimePlacement
): string | undefined {
  ...
  if (
    /road|fence|hedge|wildlife|resource marker|quest-space entry marker|encounter spawn anchor/i.test(
      raw
    )
  ) {
    return undefined;
...
function isHarthmereArchitecturalBlockCandidate(
  placement: RuntimePlacement
): boolean {
  const label = `${placement.asset} ${placement.name ?? ""}`;
  if (
    /road|fence|hedge|banner|flag|lamp|lantern|candle|sign|food_|animal_|townsperson_|table|bench|bed|crate|barrel|bookcase|bag|bucket|rack|dummy|cart|tree|bush|grass|mushroom/i.test(
      label
    )
  ) {
    return false;
  ...
  if (
    /front door overlay|window overlay|chimney|intentional roofline break|service landmark accent/i.test(
      label
    )
  ) {
    return false;
  }
  return /block-built|solid stone\/ore|arch_wall|arch_roof|arch_pillar|obj_wall|mine_stone|floor deck|ceiling and floor slab|buttress|wall block/i.test(
    label
  );
}
...
function harthmerePlacementSupportStats(
  ...
  for (const other of candidates) {
    ...
    const touchesX =
      dx <= HARTHMERE_BLOCK_TILE_METERS * 1.28 &&
      dz <= HARTHMERE_BLOCK_TILE_METERS * 0.55;
    const touchesZ =
      dz <= HARTHMERE_BLOCK_TILE_METERS * 1.28 &&
      dx <= HARTHMERE_BLOCK_TILE_METERS * 0.55;
    ...
    const alignedBelow =
      dx <= HARTHMERE_BLOCK_TILE_METERS * 0.92 &&
      dz <= HARTHMERE_BLOCK_TILE_METERS * 0.92;
...
function shouldCullUnsupportedFloatingBlock(
  ...
  return (
    placement.at[1] > GROUND_Y + 0.82 &&
    stats.horizontalNeighborCount === 0 &&
    !stats.hasBelowSupport
  );
}
...
function harthmereRuntimePerformanceProfile(): "optimized" | "full" {
  if (typeof window === "undefined")
    return HARTHMERE_RUNTIME_PERFORMANCE_PROFILE.defaultProfile;
  const value = window.localStorage.getItem(
    "biomes.localDev.harthmere.performanceProfile"
  );
...
function shouldKeepHarthmerePlacementForPerformance(
  ...
  const distanceFromTownCore = Math.hypot(
    x - HARTHMERE_RUNTIME_CORE_ORIGIN[0],
  ...
  const isFar =
    distanceFromTownCore <=
    HARTHMERE_RUNTIME_PERFORMANCE_PROFILE.farRadiusMeters;
  const isAlwaysImportant =
    placement.lodTier === "always" ||
    /gate|market|chapel|temple|reeve|inn|kettle|smith|anvil|bank|auction|guild|well|underways|bridge|watchtower|player services|quest|board|Mara Thistle|Brother Vance|Edrik Vane/i.test(
      identityLabel
    );
  const isActor =
    placement.meta?.kind === "actor" || isHarthmereLifeAsset(placement.asset);
  const isWilds =
    /wilds|forest|briarfen|gravewood|bandit ridge|greenmere/i.test(label);
  const isTiny =
    placement.lodTier === "tiny" || placement.meta?.kind === "tinyProp";
  const isAnimated = Boolean(
    placement.wander || placement.bob || placement.spin || isActor
  );
  ...
  const isLivingQuarterRepeatedRoomDetail =
    /resident room|clear center stone floor accessibility marker|full-size made bed|personal storage chest|bedside candle|small writing nightstand|personal book stack|clean wall hanging|patched sleeping pallet|shared crate storage|stub candle|stool used as table|family bundle|patched hanging cloth/i.test(
      label
    );
  if (isLivingQuarterRepeatedRoomDetail) {
    ...
    return (
      counters.tiny <=
        Math.floor(
          HARTHMERE_RUNTIME_PERFORMANCE_PROFILE.maxTinyPropsOptimized * 0.55
        ) ||
      /floor 1 room 1|floor 2 room 1|room 1 .*bed|room 1 .*table|room 1 .*storage/i.test(
        label
      )
    );
  }
  ...
  // The former optimized path returned every static core placement before it
  // incremented `counters.kept`, so the advertised placement limit did not cap
  // the densest part of town. Apply one shared total budget to every
  // non-landmark placement, including core shells and repeated decor.
  const keepWithinTotalBudget = () => {
    counters.kept += 1;
    return (
      counters.kept <=
      HARTHMERE_RUNTIME_PERFORMANCE_PROFILE.maxRuntimePlacementsOptimized
    );
  };
  ...
  if (isTiny) {
    ...
    return (
      counters.tiny <=
        HARTHMERE_RUNTIME_PERFORMANCE_PROFILE.maxTinyPropsOptimized &&
  ...
  if (isWilds && isActor) {
    ...
    return (
      counters.wildActors <=
        HARTHMERE_RUNTIME_PERFORMANCE_PROFILE.maxWildsActorsOptimized &&
      keepWithinTotalBudget()
    );
  }
  if (isWilds) {
  ...
  if (isAnimated) {
    ...
    return (
      counters.animated <=
        HARTHMERE_RUNTIME_PERFORMANCE_PROFILE.maxAnimatedLifeOptimized &&
      keepWithinTotalBudget()
    );
  }
  if (isCore) return keepWithinTotalBudget();
  return keepWithinTotalBudget();
}

// HARTHMERE_REMOVE_ARCH_WALL_STONE_RUNTIME_VERSION
const HARTHMERE_REMOVE_ARCH_WALL_STONE_RUNTIME_VERSION =
  "harthmere-remove-arch-wall-stone-runtime";
...
function isHarthmereServerVoxelOwnedStructuralAsset(asset: string, name = "") {
  ...
  const definitelyDecor =
    /bench|bell|lantern|lamp|sign|banner|torch|table|crate|barrel|book|scroll|bed|cabinet|chest|anvil|forge|cauldron|pulpit|trapdoor|tombstone|coffin|weapon|shield|rack|rope|bucket|bottle|mug|candle|coin|key|plaque|contract|ledger|shelf|food|cart|wagon/i.test(
      label
    );
  ...
  return (
    ...
    /house|cottage|hut|tower|watchtower|gatehouse|guard tower|watermill|windmill|bridge body|wall run|grave fence|building body|warehouse body/i.test(
      label
    )
  );
}
    ...
    if (
      ...
      removed.push({
        ...
        name:
          String(placement.name ?? placement.asset) +
          " removed by " +
          HARTHMERE_SERVER_VOXEL_STRUCTURAL_FILTER_VERSION,
      });
      continue;
    }
    kept.push(placement);
...
function shiftHarthmereRuntimePlacementForExtraTown(
  ...
  return snapHarthmereRuntimePlacementToGround({
    ...
    meta: placement.meta
      ? {
          ...placement.meta,
          tags: [...placement.meta.tags, "extra-town-offset"],
        }
      : placement.meta,
  });
}
...
function prepareHarthmereRuntimePlacements(
  ...
  removedFloating: RuntimePlacement[];
  removedForPerformance: RuntimePlacement[];
} {
  ...
  const shiftedPlacements = placements.map(
    shiftHarthmereRuntimePlacementForExtraTown
  );
  const serverVoxelFiltered =
    filterHarthmereServerVoxelOwnedStructuralPlacements(shiftedPlacements);
  const floating = filterHarthmereUnsupportedFloatingBlockPlacements(
    serverVoxelFiltered.placements
  );
  const placementsWithoutRemovedAssets = floating.placements.filter(
    (placement) => !shouldRemoveHarthmereRuntimePlacement(placement)
  );
  const snapshotBuiltFiltered = filterHarthmereSnapshotBuiltRuntimePlacements(
    placementsWithoutRemovedAssets
  );
...
function createHarthmereContinuousBlockWalls(
  ...
  const openings: HarthmereOpening[] =
    opts.openings ?? harthmereDefaultOpenings(shell);
  ...
  for (const { face, zConst, rotAdd } of nsFaces) {
    ...
    for (let c = 0; c < columns; c += 1) {
      ...
      for (let r = 0; r < rows; r += 1) {
        ...
        const asset = harthmereChooseWallAsset(
          c,
          r,
          columns,
          rows,
        ...
        placements.push(
          BP(
            ...
            "block-built current " +
              ...
              " floor " +
              floor +
              " solid stone/ore wall ring corner=" +
              isCornerColumn,
            blockY
          )
        );
...
function harthmereProductionPolishAccent(
  shell: BuildingShell
): HarthmereBuildingPolishAccent {
  ...
  if (/smith|anvil|workshop|craft|forge/.test(label)) {
    return {
      ...
      sign: "obj_sign_post",
      clutter: "barrel_stack",
      trim: "arch_pillar_stone",
    };
  }
  if (
    /market|bakery|loaf|provision|store|auction|bank|guild|service|inn|kettle/.test(
      label
    )
  ) {
    return {
      ...
      sign: "obj_sign_post",
      clutter: "barrel_apples",
      trim: "arch_overhang",
    };
  }
  return {
    banner: shell.theme.banner ?? "banner_yellow",
    sign: "obj_sign_post",
    clutter: "crate_a",
    trim: shell.theme.corner ?? "arch_pillar_stone",
  };
}

function harthmereProductionPolishIsServiceOrLandmark(
  shell: BuildingShell
): boolean {
  const label = `${shell.name} ${shell.district}`.toLowerCase();
  return /gate|gatehouse|toll|bridge|watchtower|tower|chapel|temple|shrine|cottage|market|mara|edrik|noble|estate|reeve|inn|kettle|smith|anvil|workshop|craft|dock|warehouse|bank|auction|guild|provision|bakery|apothecary|well|underways|player services/.test(
    label
  );
}

function harthmereProductionPolishIsMajorBannerBuilding(
  shell: BuildingShell
): boolean {
  const label = `${shell.name} ${shell.district}`.toLowerCase();
  return /gatehouse|chapel|temple|market|inn|kettle|noble|estate|reeve|bank|auction|guild/.test(
    label
  );
}
...
function createHarthmereBuildingExteriorPolish(
  ...
  const isServiceOrLandmark =
    harthmereProductionPolishIsServiceOrLandmark(shell);
  const placements: RuntimePlacement[] = [];
  ...
  return placements.slice(
    0,
    HARTHMERE_PRODUCTION_POLISH_RENDER_BUDGETS.maxExteriorAccentPlacementsPerBuilding
  );
}
...
function createBuildingShell(shell: BuildingShell): RuntimePlacement[] {
  ...
  const storyHeight =
    roofY && roofY > 0 ? Math.max(2.0, roofY) : HARTHMERE_STORY_HEIGHT_DEFAULT;
  const wallFloor = Math.max(
    1,
    Math.round(wallY / Math.max(0.1, storyHeight) + 1)
  );
  const hwOffset = Math.min(3, Math.max(2, Math.floor(shell.w / 2) - 2));
  placements.push(
    ...createHarthmereContinuousBlockWalls(shell, {
      storyHeight,
      floor: wallFloor,
    })
  );
  ...
  if (shell.w >= 22) {
    ...
    placements.push(
      BP(t.roof, shell, 0, 0, 0, roofScale, "roof volume", roofY)
    );
  }
  ...
  if (t.stair) {
    placements.push(
      BP(t.stair, shell, 0, hd + 2.2, 0, scale * 0.75, "entry step", 0)
    );
  }
...
function harthmereServiceStoneTheme(
  ...
  const defaultRoof =
    building.profile === "chapel" ||
    building.profile === "reeve_hall" ||
    building.profile === "inn"
      ? "arch_roof_high_gable"
      : "arch_roof_gable";
  return {
    ...STONE_THEME,
    wall: "arch_wall_stone",
    window:
      building.profile === "chapel" || building.profile === "magic_shop"
        ? "arch_wall_window_round"
        : "arch_wall_window_stone",
...
function createHarthmereServiceFloorDeckBlocks(
  ...
  const placements: RuntimePlacement[] = [];
  const theme = harthmereServiceStoneTheme(
    building,
    floor,
    building.floors ?? 1
  );
...
function createHarthmereServiceBlockStairRun(
  ...
  const stairScale = Math.max(
    0.55,
    Math.min(0.9, (building.scale ?? 0.8) * 0.85)
  );
  return [
    BP(
      "obj_wall_stairs",
      ...
      `${
        building.name
      } BUILDING_VOXEL_MESHES single voxel stair mesh floor ${floor} to ${
        floor + 1
      } walkable player npc accessible doorway clear not floating not separate blocks`,
      baseY
    ),
  ];
}
  ...
  switch (building.profile) {
    case "bakery":
      ...
      item(
        ...
        0.1,
        0.34,
        "bakery bread display on counter",
        0.62
      );
      item(
        "crate_wooden_fp",
        ...
        0,
        0.42,
        "bakery flour crate stack on floor"
      );
      break;
    case "provision":
      item(
    ...
    case "player_services":
      ...
      item(
        ...
        0,
        0.44,
        "vault chest beside services counter"
      );
      item(
        "scroll_1_fp",
        -3.2,
    ...
    case "smithy":
      ...
      item(
        ...
        0,
        0.42,
        "smithy ore coal block pile on floor"
      );
      item(
        "torch_lit",
        3.0,
        ...
        Math.PI / 2,
        0.54,
        "smithy forge glow supported in stone forge mouth",
        0.42
      );
      break;
    case "workshop":
      ...
      item(
        ...
        0,
        0.36,
        "workshop tool station on floor"
      );
    ...
    case "apothecary":
      ...
      item(
        ...
        Math.PI / 2,
        0.34,
        "apothecary treatment cot against wall"
      );
      item(
        "candlestick_stand_fp",
        -1.2,
    ...
    case "magic_shop":
      ...
      item(
        ...
        -Math.PI / 2,
        0.44,
        "magic component case against east stone wall"
      );
      item(
        "candlestick_triple_fp",
        0,
    ...
    case "inn":
      ...
      item(
        ...
        Math.PI / 2,
        0.42,
        "inn upstairs rentable bed against room wall",
        0,
        2
      );
      break;
    case "reeve_hall":
      ...
      item(
        ...
        Math.PI,
        0.42,
        "reeve legal archive bookcase against wall"
      );
      item(
        "scroll_2_fp",
        0,
    ...
    case "dock_warehouse":
      ...
      item(
        ...
        0,
        0.44,
        "dock sealed goods barrel on floor"
      );
      item(
        "bookcase_2",
        3.0,
        ...
        Math.PI,
        0.34,
        "dock ledger shelf against stone wall"
      );
      break;
    case "mudden_home":
      item(
    ...
    case "wash_house":
      ...
      item(
        ...
        0,
        0.38,
        "wash house water barrel on floor"
      );
      item(
        "banner_white",
        0,
        ...
        0,
        0.34,
        "wash house folded cloth on supported counter",
        0.4
      );
      break;
    case "residential_cottage":
      ...
      item(
        ...
        Math.PI,
        0.36,
        "residential cottage shelf against wall"
      );
    ...
    case "barracks":
      ...
      item(
        ...
        Math.PI / 2,
        0.32,
        "barracks bunk two against wall"
      );
      item(
        "rack",
        2.6,
        ...
        Math.PI,
        0.36,
        "barracks weapon rack against stone wall"
      );
      break;
    case "stable_office":
      item(
    ...
    case "chapel":
      item(
        ...
        0,
        0.54,
        "chapel pew row west on stone floor"
      );
      item(
        "church_bench",
        4.8,
        ...
        0,
        0.54,
        "chapel pew row east on stone floor"
      );
      item(
        "church_pulpit",
        0,
      ...
      item(
        ...
        -Math.PI / 2,
        0.42,
        "chapel archive bookcase against rebuilt stone wall"
      );
      item(
        "candlestick_stand_fp",
        -2.4,
        ...
        0,
        0.42,
        "chapel altar candle on stone floor"
      );
      break;
  }
...
function harthmereBusinessOutpostStaffWorkPoint(
...
function harthmereBusinessOutpostFixtureRotation(
  ...
  if (
    /wall|map|board|pegboard|rack|shelf|cabinet|bookcase|lantern/.test(label)
  ) {
...
function makeHarthmereBusinessOutpostVisualDecorPlacement(input: {
  ...
  const name = `${input.record.displayName} visual-only supported ${
    input.spec.support
  } ${fixture.label}${input.spec.nameSuffix ?? ""} real interior furniture`;
  const placement = P(
    input.spec.asset,
    x,
    z,
...
function isNearWideWildsVisualRoad(x: number, z: number, width = 18) {
  ...
  return roads.some(
    ([ax, az, bx, bz]) => wildsDistanceToSegment(x, z, ax, az, bx, bz) <= width
  );
}
...
function harthmereResidentStoryHeight(
  building: HarthmereResidentHousingBuilding
): number {
  return building.style === "slum" ? 2.45 : 2.7;
}

function harthmereResidentWallScale(
  building: HarthmereResidentHousingBuilding
): number {
  return building.style === "slum" ? 0.62 : 0.74;
}

function harthmereResidentFloorScale(
  building: HarthmereResidentHousingBuilding
): number {
...
function createHarthmereResidentWallBlocks(
  ...
  return createBuildingShell(storyShell).map((placement) => ({
    ...placement,
    name: `${
      placement.name ?? placement.asset
    } solid stone/ore house shell no see-through overlap-safe`,
  }));
}
...
function createHarthmereBlockStairRun(
  ...
  const stepCount = Math.ceil(
    storyHeight / HARTHMERE_RESIDENT_BLOCK_STAIR_MAX_RISE
  );
  const rise = storyHeight / stepCount;
  const tread = Math.max(
    HARTHMERE_RESIDENT_BLOCK_STAIR_MIN_TREAD,
    building.style === "slum" ? 0.86 : 0.92
  );
  ...
  for (let step = 0; step <= stepCount; step += 1) {
    placements.push(
      BP(
        ...
        `floor ${floor} to ${floor + 1} interior stone block stair riser ${
          step + 1
        } of ${
          stepCount + 1
        } max rise ${HARTHMERE_RESIDENT_BLOCK_STAIR_MAX_RISE} npc travel tread`,
        baseY + step * rise
      )
    );
  }
...
const HARTHMERE_LIVING_QUARTERS_REBUILD_RENDERER_VERSION =
  "harthmere-living-quarters-voxel-block-rebuild";
const HARTHMERE_LIVING_QUARTERS_STRUCTURAL_BLOCKS = [
  "mine_stone_01",
  "mine_stone_02",
  "arch_wall_stone",
] as const;
...
function harthmereLivingQuarterBlockAsset(index: number): string {
  return HARTHMERE_LIVING_QUARTERS_STRUCTURAL_BLOCKS[
    index % HARTHMERE_LIVING_QUARTERS_STRUCTURAL_BLOCKS.length
  ];
}
  ...
  for (let floor = 1; floor <= building.floors; floor += 1) {
    ...
    for (let c = 0; c < xColumns; c += 1) {
      ...
      for (const rowY of wallRows) {
        const isDoorGap =
          floor === 1 && rowY < 1.4 && Math.abs(dx) < tile * 0.58;
        if (!isDoorGap) {
          pushBlock(
            harthmereLivingQuarterBlockAsset(blockIndex++),
            dx,
            hd,
            baseY + rowY,
            `south solid voxel wall block floor ${floor} column ${c}`
          );
        }
        pushBlock(
          harthmereLivingQuarterBlockAsset(blockIndex++),
          ...
          -hd,
          baseY + rowY,
          `north solid voxel wall block floor ${floor} column ${c}`,
          Math.PI
        );
      }
    }
    for (let c = 1; c < zColumns - 1; c += 1) {
      ...
      for (const rowY of wallRows) {
        pushBlock(
          ...
          dz,
          baseY + rowY,
          `east solid voxel wall block floor ${floor} column ${c}`,
          -Math.PI / 2
        );
        pushBlock(
          harthmereLivingQuarterBlockAsset(blockIndex++),
          ...
          dz,
          baseY + rowY,
          `west solid voxel wall block floor ${floor} column ${c}`,
          Math.PI / 2
        );
      }
    }
    ...
    pushBlock(
      ...
      0,
      baseY - 0.08,
      `walkable stone floor slab left floor ${floor}`,
      0,
      slabScale
    );
    pushBlock(
      ...
      0,
      baseY - 0.08,
      `walkable stone floor slab right floor ${floor}`,
      0,
      slabScale
    );
    pushBlock(
      ...
      `clear roof or ceiling slab left floor ${floor}`,
      0,
      slabScale * 0.98
    );
    pushBlock(
      ...
      0,
      baseY + storyHeight - 0.12,
      `clear roof or ceiling slab right floor ${floor}`,
      0,
      slabScale * 0.98
    );
    ...
    if (floor === 1) {
      pushBlock(
        ...
        hd + 0.08,
        baseY + 0.08,
        `front oak door inserted into voxel wall floor ${floor}`,
        0,
        building.style === "slum" ? 0.58 : 0.66
      );
    }
    if (floor < building.floors) {
      const stepCount = Math.ceil(
      ...
      for (let step = 0; step <= stepCount; step += 1) {
        pushBlock(
          ...
          startZ - step * 0.46,
          baseY + 0.1 + step * rise,
          `accessible interior block stair floor ${floor} step ${
            step + 1
          } max-rise ${HARTHMERE_RESIDENT_BLOCK_STAIR_MAX_RISE}`,
          Math.PI / 2,
      ...
      pushBlock(
        ...
        startZ - (stepCount + 1) * 0.46,
        baseY + storyHeight - 0.08,
        `upper landing slab reachable from stair floor ${floor}`,
        0,
        building.style === "slum" ? 0.54 : 0.6
      );
    }
  }
...
function createHarthmereLivingQuarterDoorOverlays(
  ...
  if (floor === 1) {
    ...
    placements.push(
      BP(
        ...
        0,
        windowScale,
        `current south left window overlay floor 1`,
        baseY + 1.05
      )
    );
    placements.push(
      BP(
        ...
        0,
        windowScale,
        `current south right window overlay floor 1`,
        baseY + 1.05
      )
    );
    placements.push(
      BP(
        ...
        Math.PI,
        windowScale,
        `current north window overlay floor 1`,
        baseY + 1.05
      )
    );
    placements.push(
      BP(
        ...
        -Math.PI / 2,
        windowScale,
        `current east window overlay floor 1`,
        baseY + 1.05
      )
    );
    placements.push(
      BP(
        ...
        Math.PI / 2,
        windowScale,
        `current west window overlay floor 1`,
        baseY + 1.05
      )
    );
    return placements;
  ...
  for (let col = 0; col < columns; col += 1) {
    ...
    placements.push(
      BP(
        ...
        0,
        doorScale,
        `current front door overlay floor ${floor} room column ${
          col + 1
        } balcony entry doorway clear walkable opening`,
        baseY + 0.04
      )
    );
  }
  placements.push(
    BP(
      ...
      Math.PI,
      windowScale,
      `current north left window overlay floor ${floor}`,
      baseY + 1.05
    )
  );
  placements.push(
    BP(
      ...
      Math.PI,
      windowScale,
      `current north right window overlay floor ${floor}`,
      baseY + 1.05
    )
  );
  placements.push(
    BP(
      ...
      -Math.PI / 2,
      windowScale,
      `current east window overlay floor ${floor}`,
      baseY + 1.05
    )
  );
  placements.push(
    BP(
      ...
      Math.PI / 2,
      windowScale,
      `current west window overlay floor ${floor}`,
      baseY + 1.05
    )
  );
  return placements;
...
function createHarthmereLivingQuarterStair(
    ...
    for (let layer = 0; layer < supportLayers; layer += 1) {
      ...
      placements.push(
        BP(
          ...
          0,
          stairScale,
          `current voxel stair support block floor ${floor} step ${step} support layer ${
            layer + 1
          } solid stacked stone no floating`,
          layerY
        )
      );
    }
  ...
  placements.push(
    BP(
      ...
      0,
      stairScale * 1.3,
      `current voxel stair upper landing slab floor ${
        floor + 1
      } entry step walkable doorway clear`,
      baseY + storyHeight - 0.12
    )
  );
  ...
  for (let c = 0; c < railColumns; c += 1) {
    ...
    placements.push(
      BP(
        ...
        0,
        railScale,
        `current balcony railing post floor ${floor} column ${
          c + 1
        } stone railing fence rail`,
        baseY + 0.1
      )
    );
  }
...
function createHarthmereLivingQuarterPartitions(
  ...
  for (let col = 1; col < columns; col += 1) {
    ...
    for (let i = 0; i < partitionBlocks; i += 1) {
      const partitionZ =
        -hd + 1.2 + i * (partitionDepth / Math.max(1, partitionBlocks - 1));
      for (let r = 0; r < stackRows; r += 1) {
        placements.push(
          BP(
            ...
            Math.PI / 2,
            partitionScale,
            `current interior partition wall floor ${floor} column ${col} block ${
              i + 1
            } row ${r + 1} separates room suites solid stone`,
            baseY + r * 1.2
          )
        );
      }
...
const HARTHMERE_LIVING_QUARTERS_PERFORMANCE_COMPLETE_VERSION =
  "harthmere-living-quarters-performance-complete";
const HARTHMERE_SERVICE_MULTI_STORY_COMPLETION_VERSION =
  "harthmere-service-multi-story-completion";
...
function createHarthmereLivingQuarterVoxelShell(
  ...
  const panelScale = b.style === "slum" ? 1.92 : 2.1;
  ...
  const isInsideOpening = (
    ...
    return openings.some(
      (opening) =>
        ...
        Math.abs(offsetAlongWall - opening.offset) <=
          Math.max(1.05, opening.widthBlocks * 0.72)
    );
  };
    ...
    for (let column = 0; column < columns; column += 1) {
      ...
      for (let row = 0; row < rowOffsets.length; row += 1) {
        ...
        push(
          ...
          `${HARTHMERE_LIVING_QUARTERS_PERFORMANCE_COMPLETE_VERSION} current solid performance apartment wall panel face ${face} floor ${floor} column ${
            column + 1
          } row ${
            row + 1
          } solid stone/ore block-built no see-through residential/slum shell`,
        ...
        for (const z of [-b.d * 0.22, b.d * 0.22]) {
          for (let row = 0; row < 1; row += 1) {
            push(
              ...
              Math.PI / 2,
              panelScale * 0.78,
              `${HARTHMERE_LIVING_QUARTERS_PERFORMANCE_COMPLETE_VERSION} current upper room partition panel floor ${floor} partition ${p} row ${
                row + 1
              } solid stone/ore block-built apartment interior`,
    ...
    if (floor < b.floors) {
      ...
      for (let step = 0; step <= stepCount; step += 1) {
        ...
        push(
          ...
          Math.PI / 2,
          0.62,
          `${HARTHMERE_LIVING_QUARTERS_PERFORMANCE_COMPLETE_VERSION} current stacked solid stone stair support floor ${floor} step ${
            step + 1
          } not floating residential/slum upper-story access`,
        ...
        push(
          ...
          0,
          0.48,
          `${HARTHMERE_LIVING_QUARTERS_PERFORMANCE_COMPLETE_VERSION} current walkable stair tread floor ${floor} step ${
            step + 1
          } doorway clear no invisible blocker residential/slum upper-story access`,
          y + 0.05
        );
      }
      push(
        ...
        0,
        0.72,
        `${HARTHMERE_LIVING_QUARTERS_PERFORMANCE_COMPLETE_VERSION} current upper landing slab floor ${
          floor + 1
        } walkable doorway clear no invisible blocker complete two-story residential/slum access`,
    ...
    if (floor > 1) {
      ...
      for (let i = 0; i < deckCount; i += 1) {
        const dx =
          deckCount === 1
            ? 0
            : -b.w * 0.28 + (b.w * 0.56 * i) / Math.max(1, deckCount - 1);
        push(
          shell,
          "arch_roof_flat",
          dx,
...
function createHarthmereResidentStoryFrame(
  ...
  const fitScale = Math.max(
    0.85,
    Math.min(1.55, Math.min(building.w, building.d) / 7.0)
  );
  ...
  placements.push(
    P(
      ...
      `${
        building.name
      } BUILDING_VOXEL_MESHES whole voxel building mesh ground floor accessible doorway clear ${
        building.style === "slum"
          ? "mudden ward slum tenement"
  ...
  if (building.floors >= 2) {
    placements.push(
      P(
        ...
        `${
          building.name
        } BUILDING_VOXEL_MESHES whole voxel building mesh upper floor on top of ground floor entry not blocked ${
          building.style === "slum"
            ? "mudden ward slum upper tenement"
...
function createHarthmereResidentRoomDecorPlacements(
  ...
  for (let floor = 1; floor <= building.floors; floor += 1) {
    ...
    for (
      let roomIndex = 0;
      roomIndex < building.roomsPerFloor;
      roomIndex += 1
    ) {
      ...
      for (const item of decor) {
        ...
        placements.push(
          P(
            ...
            item.scale,
            `${building.name} floor ${floor} room ${
              roomIndex + 1
            } resident room ${item.role} ${
              item.label
            } placed on block floor deck`,
            building.district,
            floorY + item.y
          )
        );
      }
      const [markerX, markerZ] = localPoint(
        building.x,
        building.z,
        building.rot,
        center.dx,
      ...
      placements.push(
        P(
          ...
          `${building.name} floor ${floor} room ${
            roomIndex + 1
          } clear center stone floor accessibility marker`,
          building.district,
          floorY - 0.16
        )
      );
...
function createHarthmereWildlifeHerdPlacements(): RuntimePlacement[] {
  ...
  for (const herd of herds) {
    for (let index = 0; index < herd.count; index += 1) {
      const u = wildsDeterministic01(
        herd.cx + index * 41,
        herd.cz - index * 17,
        201
      );
      const v = wildsDeterministic01(
        herd.cx - index * 13,
        herd.cz + index * 37,
        202
      );
      ...
      if (
        isInsideWideWildsTownBuffer(x, z) ||
        isNearWideWildsVisualRoad(x, z, 12)
      ) {
        continue;
      }
      const animal =
        herd.animals[
          Math.floor(wildsDeterministic01(x, z, 203) * herd.animals.length)
        ] ?? herd.animals[0];
      ...
      placements.push(
        A(
          ...
          rot,
          scale,
          `${herd.district} roaming ${animal
            .replace("animal_", "")
            .replace("townsperson_", "")}`,
          herd.district,
          {
...
function createHarthmereDeepResourceMarkerPlacements(): RuntimePlacement[] {
  ...
  for (const [asset, x, z, name] of markers) {
    placements.push(
      P(asset, x, z, 0, 0.48, name, "Harthmere Wilds - Real Resource Markers")
    );
  }
...
function createHarthmereWideWildsPlacements(): RuntimePlacement[] {
  ...
  placements.push(
    ...
    ...row(
      "road",
      "Harthmere Wilds - South Road",
      "South road milestone",
      486,
      -100,
      82,
      0,
    ...
    ...row(
      "road",
      "Harthmere Wilds - East Road",
      "East river road milestone",
      604,
      -205,
      82,
      8,
    ...
    ...row(
      "road",
      "Harthmere Wilds - Northwest Hunter Track",
      "Hunter track stone",
      420,
      -304,
      60,
    ...
    ...row(
      "road",
      "Harthmere Wilds - Northeast Reed Track",
      "Reed track stone",
      604,
      -260,
      60,
    ...
    ...row(
      "road",
      "Harthmere Wilds - Southwest Orchard Track",
      "Orchard track stone",
      420,
      -100,
      60,
    ...
    ...row(
      "road",
      "Harthmere Wilds - Southeast Grave Track",
      "Grave track stone",
      568,
      -100,
      60,
    ...
    ...row(
      "road",
      "Harthmere Snapshot Edge Road",
      "HARTHMERE_CONNECTED_MAP_ROAD Snapshot edge road packed dirt and gravel",
      192,
      -209,
      26,
      8,
    ...
    ...row(
      ...
      "Harthmere Snapshot Edge Road",
      "HARTHMERE_CONNECTED_MAP_ROAD safe road lantern",
      200,
      -203.8,
      7,
      32,
    ...
    ...row(
      "banner_red",
      "Harthmere Snapshot Edge Road",
      "HARTHMERE_CONNECTED_MAP_ROAD red black watch banner",
      208,
      -214.2,
      6,
      36,
  ...
  placements.push(
    P(
      ...
      Math.PI / 2,
      0.42,
      "HARTHMERE_CONNECTED_MAP_ROAD Map boundary sign: Harthmere west road",
      "Harthmere Snapshot Edge Road"
    ),
    P(
      "obj_sign_post",
      ...
      Math.PI / 2,
      0.38,
      "HARTHMERE_CONNECTED_MAP_ROAD Road bends: town smoke and bells ahead",
      "Harthmere Snapshot Edge Road"
    ),
    P(
      ...
      0,
      0.44,
      "HARTHMERE_CONNECTED_MAP_ROAD West gate approach lamp visible from road",
      "Harthmere Snapshot Edge Road"
    ),
    P(
      "candle_lit",
      ...
      0,
      0.28,
      "HARTHMERE_CONNECTED_MAP_ROAD traveler return-safely candle shrine",
      "Harthmere Snapshot Edge Road",
      GROUND_Y + 0.18
    ),
    P(
      ...
      0,
      0.34,
      "HARTHMERE_CONNECTED_MAP_ROAD small roadside prayer stone",
      "Harthmere Snapshot Edge Road"
    ),
    P(
      "forest_bush_1a",
      ...
      -0.2,
      0.54,
      "HARTHMERE_CONNECTED_MAP_ROAD hedgerow transition from safe road to Wilds",
      "Harthmere Snapshot Edge Road"
    ),
    P(
      "forest_grass_1c",
      ...
      0.1,
      0.42,
      "HARTHMERE_CONNECTED_MAP_ROAD wagon-rut grass shoulder",
      "Harthmere Snapshot Edge Road"
    ),
    A(
      "townsperson_guard",
      ...
      Math.PI / 2,
      0.96,
      "HARTHMERE_CONNECTED_MAP_ROAD Harthmere road patrol at map boundary",
      "Harthmere Snapshot Edge Road",
      { radius: 3.2, speed: 0.12, phase: 1.7 }
    ),
    A(
      "townsperson_bandit",
      254,
      ...
      -Math.PI / 2,
      0.98,
      "HARTHMERE_CONNECTED_MAP_ROAD bandit scout watching from hedgerow, off the safe lane",
      "Harthmere Snapshot Edge Road",
      { radius: 2.6, speed: 0.08, phase: 2.4 }
    ),
    P(
      "coin_pile",
      360,
      ...
      0,
      0.16,
      "HARTHMERE_CONNECTED_MAP_ROAD bellbound bronze road nail set into west approach",
      "Harthmere Snapshot Edge Road",
      GROUND_Y + 0.06
    )
  );
  ...
  for (const zone of zones) {
    for (let index = 0; index < zone.count; index += 1) {
      ...
      if (
        isInsideWideWildsTownBuffer(x, z) ||
        isNearWideWildsVisualRoad(x, z, 19)
      ) {
        continue;
      }

      const selector = Math.floor(
        wildsDeterministic01(x, z, 3) * zone.treeAssets.length
      );
      const asset = zone.treeAssets[selector] ?? zone.treeAssets[0];
      const rot = wildsDeterministic01(x, z, 4) * Math.PI * 2;
      const scale =
        zone.treeScale * (0.78 + wildsDeterministic01(x, z, 5) * 0.62);
      placements.push(
        P(
          asset,
          x,
          ...
          rot,
          scale,
          `${zone.name} tree ${index + 1}`,
          zone.district
        )
      );

      if (index % 5 === 0) {
        const groundSelector = Math.floor(
          wildsDeterministic01(x, z, 6) * zone.groundAssets.length
        );
        const groundAsset =
          zone.groundAssets[groundSelector] ?? zone.groundAssets[0];
        placements.push(
          P(
            groundAsset,
  ...
  placements.push(
    P(
      ...
      0.2,
      0.92,
      "Far ruined watchtower silhouette",
      "Harthmere Wilds - Northwest Watchtower Ridge"
    ),
    P(
      "obj_wall_simple_windows",
      175,
    ...
    P(
      ...
      Math.PI / 2,
      0.78,
      "Briarfen raised plank path",
      "Harthmere Wilds - East Briarfen Woods"
    ),
    P(
      "arch_planks",
      890,
    ...
    P(
      ...
      0.2,
      0.66,
      "Outer gravewood fence",
      "Harthmere Wilds - Southeast Gravewood"
    ),
    P(
      "tombstone",
      770,
    ...
    P(
      ...
      0,
      0.5,
      "Buried bell shard far from town",
      "Harthmere Wilds - Southeast Gravewood"
    ),
    ...
    P(
      ...
      0.2,
      0.86,
      "Distant orchard windmill",
      "Harthmere Wilds - Southwest Orchardwood"
    ),
    P(
      "cart",
      140,
    ...
    P(
      ...
      "Harthmere Wilds - North Greenmere"
    ),
    P(
      "pillar",
      538,
      ...
      -0.2,
      0.62,
      "Broken northern standing stone",
      "Harthmere Wilds - North Greenmere"
    ),
    P(
      "candle_lit",
      ...
      0,
      0.42,
      "Old Wood ritual light supported on flat ritual stone",
      "Harthmere Wilds - North Greenmere",
      GROUND_Y + 0.28
    ),
    ...
    P(
      ...
      0,
      0.78,
      "South forest mossy boulder",
      "Harthmere Wilds - South Greenmere"
    ),
    P(
      "logs",
      492,
  ...
  placements.push(
    ...
    A(
      ...
      "South boar sounder",
      "Harthmere Wilds - South Greenmere",
      { radius: 4.5, speed: 0.34, phase: 0.8 }
    ),
    A(
      "animal_fox",
      530,
      ...
      "South fox",
      "Harthmere Wilds - South Greenmere",
      { radius: 3.2, speed: 0.46, phase: 2.2 }
    ),
    A(
      "animal_deer",
      570,
      ...
      "South forest deer",
      "Harthmere Wilds - South Greenmere",
      { radius: 5.2, speed: 0.26, phase: 2.7 }
    ),
    ...
    A(
      ...
      "Harthmere Wilds - Northwest Watchtower Ridge",
      { radius: 4.2, speed: 0.2, phase: 1.3 }
    ),
    A(
      "townsperson_bandit",
      188,
      ...
      "Harthmere Wilds - Northwest Watchtower Ridge",
      { radius: 3.2, speed: 0.17, phase: 0.4 }
    ),
    A(
      "animal_wolf",
      115,
    ...
    A(
      ...
      "Briarfen frog",
      "Harthmere Wilds - East Briarfen Woods",
      { radius: 1.1, speed: 0.42, phase: 0.2 }
    ),
    A(
      "animal_snake",
      930,
      ...
      "Harthmere Wilds - East Briarfen Woods",
      { radius: 1.4, speed: 0.2, phase: 2.3 }
    ),
    A(
      "townsperson_smuggler",
      910,
      ...
      Math.PI,
      1.1,
      "Outer Briarfen smuggler",
      "Harthmere Wilds - East Briarfen Woods",
      { radius: 4, speed: 0.18, phase: 1.7 }
    ),
    A(
      "townsperson_undead",
      955,
      ...
      -Math.PI / 2,
      1.06,
      "Drowned dead beyond reeds",
      "Harthmere Wilds - East Briarfen Woods",
      { radius: 2.2, speed: 0.1, phase: 0.6 }
    ),
    ...
    A(
      ...
      "Gravewood crow",
      "Harthmere Wilds - Southeast Gravewood",
      { radius: 1.4, speed: 0.42, phase: 0.5 }
    ),
    A(
      "townsperson_undead",
      805,
      ...
      Math.PI,
      1.08,
      "Outer gravewood dead",
      "Harthmere Wilds - Southeast Gravewood",
      { radius: 2.2, speed: 0.09, phase: 2.6 }
    ),
    A(
      "animal_wolf",
      735,
      ...
      "Pale gravewood wolf",
      "Harthmere Wilds - Southeast Gravewood",
      { radius: 3.0, speed: 0.28, phase: 1.5 }
    ),
    ...
    A(
      ...
      "Orchard chicken",
      "Harthmere Wilds - Southwest Orchardwood",
      { radius: 1.8, speed: 0.7, phase: 0.1 }
    ),
    A(
      "animal_pig",
      145,
      ...
      "Orchard pig",
      "Harthmere Wilds - Southwest Orchardwood",
      { radius: 2.2, speed: 0.32, phase: 1.8 }
    ),
    A(
      "townsperson_farmer",
      160,
      ...
      0,
      1.12,
      "Outer orchard farmer",
      "Harthmere Wilds - Southwest Orchardwood",
      { radius: 4, speed: 0.18, phase: 0.9 }
    )
  );
...
function createHarthmereDenseForestPlacements(): RuntimePlacement[] {
  ...
  for (const zone of zones) {
    for (let index = 0; index < zone.count; index += 1) {
      ...
      if (
        isInsideWideWildsTownBuffer(x, z) ||
        isNearWideWildsVisualRoad(x, z, 22)
      ) {
        continue;
      }
      ...
      const treeAsset =
        zone.trees[
          Math.floor(wildsDeterministic01(x, z, 103) * zone.trees.length)
        ] ?? zone.trees[0];
      const rot = wildsDeterministic01(x, z, 104) * Math.PI * 2;
      const scale =
        zone.treeScale * (0.74 + wildsDeterministic01(x, z, 105) * 0.82);
      placements.push(
        P(
          treeAsset,
          x,
          ...
          rot,
          scale,
          `${zone.district} dense tree ${index + 1}`,
          zone.district
        )
      );
      ...
      if (index % 2 === 0) {
        const underAsset =
          zone.understory[
            Math.floor(wildsDeterministic01(x, z, 106) * zone.understory.length)
          ] ?? zone.understory[0];
        placements.push(
          P(
            underAsset,
      ...
      if (index % 17 === 0) {
        const resourceAsset =
          zone.resources[
            Math.floor(wildsDeterministic01(x, z, 110) * zone.resources.length)
          ] ?? zone.resources[0];
        placements.push(
          P(
            resourceAsset,
  ...
  placements.push(
    P(
      ...
      0.4,
      0.72,
      "Bandit ridge abandoned minecart",
      "Harthmere Wilds - Northwest Bandit Ridge"
    ),
    P(
      "mine_coal_block",
      118,
    ...
    P(
      ...
      0.2,
      0.78,
      "Bandit ridge silver-bearing stone",
      "Harthmere Wilds - Northwest Bandit Ridge"
    ),
    P(
      "mine_stone_01",
    ...
    P(
      ...
      0.2,
      0.72,
      "Briarfen blackwater clay-stone",
      "Harthmere Wilds - East Briarfen Wood"
    ),
    P(
      "forest_rock_3f",
      910,
    ...
    P(
      ...
      0.2,
      0.84,
      "North Greenmere standing-stone quarry",
      "Harthmere Wilds - North Deep Greenmere"
    ),
    P(
      "forest_rock_2c",
      260,
      ...
      -0.3,
      0.74,
      "Southwest orchard fieldstone",
      "Harthmere Wilds - South Greenmere Thicket"
    )
  );
  ...
  placements.push(
    A(
      ...
      -Math.PI / 2,
      1.16,
      "Bandit ridge ambusher",
      "Harthmere Wilds - Northwest Bandit Ridge",
      { radius: 5.5, speed: 0.2, phase: 0.5 }
    ),
    A(
      "townsperson_bandit",
      245,
      ...
      Math.PI,
      1.12,
      "Bandit trapper in the old wood",
      "Harthmere Wilds - West Old Wood",
      { radius: 6.5, speed: 0.18, phase: 2.1 }
    ),
    A(
      "townsperson_bandit",
      870,
      ...
      "Briarfen road thief",
      "Harthmere Wilds - East Briarfen Wood",
      { radius: 4.5, speed: 0.18, phase: 1.4 }
    ),
    A(
      "townsperson_undead",
      805,
      ...
      Math.PI,
      1.1,
      "Gravewood wandering zombie",
      "Harthmere Wilds - Southeast Gravewood",
      { radius: 4.2, speed: 0.09, phase: 0.2 }
    ),
    A(
      "townsperson_undead",
      735,
      ...
      "Bell-woken corpse",
      "Harthmere Wilds - Southeast Gravewood",
      { radius: 5.2, speed: 0.08, phase: 2.5 }
    ),
    A(
      "townsperson_undead",
      928,
      ...
      -Math.PI / 2,
      1.04,
      "Drowned Briarfen zombie",
      "Harthmere Wilds - East Briarfen Wood",
      { radius: 3.8, speed: 0.08, phase: 1.1 }
    )
  );
  ...
  for (let i = 0; i < 48; i += 1) {
    ...
    placements.push(
      P(
        ...
        x,
        northZ,
        wildsDeterministic01(i, northZ, 203) * Math.PI * 2,
        1.05,
        `Northern far forest wall ${i + 1}`,
        "Harthmere Wilds - Far Edge"
      ),
      P(
        asset,
        x,
        southZ,
        wildsDeterministic01(i, southZ, 204) * Math.PI * 2,
        1.0,
        `Southern far forest wall ${i + 1}`,
        "Harthmere Wilds - Far Edge"
      )
    );
  }
  for (let i = 0; i < 48; i += 1) {
    ...
    placements.push(
      P(
        ...
        westX,
        z,
        wildsDeterministic01(westX, i, 207) * Math.PI * 2,
        1.02,
        `Western far forest wall ${i + 1}`,
        "Harthmere Wilds - Far Edge"
      ),
      P(
        asset,
        eastX,
        z,
        wildsDeterministic01(eastX, i, 208) * Math.PI * 2,
        1.04,
        `Eastern far forest wall ${i + 1}`,
        "Harthmere Wilds - Far Edge"
      )
    );
  }
...
const HARTHMERE_STREET_DECLUTTER_VERSION =
  "harthmere-street-declutter-runtime-cleanup";
const HARTHMERE_SINGLE_STORY_ROOF_CAP_VERSION =
  "harthmere-single-story-roof-cap";
const HARTHMERE_ROOF_STREET_BLOCK_CLEANUP_VERSION =
  "harthmere-roof-street-block-cleanup";
...
const HARTHMERE_STREET_CLUTTER_ASSET_RE =
  /^(crate_|crates_|barrel|barrel_|barrel_small|barrel_large|barrel_stack|farmcrate_|box_|bag_|bucket_|bench_|stall|stall_|cart|table_|table|chair|stool|rope_|lantern|obj_kiosk)$/i;
const HARTHMERE_STRUCTURAL_STREET_INTRUSION_RE =
  /(block-built|solid stone\/ore|roof volume|ceiling slab|floor slab|interior stone\/ore stair block|roof dormer|chimney|story \d+)/i;
const HARTHMERE_STREET_SEGMENTS: ReadonlyArray<
...
const HARTHMERE_NO_BUILD_BOXES: ReadonlyArray<
...
function isInsideHarthmereClearStreetRect(x: number, z: number) {
  return HARTHMERE_CLEAR_STREET_RECTS.some(([x0, x1, z0, z1]) =>
    isInsideRect(x, z, x0, x1, z0, z1)
  );
}

function isInsideAnyHarthmereRoofBuildingFootprint(
  x: number,
  z: number,
  pad = 0
) {
  return HARTHMERE_ROOF_CLEAR_BOXES.some((box) =>
    isInsideRect(x, z, box.x0, box.x1, box.z0, box.z1, pad)
  );
}

function roofClearBoxForPlacement(x: number, z: number) {
  return HARTHMERE_ROOF_CLEAR_BOXES.find((box) =>
    isInsideRect(x, z, box.x0, box.x1, box.z0, box.z1, 1)
  );
}

function isInsideUpperRoofCore(
  box: HarthmereRoofClearBox,
  x: number,
  z: number
) {
...
function isHarthmereLooseStreetOrRoofBlockPlacement(
  placement: RuntimePlacement
) {
  if (
    placement.combatOffset !== undefined ||
  ...
  if (
    /front door|doorway|public entrance|shop entrance|gate passage|road exit|town exit|window overlay/i.test(
      label
    )
  ) {
    return false;
...
function shouldRemoveStreetBlockPlacement(placement: RuntimePlacement) {
  ...
  if (
    !isInsideHarthmereClearStreetRect(x, z) &&
    !isNearHarthmereStreetCorridor(x, z)
  ) {
    return false;
  }
  ...
  const label = placementLabel(placement);
  if (
    /bridge fountain|old well|north gate tower|watchtower|town wall|parapet|solid flag pole/i.test(
      label
    )
  ) {
    return false;
...
function placementLabel(placement: RuntimePlacement) {
  return `${placement.asset} ${placement.name ?? ""} ${
    placement.district ?? ""
  }`.toLowerCase();
}
...
function isHarthmereGrovePrototypeDungeonClutter(placement: RuntimePlacement) {
  ...
  const label = placementLabel(placement);
  return /bellbinder antechamber|bell-notation|undercroft stair|chapel undercroft test entrance|q5 .*undercroft|q6 .*hidden door|hidden-door|phase-safe undercroft|mural inscription|tiny bell seal/.test(
    label
  );
}

function shouldRemoveProductionDebugClutterPlacement(
  placement: RuntimePlacement
) {
  const label = placementLabel(placement);
  if (
  ...
  if (
    /dungeon route step|bellward halls debug start|dungeon testing route placard/.test(
      label
    )
  ) {
    return true;
  ...
  if (
    /test entrance/.test(label) &&
    /chapel undercroft|old well drain|dungeon/.test(label)
  ) {
    return true;
  }
...
function isNearHarthmereStreetCorridor(x: number, z: number) {
  return HARTHMERE_STREET_SEGMENTS.some(
    ([ax, az, bx, bz, width]) =>
...
function isInsideHarthmereNoBuildBox(x: number, z: number) {
  return HARTHMERE_NO_BUILD_BOXES.some(
    ([x0, x1, z0, z1]) => x >= x0 && x <= x1 && z >= z0 && z <= z1
  );
}
...
function shouldRemoveStreetClutterPlacement(placement: RuntimePlacement) {
  ...
  if (
    !HARTHMERE_STREET_CLUTTER_ASSET_RE.test(placement.asset) &&
    !/street table|handcart|cargo stack|supply crate|vendor stool/i.test(label)
  ) {
    return false;
  }
  if (
    /interior|cellar|upstairs|archive|altar|against wall|behind tavern|warehouse floor|chapel pew|bedroom/i.test(
      label
    )
  ) {
    return false;
...
function shouldRemoveRoadIntrusionPlacement(placement: RuntimePlacement) {
  ...
  if (
    /north gate west tower|north gate east tower|north wall|north gate ironbound door|watchtower|bridge|bridge fountain|fountain|lamp|sign|banner/i.test(
      label
    )
  ) {
    return false;
...
const HARTHMERE_WALKABLE_BRIDGE_VERSION =
  "harthmere-walkable-bridge-with-parapets";
...
function createHarthmereOldBridgeWalkableParapets(): RuntimePlacement[] {
  ...
  placements.push(
    P(
      ...
      Math.PI / 2,
      0.86,
      "HARTHMERE_WALKABLE_BRIDGE Old Bridge walkable bridge deck visual under 12m clear lane no blocker",
      "Old Bridge"
    ),
    ...row(
      "road",
      "Old Bridge",
      "HARTHMERE_WALKABLE_BRIDGE old bridge pedestrian lane stone deck 12m clear bridge crack inspection lane",
      462,
      -206,
      8,
      7,
    ...
    ...row(
      "obj_wall_simple",
      "Old Bridge",
      "HARTHMERE_BRIDGE_PARAPET north bridge parapet rail outside walk lane",
      462,
      -199.5,
      8,
      7,
    ...
    ...row(
      "obj_wall_simple",
      "Old Bridge",
      "HARTHMERE_BRIDGE_PARAPET south bridge parapet rail outside walk lane",
      462,
      -212.5,
      8,
      7,
    ...
    P(
      ...
      0,
      0.44,
      "Old Bridge lantern outside pedestrian lane west",
      "Old Bridge"
    ),
    P(
      "obj_lamp_ground_small",
      ...
      Math.PI,
      0.44,
      "Old Bridge lantern outside pedestrian lane east",
      "Old Bridge"
    ),
    P(
      "coin_pile",
      ...
      0,
      0.2,
      "Bellbound Q1 bronze disc set into Old Bridge cobbles at crack center",
      "Old Bridge",
      GROUND_Y + 0.05
    ),
    P(
      ...
      0.25,
      0.16,
      "Bell-shaped crack decal flush with Old Bridge stone deck",
      "Old Bridge",
      GROUND_Y + 0.06
    ),
    P(
      ...
      Math.PI / 2,
      0.7,
      "Old Bridge west approach ramp road continuation clear mount path",
      "Old Bridge"
    ),
    P(
      "road",
      ...
      Math.PI / 2,
      0.7,
      "Old Bridge east approach ramp road continuation clear mount path",
      "Old Bridge"
    )
  );
  return placements;
...
function createHarthmereWildsBibleLandmarkPlacements(): RuntimePlacement[] {
  ...
  placements.push(
    ...
    P(
      ...
      Math.PI / 2,
      0.72,
      "HARTHMERE_WILDS_THORNBRIDGE Thornbridge Crossing walkable bridge deck over creek",
      "Harthmere Wilds - Thornbridge Crossing"
    ),
    ...row(
      "road",
      "Harthmere Wilds - Thornbridge Crossing",
      "HARTHMERE_WILDS_THORNBRIDGE Thornbridge Crossing walkable bridge path",
      324,
      -498,
      5,
      7,
    ...
    P(
      ...
      0.15,
      0.56,
      "Thornbridge Crossing thorn bushes north edge",
      "Harthmere Wilds - Thornbridge Crossing"
    ),
    P(
      "hedge_large",
      ...
      -0.12,
      0.56,
      "Thornbridge Crossing thorn bushes south edge",
      "Harthmere Wilds - Thornbridge Crossing"
    ),
    P(
      "banner_white",
      ...
      0,
      0.34,
      "Traveler luck ribbon tied at Thornbridge Crossing",
      "Harthmere Wilds - Thornbridge Crossing",
      GROUND_Y + 0.7
    ),
    P(
      ...
      -0.4,
      0.38,
      "Cut ribbon warning dagger resting on ground left by bandits at Thornbridge Crossing",
      "Harthmere Wilds - Thornbridge Crossing",
      GROUND_Y + 0.45
    ),
    ...
    P(
      ...
      0.1,
      1.22,
      "Split Oak living green half waypoint",
      "Harthmere Wilds - Split Oak"
    ),
    P(
      "tree_crooked",
      219,
      ...
      -0.2,
      1.14,
      "Split Oak black lightning-dead half waypoint",
      "Harthmere Wilds - Split Oak"
    ),
    P(
      "pillar",
      ...
      0,
      0.42,
      "Split Oak druid priest argument stone marker",
      "Harthmere Wilds - Split Oak"
    ),
    ...
    P(
      ...
      0.1,
      0.92,
      "Witchlight Pool dark water rim",
      "Harthmere Wilds - Witchlight Pool"
    ),
    P(
      "lantern",
      924,
      ...
      0,
      0.42,
      "Witchlight Pool hovering false light one",
      "Harthmere Wilds - Witchlight Pool",
      GROUND_Y + 1.05
    ),
    P(
      ...
      0,
      0.38,
      "Witchlight Pool hovering false light two",
      "Harthmere Wilds - Witchlight Pool",
      GROUND_Y + 1.18
    ),
    A(
      ...
      0,
      0.9,
      "Frog at Witchlight Pool",
      "Harthmere Wilds - Witchlight Pool",
      { radius: 1.0, speed: 0.3, phase: 1.2 }
    ),
    A(
      "townsperson_smuggler",
      938,
      ...
      Math.PI,
      1.04,
      "Smuggler listening near Witchlight Pool",
      "Harthmere Wilds - Witchlight Pool",
      { radius: 2.8, speed: 0.14, phase: 2.7 }
    ),
    ...
    P(
      ...
      0,
      0.9,
      "Old Quarry Cut rough stone face",
      "Harthmere Wilds - Old Quarry Cut"
    ),
    P(
      "mine_coal_block",
      176,
    ...
    P(
      ...
      -0.1,
      0.72,
      "Old Quarry Cut silver vein pocket",
      "Harthmere Wilds - Old Quarry Cut"
    ),
    P(
      "minecart",
      ...
      0.3,
      0.62,
      "Old Quarry Cut abandoned minecart",
      "Harthmere Wilds - Old Quarry Cut"
    ),
    P(
      "pickaxe_bronze_fp",
      ...
      -0.4,
      0.44,
      "Old Quarry Cut mining pick resting on ground beside real ore",
      "Harthmere Wilds - Old Quarry Cut",
      GROUND_Y + 0.45
    ),
    A(
      ...
      -0.5,
      0.92,
      "Quarry snake in warm stones",
      "Harthmere Wilds - Old Quarry Cut",
      { radius: 0.8, speed: 0.14, phase: 0.6 }
    )
  );
  return placements;
}
...
// empty.
const HARTHMERE_NPC_HOME_FURNITURE_VERSION = "harthmere-npc-home-furniture";
const HARTHMERE_NPC_HOME_ROOMS = [
  ...
  },
  {
    ...
    roomLabel:
      "Auction Clerk Pell Marsten room in Player Services / Auction Office",
    district: "Player Services Plaza",
    floor: 1,
    at: [505, 0.08, -221],
    furniture: ["bed", "nightstand", "personal_chest", "small_table", "candle"],
  },
  {
    ...
    roomLabel:
      "Guild Registrar Erena Voss room in Player Services / Auction Office",
    district: "Player Services Plaza",
    floor: 1,
    at: [505, 0.08, -221],
    furniture: ["bed", "nightstand", "personal_chest", "small_table", "candle"],
  ...
  },
  {
    ...
    roomLabel:
      "Veneth of the Green Threshold room in Deep Old Wood Glade Lodge",
  ...
  for (const room of HARTHMERE_NPC_HOME_ROOMS) {
    ...
    placements.push(
      ...
      P(
        ...
        0,
        0.1,
        room.roomLabel + " candle on table",
        district,
        y + 0.56
      )
    );
    ...
    const seed =
      Math.abs(Math.sin(room.at[0] * 12.9898 + room.at[2] * 78.233)) % 1;
...
const PLACEMENTS: RuntimePlacement[] = [
  ...
  ...HARTHMERE_MAIN_QUEST_SPACES.flatMap((space) => [
    ...
    P(
      ...
      0,
      0.34,
      `${space.name} current interactable anchor ${
        space.interactables?.[0] ?? "quest_anchor"
      }`,
      space.district,
    ...
    ...(space.encounters ?? [])
      .slice(0, 2)
      .map((encounter: string, index: number) =>
        P(
          "arch_wall_corner",
  ...
  A(
    ...
    "Mudden Drain Rat",
    "Mudden Ward",
    { radius: 0.8, speed: 0.28, phase: 0.2 },
    9002
  ),
  A(
    "townsperson_bandit",
    ...
    1.12,
    "Road Bandit Scout",
    "Harthmere Wilds - Mill Road",
    { radius: 1.8, speed: 0.18, phase: 2.7 },
    9003
  ),
  A(
    "animal_wolf",
    ...
    1.04,
    "Road Wolf",
    "Harthmere Wilds - Greenmere Edge",
    { radius: 2.0, speed: 0.32, phase: 2.1 },
    9004
  ),
  A(
    "townsperson_bandit",
    ...
    "Harthmere Wilds - Northwest Bandit Ridge",
    { radius: 2.2, speed: 0.16, phase: 0.5 },
    9005
  ),
  A(
    "townsperson_undead",
    ...
    1.08,
    "Bell-Woken Zombie",
    "Harthmere Wilds - Gravewood",
    { radius: 1.2, speed: 0.07, phase: 2.6 },
    9006
  ),
  A(
    "animal_deer",
    ...
    "Greenmere Deer",
    "Harthmere Wilds - North Greenmere",
    { radius: 2.8, speed: 0.18, phase: 0.4 },
    9007
  ),
  A(
    "animal_boar",
    ...
    0.95,
    "Diseased Boar",
    "Harthmere Wilds - Gate Fields",
    { radius: 1.8, speed: 0.28, phase: 1.7 },
    9008
  ),
  A(
    "animal_bear",
    ...
    "Black Bear",
    "Harthmere Wilds - Greenmere Edge",
    { radius: 1.5, speed: 0.14, phase: 0.3 },
    9009
  ),
  A(
    "animal_wolf",
    ...
    "Forest Wolf",
    "Harthmere Wilds - Gravewood",
    { radius: 1.7, speed: 0.25, phase: 1.5 },
    9010
  ),
  A(
    "animal_snake",
    ...
    1.0,
    "Briarfen Water Snake",
    "Harthmere Wilds - Briarfen",
    { radius: 0.7, speed: 0.16, phase: 2.3 },
    9011
  ),
  A(
    "animal_wolf",
    ...
    "Gravewood Pale Wolf",
    "Harthmere Wilds - Southeast Gravewood",
    { radius: 1.8, speed: 0.22, phase: 1.5 },
    9012
  ),
  A(
    "townsperson_bandit",
    ...
    "Bandit Trapper",
    "Harthmere Wilds - West Old Wood",
    { radius: 2.0, speed: 0.15, phase: 2.1 },
    9013
  ),
  AD(
    "townsperson_undead",
    ...
    -Math.PI / 2,
    1.02,
    "Combat Diagnostic Watchtower Muckling",
    "Harthmere Combat Diagnostics - Mucker",
    { radius: 1.5, speed: 0.2, phase: 0.9 },
    9020
  ),
  AD(
    "townsperson_undead",
    ...
    Math.PI,
    1.08,
    "Combat Diagnostic Greater Hexer",
    "Harthmere Combat Diagnostics - Hex",
    { radius: 1.4, speed: 0.18, phase: 1.6 },
    9021
  ),
  AD(
    "townsperson_undead",
    ...
    -0.4,
    1.04,
    "Combat Diagnostic Old Wood Mucker",
    "Harthmere Combat Diagnostics - Mucker",
    { radius: 1.6, speed: 0.18, phase: 2.4 },
    9022
  ),
  AD(
    "townsperson_undead",
    ...
    0.2,
    1.1,
    "Combat Diagnostic Lesser Hexer",
    "Harthmere Combat Diagnostics - Hex",
    { radius: 1.3, speed: 0.16, phase: 0.7 },
    9023
  ),
  AD(
    "animal_wolf",
    ...
    -Math.PI / 2,
    1.02,
    "Combat Diagnostic Road Wolf",
    "Harthmere Combat Diagnostics - Animal",
    { radius: 1.6, speed: 0.24, phase: 0.4 },
    9024
  ),
  AD(
    "animal_deer",
    ...
    Math.PI / 2,
    1.0,
    "Combat Diagnostic Greenmere Deer",
    "Harthmere Combat Diagnostics - Animal",
    { radius: 1.8, speed: 0.18, phase: 1.1 },
    9025
  ),
  ...
  ...HARTHMERE_NAMED_NPCS.map((npc) =>
    A(
      harthmereBossStaticAssetKeyForLabel(npc.name) ??
        harthmereNamedNpcActorAsset(npc),
      npc.spawn.x,
      npc.spawn.z,
      npc.spawn.rot,
      harthmereBossVisualForLabel(npc.name) ? 1 : npc.spawn.scale,
      npc.name,
      npc.district,
      undefined,
      npc.combatOffset
  ...
  P(
    ...
    0,
    1.05,
    "Watch banner left planted in tower base solid flag pole",
    "North Gate"
  ),
  P(
    "obj_flag_large_red",
    ...
    0,
    1.05,
    "Watch banner right planted in tower base solid flag pole",
    "North Gate"
  ),
  P(
    "obj_lamp_ground_large",
  ...
  ...createHarthmereBlockBuiltServiceBuilding({
  ...
  ...createHarthmereBlockBuiltServiceBuilding({
    name: "Carpenter and Tailor Workshop",
    district: "Craftsman Row",
    x: 504,
    z: -228,
    w: 18,
  ...
  P(
    ...
    Math.PI / 2,
    0.68,
    "Black Anvil Smithy sign with repair and blacksmith training notice",
    "Craftsman Row"
  ),
  P(
    "anvil_fp",
  ...
  P(
    ...
    Math.PI / 2,
    0.5,
    "Black Anvil red forge banner beside smithy entrance mounted on wall bracket",
    "Craftsman Row",
    GROUND_Y + 1.12
  ),
  P(
    ...
    0,
    0.5,
    "Black Anvil broad smoke stack base on roof",
    "Craftsman Row",
    GROUND_Y + 3.12
  ),
  P(
    ...
    0,
    0.48,
    "Black Anvil smoke stack top above forge",
    "Craftsman Row",
    GROUND_Y + 4.0
  ),
  P(
    ...
    0,
    0.66,
    "Craftsman Row forge glow lamp west of smithy",
    "Craftsman Row"
  ),
  P(
    "obj_lamp_ground_small",
    ...
    0,
    0.66,
    "Craftsman Row forge glow lamp east of smithy",
    "Craftsman Row"
  ),
  P(
    "obj_sign_post",
    ...
    Math.PI / 2,
    0.58,
    "Profession trainer lane sign for carpentry tailoring leatherworking and work orders",
    "Craftsman Row"
  ),
  P(
    "banner_brown",
    ...
    Math.PI / 2,
    0.38,
    "Workshop guild banner marking multi-profession crafting lane",
    "Craftsman Row",
    GROUND_Y + 0.96
  ),
  ...createHarthmereBlockBuiltServiceBuilding({
    name: "Green Mortar Apothecary",
    district: "Apothecary",
    x: 455,
    z: -176,
    w: 18,
  ...
  ...createHarthmereBlockBuiltServiceBuilding({
    name: "Wyrm and Candle Magic Shop",
    district: "Magic Shop",
    x: 518,
    z: -168,
    w: 20,
  ...
  P(
    ...
    Math.PI / 2,
    0.72,
    "Copper Kettle Inn signpost with room rental and food notices",
    "Copper Kettle"
  ),
  P(
    "cauldron_fp",
    ...
    0,
    0.42,
    "Copper kettle hanging sign symbol supported below inn sign",
    "Copper Kettle",
    GROUND_Y + 0.95
  ),
  P(
    ...
    Math.PI / 2,
    0.5,
    "Warm tavern banner beside Copper Kettle sign",
    "Copper Kettle",
    GROUND_Y + 1.12
  ),
  P(
    ...
    0,
    0.72,
    "Warm inn entrance lamp west of door",
    "Copper Kettle"
  ),
  P(
    "obj_lamp_ground_small",
    ...
    0,
    0.72,
    "Warm inn entrance lamp east of door",
    "Copper Kettle"
  ),
  P(
    "arch_chimney",
    ...
    0,
    0.48,
    "Copper Kettle roof chimney smoke marker",
    "Copper Kettle",
    GROUND_Y + 4.15
  ),
  ...createHarthmereBlockBuiltServiceBuilding({
  ...
  P(
    ...
    Math.PI,
    0.88,
    "Noble Rise raised terrace stair from market road",
    "Noble Rise"
  ),
  P(
    "arch_pillar_stone",
    ...
    0,
    0.74,
    "Noble Rise west estate pillar marking restricted approach",
    "Noble Rise"
  ),
  P(
    "arch_pillar_stone",
    ...
    0,
    0.74,
    "Noble Rise east estate pillar marking restricted approach",
    "Noble Rise"
  ),
  P(
    "obj_lamp_ground_small",
    ...
    0,
    0.62,
    "Noble Rise brass approach lamp west",
    "Noble Rise"
  ),
  P(
    "obj_lamp_ground_small",
    ...
    0,
    0.62,
    "Noble Rise brass approach lamp east",
    "Noble Rise"
  ),
  P(
    "obj_sign_post",
    ...
    Math.PI,
    0.58,
    "Reeve Hall court permit tax office sign",
    "Noble Rise"
  ),
  P(
    "banner_green",
    ...
    Math.PI,
    0.36,
    "Clean green shutter banner on Reeve Hall west face",
    "Noble Rise",
    GROUND_Y + 1.04
  ),
  P(
    ...
    Math.PI,
    0.36,
    "Red town authority banner on Reeve Hall east face",
    "Noble Rise",
    GROUND_Y + 1.04
  ),
  P(
    ...
    Math.PI,
    0.34,
    "White legal notice cloth under Reeve Hall balcony",
    "Noble Rise",
    GROUND_Y + 1.0
  ),
  P(
    ...
    Math.PI,
    0.52,
    "Reeve Hall west roof window for wealthy skyline variety",
    "Noble Rise",
    GROUND_Y + 3.92
  ),
  P(
    ...
    Math.PI,
    0.52,
    "Reeve Hall east roof window for wealthy skyline variety",
    "Noble Rise",
    GROUND_Y + 3.92
  ),
  P(
    ...
    0,
    0.42,
    "Reeve Hall polished chimney top",
    "Noble Rise",
    GROUND_Y + 4.08
  ),
  P(
    ...
    0,
    0.44,
    "Noble private garden fountain beside court approach",
    "Noble Rise"
  ),
  P(
    "bench_fp",
    ...
    Math.PI / 2,
    0.58,
    "Noble petition bench beside private garden",
    "Noble Rise"
  ),
  P(
    "bench_fp",
    ...
    Math.PI / 2,
    0.58,
    "Noble waiting bench beside private garden",
    "Noble Rise"
  ),
  P(
    "hedge_large",
    ...
    0,
    0.58,
    "Noble garden clipped hedge north edge",
    "Noble Rise"
  ),
  P(
    "hedge_large",
    ...
    0,
    0.58,
    "Noble garden clipped hedge south edge",
    "Noble Rise"
  ),
  P(
    "hedge",
  ...
  P(
    ...
    Math.PI / 2,
    0.5,
    "Servant entrance and deliveries sign behind Reeve Hall",
    "Noble Rise"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.46,
    "Servant delivery crate on floor near side entrance",
    "Noble Rise"
  ),
  P(
    "scroll_1_fp",
    ...
    0.2,
    0.24,
    "Servant rebellion note supported on delivery crate",
    "Noble Rise",
    GROUND_Y + 0.64
  ),
  P(
    ...
    -Math.PI / 2,
    0.5,
    "Tax protest assembly notice outside Reeve Hall",
    "Noble Rise"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.44,
    "Tax protest placard crate on floor outside court",
    "Noble Rise"
  ),
  P(
    "scroll_2_fp",
    ...
    -0.18,
    0.24,
    "Pinned tax protest placard supported on protest crate",
    "Noble Rise",
    GROUND_Y + 0.64
  ),
  ...createHarthmereBlockBuiltServiceBuilding({
    name: "Dock Ledger Warehouse",
    district: "River Docks",
    x: 596,
    z: -172,
    w: 26,
  ...
  P(
    ...
    Math.PI / 2,
    0.62,
    "River Docks ferry cargo contracts fishing trainer sign",
    "River Docks"
  ),
  P(
    "obj_lamp_ground_small",
    ...
    0,
    0.68,
    "River Docks fog lamp beside cargo road",
    "River Docks"
  ),
  P(
    "obj_lamp_ground_small",
    ...
    0,
    0.66,
    "River Docks ferry fog lamp at pier head",
    "River Docks"
  ),
  P(
    "obj_church_bells",
    ...
    Math.PI / 2,
    0.34,
    "Dock bell mounted above ferry and flood warning post",
    "River Docks",
    GROUND_Y + 1.32
  ),
  P(
    ...
    Math.PI / 2,
    0.38,
    "Wet blue dockmaster banner under bell post",
    "River Docks",
    GROUND_Y + 0.98
  ),
  P(
    ...
    -Math.PI / 2,
    0.5,
    "Dock Ledger Warehouse roof dormer facing river",
    "River Docks",
    GROUND_Y + 3.0
  ),
  P(
    ...
    0,
    0.38,
    "Dock Ledger Warehouse smoke vent for damp cargo office",
    "River Docks",
    GROUND_Y + 3.16
  ),
  P(
    ...
    Math.PI,
    0.52,
    "Cargo contracts board outside Dock Ledger Warehouse",
    "River Docks"
  ),
  P(
    "scroll_1_fp",
    ...
    0.1,
    0.25,
    "Pinned cargo contract supported on warehouse board",
    "River Docks",
    GROUND_Y + 0.86
  ),
  P(
    ...
    -0.1,
    0.25,
    "Pinned ferry schedule supported on warehouse board",
    "River Docks",
    GROUND_Y + 0.86
  ),
  P(
    ...
    Math.PI / 2,
    0.58,
    "Cargo handcart waiting beside warehouse road",
    "River Docks"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.52,
    "Sorted river goods crate beside warehouse handcart",
    "River Docks"
  ),
  P(
    "barrel_large",
    ...
    0,
    0.52,
    "Flood sand barrel beside dock bell",
    "River Docks"
  ),
  P(
    "obj_sign_post",
    ...
    -Math.PI / 2,
    0.48,
    "Ferry master water travel sign at pier",
    "River Docks"
  ),
  P(
    "obj_bridge_low_body",
    ...
    Math.PI / 2,
    0.52,
    "Low ferry skiff silhouette tied to pier",
    "River Docks"
  ),
  P(
    "rope_3_fp",
    ...
    Math.PI / 2,
    0.54,
    "Ferry mooring rope tied on pier edge",
    "River Docks"
  ),
  P(
    "chain_coil",
    ...
    0,
    0.48,
    "Ferry chain coil kept off main walking lane",
    "River Docks"
  ),
  ...createHarthmereBlockBuiltServiceBuilding({
    name: "Mudden Lean-To Home",
    district: "Mudden Ward",
    x: 412,
    z: -158,
    w: 17,
  ...
  P(
    ...
    Math.PI / 2,
    0.46,
    "Mudden patched lean-to roof left scrap",
    "Mudden Ward",
    GROUND_Y + 2.62
  ),
  P(
    ...
    Math.PI / 2,
    0.46,
    "Mudden patched lean-to roof right scrap",
    "Mudden Ward",
    GROUND_Y + 2.58
  ),
  P(
    ...
    Math.PI / 2,
    0.72,
    "Canvas roof patch tied over Mudden lean-to",
    "Mudden Ward",
    GROUND_Y + 2.34
  ),
  P(
    ...
    Math.PI / 2,
    0.42,
    "Leaning broken wall brace beside Mudden lean-to",
    "Mudden Ward"
  ),
  P(
    "arch_pillar_wood",
    ...
    0,
    0.38,
    "Crooked timber post supporting sagging alley line",
    "Mudden Ward"
  ),
  P(
    "arch_pillar_wood",
    ...
    0,
    0.38,
    "Crooked timber post supporting laundry rope",
    "Mudden Ward"
  ),
  P(
    "rope_2_fp",
    ...
    Math.PI / 2,
    0.56,
    "Laundry line tied between Mudden houses",
    "Mudden Ward",
    GROUND_Y + 1.72
  ),
  P(
    ...
    Math.PI / 2,
    0.52,
    "Washed cloth hanging from Mudden laundry line",
    "Mudden Ward",
    GROUND_Y + 1.52
  ),
  P(
    ...
    0,
    0.5,
    "Back-alley laundry rope tied to wash house",
    "Mudden Ward",
    GROUND_Y + 1.56
  ),
  P(
    ...
    0,
    0.42,
    "Pale laundry sheet hanging in wash-house lane",
    "Mudden Ward",
    GROUND_Y + 1.38
  ),
  P(
    ...
    0,
    0.42,
    "Small smoky cookfire at Mudden alley edge grounded in stone fire ring",
    "Mudden Ward",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.42,
    "Water bucket beside smoky cookfire",
    "Mudden Ward"
  ),
  P(
    "rock_wide",
    ...
    0.2,
    0.36,
    "Dark mud puddle proxy in low alley",
    "Mudden Ward"
  ),
  P(
    "rock_wide",
    ...
    -0.2,
    0.32,
    "Rain puddle proxy beside wash house",
    "Mudden Ward"
  ),
  P(
    "obj_sign_post",
    ...
    Math.PI / 2,
    0.42,
    "Mudden Ward warning sign poverty flood disease",
    "Mudden Ward"
  ),
  P(
    "scroll_2_fp",
    ...
    0.08,
    0.22,
    "Eviction notice pinned to Mudden warning sign",
    "Mudden Ward",
    GROUND_Y + 0.82
  ),
  P(
    ...
    -Math.PI / 2,
    0.38,
    "Washerwomen work board and cheap healer notice",
    "Mudden Ward"
  ),
  P(
    "scroll_1_fp",
    ...
    -0.12,
    0.22,
    "Cheap healer note supported on Mudden board",
    "Mudden Ward",
    GROUND_Y + 0.78
  ),
  P(
    ...
    Math.PI / 2,
    0.52,
    "Hidden tunnel access under broken laundry cart",
    "Mudden Ward"
  ),
  P(
    "crate_wooden_fp",
    ...
    0.1,
    0.38,
    "Broken laundry cart hiding tunnel edge",
    "Mudden Ward"
  ),
  P(
    "torch_mounted",
    ...
    Math.PI,
    0.38,
    "Dim underways marker torch near Mudden tunnel mounted on Mudden tunnel wall bracket",
    "Mudden Ward",
    GROUND_Y + 1.0
  ),
  P(
    ...
    0,
    0.36,
    "Old carved drain stone beside hidden tunnel",
    "Mudden Ward"
  ),
  ...createHarthmereBlockBuiltServiceBuilding({
    name: "Roadside Family Cottage",
    district: "Residential District",
    x: 456,
    z: -256,
    w: 20,
  ...
  ...createHarthmereBlockBuiltServiceBuilding({
    name: "Mara Thistle Two-Story House",
  ...
  P(
    ...
    Math.PI,
    0.72,
    "Empty bell-frame clue supported on rebuilt stone chapel bell arch",
    "Temple Green",
    GROUND_Y + 4.65
  ),
  P(
  ...
  P(
    ...
    Math.PI,
    0.9,
    "Pulpit on floor clear of center aisle",
    "Temple Green"
  ),
  P(
    "candlestick_stand_fp",
  ...
  P(
    ...
    0.8,
    0.75,
    "Gravedigger shovel leaning near graves",
    "Temple Green"
  ),
  ...
  P(
    ...
    Math.PI,
    0.34,
    "Temple Green healer resurrection charity sign",
    "Temple Green"
  ),
  P(
    "scroll_1_fp",
    ...
    0.04,
    0.16,
    "Temple service hours note supported on healer sign",
    "Temple Green",
    GROUND_Y + 0.68
  ),
  P(
    ...
    Math.PI,
    0.66,
    "Blue chapel healing banner mounted by entry",
    "Temple Green",
    GROUND_Y + 1.22
  ),
  P(
    ...
    Math.PI,
    0.66,
    "White chapel resurrection banner mounted by entry",
    "Temple Green",
    GROUND_Y + 1.22
  ),
  P(
  ...
  P(
    ...
    0,
    0.54,
    "Temple entry lantern right of healing path mounted on chapel entry wall bracket",
    "Temple Green",
    GROUND_Y + 1.08
  ),
  P(
    ...
    Math.PI / 2,
    0.72,
    "Lower stained-glass chapel window west wall",
    "Temple Green",
    GROUND_Y + 1.05
  ),
  P(
    ...
    -Math.PI / 2,
    0.72,
    "Lower stained-glass chapel window east wall",
    "Temple Green",
    GROUND_Y + 1.05
  ),
  P(
    ...
    Math.PI,
    0.66,
    "Upper stained-glass lantern window above altar",
    "Temple Green",
    GROUND_Y + 2.25
  ),
  P(
  ...
  P(
    ...
    Math.PI,
    0.64,
    "Resurrection altar service table",
    "Temple Green"
  ),
  P(
    "candlestick_triple_fp",
    ...
    0,
    0.34,
    "Left resurrection altar candles supported on altar",
    "Temple Green",
    GROUND_Y + 0.82
  ),
  P(
    ...
    0,
    0.34,
    "Right resurrection altar candles supported on altar",
    "Temple Green",
    GROUND_Y + 0.82
  ),
  P(
    ...
    Math.PI,
    0.34,
    "Open resurrection rite book supported on altar",
    "Temple Green",
    GROUND_Y + 0.86
  ),
  P(
    ...
    0.1,
    0.28,
    "Condition cleansing vial supported on altar",
    "Temple Green",
    GROUND_Y + 0.82
  ),
  P(
    ...
    0,
    0.34,
    "Resurrection point candle ring marker",
    "Temple Green"
  ),
  P(
    "table_medium",
    ...
    Math.PI / 2,
    0.58,
    "Charity turn-in table beside chapel aisle",
    "Temple Green"
  ),
  P(
    "coin_pile",
    ...
    0,
    0.24,
    "Donation coins supported on charity table",
    "Temple Green",
    GROUND_Y + 0.72
  ),
  P(
    ...
    0,
    0.38,
    "Charity bread supported on charity table",
    "Temple Green",
    GROUND_Y + 0.72
  ),
  P(
    ...
    -0.1,
    0.15,
    "Charity ledger supported on charity table",
    "Temple Green",
    GROUND_Y + 0.74
  ),
  P(
    ...
    0,
    0.5,
    "Charity food crate on floor beside wall",
    "Temple Green"
  ),
  P(
    "farmcrate_apple",
  ...
  P(
    ...
    -Math.PI / 2,
    0.62,
    "Chapel lore archive bookcase against east wall",
    "Temple Green"
  ),
  P(
    "bookstand_fp",
    ...
    Math.PI,
    0.42,
    "Missing Bell lore bookstand near archive",
    "Temple Green"
  ),
  P(
    "scroll_1_fp",
    ...
    0.08,
    0.15,
    "Missing Bell archive clue supported on bookstand",
    "Temple Green",
    GROUND_Y + 0.68
  ),
  P(
    ...
    0.1,
    0.3,
    "Archive books stacked on floor by bookcase",
    "Temple Green"
  ),
  P(
    "shelf_candles",
    ...
    -Math.PI / 2,
    0.48,
    "Prayer candle shelf fixed to chapel wall",
    "Temple Green"
  ),
  P(
    "church_candelabra",
    ...
    0,
    0.54,
    "Left candle rack for candle vigil",
    "Temple Green"
  ),
  P(
    "church_candelabra",
    ...
    0,
    0.54,
    "Right candle rack for candle vigil",
    "Temple Green"
  ),
  P(
    "bed_twin1",
    ...
    -Math.PI / 2,
    0.58,
    "Injured traveler cot in chapel infirmary corner",
    "Temple Green"
  ),
  P(
    "small_bottle_fp",
    ...
    0,
    0.26,
    "Healing tincture supported beside injured traveler cot",
    "Temple Green",
    GROUND_Y + 0.42
  ),
  P(
    ...
    0,
    0.44,
    "Clean water bucket beside infirmary cot",
    "Temple Green"
  ),
  P(
    "chair",
    ...
    Math.PI / 2,
    0.46,
    "Sister Maelle healer chair beside infirmary cot",
    "Temple Green"
  ),
  P(
    "obj_sign_post",
  ...
  P(
    ...
    0.12,
    0.14,
    "Funeral procession notice supported on cemetery sign",
    "Temple Green",
    GROUND_Y + 0.66
  ),
  P(
    ...
    0,
    0.48,
    "Low cemetery wall at chapel boundary",
    "Temple Green"
  ),
  P(
    "obj_church_grave_wall",
    ...
    0,
    0.48,
    "Second low cemetery wall at chapel boundary",
    "Temple Green"
  ),
  P(
    "tombstone",
    ...
    -0.2,
    0.62,
    "Leaning old grave marker near chapel",
    "Temple Green"
  ),
  P(
    "tombstone",
    ...
    0.12,
    0.58,
    "Family grave marker near cemetery path",
    "Temple Green"
  ),
  P(
    "coffin",
    ...
    0.1,
    0.42,
    "Funeral procession coffin staging prop",
    "Temple Green"
  ),
  P(
    "candle_thin_lit",
    ...
    0,
    0.3,
    "Funeral candle supported on grave edge",
    "Temple Green"
  ),
  P(
    "candle_thin_lit",
    ...
    0,
    0.3,
    "Second funeral candle supported on grave edge",
    "Temple Green"
  ),
  P(
    "shovel",
    ...
    0.9,
    0.54,
    "Gravekeeper spare shovel leaning against cemetery wall",
    "Temple Green"
  ),
  P(
    "bucket_wood",
    ...
    0,
    0.42,
    "Gravekeeper flower water bucket on ground",
    "Temple Green"
  ),
  P(
    "obj_church_crypt",
    ...
    Math.PI / 2,
    0.28,
    "Crypt disturbance sealed crypt breadcrumb",
    "Temple Green"
  ),
  P(
    "torch_mounted",
    ...
    -Math.PI / 2,
    0.28,
    "Crypt disturbance warning torch mounted on crypt wall bracket",
    "Temple Green",
    GROUND_Y + 0.9
  ),
  P(
    ...
    Math.PI,
    0.28,
    "River Blessing processional start sign",
    "Temple Green"
  ),
  P(
    "scroll_1_fp",
    ...
    0.05,
    0.14,
    "River Blessing event route note supported on sign",
    "Temple Green",
    GROUND_Y + 0.66
  ),
  P(
    ...
    Math.PI,
    0.48,
    "River Blessing blue processional banner",
    "Temple Green",
    GROUND_Y + 1.02
  ),
  P(
  ...
  P(
    ...
    0,
    0.26,
    "Flood fever cure vial supported on plague prayer table",
    "Temple Green",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.24,
    "Plague prayer candle supported on offering table",
    "Temple Green",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0.08,
    0.14,
    "Plague prayer request supported on offering table",
    "Temple Green",
    GROUND_Y + 0.6
  ),
  P(
    ...
    Math.PI / 2,
    0.24,
    "Frayed missing-bell rope beside empty bell frame",
    "Temple Green",
    GROUND_Y + 2.25
  ),
  P(
    ...
    0.2,
    0.22,
    "Tiny bronze missing-bell shard on altar stair",
    "Temple Green",
    GROUND_Y + 0.36
  ),
  P(
    ...
    -Math.PI / 2,
    0.28,
    "Green Mortar Apothecary healer referral sign",
    "Apothecary"
  ),
  P(
    "scroll_1_fp",
    ...
    0.03,
    0.14,
    "Ysabet Fenlow treatment hours note supported on apothecary sign",
    "Apothecary",
    GROUND_Y + 0.66
  ),
  P(
    ...
    0.1,
    0.3,
    "Ysabet Fenlow remedy bottles supported on treatment table",
    "Apothecary",
    GROUND_Y + 0.72
  ),
  P(
    ...
    0.08,
    0.14,
    "Flood fever cure recipe supported on treatment table",
    "Apothecary",
    GROUND_Y + 0.74
  ),
  ...
  P(
    ...
    0,
    0.86,
    "Apple crate beside produce stall",
    "Market Square"
  ),
  P(
    "farmcrate_carrot",
    ...
    0,
    0.86,
    "Carrot crate beside produce stall",
    "Market Square"
  ),
  P(
    "barrel_apples",
  ...
  P(
    ...
    0,
    0.92,
    "Bridge Fountain carved rim detail",
    "Market Square",
    GROUND_Y + 0.06
  ),
  P(
    ...
    0,
    0.62,
    "Bridge Fountain center stone marker",
    "Market Square",
    GROUND_Y + 0.12
  ),
  P(
  ...
  P(
    ...
    0,
    0.64,
    "Harvest Fair green plaza banner",
    "Market Square",
    GROUND_Y + 1.1
  ),
  P(
    ...
    0,
    0.58,
    "Harvest Fair yellow plaza banner",
    "Market Square",
    GROUND_Y + 1.08
  ),
  P(
    ...
    Math.PI / 2,
    0.62,
    "Produce sample table beside stall off mounted lane",
    "Market Square"
  ),
  P(
    "food_apple",
    ...
    0.2,
    0.28,
    "Apple sample supported on produce table",
    "Market Square",
    GROUND_Y + 0.68
  ),
  P(
    ...
    -0.3,
    0.28,
    "Green apple sample supported on produce table",
    "Market Square",
    GROUND_Y + 0.68
  ),
  P(
    ...
    Math.PI / 2,
    0.38,
    "Carrot sample supported on produce table",
    "Market Square",
    GROUND_Y + 0.68
  ),
  P(
    ...
    0,
    0.48,
    "Bread sample supported on market table",
    "Market Square",
    GROUND_Y + 0.68
  ),
  P(
    ...
    0.2,
    0.46,
    "Cheese sample supported on market table",
    "Market Square",
    GROUND_Y + 0.68
  ),
  P(
    ...
    Math.PI / 2,
    0.5,
    "Butcher shelf beside food stall with supported meat",
    "Market Square"
  ),
  P(
    "stall_stool",
    ...
    Math.PI / 2,
    0.72,
    "Produce vendor stool tucked beside stall",
    "Market Square"
  ),
  P(
    "table_long_decorated",
  ...
  P(
    ...
    -Math.PI / 2,
    0.46,
    "Blue cloth bolt supported on display counter",
    "Market Square",
    GROUND_Y + 0.74
  ),
  P(
    ...
    -Math.PI / 2,
    0.46,
    "White cloth bolt supported on display counter",
    "Market Square",
    GROUND_Y + 0.74
  ),
  P(
    ...
    -Math.PI / 2,
    0.46,
    "Brown cloth bolt supported on display counter",
    "Market Square",
    GROUND_Y + 0.74
  ),
  P(
  ...
  P(
    ...
    0,
    0.5,
    "Rope coil supported on travel goods counter",
    "Market Square",
    GROUND_Y + 0.72
  ),
  P(
    ...
    0,
    0.42,
    "Lantern supported on travel goods counter",
    "Market Square",
    GROUND_Y + 0.74
  ),
  P(
    ...
    0,
    0.54,
    "Travel sack under general goods stall",
    "Market Square"
  ),
  P(
    "chest_wood_fp",
    ...
    Math.PI / 2,
    0.52,
    "Lockbox under general goods stall",
    "Market Square"
  ),
  P(
    "fence",
    ...
    Math.PI / 2,
    0.74,
    "Livestock pen west rail at market edge",
    "Market Square"
  ),
  P(
    "fence",
    ...
    Math.PI / 2,
    0.74,
    "Livestock pen east rail at market edge",
    "Market Square"
  ),
  P(
    "fence",
    ...
    0,
    0.74,
    "Livestock pen north rail at market edge",
    "Market Square"
  ),
  P(
    "bucket_wood",
    ...
    0,
    0.58,
    "Livestock water bucket inside market pen",
    "Market Square"
  ),
  P(
    "rack",
  ...
  P(
    ...
    0,
    0.58,
    "Puppet show performance platform",
    "Market Square"
  ),
  P(
    "stool_fp",
    ...
    0,
    0.5,
    "Performer stool beside puppet show platform",
    "Market Square"
  ),
  P(
    "mug_empty",
    ...
    0,
    0.32,
    "Donation mug supported on performer platform",
    "Market Square",
    GROUND_Y + 0.64
  ),
  P(
    ...
    -Math.PI / 2,
    0.64,
    "Merchant dispute and price riot event notice",
    "Market Square"
  ),
  P(
    "crate_b",
    ...
    0.2,
    0.56,
    "Harvest Fair overflow supply crate",
    "Market Square"
  ),
  P(
    "crate_c",
    ...
    -0.1,
    0.56,
    "Seasonal market booth supply crate",
    "Market Square"
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.72,
    "Bakery supported display table",
    "Bakery"
  ),
  P(
    "bread_loaf",
  ...
  P(
    ...
    0,
    0.82,
    "Apple crate by bakery oven wall",
    "Bakery"
  ),
  ...
  P(
    ...
    -Math.PI / 2,
    0.78,
    "Provision shelves against wall",
    "Market District"
  ),
  P(
    "barrel_stack",
  ...
  P(
    ...
    Math.PI / 2,
    0.78,
    "Storage shelf against west wall",
    "Player Services"
  ),
  P(
    "wall_shelves",
  ...
  P(
    ...
    Math.PI,
    0.72,
    "Player Services bank mail auction storage signpost",
    "Player Services"
  ),
  P(
    "banner_green",
    ...
    Math.PI,
    0.58,
    "Bank and storage green service banner",
    "Player Services",
    GROUND_Y + 1.18
  ),
  P(
    ...
    Math.PI,
    0.52,
    "Auction and guild blue service banner",
    "Player Services",
    GROUND_Y + 1.12
  ),
  P(
    ...
    0,
    0.76,
    "Player Services entrance lamp west",
    "Player Services"
  ),
  P(
    "obj_lamp_ground_small",
    ...
    0,
    0.76,
    "Player Services entrance lamp east",
    "Player Services"
  ),
  ...
  P(
    ...
    0.12,
    0.42,
    "Bank ledger supported on bank teller counter",
    "Player Services",
    GROUND_Y + 0.82
  ),
  P(
    ...
    -0.2,
    0.32,
    "Vault key ring supported on bank teller counter",
    "Player Services",
    GROUND_Y + 0.86
  ),
  P(
    ...
    0.1,
    0.38,
    "Second coin pile supported on bank teller counter",
    "Player Services",
    GROUND_Y + 0.82
  ),
  P(
    ...
    Math.PI / 2,
    0.68,
    "Reinforced vault chest behind bank line",
    "Player Services"
  ),
  P(
    "key_metal_fp",
    ...
    0.1,
    0.32,
    "Vault spare key supported on vault desk",
    "Player Services",
    GROUND_Y + 0.72
  ),
  ...
  P(
    ...
    0,
    0.36,
    "Storage claim book supported on steward counter",
    "Player Services",
    GROUND_Y + 0.72
  ),
  P(
    ...
    Math.PI / 2,
    0.68,
    "Storage upgrade chest beside steward",
    "Player Services"
  ),
  P(
    "box_decorated",
    ...
    0,
    0.64,
    "Storage parcel box beside steward counter",
    "Player Services"
  ),
  ...
  P(
    ...
    -0.2,
    0.36,
    "Sealed letter supported on mail sorting counter",
    "Player Services",
    GROUND_Y + 0.72
  ),
  P(
    ...
    0.2,
    0.36,
    "Courier manifest supported on mail sorting counter",
    "Player Services",
    GROUND_Y + 0.72
  ),
  ...
  P(
    ...
    -Math.PI / 2,
    0.62,
    "Auction trade board with listing notices",
    "Player Services"
  ),
  P(
    "book_group_1",
    ...
    0.1,
    0.4,
    "Market ledger supported on auction clerk desk",
    "Player Services",
    GROUND_Y + 0.72
  ),
  P(
    ...
    0.2,
    0.26,
    "Auction listing fee coin supported on clerk desk",
    "Player Services",
    GROUND_Y + 0.74
  ),
  P(
    ...
    0,
    0.58,
    "Auction escrow lockbox on floor",
    "Player Services"
  ),
  ...
  P(
    ...
    -Math.PI / 2,
    0.42,
    "Guild creation charter book supported on registrar counter",
    "Player Services",
    GROUND_Y + 0.78
  ),
  P(
    ...
    -Math.PI / 2,
    0.36,
    "Guild charter parchment supported on registrar counter",
    "Player Services",
    GROUND_Y + 0.8
  ),
  P(
    ...
    -Math.PI / 2,
    0.58,
    "Guild registrar chair behind counter",
    "Player Services"
  ),
  ...
  P(
    ...
    -Math.PI / 2,
    0.62,
    "Cosmetic wardrobe mirror polished shield frame wall mounted",
    "Player Services",
    GROUND_Y + 0.86
  ),
  P(
    ...
    -Math.PI / 2,
    0.58,
    "Cosmetic wardrobe station cabinet",
    "Player Services"
  ),
  P(
    "chest_wood_fp",
  ...
  P(
    ...
    -Math.PI / 2,
    0.34,
    "Wardrobe sample cloak supported on outfit chest",
    "Player Services",
    GROUND_Y + 0.78
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.54,
    "Queue rope west of services plaza",
    "Player Services"
  ),
  P(
    "rope_2_fp",
    ...
    Math.PI / 2,
    0.54,
    "Queue rope east of services plaza",
    "Player Services"
  ),
  ...
  P(
    ...
    Math.PI,
    0.82,
    "Tool workbench against smith wall",
    "Craftsman Row"
  ),
  P(
    "workbench_fp",
  ...
  P(
    ...
    -Math.PI / 2,
    0.34,
    "Iron longsword displayed on weapon counter",
    "Craftsman Row",
    GROUND_Y + 0.82
  ),
  P(
    ...
    -Math.PI / 2,
    0.36,
    "Two-handed sword displayed on rack",
    "Craftsman Row",
    GROUND_Y + 0.92
  ),
  P(
    ...
    -Math.PI / 2,
    0.34,
    "Woodsman's axe displayed on rack",
    "Craftsman Row",
    GROUND_Y + 0.86
  ),
  P(
    ...
    -Math.PI / 2,
    0.32,
    "Training dagger displayed on counter",
    "Craftsman Row",
    GROUND_Y + 0.82
  ),
  P(
    ...
    -Math.PI / 2,
    0.4,
    "Round shield leaning beside weapon shop",
    "Craftsman Row",
    GROUND_Y + 0.45
  ),
  P(
    ...
    -Math.PI / 2,
    0.4,
    "Painted shield leaning beside weapon shop",
    "Craftsman Row",
    GROUND_Y + 0.45
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.78,
    "Bright forge fire glow supported in stone forge mouth",
    "Craftsman Row",
    GROUND_Y + 0.62
  ),
  P(
    ...
    0,
    0.54,
    "Coal block pile on smithy floor beside forge",
    "Craftsman Row"
  ),
  P(
    "mine_coal_piece",
    ...
    0,
    0.46,
    "Loose coal pieces on smithy floor beside forge",
    "Craftsman Row"
  ),
  P(
    "bucket_wood",
    ...
    0,
    0.6,
    "Water quench bucket on floor beside hot forge",
    "Craftsman Row"
  ),
  P(
    "barrel_fp",
    ...
    0,
    0.62,
    "Quench water barrel on floor beside forge",
    "Craftsman Row"
  ),
  P(
    "whetstone_fp",
    ...
    Math.PI / 2,
    0.62,
    "Whetstone repair station supported beside smith workbench",
    "Craftsman Row"
  ),
  P(
    "chain_coil",
    ...
    0,
    0.56,
    "Chain links and wagon rim repair coil on floor",
    "Craftsman Row"
  ),
  P(
    "pickaxe_bronze_fp",
    ...
    -Math.PI / 2,
    0.44,
    "Pickaxe repair job supported on forge assembly bench",
    "Craftsman Row",
    GROUND_Y + 0.84
  ),
  P(
    ...
    -Math.PI / 2,
    0.44,
    "Beginner blade work order supported on forge assembly bench",
    "Craftsman Row",
    GROUND_Y + 0.84
  ),
  P(
    ...
    -Math.PI / 2,
    0.44,
    "Axe head work order supported on forge assembly bench",
    "Craftsman Row",
    GROUND_Y + 0.84
  ),
  P(
    ...
    Math.PI,
    0.32,
    "Repair ledger supported on smith tool workbench",
    "Craftsman Row",
    GROUND_Y + 0.88
  ),
  P(
    ...
    0,
    0.32,
    "Blacksmith recipe stack supported on tool workbench",
    "Craftsman Row",
    GROUND_Y + 0.88
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.54,
    "Crafting order board for daily profession work orders",
    "Craftsman Row"
  ),
  P(
    "scroll_2_fp",
    ...
    0.1,
    0.28,
    "Pinned crafting order supported on work order board",
    "Craftsman Row",
    GROUND_Y + 0.88
  ),
  P(
    ...
    -0.1,
    0.28,
    "Pinned repair contract supported on work order board",
    "Craftsman Row",
    GROUND_Y + 0.88
  ),
  P(
    ...
    0,
    0.48,
    "Completed crafting orders drop crate on floor below board",
    "Craftsman Row"
  ),
  ...
  P(
    ...
    0,
    0.66,
    "Carpenter saw bench supported on workshop floor",
    "Craftsman Row"
  ),
  P(
    "logs",
    ...
    Math.PI / 2,
    0.62,
    "Cut lumber stack beside carpenter saw bench",
    "Craftsman Row"
  ),
  P(
    "pickaxe_bronze_fp",
    ...
    Math.PI / 2,
    0.34,
    "Carpenter measuring tool supported on saw bench",
    "Craftsman Row",
    GROUND_Y + 0.8
  ),
  P(
    ...
    0,
    0.28,
    "Carpentry order supported on saw bench",
    "Craftsman Row",
    GROUND_Y + 0.8
  ),
  ...
  P(
    ...
    0,
    0.24,
    "Folded brown cloth supported on tailoring table",
    "Craftsman Row",
    GROUND_Y + 0.82
  ),
  P(
    ...
    0,
    0.22,
    "Folded green cloth supported on tailoring table",
    "Craftsman Row",
    GROUND_Y + 0.84
  ),
  P(
    ...
    0.2,
    0.18,
    "Tailor recipe note supported on cloth table",
    "Craftsman Row",
    GROUND_Y + 0.85
  ),
  P(
    ...
    0,
    0.52,
    "Tailoring scrap bag on floor beside table",
    "Craftsman Row"
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.7,
    "Leather drying rack at workshop edge",
    "Craftsman Row"
  ),
  P(
    "banner_brown",
    ...
    Math.PI / 2,
    0.28,
    "Leather hide supported on drying rack",
    "Craftsman Row",
    GROUND_Y + 0.86
  ),
  P(
  ...
  P(
    ...
    0,
    0.56,
    "Alchemy prep side table for crafting reagents",
    "Craftsman Row"
  ),
  P(
    "small_bottles_1",
    ...
    0,
    0.32,
    "Small reagent bottles supported on alchemy prep table",
    "Craftsman Row",
    GROUND_Y + 0.75
  ),
  P(
    ...
    0,
    0.28,
    "Crafting solvent vial supported on alchemy prep table",
    "Craftsman Row",
    GROUND_Y + 0.75
  ),
  P(
    ...
    0,
    0.26,
    "Wax seal candle supported on alchemy prep table",
    "Craftsman Row",
    GROUND_Y + 0.75
  ),
  ...
  P(
    ...
    0,
    0.56,
    "Rare ore delivery crate for Black Anvil event",
    "Craftsman Row"
  ),
  P(
    "mine_silver_stone",
    ...
    0,
    0.38,
    "Rare ore sample supported on delivery crate",
    "Craftsman Row",
    GROUND_Y + 0.72
  ),
  P(
    ...
    Math.PI / 2,
    0.48,
    "Apprentice strike notice event anchor beside workshop",
    "Craftsman Row"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.46,
    "Stolen tools clue crate for Craftsman Row event",
    "Craftsman Row"
  ),
  P(
    "key_metal_fp",
    ...
    0,
    0.24,
    "Recovered tool key supported on stolen tools clue crate",
    "Craftsman Row",
    GROUND_Y + 0.66
  ),
  P(
    ...
    Math.PI / 2,
    0.34,
    "Crafting fair seasonal cloth marker above workshop lane",
    "Craftsman Row",
    GROUND_Y + 0.98
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.68,
    "Patient chair beside treatment table",
    "Apothecary"
  ),
  P(
    "shelf_small_bottles",
  ...
  P(
    ...
    -Math.PI / 2,
    0.72,
    "Candle remedy shelf fixed to wall",
    "Apothecary"
  ),
  ...
  P(
    ...
    0,
    0.76,
    "Water bucket beside treatment area",
    "Apothecary"
  ),
  P(
    "bookcase_2",
    ...
    Math.PI / 2,
    0.78,
    "Arcane bookcase against west wall",
    "Magic Shop"
  ),
  P(
    "bookcase_2",
    ...
    -Math.PI / 2,
    0.78,
    "Arcane bookcase against east wall",
    "Magic Shop"
  ),
  P(
    "table_long_decorated",
  ...
  P(
    ...
    Math.PI,
    0.76,
    "Ingredient shelf against north wall",
    "Magic Shop"
  ),
  ...
  P(
    ...
    0,
    0.85,
    "Bread supported on dining table",
    "Copper Kettle",
    GROUND_Y + 0.82
  ),
  ...
  P(
    ...
    0,
    0.58,
    "Copper kettle resting over hearth on floor stones",
    "Copper Kettle"
  ),
  P(
    "torch_mounted",
    ...
    Math.PI,
    0.66,
    "Wall-mounted hearth fire bracket",
    "Copper Kettle",
    GROUND_Y + 1.05
  ),
  P(
  ...
  P(
    ...
    0,
    0.62,
    "Main tavern chandelier hanging from ceiling beam",
    "Copper Kettle",
    GROUND_Y + 2.35
  ),
  P(
    ...
    Math.PI / 2,
    0.58,
    "Wall-mounted lantern by tavern room stairs",
    "Copper Kettle",
    GROUND_Y + 1.05
  ),
  P(
    ...
    -Math.PI / 2,
    0.58,
    "Wall-mounted lantern behind tavern bar",
    "Copper Kettle",
    GROUND_Y + 1.05
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.56,
    "Rumor board and group finder flavor board beside hearth",
    "Copper Kettle"
  ),
  P(
    "scroll_1_fp",
    ...
    0.1,
    0.28,
    "Pinned rumor note supported on rumor board shelf",
    "Copper Kettle",
    GROUND_Y + 0.88
  ),
  P(
    ...
    -0.1,
    0.28,
    "Group finder contract supported on rumor board shelf",
    "Copper Kettle",
    GROUND_Y + 0.88
  ),
  ...
  P(
    ...
    0,
    0.58,
    "Raised bard stage platform along tavern wall",
    "Copper Kettle",
    GROUND_Y + 0.1
  ),
  P(
    ...
    0,
    0.56,
    "Bard stool supported on stage platform",
    "Copper Kettle",
    GROUND_Y + 0.16
  ),
  P(
    ...
    Math.PI / 2,
    0.28,
    "Bard pipe prop supported on stage stool",
    "Copper Kettle",
    GROUND_Y + 0.64
  ),
  P(
    ...
    0,
    0.34,
    "Stage mug supported on bard platform",
    "Copper Kettle",
    GROUND_Y + 0.62
  ),
  ...
  P(
    ...
    0,
    0.44,
    "Plate supported on first dining table",
    "Copper Kettle",
    GROUND_Y + 0.83
  ),
  P(
    ...
    0.1,
    0.34,
    "Mug supported on first dining table",
    "Copper Kettle",
    GROUND_Y + 0.83
  ),
  P(
    ...
    0,
    0.52,
    "Cake supported on first dining table",
    "Copper Kettle",
    GROUND_Y + 0.83
  ),
  P(
    ...
    0.4,
    0.36,
    "Knife supported on first dining table",
    "Copper Kettle",
    GROUND_Y + 0.84
  ),
  ...
  P(
    ...
    0,
    0.44,
    "Plate supported on second dining table",
    "Copper Kettle",
    GROUND_Y + 0.83
  ),
  P(
    ...
    -0.2,
    0.34,
    "Full mug supported on second dining table",
    "Copper Kettle",
    GROUND_Y + 0.83
  ),
  P(
    ...
    0.1,
    0.52,
    "Bread loaf supported on second dining table",
    "Copper Kettle",
    GROUND_Y + 0.83
  ),
  P(
    ...
    -0.4,
    0.36,
    "Spoon supported on second dining table",
    "Copper Kettle",
    GROUND_Y + 0.84
  ),
  ...
  P(
    ...
    0,
    0.28,
    "Dice wager coin supported on dice table",
    "Copper Kettle",
    GROUND_Y + 0.72
  ),
  P(
    ...
    0.1,
    0.32,
    "Empty mug supported on dice table",
    "Copper Kettle",
    GROUND_Y + 0.72
  ),
  P(
    ...
    0,
    0.34,
    "House dice ledger supported on dice table",
    "Copper Kettle",
    GROUND_Y + 0.72
  ),
  ...
  P(
    ...
    0,
    0.56,
    "Rented room nightstand beside bed",
    "Copper Kettle"
  ),
  P(
    "candle_1_fp",
    ...
    0,
    0.28,
    "Rested XP candle supported on rented room nightstand",
    "Copper Kettle",
    GROUND_Y + 0.72
  ),
  P(
    ...
    Math.PI / 2,
    0.58,
    "Room rental storage chest on floor",
    "Copper Kettle"
  ),
  P(
    "banner_white",
    ...
    Math.PI / 2,
    0.32,
    "Bind point room rental cloth marker supported over chest",
    "Copper Kettle",
    GROUND_Y + 0.76
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.52,
    "Locked Copper Kettle cellar door for secret meetings",
    "Copper Kettle"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.52,
    "Cellar supply crate beside locked door",
    "Copper Kettle"
  ),
  P(
    "barrel_holder_fp",
    ...
    Math.PI / 2,
    0.56,
    "Cellar ale barrel holder against wall",
    "Copper Kettle"
  ),
  ...
  P(
    ...
    0,
    0.62,
    "Audience bench facing bard stage",
    "Copper Kettle"
  ),
  P(
    "stool_fp",
    ...
    0,
    0.54,
    "Loose tavern stool facing bard stage",
    "Copper Kettle"
  ),
  P(
    "chest_wood_fp",
    ...
    Math.PI,
    0.52,
    "Fugitive hiding chest beside back wall",
    "Copper Kettle"
  ),
  P(
    "scroll_1_fp",
    ...
    0,
    0.26,
    "Secret meeting note supported on fugitive chest",
    "Copper Kettle",
    GROUND_Y + 0.7
  ),
  ...
  P(
    ...
    Math.PI,
    0.88,
    "Supported Reeve Hall balcony fence",
    "Noble Rise",
    GROUND_Y + 1.7
  ),
  ...
  P(
    ...
    Math.PI,
    0.54,
    "Wall lamp mounted beside Reeve Hall balcony west",
    "Noble Rise",
    GROUND_Y + 1.16
  ),
  P(
    ...
    Math.PI,
    0.54,
    "Wall lamp mounted beside Reeve Hall balcony east",
    "Noble Rise",
    GROUND_Y + 1.16
  ),
  P(
    ...
    Math.PI,
    0.68,
    "Petition intake desk centered below Reeve Hall balcony",
    "Noble Rise"
  ),
  P(
    "scroll_1_fp",
    ...
    0.08,
    0.26,
    "Petition form supported on intake desk",
    "Noble Rise",
    GROUND_Y + 0.78
  ),
  P(
    ...
    0,
    0.22,
    "Permit seal key supported on petition desk",
    "Noble Rise",
    GROUND_Y + 0.78
  ),
  P(
    ...
    Math.PI / 2,
    0.66,
    "Permit clerk counter for charters and town papers",
    "Noble Rise"
  ),
  P(
    "scroll_2_fp",
    ...
    -0.1,
    0.26,
    "Town permit scroll supported on permit counter",
    "Noble Rise",
    GROUND_Y + 0.78
  ),
  P(
    ...
    0,
    0.28,
    "Permit code stack supported on permit counter",
    "Noble Rise",
    GROUND_Y + 0.8
  ),
  P(
    ...
    -Math.PI / 2,
    0.66,
    "Edrik Vane moneylender debt ledger table",
    "Noble Rise"
  ),
  P(
    "coin_pile",
    ...
    0,
    0.28,
    "Debt payment coins supported on moneylender table",
    "Noble Rise",
    GROUND_Y + 0.8
  ),
  P(
    ...
    0.18,
    0.26,
    "Debt note supported on Edrik Vane table",
    "Noble Rise",
    GROUND_Y + 0.8
  ),
  P(
    ...
    0,
    0.22,
    "Pawn lockbox key supported on moneylender table",
    "Noble Rise",
    GROUND_Y + 0.8
  ),
  P(
    ...
    0,
    0.46,
    "Moneylender secured pawn chest on floor beside table",
    "Noble Rise"
  ),
  P(
    "shelf_large",
    ...
    Math.PI,
    0.72,
    "Legal archive shelf against Reeve Hall rear wall",
    "Noble Rise"
  ),
  P(
    "book_group_1",
    ...
    0,
    0.28,
    "Town charter books supported on legal archive shelf",
    "Noble Rise",
    GROUND_Y + 1.18
  ),
  P(
    ...
    0,
    0.28,
    "Legal record books supported on legal archive shelf",
    "Noble Rise",
    GROUND_Y + 1.18
  ),
  P(
    ...
    Math.PI,
    0.46,
    "Court docket and audit notice board",
    "Noble Rise"
  ),
  P(
    "scroll_2_fp",
    ...
    0.12,
    0.24,
    "Pinned court docket supported on audit notice board",
    "Noble Rise",
    GROUND_Y + 0.86
  ),
  P(
    ...
    -0.1,
    0.24,
    "Distant lord audit notice supported on board",
    "Noble Rise",
    GROUND_Y + 0.86
  ),
  P(
    ...
    0,
    0.22,
    "Tax chest key supported on tax chest lid",
    "Noble Rise",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.22,
    "Loose tax coin supported on tax chest lid",
    "Noble Rise",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0.1,
    0.26,
    "Tax assessment supported on tax clerk desk",
    "Noble Rise",
    GROUND_Y + 0.82
  ),
  P(
    ...
    0,
    0.3,
    "Merchant Compact ledger stack supported on decorated desk",
    "Noble Rise",
    GROUND_Y + 0.82
  ),
  P(
    ...
    0,
    0.24,
    "Masked party invitation cloth supported on compact desk",
    "Noble Rise",
    GROUND_Y + 0.84
  ),
  P(
    ...
    0,
    0.58,
    "Petitioner waiting bench inside Reeve Hall",
    "Noble Rise"
  ),
  P(
    "bench_fp",
    ...
    0,
    0.58,
    "Merchant Compact waiting bench inside Reeve Hall",
    "Noble Rise"
  ),
  P(
    "shield_round_color",
    ...
    Math.PI / 2,
    0.36,
    "Elite guard ceremonial shield mounted near entrance",
    "Noble Rise",
    GROUND_Y + 0.9
  ),
  P(
    ...
    Math.PI / 2,
    0.34,
    "Elite guard ceremonial blade mounted near entrance",
    "Noble Rise",
    GROUND_Y + 0.88
  ),
  ...
  P(
    ...
    -Math.PI / 2,
    0.82,
    "Guard notice banner mounted on yard wall bracket",
    "Guard Yard",
    GROUND_Y + 1.1
  ),
  P(
  ...
  P(
    ...
    Math.PI,
    0.46,
    "Guard Yard combat training bounty board wayfinding sign",
    "Guard Yard"
  ),
  P(
    "scroll_1_fp",
    ...
    0.08,
    0.24,
    "Training notice supported on Guard Yard sign",
    "Guard Yard",
    GROUND_Y + 0.86
  ),
  P(
    ...
    -0.08,
    0.24,
    "Bounty notice supported on Guard Yard sign",
    "Guard Yard",
    GROUND_Y + 0.84
  ),
  P(
    ...
    Math.PI / 2,
    0.74,
    "Packed-earth training lane plank marker",
    "Guard Yard"
  ),
  P(
    "dummy_fp",
    ...
    0,
    0.82,
    "Forward-arc melee training dummy left",
    "Guard Yard"
  ),
  P(
    "dummy_fp",
    ...
    0,
    0.86,
    "Forward-arc melee training dummy center",
    "Guard Yard"
  ),
  P(
    "dummy_fp",
    ...
    0,
    0.82,
    "Forward-arc melee training dummy right",
    "Guard Yard"
  ),
  P(
    "fence",
    ...
    Math.PI / 2,
    0.52,
    "Training lane west boundary fence",
    "Guard Yard"
  ),
  P(
    "fence",
    ...
    Math.PI / 2,
    0.52,
    "Training lane east boundary fence",
    "Guard Yard"
  ),
  P(
    "table_medium",
  ...
  P(
    ...
    0.12,
    0.28,
    "Wolf bounty warrant supported on bounty desk",
    "Guard Yard",
    GROUND_Y + 0.76
  ),
  P(
    ...
    -0.1,
    0.28,
    "Bandit bounty warrant supported on bounty desk",
    "Guard Yard",
    GROUND_Y + 0.76
  ),
  P(
    ...
    0,
    0.24,
    "Bounty reward coins supported on warrant desk",
    "Guard Yard",
    GROUND_Y + 0.76
  ),
  P(
    ...
    Math.PI / 2,
    0.4,
    "Bounty board patrol dailies and road warrants",
    "Guard Yard"
  ),
  P(
    "scroll_2_fp",
    ...
    0.12,
    0.22,
    "Pinned road warrant supported on bounty board",
    "Guard Yard",
    GROUND_Y + 0.78
  ),
  P(
    ...
    Math.PI / 2,
    0.82,
    "Prisoner holding cage for escape event",
    "Guard Yard"
  ),
  P(
    "key_metal_fp",
    ...
    0,
    0.2,
    "Jail key supported on prisoner cage hook",
    "Guard Yard",
    GROUND_Y + 0.74
  ),
  P(
    ...
    Math.PI / 2,
    0.42,
    "Confiscated prisoner gear chest on floor",
    "Guard Yard"
  ),
  P(
    "scroll_1_fp",
    ...
    0.08,
    0.2,
    "Prisoner transfer note supported on gear chest",
    "Guard Yard",
    GROUND_Y + 0.56
  ),
  P(
    ...
    0,
    0.56,
    "Dueling ring north rope marker",
    "Guard Yard",
    GROUND_Y + 0.16
  ),
  P(
    ...
    0,
    0.56,
    "Dueling ring south rope marker",
    "Guard Yard",
    GROUND_Y + 0.16
  ),
  P(
  ...
  P(
    ...
    -Math.PI / 2,
    0.38,
    "Dueling ring PvP opt-in warning notice",
    "Guard Yard"
  ),
  P(
    "scroll_1_fp",
    ...
    -0.1,
    0.22,
    "PvP opt-in warning supported on duel sign",
    "Guard Yard",
    GROUND_Y + 0.78
  ),
  P(
    ...
    -Math.PI / 2,
    0.42,
    "Guard Yard alarm bell invasion rally point",
    "Guard Yard",
    GROUND_Y + 1.65
  ),
  P(
    ...
    Math.PI,
    0.52,
    "Guard Yard watchtower lookout silhouette",
    "Guard Yard"
  ),
  P(
    "arch_stairs_stone",
    ...
    Math.PI,
    0.48,
    "Watchtower stair access for guard roof repair",
    "Guard Yard"
  ),
  P(
    "banner_red",
    ...
    Math.PI,
    0.44,
    "Red watch banner mounted on Guard Yard tower",
    "Guard Yard",
    GROUND_Y + 1.2
  ),
  P(
    ...
    0,
    0.48,
    "Town defense shield barrel stack at yard edge",
    "Guard Yard"
  ),
  P(
    "shield_square_color",
    ...
    0.2,
    0.34,
    "Defense shield supported on barrel stack",
    "Guard Yard",
    GROUND_Y + 0.66
  ),
  P(
    ...
    0,
    0.42,
    "Barricade repair kit crate on floor",
    "Guard Yard"
  ),
  P(
    "rope_2_fp",
    ...
    Math.PI / 2,
    0.34,
    "Barricade rope supported on repair kit crate",
    "Guard Yard",
    GROUND_Y + 0.58
  ),
  P(
    ...
    -Math.PI / 2,
    0.62,
    "Quartermaster armory issue counter",
    "Guard Yard"
  ),
  P(
    "sword_bronze_fp",
    ...
    Math.PI / 2,
    0.3,
    "Starter sword supported on quartermaster counter",
    "Guard Yard",
    GROUND_Y + 0.74
  ),
  P(
    ...
    0,
    0.3,
    "Training shield supported on quartermaster counter",
    "Guard Yard",
    GROUND_Y + 0.74
  ),
  P(
    ...
    0,
    0.28,
    "Quartermaster issue ledger supported on counter",
    "Guard Yard",
    GROUND_Y + 0.74
  ),
  P(
    ...
    -Math.PI / 2,
    0.36,
    "Guard inspection roster and drill schedule",
    "Guard Yard"
  ),
  P(
    "scroll_2_fp",
    ...
    -0.08,
    0.2,
    "Pinned inspection roster supported on drill board",
    "Guard Yard",
    GROUND_Y + 0.76
  ),
  P(
    ...
    Math.PI,
    0.42,
    "Prisoner escape event red alert banner",
    "Guard Yard",
    GROUND_Y + 1.02
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.64,
    "Dockmaster ledger booth counter",
    "River Docks"
  ),
  P(
    "book_stack_1",
    ...
    0.05,
    0.32,
    "Dock ledger supported on dockmaster counter",
    "River Docks",
    GROUND_Y + 0.74
  ),
  P(
    ...
    0,
    0.24,
    "Warehouse key supported on dockmaster counter",
    "River Docks",
    GROUND_Y + 0.74
  ),
  P(
    ...
    0.16,
    0.28,
    "Cargo manifest supported on dockmaster counter",
    "River Docks",
    GROUND_Y + 0.74
  ),
  P(
    ...
    Math.PI / 2,
    0.54,
    "Dock toll lockbox on floor beside ledger booth",
    "River Docks"
  ),
  P(
    "table_long",
    ...
    Math.PI / 2,
    0.62,
    "Fish market cleaning table beside pier",
    "River Docks"
  ),
  P(
    "food_fish",
    ...
    Math.PI / 2,
    0.5,
    "Fresh river fish supported on fish market table",
    "River Docks",
    GROUND_Y + 0.73
  ),
  P(
    ...
    -0.2,
    0.42,
    "Fishbone scraps supported on fish market table",
    "River Docks",
    GROUND_Y + 0.74
  ),
  P(
    ...
    Math.PI / 2,
    0.32,
    "Fillet knife supported on fish market table",
    "River Docks",
    GROUND_Y + 0.75
  ),
  P(
    ...
    0,
    0.58,
    "Fish guts bucket on floor beside cleaning table",
    "River Docks"
  ),
  P(
    "barrel_fp",
    ...
    0,
    0.54,
    "Salt barrel beside fish market table",
    "River Docks"
  ),
  P(
    "obj_sign_post",
    ...
    -Math.PI / 2,
    0.44,
    "Fishing trainer and river goods vendor board",
    "River Docks"
  ),
  P(
    "rope_1_fp",
    ...
    0,
    0.52,
    "Net rope coil beside fish market",
    "River Docks"
  ),
  P(
    "crate_metal_fp",
    ...
    0,
    0.58,
    "Customs inspection crate on floor",
    "River Docks"
  ),
  P(
    "scroll_2_fp",
    ...
    0.2,
    0.25,
    "Customs seal supported on inspection crate",
    "River Docks",
    GROUND_Y + 0.66
  ),
  P(
  ...
  P(
    ...
    0,
    0.52,
    "River ale and lamp-oil barrel stack",
    "River Docks"
  ),
  P(
    "rope_2_fp",
    ...
    Math.PI / 2,
    0.52,
    "Blue rope marking inspected cargo lane",
    "River Docks"
  ),
  P(
    "table_small",
    ...
    -Math.PI / 2,
    0.56,
    "Smuggler contact marker table behind cargo",
    "River Docks"
  ),
  P(
    "small_bottle_fp",
    ...
    0,
    0.3,
    "Smuggler signal bottle supported on contact table",
    "River Docks",
    GROUND_Y + 0.68
  ),
  P(
    ...
    -0.2,
    0.24,
    "River Knots coded note supported on contact table",
    "River Docks",
    GROUND_Y + 0.68
  ),
  P(
    ...
    0,
    0.48,
    "Contraband false-bottom crate tucked behind cargo",
    "River Docks"
  ),
  P(
    "key_metal_fp",
    ...
    0,
    0.22,
    "Contraband key supported on false-bottom crate",
    "River Docks",
    GROUND_Y + 0.62
  ),
  P(
    ...
    0,
    0.48,
    "Dock fire pitch barrel event anchor",
    "River Docks"
  ),
  P(
    "obj_sign_post",
    ...
    -Math.PI / 2,
    0.44,
    "Dock fire bucket-line event notice",
    "River Docks"
  ),
  P(
    "bucket_wood",
    ...
    0,
    0.48,
    "Fire bucket on floor beside event notice",
    "River Docks"
  ),
  P(
    "obj_sign_post",
    ...
    Math.PI,
    0.44,
    "Flood warning and river beast sighting board",
    "River Docks"
  ),
  P(
    "scroll_2_fp",
    ...
    0.12,
    0.24,
    "River beast warning supported on flood board",
    "River Docks",
    GROUND_Y + 0.84
  ),
  P(
    ...
    0,
    0.46,
    "Flood rescue rope crate on floor",
    "River Docks"
  ),
  P(
    "rope_3_fp",
    ...
    Math.PI / 2,
    0.42,
    "Rescue rope supported on flood rescue crate",
    "River Docks",
    GROUND_Y + 0.62
  ),
  P(
    ...
    0,
    0.42,
    "Corpse-under-bridge clue box near shadowed pier",
    "River Docks"
  ),
  P(
    "food_fishbone",
    ...
    0,
    0.28,
    "Riverbone clue supported on corpse-under-bridge box",
    "River Docks",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.44,
    "Low lantern marking corpse-under-bridge clue",
    "River Docks",
    GROUND_Y + 0.7
  ),
  ...
  P(
    ...
    0,
    0.26,
    "Cheap healer fever tonic supported on table",
    "Mudden Ward",
    GROUND_Y + 0.66
  ),
  P(
    ...
    0.1,
    0.28,
    "Cheap healer green bottle supported on table",
    "Mudden Ward",
    GROUND_Y + 0.66
  ),
  P(
    ...
    0,
    0.42,
    "Bandage sack on floor beside cheap healer",
    "Mudden Ward"
  ),
  P(
    "bed_twin2",
    ...
    Math.PI / 2,
    0.42,
    "Sick traveler cot in Mudden cheap healer corner",
    "Mudden Ward"
  ),
  P(
    "table_small",
    ...
    Math.PI / 2,
    0.48,
    "Fence vendor hidden deal table",
    "Mudden Ward"
  ),
  P(
    "coin_pile",
    ...
    0,
    0.22,
    "Fence vendor coin pile supported on hidden table",
    "Mudden Ward",
    GROUND_Y + 0.64
  ),
  P(
    ...
    Math.PI / 2,
    0.3,
    "Illegal blade supported on fence vendor table",
    "Mudden Ward",
    GROUND_Y + 0.66
  ),
  P(
    ...
    -0.2,
    0.2,
    "Stolen key supported on fence vendor table",
    "Mudden Ward",
    GROUND_Y + 0.65
  ),
  P(
  ...
  P(
    ...
    0,
    0.46,
    "Rat-catcher live cage on floor beside wall",
    "Mudden Ward"
  ),
  P(
    "cage_small_fp",
    ...
    0.2,
    0.4,
    "Second rat-catcher cage beside drain",
    "Mudden Ward"
  ),
  P(
    "cheese",
    ...
    0,
    0.28,
    "Rat bait cheese on floor near trap",
    "Mudden Ward"
  ),
  P(
    "bucket_wood",
    ...
    0,
    0.44,
    "Washerwoman rinse bucket on floor",
    "Mudden Ward"
  ),
  P(
    "barrel_small",
    ...
    0,
    0.5,
    "Washerwoman rain barrel on floor",
    "Mudden Ward"
  ),
  P(
    "table_small",
  ...
  P(
    ...
    0,
    0.34,
    "Folded laundry cloth supported on table",
    "Mudden Ward",
    GROUND_Y + 0.64
  ),
  P(
    ...
    0,
    0.42,
    "Orphan blanket bundle on floor",
    "Mudden Ward"
  ),
  P(
    "bread_slice",
    ...
    0,
    0.3,
    "Half meal beside orphan blanket",
    "Mudden Ward",
    GROUND_Y + 0.1
  ),
  P(
    ...
    0.1,
    0.36,
    "Mudden Kin reputation stash chest",
    "Mudden Ward"
  ),
  P(
    "scroll_1_fp",
    ...
    -0.1,
    0.2,
    "Mudden Kin favor list supported on stash chest",
    "Mudden Ward",
    GROUND_Y + 0.54
  ),
  P(
    ...
    Math.PI,
    0.38,
    "Mudden Kin hidden reputation vendor mark",
    "Mudden Ward"
  ),
  P(
    "barrel_stack",
    ...
    0,
    0.42,
    "Flood rescue barrel stack kept against wall",
    "Mudden Ward"
  ),
  P(
    "rope_3_fp",
    ...
    Math.PI / 2,
    0.36,
    "Flood rescue rope supported on barrel stack",
    "Mudden Ward",
    GROUND_Y + 0.62
  ),
  P(
    ...
    0,
    0.36,
    "Flood bucket-line bucket on floor",
    "Mudden Ward"
  ),
  P(
    "obj_sign_post",
  ...
  P(
    ...
    Math.PI / 2,
    0.36,
    "Witch accusation and missing children notice",
    "Mudden Ward"
  ),
  P(
    "scroll_2_fp",
    ...
    0.12,
    0.2,
    "Missing children notice supported on accusation board",
    "Mudden Ward",
    GROUND_Y + 0.76
  ),
  P(
    ...
    0,
    0.28,
    "Small vigil candle beside missing children notice",
    "Mudden Ward"
  ),
  P(
    "box_stacked",
    ...
    0,
    0.4,
    "Stealth trainer crate step to back route",
    "Mudden Ward"
  ),
  P(
    "rope_1_fp",
    ...
    Math.PI / 2,
    0.36,
    "Back-route rope showing climbable shortcut",
    "Mudden Ward",
    GROUND_Y + 0.62
  ),
  P(
  ...
  P(
    ...
    Math.PI / 2,
    0.76,
    "Mounted Underways warning torch",
    "Old Well",
    GROUND_Y + 1.0
  ),
  ...
  P(
    ...
    0,
    0.46,
    "Old Well cracked circular stone ring landmark",
    "Old Well / Underways"
  ),
  P(
    "obj_church_trapdoor_metal",
    ...
    0,
    0.42,
    "Old Well iron grate covering barred shaft",
    "Old Well / Underways",
    GROUND_Y + 0.08
  ),
  P(
    ...
    Math.PI / 2,
    0.34,
    "Rusty chain wrapped around Old Well grate",
    "Old Well / Underways",
    GROUND_Y + 0.24
  ),
  P(
    ...
    0,
    0.34,
    "North iron bar fence around Old Well shaft",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_fence",
    ...
    Math.PI,
    0.34,
    "South iron bar fence around Old Well shaft",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_fence",
    ...
    Math.PI / 2,
    0.34,
    "West iron bar fence around Old Well shaft",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_fence",
    ...
    -Math.PI / 2,
    0.34,
    "East iron bar fence around Old Well shaft",
    "Old Well / Underways"
  ),
  P(
    "rock_small",
    ...
    0.2,
    0.42,
    "Mossy displaced stone from cracked Old Well rim",
    "Old Well / Underways"
  ),
  P(
    "forest_rock_1a",
    ...
    -0.4,
    0.38,
    "Moss-grown stone beside Old Well grate",
    "Old Well / Underways"
  ),
  P(
    "pillar",
    ...
    0,
    0.54,
    "Old carved Harthmere drain pillar",
    "Old Well / Underways"
  ),
  P(
    "scroll_1_fp",
    ...
    0.1,
    0.18,
    "Rubbing of old drain carvings pinned to pillar",
    "Old Well / Underways",
    GROUND_Y + 0.8
  ),
  P(
    ...
    0.2,
    0.22,
    "Bronze bell fragment faintly ringing beside Old Well",
    "Old Well / Underways",
    GROUND_Y + 0.18
  ),
  P(
    ...
    -0.2,
    0.18,
    "Second bronze bell shard half-buried near drain marker",
    "Old Well / Underways",
    GROUND_Y + 0.12
  ),
  P(
    ...
    0,
    0.24,
    "Green-tinged candle at whispering well edge",
    "Old Well / Underways"
  ),
  P(
    "candle_thin_lit",
    ...
    0,
    0.24,
    "Green-tinged candle at sealed drain edge",
    "Old Well / Underways"
  ),
  P(
    "torch_mounted",
  ...
  P(
    ...
    Math.PI / 2,
    0.42,
    "Hidden drain stair revealed by Old Well quest phase",
    "Old Well / Underways"
  ),
  P(
    "obj_church_crypt",
    ...
    Math.PI / 2,
    0.32,
    "Underways dungeon entrance proxy behind old drain stair",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    Math.PI / 2,
    0.34,
    "Wet stone boundary wall at Underways breach",
    "Old Well / Underways"
  ),
  P(
    "arch_wall_broken",
    ...
    Math.PI / 2,
    0.32,
    "Drain breach broken masonry event anchor",
    "Old Well / Underways"
  ),
  P(
    "torch_lit",
    ...
    Math.PI / 2,
    0.34,
    "Exit breadcrumb torch inside Underways breach mounted on Underways breach wall bracket",
    "Old Well / Underways",
    GROUND_Y + 0.92
  ),
  P(
    ...
    Math.PI,
    0.32,
    "Hidden map marker sign unlocked after Old Well discovery",
    "Old Well / Underways"
  ),
  P(
    "scroll_2_fp",
    ...
    0.1,
    0.18,
    "Old Well warning note supported on discovery sign",
    "Old Well / Underways",
    GROUND_Y + 0.7
  ),
  P(
    ...
    Math.PI / 2,
    0.3,
    "Temple Green clue board pointing to missing bell underways",
    "Old Well / Underways"
  ),
  P(
    "scroll_1_fp",
    ...
    -0.1,
    0.16,
    "Father Aldren chapel rubbing supported on clue board",
    "Old Well / Underways",
    GROUND_Y + 0.68
  ),
  P(
    ...
    0.2,
    0.34,
    "Safety rope tied beside hidden drain stair",
    "Old Well / Underways",
    GROUND_Y + 0.28
  ),
  P(
    ...
    0,
    0.32,
    "Old bucket lowered into whispering well",
    "Old Well / Underways"
  ),
  P(
    "book_stack_1",
    ...
    0.2,
    0.22,
    "Lore notes from old archive supported on stone ledge",
    "Old Well / Underways",
    GROUND_Y + 0.34
  ),
  P(
    ...
    -0.1,
    0.2,
    "Investigation candle cluster supported beside lore notes",
    "Old Well / Underways",
    GROUND_Y + 0.38
  ),
  P(
    ...
    Math.PI / 2,
    0.28,
    "Damp cloth marker for Underways exit breadcrumb",
    "Old Well / Underways",
    GROUND_Y + 0.76
  ),
  P(
    ...
    Math.PI / 2,
    0.42,
    "Mudden tunnel connection grate tied to Old Well Underways",
    "Mudden Ward",
    GROUND_Y + 0.04
  ),
  P(
    ...
    Math.PI / 2,
    0.28,
    "Matching green torch at Mudden Underways connection mounted on Mudden Underways wall bracket",
    "Mudden Ward",
    GROUND_Y + 0.9
  ),
  P(
    ...
    0,
    0.28,
    "Temple vigil candle aligned with Old Well missing bell clue",
    "Temple Green"
  ),
  P(
    "scroll_2_fp",
    ...
    0.1,
    0.18,
    "Temple prayer note naming the Old Well resting on supported chapel lectern",
    "Temple Green",
    GROUND_Y + 0.26
  ),
  P(
    ...
    Math.PI,
    0.3,
    "Child dare event warning near Old Well",
    "Old Well / Underways"
  ),
  P(
    "scroll_1_fp",
    ...
    0.04,
    0.16,
    "Child dare note supported on Old Well warning post",
    "Old Well / Underways",
    GROUND_Y + 0.68
  ),
  P(
    ...
    0,
    0.22,
    "Night bell event candle at Old Well",
    "Old Well / Underways"
  ),
  P(
    "candle_lit",
    ...
    0,
    0.22,
    "Second night bell event candle at Old Well",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    0,
    0.32,
    "Phase-safe collapsed stone marker for undead emergence",
    "Old Well / Underways"
  ),
  P(
    "coffin",
    ...
    -0.2,
    0.28,
    "Sealed coffin prop for ancient spirit emergence event",
    "Old Well / Underways"
  ),
  ...
  P(
    ...
    Math.PI,
    0.42,
    "Clothes cabinet against cottage side wall clear of interior aisle",
    "Residential District"
  ),
  P(
    "bookcase_2",
    ...
    Math.PI,
    0.78,
    "Family bookcase against north wall",
    "Residential District"
  ),
  P(
    "bench_fp",
  ...
  P(
    ...
    Math.PI,
    0.24,
    "Family supper candle cluster supported on cottage shelf",
    "Residential District",
    GROUND_Y + 0.72
  ),
  P(
  ...
  A(
    ...
    Math.PI,
    0.9,
    "Roadside cottage resident returning from market",
    "Residential District"
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.9,
    "River watermill near docks set back from service cart lane",
    "River Docks"
  ),
  ...
  P(
    ...
    0.2,
    0.78,
    "Gathering node: North Road iron vein",
    "Gathering"
  ),
  P(
    "pickaxe_bronze_fp",
    ...
    -0.4,
    0.58,
    "Mining tool marker beside iron vein",
    "Gathering"
  ),
  P(
    "logs",
    ...
    Math.PI / 2,
    0.84,
    "Gathering node: orchard fallen branches",
    "Gathering"
  ),
  P(
    "rock_small",
    ...
    0,
    0.5,
    "Gathering marker beside resin log",
    "Gathering"
  ),
  ...
  P(
    ...
    0.2,
    0.78,
    "Gathering node: Mudden Ward scrap pile",
    "Gathering"
  ),
  ...
  P(
    ...
    0,
    0.78,
    "Building plot: West Lane cottage crate",
    "Building"
  ),
  ...
  P(
    ...
    0,
    0.58,
    "Watchtower repair torch marker",
    "Building"
  ),
  P(
    "crate_wooden_fp",
  ...
  P(
    ...
    Math.PI,
    0.42,
    "Halene letter archive bookcase against east chapel office wall",
    "Temple Green"
  ),
  P(
    "cabinet",
    ...
    Math.PI,
    0.38,
    "Iron-bound Halene letter cabinet against east chapel office wall",
    "Temple Green"
  ),
  P(
    "table_small",
    ...
    Math.PI,
    0.38,
    "Sister's Letters reading table inside east chapel archive",
    "Temple Green"
  ),
  P(
    "scroll_2_fp",
    ...
    0.12,
    0.16,
    "Halene Letter Seven supported on chapel archive reading table",
    "Temple Green",
    GROUND_Y + 0.58
  ),
  P(
    ...
    -0.08,
    0.16,
    "Verenine Annals floor-map volume supported on chapel archive reading table",
    "Temple Green",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.16,
    "Archive witness candle cluster supported on chapel reading table",
    "Temple Green",
    GROUND_Y + 0.61
  ),
  P(
    ...
    0,
    0.38,
    "Phase-safe chapel cellar trapdoor under altar leading to hidden door",
    "Temple Green",
    GROUND_Y + 0.04
  ),
  P(
    ...
    0,
    0.22,
    "Iron chain coiled on floor beside buried bell pit",
    "Temple Green",
    GROUND_Y + 0.12
  ),
  P(
    ...
    Math.PI,
    0.18,
    "Buried bell fragment supported in iron-lined altar pit",
    "Temple Green",
    GROUND_Y + 0.18
  ),
  P(
    ...
    Math.PI / 2,
    0.28,
    "Phase-safe broken brick wall revealing chapel hidden door",
    "Temple Green"
  ),
  P(
    "obj_church_trapdoor_metal",
    ...
    Math.PI / 2,
    0.28,
    "Bronze listening plate mounted on hidden undercroft door",
    "Temple Green",
    GROUND_Y + 0.1
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.3,
    "Bridge crack planning table against Reeve Hall side wall clear of civic aisle",
    "Noble Rise"
  ),
  P(
    "scroll_2_fp",
    ...
    0.08,
    0.18,
    "Bell-shaped bridge crack parchment supported on Reeve planning table",
    "Noble Rise",
    GROUND_Y + 0.62
  ),
  P(
    ...
    -0.2,
    0.2,
    "Civic bridge repair ledger supported on Reeve planning table",
    "Noble Rise",
    GROUND_Y + 0.65
  ),
  P(
    ...
    0,
    0.16,
    "Small council candle supported on bridge crack table",
    "Noble Rise",
    GROUND_Y + 0.66
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.36,
    "Bell-casting anvil block on floor in Black Anvil back bay",
    "Craftsman Row"
  ),
  P(
    "cauldron_fp",
    ...
    0,
    0.26,
    "Bell-bronze alloy crucible tucked beside Black Anvil back bay wall",
    "Craftsman Row"
  ),
  P(
    "bucket_wood",
    ...
    0,
    0.32,
    "Quench bucket on floor beside bell-casting bay",
    "Craftsman Row"
  ),
  P(
    "workbench_drawers_fp",
    ...
    Math.PI,
    0.36,
    "Bellbinder tuning workbench against smithy back wall",
    "Craftsman Row"
  ),
  P(
    "mine_gold_fragment",
    ...
    0.1,
    0.18,
    "Meteoric trace shard supported on tuning workbench",
    "Craftsman Row",
    GROUND_Y + 0.64
  ),
  P(
    ...
    0.05,
    0.14,
    "Bellbinder's Voice handbell supported on tuning workbench",
    "Craftsman Row",
    GROUND_Y + 0.68
  ),
  P(
    ...
    -0.1,
    0.18,
    "Tone-shaping whetstone supported on tuning workbench",
    "Craftsman Row",
    GROUND_Y + 0.64
  ),
  ...
  P(
    ...
    Math.PI,
    0.36,
    "Wyrm and Candle Bellbinder shelf against magic shop wall",
    "Magic Shop"
  ),
  P(
    "spellbook_open",
    ...
    0,
    0.18,
    "Open bell-tone spellbook supported on Wyrm and Candle shelf",
    "Magic Shop",
    GROUND_Y + 0.72
  ),
  P(
    ...
    0,
    0.26,
    "Candle shelf against Wyrm and Candle side wall",
    "Magic Shop",
    GROUND_Y + 0.74
  ),
  P(
    ...
    Math.PI / 2,
    0.16,
    "Tuning wand supported on magic shop wall shelf",
    "Magic Shop",
    GROUND_Y + 0.78
  ),
  ...
  P(
    ...
    -Math.PI / 2,
    0.38,
    "Dock Ledger Warehouse cargo-contract table against inner wall",
    "River Docks"
  ),
  P(
    "scroll_2_fp",
    ...
    0.1,
    0.18,
    "Ferry schedule and sealed cargo contract supported on warehouse table",
    "River Docks",
    GROUND_Y + 0.62
  ),
  P(
    ...
    -0.05,
    0.16,
    "Warehouse lock key supported on cargo-contract table",
    "River Docks",
    GROUND_Y + 0.64
  ),
  ...
  P(
    ...
    0,
    0.72,
    "Phase-safe underground Bellward Halls bronze floor ring path",
    "Old Well / Underways"
  ),
  P(
    "pillar",
    ...
    0,
    0.42,
    "Bellward Halls central pillar on floor inside solo dungeon hub",
    "Old Well / Underways"
  ),
  P(
    "obj_church_bells",
    ...
    Math.PI,
    0.24,
    "Bell-pattern bronze inlay supported in Bellward Halls central hub floor",
    "Old Well / Underways",
    GROUND_Y + 0.08
  ),
  P(
    ...
    Math.PI / 2,
    0.28,
    "Bellward Halls west wall torch mounted on stone bracket",
    "Old Well / Underways",
    GROUND_Y + 1.08
  ),
  P(
    ...
    -Math.PI / 2,
    0.28,
    "Bellward Halls east wall torch mounted on stone bracket",
    "Old Well / Underways",
    GROUND_Y + 1.08
  ),
  P(
    ...
    0,
    0.28,
    "Phase-safe Chamber of Aevith river-wyrm altar wall",
    "Old Well / Underways"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.18,
    "Aevith prayer chamber lore stand supported on floor",
    "Old Well / Underways"
  ),
  P(
    "rock_small",
    ...
    0.2,
    0.18,
    "Aevith river stone relic on floor beside lore stand",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    0,
    0.28,
    "Phase-safe Chamber of Karag-Drath mountain-wyrm altar wall",
    "Old Well / Underways"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.18,
    "Karag-Drath prayer chamber lore stand supported on floor",
    "Old Well / Underways"
  ),
  P(
    "mine_stone_01",
    ...
    0.2,
    0.18,
    "Karag-Drath black mountain glass relic on floor",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    0,
    0.28,
    "Phase-safe Chamber of Vyrenia sky-wyrm altar wall",
    "Old Well / Underways"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.18,
    "Vyrenia prayer chamber lore stand supported on floor",
    "Old Well / Underways"
  ),
  P(
    "banner_white",
    ...
    0,
    0.22,
    "White feather relic cloth supported over Vyrenia chamber altar",
    "Old Well / Underways",
    GROUND_Y + 0.44
  ),
  P(
    ...
    0,
    0.28,
    "Phase-safe Chamber of Murvath sea-wyrm altar wall",
    "Old Well / Underways"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.18,
    "Murvath prayer chamber lore stand supported on floor",
    "Old Well / Underways"
  ),
  P(
    "food_fishbone",
    ...
    0,
    0.16,
    "Murvath shell relic supported beside sea-wyrm altar",
    "Old Well / Underways",
    GROUND_Y + 0.22
  ),
  P(
    ...
    0,
    0.26,
    "Phase-safe Chamber of Sylenne forest-wyrm root altar wall",
    "Old Well / Underways"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.16,
    "Sylenne prayer chamber lore stand supported on floor",
    "Old Well / Underways"
  ),
  P(
    "forest_grass_2d",
    ...
    0,
    0.16,
    "Sylenne living leaf relic on floor beside root altar",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    0,
    0.26,
    "Phase-safe Chamber of Korruthax fire-wyrm altar wall",
    "Old Well / Underways"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.16,
    "Korruthax prayer chamber lore stand supported on floor",
    "Old Well / Underways"
  ),
  P(
    "mine_gold_fragment",
    ...
    0,
    0.16,
    "Korruthax obsidian arrow relic supported beside fire altar",
    "Old Well / Underways",
    GROUND_Y + 0.2
  ),
  P(
    ...
    0,
    0.28,
    "Listening Chamber stone bench on floor beside blue shaft",
    "Old Well / Underways"
  ),
  P(
    "fountain_center",
    ...
    0,
    0.18,
    "Listening Chamber blue light shaft set into floor",
    "Old Well / Underways",
    GROUND_Y + 0.04
  ),
  P(
    ...
    0,
    0.14,
    "Blue listening candle supported at floor shaft rim",
    "Old Well / Underways",
    GROUND_Y + 0.18
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.38,
    "Old Harth antechamber sarcophagus on floor",
    "Old Well / Underways"
  ),
  P(
    "pillar",
    ...
    0,
    0.32,
    "Memory Stone suspended over Old Harth sarcophagus by chain anchors",
    "Old Well / Underways",
    GROUND_Y + 0.86
  ),
  P(
    ...
    0,
    0.18,
    "First Memory Stone chain anchor on floor",
    "Old Well / Underways",
    GROUND_Y + 0.12
  ),
  P(
    ...
    0,
    0.18,
    "Second Memory Stone chain anchor on floor",
    "Old Well / Underways",
    GROUND_Y + 0.12
  ),
  P(
    ...
    0,
    0.22,
    "Pulse Hall dragon-vein glow supported in floor channel",
    "Old Well / Underways",
    GROUND_Y + 0.12
  ),
  P(
    ...
    0.1,
    0.22,
    "Second Pulse Hall dragon-vein glow supported in floor channel",
    "Old Well / Underways",
    GROUND_Y + 0.12
  ),
  P(
    ...
    0,
    0.24,
    "Echo Hall phase-safe essence pool set into floor",
    "Old Well / Underways",
    GROUND_Y + 0.04
  ),
  AD(
    ...
    "The Echo-Singer",
    "Old Well / Underways",
    { radius: 0.45, speed: 0.055, phase: 2.2 },
    9133
  ),
  P(
    "obj_church_grave_wall",
    ...
    Math.PI / 2,
    0.28,
    "Spine Hall rib wall phase-safe dragon bone marker",
    "Old Well / Underways"
  ),
  P(
    "rock_wide",
    ...
    Math.PI / 2,
    0.26,
    "Shed-skin scale pile on floor beside Spine Hall wall",
    "Old Well / Underways"
  ),
  P(
    "coffin",
    ...
    Math.PI / 2,
    0.36,
    "Bellbinder Tomb Old Harth sealed sarcophagus on floor",
    "Old Well / Underways"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.16,
    "Bellbinder Stole regalia plinth supported on tomb floor",
    "Old Well / Underways"
  ),
  P(
    "whetstone_fp",
    ...
    0,
    0.16,
    "Bellbinder Hammer regalia plinth supported on tomb floor",
    "Old Well / Underways"
  ),
  P(
    "wand",
    ...
    Math.PI / 2,
    0.16,
    "Bellbinder Tuning Fork regalia plinth supported on tomb floor",
    "Old Well / Underways",
    GROUND_Y + 0.18
  ),
  P(
    ...
    0,
    0.14,
    "Bellbinder Handbell regalia plinth supported on tomb floor",
    "Old Well / Underways",
    GROUND_Y + 0.18
  ),
  P(
    ...
    0,
    0.16,
    "Bellbinder Chain regalia plinth supported on tomb floor",
    "Old Well / Underways"
  ),
  P(
    "shield_round_color",
    ...
    0,
    0.16,
    "Bellbinder Ring seal regalia plinth supported on tomb floor",
    "Old Well / Underways",
    GROUND_Y + 0.18
  ),
  P(
    ...
    Math.PI,
    0.42,
    "True Bell hanging in Bellward Chamber from ceiling chain supports",
    "Old Well / Underways",
    GROUND_Y + 1.2
  ),
  P(
    ...
    0,
    0.2,
    "True Bell left chain anchor on Bellward Chamber floor",
    "Old Well / Underways"
  ),
  P(
    "chain_coil",
    ...
    0,
    0.2,
    "True Bell right chain anchor on Bellward Chamber floor",
    "Old Well / Underways"
  ),
  P(
    "rock_wide",
    ...
    0,
    0.32,
    "Wyrm's Bed bronze-stone snout silhouette phase-safe floor marker",
    "Old Well / Underways"
  ),
  P(
    "rock_wide",
    ...
    Math.PI / 3,
    0.28,
    "Wyrm's Bed folded wing silhouette phase-safe floor marker left",
    "Old Well / Underways"
  ),
  P(
    "rock_wide",
    ...
    -Math.PI / 3,
    0.28,
    "Wyrm's Bed folded wing silhouette phase-safe floor marker right",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    0,
    0.3,
    "Wyrm's Bed final threshold wall phase-safe dragon chamber marker",
    "Old Well / Underways"
  ),
  ...
  P(
    ...
    Math.PI,
    0.32,
    "North Gate stable tack ledger table against stable side wall",
    "North Gate"
  ),
  P(
    "chain_coil",
    ...
    0,
    0.16,
    "Stable lead rope coil supported on tack table",
    "North Gate",
    GROUND_Y + 0.52
  ),
  P(
    ...
    0,
    0.16,
    "Mount feed and travel permit ledger supported on stable tack table",
    "North Gate",
    GROUND_Y + 0.55
  ),
  P(
    ...
    Math.PI,
    0.34,
    "Stable tack cabinet against North Gate stable wall",
    "North Gate"
  ),
  P(
    "table_large_fp",
    ...
    0,
    0.36,
    "Guard Barracks command table inside watch planning room",
    "Guard Yard"
  ),
  P(
    "scroll_2_fp",
    ...
    0.1,
    0.16,
    "Gate patrol roster supported on guard command table",
    "Guard Yard",
    GROUND_Y + 0.62
  ),
  P(
    ...
    0,
    0.18,
    "Town Watch wall shield mounted over guard command table",
    "Guard Yard",
    GROUND_Y + 0.92
  ),
  P(
    ...
    Math.PI / 2,
    0.18,
    "Armory inspection sword supported on guard command table",
    "Guard Yard",
    GROUND_Y + 0.64
  ),
  P(
    ...
    0,
    0.3,
    "Barracks prisoner holding cell fence on floor beside command room wall",
    "Guard Yard"
  ),
  P(
    "chest",
    ...
    0,
    0.32,
    "Guard armory evidence chest on floor against barracks wall",
    "Guard Yard"
  ),
  ...
  P(
    ...
    0,
    0.28,
    "Bellbinder Antechamber column one on floor showing wyrm over river mural",
    "Temple Green"
  ),
  P(
    "pillar",
    ...
    0,
    0.28,
    "Bellbinder Antechamber column two on floor showing robed bell-ringer mural",
    "Temple Green"
  ),
  P(
    "pillar",
    ...
    0,
    0.28,
    "Bellbinder Antechamber column three on floor showing wyrm landing on stone mural",
    "Temple Green"
  ),
  P(
    "pillar",
    ...
    0,
    0.28,
    "Bellbinder Antechamber column four on floor showing great bell ringing mural",
    "Temple Green"
  ),
  P(
    "pillar",
    ...
    0,
    0.28,
    "Bellbinder Antechamber column five on floor showing wyrm falling asleep mural",
    "Temple Green"
  ),
  P(
    "pillar",
    ...
    0,
    0.28,
    "Bellbinder Antechamber column six on floor showing buried dragon mural",
    "Temple Green"
  ),
  P(
    "pillar",
    ...
    0,
    0.28,
    "Bellbinder Antechamber column seven on floor showing chapel built over binding mural",
    "Temple Green"
  ),
  P(
    "pillar",
    ...
    0,
    0.28,
    "Bellbinder Antechamber column eight on floor showing town growing around chapel mural",
    "Temple Green"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.18,
    "Bellbinder founding mural reader stand supported on antechamber floor",
    "Temple Green"
  ),
  P(
    "road",
    ...
    Math.PI / 2,
    0.34,
    "Bell-notation spiral stair floor marker descending from chapel antechamber",
    "Temple Green"
  ),
  P(
    "scroll_2_fp",
    ...
    0.18,
    0.14,
    "Carved bell-tone notation supported along descending stair wall",
    "Temple Green",
    GROUND_Y + 0.42
  ),
  P(
    ...
    -Math.PI / 2,
    0.24,
    "Undercroft stair torch mounted on carved notation wall bracket",
    "Temple Green",
    GROUND_Y + 1.0
  ),
  ...
  P(
    ...
    0,
    0.34,
    "Sleepwalkers encounter circle floor marker in Bellward Halls",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    Math.PI / 2,
    0.22,
    "Failed Apprentice alcove wall with broken handbell shelf",
    "Old Well / Underways"
  ),
  P(
    "obj_church_bells",
    ...
    0,
    0.12,
    "Broken apprentice handbell supported on Failed Apprentice alcove shelf",
    "Old Well / Underways",
    GROUND_Y + 0.42
  ),
  AD(
    "boss_failed_apprentice",
    354.0,
    -313.4,
    Math.PI,
    1,
    ...
    "Old Well / Underways",
    { radius: 0.35, speed: 0.04, phase: 0.2 },
    9131
  ),
  P(
    "bench_fp",
    ...
    0,
    0.24,
    "Listening Choir noncombat spirit dais bench on floor",
    "Old Well / Underways"
  ),
  P(
    "cabinet",
    ...
    Math.PI,
    0.24,
    "Hidden alcove cabinet opened by sparing Listening Choir chant",
    "Old Well / Underways"
  ),
  P(
    "shield_round_color",
    ...
    0,
    0.14,
    "Choir-Singer's Ring relic supported inside hidden alcove cabinet",
    "Old Well / Underways",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.38,
    "First Choir boss triad floor sigil in Bellward central hub",
    "Old Well / Underways"
  ),
  P(
    "candle_triple",
    ...
    0,
    0.14,
    "First Choir left harmony candle supported on floor sigil rim",
    "Old Well / Underways",
    GROUND_Y + 0.18
  ),
  P(
    ...
    0,
    0.14,
    "First Choir high harmony candle supported on floor sigil rim",
    "Old Well / Underways",
    GROUND_Y + 0.18
  ),
  P(
    ...
    0,
    0.14,
    "First Choir right harmony candle supported on floor sigil rim",
    "Old Well / Underways",
    GROUND_Y + 0.18
  ),
  AD(
    ...
    "The First Choir",
    "Old Well / Underways",
    { radius: 0.2, speed: 0.025, phase: 1.1 },
    9132
  ),
  ...
  P(
    ...
    0,
    0.28,
    "Bellbinder Tomb sealed bronze door with bell-clapper mechanism",
    "Old Well / Underways"
  ),
  P(
    "obj_church_bells",
    ...
    0,
    0.14,
    "Bell-clapper door mechanism mounted on sealed tomb door",
    "Old Well / Underways",
    GROUND_Y + 0.72
  ),
  P(
    ...
    0,
    0.12,
    "Group ritual bell station one supported on tomb floor plinth",
    "Old Well / Underways",
    GROUND_Y + 0.32
  ),
  P(
    ...
    0,
    0.12,
    "Group ritual bell station two supported on tomb floor plinth",
    "Old Well / Underways",
    GROUND_Y + 0.32
  ),
  P(
    ...
    0,
    0.12,
    "Group ritual bell station three supported on tomb floor plinth",
    "Old Well / Underways",
    GROUND_Y + 0.32
  ),
  P(
    ...
    0,
    0.12,
    "Group ritual bell station four supported on tomb floor plinth",
    "Old Well / Underways",
    GROUND_Y + 0.32
  ),
  P(
    ...
    0,
    0.18,
    "Founder's Seal codex stand supported beside Old Harth open sarcophagus",
    "Old Well / Underways"
  ),
  P(
    "shield_round_color",
    ...
    0,
    0.14,
    "Founder's Seal medallion supported on tomb codex stand",
    "Old Well / Underways",
    GROUND_Y + 0.48
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.52,
    "Bronze descent corridor floor path toward Wyrm's Bed antechamber",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    Math.PI / 2,
    0.22,
    "Compact Saboteurs ambush side wall marker in bronze corridor",
    "Old Well / Underways"
  ),
  P(
    "crate_metal_fp",
    ...
    0,
    0.22,
    "Compact smuggling payoff crate on corridor floor marker",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    0,
    0.22,
    "Bell-Mad tragedy wave threshold wall marker",
    "Old Well / Underways"
  ),
  P(
    "candle_1_fp",
    ...
    0,
    0.14,
    "Bell-Mad mercy candle supported on corridor floor",
    "Old Well / Underways",
    GROUND_Y + 0.18
  ),
  P(
    ...
    0,
    0.24,
    "Threshold Wyrm-Voice resonance pillar on corridor floor",
    "Old Well / Underways"
  ),
  P(
    "scroll_2_fp",
    ...
    0.12,
    0.14,
    "Old Tongue name-list inscription supported on resonance pillar face",
    "Old Well / Underways",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.16,
    "Rebind path Hammer ritual station supported on Bellward Chamber floor",
    "Old Well / Underways",
    GROUND_Y + 0.22
  ),
  P(
    ...
    Math.PI / 2,
    0.16,
    "Rebind path Tuning Fork ritual station supported on Bellward Chamber floor",
    "Old Well / Underways",
    GROUND_Y + 0.22
  ),
  P(
    ...
    0,
    0.18,
    "Rebind path Chain ritual station on Bellward Chamber floor",
    "Old Well / Underways"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.16,
    "Rebind path Stole chant lectern supported on Bellward Chamber floor",
    "Old Well / Underways"
  ),
  P(
    "shield_round_color",
    ...
    0,
    0.14,
    "Rebind path Ring seal ritual station supported on Bellward Chamber floor",
    "Old Well / Underways",
    GROUND_Y + 0.22
  ),
  P(
    ...
    0,
    0.36,
    "Slay path three-stroke cracked floor marker below True Bell",
    "Old Well / Underways"
  ),
  P(
    "obj_church_bells",
    ...
    0,
    0.14,
    "Wake path greeting handbell station supported beside True Bell floor",
    "Old Well / Underways",
    GROUND_Y + 0.28
  ),
  ...
  P(
    ...
    0,
    0.72,
    "Wyrm's Bed vast chamber floor ring marker beneath sleeping dragon",
    "Old Well / Underways"
  ),
  P(
    "rock_wide",
    ...
    Math.PI / 5,
    0.3,
    "Wyrm's Bed coiled dragon tail bronze-stone silhouette floor marker left",
    "Old Well / Underways"
  ),
  P(
    "rock_wide",
    ...
    -Math.PI / 5,
    0.3,
    "Wyrm's Bed coiled dragon tail bronze-stone silhouette floor marker right",
    "Old Well / Underways"
  ),
  P(
    "mine_gold_fragment",
    ...
    0,
    0.18,
    "Thaedryn low candle-flame eye glow supported in dragon chamber floor channel",
    "Old Well / Underways",
    GROUND_Y + 0.16
  ),
  P(
    ...
    0,
    0.18,
    "Second Thaedryn candle-flame eye glow supported in dragon chamber floor channel",
    "Old Well / Underways",
    GROUND_Y + 0.16
  ),
  P(
    ...
    Math.PI / 2,
    0.24,
    "Wyrm's Bed ribbed mural wall showing thousands of bell spirals",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    Math.PI / 2,
    0.24,
    "Second Wyrm's Bed ribbed mural wall showing thousands of bell spirals",
    "Old Well / Underways"
  ),
  ...
  P(
    ...
    0,
    0.34,
    "Player Services bank teller desk inside bank room",
    "Player Services"
  ),
  P(
    "chest",
    ...
    0,
    0.3,
    "Bank room coin chest on floor behind teller desk",
    "Player Services"
  ),
  P(
    "cabinet",
    ...
    Math.PI,
    0.32,
    "Bank vault cabinet against reinforced service wall",
    "Player Services"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.16,
    "Deposit ledger supported on bank teller desk",
    "Player Services",
    GROUND_Y + 0.56
  ),
  P(
    ...
    0,
    0.3,
    "Mail sorting interior table inside courier room",
    "Player Services"
  ),
  P(
    "scroll_2_fp",
    ...
    0.2,
    0.14,
    "Mail route packet supported on sorting table",
    "Player Services",
    GROUND_Y + 0.54
  ),
  P(
    ...
    Math.PI,
    0.3,
    "Courier parcel cabinet against mailroom wall",
    "Player Services"
  ),
  P(
    "table_long",
    ...
    0,
    0.32,
    "Auction escrow interior counter inside trading room",
    "Player Services"
  ),
  P(
    "crate_metal_fp",
    ...
    0,
    0.22,
    "Auction escrow lockbox supported behind trading counter",
    "Player Services"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.16,
    "Auction listing book supported on escrow counter",
    "Player Services",
    GROUND_Y + 0.56
  ),
  P(
    ...
    0,
    0.32,
    "Guild registrar charter desk inside guild alcove",
    "Player Services"
  ),
  P(
    "scroll_2_fp",
    ...
    -0.2,
    0.15,
    "Blank guild charter supported on registrar desk",
    "Player Services",
    GROUND_Y + 0.56
  ),
  P(
    ...
    Math.PI,
    0.3,
    "Guild seal cabinet against registrar wall",
    "Player Services"
  ),
  P(
    "cabinet",
    ...
    Math.PI,
    0.32,
    "Cosmetic wardrobe cabinet against dressing room wall",
    "Player Services"
  ),
  P(
    "shield_square_color",
    ...
    0,
    0.16,
    "Cosmetic mirror frame mounted on dressing room wall",
    "Player Services",
    GROUND_Y + 0.92
  ),
  P(
    ...
    Math.PI / 2,
    0.28,
    "Player Services waiting bench leaving clear customer lane",
    "Player Services"
  ),
  ...
  P(
    ...
    0,
    0.34,
    "Copper Kettle kitchen prep table inside cook room",
    "Copper Kettle"
  ),
  P(
    "cauldron_fp",
    ...
    0,
    0.22,
    "Kitchen stew kettle supported on cookroom hearth stones",
    "Copper Kettle",
    GROUND_Y + 0.18
  ),
  P(
    ...
    Math.PI,
    0.28,
    "Copper Kettle pantry rack against kitchen wall",
    "Copper Kettle"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.24,
    "Pantry supply crate on kitchen floor beside rack",
    "Copper Kettle"
  ),
  P(
    "bed_twin1",
    ...
    Math.PI / 2,
    0.38,
    "Copper Kettle rented room bed against upstairs room wall",
    "Copper Kettle"
  ),
  P(
    "table_small",
    ...
    0,
    0.24,
    "Rented room bedside table with travel candle",
    "Copper Kettle"
  ),
  P(
    "candle_1_fp",
    ...
    0,
    0.12,
    "Rented room travel candle supported on bedside table",
    "Copper Kettle",
    GROUND_Y + 0.52
  ),
  P(
    ...
    0,
    0.28,
    "Rented room footlocker on floor at bed end",
    "Copper Kettle"
  ),
  P(
    "obj_church_trapdoor_metal",
    ...
    0,
    0.28,
    "Copper Kettle cellar hatch set into kitchen floor clear of animal route",
    "Copper Kettle"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.24,
    "Cellar ale cask crate on floor beside hatch",
    "Copper Kettle"
  ),
  P(
    "bench_fp",
    ...
    0,
    0.28,
    "Copper Kettle hearth bench with clear aisle to bar",
    "Copper Kettle"
  ),
  ...
  P(
    ...
    0,
    0.36,
    "Black Anvil repair order table inside smithy room",
    "Craftsman Row"
  ),
  P(
    "anvil_fp",
    ...
    0,
    0.28,
    "Black Anvil floor anvil with clear hammering space",
    "Craftsman Row"
  ),
  P(
    "whetstone_fp",
    ...
    0,
    0.16,
    "Repair whetstone supported on smithy work table",
    "Craftsman Row",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.24,
    "Smithy ore crate on floor against forge wall",
    "Craftsman Row"
  ),
  P(
    "table_large_fp",
    ...
    0,
    0.34,
    "Carpenter drafting table inside woodworking room",
    "Craftsman Row"
  ),
  P(
    "scroll_2_fp",
    ...
    0.1,
    0.14,
    "Carpenter blueprint supported on drafting table",
    "Craftsman Row",
    GROUND_Y + 0.58
  ),
  P(
    ...
    Math.PI,
    0.28,
    "Timber rack against carpentry wall",
    "Craftsman Row"
  ),
  P(
    "table_long_decorated",
    ...
    0,
    0.32,
    "Tailor cutting table inside cloth room",
    "Craftsman Row"
  ),
  P(
    "banner_blue",
    ...
    0,
    0.16,
    "Folded blue cloth supported on tailor cutting table",
    "Craftsman Row",
    GROUND_Y + 0.6
  ),
  P(
    ...
    Math.PI,
    0.28,
    "Leather drying rack against workshop wall",
    "Craftsman Row"
  ),
  P(
    "shelf_small_bottles",
    ...
    Math.PI,
    0.26,
    "Alchemy prep shelf against craftsman side wall",
    "Craftsman Row"
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.28,
    "Reeve Hall public hearing table clear of dock warehouse patrol aisle",
    "Noble Rise"
  ),
  P(
    "bench_fp",
    ...
    0,
    0.28,
    "Petitioner waiting bench inside Reeve Hall chamber",
    "Noble Rise"
  ),
  P(
    "bookcase_2",
    ...
    Math.PI,
    0.34,
    "Reeve Hall legal archive bookcase against west wall",
    "Noble Rise"
  ),
  P(
    "table_medium",
    ...
    0,
    0.3,
    "Legal archive review table with clear clerk aisle",
    "Noble Rise"
  ),
  P(
    "scroll_2_fp",
    ...
    0.15,
    0.14,
    "Court docket supported on legal archive table",
    "Noble Rise",
    GROUND_Y + 0.56
  ),
  P(
    ...
    0,
    0.28,
    "Reeve private office writing desk beside balcony wall",
    "Noble Rise"
  ),
  P(
    "chest",
    ...
    0,
    0.28,
    "Tax treasury chest on floor inside private office",
    "Noble Rise"
  ),
  P(
    "cabinet",
    ...
    Math.PI,
    0.3,
    "Permit seal cabinet against Reeve private office wall",
    "Noble Rise"
  ),
  P(
    "bench_fp",
    ...
    Math.PI / 2,
    0.26,
    "Servant side corridor bench leaving noble passage clear",
    "Noble Rise"
  ),
  ...
  P(
    ...
    0,
    0.28,
    "Chapel nave pew row one left with clear aisle",
    "Temple Green"
  ),
  P(
    "bench_fp",
    ...
    0,
    0.28,
    "Chapel nave pew row one right with clear aisle",
    "Temple Green"
  ),
  P(
    "bench_fp",
    ...
    0,
    0.28,
    "Chapel nave pew row two left with clear aisle",
    "Temple Green"
  ),
  P(
    "bench_fp",
    ...
    0,
    0.28,
    "Chapel nave pew row two right with clear aisle",
    "Temple Green"
  ),
  P(
    "bed_twin1",
    ...
    Math.PI / 2,
    0.34,
    "Chapel infirmary cot against healing alcove wall",
    "Temple Green"
  ),
  P(
    "table_small",
    ...
    0,
    0.24,
    "Infirmary herb table beside cot",
    "Temple Green"
  ),
  P(
    "shelf_small_bottles",
    ...
    Math.PI,
    0.24,
    "Infirmary remedy shelf against chapel wall",
    "Temple Green"
  ),
  P(
    "table_medium",
    ...
    0,
    0.28,
    "Charity office donation table inside chapel side room",
    "Temple Green"
  ),
  P(
    "chest",
    ...
    0,
    0.24,
    "Charity blanket chest on floor beside donation table",
    "Temple Green"
  ),
  P(
    "cabinet",
    ...
    Math.PI,
    0.28,
    "Gravekeeper tool cabinet against cemetery shed wall",
    "Temple Green"
  ),
  P(
    "shovel",
    ...
    Math.PI / 2,
    0.16,
    "Gravekeeper shovel leaning against cemetery shed cabinet",
    "Temple Green",
    GROUND_Y + 0.44
  ),
  P(
    ...
    0,
    0.34,
    "Chapel Undercroft Test Entrance hatch set into bell-notation stair floor",
    "Temple Green"
  ),
  ...
  P(
    ...
    0,
    0.32,
    "Dock Ledger Warehouse office desk inside cargo room",
    "River Docks"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.14,
    "Warehouse cargo manifest supported on office desk",
    "River Docks",
    GROUND_Y + 0.56
  ),
  P(
    ...
    Math.PI,
    0.3,
    "Warehouse tariff cabinet against office wall",
    "River Docks"
  ),
  P(
    "table_long",
    ...
    0,
    0.32,
    "Customs inspection counter inside dock warehouse",
    "River Docks"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.24,
    "Sealed customs cargo crate on warehouse floor",
    "River Docks"
  ),
  P(
    "shelf_meat",
    ...
    Math.PI,
    0.28,
    "Cold fish shelf against warehouse river wall",
    "River Docks"
  ),
  P(
    "table_small",
    ...
    0,
    0.26,
    "Ferry ticket table inside warehouse travel corner",
    "River Docks"
  ),
  P(
    "scroll_2_fp",
    ...
    0.1,
    0.14,
    "Ferry ticket ledger supported on travel table",
    "River Docks",
    GROUND_Y + 0.54
  ),
  P(
    ...
    Math.PI,
    0.28,
    "Rope and oar rack against ferry staging wall",
    "River Docks"
  ),
  ...
  P(
    ...
    0,
    0.22,
    "Mudden Lean-To Home meal table beside sleeping pallet with clear doorway approach",
    "Mudden Ward"
  ),
  P(
    "chest",
    ...
    0,
    0.22,
    "Mudden Lean-To Home patched storage chest on floor",
    "Mudden Ward"
  ),
  P(
    "bed_twin1",
  ...
  P(
    ...
    0,
    0.18,
    "Mudden Poorhouse communal table tucked beside wall with clear door lane",
    "Mudden Ward"
  ),
  P(
    "candle_1_fp",
    ...
    0,
    0.1,
    "Mudden Poorhouse candle supported on communal table",
    "Mudden Ward",
    GROUND_Y + 0.52
  ),
  P(
    ...
    0,
    0.28,
    "Mudden washhouse folding table beneath laundry line clear of rat-catcher route",
    "Mudden Ward"
  ),
  P(
    "banner_white",
    ...
    0,
    0.14,
    "Fresh wash cloth supported across washhouse folding table",
    "Mudden Ward",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.24,
    "Cheap healer interior remedy table inside Mudden clinic",
    "Mudden Ward"
  ),
  P(
    "shelf_small_bottles",
    ...
    Math.PI,
    0.24,
    "Cheap healer bottle shelf against clinic wall",
    "Mudden Ward"
  ),
  P(
    "table_small",
    ...
    0,
    0.24,
    "Fence vendor hidden goods table inside Mudden back room",
    "Mudden Ward"
  ),
  P(
    "crate_metal_fp",
    ...
    0,
    0.22,
    "Fence vendor locked stash on floor beside hidden goods table",
    "Mudden Ward"
  ),
  P(
    "obj_church_trapdoor_metal",
    ...
    0,
    0.34,
    "Old Well Drain Test Entrance hatch set into Underways stair floor",
    "Old Well / Underways"
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.32,
    "Guard Barracks bunk one against west wall",
    "Guard Yard"
  ),
  P(
    "bed_twin1",
    ...
    Math.PI / 2,
    0.32,
    "Guard Barracks bunk two against west wall",
    "Guard Yard"
  ),
  P(
    "bed_twin1",
    ...
    Math.PI / 2,
    0.32,
    "Guard Barracks bunk three against west wall",
    "Guard Yard"
  ),
  P(
    "table_long",
    ...
    0,
    0.3,
    "Guard Barracks mess table with clear patrol aisle",
    "Guard Yard"
  ),
  P(
    "candle_1_fp",
    ...
    0,
    0.12,
    "Barracks mess candle supported on mess table",
    "Guard Yard",
    GROUND_Y + 0.52
  ),
  P(
    ...
    Math.PI,
    0.3,
    "Guard Barracks uniform cabinet against north wall",
    "Guard Yard"
  ),
  P(
    "rack",
    ...
    Math.PI,
    0.3,
    "Guard armory weapon rack against barracks wall",
    "Guard Yard"
  ),
  P(
    "shield_square_color",
    ...
    0,
    0.16,
    "Guard armory shield mounted on barracks wall rack",
    "Guard Yard",
    GROUND_Y + 0.92
  ),
  P(
    ...
    0,
    0.16,
    "Duty board roster supported on barracks command stand",
    "Guard Yard",
    GROUND_Y + 0.58
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.42,
    "Dungeon route step 01 Chapel Undercroft Test Entrance floor marker",
    "Temple Green"
  ),
  P(
    "road",
    ...
    Math.PI / 2,
    0.42,
    "Dungeon route step 02 Old Well Drain Test Entrance floor marker",
    "Old Well / Underways"
  ),
  P(
    "road",
    ...
    0,
    0.46,
    "Dungeon route step 03 Bellward Halls Debug Start floor marker",
    "Old Well / Underways"
  ),
  P(
    "road",
    ...
    0,
    0.42,
    "Dungeon route step 04 First Choir Arena threshold floor marker",
    "Old Well / Underways"
  ),
  P(
    "road",
    ...
    Math.PI / 2,
    0.46,
    "Dungeon route step 05 Old Harth Antechamber threshold floor marker",
    "Old Well / Underways"
  ),
  P(
    "road",
    ...
    Math.PI / 2,
    0.46,
    "Dungeon route step 06 Bellbinder Tomb corridor floor marker",
    "Old Well / Underways"
  ),
  P(
    "road",
    ...
    Math.PI / 2,
    0.46,
    "Dungeon route step 07 Bellward Chamber True Bell threshold floor marker",
    "Old Well / Underways"
  ),
  P(
    "road",
    ...
    Math.PI / 2,
    0.46,
    "Dungeon route step 08 Wyrm's Bed final chamber threshold floor marker",
    "Old Well / Underways"
  ),
  P(
    "torch_mounted",
    ...
    Math.PI,
    0.26,
    "Bellward Halls Debug Start torch mounted on dungeon entry wall bracket",
    "Old Well / Underways",
    GROUND_Y + 1.0
  ),
  P(
    ...
    0,
    0.16,
    "Dungeon testing route placard supported on Bellward Halls floor stand",
    "Old Well / Underways",
    GROUND_Y + 0.58
  ),
  ...
  P(
    ...
    0,
    0.34,
    "Dawn Loaf Bakery brick oven hearth body on floor against rear bakehouse wall",
    "Bakery"
  ),
  P(
    "table_long",
    ...
    0,
    0.32,
    "Dawn Loaf Bakery kneading table on floor with clear baker work lane",
    "Bakery"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.24,
    "Dawn Loaf flour sack crate stack on floor beside oven",
    "Bakery"
  ),
  P(
    "table_small",
    ...
    0,
    0.24,
    "Dawn Loaf cooling counter on floor beside bread shelf",
    "Bakery"
  ),
  P(
    "food_fishbone",
    ...
    0.1,
    0.14,
    "Fresh Dawn Loaf bread sample supported on cooling counter",
    "Bakery",
    GROUND_Y + 0.62
  ),
  P(
    ...
    0,
    0.12,
    "Bakery table candle supported on kneading table corner",
    "Bakery",
    GROUND_Y + 0.62
  ),
  P(
    ...
    3.14159,
    0.34,
    "Brindle Provision staple goods shelf against back wall",
    "Provision House"
  ),
  P(
    "cabinet",
  ...
  P(
    ...
    0,
    0.3,
    "Brindle Provision customer counter on floor with clear aisle",
    "Provision House"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.24,
    "Brindle Provision ration crate stack on floor",
    "Provision House"
  ),
  P(
    "bucket_wood",
    ...
    0,
    0.22,
    "Brindle Provision travel water bucket on floor beside counter",
    "Provision House"
  ),
  P(
    "scroll_2_fp",
    ...
    0,
    0.14,
    "Brindle Provision price ledger supported on customer counter",
    "Provision House",
    GROUND_Y + 0.62
  ),
  P(
    ...
    3.14159,
    0.32,
    "Green Mortar remedy bottle shelf against apothecary wall",
    "Apothecary"
  ),
  P(
    "table_long",
    ...
    0,
    0.28,
    "Green Mortar mortar mixing table on floor clear of Wyrm and Candle doorway approach",
    "Apothecary"
  ),
  P(
    "bed_twin1",
    ...
    1.5708,
    0.28,
    "Green Mortar treatment cot on floor beside healer lane",
    "Apothecary"
  ),
  P(
    "cabinet",
  ...
  P(
    ...
    0.1,
    0.14,
    "Saint Verena medicine notes supported on mixing table",
    "Apothecary",
    GROUND_Y + 0.62
  ),
  P(
    ...
    0,
    0.12,
    "Apothecary triage candle cluster supported on mixing table",
    "Apothecary",
    GROUND_Y + 0.64
  ),
  P(
    ...
    3.14159,
    0.3,
    "Wyrm and Candle wax shelf mounted against magic shop wall",
    "Magic Shop",
    GROUND_Y + 0.62
  ),
  P(
    ...
    3.14159,
    0.34,
    "Wyrm and Candle spellbook case against back wall",
    "Magic Shop"
  ),
  P(
    "table_small",
    ...
    0,
    0.26,
    "Wyrm and Candle bell-tone study table on floor",
    "Magic Shop"
  ),
  P(
    "spellbook_open",
    ...
    0.08,
    0.14,
    "Open bell resonance spellbook supported on study table",
    "Magic Shop",
    GROUND_Y + 0.62
  ),
  P(
    ...
    1.5708,
    0.12,
    "Wyrm and Candle tuning wand supported on study table",
    "Magic Shop",
    GROUND_Y + 0.66
  ),
  P(
    ...
    0,
    0.12,
    "Wyrm and Candle listening candle supported on table",
    "Magic Shop",
    GROUND_Y + 0.64
  ),
  P(
    ...
    1.5708,
    0.28,
    "Roadside Family Cottage sleeping bed on floor against cottage wall",
    "Residential"
  ),
  P(
    "table_small",
    ...
    0,
    0.24,
    "Roadside Family Cottage meal table on floor with walking lane",
    "Residential"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.22,
    "Roadside Family Cottage household storage chest crate on floor",
    "Residential"
  ),
  P(
    "candle_1_fp",
    ...
    0,
    0.12,
    "Family cottage supper candle supported on meal table",
    "Residential",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.16,
    "Family cottage prayer cloth mounted on interior wall peg",
    "Residential",
    GROUND_Y + 0.74
  ),
  P(
    ...
    0,
    0.3,
    "Brass Scale moneylender counter on floor inside Reeve Hall side office",
    "Noble Rise"
  ),
  P(
    "crate_metal_fp",
    ...
    0,
    0.22,
    "Brass Scale strongbox on floor behind moneylender counter",
    "Noble Rise"
  ),
  P(
    "cabinet",
    ...
    3.14159,
    0.28,
    "Brass Scale pawned valuables cabinet against office wall",
    "Noble Rise"
  ),
  P(
    "scroll_2_fp",
    ...
    0.08,
    0.14,
    "Brass Scale debt ledger supported on moneylender counter",
    "Noble Rise",
    GROUND_Y + 0.62
  ),
  P(
    ...
    0,
    0.12,
    "Brass Scale strongbox key supported on counter",
    "Noble Rise",
    GROUND_Y + 0.64
  ),
  P(
    ...
    1.5708,
    0.3,
    "River Dock Supply shop counter on warehouse floor",
    "River Docks"
  ),
  P(
    "bookcase_2",
    ...
    1.5708,
    0.3,
    "River Dock Supply rope hook and tool shelf against pier wall",
    "River Docks"
  ),
  P(
    "crate_wooden_fp",
    ...
    0,
    0.22,
    "River Dock Supply waterproof satchel crate on floor",
    "River Docks"
  ),
  P(
    "bucket_wood",
    ...
    0,
    0.22,
    "River Dock Supply bait tin bucket on floor beside counter",
    "River Docks"
  ),
  P(
    "food_fishbone",
    ...
    0,
    0.12,
    "River Dock Supply fishing hook sample supported on counter",
    "River Docks",
    GROUND_Y + 0.62
  ),
  P(
    ...
    0,
    0.18,
    "River Dock Supply fog lantern grounded beside shop counter",
    "River Docks"
  ),
  P(
    "obj_church_grave_wall",
    ...
    1.5708,
    0.28,
    "Q5 Beneath the Stones undercroft entry wall threshold phase-safe dungeon test entrance",
    "Old Well / Underways"
  ),
  P(
    "obj_church_trapdoor_metal",
    ...
    1.5708,
    0.24,
    "Q5 chapel undercroft return plate set into floor",
    "Old Well / Underways",
    GROUND_Y + 0.04
  ),
  P(
    ...
    0,
    0.22,
    "Q5 undercroft mapping table on floor before first hazard pocket",
    "Old Well / Underways"
  ),
  P(
    "scroll_2_fp",
    ...
    0,
    0.12,
    "Q5 undercroft map supported on mapping table",
    "Old Well / Underways",
    GROUND_Y + 0.58
  ),
  P(
    ...
    1.5708,
    0.24,
    "Q5 undercroft wall torch mounted on safe reset alcove bracket",
    "Old Well / Underways",
    GROUND_Y + 1.04
  ),
  P(
    ...
    1.5708,
    0.26,
    "Q6 Hidden Door mural walk wall segment phase-safe undercroft corridor",
    "Old Well / Underways"
  ),
  P(
    "bookstand_fp",
    ...
    0,
    0.16,
    "Q6 mural inscription stand supported on floor beside hidden door path",
    "Old Well / Underways"
  ),
  P(
    "obj_church_bells",
    ...
    1.5708,
    0.16,
    "Q6 tiny bell seal supported in hidden-door floor plate",
    "Old Well / Underways",
    GROUND_Y + 0.18
  ),
  P(
    ...
    1.5708,
    0.26,
    "Q6 first combat pocket broken wall cover phase-safe",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    0,
    0.28,
    "Q7 First Choir arena north threshold wall phase-safe",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    1.5708,
    0.24,
    "Q7 First Choir west line-of-sight pillar wall phase-safe",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    1.5708,
    0.24,
    "Q7 First Choir east line-of-sight pillar wall phase-safe",
    "Old Well / Underways"
  ),
  P(
    "fountain_center",
    ...
    0,
    0.16,
    "Q7 central bell-pattern safe floor marker set into arena floor",
    "Old Well / Underways",
    GROUND_Y + 0.05
  ),
  P(
    ...
    0,
    0.24,
    "Q7 recovery bench on floor near Listening Chamber lane",
    "Old Well / Underways"
  ),
  P(
    "rock_wide",
    ...
    0,
    0.24,
    "Q9 Pulse Hall first gathering vein rock on floor channel",
    "Old Well / Underways"
  ),
  P(
    "rock_wide",
    ...
    0.2,
    0.24,
    "Q9 Pulse Hall second gathering vein rock on floor channel",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    1.5708,
    0.24,
    "Q9 narrow stair tunnel wall from Pulse Hall to Echo Hall phase-safe",
    "Old Well / Underways"
  ),
  P(
    "fountain_center",
    ...
    0,
    0.2,
    "Q9 Echo Hall essence pool set into floor with clear party lane",
    "Old Well / Underways",
    GROUND_Y + 0.04
  ),
  P(
    ...
    1.5708,
    0.24,
    "Q9 narrow stair tunnel wall from Echo Hall to Spine Hall phase-safe",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    1.5708,
    0.26,
    "Q9 Spine Hall rib wall line-of-sight blocker phase-safe",
    "Old Well / Underways"
  ),
  AD(
    "boss_vyrahel_vein_keeper",
    642.0,
    -334.0,
    Math.PI,
    1,
    ...
    "Old Well / Underways",
    { radius: 0.85, speed: 0.14, phase: 0.6 },
    9134
  ),
  P(
    "coffin",
    ...
    1.5708,
    0.34,
    "Q10 Bellbinder Tomb sarcophagus boss-room centerpiece on floor",
    "Old Well / Underways"
  ),
  P(
    "obj_church_bells",
    ...
    0,
    0.16,
    "Q10 north ritual bell supported on tomb plinth",
    "Old Well / Underways",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.16,
    "Q10 south ritual bell supported on tomb plinth",
    "Old Well / Underways",
    GROUND_Y + 0.58
  ),
  P(
    ...
    0,
    0.2,
    "Q10 regalia plinth left on tomb floor",
    "Old Well / Underways"
  ),
  P(
    "table_small",
    ...
    0,
    0.2,
    "Q10 regalia plinth right on tomb floor",
    "Old Well / Underways"
  ),
  P(
    "whetstone_fp",
    ...
    0,
    0.16,
    "Q10 bell-clapper mechanism supported on tomb floor plate",
    "Old Well / Underways",
    GROUND_Y + 0.16
  ),
  P(
    ...
    3.14159,
    0.4,
    "Q11 Bellward Chamber True Bell supported by ceiling chain bracket",
    "Old Well / Underways",
    GROUND_Y + 1.2
  ),
  P(
    ...
    0,
    0.16,
    "Q11 left True Bell chain anchor on chamber floor",
    "Old Well / Underways",
    GROUND_Y + 0.12
  ),
  P(
    ...
    0,
    0.16,
    "Q11 right True Bell chain anchor on chamber floor",
    "Old Well / Underways",
    GROUND_Y + 0.12
  ),
  P(
    ...
    0,
    0.2,
    "Q11 Rebind ritual station on floor with safe lane",
    "Old Well / Underways"
  ),
  P(
    "table_small",
    ...
    0,
    0.2,
    "Q11 Slay ritual station on floor with safe lane",
    "Old Well / Underways"
  ),
  P(
    "table_small",
    ...
    0,
    0.2,
    "Q11 Wake ritual station on floor with countdown path",
    "Old Well / Underways"
  ),
  P(
    "rock_wide",
    ...
    0,
    0.3,
    "Q12 Wyrm's Bed raid arena central snout silhouette floor marker",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    1.5708,
    0.24,
    "Q12 Wyrm's Bed west line-of-sight blocker phase-safe wall",
    "Old Well / Underways"
  ),
  P(
    "obj_church_grave_wall",
    ...
    1.5708,
    0.24,
    "Q12 Wyrm's Bed east line-of-sight blocker phase-safe wall",
    "Old Well / Underways"
  ),
  P(
    "fountain_center",
    ...
    0,
    0.18,
    "Q12 raid safe-zone floor sigil set into stone",
    "Old Well / Underways",
    GROUND_Y + 0.04
  ),
  P(
    ...
    0,
    0.24,
    "Q12 memorial replay stone on floor at raid exit",
    "Old Well / Underways"
  ),
  ...
  A(
    ...
    "Stable horse",
    "North Gate",
    { radius: 1.3, speed: 0.22, phase: 1.1 }
  ),
  A("animal_dog", 475, -258, 0.2, 1.15, "Gate dog", "North Gate", {
    radius: 2.2,
    speed: 0.55,
    phase: 0.4,
  }),
  A(
    ...
    0,
    1.28,
    "Sergeant Bram gate patrol",
    "North Gate",
    { radius: 4.4, speed: 0.25, phase: 0.8 }
  ),
  A(
    "townsperson_guard",
    472.4,
    ...
    Math.PI / 2,
    1.18,
    "West gate watch sentry",
    "North Gate",
    { radius: 2.1, speed: 0.16, phase: 0.4 }
  ),
  A(
    "townsperson_guard",
    499.7,
    ...
    -Math.PI / 2,
    1.18,
    "East gate watch sentry",
    "North Gate",
    { radius: 2.1, speed: 0.16, phase: 1.8 }
  ),
  A(
    "townsperson_guard",
    489.0,
    ...
    Math.PI,
    1.15,
    "Market square guard patrol",
    "Market Square",
    { radius: 5.2, speed: 0.22, phase: 2.4 }
  ),
  A(
    "townsperson_guard",
    536,
    ...
    -Math.PI / 2,
    1.14,
    "Services plaza guard",
    "Player Services",
    { radius: 2.4, speed: 0.18, phase: 0.9 }
  ),
  A(
    "townsperson_guard",
    468,
    ...
    "Temple peacekeeper",
    "Temple Green",
    { radius: 2.0, speed: 0.15, phase: 2.2 }
  ),
  A(
    "townsperson_guard",
    596,
    ...
    "Dock watch patrol",
    "River Docks",
    { radius: 4.6, speed: 0.24, phase: 1.7 }
  ),
  A(
    "townsperson_guard",
    421,
    ...
    0,
    1.1,
    "Mudden Ward foot patrol",
    "Mudden Ward",
    { radius: 4.0, speed: 0.2, phase: 2.9 }
  ),
  A(
    "animal_pigeon",
    478.7,
    ...
    "Pigeon at fountain",
    "Market Square",
    { radius: 1.1, speed: 0.55, phase: 0.2 }
  ),
  A(
    "animal_pigeon",
    493.5,
    ...
    "Pigeon near board",
    "Market Square",
    { radius: 0.9, speed: 0.65, phase: 2.1 }
  ),
  A(
    "townsperson_market",
    476,
    ...
    Math.PI / 2,
    1.3,
    "Mara Thistle market guide",
    "Market Square",
    { radius: 3.4, speed: 0.2, phase: 0.1 }
  ),
  A(
    "townsperson_market",
    498,
    ...
    "Produce customer",
    "Market Square",
    { radius: 4.4, speed: 0.28, phase: 1.9 }
  ),
  A(
    "townsperson_courier",
    514,
    ...
    -Math.PI / 2,
    1.22,
    "Courier crossing market",
    "Market Square",
    { radius: 6.4, speed: 0.32, phase: 3.1 }
  ),
  ...
  A(
    ...
    Math.PI,
    1.18,
    "Town crier at Bridge Fountain",
    "Market Square",
    { radius: 1.6, speed: 0.14, phase: 1.2 }
  ),
  A(
    "townsperson_market",
    515,
    ...
    Math.PI,
    1.08,
    "Market performer puppet show",
    "Market Square",
    { radius: 1.1, speed: 0.17, phase: 0.7 }
  ),
  A(
    "townsperson_market",
    508.5,
    ...
    -Math.PI / 2,
    0.92,
    "Pickpocket child chase route",
    "Market Square",
    { radius: 3.8, speed: 0.42, phase: 2.5 }
  ),
  A(
    "townsperson_market",
    432,
    ...
    Math.PI / 2,
    1.12,
    "Produce vendor behind stall",
    "Market Square",
    { radius: 1.0, speed: 0.14, phase: 1.4 }
  ),
  A(
    "townsperson_market",
    536.5,
    ...
    -Math.PI / 2,
    1.1,
    "Cloth merchant behind stall",
    "Market Square",
    { radius: 1.0, speed: 0.14, phase: 0.3 }
  ),
  A(
    "townsperson_market",
    538.0,
    ...
    -Math.PI / 2,
    1.1,
    "Travel goods merchant behind stall",
    "Market Square",
    { radius: 1.0, speed: 0.14, phase: 2.2 }
  ),
  A(
    "animal_chicken",
    451,
    ...
    0.2,
    0.78,
    "Market livestock chicken",
    "Market Square",
    { radius: 0.8, speed: 0.58, phase: 1.5 }
  ),
  A(
    "animal_sheep",
    456,
    ...
    -0.2,
    0.86,
    "Market livestock sheep",
    "Market Square",
    { radius: 0.7, speed: 0.28, phase: 0.9 }
  ),
  A(
    "townsperson_courier",
    548.4,
    ...
    -Math.PI / 2,
    1.2,
    "Courier Anwen mail loop",
    "Player Services",
    { radius: 3.5, speed: 0.36, phase: 1.4 }
  ),
  ...
  A(
    ...
    Math.PI,
    1.08,
    "Banker Merl Voss bank access",
    "Player Services",
    { radius: 0.8, speed: 0.1, phase: 0.2 }
  ),
  A(
    "townsperson_market",
    507.9,
    ...
    -Math.PI / 2,
    1.04,
    "Auction Clerk Pell trade board",
    "Player Services",
    { radius: 0.9, speed: 0.1, phase: 1.8 }
  ),
  A(
    "townsperson_market",
    546.2,
    ...
    Math.PI / 2,
    1.02,
    "Storage Steward account storage",
    "Player Services",
    { radius: 0.8, speed: 0.09, phase: 2.5 }
  ),
  A(
    "townsperson_market",
    561.1,
    ...
    -Math.PI / 2,
    1.02,
    "Guild Registrar guild creation",
    "Player Services",
    { radius: 0.8, speed: 0.09, phase: 1.1 }
  ),
  A(
    "townsperson_market",
    563.8,
    ...
    -Math.PI / 2,
    1.0,
    "Cosmetic Wardrobe Attendant outfit station",
    "Player Services",
    { radius: 0.7, speed: 0.08, phase: 0.6 }
  ),
  ...
  A(
    ...
    Math.PI,
    1.22,
    "Master Osric Vale blacksmith trainer repair service",
    "Craftsman Row",
    { radius: 0.9, speed: 0.1, phase: 0.4 }
  ),
  A(
    "townsperson_guard",
    522.6,
    ...
    Math.PI / 2,
    1.04,
    "Apprentice Luth tending forge work orders",
    "Craftsman Row",
    { radius: 1.4, speed: 0.16, phase: 1.1 }
  ),
  A(
    "townsperson_market",
    505.6,
    ...
    Math.PI,
    1.02,
    "Carpentry profession trainer at saw bench",
    "Craftsman Row",
    { radius: 0.9, speed: 0.1, phase: 1.8 }
  ),
  A(
    ...
    0,
    1.0,
    "Tailoring and weaving profession trainer",
    "Craftsman Row",
    { radius: 0.8, speed: 0.1, phase: 2.3 }
  ),
  A(
    "townsperson_market",
    499.9,
    ...
    Math.PI / 2,
    1.0,
    "Leatherworker profession trainer by drying rack",
    "Craftsman Row",
    { radius: 0.8, speed: 0.09, phase: 1.4 }
  ),
  A(
    "townsperson_courier",
    515.4,
    ...
    -Math.PI / 2,
    1.0,
    "Rare ore delivery runner for Black Anvil",
    "Craftsman Row",
    { radius: 1.3, speed: 0.18, phase: 2.7 }
  ),
  A(
    "townsperson_guard",
    535.6,
    ...
    -Math.PI / 2,
    1.06,
    "Guard quartermaster inspecting weapon repairs",
    "Craftsman Row",
    { radius: 1.0, speed: 0.1, phase: 0.8 }
  ),
  A(
    "townsperson_guard",
    524,
    ...
    0,
    1.22,
    "Quartermaster walking loop",
    "Craftsman Row",
    { radius: 3.4, speed: 0.25, phase: 2.2 }
  ),
  A("animal_cat", 458, -172, -0.4, 0.95, "Apothecary cat", "Apothecary", {
    radius: 0.9,
    speed: 0.38,
    phase: 0.7,
  }),
  A(
    ...
    "Tavern cat",
    "Copper Kettle",
    { radius: 0.9, speed: 0.35, phase: 0.9 }
  ),
  A(
    "townsperson_market",
    548,
    ...
    Math.PI,
    1.22,
    "Elowen Pike innkeeper bind point rested XP tavern loop",
    "Copper Kettle",
    { radius: 2.0, speed: 0.18, phase: 2.8 }
  ),
  ...
  A(
    ...
    Math.PI,
    1.06,
    "Copper Kettle bard stage performer",
    "Copper Kettle",
    { radius: 0.7, speed: 0.08, phase: 0.3 }
  ),
  A(
    "townsperson_market",
    559.6,
    ...
    Math.PI / 2,
    1.04,
    "Copper Kettle cook serving food buffs",
    "Copper Kettle",
    { radius: 1.2, speed: 0.14, phase: 1.0 }
  ),
  A(
    "townsperson_courier",
    551.2,
    ...
    -Math.PI / 2,
    1.0,
    "Copper Kettle server carrying drinks",
    "Copper Kettle",
    { radius: 2.4, speed: 0.28, phase: 1.6 }
  ),
  A(
    "townsperson_market",
    543.7,
    ...
    Math.PI / 2,
    1.0,
    "Dice gambler at Copper Kettle table",
    "Copper Kettle",
    { radius: 0.9, speed: 0.1, phase: 2.4 }
  ),
  A(
    "townsperson_market",
    539.4,
    ...
    Math.PI / 2,
    1.02,
    "Traveling quest NPC reading rumor board",
    "Copper Kettle",
    { radius: 1.1, speed: 0.12, phase: 2.9 }
  ),
  A(
    "townsperson_market",
    548.0,
    ...
    0,
    0.98,
    "Tavern patron listening to bard",
    "Copper Kettle",
    { radius: 1.0, speed: 0.1, phase: 0.8 }
  ),
  A(
    ...
    Math.PI,
    1.22,
    "Father Aldren chapel loop",
    "Temple Green",
    { radius: 2.8, speed: 0.2, phase: 1.6 }
  ),
  ...
  A(
    ...
    -Math.PI / 2,
    0.92,
    "Sister Maelle chapel healer and condition cleansing NPC",
    "Temple Green",
    { radius: 0.7, speed: 0.08, phase: 0.5 }
  ),
  A(
    "townsperson_clergy",
    479.0,
    ...
    Math.PI,
    0.78,
    "Choir child candle vigil singer left",
    "Temple Green",
    { radius: 0.45, speed: 0.05, phase: 1.1 }
  ),
  A(
    "townsperson_clergy",
    483.0,
    ...
    Math.PI,
    0.78,
    "Choir child candle vigil singer right",
    "Temple Green",
    { radius: 0.45, speed: 0.05, phase: 2.1 }
  ),
  A(
    "townsperson_market",
    493.8,
    ...
    -Math.PI / 2,
    0.82,
    "Injured traveler waiting for Sister Maelle",
    "Temple Green",
    { radius: 0.35, speed: 0.04, phase: 0.9 }
  ),
  A(
    "townsperson_clergy",
    468.3,
    ...
    Math.PI / 2,
    0.86,
    "Chapel charity steward receiving turn-ins",
    "Temple Green",
    { radius: 0.6, speed: 0.07, phase: 1.8 }
  ),
  A(
    "townsperson_market",
    469.3,
    ...
    0,
    0.82,
    "Pilgrim placing plague prayer offering",
    "Temple Green",
    { radius: 0.8, speed: 0.08, phase: 2.8 }
  ),
  A(
    "townsperson_clergy",
    522.0,
    ...
    Math.PI,
    0.88,
    "Gravekeeper maintaining quiet cemetery path",
    "Temple Green",
    { radius: 1.0, speed: 0.07, phase: 2.4 }
  ),
  A(
    "townsperson_clergy",
    527.6,
    ...
    -Math.PI / 2,
    0.84,
    "Crypt disturbance witness by sealed crypt marker",
    "Temple Green",
    { radius: 0.55, speed: 0.06, phase: 1.5 }
  ),
  A(
    "animal_crow",
    524.8,
    ...
    0.2,
    0.58,
    "Cemetery crow watching missing bell graves",
    "Temple Green",
    { radius: 0.7, speed: 0.32, phase: 0.3 }
  ),
  A(
    "townsperson_clergy",
    490.6,
    ...
    Math.PI,
    0.84,
    "River Blessing procession organizer",
    "Temple Green",
    { radius: 0.65, speed: 0.07, phase: 1.3 }
  ),
  A(
    "townsperson_clergy",
    475.9,
    ...
    Math.PI / 2,
    0.84,
    "Chapel reputation vendor and blessing quartermaster",
    "Temple Green",
    { radius: 0.6, speed: 0.07, phase: 2.6 }
  ),
  A(
    "townsperson_clergy",
    454.7,
    ...
    Math.PI / 2,
    0.92,
    "Ysabet Fenlow Green Mortar Apothecary alchemy healer NPC",
    "Apothecary",
    { radius: 0.7, speed: 0.07, phase: 1.7 }
  ),
  ...
  A(
    ...
    Math.PI / 2,
    1.0,
    "Father Aldren investigating Old Well missing bell clue",
    "Old Well / Underways",
    { radius: 0.8, speed: 0.08, phase: 1.0 }
  ),
  A(
    "townsperson_mudden",
    414.5,
    ...
    -Math.PI / 2,
    0.92,
    "Nessa Crowe Underways access contact at hidden drain stair",
    "Old Well / Underways",
    { radius: 0.7, speed: 0.09, phase: 2.2 }
  ),
  A(
    "townsperson_clergy",
    404.8,
    ...
    0,
    0.92,
    "Gravekeeper watching Old Well stones",
    "Old Well / Underways",
    { radius: 0.7, speed: 0.07, phase: 0.4 }
  ),
  A(
    "townsperson_market",
    410.8,
    ...
    Math.PI,
    0.86,
    "Old woman witness hearing well whispers",
    "Old Well / Underways",
    { radius: 0.7, speed: 0.07, phase: 1.8 }
  ),
  A(
    "townsperson_mudden",
    412.2,
    ...
    Math.PI,
    0.72,
    "Child dare event NPC near barred Old Well",
    "Old Well / Underways",
    { radius: 0.65, speed: 0.12, phase: 2.8 }
  ),
  A(
    "townsperson_undead",
    417.2,
    ...
    Math.PI,
    0.74,
    "Ghostly audio cue ancient spirit silhouette",
    "Old Well / Underways",
    { radius: 0.5, speed: 0.04, phase: 0.7 }
  ),
  A(
    "animal_crow",
    411.0,
    ...
    -0.3,
    0.62,
    "Crow reacting to night bell whisper",
    "Old Well / Underways",
    { radius: 0.7, speed: 0.36, phase: 1.3 }
  ),
  ...
  A(
    ...
    Math.PI,
    1.16,
    "Reeve Caldus Merrow balcony petitioner hearing",
    "Noble Rise",
    { radius: 0.8, speed: 0.08, phase: 0.5 }
  ),
  A(
    "townsperson_market",
    572.0,
    ...
    -Math.PI / 2,
    1.08,
    "Edrik Vane moneylender debt records",
    "Noble Rise",
    { radius: 0.7, speed: 0.08, phase: 1.1 }
  ),
  A(
    "townsperson_market",
    553.5,
    ...
    Math.PI / 2,
    1.02,
    "Permit clerk issuing town charters",
    "Noble Rise",
    { radius: 0.7, speed: 0.09, phase: 1.8 }
  ),
  A(
    "townsperson_market",
    556.4,
    ...
    Math.PI,
    1.02,
    "Tax collector checking assessment desk",
    "Noble Rise",
    { radius: 0.9, speed: 0.1, phase: 2.2 }
  ),
  A(
    "townsperson_market",
    559.6,
    ...
    0,
    1.0,
    "Legal archivist guarding town records",
    "Noble Rise",
    { radius: 0.7, speed: 0.08, phase: 2.9 }
  ),
  A(
    "townsperson_guard",
    550.6,
    ...
    Math.PI,
    1.18,
    "Elite Reeve Hall guard west post",
    "Noble Rise",
    { radius: 0.9, speed: 0.1, phase: 0.4 }
  ),
  A(
    ...
    Math.PI,
    1.18,
    "Elite Reeve Hall guard east post",
    "Noble Rise",
    { radius: 0.9, speed: 0.1, phase: 1.4 }
  ),
  A(
    "townsperson_market",
    548.5,
    ...
    Math.PI / 2,
    1.0,
    "Noble petitioner waiting near garden",
    "Noble Rise",
    { radius: 1.2, speed: 0.12, phase: 1.7 }
  ),
  A(
    "townsperson_market",
    577.2,
    ...
    -Math.PI / 2,
    1.0,
    "Tax protest organizer outside Reeve Hall",
    "Noble Rise",
    { radius: 1.1, speed: 0.12, phase: 2.4 }
  ),
  A(
    "townsperson_courier",
    553.2,
    ...
    0,
    0.98,
    "Servant delivery runner on Noble side path",
    "Noble Rise",
    { radius: 1.1, speed: 0.18, phase: 0.9 }
  ),
  A(
    "townsperson_guard",
    565.4,
    ...
    Math.PI,
    1.14,
    "Distant lord audit envoy inspecting records",
    "Noble Rise",
    { radius: 0.8, speed: 0.09, phase: 2.8 }
  ),
  A(
    "townsperson_market",
    566,
    ...
    Math.PI,
    1.22,
    "Noble servant walking loop",
    "Noble Rise",
    { radius: 2.6, speed: 0.19, phase: 0.3 }
  ),
  A(
    "townsperson_guard",
    555,
    ...
    Math.PI,
    1.24,
    "Noble guard walking post",
    "Noble Rise",
    { radius: 2.2, speed: 0.18, phase: 1.3 }
  ),
  ...
  A(
    ...
    Math.PI,
    1.24,
    "Sergeant Bram Holt combat onboarding guard faction",
    "Guard Yard",
    { radius: 1.1, speed: 0.12, phase: 0.2 }
  ),
  A(
    "townsperson_guard",
    512.0,
    ...
    0,
    1.12,
    "Drill instructor calling training dummy drills",
    "Guard Yard",
    { radius: 1.2, speed: 0.16, phase: 1.1 }
  ),
  A(
    "townsperson_market",
    500.7,
    ...
    Math.PI / 2,
    1.0,
    "Bounty clerk processing patrol dailies",
    "Guard Yard",
    { radius: 0.7, speed: 0.08, phase: 2.4 }
  ),
  A(
    "townsperson_guard",
    526.8,
    ...
    -Math.PI / 2,
    1.1,
    "Guard Yard quartermaster armory service",
    "Guard Yard",
    { radius: 0.8, speed: 0.09, phase: 1.7 }
  ),
  A(
    "townsperson_guard",
    507.2,
    ...
    Math.PI / 2,
    1.06,
    "Sparring guard dueling ring left",
    "Guard Yard",
    { radius: 0.9, speed: 0.18, phase: 0.8 }
  ),
  A(
    "townsperson_guard",
    514.8,
    ...
    -Math.PI / 2,
    1.06,
    "Sparring guard dueling ring right",
    "Guard Yard",
    { radius: 0.9, speed: 0.18, phase: 2.2 }
  ),
  A(
    "townsperson_bandit",
    503.8,
    ...
    Math.PI,
    0.96,
    "Captured bandit prisoner escape event hook",
    "Guard Yard",
    { radius: 0.35, speed: 0.04, phase: 0.1 }
  ),
  A(
    "townsperson_guard",
    518.4,
    ...
    -Math.PI / 2,
    1.04,
    "Duel challenge guard PvP opt-in witness",
    "Guard Yard",
    { radius: 0.8, speed: 0.1, phase: 2.6 }
  ),
  A(
    "townsperson_guard",
    505.0,
    ...
    Math.PI,
    1.02,
    "Watchtower stair sentry roof repair lookout",
    "Guard Yard",
    { radius: 0.7, speed: 0.09, phase: 1.4 }
  ),
  A(
    "townsperson_courier",
    523.8,
    ...
    -Math.PI / 2,
    0.98,
    "Town defense runner checking barricade kit",
    "Guard Yard",
    { radius: 1.1, speed: 0.2, phase: 0.6 }
  ),
  A(
    "townsperson_guard",
    512,
    ...
    Math.PI,
    1.24,
    "Guard patrol around yard",
    "Guard Yard",
    { radius: 4.1, speed: 0.27, phase: 0.8 }
  ),
  A("animal_pigeon", 592, -183, Math.PI, 0.9, "Dock pigeon", "River Docks", {
    radius: 0.9,
    speed: 0.65,
    phase: 1.3,
  }),
  A(
    ...
    -Math.PI / 2,
    1.24,
    "Tovin Reed dock ledger loop",
    "River Docks",
    { radius: 4.8, speed: 0.31, phase: 2.7 }
  ),
  A(
    "townsperson_dockhand",
    586,
    ...
    Math.PI / 2,
    1.2,
    "Cargo hauler walking loop",
    "River Docks",
    { radius: 3.2, speed: 0.29, phase: 0.5 }
  ),
  ...
  A(
    ...
    -Math.PI / 2,
    1.16,
    "Dockmaster Tovin Reed cargo contracts and ferry ledger",
    "River Docks",
    { radius: 1.1, speed: 0.14, phase: 0.9 }
  ),
  A(
    "townsperson_dockhand",
    602.0,
    ...
    Math.PI,
    1.08,
    "Fishing trainer gutting river catch",
    "River Docks",
    { radius: 1.0, speed: 0.13, phase: 2.3 }
  ),
  A(
    "townsperson_market",
    603.2,
    ...
    -Math.PI / 2,
    1.0,
    "River goods vendor near fish market",
    "River Docks",
    { radius: 1.3, speed: 0.16, phase: 1.5 }
  ),
  A(
    "townsperson_dockhand",
    610.6,
    ...
    -Math.PI / 2,
    1.08,
    "Ferry master water travel post",
    "River Docks",
    { radius: 0.9, speed: 0.11, phase: 2.8 }
  ),
  A(
    "townsperson_smuggler",
    606.1,
    ...
    Math.PI,
    1.0,
    "River Knots smuggler contact behind cargo",
    "River Docks",
    { radius: 1.0, speed: 0.09, phase: 0.4 }
  ),
  A(
    "townsperson_guard",
    590.4,
    ...
    Math.PI / 2,
    1.08,
    "Customs guard inspecting sealed crates",
    "River Docks",
    { radius: 1.4, speed: 0.14, phase: 1.1 }
  ),
  A(
    "townsperson_dockhand",
    594.4,
    ...
    Math.PI,
    1.02,
    "Flood rescue rope handler",
    "River Docks",
    { radius: 1.2, speed: 0.15, phase: 2.0 }
  ),
  A(
    "animal_cat",
    598.5,
    ...
    0.3,
    0.72,
    "Dock cat watching fish table",
    "River Docks",
    { radius: 0.8, speed: 0.32, phase: 0.6 }
  ),
  A(
    "animal_crow",
    616.0,
    ...
    Math.PI,
    0.72,
    "Crow circling corpse-under-bridge clue",
    "River Docks",
    { radius: 0.9, speed: 0.45, phase: 1.9 }
  ),
  A(
    "animal_dog",
    411,
    ...
    "Mudden Ward dog",
    "Mudden Ward",
    { radius: 1.8, speed: 0.42, phase: 1.2 }
  ),
  A(
    "townsperson_mudden",
    416,
    ...
    Math.PI / 2,
    1.18,
    "Nessa Crowe alley loop",
    "Mudden Ward",
    { radius: 3.6, speed: 0.24, phase: 2.1 }
  ),
  ...
  A(
    ...
    -Math.PI / 2,
    0.96,
    "Mudden cheap healer tending fever cot",
    "Mudden Ward",
    { radius: 0.8, speed: 0.1, phase: 0.8 }
  ),
  A(
    "townsperson_smuggler",
    404.4,
    ...
    Math.PI / 2,
    0.96,
    "Mudden fence vendor hidden table",
    "Mudden Ward",
    { radius: 0.7, speed: 0.08, phase: 1.4 }
  ),
  A(
    "townsperson_mudden",
    407.3,
    ...
    Math.PI,
    0.92,
    "Mudden rat-catcher checking cages",
    "Mudden Ward",
    { radius: 0.9, speed: 0.13, phase: 2.2 }
  ),
  A(
    "townsperson_mudden",
    425.3,
    ...
    -Math.PI / 2,
    0.94,
    "Mudden washerwoman working wash line",
    "Mudden Ward",
    { radius: 0.9, speed: 0.12, phase: 0.2 }
  ),
  A(
    "townsperson_mudden",
    413.6,
    ...
    Math.PI,
    0.82,
    "Mudden orphan child blanket corner",
    "Mudden Ward",
    { radius: 0.7, speed: 0.18, phase: 1.7 }
  ),
  A(
    "townsperson_mudden",
    415.8,
    ...
    Math.PI / 2,
    0.96,
    "Mudden Kin reputation vendor at stash",
    "Mudden Ward",
    { radius: 0.8, speed: 0.09, phase: 2.7 }
  ),
  A(
    "townsperson_market",
    401.8,
    ...
    Math.PI / 2,
    1.0,
    "Debt collector serving eviction notice",
    "Mudden Ward",
    { radius: 0.8, speed: 0.11, phase: 2.1 }
  ),
  A(
    "townsperson_mudden",
    421.6,
    ...
    -Math.PI / 2,
    0.96,
    "Flood rescue volunteer by bucket line",
    "Mudden Ward",
    { radius: 0.9, speed: 0.12, phase: 1.0 }
  ),
  A(
    "animal_rat",
    407.0,
    ...
    0,
    0.58,
    "Mudden rat swarm near cages",
    "Mudden Ward",
    { radius: 0.7, speed: 0.36, phase: 0.5 }
  ),
  A(
    "animal_cat",
    427.1,
    ...
    -0.2,
    0.62,
    "Mudden alley cat watching laundry",
    "Mudden Ward",
    { radius: 0.8, speed: 0.3, phase: 1.5 }
  ),
  A("animal_chicken", 438, -236, 0, 0.9, "Chicken", "Farm", {
    radius: 1.4,
    speed: 0.75,
    phase: 0,
  }),
  A("animal_chicken", 446, -232, 0.5, 0.9, "Chicken", "Farm", {
    radius: 1.2,
    speed: 0.7,
    phase: 1.6,
  }),
  A("animal_pig", 452, -242, 0, 1.0, "Pig", "Farm", {
    radius: 1.8,
    speed: 0.34,
    phase: 0.2,
  }),
  A("animal_sheep", 438, -229, Math.PI / 2, 1.0, "Sheep", "Farm", {
    radius: 1.6,
    speed: 0.32,
    phase: 2.4,
  }),
  A("animal_cow", 456, -232, -Math.PI / 2, 1.0, "Cow", "Farm", {
    radius: 1.5,
    speed: 0.25,
    phase: 3.1,
  }),
  A("townsperson_farmer", 447, -228, 0, 1.22, "Farmhand walking loop", "Farm", {
    radius: 4.2,
    speed: 0.22,
    phase: 0.6,
  }),
  ...
  P(
    ...
    0,
    0.82,
    "Last Watch Post tower",
    "Harthmere Wilds - Last Watch Post"
  ),
  P(
    "obj_wall_simple",
    482,
  ...
  P(
    ...
    0,
    0.74,
    "Last Watch red-black warning banner planted in watch post base solid flag pole",
    "Harthmere Wilds - Last Watch Post"
  ),
  P(
    "torch_metal_fp",
  ...
  P(
    ...
    0,
    0.5,
    "Crossbow resting on watch table",
    "Harthmere Wilds - Last Watch Post",
    GROUND_Y + 0.72
  ),
  A(
    ...
    Math.PI,
    1.18,
    "Last Watch road guard",
    "Harthmere Wilds - Last Watch Post",
    { radius: 2.8, speed: 0.2, phase: 0.2 }
  ),
  A(
    "townsperson_guard",
    488,
    ...
    -Math.PI / 2,
    1.16,
    "Last Watch tired sentry",
    "Harthmere Wilds - Last Watch Post",
    { radius: 2.2, speed: 0.18, phase: 1.1 }
  ),
  ...
  P(
    ...
    Math.PI / 3,
    0.72,
    "Farmer wagon with owned field goods",
    "Harthmere Wilds - Gate Fields"
  ),
  P(
    "bucket_wood",
  ...
  A(
    ...
    0,
    0.9,
    "Rabbit in the hedgerow",
    "Harthmere Wilds - Gate Fields",
    { radius: 2.2, speed: 0.72, phase: 0.4 }
  ),
  A(
    "animal_crow",
    456,
    ...
    0,
    0.88,
    "Crow on the field edge",
    "Harthmere Wilds - Gate Fields",
    { radius: 1.0, speed: 0.48, phase: 2.2 }
  ),
  A(
    "animal_boar",
    468,
    ...
    -0.3,
    0.9,
    "Diseased boar near the crop line",
    "Harthmere Wilds - Gate Fields",
    { radius: 2.8, speed: 0.35, phase: 1.7 }
  ),
  A(
    "townsperson_farmer",
    438,
    ...
    Math.PI / 2,
    1.14,
    "Field farmer watching owned crops",
    "Harthmere Wilds - Gate Fields",
    { radius: 4.0, speed: 0.18, phase: 0.7 }
  ),
  ...
  P(
    ...
    0,
    0.62,
    "Fallen apple collection crate",
    "Harthmere Wilds - Orchard Lane"
  ),
  P(
    "barrel_apples",
    396,
    ...
    0,
    0.62,
    "Apple barrel beside orchard lane",
    "Harthmere Wilds - Orchard Lane"
  ),
  ...row(
    "tree",
  ...
  P(
    ...
    0,
    0.48,
    "Bandit blade leaning in fence post",
    "Harthmere Wilds - Mill Road",
    GROUND_Y + 0.78
  ),
  A(
    ...
    -0.4,
    0.92,
    "Fox watching the orchard",
    "Harthmere Wilds - Orchard Lane",
    { radius: 2.4, speed: 0.45, phase: 0.8 }
  ),
  A(
    "animal_boar",
    404,
    ...
    Math.PI / 2,
    1.05,
    "Orchard boar sounder",
    "Harthmere Wilds - Orchard Lane",
    { radius: 3.0, speed: 0.33, phase: 1.9 }
  ),
  A(
    "townsperson_bandit",
    421,
    ...
    -Math.PI / 2,
    1.12,
    "Bandit grain lookout",
    "Harthmere Wilds - Mill Road",
    { radius: 3.0, speed: 0.22, phase: 2.7 }
  ),
  ...
  P(
    ...
    0.2,
    0.74,
    "Mossy forest stone with herb growth",
    "Harthmere Wilds - Greenmere Edge"
  ),
  P(
    "bottle_green",
    ...
    0,
    0.38,
    "Herbalism marker: moss vial on stone",
    "Harthmere Wilds - Greenmere Edge",
    GROUND_Y + 0.52
  ),
  P(
  ...
  P(
    ...
    -0.2,
    0.42,
    "Woodcutting axe resting on fallen oak",
    "Harthmere Wilds - Greenmere Edge",
    GROUND_Y + 0.52
  ),
  P(
  ...
  A(
    ...
    "Harthmere Wilds - Greenmere Edge",
    { radius: 4.0, speed: 0.3, phase: 1.2 }
  ),
  A(
    "animal_wolf",
    552,
    ...
    -0.8,
    1.04,
    "Wolf stalking between roots",
    "Harthmere Wilds - Greenmere Edge",
    { radius: 3.8, speed: 0.42, phase: 2.1 }
  ),
  A(
    "animal_bear",
    575,
    ...
    0.2,
    0.95,
    "Black bear guarding berry thicket",
    "Harthmere Wilds - Greenmere Edge",
    { radius: 2.2, speed: 0.22, phase: 0.3 }
  ),
  A(
    "townsperson_hunter",
    506,
    ...
    Math.PI / 2,
    1.12,
    "Greenmere hunter trail guide",
    "Harthmere Wilds - Greenmere Edge",
    { radius: 2.6, speed: 0.18, phase: 1.0 }
  ),

  // Old Hunter's Track: traps, caches, stealth/survival fantasy.
  ...row(
    "fence_broken",
    "Harthmere Wilds - Old Hunter's Track",
    "Bent branch hunter marker",
    430,
    -420,
    6,
    -6,
  ...
  P(
    ...
    Math.PI / 2,
    0.52,
    "Snare line stretched near trail",
    "Harthmere Wilds - Old Hunter's Track"
  ),
  P(
    "bow",
  ...
  A(
    ...
    0,
    1.08,
    "Retired hunter by cache",
    "Harthmere Wilds - Old Hunter's Track",
    { radius: 2.1, speed: 0.14, phase: 2.0 }
  ),
  A(
    "animal_fox",
    439,
    ...
    Math.PI,
    0.86,
    "Silver fox trail marker",
    "Harthmere Wilds - Old Hunter's Track",
    { radius: 2.4, speed: 0.48, phase: 1.4 }
  ),
  A(
    "animal_snake",
    447,
    ...
    -0.3,
    0.9,
    "Snake near sun-warmed stone",
    "Harthmere Wilds - Old Hunter's Track",
    { radius: 0.7, speed: 0.16, phase: 0.9 }
  ),
  ...
  P(
    ...
    Math.PI / 2,
    0.52,
    "Blue rope marker tied by smugglers",
    "Harthmere Wilds - Briarfen"
  ),
  P(
    "lantern",
    ...
    0,
    0.5,
    "Witchlight lantern over mud path",
    "Harthmere Wilds - Briarfen",
    GROUND_Y + 0.8
  ),
  P(
    ...
    0,
    0.52,
    "Blackwater clay collection bucket",
    "Harthmere Wilds - Briarfen"
  ),
  A(
    "animal_frog",
    ...
    0,
    1.0,
    "Frog on Briarfen bank",
    "Harthmere Wilds - Briarfen",
    { radius: 0.9, speed: 0.42, phase: 0.2 }
  ),
  A(
    "animal_snake",
    655,
    ...
    -0.6,
    1.0,
    "Water snake in wet grass",
    "Harthmere Wilds - Briarfen",
    { radius: 0.8, speed: 0.2, phase: 2.3 }
  ),
  A(
    "townsperson_smuggler",
    650,
    ...
    Math.PI,
    1.1,
    "River Knot smuggler lookout",
    "Harthmere Wilds - Briarfen",
    { radius: 2.8, speed: 0.18, phase: 1.7 }
  ),
  A(
    "townsperson_undead",
    666,
    ...
    -Math.PI / 2,
    1.06,
    "Drowned dead in the reeds",
    "Harthmere Wilds - Briarfen",
    { radius: 1.8, speed: 0.1, phase: 0.6 }
  ),
  ...
  P(
    ...
    0.2,
    0.92,
    "Ruined Watchtower Ridge cracked tower",
    "Harthmere Wilds - Watchtower Ridge"
  ),
  P(
    "obj_wall_simple_windows",
    392,
  ...
  P(
    ...
    0.2,
    0.58,
    "Torn Harthmere banner planted in bandit watchtower base solid flag pole",
    "Harthmere Wilds - Watchtower Ridge"
  ),
  P(
    "crate_metal_fp",
  ...
  P(
    ...
    -0.5,
    0.48,
    "Mining pick leaning at quarry cut",
    "Harthmere Wilds - Watchtower Ridge",
    GROUND_Y + 0.44
  ),
  A(
    ...
    "Bandit hedge archer",
    "Harthmere Wilds - Watchtower Ridge",
    { radius: 3.2, speed: 0.2, phase: 1.3 }
  ),
  A(
    "townsperson_bandit",
    382,
    ...
    Math.PI / 2,
    1.18,
    "Rusk Hallowhand outlaw captain marker",
    "Harthmere Wilds - Watchtower Ridge",
    { radius: 2.2, speed: 0.16, phase: 0.4 }
  ),
  A(
    "animal_wolf",
    410,
    ...
    Math.PI,
    1.0,
    "Wolf drawn by bandit scraps",
    "Harthmere Wilds - Watchtower Ridge",
    { radius: 2.4, speed: 0.36, phase: 2.0 }
  ),
  ...
  A(
    ...
    0,
    0.9,
    "Raven at the old grave",
    "Harthmere Wilds - Gravewood",
    { radius: 0.9, speed: 0.42, phase: 0.5 }
  ),
  A(
    "townsperson_undead",
    536,
    ...
    Math.PI,
    1.08,
    "Bell-woken grave dead",
    "Harthmere Wilds - Gravewood",
    { radius: 1.6, speed: 0.09, phase: 2.6 }
  ),
  A(
    "animal_wolf",
    548,
    ...
    -Math.PI / 2,
    0.94,
    "Pale wolf at Gravewood edge",
    "Harthmere Wilds - Gravewood",
    { radius: 2.0, speed: 0.3, phase: 1.5 }
  ),

  // Deep Old Wood: distant high-danger visual promise for future group play.
  ...row(
    "tree_high",
    "Harthmere Wilds - Deep Old Wood",
    "Ancient old wood trunk",
    582,
    -470,
    8,
    14,
  ...
  ...row(
    "tree_crooked",
    "Harthmere Wilds - Deep Old Wood",
    "Face-like crooked old wood tree",
    596,
    -486,
    7,
    13,
  ...
  P(
    ...
    0,
    0.48,
    "Cold blue-green ritual light marker",
    "Harthmere Wilds - Deep Old Wood",
    GROUND_Y + 0.28
  ),
  P(
    ...
    0.2,
    0.36,
    "Forbidden rite book on standing stone base",
    "Harthmere Wilds - Deep Old Wood",
    GROUND_Y + 0.5
  ),
  P(
    ...
    0,
    0.42,
    "Mooncap mushroom alchemy marker",
    "Harthmere Wilds - Deep Old Wood",
    GROUND_Y + 0.35
  ),
  A(
    ...
    0.4,
    1.08,
    "Root-bound bear marker",
    "Harthmere Wilds - Deep Old Wood",
    { radius: 2.0, speed: 0.18, phase: 2.0 }
  ),
  A(
    "animal_deer",
    632,
    ...
    -0.4,
    1.0,
    "Possessed deer at standing stones",
    "Harthmere Wilds - Deep Old Wood",
    { radius: 2.4, speed: 0.24, phase: 1.0 }
  ),
  A(
    "boss_root_crowned_dead",
    620,
    -505,
    Math.PI,
    1,
    "Root-Crowned Dead public-event marker",
    "Harthmere Wilds - Deep Old Wood",
    { radius: 1.4, speed: 0.07, phase: 0.8 }
  ),

  // Charcoal Burners' Camp and Broken Toll Road connector.
  ...row(
    "road",
    "Harthmere Wilds - Broken Toll Road",
    "Broken toll road stones",
    454,
    -422,
    8,
    11,
  ...
  P(
    ...
    -0.4,
    0.58,
    "Charcoal cart under canvas",
    "Harthmere Wilds - Charcoal Camp"
  ),
  P(
    "barrel_stack",
    462,
  ...
  A(
    ...
    -Math.PI / 2,
    1.1,
    "Charcoal burner with forest rumors",
    "Harthmere Wilds - Charcoal Camp",
    { radius: 2.2, speed: 0.14, phase: 2.9 }
  ),
...
const HARTHMERE_RESIDENT_HOME_ASSIGNMENT_SUMMARY =
  createHarthmereResidentHomeAssignmentSummary(
    PLACEMENTS.filter((placement) => placement.meta?.kind === "actor").map(
      (placement) => ({
        asset: placement.asset,
        name: placement.name,
        district: placement.district,
...
if (typeof window !== "undefined") {
  (
    window as typeof window & { __harthmereResidentHousing?: unknown }
  ).__harthmereResidentHousing = HARTHMERE_RESIDENT_HOME_ASSIGNMENT_SUMMARY;
}
...
function pickHarthmereLocomotionClips(clips: THREE.AnimationClip[]): {
  ...
  const run =
    clips.find((c) => /^running?$/i.test(c.name)) ??
    find(/run|sprint|jog|gallop|trot/i);
  return { idle, walk, run };
}

function installHarthmereLocomotion(
  ...
  const canUseMovingClipAsDefault = Boolean(
    animated.wander || animated.bob || animated.spin
  );
  const start =
    idleAction ??
...
const harthmereRuntimeRoundedVoxelGeometryCache = new Map<
  string,
...
function getHarthmereRuntimeToonGradientMap() {
  ...
  const data = new Uint8Array([
    56, 56, 56, 255, 132, 132, 132, 255, 204, 204, 204, 255, 255, 255, 255, 255,
  ]);
...
function rememberHarthmereRuntimeFacePartNeutralTransform(
  object: THREE.Object3D
) {
...
function restoreHarthmereRuntimeFacePartNeutralTransform(
  object: THREE.Object3D
) {
...
function harthmereRuntimeColorChannelMix(
  source: number,
  target: number,
  amount: number
) {
...
function harthmereRuntimeColorMix(
  source: number,
  target: number,
  amount: number
) {
...
function harthmereRuntimeAppearanceForPlacement(
  ...
  const roleHint = `${placement.asset} ${placement.name ?? ""} ${
    placement.district ?? ""
  }`;
...
function harthmereRuntimePaletteForAppearance(
  ...
  return {
    skin:
      HARTHMERE_RUNTIME_SKIN_COLORS[appearance.face.skinTone] ?? fallback.skin,
    hair:
      HARTHMERE_RUNTIME_HAIR_COLORS[appearance.face.hairColor] ?? fallback.hair,
    tunic:
      HARTHMERE_RUNTIME_OUTFIT_COLORS[appearance.body.outfitColor] ??
      fallback.tunic,
    legs: fallback.legs,
    accent:
...
function harthmereRuntimeBodyMetrics(
  ...
  const shoulder =
    contract.shoulderWidth[body.shoulderWidth] ??
    contract.shoulderWidth.average;
  const arm = contract.armLength[body.armLength] ?? contract.armLength.average;
  const leg = contract.legLength[body.legLength] ?? contract.legLength.average;
  const stance = contract.stance[body.stance] ?? contract.stance.upright;
...
function harthmereRuntimeHeadSize(
...
function harthmereRuntimeFaceShapeMetrics(face: HarthmereVoxelFaceConfig) {
  const faceShapeExpressionContract =
    HARTHMERE_RUNTIME_APPEARANCE_OPTION_EXPRESSION_CONTRACT.faceShape;
...
function createHarthmereRuntimeVoxelHead(
  ...
  const faceEyeSpread =
    face.eyeShape === "wide" ? 0.088 : face.eyeShape === "small" ? 0.055 : 0.07;
  const eyeY =
    headY +
  ...
  const browTilt =
    face.browStyle === "arched" ? 0.2 : face.browStyle === "stern" ? -0.18 : 0;
  ...
  add(
    boxMesh(
      `${namePrefix}-skin-shadow`,
      [headWidth, 0.035, headDepth],
      [0, headY - headHeight / 2 + 0.035, -0.01],
      skinShadow
    )
  );
  add(
    boxMesh(
      `${namePrefix}-forehead-light`,
      [headWidth * 0.52, 0.02, 0.014],
      [0, eyeY + 0.1, faceFrontZ - 0.01],
      skinHighlight
    )
  );
  add(
    boxMesh(
      `${namePrefix}-jaw-shadow`,
      [headWidth * 0.62, 0.024, 0.014],
      [0, headY - headHeight / 2 + 0.065, faceFrontZ - 0.012],
      skinShadow
    )
  );
  ...
  add(
    boxMesh(
      ...
      [
        0.022 * sideProfile.leftWidthScale,
        headHeight * 0.54 * sideProfile.leftHeightScale,
        Math.max(0.06, headDepth * 0.72 + sideProfile.leftZOffset),
      ],
      [
        -headWidth / 2 - 0.011,
        headY + sideProfile.leftYOffset,
  ...
  add(
    boxMesh(
      ...
      [
        0.022 * sideProfile.rightWidthScale,
        headHeight * 0.54 * sideProfile.rightHeightScale,
        Math.max(0.06, headDepth * 0.72 + sideProfile.rightZOffset),
      ],
      [
        headWidth / 2 + 0.011,
        headY + sideProfile.rightYOffset,
  ...
  if (sideProfile.jawNotchSide === "left") {
    add(
      boxMesh(
        `${namePrefix}-left-jaw-notch-asym`,
        [0.032, 0.06, 0.018],
        [
          -headWidth / 2 + 0.024,
          headY - headHeight / 2 + 0.095,
          faceFrontZ - 0.008,
        ],
        skinShadow
      )
    ...
    add(
      boxMesh(
        `${namePrefix}-right-jaw-notch-asym`,
        [0.032, 0.06, 0.018],
        [
          headWidth / 2 - 0.024,
          headY - headHeight / 2 + 0.095,
  ...
  if (sideProfile.markSide === "left") {
    add(
      boxMesh(
        `${namePrefix}-left-face-mark-asym`,
        [0.017, 0.017, 0.011],
        [-headWidth * 0.28, mouthY + 0.055, faceFrontZ - 0.012],
        0x2a1712
      )
    );
    ...
    add(
      boxMesh(
        `${namePrefix}-right-face-mark-asym`,
        [0.017, 0.017, 0.011],
        [headWidth * 0.28, mouthY + 0.055, faceFrontZ - 0.012],
        0x2a1712
      )
  ...
  switch (face.hairStyle) {
    case "shaved":
      add(
        boxMesh(
          `${namePrefix}-hair-shaved`,
          [headWidth + 0.02, 0.025, headDepth + 0.02],
          [0, headY + headHeight / 2 + 0.018, -0.01],
          hair
        )
      );
      break;
    case "balding":
      add(
        boxMesh(
          `${namePrefix}-hair-balding-back`,
          [headWidth + 0.03, 0.055, headDepth + 0.02],
          [0, headY + headHeight / 2 + 0.025, 0.04],
          hair
        )
      );
      add(
        boxMesh(
          `${namePrefix}-hair-balding-left`,
          [0.045, 0.16, 0.05],
          [-headWidth / 2 - 0.015, headY, faceFrontZ + 0.03],
          hair
        )
      );
      add(
        boxMesh(
          `${namePrefix}-hair-balding-right`,
          [0.045, 0.16, 0.05],
          [headWidth / 2 + 0.015, headY, faceFrontZ + 0.03],
          hair
        )
    ...
    case "hood":
      add(
        boxMesh(
          `${namePrefix}-hood-back`,
          [headWidth + 0.14, headHeight + 0.1, headDepth + 0.11],
          [0, headY, 0.03],
          harthmereRuntimeColorDarken(hair, 0.12)
        )
      );
      add(
        boxMesh(
          `${namePrefix}-hood-rim`,
          [headWidth + 0.1, 0.055, 0.055],
          [0, eyeY + 0.07, faceFrontZ - 0.018],
          hair
        )
      );
      break;
    case "cap":
      add(
        boxMesh(
          `${namePrefix}-cap`,
          [headWidth + 0.1, 0.08, headDepth + 0.06],
          [0, headY + headHeight / 2 + 0.04, -0.01],
          hair
        )
      );
      add(
        boxMesh(
          `${namePrefix}-cap-brim`,
          [headWidth + 0.16, 0.032, 0.08],
          [0, eyeY + 0.075, faceFrontZ - 0.018],
          hair
        )
      );
      break;
    case "braids":
      add(
        boxMesh(
          `${namePrefix}-hair-top`,
          [headWidth + 0.04, 0.075, headDepth + 0.035],
          [0, headY + headHeight / 2 + 0.04, -0.01],
          hair
        )
      );
      add(
        boxMesh(
          `${namePrefix}-left-braid`,
          [0.05, 0.28, 0.06],
          [-headWidth / 2 - 0.035, headY - 0.04, faceFrontZ + 0.03],
          hair
        )
      );
      add(
        boxMesh(
          `${namePrefix}-right-braid`,
          [0.05, 0.28, 0.06],
          [headWidth / 2 + 0.035, headY - 0.04, faceFrontZ + 0.03],
          hair
        )
    ...
    case "bun":
      add(
        boxMesh(
          `${namePrefix}-hair-top`,
          [headWidth + 0.04, 0.08, headDepth + 0.04],
          [0, headY + headHeight / 2 + 0.04, -0.01],
          hair
        )
      );
    ...
    case "pigtails":
      ...
      add(
        boxMesh(
          `${namePrefix}-left-pigtail`,
          [0.07, 0.2, 0.07],
          [-headWidth / 2 - 0.05, headY - 0.02, faceFrontZ + 0.03],
          hair
        )
      );
      add(
        boxMesh(
          `${namePrefix}-right-pigtail`,
          [0.07, 0.2, 0.07],
          [headWidth / 2 + 0.05, headY - 0.02, faceFrontZ + 0.03],
          hair
        )
    ...
    case "wavy":
      add(
        boxMesh(
          `${namePrefix}-hair-top`,
          [headWidth + 0.06, 0.09, headDepth + 0.055],
          [0, headY + headHeight / 2 + 0.05, -0.01],
          hair
        )
      );
      add(
        boxMesh(
          `${namePrefix}-hair-wave-left`,
          [0.065, 0.06, 0.055],
          [-headWidth / 2 + 0.035, eyeY + 0.04, faceFrontZ - 0.012],
          hairHighlight
        )
      );
      add(
        boxMesh(
          `${namePrefix}-hair-wave-right`,
          [0.065, 0.06, 0.055],
          [headWidth / 2 - 0.035, eyeY + 0.02, faceFrontZ - 0.012],
          hairShadow
        )
      );
      break;
    ...
    case "bob":
      add(
        boxMesh(
          `${namePrefix}-hair-top`,
          [headWidth + 0.05, 0.08, headDepth + 0.05],
          [0, headY + headHeight / 2 + 0.04, -0.01],
          hair
        )
      );
      add(
        boxMesh(
          `${namePrefix}-left-hair`,
          [0.065, face.hairStyle === "long" ? 0.34 : 0.22, headDepth + 0.03],
          [-headWidth / 2 - 0.02, headY - 0.02, -0.01],
          hair
        )
      );
      add(
        boxMesh(
          `${namePrefix}-right-hair`,
          [0.065, face.hairStyle === "long" ? 0.34 : 0.22, headDepth + 0.03],
          [headWidth / 2 + 0.02, headY - 0.02, -0.01],
          hair
        )
    ...
    default:
      ...
      add(
        boxMesh(
          `${namePrefix}-hair-fringe`,
          [headWidth * 0.62, 0.032, 0.035],
          [-headWidth * 0.06, eyeY + 0.077, faceFrontZ - 0.02],
          hairHighlight
        )
      );
      break;
  ...
  const leftEye = add(
    boxMesh(
      `${namePrefix}-left-eye`,
      [eyeWidth, eyeHeight, 0.022],
      [-faceEyeSpread, eyeY, faceFrontZ],
      eye
    )
  );
  const hairPartX =
    sideProfile.hairPartSide === "left" ? -headWidth * 0.18 : headWidth * 0.18;
  add(
    boxMesh(
      `${namePrefix}-hair-part-asym`,
      [0.016, 0.095, 0.024],
      [hairPartX, eyeY + 0.078, faceFrontZ - 0.024],
      hairShadow
    )
  );
  if (sideProfile.hairLockSide === "left") {
    add(
      boxMesh(
        `${namePrefix}-left-side-hair-lock-asym`,
        [0.04, 0.16, 0.045],
        [-headWidth / 2 - 0.03, eyeY + 0.005, faceFrontZ + 0.026],
        hairHighlight
      )
    );
    ...
    add(
      boxMesh(
        `${namePrefix}-right-side-hair-lock-asym`,
        [0.04, 0.16, 0.045],
        [headWidth / 2 + 0.03, eyeY + 0.005, faceFrontZ + 0.026],
        hairHighlight
      )
  ...
  add(
    boxMesh(
      `${namePrefix}-left-eye-glint`,
      [0.012, 0.011, 0.01],
      [-faceEyeSpread - 0.008, eyeY + 0.009, faceFrontZ - 0.01],
      0xf5f1dc
    )
  );
  add(
    boxMesh(
      `${namePrefix}-right-eye-glint`,
      [0.012, 0.011, 0.01],
      [faceEyeSpread - 0.008, eyeY + 0.009, faceFrontZ - 0.01],
      0xf5f1dc
    )
  ...
  if (face.eyeShape === "sleepy") {
    add(
      boxMesh(
        `${namePrefix}-left-sleepy-lid`,
        [eyeWidth + 0.02, 0.012, 0.01],
        [-faceEyeSpread, eyeY + 0.018, faceFrontZ - 0.01],
        skinShadow
      )
    );
    add(
      boxMesh(
        `${namePrefix}-right-sleepy-lid`,
        [eyeWidth + 0.02, 0.012, 0.01],
        [faceEyeSpread, eyeY + 0.018, faceFrontZ - 0.01],
        skinShadow
      )
  ...
  if (face.faceShape === "wide" || face.faceShape === "soft") {
    add(
      boxMesh(
        ...
        [
          headWidth * 0.86 * harthmereRuntimeFaceShapeMetrics(face).jaw,
          0.04,
          0.018,
        ],
  ...
  add(
    boxMesh(
      `${namePrefix}-nose`,
      noseSize,
      [0, headY - 0.025, faceFrontZ - 0.015],
      skinShadow
    )
  );
  if (face.noseStyle === "button") {
    add(
      boxMesh(
        `${namePrefix}-button-nose-tip`,
        [0.065, 0.02, 0.024],
        [0, headY - 0.055, faceFrontZ - 0.035],
        skinShadow
      )
    );
  }
  add(
    boxMesh(
      `${namePrefix}-mouth`,
      [mouthWidth, face.mouthStyle === "open" ? 0.045 : 0.02, 0.016],
      [face.mouthStyle === "smirk" ? 0.015 : 0, mouthY, faceFrontZ - 0.006],
      face.mouthStyle === "open" ? 0x6b2f33 : 0x2a1712
    )
  );
  if (face.mouthStyle === "open") {
    add(
      boxMesh(
        `${namePrefix}-mouth-teeth`,
        [mouthWidth * 0.75, 0.012, 0.012],
        [0, mouthY + 0.014, faceFrontZ - 0.018],
        0xf6e6d0
      )
    );
  } else if (face.mouthStyle === "smile" || face.mouthStyle === "frown") {
    const sign = face.mouthStyle === "smile" ? 1 : -1;
    const leftCorner = add(
      boxMesh(
        `${namePrefix}-mouth-left-corner`,
        [0.032, 0.018, 0.012],
        [-mouthWidth / 2, mouthY + sign * 0.015, faceFrontZ - 0.014],
        0x2a1712
      )
    );
    leftCorner.rotation.z = sign * 0.32;
    const rightCorner = add(
      boxMesh(
        `${namePrefix}-mouth-right-corner`,
        [0.032, 0.018, 0.012],
        [mouthWidth / 2, mouthY + sign * 0.015, faceFrontZ - 0.014],
        0x2a1712
      )
  ...
  const leftBrow = boxMesh(
    `${namePrefix}-left-brow`,
    [
      face.browStyle === "soft" ? 0.05 : 0.065,
      face.browStyle === "scarred" ? 0.022 : 0.016,
      0.018,
    ],
    [-faceEyeSpread, eyeY + 0.045, faceFrontZ - 0.004],
    hair
  );
  leftBrow.rotation.z = browTilt;
  const rightBrow = boxMesh(
    `${namePrefix}-right-brow`,
    [
      face.browStyle === "soft" ? 0.05 : 0.065,
      face.browStyle === "scarred" ? 0.022 : 0.016,
      0.018,
  ...
  add(
    boxMesh(
      `${namePrefix}-left-brow-shadow`,
      [0.05, 0.008, 0.01],
      [-faceEyeSpread, eyeY + 0.022, faceFrontZ - 0.012],
      hairShadow
    )
  );
  add(
    boxMesh(
      `${namePrefix}-right-brow-shadow`,
      [0.05, 0.008, 0.01],
      [faceEyeSpread, eyeY + 0.022, faceFrontZ - 0.012],
      hairShadow
    )
  ...
  if (face.browStyle === "scarred") {
    const scar = add(
      boxMesh(
        `${namePrefix}-brow-scar`,
        [0.016, 0.08, 0.012],
        [-faceEyeSpread + 0.04, eyeY + 0.035, faceFrontZ - 0.016],
        0xf1d0b8
      )
    );
  ...
  if (face.cheekStyle && face.cheekStyle !== "none") {
    ...
    add(
      boxMesh(
        ...
        [0.045, 0.032, 0.012],
        [-0.12, mouthY + 0.045, faceFrontZ - 0.012],
        cheekColor
      )
    );
    add(
      boxMesh(
        `${namePrefix}-right-cheek`,
        [0.045, 0.032, 0.012],
        [0.12, mouthY + 0.045, faceFrontZ - 0.012],
        cheekColor
      )
  ...
  if (face.accessory === "cap" || face.hairStyle === "cap") {
    add(
      boxMesh(
        `${namePrefix}-accessory-cap-brim`,
        [headWidth + 0.12, 0.035, 0.08],
        [0, eyeY + 0.08, faceFrontZ - 0.015],
        0x38405a
      )
    );
  } else if (face.accessory === "headband") {
    add(
      boxMesh(
        `${namePrefix}-headband`,
        [headWidth + 0.08, 0.035, 0.035],
        [0, eyeY + 0.085, faceFrontZ - 0.002],
        0xd6a632
      )
    );
  } else if (face.accessory === "spectacles") {
    group.add(
      boxMesh(
        `${namePrefix}-left-spectacles`,
        [0.07, 0.01, 0.012],
        [-faceEyeSpread, eyeY, faceFrontZ - 0.008],
        0xd8d3c1
      ),
      boxMesh(
        `${namePrefix}-right-spectacles`,
        [0.07, 0.01, 0.012],
        [faceEyeSpread, eyeY, faceFrontZ - 0.008],
        0xd8d3c1
      ),
      boxMesh(
        `${namePrefix}-spectacles-bridge`,
        [0.035, 0.008, 0.01],
        [0, eyeY, faceFrontZ - 0.008],
        0xd8d3c1
      )
    );
  }
  if (face.facialHair !== "none") {
    group.add(
      boxMesh(
        `${namePrefix}-facial-hair`,
        [0.16, face.facialHair === "full_beard" ? 0.12 : 0.05, 0.018],
        [0, mouthY - 0.05, faceFrontZ - 0.01],
        hair
      )
    );
  }
...
function findHarthmereRuntimeExpressionPart(
  root: THREE.Object3D,
  suffix: string
) {
  let found: THREE.Object3D | undefined;
  root.traverse((object) => {
    if (!found && object.name.endsWith(suffix)) found = object;
  });
  return found;
}
function scaleHarthmereRuntimeExpressionPart(
...
function moveHarthmereRuntimeExpressionPart(
  object: THREE.Object3D | undefined,
  x: number,
  y: number,
  z: number
...
function rotateHarthmereRuntimeExpressionPart(
  object: THREE.Object3D | undefined,
  z: number
) {
  if (!object) return;
...
function applyHarthmereRuntimeFacialExpressionToFaceRoot(
  ...
  faceRoot.traverse((object) =>
    restoreHarthmereRuntimeFacePartNeutralTransform(object)
  );
  ...
  const leftCorner = findHarthmereRuntimeExpressionPart(
    faceRoot,
    "-mouth-left-corner"
  );
  const rightCorner = findHarthmereRuntimeExpressionPart(
    faceRoot,
    "-mouth-right-corner"
  );
  ...
  switch (state.expression) {
    case "happy":
    case "friendly":
      scaleHarthmereRuntimeExpressionPart(leftEye, [1.08, 0.78, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.08, 0.78, 1]);
      moveHarthmereRuntimeExpressionPart(leftBrow, 0, 0.012 * intensity, 0);
      moveHarthmereRuntimeExpressionPart(rightBrow, 0, 0.012 * intensity, 0);
      scaleHarthmereRuntimeExpressionPart(mouth, [1.22, 0.85, 1]);
      moveHarthmereRuntimeExpressionPart(mouth, 0, 0.014 * intensity, 0);
      rotateHarthmereRuntimeExpressionPart(leftCorner, 0.28 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightCorner, -0.28 * intensity);
      break;
    case "sad":
    case "crying":
    case "ashamed":
      scaleHarthmereRuntimeExpressionPart(leftEye, [0.92, 0.78, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [0.92, 0.78, 1]);
      moveHarthmereRuntimeExpressionPart(leftBrow, 0, -0.012 * intensity, 0);
      moveHarthmereRuntimeExpressionPart(rightBrow, 0, -0.012 * intensity, 0);
      rotateHarthmereRuntimeExpressionPart(leftBrow, -0.18 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightBrow, 0.18 * intensity);
      rotateHarthmereRuntimeExpressionPart(leftCorner, -0.24 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightCorner, 0.24 * intensity);
      if (state.expression === "crying") {
        scaleHarthmereRuntimeExpressionPart(leftEye, [0.84, 0.68, 1]);
        scaleHarthmereRuntimeExpressionPart(rightEye, [0.84, 0.68, 1]);
      } else if (state.expression === "ashamed") {
        moveHarthmereRuntimeExpressionPart(leftEye, 0, -0.012 * intensity, 0);
        moveHarthmereRuntimeExpressionPart(rightEye, 0, -0.012 * intensity, 0);
      }
      break;
    case "angry":
    case "determined":
    case "furious":
      scaleHarthmereRuntimeExpressionPart(leftEye, [1.08, 0.62, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.08, 0.62, 1]);
      rotateHarthmereRuntimeExpressionPart(leftBrow, -0.34 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightBrow, 0.34 * intensity);
      scaleHarthmereRuntimeExpressionPart(mouth, [0.88, 0.72, 1]);
      if (state.expression === "furious") {
        scaleHarthmereRuntimeExpressionPart(leftEye, [1.08, 0.72, 1]);
        scaleHarthmereRuntimeExpressionPart(rightEye, [1.08, 0.72, 1]);
        if (teeth) teeth.visible = true;
      }
      break;
    case "surprised":
      scaleHarthmereRuntimeExpressionPart(leftEye, [1.25, 1.26, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.25, 1.26, 1]);
      moveHarthmereRuntimeExpressionPart(leftBrow, 0, 0.03 * intensity, 0);
      moveHarthmereRuntimeExpressionPart(rightBrow, 0, 0.03 * intensity, 0);
      scaleHarthmereRuntimeExpressionPart(mouth, [0.76, 1.9, 1]);
      if (teeth) teeth.visible = true;
      break;
    case "afraid":
    case "terrified":
      scaleHarthmereRuntimeExpressionPart(leftEye, [1.16, 1.16, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.16, 1.16, 1]);
      moveHarthmereRuntimeExpressionPart(leftBrow, 0, 0.024 * intensity, 0);
      moveHarthmereRuntimeExpressionPart(rightBrow, 0, 0.024 * intensity, 0);
      rotateHarthmereRuntimeExpressionPart(leftBrow, 0.18 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightBrow, -0.18 * intensity);
      scaleHarthmereRuntimeExpressionPart(mouth, [0.88, 1.45, 1]);
      if (state.expression === "terrified") {
        scaleHarthmereRuntimeExpressionPart(leftEye, [1.15, 1.18, 1]);
        scaleHarthmereRuntimeExpressionPart(rightEye, [1.15, 1.18, 1]);
        scaleHarthmereRuntimeExpressionPart(mouth, [0.82, 1.35, 1]);
      }
      break;
    case "hurt":
      scaleHarthmereRuntimeExpressionPart(leftEye, [0.62, 0.55, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.05, 0.82, 1]);
      rotateHarthmereRuntimeExpressionPart(leftBrow, -0.28 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightBrow, 0.18 * intensity);
      moveHarthmereRuntimeExpressionPart(
        mouth,
        0.012 * intensity,
        -0.012 * intensity,
        0
      );
      rotateHarthmereRuntimeExpressionPart(mouth, -0.12 * intensity);
      break;
    case "dead":
      scaleHarthmereRuntimeExpressionPart(leftEye, [1.2, 0.34, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.2, 0.34, 1]);
      rotateHarthmereRuntimeExpressionPart(leftEye, 0.48);
      rotateHarthmereRuntimeExpressionPart(rightEye, -0.48);
      scaleHarthmereRuntimeExpressionPart(mouth, [0.8, 0.55, 1]);
      break;
    case "thinking":
    case "suspicious":
    case "confused":
    case "bored":
    case "annoyed":
      scaleHarthmereRuntimeExpressionPart(leftEye, [0.92, 0.72, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.08, 0.88, 1]);
      rotateHarthmereRuntimeExpressionPart(
        leftBrow,
      ...
      if (state.expression === "confused") {
        rotateHarthmereRuntimeExpressionPart(leftBrow, 0.28 * intensity);
        rotateHarthmereRuntimeExpressionPart(rightBrow, -0.28 * intensity);
        rotateHarthmereRuntimeExpressionPart(mouth, -0.12 * intensity);
      } else if (state.expression === "bored") {
        scaleHarthmereRuntimeExpressionPart(leftEye, [1, 0.62, 1]);
        scaleHarthmereRuntimeExpressionPart(rightEye, [1, 0.62, 1]);
        moveHarthmereRuntimeExpressionPart(mouth, 0, -0.012 * intensity, 0);
      }
      break;
    case "embarrassed":
      scaleHarthmereRuntimeExpressionPart(leftEye, [1.02, 0.75, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [0.9, 0.7, 1]);
      moveHarthmereRuntimeExpressionPart(leftBrow, 0, 0.012 * intensity, 0);
      rotateHarthmereRuntimeExpressionPart(leftCorner, 0.16 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightCorner, -0.08 * intensity);
      break;
    case "tired":
      scaleHarthmereRuntimeExpressionPart(leftEye, [1.05, 0.45, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.05, 0.45, 1]);
      moveHarthmereRuntimeExpressionPart(leftBrow, 0, -0.018 * intensity, 0);
      moveHarthmereRuntimeExpressionPart(rightBrow, 0, -0.018 * intensity, 0);
      scaleHarthmereRuntimeExpressionPart(mouth, [0.78, 0.72, 1]);
      break;
    case "dizzy":
      rotateHarthmereRuntimeExpressionPart(leftEye, 0.48 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightEye, -0.48 * intensity);
      rotateHarthmereRuntimeExpressionPart(mouth, 0.18 * intensity);
      break;
    case "cold":
      scaleHarthmereRuntimeExpressionPart(leftEye, [0.92, 0.68, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [0.92, 0.68, 1]);
      scaleHarthmereRuntimeExpressionPart(mouth, [0.72, 0.6, 1]);
      moveHarthmereRuntimeExpressionPart(mouth, 0, -0.01 * intensity, 0);
      break;
    case "relieved":
      scaleHarthmereRuntimeExpressionPart(leftEye, [1.08, 0.58, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.08, 0.58, 1]);
      rotateHarthmereRuntimeExpressionPart(leftCorner, 0.2 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightCorner, -0.2 * intensity);
      break;
    case "disgusted":
      scaleHarthmereRuntimeExpressionPart(leftEye, [0.96, 0.62, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.04, 0.72, 1]);
      rotateHarthmereRuntimeExpressionPart(leftBrow, -0.24 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightBrow, 0.08 * intensity);
      rotateHarthmereRuntimeExpressionPart(mouth, -0.24 * intensity);
      moveHarthmereRuntimeExpressionPart(mouth, 0.012 * intensity, 0, 0);
      break;
    case "loving":
      scaleHarthmereRuntimeExpressionPart(leftEye, [1.1, 0.66, 1]);
      scaleHarthmereRuntimeExpressionPart(rightEye, [1.1, 0.66, 1]);
      moveHarthmereRuntimeExpressionPart(leftBrow, 0, 0.018 * intensity, 0);
      moveHarthmereRuntimeExpressionPart(rightBrow, 0, 0.018 * intensity, 0);
      rotateHarthmereRuntimeExpressionPart(leftCorner, 0.34 * intensity);
      rotateHarthmereRuntimeExpressionPart(rightCorner, -0.34 * intensity);
      break;
    case "neutral":
    default:
      break;
...
function applyHarthmereRuntimeFacialExpressionToObject(
  root: THREE.Object3D,
  state: HarthmereFacialExpressionState
) {
  root.traverse((object) => {
    if (object.userData.harthmereRuntimeFaceExpressionRoot)
      applyHarthmereRuntimeFacialExpressionToFaceRoot(object, state);
  });
...
function addHarthmereRuntimeHumanAnchors(
  ...
  addHarthmereRuntimeAnchor(root, "harthmere-anchor-neck", [
    0,
    headY - 0.18,
    -0.005,
  ]);
  addHarthmereRuntimeAnchor(root, "harthmere-anchor-right-hand", [
    body.shoulderWidth / 2 + 0.04,
    shoulderY - body.armLength / 2,
    -0.03,
  ]);
  addHarthmereRuntimeAnchor(root, "harthmere-anchor-left-hand", [
    -(body.shoulderWidth / 2 + 0.04),
    shoulderY - body.armLength / 2,
    -0.03,
  ]);
  addHarthmereRuntimeAnchor(root, "harthmere-anchor-hip", [
    0.16,
    body.legLength + 0.08,
    0.1,
  ]);
  addHarthmereRuntimeAnchor(root, "harthmere-anchor-back", [
    0,
...
function addHarthmereRuntimeOutfitAndGearPolish(
  ...
  root.userData.harthmereModularClothingRuntime =
    "harthmere-modular-clothing-runtime-body-fit";
  ...
  root.userData.harthmereThreeJsClothingRenderer =
    "harthmere-threejs-clothing-body-fit";
  root.userData.harthmereHiddenBodyZones = clothingSlots.flatMap(
    (slot) =>
      appearance.clothing[slot as keyof typeof appearance.clothing]
        ?.hidesBodyZones ?? []
  );
  queueHarthmereRuntimeLicensedClothingModels(
    root,
    appearance.clothing as any,
    body
  );
  ...
  if (appearance.clothing.torso) {
    root.add(
      boxMesh(
        "townsperson-clothing-torso-body-fit",
        [body.torsoWidth + 0.05, body.torsoHeight + 0.04, 0.05],
        [0, torsoY, -0.14],
        palette.tunic
      ),
      boxMesh(
        "townsperson-clothing-collar-body-fit",
        [body.torsoWidth + 0.08, 0.035, 0.06],
        [0, torsoY + body.torsoHeight * 0.42, -0.165],
        accent
      ),
      boxMesh(
        "townsperson-clothing-hem-body-fit",
        [body.torsoWidth + 0.1, 0.035, 0.06],
        [0, torsoY - body.torsoHeight * 0.42, -0.165],
        trim
      )
    );
  ...
  if (appearance.clothing.legs) {
    root.add(
      boxMesh(
        "townsperson-clothing-left-trouser-body-fit",
        [body.legWidth + 0.035, body.legLength * 0.82, 0.045],
        [
          -(body.torsoWidth / 4 + body.legSpread),
          body.legLength * 0.54,
          -0.105,
        ],
        trim
      ...
      boxMesh(
        "townsperson-clothing-right-trouser-body-fit",
        [body.legWidth + 0.035, body.legLength * 0.82, 0.045],
        [body.torsoWidth / 4 + body.legSpread, body.legLength * 0.54, -0.105],
        trim
      )
    );
  }
  if (appearance.clothing.belt) {
    root.add(
      boxMesh(
        "townsperson-clothing-belt",
        [body.torsoWidth + 0.08, 0.045, 0.06],
        [0, body.legLength + 0.08, -0.14],
        leather
      ),
      boxMesh(
        "townsperson-clothing-buckle",
        [0.06, 0.055, 0.025],
        [0, body.legLength + 0.08, -0.18],
        metal
      )
    );
  }
  if (appearance.clothing.feet) {
    root.add(
      boxMesh(
        "townsperson-clothing-left-boot",
        [body.legWidth + 0.04, 0.07, 0.07],
        [-(body.torsoWidth / 4 + body.legSpread), 0.055, -0.09],
        0x171717
      ),
      boxMesh(
        "townsperson-clothing-right-boot",
        [body.legWidth + 0.04, 0.07, 0.07],
        [body.torsoWidth / 4 + body.legSpread, 0.055, -0.09],
        0x171717
      )
  ...
  root.add(
    boxMesh(
      "townsperson-collar-polish",
      [body.torsoWidth + 0.035, 0.032, 0.22],
      [0, torsoY + body.torsoHeight * 0.42, -0.03],
      accent
    ),
    boxMesh(
      "townsperson-hem-polish",
      [body.torsoWidth + 0.05, 0.032, 0.22],
      [0, torsoY - body.torsoHeight * 0.42, -0.03],
      accent
    ),
    boxMesh(
      "townsperson-left-cuff-polish",
      [body.armWidth + 0.035, 0.032, 0.12],
      [-body.shoulderWidth / 2, shoulderY - body.armLength * 0.42, -0.02],
      trim
    ),
    boxMesh(
      "townsperson-right-cuff-polish",
      [body.armWidth + 0.035, 0.032, 0.12],
      [body.shoulderWidth / 2, shoulderY - body.armLength * 0.42, -0.02],
      trim
    ),
    boxMesh(
      "townsperson-left-boot-polish",
      [body.legWidth + 0.035, 0.07, 0.14],
      [-(body.torsoWidth / 4 + body.legSpread), 0.05, -0.02],
      0x151515
    ),
    boxMesh(
      "townsperson-right-boot-polish",
      [body.legWidth + 0.035, 0.07, 0.14],
      [body.torsoWidth / 4 + body.legSpread, 0.05, -0.02],
      0x151515
    )
  ...
  if (appearance.equipment.head === "guard_helmet") {
    ...
    root.add(
      boxMesh(
        "farmer-straw-hat-brim-polish",
        [0.48, 0.035, 0.38],
        [0, headY + 0.2, -0.01],
        0xc7a45a
      ),
      boxMesh(
        "farmer-straw-hat-crown-polish",
        [0.26, 0.12, 0.24],
        [0, headY + 0.27, -0.01],
        0xd8b86a
      )
  ...
  if (/shield/i.test(appearance.equipment.offHand ?? "")) {
    root.add(
      boxMesh(
        "townsperson-left-shield-polish",
        [0.18, 0.24, 0.055],
        [
          -(body.shoulderWidth / 2 + 0.08),
          shoulderY - body.armLength * 0.25,
          -0.085,
        ],
        metal
      ),
      boxMesh(
        "townsperson-left-shield-boss-polish",
        [0.07, 0.07, 0.065],
        [
          -(body.shoulderWidth / 2 + 0.08),
          shoulderY - body.armLength * 0.25,
  ...
  if (
    /sword|dagger|knife|axe/i.test(appearance.equipment.mainHand ?? "") ||
    appearance.equipment.hip
  ) {
    const sheath = boxMesh(
      "townsperson-hip-sheath-polish",
      [0.045, 0.32, 0.065],
      [body.torsoWidth / 2 + 0.055, body.legLength + 0.08, 0.08],
      leather
    );
  ...
  if (
    /bow|quiver/i.test(
      `${appearance.equipment.mainHand ?? ""} ${
        appearance.equipment.back ?? ""
      }`
    )
  ) {
    const bow = boxMesh(
      "townsperson-back-bow-polish",
      [0.04, 0.58, 0.045],
      [body.torsoWidth / 2 + 0.06, torsoY + 0.05, 0.16],
      leather
    );
    bow.rotation.z = -0.22;
    root.add(
      bow,
      boxMesh(
        "townsperson-quiver-fletching-polish",
        [0.12, 0.05, 0.08],
        [body.torsoWidth / 2 + 0.02, torsoY + 0.28, 0.18],
        0xf0e6d2
      )
    );
  }
  if (/staff/i.test(appearance.equipment.mainHand ?? "")) {
    const staff = boxMesh(
      "townsperson-staff-polish",
      [0.035, 0.66, 0.035],
      [body.shoulderWidth / 2 + 0.12, shoulderY - 0.12, -0.05],
      leather
    );
    ...
    root.add(
      staff,
      boxMesh(
        "townsperson-staff-cap-polish",
        [0.07, 0.07, 0.07],
        [body.shoulderWidth / 2 + 0.1, shoulderY + 0.2, -0.05],
        0x6f5ca8
      )
    );
  }
  if (
    appearance.equipment.back === "merchant_satchel" ||
    appearance.equipment.accessory === "ledger_pouch"
  ) {
    root.add(
      boxMesh(
        "merchant-satchel-polish",
        [0.17, 0.18, 0.075],
        [-body.shoulderWidth / 2 - 0.02, torsoY, 0.13],
        0x7a4f2a
      )
    );
  }
  if (appearance.equipment.accessory === "clergy_sash") {
    root.add(
      boxMesh(
        "clergy-sash-polish",
        [0.07, body.torsoHeight + 0.04, 0.225],
        [-0.1, torsoY, -0.04],
        accent
      )
    );
  ...
  if (appearance.equipment.accessory === "torn_cloth") {
    root.add(
      boxMesh(
        "undead-torn-cloth-left-polish",
        [0.08, 0.18, 0.03],
        [-body.torsoWidth / 2 + 0.06, torsoY - 0.18, -0.13],
        trim
      ),
      boxMesh(
        "undead-torn-cloth-right-polish",
        [0.07, 0.14, 0.03],
        [body.torsoWidth / 2 - 0.04, torsoY - 0.21, -0.13],
        trim
      )
    );
  }
...
function addHarthmereRuntimeProductMinecraftClothingPolish(
  ...
  root.userData.harthmereRuntimeProductMinecraftPolish =
    HARTHMERE_RUNTIME_PRODUCT_MINECRAFT_POLISH_VERSION;
  root.userData.harthmereRuntimeProductMinecraftPolishSlots = slots;

  const add = (mesh: THREE.Object3D) => {
    mesh.userData.harthmereRuntimeProductMinecraftPolish =
      HARTHMERE_RUNTIME_PRODUCT_MINECRAFT_POLISH_VERSION;
    root.add(mesh);
    return mesh;
  };
  ...
  for (const slot of slots) {
    ...
    if (slot === "torso") {
      ...
      addBox(
        "townsperson-product-torso-front",
        [body.torsoWidth + 0.1, height, 0.055],
        [0, y, -0.18],
        armor ? metal : hunter || apron ? leather : palette.tunic
      );
      addBox(
        "townsperson-product-torso-trim",
        [body.torsoWidth + 0.15, 0.032, 0.065],
        [0, torsoY + body.torsoHeight * 0.43, -0.225],
        light
      );
      addBox(
        "townsperson-product-hem-trim",
        [body.torsoWidth + 0.15, 0.032, 0.065],
        [0, y - height * 0.48, -0.22],
        trim
      );
      if (armor) {
        addBox(
          "townsperson-product-left-pauldron",
          [0.16, 0.07, 0.22],
          [-(body.shoulderWidth / 2 + 0.02), shoulderY + 0.02, -0.035],
          metal
        );
        addBox(
          "townsperson-product-right-pauldron",
          [0.16, 0.07, 0.22],
          [body.shoulderWidth / 2 + 0.02, shoulderY + 0.02, -0.035],
          metal
        );
        addBox(
          "townsperson-product-tabard",
          [0.1, height * 0.82, 0.07],
          [0, y, -0.255],
          palette.accent
        );
      }
      if (robe) {
        addBox(
          "townsperson-product-robe-sash",
          [0.07, height * 0.96, 0.075],
          [-0.11, y, -0.25],
          palette.accent,
          [0, 0, -0.16]
        );
      ...
      if (apron) {
        addBox(
          "townsperson-product-apron",
          [body.torsoWidth * 0.72, height * 0.82, 0.07],
          [0, y - 0.02, -0.255],
          leather
        );
      }
      if (merchant) {
        addBox(
          "townsperson-product-left-lapel",
          [0.07, height * 0.62, 0.075],
          [-0.12, y + 0.04, -0.258],
          light,
          [0, 0, -0.08]
        );
        addBox(
          "townsperson-product-right-lapel",
          [0.07, height * 0.62, 0.075],
          [0.12, y + 0.04, -0.258],
          light,
          [0, 0, 0.08]
      ...
      if (torn) {
        addBox(
          "townsperson-product-rag-left",
          [0.12, 0.11, 0.075],
          [-(body.torsoWidth * 0.24), torsoY - 0.04, -0.28],
          trim,
          [0, 0, -0.14]
        );
        addBox(
          "townsperson-product-rag-right",
          [0.1, 0.1, 0.075],
          [body.torsoWidth * 0.25, torsoY + 0.1, -0.28],
          dark,
          [0, 0, 0.16]
        );
      }
      ...
      if (robe) {
        addBox(
          "townsperson-product-robe-skirt",
          [body.torsoWidth + 0.1, body.legLength * 0.82, 0.06],
          [0, body.legLength * 0.52, -0.155],
          palette.tunic
        );
      } else {
        addBox(
          "townsperson-product-left-knee",
          [body.legWidth + 0.055, 0.05, 0.06],
          [lx, body.legLength * 0.52, -0.14],
          armor ? metal : light
        );
        addBox(
          "townsperson-product-right-knee",
          [body.legWidth + 0.055, 0.05, 0.06],
          [rx, body.legLength * 0.52, -0.14],
          armor ? metal : light
        );
      }
    } else if (slot === "feet") {
      ...
      const rx = body.torsoWidth / 4 + body.legSpread;
      addBox(
        "townsperson-product-left-boot-cuff",
        [body.legWidth + 0.055, 0.045, 0.13],
        [lx, 0.13, -0.045],
        leather
      );
      addBox(
        "townsperson-product-right-boot-cuff",
        [body.legWidth + 0.055, 0.045, 0.13],
        [rx, 0.13, -0.045],
        leather
      );
      ...
      addBox(
        "townsperson-product-left-bracer",
        [body.armWidth + 0.035, 0.05, 0.12],
        [-body.shoulderWidth / 2, shoulderY - body.armLength * 0.24, -0.03],
        leather
      );
      addBox(
        "townsperson-product-right-bracer",
        [body.armWidth + 0.035, 0.05, 0.12],
        [body.shoulderWidth / 2, shoulderY - body.armLength * 0.24, -0.03],
        leather
      );
      ...
      addBox(
        "townsperson-product-belt-pouch-left",
        [0.085, 0.105, 0.055],
        [-(body.torsoWidth * 0.36), body.legLength + 0.04, -0.16],
        dark
      );
      addBox(
        "townsperson-product-belt-pouch-right",
        [0.075, 0.095, 0.055],
        [body.torsoWidth * 0.35, body.legLength + 0.035, -0.16],
        dark
      );
    } else if (slot === "head") {
      if (/helmet|guard|halfhelm/i.test(variant)) {
        addBox(
          "townsperson-product-helmet-brow",
          [0.46, 0.035, 0.05],
          [0, headY + 0.15, -0.16],
          dark
        );
        ...
        addBox(
          "townsperson-product-hood-drape",
          [0.36, 0.16, 0.055],
          [0, headY + 0.04, 0.13],
          dark
        );
      ...
      if (/cape|cloak|shroud/i.test(variant)) {
        addBox(
          "townsperson-product-cape-clasp",
          [0.1, 0.05, 0.06],
        ...
        addBox(
          "townsperson-product-pack-flap",
          [0.18, 0.1, 0.075],
          [0.04, torsoY + 0.08, 0.215],
          trim
        );
      }
    }
...
function ensureHarthmereLargeBodyClothingVisibility(
  ...
  root.traverse((child) => {
    ...
    if (
      !/(townsperson-(clothing|product|collar|hem|left-boot|right-boot)|merchant-|clergy-|guard-|bandit-|farmer-|undead-)/i.test(
        child.name
      )
    ) {
      return;
...
function addHarthmereRuntimeOutsideClothingShell(
  ...
  if (clothing?.torso) {
    mark(
      boxMesh(
        "runtime-outside-clothing-torso-front",
        [torsoWidth, torsoHeight, 0.04],
        [0, torsoY, frontZ],
        cloth
      )
    );
    mark(
      boxMesh(
        "runtime-outside-clothing-torso-back",
        [torsoWidth, torsoHeight, 0.04],
        [0, torsoY, backZ],
        cloth
      )
    ...
    mark(
      boxMesh(
        "runtime-outside-clothing-torso-left",
        [0.04, torsoHeight, torsoDepth + 0.04],
        [-sideX, torsoY, 0],
        cloth
      )
    );
    mark(
      boxMesh(
        "runtime-outside-clothing-torso-right",
        [0.04, torsoHeight, torsoDepth + 0.04],
        [sideX, torsoY, 0],
        cloth
      )
    ...
    mark(
      boxMesh(
        "runtime-outside-clothing-collar",
        [torsoWidth + 0.03, 0.04, torsoDepth + 0.05],
        [0, torsoY + torsoHeight * 0.48, 0],
        trim
      )
    );
    mark(
      boxMesh(
        "runtime-outside-clothing-hem",
        [torsoWidth + 0.04, 0.045, torsoDepth + 0.055],
        [0, torsoY - torsoHeight * 0.49, 0],
        trim
      )
    );
  }
  ...
  if (clothing?.legs) {
    mark(
      boxMesh(
        "runtime-outside-clothing-left-leg",
        [legWidth, legLength, body.legDepth + 0.045],
        [-legX, body.legLength * 0.52, -0.035],
        pants
      )
    );
    mark(
      boxMesh(
        "runtime-outside-clothing-right-leg",
        [legWidth, legLength, body.legDepth + 0.045],
        [legX, body.legLength * 0.52, -0.035],
        pants
      )
  ...
  if (clothing?.feet) {
    mark(
      boxMesh(
        "runtime-outside-clothing-left-boot",
        [legWidth + 0.045, 0.1, body.legDepth + 0.065],
        [-legX, 0.055, -0.04],
        dark
      )
    );
    mark(
      boxMesh(
        "runtime-outside-clothing-right-boot",
        [legWidth + 0.045, 0.1, body.legDepth + 0.065],
        [legX, 0.055, -0.04],
        dark
      )
  ...
  if (clothing?.belt) {
    mark(
      boxMesh(
        "runtime-outside-clothing-belt",
        [torsoWidth + 0.055, 0.055, torsoDepth + 0.065],
        [0, waistY, 0],
        leather
      )
    );
    mark(
      boxMesh(
        "runtime-outside-clothing-buckle",
        [0.075, 0.07, 0.04],
        [0, waistY, frontZ - 0.035],
        0xd0b56b
      )
    );
  }
  ...
  if (clothing?.hands) {
    ...
    mark(
      boxMesh(
        "runtime-outside-clothing-left-sleeve",
        [armWidth, armLength, body.armDepth + 0.04],
        [
          -(body.shoulderWidth / 2 + 0.015),
          shoulderY - armLength * 0.45,
          -0.03,
        ],
        cloth
      )
    ...
    mark(
      boxMesh(
        "runtime-outside-clothing-right-sleeve",
        [armWidth, armLength, body.armDepth + 0.04],
        [body.shoulderWidth / 2 + 0.015, shoulderY - armLength * 0.45, -0.03],
        cloth
      )
    );
  }
...
function addHarthmereRuntimeAlwaysVisibleNpcClothing(
  ...
  const add = (mesh: THREE.Object3D) => {
    mesh.userData.harthmereRuntimeAlwaysVisibleNpcClothing = true;
    root.add(mesh);
    return mesh;
  };
  add(
    boxMesh(
      "runtime-always-visible-clothing-torso-front",
      [torsoWidth, torsoHeight, 0.045],
      [0, torsoY, frontZ],
      cloth
    )
  );
  add(
    boxMesh(
      "runtime-always-visible-clothing-torso-back",
      [torsoWidth, torsoHeight, 0.045],
      [0, torsoY, backZ],
      cloth
    )
  ...
  add(
    boxMesh(
      "runtime-always-visible-clothing-torso-left",
      [0.045, torsoHeight, torsoDepth],
      [-sideX, torsoY, 0],
      cloth
    )
  );
  add(
    boxMesh(
      "runtime-always-visible-clothing-torso-right",
      [0.045, torsoHeight, torsoDepth],
      [sideX, torsoY, 0],
      cloth
    )
  ...
  add(
    boxMesh(
      "runtime-always-visible-clothing-collar",
      [torsoWidth + 0.04, 0.045, torsoDepth + 0.035],
      [0, torsoY + torsoHeight * 0.48, 0],
      trim
    )
  );
  add(
    boxMesh(
      "runtime-always-visible-clothing-hem",
      [torsoWidth + 0.04, 0.05, torsoDepth + 0.04],
      [0, torsoY - torsoHeight * 0.49, 0],
      trim
    )
  );
  add(
    boxMesh(
      "runtime-always-visible-clothing-belt",
      [torsoWidth + 0.05, 0.06, torsoDepth + 0.045],
      [0, waistY, 0],
      leather
    )
  );
  add(
    boxMesh(
      "runtime-always-visible-clothing-buckle",
      [0.08, 0.07, 0.04],
      [0, waistY, frontZ - 0.04],
      0xd0b56b
    )
  );
  add(
    boxMesh(
      "runtime-always-visible-clothing-left-sleeve",
      [armWidth, armLength, body.armDepth + 0.05],
      [-(body.shoulderWidth / 2 + 0.02), shoulderY - armLength * 0.46, -0.035],
      cloth
    )
  );
  add(
    boxMesh(
      "runtime-always-visible-clothing-right-sleeve",
      [armWidth, armLength, body.armDepth + 0.05],
      [body.shoulderWidth / 2 + 0.02, shoulderY - armLength * 0.46, -0.035],
      cloth
    )
  ...
  add(
    boxMesh(
      "runtime-always-visible-clothing-left-pants",
      [legWidth, legLength, body.legDepth + 0.055],
      [-legX, body.legLength * 0.52, -0.035],
      pants
    )
  );
  add(
    boxMesh(
      "runtime-always-visible-clothing-right-pants",
      [legWidth, legLength, body.legDepth + 0.055],
      [legX, body.legLength * 0.52, -0.035],
      pants
    )
  ...
  add(
    boxMesh(
      "runtime-always-visible-clothing-left-boot",
      [legWidth + 0.055, 0.105, body.legDepth + 0.07],
      [-legX, 0.06, -0.04],
      dark
    )
  );
  add(
    boxMesh(
      "runtime-always-visible-clothing-right-boot",
      [legWidth + 0.055, 0.105, body.legDepth + 0.07],
      [legX, 0.06, -0.04],
      dark
    )
...
function setHarthmereRuntimeNpcMeshColor(
  object: THREE.Object3D | undefined,
  color: number
) {
...
function addHarthmereRuntimeNpcClothingMaterialRecolor(
  ...
  const changed = [
    setHarthmereRuntimeNpcMeshColor(
      root.getObjectByName("townsperson-body"),
      cloth
    ),
    setHarthmereRuntimeNpcMeshColor(
      root.getObjectByName("townsperson-left-arm"),
      cloth
    ),
    setHarthmereRuntimeNpcMeshColor(
      root.getObjectByName("townsperson-right-arm"),
      cloth
    ),
    setHarthmereRuntimeNpcMeshColor(
      root.getObjectByName("townsperson-left-leg"),
      pants
    ),
    setHarthmereRuntimeNpcMeshColor(
      root.getObjectByName("townsperson-right-leg"),
      pants
    ),
  ...
  add(
    boxMesh(
      "runtime-material-clothing-collar",
      [torsoWidth + 0.08, 0.055, torsoDepth],
      [0, torsoY + body.torsoHeight * 0.45, -0.03],
      trim
    )
  );
  add(
    boxMesh(
      "runtime-material-clothing-hem",
      [torsoWidth + 0.1, 0.06, torsoDepth],
      [0, torsoY - body.torsoHeight * 0.48, -0.03],
      trim
    )
  );
  add(
    boxMesh(
      "runtime-material-clothing-belt",
      [torsoWidth + 0.12, 0.075, torsoDepth + 0.04],
      [0, waistY, -0.035],
      leather
    )
  );
  add(
    boxMesh(
      "runtime-material-clothing-buckle",
      [0.1, 0.09, 0.055],
      [0, waistY, -0.24],
      0xd0b56b
    )
  ...
  add(
    boxMesh(
      "runtime-material-clothing-left-cuff",
      [Math.max(body.armWidth + 0.05, 0.13), 0.08, 0.14],
      [-(body.shoulderWidth / 2), shoulderY - body.armLength * 0.52, -0.06],
      trim
    )
  );
  add(
    boxMesh(
      "runtime-material-clothing-right-cuff",
      [Math.max(body.armWidth + 0.05, 0.13), 0.08, 0.14],
      [body.shoulderWidth / 2, shoulderY - body.armLength * 0.52, -0.06],
      trim
    )
  ...
  add(
    boxMesh(
      "runtime-material-clothing-left-boot",
      [Math.max(body.legWidth + 0.08, 0.18), 0.13, 0.22],
      [-legX, 0.065, -0.08],
      dark
    )
  );
  add(
    boxMesh(
      "runtime-material-clothing-right-boot",
      [Math.max(body.legWidth + 0.08, 0.18), 0.13, 0.22],
      [legX, 0.065, -0.08],
      dark
    )
...
function addHarthmereRuntimeProceduralWeaponSlot(
  ...
  if (/staff|wand|magic|scholar|mage|arcane/i.test(weaponId)) {
    add(
      boxMesh(
        "runtime-procedural-weapon-staff-shaft",
        [0.035, body.armLength + 0.34, 0.035],
        [handX + 0.06, handY + 0.14, -0.18],
        wood
      )
    );
    add(
      boxMesh(
        "runtime-procedural-weapon-staff-gem",
        [0.09, 0.09, 0.09],
        [handX + 0.06, handY + body.armLength * 0.72, -0.18],
        arcane
      )
    );
  } else if (/bow|ranged|hunter/i.test(weaponId)) {
    const bow = boxMesh(
      "runtime-procedural-weapon-bow",
      [0.035, body.armLength + 0.18, 0.035],
      [handX + 0.08, handY + 0.12, -0.2],
      wood
    );
    bow.rotation.z = -0.28;
    add(bow);
    add(
      boxMesh(
        "runtime-procedural-weapon-bow-string",
        [0.012, body.armLength + 0.02, 0.012],
        [handX + 0.15, handY + 0.12, -0.205],
        0xf8fafc
      )
    );
    ...
    add(
      boxMesh(
        "runtime-procedural-weapon-tool-handle",
        [0.035, body.armLength + 0.16, 0.035],
        [handX + 0.05, handY + 0.08, -0.18],
        wood
      )
    );
    add(
      boxMesh(
        "runtime-procedural-weapon-tool-head",
        [0.16, 0.08, 0.05],
        [handX + 0.05, handY + body.armLength * 0.58, -0.18],
        metal
      )
    );
  } else {
    const blade = boxMesh(
      "runtime-procedural-weapon-sword-blade",
      [0.045, body.armLength + 0.18, 0.035],
      [handX + 0.06, handY + 0.12, -0.18],
      metal
    );
    ...
    add(
      boxMesh(
        "runtime-procedural-weapon-sword-hilt",
        [0.14, 0.035, 0.045],
        [handX + 0.04, handY - body.armLength * 0.32, -0.18],
        leather
      )
    );
  }
...
function addHarthmereProceduralBodyExpression(
  ...
  add(
    boxMesh(
      "townsperson-neck",
      [
        Math.max(body.torsoWidth * 0.24, 0.11),
        Math.max(expressionHeadHeight * 0.22, 0.07),
        0.13,
      ],
      [0, neckY, 0],
      skin
...
function createProceduralTownsperson(
  ...
  const appearance = harthmereRuntimeAppearanceForPlacement(placement);
  const palette = harthmereRuntimePaletteForAppearance(
    placement.asset,
    appearance
  );
  ...
  root.scale.setScalar(
    (placement.scale ?? 1) * HARTHMERE_PROCEDURAL_TOWNSPERSON_WORLD_SCALE
  );
  ...
  root.userData.harthmereForceProceduralTownspersonClothing =
    HARTHMERE_FORCE_PROCEDURAL_TOWNSPERSON_CLOTHING_VERSION;
  root.userData.harthmereForceProceduralTownspersonClothingKeys = Object.keys(
    (appearance.clothing ?? {}) as Record<string, unknown>
  );
  root.userData.harthmereForceProceduralTownspersonClothingKeys = Object.keys(
    (appearance.clothing ?? {}) as Record<string, unknown>
  );
  ...
  root.add(
    boxMesh(
      ...
      [body.legWidth, body.legLength, body.legDepth],
    ...
    boxMesh(
      "townsperson-body",
      [body.torsoWidth, body.torsoHeight, body.torsoDepth],
      [0, torsoY, 0],
      outfitTint
    ),
    boxMesh(
      "townsperson-left-arm",
      [body.armWidth, body.armLength, body.armDepth],
      [-body.shoulderWidth / 2, shoulderY, 0],
      palette.skin
    ),
    boxMesh(
  ...
  addHarthmereRuntimeNpcClothingMaterialRecolor(
    root,
    appearance,
    body,
    palette,
  ...
  addHarthmereRuntimeAlwaysVisibleNpcClothing(
    root,
    appearance,
    body,
    palette,
  ...
  addHarthmereRuntimeOutfitAndGearPolish(
    root,
    appearance,
    body,
    palette,
  ...
  addHarthmereRuntimeProductMinecraftClothingPolish(
    root,
    appearance,
    body,
    palette,
  ...
  addHarthmereRuntimeProceduralWeaponSlot(
    root,
    appearance.clothing,
    body,
    palette
  );
  addHarthmereRuntimeOutsideClothingShell(
    root,
    appearance.clothing,
    body,
    palette
  ...
  if (appearance.role === "guard") {
    root.add(
      boxMesh(
        "guard-tabard",
        [body.torsoWidth + 0.02, 0.12, 0.22],
        [0, torsoY + body.torsoHeight * 0.28, -0.02],
        0x141414
      ),
      boxMesh(
        "guard-belt",
        [body.torsoWidth + 0.08, 0.045, 0.23],
        [0, body.legLength + 0.08, -0.015],
        0x222222
      )
    );
    ...
    root.add(
      boxMesh(
        "clergy-sash",
        [0.08, body.torsoHeight + 0.02, 0.22],
        [-0.1, torsoY, -0.03],
        palette.accent
      )
    );
    ...
    root.add(
      ...
      boxMesh(
        "bandit-knife-belt",
        [0.16, 0.035, 0.08],
        [body.torsoWidth / 2 + 0.04, body.legLength + 0.05, 0.08],
        0x8c2f2a
      )
    );
  }
...
function collectAnimalLeg(object: THREE.Object3D, names: string[]) {
  for (const name of names) {
...
function harthmereGaitBaseY(node: THREE.Object3D): number {
  const existing = node.userData.harthmereGaitBaseY;
  if (typeof existing === "number") {
    return existing;
  }
  node.userData.harthmereGaitBaseY = node.position.y;
  return node.position.y;
}

function harthmereGaitBaseRotationX(node: THREE.Object3D): number {
  const existing = node.userData.harthmereGaitBaseRotationX;
  if (typeof existing === "number") {
    return existing;
  }
  node.userData.harthmereGaitBaseRotationX = node.rotation.x;
...
function applyAnimalLegSwing(
  ...
  let frontAmplitude = 0.34;
  if (/horse|cow|bear/.test(asset)) {
    frontAmplitude = 0.42;
  } else if (/wolf|dog|fox|boar|deer|sheep|pig/.test(asset)) {
    frontAmplitude = 0.5;
  }
  const backAmplitude =
    frontAmplitude *
    (/frog|snake|crow|pigeon|chicken|bunny|rat/.test(asset) ? 0.82 : 0.94);
  ...
  if (frontLeft) {
    frontLeft.rotation.x =
      harthmereGaitBaseRotationX(frontLeft) + stride * frontAmplitude;
    frontLeft.position.y = harthmereGaitBaseY(frontLeft) + sway;
    animatedNames.push(frontLeft.name);
  }
  if (frontRight) {
    frontRight.rotation.x =
      harthmereGaitBaseRotationX(frontRight) - stride * frontAmplitude;
    frontRight.position.y = harthmereGaitBaseY(frontRight) - sway;
    animatedNames.push(frontRight.name);
  }
  if (backLeft) {
    backLeft.rotation.x =
      harthmereGaitBaseRotationX(backLeft) - stride * backAmplitude;
    backLeft.position.y = harthmereGaitBaseY(backLeft) - sway;
    animatedNames.push(backLeft.name);
  }
  if (backRight) {
    backRight.rotation.x =
      harthmereGaitBaseRotationX(backRight) + stride * backAmplitude;
    backRight.position.y = harthmereGaitBaseY(backRight) + sway;
  ...
  if (tail) {
    tail.rotation.x =
      harthmereGaitBaseRotationX(tail) + Math.sin(time * 5.5) * 0.12;
  }
  ...
  if (head) {
    head.rotation.x =
      harthmereGaitBaseRotationX(head) + Math.sin(time * 3.8) * 0.05;
  }
...
function resetHarthmereCutscenePose(object: THREE.Object3D) {
  ...
  if (!baseScale) {
    ...
    object.scale.fromArray(baseScale);
  }
  // Cinematic root reactions are reapplied from a clean pose every tick. The
  // world-facing yaw is set by the bridge immediately before this function.
  object.rotation.x = 0;
  object.rotation.z = 0;
  object.traverse((node) => {
    const baseY = node.userData.harthmereGaitBaseY;
    if (typeof baseY === "number") {
      node.position.y = baseY;
    }
    const baseRotationX = node.userData.harthmereGaitBaseRotationX;
    if (typeof baseRotationX === "number") {
      node.rotation.x = baseRotationX;
    }
  });
  for (const name of CUTSCENE_LIMB_NAMES) {
    const node = object.getObjectByName(name);
    if (node) {
...
function applyHarthmereCutscenePose(
  object: THREE.Object3D,
  record: HarthmereLiveCreatureBridgeRecord
) {
  resetHarthmereCutscenePose(object);
  const leftLeg = object.getObjectByName("townsperson-left-leg");
  const rightLeg = object.getObjectByName("townsperson-right-leg");
  const leftArm = object.getObjectByName("townsperson-left-arm");
  const rightArm = object.getObjectByName("townsperson-right-arm");
  const time = record.animationTime ?? record.motionTime ?? 0;

  if (record.moving) {
    if (isProceduralAnimalKey(record.asset)) {
      applyAnimalLegSwing(object, record.motionTime ?? time, record.asset);
    } else {
      const stride = Math.sin((record.motionTime ?? time) * 7) * 0.42;
      if (leftLeg) leftLeg.rotation.x = stride;
      if (rightLeg) rightLeg.rotation.x = -stride;
      if (leftArm) leftArm.rotation.x = -stride * 0.65;
      if (rightArm) rightArm.rotation.x = stride * 0.65;
    }
  }

  const wave = Math.sin(time * 8);
  if (!applyHarthmereCinematicExpressionPose(object, record.animation, time)) {
    switch (record.animation) {
      case "talkGesture":
      case "vendorIdle":
        if (leftArm) leftArm.rotation.x = -0.3 + wave * 0.15;
        if (rightArm) rightArm.rotation.x = -0.55 - wave * 0.2;
        break;
      case "questGesture":
      case "point":
        if (rightArm) rightArm.rotation.x = -1.25;
        if (rightArm) rightArm.rotation.z = -0.18;
        break;
      case "wave":
        if (rightArm) rightArm.rotation.x = -1.55;
        if (rightArm) rightArm.rotation.z = wave * 0.45;
        break;
      case "applause":
        if (leftArm) leftArm.rotation.x = -1.1 + wave * 0.15;
        if (rightArm) rightArm.rotation.x = -1.1 - wave * 0.15;
      ...
      case "attack1":
      case "attack2":
        object.rotation.x = -0.12 + Math.sin(time * 10) * 0.08;
        object.rotation.z = Math.sin(time * 10) * 0.13;
        if (rightArm) rightArm.rotation.x = -1.7 + Math.sin(time * 10) * 0.7;
        if (leftArm) leftArm.rotation.x = -0.45;
        break;
      case "hitReact": {
        const hitWave = Math.sin(Math.min(1, time / 0.45) * Math.PI);
        object.rotation.x = 0.18 * hitWave;
        object.rotation.z = -0.34 * hitWave;
        if (leftArm) leftArm.rotation.x = -0.7 * hitWave;
        if (rightArm) rightArm.rotation.x = -0.95 * hitWave;
        break;
      }
      case "death": {
        const fall = Math.min(1, time / 0.65);
        object.rotation.x = -Math.PI * 0.5 * fall;
        object.rotation.z = 0.12 * Math.sin(fall * Math.PI);
        object.position.y -= 0.38 * fall;
        object.scale.multiplyScalar(1 - 0.14 * fall);
        break;
      }
      case "sit":
      ...
      case "eat":
      case "drink":
        if (rightArm) rightArm.rotation.x = -1.4 + wave * 0.08;
        break;
      case "workLoop":
      case "crowdEmote":
        if (leftArm) leftArm.rotation.x = -0.8 + wave * 0.45;
        if (rightArm) rightArm.rotation.x = -0.8 - wave * 0.45;
        break;
      case "sleep":
        if (leftArm) leftArm.rotation.x = -0.25;
        if (rightArm) rightArm.rotation.x = -0.25;
        if (leftLeg) leftLeg.rotation.x = 0.15;
        if (rightLeg) rightLeg.rotation.x = -0.15;
        break;
    }
  }
  object.userData.harthmereCutsceneAnimation = {
...
const HARTHMERE_OBJECT_EFFECT_RANGE_VERSION = "harthmere-object-effect-range";
...
function applyHarthmereNpcRouteDistribution(
  ...
  movedActors: number;
  localDensityLimits: {
    maxActorsWithin12m: number;
    maxActorsWithin20m: number;
  };
} {
  let movedActors = 0;
  ...
  const distributed: RuntimePlacement[] = placements.map(
    (placement): RuntimePlacement => {
      ...
      movedActors += 1;
      const [x, z] = route[0];
      const next = placementWithHarthmereRuntimeAt(placement, [
        x,
        placement.at[1],
        z,
      ]);
      ...
      return {
        ...
        name: `${
          placement.name ?? placement.asset
        } route-dispersed ${HARTHMERE_NPC_ROUTE_DISTRIBUTION_VERSION}`,
      };
    }
  );
  return {
...
const HARTHMERE_NPC_DISTRIBUTION =
  applyHarthmereNpcRouteDistribution(PLACEMENTS);
...
export const HARTHMERE_BUILDING_VOXEL_MESHES_MARKER =
  "harthmere-building-voxel-meshes" as const;
const HARTHMERE_RUNTIME_PLACEMENT_CLEANUP =
  applyHarthmereRuntimePlacementCleanup(HARTHMERE_NPC_DISTRIBUTION.placements);
const RUNTIME_PLACEMENTS = HARTHMERE_RUNTIME_PLACEMENT_CLEANUP.placements;

interface NativeCutsceneActor {
  object: THREE.Object3D;
  mixer: THREE.AnimationMixer;
...
export class HarthmereRuntimeAssetsRenderer implements Renderer {
  ...
  private readonly harthmerePlayerSwordClipActions = new Map<
    string,
  ...
  private readonly harthmereProjectileVisuals =
  ...
  // bridge updates.
  private readonly harthmereEcsLiveCreatures = new Map<
    number,
    THREE.Object3D
  >();
  private readonly nativeCutsceneActors = new Map<
    number,
    NativeCutsceneActor
  >();
  private readonly nativeCutsceneActorLoads = new Map<number, Promise<void>>();
  private readonly latestSyntheticCutsceneRecords = new Map<
    number,
  ...
  draw(scenes: Scenes, dt: number) {
    ...
    for (const instance of this.animated) {
      if (
        ...
        !this.deadCombatObjects.has(instance.object)
      ) {
        continue;
      }
      const dxFromCamera = instance.object.position.x - camX;
      const dzFromCamera = instance.object.position.z - camZ;
      const distSqFromCamera =
        dxFromCamera * dxFromCamera + dzFromCamera * dzFromCamera;
      ...
      if (
        !shouldUpdateMotion &&
        (instance.wander || instance.bob || instance.spin)
      ) {
        continue;
      }
      if (instance.wander) {
        ...
        const jitterSeed = Math.abs(
          (instance.base[0] * 13.37 + instance.base[2] * 7.31) % 1
        );
        ...
        const dx = nextRoutePoint
          ? nextRoutePoint[0] - instance.base[0]
          : Math.cos(angle) * instance.wander.radius * radiusJitter;
        const dz = nextRoutePoint
          ? nextRoutePoint[1] - instance.base[2]
          : Math.sin(angle) * instance.wander.radius * radiusJitter;
        ...
        if (speedSq > HARTHMERE_NPC_COLLISION_MIN_MOVE_SQ) {
          ...
          if (
            instance.locomotion &&
            (instance.locomotion.walk || instance.locomotion.run)
          ) {
    ...
    if (updateCombatAuxiliary) {
      this.harthmereCombatAuxiliaryUpdateIn = 0.25;
    }
    for (const instance of this.combatLifeInstances) {
      this.applyCombatPulse(instance);
      if (updateCombatAuxiliary) {
        this.tickHarthmereCreatureIdleSound(instance);
      }
      if (updateCombatAuxiliary && instance.bossVisual) {
        applyHarthmereScratchBossDamagePose(
          instance.object,
          instance.bossVisual.id,
          Number(instance.object.userData.harthmereBossHealthRatio ?? 1)
        );
      }
    }
  ...
  private resolveHarthmereRuntimePlacement(
    ...
    const startY = Number.isFinite(placement.at[1])
      ? placement.at[1]
      : GROUND_Y;
    ...
    const actorRadius = Math.max(
      0.42,
      Math.min(0.92, (placement.scale ?? 1) * 0.58)
    );
    const isIgnoredActorSpawnObstacle = (
      obstacle?: HarthmereNpcCollisionObstacle
    ) => {
      if (!obstacle) {
        return true;
    ...
    const crowded = this.spawnedLifePositions.some(
      ([x, z]) =>
        distanceSq2d(x, z, startX, startZ) <
        1.6 * 1.6 /* HARTHMERE_POLISH_NPC_SPACING */
    );
    ...
    for (const radius of rings) {
      for (let i = 0; i < 32; i += 1) {
        ...
        if (
          this.spawnedLifePositions.some(
            ([sx, sz]) =>
              distanceSq2d(sx, sz, x, z) <
              1.6 * 1.6 /* HARTHMERE_POLISH_NPC_SPACING */
          )
        ) {
          continue;
  ...
  private registerHarthmerePlacementInstance(
    placement: RuntimePlacement,
    object: THREE.Object3D
  ) {
    const meta =
    ...
    object.userData.harthmereTownSystemsVersion =
      HARTHMERE_TOWN_SYSTEMS_VERSION;
  ...
  private updateHarthmerePlacementLod(dt: number, camera?: THREE.Camera) {
    ...
    const structuralGroups = new Map<
      string,
      { x: number; z: number; count: number; tier: HarthmereLodTier }
    >();
    if (origin) {
    ...
    if (origin) {
      for (const [key, group] of structuralGroups) {
        ...
        structuralVisibility.set(
          ...
          shouldShowHarthmerePlacementAtDistanceSq(
            group.tier,
            distanceSq2d(origin[0], origin[1], gx, gz)
          )
        );
      }
    }
  ...
  private resolveHarthmereNpcWanderPosition(
    ...
    if (
      !sweepHarthmereNpcCollisionObstacle(currentX, currentZ, nextX, currentZ)
    ) {
      candidates.push([nextX, groundedNextY, currentZ]);
    }
    if (
      !sweepHarthmereNpcCollisionObstacle(currentX, currentZ, currentX, nextZ)
    ) {
    ...
    const fallback = instance.lastSafePosition ?? [
      currentX,
      currentY,
      currentZ,
    ];
    ...
    const position: [number, number, number] = [
      fallback[0],
      harthmereNpcGroundedY(instance, fallback[1], fallback[0], fallback[2]),
      fallback[2],
    ];
  ...
  private onFacialExpressionEvent = (event: Event) => {
    ...
    debugHarthmereRenderer("renderer.facial_expression.apply", {
    ...
    if (remaining > 0) {
      window.setTimeout(() => {
        ...
        applyHarthmereRuntimeFacialExpressionToObject(
          actor.object,
          makeHarthmereFacialExpressionState({
            actorId: state.actorId ?? String(actor.combatOffset ?? actor.label),
            expression: "neutral",
            source: "ambient",
  ...
  private findCombatLifeForFacialExpression(
    detail: HarthmereFacialExpressionState
  ) {
    ...
    return this.combatLifeInstances.find(
      (actor) =>
        actor.label.toLowerCase() === lowered ||
        actor.label.toLowerCase().includes(lowered) ||
        actor.asset.toLowerCase() === lowered
    );
  }
  ...
  private publishCombatActorSnapshot(camera?: THREE.Camera) {
    ...
    for (const actor of this.combatLifeInstances) {
      ...
      if (
        actor.combatOffset === undefined ||
        !Number.isFinite(actor.combatOffset)
      ) {
        continue;
      }
      const { radius, species, behavior, socialRole, family, clips } =
        actor.snapshotStatic;
      ...
      positions[String(actor.combatOffset)] = {
        ...
        screen,
        radius,
        forward: harthmereWorldForwardForYaw(
        ...
        facialExpression:
          actor.object.userData.harthmereFacialExpression ??
          actor.appearance?.facialExpression,
  ...
  private attachHarthmereTownWalkDebugMetadata(
    ...
    const expectedCollisionReason =
      harthmereTownDebugExpectedCollisionReason(placement);
    object.userData.harthmereTownWalkDebug = {
      ...
      isActor:
        harthmereTownDebugIsActorAsset(placement.asset) ||
        placement.combatOffset !== undefined,
  ...
  private harthmereTownWalkDebugPlayerSnapshot() {
    ...
    if (runtime && Array.isArray(runtime.position)) {
      const runtimePosition = runtime.position.map((value: unknown) =>
        Number(value)
      );
      if (
  ...
  private harthmereTownWalkDebugObjectReport(
    ...
    const isActor =
      meta?.isActor === true || harthmereTownDebugIsActorAsset(asset);
    const flags: string[] = [];
    ...
    if (
      !isActor &&
      /coin|key|mug|cup|plate|bread|apple|carrot|bottle|vial|book|scroll|candle|knife|spoon/i.test(
        String(asset ?? object.name)
      ) &&
      maxDim > 5.5
    ) {
    ...
    if (
      !isActor &&
      /wall|tower|church|chapel|warehouse|hall|house|roof|building/i.test(
        String(asset ?? object.name)
      ) &&
      maxDim < 2.0
    ) {
    ...
    return {
      ...
      distanceToPlayer:
        distanceToPlayer === undefined
          ? undefined
          : harthmereTownDebugRound(distanceToPlayer),
      placement: meta,
  ...
  private harthmereTownWalkDebugSnapshot(
    options: {
      radius?: number;
    ...
    const allReports = this.root.children
      .map((child) =>
        this.harthmereTownWalkDebugObjectReport(child, playerPosition)
      )
      .filter((report): report is HarthmereTownWalkDebugObjectReport =>
        Boolean(report)
      );
    ...
    if (Number.isFinite(radius) && playerPosition) {
      objects = objects.filter(
        (report) =>
          report.distanceToPlayer !== undefined &&
          report.distanceToPlayer <= radius
      );
    }
    ...
    const report: HarthmereTownWalkDebugReport & { overlaps?: unknown[] } = {
      ...
      counts: {
        ...
        actors: objects.filter(
          (entry) =>
            entry.placement &&
            (entry.placement as Record<string, unknown>).isActor === true
        ).length,
        suspects: suspects.length,
        animatedProps: suspects.filter((entry) =>
          entry.flags.includes("animated_prop_possible_random_open_close")
        ).length,
        walkThroughSuspects: suspects.filter((entry) =>
          entry.flags.some((flag) =>
            flag.startsWith("walk_through_collision_missing")
          )
        ).length,
        groundSuspects: suspects.filter((entry) =>
          entry.flags.some(
            (flag) =>
              flag.includes("ground") ||
              flag.includes("floating") ||
              flag.includes("sunk")
          )
        ).length,
      },
    ...
    if (options.includeOverlaps) {
      const boxed = this.root.children
        ...
        .map((child) => {
          const objectReport = this.harthmereTownWalkDebugObjectReport(
            child,
            playerPosition
          );
        ...
        .filter(
          (
            ...
            objectReport: HarthmereTownWalkDebugObjectReport;
            box: THREE.Box3;
          } => Boolean(entry)
        )
        .filter(
          (entry) =>
            ...
            !playerPosition ||
            (entry.objectReport.distanceToPlayer !== undefined &&
              entry.objectReport.distanceToPlayer <= radius)
        )
        .slice(0, 180);
      ...
      for (let i = 0; i < boxed.length; i += 1) {
        for (let j = i + 1; j < boxed.length; j += 1) {
          if (
            !harthmereTownDebugBoxXZOverlap(boxed[i].box, boxed[j].box, 0.08)
          ) {
            continue;
          }
  ...
  private harthmereTownWalkDebugAddOverlayForReports(
    ...
    const byName = new Map(
      this.root.children.map((child) => [child.name, child])
    );
    for (const report of reports.slice(0, 160)) {
      ...
      const color = new THREE.Color(
        report.flags.length > 0 ? 0xff3333 : 0x33ccff
      );
  ...
  private createHarthmereTownWalkDebugApi() {
    ...
    return {
      ...
      help: () => ({
        summary:
          "Walk around, then run __harthmereTownWalkDebug.sample('note') at bad spots. Export with __harthmereTownWalkDebug.download().",
        commands: [
      ...
      setPlayer: (x: number, z: number, y = GROUND_Y) => {
        const win = window as typeof window & Record<string, unknown>;
        win.__harthmereTownWalkDebugManualPlayer = [
          Number(x),
          Number(z),
          Number(y),
        ];
      ...
      where: () => {
        ...
        return {
          ...
          insideObstacles: position
            ? harthmereNpcCollisionObstacles().filter((obstacle) =>
                harthmereNpcObstacleContainsPoint(
                  obstacle,
                  position[0],
                  position[2]
                )
      ...
      snapshot: (radius?: number) =>
        this.harthmereTownWalkDebugSnapshot({ radius }),
      near: (radius = 12) =>
        this.harthmereTownWalkDebugSnapshot({ radius }).objects.sort(
          (a, b) =>
            (a.distanceToPlayer ?? Infinity) - (b.distanceToPlayer ?? Infinity)
        ),
      suspects: (radius?: number) =>
        this.harthmereTownWalkDebugSnapshot({ radius, suspectsOnly: true })
          .objects,
      overlaps: (radius = 30) =>
      ...
      watch: (radius = 12, intervalMs = 1000) => {
        ...
        return {
          watching: true,
          radius,
          intervalMs: Math.max(250, Number(intervalMs) || 1000),
        };
      },
      stopWatch: () => {
        if (watchTimer !== undefined) {
      ...
      pauseAnimatedProps: () => {
        ...
        for (const instance of this.animated) {
          if (
            !instance.mixer ||
            harthmereTownDebugIsActorAsset(instance.asset) ||
            instance.wander ||
            instance.bob ||
            instance.spin
          ) {
  ...
  private createHarthmereTownAuditExportApi() {
    ...
    const isProbablyActor = (object: THREE.Object3D) => {
      ...
      return /townsperson|animal_|monster_|guard|merchant|vendor|clerk|banker|auction|trainer|healer|priest|clergy|farmer|dockhand|smuggler|bandit|prisoner|courier|cat|dog|rat|crow|chicken|sheep|wolf|boar|bear|deer/i.test(
        `${asset} ${name}`
      );
    };
    const isProbablyBlocker = (object: THREE.Object3D) => {
      const asset = objectAsset(object);
      const name = objectPlacementName(object);
      return /wall|tower|gate|fence|hedge|building|chapel|barracks|warehouse|hall|smithy|inn|counter|vault|well|cart|stall|bridge|fountain|pillar|roof|house|door|crate|barrel|chest|table|desk|bench|shelf|rack|anvil|forge/i.test(
        `${asset} ${name}`
      );
    };
    const playerPosition = () => {
      ...
      const candidates = [
        ...
        (runtime?.snapshot as Record<string, unknown> | undefined)
          ?.playerPosition,
        (
      ...
      for (const candidate of candidates) {
        ...
        const z =
          candidate.length >= 3 ? Number(candidate[2]) : Number(candidate[1]);
        if (Number.isFinite(x) && Number.isFinite(z)) {
          return {
            source: "runtime",
            position: [x, Number.isFinite(y) ? y : 0, z],
          };
        }
      }
      return undefined;
    ...
    const reportObject = (
      object: THREE.Object3D,
      player?: { position: number[]; source: string }
    ) => {
      object.updateWorldMatrix(true, true);
      const box = new THREE.Box3().setFromObject(object);
      ...
      const collisionMissing =
        !collision &&
        !walkDebug?.obstacle &&
        isProbablyBlocker(object) &&
        Math.max(size.x, size.z) > 0.8;
      if (collisionMissing)
        flags.push("walk_through_collision_missing_or_unregistered");
      if (isProbablyActor(object)) flags.push("actor_or_npc");
      if (isProbablyBlocker(object)) flags.push("probable_blocker");
      if (Math.max(size.x, size.y, size.z) > 18)
        flags.push("massive_object_check_scale");
      if (Math.max(worldScale.x, worldScale.y, worldScale.z) > 8)
        flags.push("massive_world_scale_check");
      if (Math.min(worldScale.x, worldScale.y, worldScale.z) <= 0)
        flags.push("invalid_zero_or_negative_scale");
      if (isProbablyActor(object) && box.min.y < -0.25)
        flags.push("actor_or_npc_below_ground");
      if (!isProbablyActor(object) && box.min.y < -1.0)
        flags.push("prop_below_ground");
      if (
        box.min.y > 1.5 &&
        !/lamp|lantern|banner|sign|flag|roof|window|chandelier|hanging|bell|torch|smoke|chimney/i.test(
          `${asset} ${name}`
        )
      ) {
        flags.push("floating_or_unsupported_prop_check");
      }
      if (object.animations?.length && !isProbablyActor(object))
        flags.push("animated_prop_possible_random_open_close");
      ...
      const roots = includeMeshes
        ? (() => {
            ...
            renderer.root.traverse((object) => {
              ...
              if (/debug-overlay|town-walk-debug-overlay/i.test(object.name))
                return;
              if (
                (object as THREE.Mesh).isMesh ||
                object.userData?.harthmereTownWalkDebug ||
                object.userData?.harthmerePlacementMeta ||
                object.userData?.harthmereCollision
              ) {
                list.push(object);
              }
            });
        ...
        : renderer.root.children.filter(
            (object) =>
              !/debug-overlay|town-walk-debug-overlay/i.test(object.name)
          );
    ...
    const overlapVolume = (
      a: { box: { min: number[]; max: number[] } },
      b: { box: { min: number[]; max: number[] } }
    ) => {
      const ox =
    ...
    const overlaps = (limit = 250, options: Record<string, unknown> = {}) => {
      const objects = collectObjects(options).filter((object) => {
        ...
        return (
          Math.max(Number(size[0]), Number(size[1]), Number(size[2])) > 0.25
        );
      });
      const hits: unknown[] = [];
      for (let i = 0; i < objects.length; i += 1) {
        for (let j = i + 1; j < objects.length; j += 1) {
          ...
          if (!aActor && !bActor && !aBlocker && !bBlocker && volume < 0.5)
            continue;
          hits.push({
            volume: round(volume),
            severity:
      ...
      return hits
        .sort(
          (a, b) =>
            Number((b as Record<string, unknown>).volume) -
            Number((a as Record<string, unknown>).volume)
        )
    ...
    const dump = (options: Record<string, unknown> = {}) => {
      ...
      if (player?.position) {
        currentObstacle = findHarthmereNpcCollisionObstacle(
      ...
      const report = {
        ...
        counts: {
          ...
          objects: objects.length,
          suspects: suspects.length,
          walkThroughSuspects: suspects.filter((object) =>
            object?.flags?.includes(
              "walk_through_collision_missing_or_unregistered"
            )
          ).length,
          scaleSuspects: suspects.filter((object) =>
            object?.flags?.some((flag) => /massive|scale/.test(flag))
          ).length,
          groundSuspects: suspects.filter((object) =>
            object?.flags?.some((flag) => /ground|floating/.test(flag))
          ).length,
          animatedPropSuspects: suspects.filter((object) =>
            object?.flags?.includes("animated_prop_possible_random_open_close")
          ).length,
          collisionObstacles: harthmereNpcCollisionObstacles().length,
          overlaps:
            Number(options.includeOverlaps) === 0
              ? 0
              : overlaps(500, options).length,
        },
        ...
        suspects,
        overlaps:
          Number(options.includeOverlaps) === 0 ? [] : overlaps(500, options),
    ...
    const watch = (radius = 18, intervalMs = 1000) => {
      const existing = (
        win as typeof win & { __harthmereTownAuditWatchTimer?: number }
      ).__harthmereTownAuditWatchTimer;
      ...
      const tick = () => {
        ...
        const nearby = player?.position
          ? objects.filter(
              (object) =>
                Number(object?.distanceToPlayer ?? Infinity) <= Number(radius)
            )
          : [];
        const entry = {
          at: Date.now(),
          player,
          currentObstacle: player?.position
            ? findHarthmereNpcCollisionObstacle(
                Number(player.position[0]),
                Number(player.position[2])
              )
            : undefined,
          nearbySuspects: nearby
            .filter((object) => object?.flags?.length)
            .slice(0, 30),
      ...
      (
        win as typeof win & { __harthmereTownAuditWatchTimer?: number }
      ).__harthmereTownAuditWatchTimer = window.setInterval(
        tick,
        Math.max(250, Number(intervalMs))
      );
      return {
        ok: true,
        message: "Harthmere town audit watch started",
        radius,
        intervalMs,
      };
    };
    const stopWatch = () => {
      ...
      (
        win as typeof win & { __harthmereTownAuditWatchTimer?: number }
      ).__harthmereTownAuditWatchTimer = undefined;
    ...
    const sample = (note = "", radius = 30) => {
      ...
      const nearby = player?.position
        ? collectObjects({ includeMeshes: false })
            ...
            .sort(
              (a, b) =>
                Number(a?.distanceToPlayer ?? Infinity) -
                Number(b?.distanceToPlayer ?? Infinity)
            )
        : [];
      const entry = {
        at: Date.now(),
    ...
    const download = (
      filename = `harthmere-town-audit-${Date.now()}.json`,
    ...
    const setPlayer = (x: number, z: number, y = 0) => {
      win.__harthmereTownAuditManualPlayerPosition = [
        Number(x),
        Number(y),
        Number(z),
      ];
    ...
    const clearPreviousSessions = (options: Record<string, unknown> = {}) => {
      ...
      const safePattern =
        /(harthmere|biomes).*?(walk|audit|debug|sample|snapshot|collision|session)/i;
      ...
      for (const storage of [window.localStorage, window.sessionStorage]) {
        ...
        for (const key of keys) {
          if (
            safePattern.test(key) ||
            (includeGameState && gamePattern.test(key))
          ) {
    ...
    const findSolidFixtureObstacle = (fixture: Record<string, unknown>) => {
      ...
      const hints = Array.isArray(fixture.assetHints)
        ? fixture.assetHints.map((hint) => String(hint).toLowerCase())
        : [];
      return harthmereAllCollisionObstacles().find((obstacle) => {
        const haystack = [obstacle.asset, obstacle.name, obstacle.district]
    ...
    const runSolidFixtureMovementTest = async (
      ...
      const results = cases.map((fixture) => {
        ...
        if (!obstacle) {
          const failure = {
            id: fixture.id,
            reason: "no exported player obstacle matched fixture hints",
            fixture,
          };
        ...
        if (
          ...
          obstacle.collisionHardness === "none" ||
          obstacle.collisionProfile === "visual_only"
        ) {
          const failure = {
            id: fixture.id,
            reason: "matched obstacle is still pass-through",
            obstacle,
          };
          failures.push(failure);
          return failure;
      ...
      return {
        ...
        failures,
        results,
        obstacleCount: harthmereAllCollisionObstacles().length,
      };
    };
    ...
    const captureSolidFixtureOverlayReport = async (
      viewpoints: Record<string, unknown>[] = []
    ) => {
      const activeSolidFixtureViewpoints = viewpoints.length
        ? viewpoints
        : solidFixtureAuditTargets;
      const failures: Record<string, unknown>[] = [];
      const objects = collectObjects({ includeMeshes: false });
      const results = activeSolidFixtureViewpoints.map((viewpoint) => {
        ...
        const targets = Array.isArray(viewpoint.targets)
          ? viewpoint.targets.map((target) => String(target).toLowerCase())
          : [];
        const matchedObstacles = harthmereAllCollisionObstacles().filter(
          (obstacle) => {
            const haystack = [obstacle.asset, obstacle.name, obstacle.district]
              .map((value) => String(value ?? "").toLowerCase())
              .join(" ");
            const districtOk =
              ...
              String(obstacle.district ?? "")
                ...
                .includes(district);
            return (
              districtOk && targets.some((target) => haystack.includes(target))
            );
          }
        );
        const matchedObjects = objects.filter((object) => {
          if (!object) return false;
          const haystack = [object.asset, object.name, object.district]
            .map((value) => String(value ?? "").toLowerCase())
            .join(" ");
          const districtOk =
            !district ||
        ...
        if (!matchedObjects.length) {
          failures.push({
            ...
            reason: "no visible mesh/object report matched viewpoint targets",
          });
        }
        if (!matchedObstacles.length) {
          failures.push({
            viewpoint: viewpoint.id,
            reason: "no collision proxy matched viewpoint targets",
          });
        }
        return {
          viewpoint: viewpoint.id,
          ok: matchedObjects.length > 0 && matchedObstacles.length > 0,
          matchedObjects,
          matchedObstacles,
        };
      });
    ...
    const runRadiusVariantCollisionCases = async (
      variantNames: string[] = []
    ) => {
      const failures: Record<string, unknown>[] = [];
    ...
    return {
      ...
      stopWatch,
      overlaps,
      suspects: (options: Record<string, unknown> = {}) =>
        collectObjects(options).filter((object) => object?.flags?.length),
      near: (radius = 20, options: Record<string, unknown> = {}) =>
        collectObjects(options)
          .filter(
            (object) =>
              Number(object?.distanceToPlayer ?? Infinity) <= Number(radius)
          )
  ...
  private installDebugBridge() {
    ...
    win.__harthmereAssetDimensions = {
      ...
      collisionFootprint: (asset: string, scale?: number) =>
        harthmereUploadedAssetCollisionFootprint(asset, scale),
    };
    (
      win as typeof win & {
        __harthmereSetFacialExpression?: (
    ...
    const harthmereTownAuditApi =
      this.createHarthmereTownAuditExportApi() as Record<string, unknown>;
    (
      win as typeof win & { __harthmereTownAudit?: unknown }
    ).__harthmereTownAudit = harthmereTownAuditApi;
    ...
    const harthmereDungeonTeleportTargets: Record<
      string,
    ...
    const harthmereStrictLiveDungeonTeleport = (
      targetName = "bellwardHalls"
    ) => {
      const target =
    ...
    (
      win as typeof win & { __harthmereTownAudit?: Record<string, unknown> }
    ...
    (
      win as typeof win & { __harthmereDungeonTest?: Record<string, unknown> }
    ).__harthmereDungeonTest = {
      ...((
        win as typeof win & { __harthmereDungeonTest?: Record<string, unknown> }
      ).__harthmereDungeonTest ?? {}),
      version: "harthmere-dungeon-console-teleport-uses-live-player-hook",
      teleportToBellwardHalls: () =>
        harthmereStrictLiveDungeonTeleport("bellwardHalls"),
      teleportToChapelUndercroft: () =>
        harthmereStrictLiveDungeonTeleport("chapelUndercroft"),
      teleportToOldWellDrain: () =>
        harthmereStrictLiveDungeonTeleport("oldWellDrain"),
      teleportToFirstChoir: () =>
        harthmereStrictLiveDungeonTeleport("firstChoir"),
      teleportToOldHarth: () => harthmereStrictLiveDungeonTeleport("oldHarth"),
      teleportToBellbinderTomb: () =>
        harthmereStrictLiveDungeonTeleport("bellbinderTomb"),
      teleportToBellwardChamber: () =>
        harthmereStrictLiveDungeonTeleport("bellwardChamber"),
    ...
    const collisionE2E =
      (
        win as typeof win & {
          __harthmereCollisionE2E?: Record<string, unknown>;
        }
      ).__harthmereCollisionE2E ?? {};
    const overlayAudit =
      (
        win as typeof win & {
          __harthmereCollisionOverlayAudit?: Record<string, unknown>;
        }
      ).__harthmereCollisionOverlayAudit ?? {};
    const obstacleReport = () => {
      const obstacles =
        typeof harthmereAllCollisionObstacles === "function"
    ...
    const findFixtureObstacle = (fixture: Record<string, unknown>) => {
      ...
      return obstacleReport().find((obstacle) => {
        const text = [
          ...
          obstacle.name,
          obstacle.district,
          obstacle.collisionProfile,
          obstacle.collisionHardness,
        ]
          .map((value) => String(value ?? "").toLowerCase())
          .join(" ");
        const districtOk =
    ...
    const runSolidFixtureMovementTest = async (
      ...
      const results = cases.map((fixture) => {
        ...
        if (obstacle.passThrough || !obstacle.hasPlayerShape) {
          const failure = {
            id: fixture.id,
            reason:
              "matched obstacle is still pass-through or has no player shape",
            obstacle,
          };
          failures.push(failure);
          return failure;
      ...
      (
        win as typeof win & {
          __harthmereHorizontalPlayerTownCollisionStats?: Record<
            string,
            unknown
          >;
        }
    ...
    const captureSolidFixtureOverlayReport = async (
      ...
      const failures: Record<string, unknown>[] = [];
      const results = activeSolidFixtureViewpoints.map(
        (viewpoint: Record<string, unknown>) => {
          const district = String(viewpoint.district ?? "").toLowerCase();
          const targets = Array.isArray(viewpoint.targets)
            ? viewpoint.targets.map((target: unknown) =>
                String(target).toLowerCase()
              )
            : [];
          const matchedObstacles = obstacleReport().filter((obstacle) => {
            const text = [obstacle.asset, obstacle.name, obstacle.district]
              .map((value) => String(value ?? "").toLowerCase())
              .join(" ");
            const districtOk =
              !district ||
              String(obstacle.district ?? "")
                ...
                .includes(district);
            return (
              districtOk && targets.some((target) => text.includes(target))
            );
          });
          if (!matchedObstacles.length)
            failures.push({
          ...
          return {
            viewpoint: viewpoint.id,
            ok: matchedObstacles.length > 0,
            matchedObstacles,
            matchedObjects: matchedObstacles,
          };
        }
      );
      return { ok: failures.length === 0, failures, results };
    ...
    const runRadiusVariantCollisionCases = async (
      ...
      const failures = obstacles.length
        ? []
        : [{ reason: "no exported renderer obstacles" }];
      const results = variantNames.map((variantName) => ({
        variantName,
    ...
    (
      win as typeof win & { __harthmereCollisionE2E?: Record<string, unknown> }
    ).__harthmereCollisionE2E = {
      ...collisionE2E,
      runSolidFixtureMovementTest,
      runRadiusVariantCollisionCases,
    ...
    win.__harthmereRendererDebug = {
      actors: () =>
        this.combatLifeInstances.map((actor) => ({
          ...
          proceduralWalkCheck:
            actor.object.userData.harthmereRuntimeWalkAnimationCheck,
      ...
      forcePulse: (offset = 9003, kind: CombatPulseKind = "attack") => {
        ...
        if (!actor) {
          console.warn(
            "No Harthmere combat actor for offset",
            offset,
            this.combatLifeInstances
          );
          return false;
        }
        this.startCombatPulse(actor, kind);
      ...
      swordState: () => ({
        ...
        equipmentId: resolveHarthmerePlayerWeaponEquipmentEntry(
          this.harthmerePlayerSwordState.itemId
        )?.equipmentId,
        category: resolveHarthmerePlayerWeaponEquipmentEntry(
          this.harthmerePlayerSwordState.itemId
        )?.profile,
        clipProfile: resolveHarthmerePlayerWeaponEquipmentEntry(
          this.harthmerePlayerSwordState.itemId
        )?.clips,
        weaponCatalog: {
        ...
        realVisual:
          this.harthmerePlayerSword?.userData?.harthmereRealVisualAnimation,
        ...
        manualSwing: this.harthmerePlayerSwordManualSwing
          ...
          : undefined,
        anchorMode:
          this.harthmerePlayerSword?.userData?.harthmereAttachmentMode ??
          "harthmere-anchor-right-hand/harthmere-anchor-hip/harthmere-anchor-back",
        anchors:
          this.harthmerePlayerSword?.userData?.harthmereAttachmentAnchors,
        weaponHandTracking: {
      ...
      combatAnimationPolish: () => ({
        ...
        productionVisualTestContracts:
          HARTHMERE_COMBAT_ANIMATION_PRODUCTION_TEST_CONTRACTS,
        productionVisualSnapshot: {
          requiredFields:
            HARTHMERE_COMBAT_ANIMATION_PRODUCTION_TEST_CONTRACTS.browserVisualSnapshotFields,
          playerWeaponHand:
            HARTHMERE_COMBAT_ANIMATION_HAND_POLICY.mainWeaponHand,
      ...
      resourceHitTelegraphState: () =>
        this.harthmereLastResourceHitTelegraph ?? null,
      ...
      swordVisualRegressionPose: (
        ...
        if (pose === "sheathed") {
          ...
          this.triggerHarthmereBlockContactFeedback(
            this.harthmerePlayerSword?.position,
            "screenshot_regression_block"
          );
        } else if (pose === "npc_attack") {
          const actor =
            this.combatLifeInstances.find((candidate) =>
              this.harthmereNpcWeaponVisuals.has(candidate.object)
            ) ?? this.combatLifeInstances[0];
          if (actor) {
            this.startCombatPulse(actor, "attack", [
              "Attack",
              "HeavyAttack",
              "SideSwing",
            ]);
          ...
          window.dispatchEvent(
            new CustomEvent("biomes:harthmere-player-sword-visual", {
              detail: { action: "sync", drawn: true, at: Date.now() },
            })
          );
        }
        this.updateHarthmerePlayerSwordVisual();
      ...
      creatureAnimationAudit: () => ({
        ...
        actors: this.combatLifeInstances
          .filter((actor) =>
            /animal|wolf|rat|boar|bear|deer|fox|crow|snake|undead/i.test(
      ...
      socialWorkAnimationAudit: () => ({
        ...
        actors: this.combatLifeInstances
          .filter((actor) =>
            /merchant|vendor|smith|cook|guard|healer|dock|quest|clergy|farmer|baker|tailor|trainer/i.test(
              `${actor.label} ${actor.asset} ${actor.appearance?.role ?? ""}`
            )
          )
          .map((actor) => ({
            label: actor.label,
      ...
      deathRespawnCinematicAudit: () => ({
        ...
        deadActors: this.combatLifeInstances
          .filter(
            (actor) =>
              this.deadCombatObjects.has(actor.object) ||
              actor.combatPulse?.kind === "death"
          )
          .map((actor) => ({
            label: actor.label,
            asset: actor.asset,
      ...
      log: () =>
        (window as typeof window & { __harthmereRendererDebugLog?: unknown[] })
          .__harthmereRendererDebugLog ?? [],
    };
  }
  ...
  private readonly onCombatEffect = (event: Event) => {
    ...
    const withoutMagicClips = (clips: string[]) =>
      clips.filter(
        (clip) => !/basicmagic|heavymagic|spark|spell|arcane/i.test(clip)
      );
    ...
    this.spawnHarthmereProjectileVisual(detailAny, attacker, target);

    if (target) {
      const bossVisual = harthmereBossVisualForRuntimeActor(target);
      const hpBefore = Number(detail.targetHpBefore ?? Number.NaN);
      const hpAfter = Number(detail.targetHpAfter ?? Number.NaN);
      if (bossVisual && Number.isFinite(hpAfter)) {
        const previousObservedMax = Number(
          target.object.userData.harthmereBossObservedMaxHp ?? 0
        );
        const observedMaxHp = Math.max(
          previousObservedMax,
          Number.isFinite(hpBefore) ? hpBefore : 0,
          hpAfter,
          1
        );
        target.object.userData.harthmereBossObservedMaxHp = observedMaxHp;
        const healthRatio = hpAfter / observedMaxHp;
        target.object.userData.harthmereBossHealthRatio = healthRatio;
        applyHarthmereScratchBossDamagePose(
          target.object,
          bossVisual.id,
          healthRatio
    ...
    if (typeof debugHarthmereRenderer === "function") {
      ...
      debugHarthmereRenderer("renderer.combat_event.effect_route", {
        attack:
          detail.attack ?? detail.ability ?? detail.attackType ?? detail.action,
      ...
      debugHarthmereRenderer("renderer.combat_event.attacker_match", {
        ...
        matched: attacker
          ? {
              ...
              offset: attacker.combatOffset,
              asset: attacker.asset,
            }
          : undefined,
      });
    ...
    if (targetKind === "block") {
      this.triggerHarthmereBlockContactFeedback(
        target?.object.position,
        String(detail.result ?? detail.animationKind ?? "block")
      );
    }
  ...
  private readonly onProjectileVisualRequest = (event: Event) => {
    const detail = (event as CustomEvent<Record<string, unknown>>).detail;
    if (!detail) {
      return;
    }
    const attackerOffset = Number(detail.attackerOffset);
    const targetOffset = Number(detail.targetOffset);
    this.spawnHarthmereProjectileVisual(
      detail,
      Number.isFinite(attackerOffset)
        ? this.findCombatLifeByOffset(attackerOffset)
        : undefined,
      Number.isFinite(targetOffset)
        ? this.findCombatLifeByOffset(targetOffset)
        : undefined
    );
  };
  ...
  private harthmereProjectilePlayerPoint() {
    ...
    const position = runtime?.position ?? [0, GROUND_Y, 0];
    const forward = runtime?.bodyForward ?? runtime?.forward ?? [0, -1];
  ...
  private harthmereProjectileActorPoint(actor: CombatLifeInstance) {
    const box = new THREE.Box3().setFromObject(actor.object);
    if (!box.isEmpty()) {
      const center = box.getCenter(new THREE.Vector3());
      center.y = THREE.MathUtils.lerp(box.min.y, box.max.y, 0.56);
      return center;
    }
    return actor.object
  ...
  private harthmereProjectileVector(value: unknown) {
  ...
  private spawnHarthmereProjectileVisual(
    detail: Record<string, unknown>,
    attacker?: CombatLifeInstance,
    target?: CombatLifeInstance
  ) {
    const definition = resolveHarthmereProjectileVisual({
      projectileVisualId: detail.projectileVisualId,
      abilityId: detail.abilityId,
    ...
    const player = this.harthmereProjectilePlayerPoint();
    const explicitOrigin = this.harthmereProjectileVector(detail.origin);
    const explicitTarget = this.harthmereProjectileVector(detail.targetPoint);
    const explicitOriginGround = this.harthmereProjectileVector(
      detail.originGroundPoint
    );
    const explicitTargetGround = this.harthmereProjectileVector(
      detail.targetGroundPoint
    );
    const origin =
      explicitOrigin ??
      (attacker
        ? this.harthmereProjectileActorPoint(attacker)
        : player.position);
    const targetIsPlayer = /^(you|player)$/i.test(String(detail.target ?? ""));
    const destination =
      explicitTarget ??
      (target
    ...
    return this.harthmereProjectileVisuals.spawn({
      ...
      origin,
      target: destination,
      originGround: explicitOriginGround,
      targetGround: explicitTargetGround,
      result: String(detail.result ?? ""),
      finalDamage: Number(detail.finalDamage ?? 0),
      attackShape:
        detail.attackShape === "beam" ||
        detail.attackShape === "cone" ||
        detail.attackShape === "ground_aoe" ||
        detail.attackShape === "self_aoe"
          ? detail.attackShape
          : "projectile",
      attackDistance: Number(detail.attackDistance ?? 0),
  ...
  private ensureHarthmerePlayerSwordAnchorRig() {
    ...
    addAnchor(
      "harthmere-anchor-hip",
      [-0.36, 0.88, -0.22],
      [-0.72, Math.PI * 0.82, 0.34]
    );
    addAnchor("harthmere-anchor-back", [0, 1.18, -0.3], [-0.62, Math.PI, 0.06]);
  ...
  private async loadHarthmerePlayerSwordGltf(
    ...
    try {
      ...
      const legacySwordEntryProbe = HARTHMERE_PLAYER_SWORD_EQUIPMENT_IDS.map(
        (id) => getHarthmereEquipmentAnimation(id)
      ).find(Boolean);
      ...
      this.normalizeHarthmerePlayerSwordGltfScale(
        gltf.scene,
        "targetLength" in entry ? entry.targetLength : undefined
      );
      ...
      this.harthmerePlayerSwordMixer.addEventListener("finished", () => {
        if (this.harthmerePlayerSwordState.drawn) {
          const clips = resolveHarthmerePlayerWeaponVisualClips(
            this.harthmerePlayerSwordState.itemId
          );
          this.playHarthmerePlayerSwordClip(
            clips.idle ?? HARTHMERE_PLAYER_SWORD_CLIPS.idle,
            true
          );
        }
      });
  ...
  private resolveHarthmereSwordObject3D(
    ...
    if (
      maybeObject.object &&
      typeof maybeObject.object.traverse === "function"
    ) {
  ...
  private normalizeHarthmerePlayerSwordGltfScale(
    value: unknown,
    desiredLongestSide = 1.12
  ) {
    const object = this.resolveHarthmereSwordObject3D(value);
    if (!object) {
      return;
  ...
  private playHarthmerePlayerSwordAnimationForCurrentState(reason: string) {
    ...
    if (state.action === "attack") {
      const legacySwordAttackClip =
        state.attack === "heavy"
          ? HARTHMERE_PLAYER_SWORD_CLIPS.heavy
          : HARTHMERE_PLAYER_SWORD_CLIPS.basic;
      const weaponAttackClip =
        state.attack === "heavy" ? clips.heavy : clips.basic;
      this.playHarthmerePlayerSwordClip(
  ...
  private installHarthmerePlayerSwordVisuals() {
    ...
    window.addEventListener("biomes:harthmere-player-sword-visual", (event) => {
      const detail =
        (event as CustomEvent<Partial<HarthmerePlayerSwordVisualState>>)
          .detail ?? {};
      ...
      this.harthmerePlayerSwordState = {
        ...
        attack: detail.attack,
        theme:
          (detail as { theme?: HarthmereAttackVisualThemeId }).theme ??
          this.harthmerePlayerSwordState.theme,
        variation:
          (detail as { variation?: string }).variation ??
          this.harthmerePlayerSwordState.variation,
      ...
      if (detail.action === "attack") {
        this.harthmerePlayerSwordSwingUntil =
          Date.now() + (detail.attack === "heavy" ? 520 : 340);
      }
      if (
        this.harthmerePlayerSwordState.itemId !== previousItemId ||
        !this.harthmerePlayerSwordUsingGltf
      ) {
        void this.loadHarthmerePlayerSwordGltf(
          this.harthmerePlayerSwordState.itemId
  ...
  private startHarthmerePlayerSwordManualSwing(attack: "basic" | "heavy") {
    ...
    const activeWeaponProfile =
      resolveHarthmerePlayerWeaponEquipmentEntry(
        this.harthmerePlayerSwordState.itemId
      )?.profile ?? "melee";
    ...
    const nextTheme =
      this.harthmerePlayerSwordState.theme ??
      themeSequence[
        this.harthmereCombatPolishAttackCounter % themeSequence.length
      ] ??
      "physical";
    ...
    this.harthmereCombatPolishActiveProfile =
      harthmereCombatAnimationProfileForRandomizedAction({
        ...
        lastShape: this.harthmereCombatPolishLastShape,
        requestedShape:
          requestedVariation.length > 0 ? requestedVariation : undefined,
        theme: nextTheme,
      });
    this.harthmereCombatPolishLastShape =
      this.harthmereCombatPolishActiveProfile.shape;
    this.harthmereCombatPolishRecentShapes.push(
      this.harthmereCombatPolishActiveProfile.shape
    );
    ...
    this.debugHarthmereSwordRendererEvent(
      "renderer.player_sword.manual_swing_start",
      {
        attack,
        durationMs: this.harthmerePlayerSwordManualSwing.durationMs,
      }
  ...
  private applyHarthmerePlayerSwordManualSwing(now = performance.now()) {
    ...
    const forwardForSwing = runtimeForSwing?.bodyForward ??
      runtimeForSwing?.forward ?? [0, 1];
    const positionForSwing = runtimeForSwing?.position ?? [0, 0, 0];
    const rawSwingForward = new THREE.Vector3(
      Number(forwardForSwing[0]),
      0,
      Number(forwardForSwing[1])
    );
    const centerForSwing = new THREE.Vector3(
      Number(positionForSwing[0]),
      Number(positionForSwing[1]),
      Number(positionForSwing[2])
    );
    ...
    const t = Math.min(
      1,
      Math.max(0, (now - swing.startedAt) / swing.durationMs)
    );
    ...
    if (handAnchor) {
      ...
      this.applyHarthmereWeaponStraightPointing(
        sword,
        handAnchor,
        rawSwingForward,
        activeWeaponProfile,
    ...
    this.recordHarthmereWeaponTipHiltDirection(
      sword,
      handAnchor,
      rawSwingForward,
      activeWeaponProfile,
    ...
    if (t >= 1) {
      ...
      sword.userData.harthmereWeaponHandTracking =
        this.getHarthmereWeaponHandTrackingSnapshot(sword, handAnchor);
      this.debugHarthmereSwordRendererEvent(
        "renderer.player_sword.manual_swing_done",
        {
          attack: swing.attack,
          handTracked: true,
  ...
  private resolveHarthmerePlayerVisualHandAnchor(
    ...
    const namedRightAnchor = this.getHarthmerePlayerSwordAnchor(
      "harthmere-anchor-right-hand"
    );
    const namedLeftAnchor = this.getHarthmerePlayerSwordAnchor(
      "harthmere-anchor-left-hand"
    );
  ...
  private resolveHarthmerePlayerBoneAnchor(candidates: readonly string[]) {
    ...
    this.root.traverse((object) => {
      ...
      const belongsToLocalPlayer =
        ...
        /local.*player|harthmere.*player|player.*mesh/i.test(
          object.parent?.name ?? ""
        );
      if (
        !belongsToLocalPlayer &&
        harthmereCombatPolishRejectSceneWideBoneAnchors
      ) {
        return;
      }
  ...
  private updateHarthmereCombatPolishLocomotion(
    center: THREE.Vector3,
    dt: number
  ) {
    if (this.harthmereCombatPolishLastCenter.lengthSq() > 0 && dt > 0) {
      this.harthmereCombatPolishSpeed =
        center.distanceTo(this.harthmereCombatPolishLastCenter) / dt;
    }
  ...
  private applyHarthmereCombatPolishAnchorPose(
    ...
    const right = this.getHarthmerePlayerSwordAnchor(
      "harthmere-anchor-right-hand"
    );
    const left = this.getHarthmerePlayerSwordAnchor(
      "harthmere-anchor-left-hand"
    );
    ...
    const gaitClock =
      typeof performance !== "undefined"
        ...
        : Date.now() * 0.012;
    const runBob =
      Math.min(0.075, this.harthmereCombatPolishSpeed * 0.008) *
      Math.sin(gaitClock);

    const resetRight: [number, number, number] = [0.54, 0.98 + runBob, 0.3];
    const resetLeft: [number, number, number] = [
      -0.54,
      0.98 - runBob * 0.7,
      0.3,
    ];
    ...
    switch (profile.shape) {
      ...
      case "slash_diagonal":
        ...
        break;
      case "slash_vertical":
        right.position.set(0.3, 1.32 - 0.52 * release, 0.26);
      ...
      case "spin_cleave":
        right.position.set(
          0.44 * Math.cos(release * Math.PI * 1.5),
          1.0 + 0.05 * wave,
          0.3 + 0.24 * Math.sin(release * Math.PI * 1.5)
        );
        right.rotation.set(-0.3, release * Math.PI * 1.4, -0.36, "XYZ");
        left.position.set(
          -right.position.x * 0.55,
          1.0,
          0.18 - right.position.z * 0.2
        );
        break;
      case "slam_ground":
        right.position.set(0.2, 1.26 - 0.58 * release, 0.32);
        left.position.set(-0.2, 1.22 - 0.5 * release, 0.28);
      ...
      case "double_slash":
        right.position.set(
          0.58 - 0.28 * Math.sin(t * Math.PI * 2),
          1.05 - 0.16 * Math.sin(t * Math.PI * 2),
          0.34
        );
        right.rotation.set(
          -0.22 * wave,
          Math.sin(t * Math.PI * 2) * 0.76,
          -Math.sin(t * Math.PI * 2) * 0.96,
          "XYZ"
        );
        break;
    ...
    right.userData.harthmereCombatAnimationPolish = {
      ...
      weaponGripMustFollowHandEveryFrame:
        HARTHMERE_COMBAT_POLISH_RUNTIME_RULES.weaponGripMustFollowHandEveryFrame,
      capturedBaseTransformAllowedDuringAttack: false,
    };
  }
  ...
  private applyHarthmerePolishedTrailProfile(
    ...
    if (/thrust|piercing|beam/.test(profile.trailShape)) {
      const line = new THREE.Mesh(
        makeHarthmereRuntimeRoundedVoxelGeometry([0.055, 0.055, 1.35]),
        baseMaterial.clone()
      );
      ...
      const a = new THREE.Mesh(
        makeHarthmereRuntimeRoundedVoxelGeometry([0.035, 0.035, 1.15]),
        baseMaterial.clone()
      );
      ...
      const ring = new THREE.Mesh(
        new THREE.TorusGeometry(0.72, 0.024, 8, 64, Math.PI * 2),
        baseMaterial.clone()
      );
      ...
      for (let i = 0; i < 8; i += 1) {
        const shard = new THREE.Mesh(
          makeHarthmereRuntimeRoundedVoxelGeometry([0.035, 0.035, 0.28]),
          secondaryMaterial.clone()
        );
      ...
      for (let i = 0; i < 4; i += 1) {
        const ghost = new THREE.Mesh(
          makeHarthmereRuntimeRoundedVoxelGeometry([0.08, 0.42, 0.22]),
          baseMaterial.clone()
        );
      ...
      const arc = new THREE.Mesh(
        new THREE.TorusGeometry(0.86, 0.018, 6, 56, Math.PI * 0.92),
        baseMaterial.clone()
      );
  ...
  private applyHarthmereNpcCombatPolishPulse(
    ...
    const npcAttackType = /mage|priest|cleric|magic|staff|wand/i.test(
      `${actor.asset} ${actor.label}`
    )
      ? "magic"
    ...
    const profile = harthmereCombatAnimationProfileForRandomizedAction({
      ...
      seed: `${actor.combatOffset ?? actor.label.length}:${Math.floor(
        progress * 8
      )}:${Date.now()}`,
      lastShape: previousNpcShape,
      theme: /undead|ghost|shadow/i.test(`${actor.asset} ${actor.label}`)
        ? "shadow"
        : "physical",
    });
    ...
    visual.scale.setScalar(
      Math.max(0.7, Math.min(1.2, actor.baseScale)) * (1 + 0.08 * wave)
    );
  ...
  private ensureHarthmerePlayerSwordTrail() {
    ...
    const arc = new THREE.Mesh(
      new THREE.TorusGeometry(0.86, 0.018, 6, 48, Math.PI * 0.88),
      material
    );
  ...
  private spawnHarthmerePlayerSwordTrail(
    attack: "basic" | "heavy",
    facingYaw: number
  ) {
    const sword = this.getHarthmerePlayerSwordObjectForManualSwing();
  ...
  private ensureHarthmereBlockContactFeedback() {
    ...
    for (let i = 0; i < 5; i += 1) {
      const spark = new THREE.Mesh(
        makeHarthmereRuntimeRoundedVoxelGeometry([0.045, 0.045, 0.38]),
        sparkMaterial.clone()
      );
  ...
  private triggerHarthmereBlockContactFeedback(
    ...
    emitHarthmereSoundEffect("parry", {
  ...
  private makeHarthmereNpcFallbackWeaponVisual(actor: CombatLifeInstance) {
    ...
    weapon.name = `harthmere-npc-equipped-weapon-${
      actor.combatOffset ?? actor.label
    }`;
    const blade = new THREE.Mesh(
  ...
  private attachHarthmereNpcWeaponVisual(actor: CombatLifeInstance) {
    ...
    // a wolf/bear/deer.
    const animalAssetRe =
      /^animal_|_animal_|\bwolf\b|\bbear\b|\bdeer\b|\bstag\b|\bboar\b|\bfox\b|\bdog\b|\bhound\b|\bcat\b|\brat\b|\bpig\b|\bcow\b|\bsheep\b|\bgoat\b|\bhorse\b|\bchicken\b|\bpigeon\b|\bcrow\b|\brabbit\b|\bsnake\b/i;
    ...
    const bodyAttackerCreatureText = `${labelLower} ${assetLower} ${
      actor.district ?? ""
    }`;
    ...
    if (
      !/sword|blade|axe|mace|hammer|dagger|club|bow|crossbow|staff|wand|shield|ranged|magic|weapon/i.test(
        mainHand
      )
    ) {
      return;
  ...
  private recordHarthmereWeaponTipHiltDirection(
    ...
    const straightForward = this.normalizedHarthmereWeaponForward(forward);
    const localHilt = new THREE.Vector3(
      0,
      0,
      HARTHMERE_COMBAT_ANIMATION_HAND_POLICY.hiltLocalZ
    );
    const localTip = new THREE.Vector3(
      0,
      0,
      HARTHMERE_COMBAT_ANIMATION_HAND_POLICY.tipLocalZ
    );
    const hiltWorld = localHilt
      .clone()
      .applyQuaternion(weapon.quaternion)
      .add(weapon.position);
    const tipWorld = localTip
      .clone()
      .applyQuaternion(weapon.quaternion)
      .add(weapon.position);
    ...
    const hiltToTipForwardDot =
      hiltToTip.lengthSq() > 0.0001
    ...
    weapon.userData.harthmereWeaponTipHiltDirection = {
      ...
      hiltToTipForwardDot,
      minimumTipForwardDot:
        HARTHMERE_COMBAT_ANIMATION_HAND_POLICY.minimumTipForwardDot,
      pointsStraightEnough:
        hiltToTipForwardDot >=
        HARTHMERE_COMBAT_ANIMATION_HAND_POLICY.minimumTipForwardDot,
      handToHiltDistanceMeters,
      handToHiltBudgetMeters: 0.62,
      tipIsInFrontOfHilt:
        tipWorld.clone().sub(hiltWorld).dot(straightForward) > 0,
    };
  }

  private getHarthmereWeaponHandTrackingSnapshot(
    ...
    const gripDistanceMeters =
      weapon && anchor
        ? weaponPosition.distanceTo(handPosition)
  ...
  private createHarthmereResourceHitTelegraph(
    ...
    const rules =
      HARTHMERE_OBJECT_EFFECT_RANGES[kind] ??
      HARTHMERE_OBJECT_EFFECT_RANGES.generic_resource;
    ...
    const telegraph = {
      ...
      failedReasonText: options.failedReason
        ? rules.visibleHitPolicy.failReasonText
        : undefined,
      edgeCases: [...HARTHMERE_RESOURCE_HIT_EDGE_CASES],
    };
  ...
  private updateHarthmerePlayerSwordVisual() {
    ...
    const dt = Math.min(
      0.05,
      Math.max(0.001, (now - this.harthmerePlayerSwordLastFrameAt) / 1000)
    );
    ...
    this.harthmerePlayerSwordDrawAmount +=
      (targetDraw - this.harthmerePlayerSwordDrawAmount) * smoothing;
    ...
    const combatPolishCenter = new THREE.Vector3(
      Number(position[0]),
      Number(position[1]),
      Number(position[2])
    );
    ...
    const combatPolishAttackProgress = combatPolishSwing
      ? clamp01(
          (performance.now() - combatPolishSwing.startedAt) /
            combatPolishSwing.durationMs
        )
      : 0;
    this.applyHarthmereCombatPolishAnchorPose(
      this.harthmereCombatPolishActiveProfile,
    ...
    const harthmerePlayerCenter = new THREE.Vector3(
      Number(position[0]),
      Number(position[1]),
      Number(position[2])
    );
    ...
    const curveLift =
      Math.sin(t * Math.PI) *
      (this.harthmerePlayerSwordState.action === "draw" ||
      this.harthmerePlayerSwordState.action === "sheathe"
        ? 0.26
        : 0.08);
    const wristTwist =
      Math.sin(t * Math.PI) *
      (this.harthmerePlayerSwordState.action === "sheathe" ? -0.42 : 0.36);
    ...
    const weaponGripPitchOffset =
      activeWeaponProfile === "ranged" ? -0.18 : -Math.PI * 0.5;
    const weaponGripYawOffset = activeWeaponProfile === "shield" ? Math.PI : 0;
    const weaponGripRollOffset =
      activeWeaponProfile === "shield" ? 0.04 : -0.08;
    ...
    sword.userData.harthmereAttachmentMode =
      boneHandAnchor || boneSheatheAnchor ? "bone" : "anchor-rig";
    ...
    sword.userData.harthmereHandednessDeathBounds = {
      ...
      expectedMainHandFallbackAnchor:
        activeWeaponProfile === "shield"
          ? "harthmere-anchor-right-hand"
          : "harthmere-anchor-left-hand",
      actualHandAnchor: handAnchor.name,
      visualHandMatchesAttack:
        activeWeaponProfile === "shield"
    ...
    const harthmereVisualRightVector = new THREE.Vector3(
      nz,
      0,
      -nx
    ).normalize();
    const harthmereVisualMainHandVector = new THREE.Vector3(
      -nz,
      0,
      nx
    ).normalize();
    const rightHandAnchor = this.getHarthmerePlayerSwordAnchor(
      "harthmere-anchor-right-hand"
    );
    const leftHandAnchor = this.getHarthmerePlayerSwordAnchor(
      "harthmere-anchor-left-hand"
    );
    ...
    sword.userData.harthmereRealVisualAnimation = {
      ...
      actualHandAnchor: handAnchor.name,
      mainHandSideScore: activeHandWorld
        .clone()
        .sub(harthmerePlayerCenter)
        .dot(harthmereVisualMainHandVector),
      mainHandDistanceMeters: swordWorld.distanceTo(activeHandWorld),
      rightHandDistanceMeters: rightHandAnchor
        ? swordWorld.distanceTo(rightWorld)
        : Number.POSITIVE_INFINITY,
      leftHandDistanceMeters: leftHandAnchor
        ? swordWorld.distanceTo(leftWorld)
        : Number.POSITIVE_INFINITY,
  ...
  private registerCombatLife(
    ...
    const actor: CombatLifeInstance = {
      object,
      label,
      asset: placement.asset,
      district: placement.district,
    ...
    actor.bossVisual = harthmereBossVisualForRuntimeActor(actor);
    actor.snapshotStatic = {
      radius: harthmereCombatActorRadius(actor.asset, actor.baseScale),
      species: harthmereCombatActorSpecies(actor.asset, actor.label),
      behavior: harthmereCombatActorBehavior(
        actor.asset,
        actor.label,
        actor.district
      ),
      socialRole: harthmereCombatActorSocialRole(
        actor.asset,
        actor.label,
        actor.district
      ),
    ...
    this.combatLifeInstances.push(actor);
    this.attachHarthmereNpcWeaponVisual(actor);
  }
  ...
  private findCombatLife(name: string) {
    ...
    const hints =
      COMBAT_TARGET_HINTS[name] ??
      wanted.split(" ").filter((part) => part.length >= 4);
  ...
  private publishNonNpcNativeCombatAnimationAudit(
    ...
    const forward = harthmereWorldForwardForYaw(
      actor.object.rotation.y,
      actor.forwardAxis
    );
    const entry = {
  ...
  private startCombatPulse(
    ...
    let routedPreferredClipNames = preferredClipNames;
    if (kind === "attack" && routedPreferredClipNames.length === 0) {
      const bossVisual = harthmereBossVisualForRuntimeActor(actor);
      const specialClips = bossVisual?.combatSpecialClips ?? [];
      if (specialClips.length > 0) {
        const previousIndex = Number(
          actor.object.userData.harthmereBossSpecialAttackIndex ?? -1
        );
        const nextIndex = (previousIndex + 1) % specialClips.length;
        actor.object.userData.harthmereBossSpecialAttackIndex = nextIndex;
        routedPreferredClipNames = [specialClips[nextIndex]];
      }
    }
    const chosenClip = bestCombatClip(
      kind,
      actor.clips,
      routedPreferredClipNames
    );
    if (typeof debugHarthmereRenderer === "function") {
      debugHarthmereRenderer("renderer.start_pulse", {
        ...
        combatOffset: actor.combatOffset,
        kind,
        preferredClipNames: routedPreferredClipNames,
    ...
    actor.combatPulse = {
      ...
      baseYaw: actor.object.rotation.y,
      durationMs:
        kind === "death"
          ? 1650
          : kind === "attack"
    ...
    if (kind === "attack" || kind === "hit" || kind === "death") {
      ...
      delete actor.object.userData.harthmereNextCreatureIdleSoundAtMs;
    }
  ...
  private harthmereCreatureSoundProfile(actor: CombatLifeInstance) {
    if (actor.soundProfile !== undefined) {
      return actor.soundProfile ?? undefined;
    }
  ...
  private playHarthmereCreatureSound(
    actor: CombatLifeInstance,
    phase: Exclude<HarthmereCreatureSoundPhase, "idle">
  ) {
    const profile = this.harthmereCreatureSoundProfile(actor);
    if (!profile) return;
    const identity =
      actor.object.userData.harthmereEcsCreatureId ??
      actor.combatOffset ??
      actor.label;
    if (phase === "attack") {
      const nowMs =
      ...
      if (
        !harthmereCreatureShouldPlayAttackSound(profile, identity, attackCount)
      ) {
        return;
      }
    }
    emitHarthmereSoundEffect(harthmereCreatureSoundEffectId(profile, phase), {
      position: actor.object.getWorldPosition(new THREE.Vector3()).toArray(),
    });
  }

  private tickHarthmereCreatureIdleSound(actor: CombatLifeInstance) {
    if (
      !actor.object.visible ||
      this.deadCombatObjects.has(actor.object) ||
      actor.combatPulse
    ...
    const identity =
      ...
      actor.label;
    const nextAt = Number(
      actor.object.userData.harthmereNextCreatureIdleSoundAtMs
    );
    if (!Number.isFinite(nextAt)) {
      const sequence = Number(
        actor.object.userData.harthmereCreatureIdleSoundSequence ?? 0
      );
      actor.object.userData.harthmereCreatureIdleSoundSequence = sequence + 1;
      actor.object.userData.harthmereNextCreatureIdleSoundAtMs =
        nowMs + harthmereCreatureIdleDelayMs(profile, identity, sequence);
      return;
    }
    if (nowMs < nextAt) return;
    emitHarthmereSoundEffect(harthmereCreatureSoundEffectId(profile, "idle"), {
      position: actor.object.getWorldPosition(new THREE.Vector3()).toArray(),
    });
    const sequence = Number(
      actor.object.userData.harthmereCreatureIdleSoundSequence ?? 0
  ...
  private applyCombatPulse(actor: CombatLifeInstance) {
    ...
    if (
      nowMs < this.harthmereHitStopUntil &&
      (pulse.kind === "hit" || pulse.kind === "block")
    ) {
    ...
    if (pulse.kind === "attack") {
      const recoveryWave =
        nowMs < this.harthmereAttackerRecoveryUntil ? 0.12 : 0;
    ...
    } else if (pulse.kind === "evade") {
      const directionSign =
        (pulse.direction?.[0] ?? 0) + (pulse.direction?.[2] ?? 0) >= 0 ? 1 : -1;
      actor.object.rotation.y = pulse.baseYaw ?? actor.object.rotation.y;
      switch (pulse.evadeFamily) {
        case "heavy":
          actor.object.position.y = actor.baseY + 0.05 * wave;
          actor.object.rotation.x = 0.12 * wave;
          actor.object.rotation.z = -0.16 * directionSign * wave;
          actor.object.scale.setScalar(actor.baseScale * (1 + 0.04 * wave));
          break;
        case "rabbit":
          actor.object.position.y = actor.baseY + 0.46 * wave;
          actor.object.rotation.x = -0.22 * wave;
          actor.object.rotation.z = -0.12 * directionSign * wave;
          actor.object.scale.setScalar(actor.baseScale * (1 + 0.08 * wave));
          break;
        case "bird":
          actor.object.position.y = actor.baseY + 0.34 * wave;
          actor.object.rotation.x = -0.16 * wave;
          actor.object.rotation.z = -0.2 * directionSign * wave;
          actor.object.scale.setScalar(actor.baseScale * (1 + 0.12 * wave));
          break;
        case "swim":
          actor.object.position.y = actor.baseY + 0.08 * wave;
          actor.object.rotation.x = -0.08 * wave;
          actor.object.rotation.z = -0.28 * directionSign * wave;
          actor.object.scale.setScalar(actor.baseScale * (1 + 0.06 * wave));
          break;
        case "hexer":
          actor.object.position.y = actor.baseY + 0.22 * wave;
          actor.object.rotation.z = -0.2 * directionSign * wave;
          actor.object.scale.setScalar(actor.baseScale * (1 + 0.16 * wave));
          break;
        case "mucker":
          actor.object.position.y = actor.baseY + 0.25 * wave;
          actor.object.rotation.x = -0.3 * wave;
          actor.object.rotation.z = -0.22 * directionSign * wave;
          actor.object.scale.setScalar(actor.baseScale * (1 + 0.08 * wave));
          break;
        case "robot":
          actor.object.position.y = actor.baseY + 0.12 * wave;
          actor.object.rotation.x = 0.18 * wave;
          actor.object.rotation.z = -0.3 * directionSign * wave;
          actor.object.scale.setScalar(actor.baseScale * (1 + 0.05 * wave));
          break;
        case "sideLeap":
          actor.object.position.y = actor.baseY + 0.3 * wave;
          actor.object.rotation.x = -0.1 * wave;
          actor.object.rotation.z = -0.38 * directionSign * wave;
          actor.object.scale.setScalar(actor.baseScale * (1 + 0.08 * wave));
          break;
        case "generic":
        default:
          actor.object.position.y = actor.baseY + 0.18 * wave;
          actor.object.rotation.x = -0.14 * wave;
          actor.object.rotation.z = -0.24 * directionSign * wave;
          actor.object.scale.setScalar(actor.baseScale * (1 + 0.08 * wave));
          break;
      }
    } else {
    ...
    if (progress >= 1) {
      actor.object.rotation.x = 0;
      actor.object.rotation.z = 0;
      if (pulse.kind === "evade" && pulse.baseYaw !== undefined) {
        actor.object.rotation.y = pulse.baseYaw;
      }
  ...
  private addHarthmereProceduralLifePlacement(_placement: RuntimePlacement) {
    // Native-avatar-only policy: living actors must come from ECS through the
    // original player/NPC mesh resources. Keeping this method as a hard false
    // boundary prevents older placement call sites from silently resurrecting
    // the rounded-box Three.js avatar fallback.
    return false;
  }
  ...
  private reconcileHarthmereEcsLiveCreatures() {
    if (!harthmereEcsCreatureRenderEnabled()) {
      this.lastHarthmereLiveCreatureBridgeAt = undefined;
      this.latestSyntheticCutsceneRecords.clear();
    ...
    const snapshot = readHarthmereLiveCreatureBridgeSnapshot();
    if (snapshot.at === this.lastHarthmereLiveCreatureBridgeAt) {
      return;
    }
    this.lastHarthmereLiveCreatureBridgeAt = snapshot.at;
    const records = snapshot.records;
    // Player/Grove-style humans already render through NpcRenderState's
    // generated avatar pipeline. Keep their records only for static-fallback
    // suppression; never generate a second procedural townsperson for them.
    const harthmereRenderedRecords = records.filter(
      (record) => !record.nativeNpcRenderer
    );
    this.latestSyntheticCutsceneRecords.clear();
    for (const record of harthmereRenderedRecords) {
      this.latestSyntheticCutsceneRecords.set(record.id, record);
    }
    const { toAdd, toUpdate, toRemove } = reconcileHarthmereLiveCreatureBridge(
      new Set(this.harthmereEcsLiveCreatures.keys()),
      harthmereRenderedRecords
    );
    ...
    this.updateHarthmereStaticCreatureFallbackVisibility(
      harthmereLiveCreatureStaticFallbackTargetIds(
        records.filter(
          (record) =>
            // Authored Mucker/Hex GLTFs and Grove humans now render through
            // NpcRenderState instead of this overlay map. They still represent
            // the authoritative ECS actor, so their legacy static fallback must
            // be suppressed just like an overlay-rendered animal/compat actor.
            record.nativeNpcRenderer ||
  ...
  private nativeCutsceneClipNames(animation: string | undefined): string[] {
    if (!animation) return ["Idle"];
    if (isHarthmereCinematicExpression(animation)) {
      const spec = harthmereCinematicExpressionSpec(animation);
      return [spec.clip, ...spec.fallbacks];
    }
    const aliases: Readonly<Record<string, string[]>> = {
      idle: ["Idle"],
      walk: ["Walking", "Walk", "Idle"],
      run: ["Running", "Run", "Walking"],
      attack: ["Attack", "Attack2", "Idle"],
      attack1: ["Attack", "Idle"],
      attack2: ["Attack2", "HeavyAttack", "Attack"],
      talkGesture: ["TalkGesture", "Waving", "Point", "Idle"],
      questGesture: ["QuestGesture", "Point", "Waving", "Idle"],
      crowdEmote: ["Applause", "Waving", "Idle"],
      wave: ["Waving", "Idle"],
      point: ["Point", "Waving", "Idle"],
      applause: ["Applause", "Waving", "Idle"],
      hitReact: ["HitReact", "Stagger", "Idle"],
      death: ["Death", "Fall", "Idle"],
    };
    return aliases[animation] ?? [animation, "Idle"];
  }

  private applyNativeCutsceneActorRecord(
    actor: NativeCutsceneActor,
    record: HarthmereLiveCreatureBridgeRecord
  ) {
    actor.object.position.fromArray(record.at);
    actor.object.rotation.y = record.yaw;
    actor.object.scale.setScalar(record.scale);
    ...
    const clip = this.nativeCutsceneClipNames(record.animation)
      ...
      .find(Boolean);
    if (!clip) return;
    if (actor.activeAnimation !== clip.name) {
      actor.activeAction?.stop();
      const action = actor.mixer.clipAction(clip);
      action.reset();
      if (record.animation === "death") {
        action.setLoop(THREE.LoopOnce, 1);
        action.clampWhenFinished = true;
      } else if (
        isHarthmereCinematicExpression(record.animation) &&
        harthmereCinematicExpressionSpec(record.animation).playback !== "loop"
      ) {
        action.setLoop(THREE.LoopOnce, 1);
        action.clampWhenFinished =
          harthmereCinematicExpressionSpec(record.animation).playback ===
          "hold";
      } else {
        action.setLoop(THREE.LoopRepeat, Infinity);
      }
      ...
      actor.activeAnimation = clip.name;
    }
    if (actor.activeAction) {
      actor.activeAction.time = THREE.MathUtils.clamp(
        record.animationTime ?? record.motionTime ?? 0,
        0,
        clip.duration
      );
  ...
  private async loadNativeCutsceneActor(
    ...
    const load = (async () => {
      ...
      if (
        record.nativeSnapshotAvatar ||
        record.asset === "snapshot/player_mesh"
      ) {
        ...
        actor = {
          ...
          clips: new Map(),
          snapshotPlayerMesh: mesh,
          dispose: () => mesh.dispose(),
        };
      } else {
        const url = record.asset.startsWith("/")
          ? record.asset
          : resolveAssetUrlUntyped(record.asset);
        if (!url) {
          throw new Error(
            `No native cutscene NPC asset for ${record.asset} (${record.label})`
          );
        }
      ...
      .finally(() => {
        this.nativeCutsceneActorLoads.delete(record.id);
      });
    this.nativeCutsceneActorLoads.set(record.id, load);
    await load;
  }
  ...
  private addHarthmereEcsLiveCreature(
    ...
    // The only bridge-rendered actors are synthetic cutscene cast members.
    // Real ECS NPCs are filtered above and stay in NpcRenderState. Synthetic
    // humans use /api/assets/player_mesh.glb; synthetic creatures use authored
    // Galois GLTFs. Neither path constructs a procedural body.
    void this.loadNativeCutsceneActor(record);
  }

  private syncHarthmereEcsCinematicExpressionFace(
    record: HarthmereLiveCreatureBridgeRecord,
    object: THREE.Object3D
  ) {
    ...
    object.userData.harthmereCinematicExpressionPlayback = {
      expression: transition.expression,
      time: transition.time,
    } satisfies HarthmereCinematicExpressionPlaybackCursor;
    if (transition.started && transition.expression) {
      const spec = harthmereCinematicExpressionSpec(transition.expression);
      dispatchHarthmereFacialExpressionEvent({
        actorId: String(record.id),
        expression: spec.face,
        source: "script",
        reason: `cutscene-expression:${transition.expression}`,
        durationMs:
          spec.playback === "once"
            ? harthmereCinematicExpressionDurationMs(transition.expression)
            : undefined,
      });
      ...
      dispatchHarthmereFacialExpressionEvent({
        ...
        expression: "neutral",
        source: "script",
        reason: "cutscene-expression-ended",
      });
    }
  }

  private startHarthmereEcsMovementAction(
    record: HarthmereLiveCreatureBridgeRecord,
    object: THREE.Object3D
  ) {
    const action = record.movementAction;
    const nowSeconds = Date.now() / 1000;
    if (
      record.cinematic ||
      !action ||
      nowSeconds < action.startTime ||
      nowSeconds >= action.expiryTime
    ) {
      return;
    }
    const actionKey = `${action.action}:${action.nonce ?? action.startTime}`;
    if (object.userData.harthmereMovementActionKey === actionKey) {
      return;
    }
    const actor = this.combatLifeInstances.find(
      (candidate) => candidate.object === object
    );
    if (!actor) {
      return;
    }
    object.userData.harthmereMovementActionKey = actionKey;
    const visual = harthmereLiveCreatureEvadeVisual(record);
    this.startCombatPulse(actor, "evade", visual.preferredClipNames);
    if (actor.combatPulse?.kind === "evade") {
      actor.combatPulse.durationMs = Math.max(
        250,
        Math.min(1000, (action.expiryTime - action.startTime) * 1000)
      );
      actor.combatPulse.direction = [...action.direction];
      actor.combatPulse.evadeFamily = visual.family;
    }
    object.userData.harthmereMovementActionVisual = {
      actionKey,
      family: visual.family,
      preferredClipNames: visual.preferredClipNames,
  ...
  private updateHarthmereEcsLiveCreature(
    ...
    const nativeActor = this.nativeCutsceneActors.get(record.id);
    if (nativeActor) {
      this.applyNativeCutsceneActorRecord(nativeActor, record);
      return;
    }
    ...
    if (record.cinematic) {
      // Cinematic poses and promotional captures must land on their exact
      // authored transform in the frame where the director stages them.
      object.position.fromArray(record.at);
    } else {
      // Smoothly chase the entity's authoritative position so movement reads
      // naturally between ordinary bridge ticks.
    ...
    applyHarthmereCutscenePose(object, record);
    this.syncHarthmereEcsCinematicExpressionFace(record, object);
    const actor = this.combatLifeInstances.find(
      (candidate) => candidate.object === object
    );
    if (actor) {
      actor.baseY = object.position.y;
    }
    this.startHarthmereEcsMovementAction(record, object);
  }
  ...
  private removeHarthmereEcsLiveCreature(id: number) {
    ...
    if (nativeActor) {
      ...
      nativeActor.object.removeFromParent();
      nativeActor.dispose();
      return;
    }
    ...
    if (object.userData.harthmereSharedPrototypeResources === true) {
      return;
    }
    object.traverse((child) => {
      const mesh = child as THREE.Mesh;
  ...
  private async loadAll() {
    ...
    const requiredAssets = [
      ...new Set(
        runtimePlacements
          ...
          .filter((key) => assetByKey.has(key) && !isProceduralLifeKey(key))
      ),
    ];
    ...
    if (debugWindowForPrep) {
      debugWindowForPrep.__harthmereFloatingBlockIntegrityReport = {
        ...
        removedFloating: preparedRuntimePlacements.removedFloating.map(
          (placement) => ({
            asset: placement.asset,
            name: placement.name,
            district: placement.district,
    ...
    for (const authoredPlacement of runtimePlacements) {
      // All living actors are authoritative ECS entities rendered by the
      // snapshot player/NPC mesh pipelines. Runtime placements remain valid
      // for buildings, props, VFX, and collision, but never create a second
      // procedural person, animal, monster, boss, or robot.
      if (
        isProceduralLifeKey(authoredPlacement.asset) ||
      ...
      if (
        placementsStarted > 0 &&
        placementsStarted % placementBatchSize === 0
      ) {
        this.ready = true;
    ...
    const appearanceDebugActors = this.combatLifeInstances
      .slice(0, 80)
      .map((actor) => {
        const appearance = (
          actor as {
    ...
    if (debugWindow) {
      ...
      const harthmereDebugCollisionObstacles =
        harthmereAllCollisionObstacles().map((obstacle) => ({
          name: obstacle.name,
      ...
      debugWindow.__harthmereNpcCollisionObstacles =
        harthmereDebugCollisionObstacles;
      debugWindow.__harthmerePlayerCollisionObstacles =
        harthmereDebugCollisionObstacles;
      debugWindow.__harthmereTownCollisionObstacles =
        harthmereDebugCollisionObstacles;
      debugWindow.__harthmereTownRegistry = {
        ...
        performance: {
          ...
          performanceProfileVersion:
            HARTHMERE_RUNTIME_PERFORMANCE_PROFILE_RUNTIME_VERSION,
          surveyPerformanceResponseVersion:
            HARTHMERE_SURVEY_PERFORMANCE_RESPONSE_VERSION,
          performanceProfile: harthmereRuntimePerformanceProfile(),
      ...
      debugWindow.__harthmerePlacementCleanupReport =
        HARTHMERE_RUNTIME_PLACEMENT_CLEANUP;
      debugWindow.__harthmereNpcDistributionReport = HARTHMERE_NPC_DISTRIBUTION;
      debugWindow.__harthmereTownCollisionQuery = {
        version: HARTHMERE_TOWN_SYSTEMS_VERSION,
        containsNpc: (x: number, z: number) =>
          Boolean(findHarthmereNpcCollisionObstacle(x, z)),
        obstacleAt: (x: number, z: number) =>
          findHarthmereNpcCollisionObstacle(x, z)?.name,
      };
    }
    ...
    debugHarthmereRenderer("renderer.load_complete", {
      ...
      offsetActors: this.combatLifeInstances
        .filter((actor) => actor.combatOffset !== undefined)
        .map((actor) => ({
          label: actor.label,
          offset: actor.combatOffset,
          asset: actor.asset,
    ...
    log.info("Loaded rebuilt Harthmere town and Wilds assets", {
      ...
      districts: [
        ...new Set(
          RUNTIME_PLACEMENTS.map((placement) => placement.district).filter(
            Boolean
          )
        ),
      ],
...
function queueHarthmereRuntimeLicensedClothingModel(
  ...
  const runtimeKeys =
    (root.userData.harthmereRuntimeLicensedClothingModelKeys ??= {}) as Record<
      string,
      boolean
    >;
  ...
  root.userData.harthmereRuntimeLicensedClothingModelsRuntime =
    HARTHMERE_RUNTIME_LICENSED_CLOTHING_MODELS_VERSION;
  ...
  void loadHarthmereRuntimeLicensedClothingModel(modelUrl)
    .then((model) => {
      ...
      model.name = `harthmere-runtime-licensed-clothing-${slot}-${
        item.id ?? "model"
      }`;
      model.userData.harthmereRuntimeLicensedClothingModel = true;
      model.userData.harthmereRuntimeLicensedClothingRuntime =
        HARTHMERE_RUNTIME_LICENSED_CLOTHING_MODELS_VERSION;
      ...
      root.userData.harthmereRuntimeLicensedClothingModelsLoaded = [
        ...((root.userData
          .harthmereRuntimeLicensedClothingModelsLoaded as any[]) ?? []),
        {
          slot,
          id: item.id,
          modelUrl,
    ...
    .catch((error) => {
      console.warn(
        "Failed to load Harthmere runtime licensed clothing model; keeping current proxy",
        {
          slot,
          id: item.id,
          modelUrl,
...
function extractHarthmereRuntimeGltfScene(
  gltf: unknown
): THREE.Object3D | undefined {
...
function fitHarthmereRuntimeLicensedClothingModel(
  ...
  const targetHeight = getHarthmereRuntimeLicensedClothingTargetHeight(
    slot,
    body
  );
  ...
  const targetCenter = getHarthmereRuntimeLicensedClothingTargetCenter(
    slot,
    body
  );
...
function auditHarthmereRuntimeClothingLayers(root: THREE.Object3D): void {
  ...
  root.traverse((object) => {
    ...
    if (
      /runtime-outward-/.test(name) ||
      /runtime-outward-clothing-/.test(name)
    ) {
...
function addHarthmereRuntimeOutwardClothingDetailLayer(
  ...
  const variant =
    Math.abs(hashHarthmereRuntimeClothingSignature(signature)) % 5;
  ...
  const colors = getHarthmereRuntimeOutwardClothingColors(
    role,
    variant,
    palette
  );
  ...
  if (clothing?.torso) {
    root.add(
      boxMesh(
        "runtime-outward-clothing-front-panel",
        [torsoWidth * 0.74, torsoHeight * 0.92, 0.045],
        [0, torsoY, frontZ],
        cloth
      ),
      boxMesh(
        "runtime-outward-clothing-back-panel",
        [torsoWidth * 0.74, torsoHeight * 0.88, 0.045],
        [0, torsoY, backZ],
        cloth
      ),
      boxMesh(
        "runtime-outward-clothing-front-trim",
        [torsoWidth * 0.78, 0.045, 0.055],
        [0, torsoY + torsoHeight * 0.42, frontZ - 0.018],
        trim
      ),
      boxMesh(
        "runtime-outward-clothing-bottom-trim",
  ...
  if (clothing?.legs) {
    root.add(
      boxMesh(
        "runtime-outward-clothing-left-pant-front",
        [legWidth, legLength, 0.055],
        [-legX, legY, frontZ + 0.035],
        dark
      ),
      boxMesh(
        "runtime-outward-clothing-right-pant-front",
        [legWidth, legLength, 0.055],
        [legX, legY, frontZ + 0.035],
        dark
      ),
      boxMesh(
        "runtime-outward-clothing-left-knee-trim",
        [legWidth + 0.03, 0.045, 0.065],
        [-legX, body.legLength * 0.46, frontZ + 0.015],
        trim
      ),
      boxMesh(
        "runtime-outward-clothing-right-knee-trim",
        [legWidth + 0.03, 0.045, 0.065],
        [legX, body.legLength * 0.46, frontZ + 0.015],
        trim
      )
  ...
  if (clothing?.feet) {
    root.add(
      boxMesh(
        "runtime-outward-clothing-left-boot",
        [legWidth + 0.06, 0.12, 0.23],
        [-legX, 0.075, frontZ + 0.02],
        0x101010
      ),
      boxMesh(
        "runtime-outward-clothing-right-boot",
        [legWidth + 0.06, 0.12, 0.23],
        [legX, 0.075, frontZ + 0.02],
        0x101010
      ),
      boxMesh(
        "runtime-outward-clothing-left-boot-cuff",
        [legWidth + 0.07, 0.045, 0.2],
        [-legX, 0.145, frontZ + 0.02],
        leather
      ),
      boxMesh(
        "runtime-outward-clothing-right-boot-cuff",
        [legWidth + 0.07, 0.045, 0.2],
        [legX, 0.145, frontZ + 0.02],
        leather
      )
  ...
  if (clothing?.belt) {
    root.add(
      boxMesh(
        "runtime-outward-clothing-belt",
        [torsoWidth + 0.08, 0.065, 0.07],
        [0, waistY, frontZ - 0.032],
        leather
      ),
      boxMesh(
        "runtime-outward-clothing-buckle",
        [0.08, 0.075, 0.04],
        [0, waistY, frontZ - 0.07],
        metal
      ),
      boxMesh(
        "runtime-outward-clothing-left-pouch",
        [0.11, 0.13, 0.055],
        [-torsoWidth * 0.31, waistY - 0.08, frontZ - 0.055],
        leather
      ),
      boxMesh(
        "runtime-outward-clothing-right-pouch",
        [0.11, 0.13, 0.055],
        [torsoWidth * 0.31, waistY - 0.08, frontZ - 0.055],
        leather
      )
  ...
  if (role === "guard") {
    root.add(
      boxMesh(
        "runtime-outward-guard-left-pauldron",
        [0.16, 0.09, 0.16],
        [-(body.shoulderWidth / 2 + 0.04), shoulderY + 0.03, -0.02],
        metal
      ),
      boxMesh(
        "runtime-outward-guard-right-pauldron",
        [0.16, 0.09, 0.16],
        [body.shoulderWidth / 2 + 0.04, shoulderY + 0.03, -0.02],
        metal
      ),
      boxMesh(
        "runtime-outward-guard-tabard-stripe",
    ...
    root.add(
      boxMesh(
        "runtime-outward-hunter-diagonal-strap",
        [0.075, torsoHeight * 1.05, 0.06],
        [-0.05, torsoY, frontZ - 0.055],
        leather
      ),
      boxMesh(
        "runtime-outward-hunter-quiver",
        [0.15, 0.38, 0.11],
    ...
    root.add(
      boxMesh(
        "runtime-outward-merchant-left-lapel",
        [0.09, torsoHeight * 0.48, 0.055],
        [-torsoWidth * 0.17, torsoY + 0.08, frontZ - 0.055],
        trim
      ),
      boxMesh(
        "runtime-outward-merchant-right-lapel",
        [0.09, torsoHeight * 0.48, 0.055],
        [torsoWidth * 0.17, torsoY + 0.08, frontZ - 0.055],
        trim
      ),
      boxMesh(
        "runtime-outward-merchant-coin-pouch",
        [0.12, 0.14, 0.06],
    ...
    root.add(
      boxMesh(
        "runtime-outward-clergy-robe-center",
    ...
    root.add(
      boxMesh(
        "runtime-outward-bandit-torn-sash",
    ...
    root.add(
      boxMesh(
        "runtime-outward-undead-bandage-a",
        [torsoWidth * 0.72, 0.055, 0.06],
        [0, torsoY + 0.17, frontZ - 0.06],
        0xc8c1a6
      ),
      boxMesh(
        "runtime-outward-undead-bandage-b",
    ...
    root.add(
      boxMesh(
        "runtime-outward-civilian-sash",
        [0.07, torsoHeight * 0.9, 0.055],
        [sashX, torsoY, frontZ - 0.055],
        trim
      ),
      boxMesh(
        "runtime-outward-civilian-pocket",
        [0.1, 0.1, 0.055],
  ...
  root.userData.harthmereRuntimeOutwardClothingDetailLayer =
    HARTHMERE_RUNTIME_OUTWARD_CLOTHING_DETAIL_LAYER_VERSION;
...
function addHarthmereRuntimeVisibleClothingGuarantee(
  ...
  if (clothing?.torso) {
    root.add(
      boxMesh(
        "runtime-visible-clothing-torso-front",
        [torsoWidth, torsoHeight, 0.06],
        [0, torsoY, -(torsoDepth / 2 + 0.035)],
        cloth
      ),
      boxMesh(
        "runtime-visible-clothing-torso-back",
        [torsoWidth, torsoHeight, 0.06],
        [0, torsoY, torsoDepth / 2 + 0.035],
        cloth
      ),
      boxMesh(
        "runtime-visible-clothing-torso-left",
        [0.06, torsoHeight, torsoDepth + 0.09],
        [-(torsoWidth / 2), torsoY, 0],
        cloth
      ),
      boxMesh(
        "runtime-visible-clothing-torso-right",
        [0.06, torsoHeight, torsoDepth + 0.09],
        [torsoWidth / 2, torsoY, 0],
        cloth
      ),
      boxMesh(
        "runtime-visible-clothing-collar",
        [torsoWidth + 0.035, 0.04, torsoDepth + 0.11],
        [0, torsoY + torsoHeight * 0.48, -0.005],
        accent
      ),
      boxMesh(
        "runtime-visible-clothing-hem",
  ...
  if (clothing?.legs) {
    root.add(
      boxMesh(
        "runtime-visible-clothing-left-leg",
        [legWidth, legLength, 0.16],
        [-legX, legCenterY, -0.025],
        legCloth
      ),
      boxMesh(
        "runtime-visible-clothing-right-leg",
        [legWidth, legLength, 0.16],
        [legX, legCenterY, -0.025],
        legCloth
      )
  ...
  if (clothing?.feet) {
    root.add(
      boxMesh(
        "runtime-visible-clothing-left-foot",
        [legWidth + 0.04, 0.095, 0.2],
        [-legX, 0.055, -0.035],
        dark
      ),
      boxMesh(
        "runtime-visible-clothing-right-foot",
        [legWidth + 0.04, 0.095, 0.2],
        [legX, 0.055, -0.035],
        dark
      )
  ...
  if (clothing?.hands) {
    ...
    root.add(
      boxMesh(
        "runtime-visible-clothing-left-sleeve",
      ...
      boxMesh(
        "runtime-visible-clothing-right-sleeve",
        [armWidth + 0.045, armLength, 0.13],
  ...
  root.userData.harthmereRuntimeTallNpcClothingVisibility =
    HARTHMERE_RUNTIME_TALL_NPC_CLOTHING_VISIBILITY_VERSION;
...
type HarthmereRendererAnimationFamily =
  ...
  | "boss"
  | "screenshot";
const HARTHMERE_RENDERER_ANIMATION_DEBUG_POSES: Record<
  HarthmereRendererAnimationFamily,
  readonly string[]
> = {
  creature: [
    "idle",
...
function installHarthmereRendererFullAnimationDebug() {
  if (typeof window === "undefined") return;
  const win = window as typeof window & {
    __harthmereRendererAnimationRuntime?: unknown;
  };
  ...
  const api = {
    ...
    snapshot: () => ({
      ...
      weaponHandTracking: safeCall("weaponHandTracking", {
        gripBudgetMeters:
          HARTHMERE_LIVE_ANIMATION_SCENARIO_CONTRACT.gripBudgetMeters,
    ...
    locomotionActionProbe: () => ({
      ok: true,
      scenarios:
        HARTHMERE_LIVE_ANIMATION_SCENARIO_CONTRACT.locomotionWhileActing,
    ...
    playerDeathRespawnProbe: () => ({
      ...
      playerDeathHoldMs:
        HARTHMERE_LIVE_ANIMATION_SCENARIO_CONTRACT.playerDeathHoldMs,
    }),
    resourceHitVisibilityProbe: (resourceType = "rock") => ({
      ok: true,
      resourceType,
      scenarios:
        HARTHMERE_LIVE_ANIMATION_SCENARIO_CONTRACT.resourceHitVisibility,
      requiresSurfaceReticle: true,
      requiresToolTipLine: true,
      requiresImpactParticlesAtHitPoint: true,
      requiresFailureReasonText: true,
      reticleMaxSnapMeters:
        HARTHMERE_LIVE_ANIMATION_SCENARIO_CONTRACT.maxReticleSnapMeters,
    }),
    multiplayerAnimationProbe: () => ({
      ok: true,
      scenarios:
        HARTHMERE_LIVE_ANIMATION_SCENARIO_CONTRACT.multiplayerAnimation,
      remoteWeaponMustFollowRemoteHand: true,
      lateJoinerMustSeeWeaponState: true,
      maxPredictionSnapMeters:
        HARTHMERE_LIVE_ANIMATION_SCENARIO_CONTRACT.maxWeaponSnapMeters,
    }),
    performanceProbe: () => ({
      ok: true,
...
function installHarthmereAnimationHandednessDeathBounds() {
  ...
  const readSwordState = () => {
    try {
      const fromDebug =
        rendererDebug?.swordState?.() ?? rendererDebug?.playerSword?.();
      return fromDebug ?? {};
    } catch (error) {
  ...
  const api = {
    ...
    handednessProbe: () => ({
      ...
      requirement:
        "main-hand weapon is on the visible attack-hand side; offhand shield is opposite",
    }),
    deathAllActorsProbe: () => ({
      ok: true,
  ...
  if (rendererDebug && typeof rendererDebug === "object") {
    ...
    rendererDebug.worldEffectVisibilityProbe = () =>
      api.worldEffectVisibilityProbe();
    w.__harthmereRendererDebug = rendererDebug;
  }
}
...
export const HARTHMERE_ATTACK_VARIATION_VERSION_RENDERER =
  "harthmere-attack-variation";
(globalThis as any).__harthmereRendererDebug =
  (globalThis as any).__harthmereRendererDebug || {};
