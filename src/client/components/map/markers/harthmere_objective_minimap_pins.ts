...
export interface HarthmereObjectiveMiniMapPin {
  key: string;
  markerId: string;
  label: string;
...
export function harthmereObjectiveMiniMapPinsFromLandmarks(
  landmarks: ReadonlyArray<{
    id?: unknown;
    label?: unknown;
    position?: unknown;
  }>
): HarthmereObjectiveMiniMapPin[] {
  const seen = new Set<string>();
  const pins: HarthmereObjectiveMiniMapPin[] = [];
