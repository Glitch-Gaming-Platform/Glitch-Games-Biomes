...
import {
  HARTHMERE_BUSINESS_MINIMAP_MAX_DISTANCE_METERS,
  HARTHMERE_BUSINESS_MINIMAP_PIN_LIMIT,
  harthmereBusinessMiniMapPinsForPlayerForTest,
} from "../harthmere_business_minimap_pins";
import {
  HARTHMERE_BUSINESS_OUTPOSTS,
} from "@/shared/harthmere/business_customer_simulator";
import { harthmereBusinessOutpostMapLandmarks } from "@/client/components/biomes_ui/adapters/harthmereBusinessMapMarkers";

describe("Harthmere business minimap pins current", () => {
  it("returns nearby business outposts for the HUD minimap without exceeding the visual pin budget", () => {
    ...
    assert.ok(pins.length <= HARTHMERE_BUSINESS_MINIMAP_PIN_LIMIT);
    assert.ok(pins.every((pin) => pin.distanceMeters <= HARTHMERE_BUSINESS_MINIMAP_MAX_DISTANCE_METERS));
    assert.deepEqual(
  ...
  it("uses the same canonical business outpost marker positions as the BiomesUI map", () => {
    ...
    assert.equal(pins.length, HARTHMERE_BUSINESS_OUTPOSTS.length);
    for (const marker of harthmereBusinessOutpostMapLandmarks()) {
