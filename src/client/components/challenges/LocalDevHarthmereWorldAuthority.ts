...
function fail<T extends Record<string, unknown> = Record<string, never>>(
  code: HarthmereAuthorityFailureCode,
  message: string,
  evidence: string[] = [],
): HarthmereAuthorityResult<T> {
  return { ok: false, mode: HARTHMERE_AUTHORITY_MODE, code, message, evidence };
}

function pass<T extends Record<string, unknown> = Record<string, never>>(
  message: string,
  extra?: T,
): HarthmereAuthorityResult<T> {
  return { ok: true, mode: HARTHMERE_AUTHORITY_MODE, message, ...(extra ?? ({} as T)) };
}
...
export * from "./LocalDevHarthmereRemainderSystems";
