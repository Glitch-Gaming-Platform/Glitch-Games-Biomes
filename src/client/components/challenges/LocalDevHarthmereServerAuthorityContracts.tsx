...
export const HARTHMERE_SERVER_AUTHORITY_MODELS = {
  ...
  combatAction: ["request_id", "idempotency_key", "attacker_id", "target_id", "ability_id", "server_range_check", "server_line_of_sight", "server_hit_result", "server_damage", "server_state_version"],
  combatDeathRespawn: ["death_record_id", "entity_id", "death_state", "downed_until", "respawn_point_id", "protection_until", "penalty_ids"],
  combatContribution: ["encounter_id", "player_id", "damage", "healing", "shielding", "objectives", "revives", "crowd_control", "loot_eligible"],
};
...
export function validateHarthmereServerCombatAuthority(payload: Record<string, unknown>) {
  const rejected = [
    "clientHitResult",
    "clientFinalDamage",
    "clientTargetHpAfter",
    ...
    "clientContributionCredit",
  ].filter((key) => key in payload);
  return {
    ok: rejected.length === 0,
    rejected,
    serverAuthoritativeCombatRequired: true,
