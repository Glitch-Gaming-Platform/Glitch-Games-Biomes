...
// harthmere-hard-router-bnl-safety-v11
...
function defaultState(): HarthmereMultiplayerCombatState {
  ...
  return {
    ...
    recent: [
      logEntry(
        "Controls Ready",
        "Press X to draw/sheathe, Tab to cycle target, B for Basic Attack → GLTF Attack, N for Heavy Attack → GLTF HeavyAttack, L for Spark → GLTF BasicMagic, and P for PvP. These keys are reserved and do not overlap with map/quest/menu keys.",
      ),
    ],
  };
}
...
export function selectHarthmereCombatTarget(
  ...
  writeHarthmereMultiplayerCombatState({
    ...appendLog(
      state,
      reason,
      `Current combat target: ${label}. B runs Basic Attack/Attack, N runs Heavy Attack/HeavyAttack, and L runs Spark/BasicMagic.`,
    ),
...
export function useHarthmereCombatHotkeys() {
  useEffect(() => {
    ...
    const handler = (event: KeyboardEvent) => {
      ...
      if (["KeyX", "Tab", "KeyB", "KeyN", "KeyL", "KeyP"].includes(code)) {
  ...
  return (
    <div className="pointer-events-auto mt-2 max-h-[55vh] w-[30rem] overflow-y-auto rounded-lg border border-orange-300/25 bg-black/75 p-3 text-white shadow-xl">
      ...
      <div className="mb-2 rounded border border-emerald-300/20 bg-emerald-950/20 p-2 text-[11px] leading-snug text-emerald-50/80">
        ...
        <div><span className="font-semibold text-white">N</span> Heavy Attack → HeavyAttack, Attack2</div>
...
const HARTHMERE_HARD_COMBAT_KEY_ROUTER_VERSION = "harthmere-hard-key-router-v11";
...
function installHarthmereHardCombatKeyRouter() {
  ...
  const handler = (event: KeyboardEvent) => {
    ...
    // Critical part: stop older capture/bubble handlers from seeing B/N/L.
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
