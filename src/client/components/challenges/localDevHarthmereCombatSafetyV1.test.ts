...
} from "./localDevHarthmereCombatSafetyV1";
...
describe("local-dev Harthmere combat safety", () => {
  ...
  it("treats The Grove respawn and town core as safe from monster damage", () => {
    assert.equal(
      isHarthmereLocalCombatSafeZonePositionV1([496, 70, -126]),
      true
    );
    assert.equal(
      isHarthmereLocalCombatSafeZonePositionV1([612, 60, -245]),
      true
    );
    assert.equal(
      isHarthmereLocalCombatSafeZonePositionV1([333, 54, -390]),
      false
    );
  });
  ...
  it("requires actual current melee range instead of virtual chase distance", () => {
    const far = harthmereLocalCombatDamageGateV1({
      npc,
      npcPosition: [0, 54, 0],
    ...
    const close = harthmereLocalCombatDamageGateV1({
      npc,
      npcPosition: [0, 54, 0],
  ...
  it("requires line of sight for local combat damage", () => {
    assert.equal(
      harthmereLocalCombatHasLineOfSightV1([0, 54, 0], [28, 54, 0]),
      true
    );
    assert.equal(
      harthmereLocalCombatHasLineOfSightV1([0, 54, 0], [28.1, 54, 0]),
      false
    );
    assert.equal(
      harthmereLocalCombatHasLineOfSightV1([0, 54, 0], [40, 54, 0]),
      false
    );
    assert.equal(
      harthmereLocalCombatDamageGateV1({
