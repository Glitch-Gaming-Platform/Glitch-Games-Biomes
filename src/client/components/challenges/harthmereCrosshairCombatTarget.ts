// HARTHMERE_CROSSHAIR_COMBAT_TARGET
//
...
export type HarthmereCrosshairAim = {
  x: number;
  y: number;
...
export type HarthmereCrosshairPickInput = {
  actors: ReadonlyArray<HarthmereCrosshairCombatActor>;
...
export type HarthmereCrosshairPickResult = {
  offset: number;
 ...
 */
export function pickHarthmereCrosshairCombatTarget(
  input: HarthmereCrosshairPickInput
): HarthmereCrosshairPickResult | undefined {
  const { aim } = input;
  ...
  let best: HarthmereCrosshairPickResult | undefined;
...
export function harthmereHasCrosshairCombatTarget(
  input: HarthmereCrosshairPickInput
): boolean {
  return pickHarthmereCrosshairCombatTarget(input) !== undefined;
}
 ...
 */
export function readHarthmereCrosshairCombatActors(): HarthmereCrosshairCombatActor[] {
  if (!isBrowserEnv()) {
    return [];
  }
  ...
  const now = Date.now();
  const byOffset = new Map<number, HarthmereCrosshairCombatActor>();
 ...
 * coordinates.
 */
export function harthmereCrosshairAimFromEvent(input: {
  pointerLocked: boolean;
  clientX: number;
  clientY: number;
  viewportWidth: number;
  viewportHeight: number;
}): HarthmereCrosshairAim {
