...
import {
  ...
  snapshotMuckerHexerTileClearancePassV138,
  snapshotPointInBoundsV78,
  snapshotRemainingPortAuditV78,
  snapshotLabelIsMuckerOrHexerV132,
  SNAPSHOT_GROVE_LIVE_BOUNDS_V78,
  SNAPSHOT_MUCKER_HEXER_TILE_CLEARANCE_VERSION_V138,
...
function localPlayerPositionV78(
...
function collectLiveNpcAuditV78(
  ctx: ReturnType<typeof useClientContext>
): SnapshotLiveNpcAuditRecordV78[] {
...
interface HarthmereMuckerHexerTileClearanceSampleV138 {
  version: typeof SNAPSHOT_MUCKER_HEXER_TILE_CLEARANCE_VERSION_V138;
  id?: BiomesId;
  label: string;
  position?: Vec3;
  rendered: boolean;
...
function collectNpcGroundSamplesV84(
  ...
  for (const entity of ctx.table.scan(NpcMetadataSelector.query.all())) {
    ...
    const distance =
      position && playerPos ? distanceV84(position, playerPos) : undefined;
    ...
    const column = sampleTerrainColumnV84(
      ctx,
      position[0],
      position[2],
      position[1],
...
function renderedMuckerHexerActorsV138(): Array<{
  id?: BiomesId;
  label: string;
  position?: Vec3;
  rendered: boolean;
  ...
  const readSource = (
    ...
    for (const [key, value] of Object.entries(raw)) {
      ...
      if (!snapshotLabelIsMuckerOrHexerV132(label)) continue;
  ...
  readSource(win.__harthmereVoxelNpcMotionActorPositionsV193, true);
  return Array.from(records.values());
}

function collectMuckerHexerTileClearanceV138(
  ctx: ReturnType<typeof useClientContext>,
  playerPos: Vec3 | undefined,
  radius = 128
): HarthmereMuckerHexerTileClearanceSampleV138[] {
  const actors = renderedMuckerHexerActorsV138();
  const out: HarthmereMuckerHexerTileClearanceSampleV138[] = [];
  for (const actor of actors) {
    const distance =
      actor.position && playerPos
        ? distanceV84(actor.position, playerPos)
        : undefined;
    if (distance !== undefined && distance > radius) continue;
    const column = actor.position
      ? sampleTerrainColumnV84(
          ctx,
          actor.position[0],
          actor.position[2],
          actor.position[1],
      ...
      : undefined;
    const verdict = snapshotMuckerHexerTileClearancePassV138({
      label: actor.label,
    ...
    out.push({
      ...
      id: actor.id,
      label: actor.label,
      position: roundVec3V84(actor.position),
...
function muckerHexerTileClearanceSummaryV138(
  samples: HarthmereMuckerHexerTileClearanceSampleV138[]
) {
...
function missionTargetLabelMatchV90(
  entityLabel: string,
...
function auditMissionQuestV90(
  ...
  const targetPos = target
    ? (getHarthmereQuestTargetWorldPosV71(target) as Vec3)
    : undefined;
  ...
  const targetFootDelta =
    targetPos && targetTerrain?.feetY !== undefined
      ...
      : undefined;
  const nearbyTargets = collectMissionTargetCandidatesV90(
    ctx,
    targetPos,
    target?.label
  );
  const distance =
    playerPos && targetPos ? distanceV84(playerPos, targetPos) : undefined;
  ...
  if (targetFootDelta !== undefined && Math.abs(targetFootDelta) > 16) {
    issues.push(
      `mission target Y looks wrong; target delta is ${roundV84(targetFootDelta)} blocks`
    );
  }
  ...
  return {
    ...
    targetTerrain,
    targetFootDelta:
      targetFootDelta === undefined ? undefined : roundV84(targetFootDelta),
    nearbyTargets,
    textChecks,
    issues,
  };
...
export const SnapshotLiveDiagnosticsRuntimeControllerV78: React.FunctionComponent<{}> =
  () => {
    ...
    const captureMissionAuditV90 = (
      position?: Vec3
    ): HarthmereMissionAuditV90 => {
      ...
      const active = activeQuests
        ...
        .map((quest) =>
          auditMissionQuestV90(ctx, quest, state, position, bodyText)
        );
    ...
    const captureAutoSurveySample = (opts?: {
      ...
      const terrainStreaming = terrainStreamingStatusV84(
        ctx,
        position,
        streamingRadius
      );
      const resourceStats = slowResourceStatsV84(
      ...
      if (playerFootDelta !== undefined && Math.abs(playerFootDelta) > 2.5) {
        warnings.push(
          `player foot delta ${roundV84(playerFootDelta)} from terrain feet ${center?.feetY}`
        );
      }
      ...
      if (maxFrameMs > 80) {
        warnings.push(
          `slow frame ${roundV84(maxFrameMs)}ms near ${roundVec3V84(position)?.join(",") ?? "unknown"}`
        );
      }
      ...
      if (
        ...
        HARTHMERE_SURVEY_RAW_SAMPLE_CAP_V94
      ) {
      ...
      if (warnings.length && autoSurveyRunningRef.current) {
        const lastWarn =
          (window as any).__harthmereAutoSurveyLastWarnAtV84 ?? 0;
    ...
    const report = () => {
      ...
      return {
        ...
        floatingAudit: snapshotLiveNpcAuditSummaryV78(
          collectLiveNpcAuditV78(ctx)
        ),
        navigation:
    ...
    const autoSurveyReport = () => {
      ...
      return {
        ...
        latestMission:
          ...
          captureMissionAuditV90(localPlayerPositionV78(ctx)),
    ...
    useEffect(() => {
      ...
      win.__snapshotPerfV78 = {
        ...
        download: (filename = `snapshot-perf-walk-v78-${Date.now()}.json`) =>
          downloadJsonV78(filename, report()),
      };
      win.__snapshotDiagnosticsV78 = {
        ...
        muckerHexerGroundAuditV138: () =>
          collectMuckerHexerTileClearanceV138(ctx, localPlayerPositionV78(ctx)),
        muckerHexerGroundSummaryV138: () =>
          muckerHexerTileClearanceSummaryV138(
            collectMuckerHexerTileClearanceV138(
              ctx,
              localPlayerPositionV78(ctx)
            )
          ),
        ...
        missionAuditV90: () =>
          captureMissionAuditV90(localPlayerPositionV78(ctx)),
        downloadMissionAuditV90: (
          filename = `harthmere-mission-audit-v90-${Date.now()}.json`
        ) =>
          downloadJsonV78(
            filename,
            captureMissionAuditV90(localPlayerPositionV78(ctx))
        ...
        downloadFloatingAudit: (
          filename = `snapshot-floating-npc-audit-v78-${Date.now()}.json`
        ) => downloadJsonV78(filename, collectLiveNpcAuditV78(ctx)),
        downloadMuckerHexerGroundAuditV138: (
          filename = `mucker-hexer-ground-audit-v138-${Date.now()}.json`
        ) =>
          downloadJsonV78(
            filename,
            collectMuckerHexerTileClearanceV138(
      ...
      win.__harthmereAutoSurveyV84 = {
        ...
        explain: () => ({
          ...
          usage:
            "Run window.__harthmereAutoSurveyV84.start(); accept/advance a mission; walk to the marker; then run stop(), report(), or download(). Use start({npcRadius:96, streamingRadius:128}) only for a deeper audit pass.",
        }),
      };
      return () => {
        if (intervalRef.current !== undefined)
    ...
    return (
      <span
        className="hidden"
        data-snapshot-live-debug-player-scope-v78={
          SNAPSHOT_LIVE_DEBUG_PLAYER_SCOPE_VERSION_V78
        }
      />
    );
...
export const SnapshotLiveGroundingAuditPanelV78: React.FunctionComponent<{}> =
  () => {
    const ctx = useClientContext();
    const [audit, setAudit] = useState<SnapshotLiveNpcAuditRecordV78[]>(() =>
      collectLiveNpcAuditV78(ctx)
    );

    useEffect(() => {
      const interval = window.setInterval(
        () => setAudit(collectLiveNpcAuditV78(ctx)),
        1500
      );
    ...
    const summary = useMemo(
      () => snapshotLiveNpcAuditSummaryV78(audit),
      [audit]
    );
    ...
    return (
      <div className="rounded border border-red-200/20 bg-red-950/30 p-2 text-white">
        ...
        <div className="mt-1 text-[11px] text-white/60">
          Tolerance ≤ {SNAPSHOT_LIVE_NPC_MAX_FOOT_CLEARANCE_V78}m for
  ...
  () => {
    ...
    return (
      <div className="rounded border border-lime-200/20 bg-lime-950/30 p-2 text-white">
        ...
        <div className="mt-1 text-xs text-white/75">
          Console: window.__harthmereAutoSurveyV84.start(); walk around; stop();
          download()
        </div>
        <div className="mt-1 text-[11px] text-white/60">
          Also available: window.__snapshotPerfV78.start(),
