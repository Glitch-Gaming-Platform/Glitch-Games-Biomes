...
describe("HARTHMERE_JOB_REWARD_BRIDGE_V151 — reward grant on turn-in", () => {
  it("grants gold + items into the HUD state once", () => {
    ...
    const out = applyHarthmereJobRewardToStateV151(base, new Set(), reward);
  ...
  it("is idempotent per jobId — a re-fired turn-in never double-pays", () => {
    ...
    const first = applyHarthmereJobRewardToStateV151(base, new Set(), reward);
    const second = applyHarthmereJobRewardToStateV151(
      first.state,
      first.granted,
      reward
    );
  ...
  it("supports a gold-only reward (no items)", () => {
    const base = readHarthmereInventoryState();
    const out = applyHarthmereJobRewardToStateV151(base, new Set(), {
      jobId: "job_c",
      rewardGold: 25,
    });
