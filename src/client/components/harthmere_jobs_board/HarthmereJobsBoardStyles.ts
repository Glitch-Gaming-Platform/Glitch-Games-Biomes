...
export const HARTHMERE_JOBS_BOARD_STYLES_ID =
  "harthmere-jobs-board-styles";

export const HARTHMERE_JOBS_BOARD_CSS = `
...
.harthmere-jobs-board__spinner {
  ...
  border-radius: 999px;
  animation: harthmere-jobs-board-spin 780ms linear infinite;
  flex: 0 0 auto;
}
@keyframes harthmere-jobs-board-spin {
...
export function installHarthmereJobsBoardStyles(): void {
  if (typeof document === "undefined") return;
  if (document.getElementById(HARTHMERE_JOBS_BOARD_STYLES_ID)) return;
  const style = document.createElement("style");
  style.id = HARTHMERE_JOBS_BOARD_STYLES_ID;
  style.setAttribute("data-source", "harthmere_jobs_board");
  style.appendChild(document.createTextNode(HARTHMERE_JOBS_BOARD_CSS));
