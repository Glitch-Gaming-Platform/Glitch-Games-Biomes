...
import {
  ...
  POINTER_LOCK_UNLOCK_WHILE_OPEN_SELECTOR,
...
describe("Harthmere jobs board pointer lock lifecycle", () => {
  it("shows the cursor on open and restores pointer lock only when it owned the unlock", () => {
    ...
    openHarthmereJobsBoardPointerLock(manager, shouldReturn);
    ...
    closeHarthmereJobsBoardPointerLock(manager, shouldReturn);
  ...
  it("marks BiomesUI-style unlock surfaces as suppressing the pause menu", () => {
    const root = {
      querySelector(selector: string) {
        return selector === POINTER_LOCK_UNLOCK_WHILE_OPEN_SELECTOR
          ? { nodeType: 1 }
          : null;
      },
    };
    ...
    assert.equal(hasPointerLockUnlockWhileOpenSurface(root as any), true);
    assert.equal(hasPointerLockUnlockWhileOpenSurface(emptyRoot as any), false);
  });
});
