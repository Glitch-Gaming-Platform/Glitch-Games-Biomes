...
describe("Harthmere property map markers V1", () => {
  it("returns owned muck-designation land for both world map and HUD minimap feeds", () => {
    ...
    const landmarks = harthmerePropertyMapLandmarksFromBuildingStateV1(state as any);
    ...
    assert.equal(pins[0].markerId, "property:grove_muckstead_cottage_lot");
  ...
  it("marks terraformed owned land and falls back to the real plot center when marker data is invalid", () => {
    const landmarks = harthmerePropertyMapLandmarksFromBuildingStateV1({
      ownedPlotIds: ["grove_muckstead_cottage_lot", "unknown_plot"],
      safeZones: {
        grove_muckstead_cottage_lot: {
          safeFromMuck: true,
          activatedAtMs: 456,
