// HARTHMERE_MAP_TERRAIN_REGIONS_V1
//
...
//     (HARTHMERE_TOWN_ROUTE_ANCHORS_V48 in town_routes.ts): the market street,
...
// markers use (see harthmereMapTerrainRegionsForBoundsV1), so the terrain lines
// up exactly with the markers drawn on top of it.

export type HarthmereMapTerrainKindV1 =
  | "land"
  | "town"
  | "road"
  | "water"
...
export interface HarthmereMapTerrainRegionWorldV1 {
  id: string;
  label: string;
...
export type MapTerrainRegionShapeV1 =
  | { type: "ellipse"; cx: number; cy: number; rx: number; ry: number }
  | { type: "rect"; x: number; y: number; w: number; h: number }
  | { type: "polyline"; points: Array<{ x: number; y: number }>; width: number };

export interface MapTerrainRegionV1 {
  id: string;
  label: string;
  kind: HarthmereMapTerrainKindV1;
  shape: MapTerrainRegionShapeV1;
}
...
export const HARTHMERE_MAP_TERRAIN_REGIONS_WORLD_V1: HarthmereMapTerrainRegionWorldV1[] =
  [
...
function normX(worldX: number, bounds: MapBoundsV1): number {
  return ((worldX - bounds.minX) / Math.max(1, bounds.maxX - bounds.minX)) * 100;
}

function normZ(worldZ: number, bounds: MapBoundsV1): number {
...
function scaleWorldToMap(worldSize: number, bounds: MapBoundsV1): number {
...
export function harthmereMapTerrainRegionsForBoundsV1(
  bounds: MapBoundsV1 | undefined,
  regions: HarthmereMapTerrainRegionWorldV1[] = HARTHMERE_MAP_TERRAIN_REGIONS_WORLD_V1
): MapTerrainRegionV1[] {
  if (
    !bounds ||
  ...
  const projected: MapTerrainRegionV1[] = [
...
export function harthmereMapElevationBandForHeightV1(
