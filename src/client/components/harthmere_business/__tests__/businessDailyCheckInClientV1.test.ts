...
import {
  businessCheckInDisplayModelV1,
  harthmereDayIndexV1,
} from "@/client/components/harthmere_business/businessDailyCheckInClientV1";
import {
...
describe("business check-in client: day index", () => {
  it("maps timestamps within the same UTC day to one index, next day to the next", () => {
    const day0 = harthmereDayIndexV1(0);
    const sameDay = harthmereDayIndexV1(86_400_000 - 1);
    const nextDay = harthmereDayIndexV1(86_400_000);
    assert.strictEqual(sameDay, day0);
    assert.strictEqual(nextDay, day0 + 1);
  });

  it("guards non-finite input", () => {
    assert.strictEqual(harthmereDayIndexV1(NaN), 0);
  });
});
...
describe("business check-in client: display model", () => {
  function statusAfter(
    ...
    for (const d of checkInDays) {
      s = processBusinessCheckInV1(s, d, baseRevenue).state;
    }
    return businessCheckInStatusV1(s, todayDay, baseRevenue);
  }

  it("shows a 'checked in today' call to action and full revenue", () => {
    const m = businessCheckInDisplayModelV1(statusAfter([100], 100));
  ...
  it("prompts a check-in and warns about the skip cost when neglected", () => {
    ...
    const m = businessCheckInDisplayModelV1(statusAfter([100], 102));
  ...
  it("flags when the business has slid into losses", () => {
    ...
    const m = businessCheckInDisplayModelV1(statusAfter([100], 105));
  ...
  it("surfaces the streak and the made-vs-lost totals", () => {
    ...
    for (const d of [100, 101, 102]) {
      s = processBusinessCheckInV1(s, d, 1000).state;
    }
    s = processBusinessCheckInV1(s, 105, 1000).state; // streak resets to 1
    const m = businessCheckInDisplayModelV1(
      businessCheckInStatusV1(s, 105, 1000)
    );
