...
// HARTHMERE_UI_V147: structural test for EscGameMenu's suppression behavior
...
// `pointerLockUnlockWhileOpenSubscribeV1.test.ts`.
...
const HOOK_PATH = path.join(
  process.cwd(),
  "src/client/components/contexts/usePointerLockUnlockWhileOpenActiveV1.ts"
);
...
describe("EscGameMenu unlock-while-open suppression (V147)", () => {
  it("imports the unlock-while-open active hook", () => {
    ...
    assert.ok(
      source.includes("usePointerLockUnlockWhileOpenActiveV1"),
      "EscGameMenu must import usePointerLockUnlockWhileOpenActiveV1"
    );
  });
  ...
  it("invokes the hook inside the component body", () => {
    const source = readFileSync(ESC_GAME_MENU_PATH, "utf8");
    assert.ok(
      /const\s+unlockWhileOpenActive\s*=\s*usePointerLockUnlockWhileOpenActiveV1\s*\(\s*\)\s*;/.test(
        source
      ),
      "EscGameMenu must call usePointerLockUnlockWhileOpenActiveV1() and store the result"
    );
  });
  ...
  it("the hook reads the policy's depth and subscribes to its updates", () => {
    ...
    assert.ok(
      source.includes("isPointerLockUnlockWhileOpenActiveV1"),
      "hook must read the policy's active flag"
    );
    assert.ok(
      source.includes("subscribePointerLockUnlockWhileOpenV1"),
  ...
  it("the policy module exports a subscriber API for the hook to consume", () => {
    ...
    assert.ok(
      /export\s+function\s+subscribePointerLockUnlockWhileOpenV1/.test(source),
      "policy must export subscribePointerLockUnlockWhileOpenV1"
    );
    assert.ok(
      source.includes("notifyPointerLockUnlockWhileOpenSubscribersV1"),
