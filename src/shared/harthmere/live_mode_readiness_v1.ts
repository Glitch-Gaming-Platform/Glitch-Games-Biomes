...
export const HARTHMERE_LIVE_MODE_READINESS_VERSION_V1 =
...
export function validateHarthmereLiveModeAuthorityEnvelopeV1(
  envelope: HarthmereLiveModeAuthorityEnvelopeV1
): HarthmereLiveModeValidationResultV1 {
  const errors: string[] = [];
  const warnings: string[] = [];
  const rejectedClientClaims: string[] = [];
  const requiredFields: Array<keyof HarthmereLiveModeAuthorityEnvelopeV1> = [
    "requestId",
    "idempotencyKey",
    "actorId",
    "actionKind",
...
export function buildHarthmereLiveModePersistenceMutationPlanV1(
  envelope: HarthmereLiveModeAuthorityEnvelopeV1
): HarthmereLiveModePersistenceMutationPlanV1 {
  const pipeline = HARTHMERE_LIVE_MODE_REQUIRED_PIPELINES_V1.find(
    (item) =>
  ...
  const transactionScope: HarthmereLiveModePersistenceMutationPlanV1["transactionScope"] =
    envelope.raidId
      ? "raid"
      : envelope.partyId
      ? "party"
...
export function resolveHarthmereWaveRespawnSlotV1(input: {
  deathAtMs: number;
...
export function validateHarthmereLiveModeIdempotencyReplayV1(input: {
  idempotencyKey: string;
  seenKeys: string[];
...
export function validateHarthmereLiveModeReadinessV1(input?: {
  pipelines?: HarthmereLiveModePipelineDefinitionV1[];
  installedModules?: string[];
}): HarthmereLiveModeReadinessReportV1 {
  const pipelines =
    input?.pipelines ?? HARTHMERE_LIVE_MODE_REQUIRED_PIPELINES_V1;
  ...
  const missingPipelines = HARTHMERE_LIVE_MODE_REQUIRED_PIPELINES_V1.filter(
    (pipeline) => !pipelineIds.has(pipeline.id)
  ).map((pipeline) => pipeline.id);
  const missingSubsystems = HARTHMERE_LIVE_MODE_REQUIRED_SUBSYSTEMS_V1.filter(
    (subsystem) => !subsystemSet.has(subsystem)
  );
  const installedModules = input?.installedModules ?? [
    "combat_system_v1",
    "complete_combat_progression_v1",
    "live_mode_readiness_v1",
  ];
  const warnings: string[] = [];
  if (!installedModules.includes("combat_system_v1"))
    warnings.push("combat_system_v1_missing");
  if (!installedModules.includes("complete_combat_progression_v1"))
    warnings.push("complete_combat_progression_v1_missing");
...
export function simulateHarthmereLiveModeEndToEndScenarioV1() {
  const attackEnvelope: HarthmereLiveModeAuthorityEnvelopeV1 = {
    ...
    payload: {
      ...
      clientAnimationOnly: "sword_slash_v3",
    },
    clientClaims: {},
  };
  ...
  const deathValidation =
    validateHarthmereLiveModeAuthorityEnvelopeV1(deathEnvelope);
  const respawnValidation =
    validateHarthmereLiveModeAuthorityEnvelopeV1(respawnEnvelope);
  const attackPlan =
    buildHarthmereLiveModePersistenceMutationPlanV1(attackEnvelope);
  const deathPlan =
    buildHarthmereLiveModePersistenceMutationPlanV1(deathEnvelope);
  const respawnPlan =
    buildHarthmereLiveModePersistenceMutationPlanV1(respawnEnvelope);
