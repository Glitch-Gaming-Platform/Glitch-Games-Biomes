/*
 * Harthmere complete combat/progression rules current.
 *
 ...
 */
export const HARTHMERE_COMPLETE_COMBAT_PROGRESSION_VERSION = "harthmere-complete-combat-progression";

export type HarthmereAttribute = "strength" | "dexterity" | "intelligence" | "wisdom" | "constitution" | "charisma" | "perception" | "willpower" | "luck";
export type HarthmereClassId = keyof typeof HARTHMERE_CLASS_CATALOG;
export type HarthmereRole = "tank" | "healer" | "damage_dealer" | "support" | "controller" | "summoner" | "scout" | "crafter_utility" | "hybrid";
export type HarthmereSkillCategory = "combat" | "weapon" | "armor" | "magic" | "profession" | "gathering" | "crafting" | "social" | "exploration" | "survival" | "movement" | "stealth" | "leadership";
export type HarthmereResourceType = "none" | "mana" | "faith" | "rage" | "stamina" | "momentum" | "energy" | "combo_points" | "focus" | "conviction" | "holy_power" | "souls" | "corruption" | "nature_energy" | "spirit" | "inspiration" | "rhythm" | "chi" | "gadget_charge" | "bond";
export type HarthmereAbilityType = "basic_attack" | "active_ability" | "active_spell" | "basic_spell" | "passive_ability" | "toggle_ability" | "channeled_spell" | "cast_time_spell" | "instant_ability" | "ultimate_ability" | "reaction_ability" | "combo_ability" | "movement_ability" | "movement_spell" | "defensive_ability" | "defensive_spell" | "profession_ability" | "world_interaction_ability";
export type HarthmereTargetType = "self" | "enemy" | "ally" | "ally_or_enemy" | "party" | "area" | "cone" | "ground" | "object" | "corpse" | "dead_or_downed_ally" | "beast" | "pet" | "ally_or_object";
export type HarthmereWeaponType = "sword" | "axe" | "mace" | "spear" | "great_weapon" | "shield" | "dagger" | "short_sword" | "bow" | "crossbow" | "throwing" | "trap" | "staff" | "wand" | "orb" | "spell_focus" | "holy_symbol" | "tome" | "holy_weapon" | "skull_focus" | "sickle" | "claw_form" | "rapier" | "instrument" | "unarmed" | "fist_weapon" | "pistol" | "tool" | "gadget" | "engineering_gadget" | "summoning_focus";
export type HarthmereCombatState = "idle" | "alert" | "in_combat" | "casting" | "stunned" | "rooted" | "fleeing" | "evading" | "dead" | "downed" | "respawning" | "invulnerable" | "leashed" | "returning_home" | "teleporting" | "loading" | "spawn_protected";

export interface HarthmereClassDefinition {
  id: string;
  name: string;
  roles: HarthmereRole[];
  primaryAttributes: HarthmereAttribute[];
  secondaryAttributes: HarthmereAttribute[];
  armorAccess: string[];
  weaponAccess: string[];
  resourceTypes: HarthmereResourceType[];
...
export interface HarthmerePlayerProgressionState {
  playerId: string;
  classId: string;
  ...
  resources: Partial<Record<HarthmereResourceType, number>>;
  cooldowns: Record<string, number>;
  equipment: Record<string, string | undefined>;
  activeCasts?: Array<{ abilityId: string; startedAtMs: number; entityVersion: number }>;
  combatState?: HarthmereCombatState;
  pvpFlag?: string;
  restedXpPool?: number;
  serverEntityVersion?: number;
}

export interface HarthmereRewardContext {
  playerLevel: number;
  targetLevel: number;
...
export type HarthmereDeathDropCandidateCategory =
  | "trade_good"
  | "gathered_resource"
  | "material"
  | "currency"
...
export interface HarthmereDeathDropCandidate {
  itemId: string;
  count: number;
  category: HarthmereDeathDropCandidateCategory;
...
export const HARTHMERE_CLASS_CATALOG = {
  "warrior": {
    "id": "warrior",
    "name": "Warrior",
    "roles": [
...
} as Record<string, HarthmereClassDefinition>;

export const HARTHMERE_SKILL_CATALOG = {
  "melee_combat": {
    "id": "melee_combat",
    "name": "Melee Combat",
    "category": "combat",
...
} as Record<string, HarthmereSkillDefinition>;

export const HARTHMERE_ABILITY_CATALOG = {
  "basic_strike": {
    "id": "basic_strike",
    "name": "Basic Strike",
    "type": "basic_attack",
...
} as Record<string, HarthmereAbilityDefinition>;

export const HARTHMERE_EQUIPMENT_CATALOG = {
  "training_dagger": {
    "id": "training_dagger",
...
export const HARTHMERE_LOOT_TABLES = {
  "training_dummy": {
    "id": "training_dummy",
    "levelRange": [
      1,
...
export const HARTHMERE_NPC_COMBAT_PROFILES = {
  "wolf": {
    "id": "wolf",
    "name": "Wolf",
    "classId": "ranger",
...
export const HARTHMERE_COMPLETE_EDGE_CASE_REGISTRY = [
...
export const HARTHMERE_COMPLETE_TDD_TESTS = [
...
export function xpRequiredForHarthmereLevel(level: number): number {
  if (level >= 50) return Number.POSITIVE_INFINITY;
  return Math.round(100 * Math.pow(Math.max(1, level), 1.6) / 100) * 100;
}

export function levelDifferenceXpModifier(playerLevel: number, enemyLevel: number): number {
...
export function questXpReward(questLevel: number, difficulty: "trivial" | "easy" | "normal" | "hard" | "elite" | "group" | "dungeon" | "raid"): number {
...
export function enemyKillXpReward(context: HarthmereRewardContext): { xp: number; reasons: string[] } {
  ...
  let xp = Math.round(35 * Math.max(1, context.targetLevel) * levelDifferenceXpModifier(context.playerLevel, context.targetLevel) * rankModifier);
...
export function resolveHarthmereLevelGain(state: HarthmerePlayerProgressionState, xpDelta: number): {
  level: number;
  xp: number;
  ...
  while (level < 50 && xp >= xpRequiredForHarthmereLevel(level)) {
    xp -= xpRequiredForHarthmereLevel(level);
    level += 1;
...
export function validateHarthmereSkillProgressAction(args: {
  skillId: string;
  actionType: string;
  targetLevel?: number;
  playerLevel?: number;
  ...
  const skill = HARTHMERE_SKILL_CATALOG[args.skillId];
...
export function applyHarthmereSkillProgress(state: HarthmerePlayerProgressionState, skillId: string, xpDelta: number): {
  nextState: HarthmerePlayerProgressionState;
  leveledUp: boolean;
  unlocks: string[];
} {
  const skill = HARTHMERE_SKILL_CATALOG[skillId];
...
export function validateHarthmereEquipmentChange(args: {
  state: HarthmerePlayerProgressionState;
  itemId: string;
  slot: string;
  inCombat?: boolean;
  isCasting?: boolean;
  ...
  const item = HARTHMERE_EQUIPMENT_CATALOG[args.itemId];
...
export function applyHarthmereDurabilityLoss(item: { durability: { current: number; max: number } }, percent: number): { current: number; max: number; broken: boolean } {
...
export function repairCostCopper(item: { baseValueCopper: number; durability: { current: number; max: number } }, reputationModifier = 1): number {
...
export function validateHarthmereAbilityUse(args: {
  state: HarthmerePlayerProgressionState;
  abilityId: string;
  targetType: HarthmereTargetType;
  distance: number;
  ...
  const ability = HARTHMERE_ABILITY_CATALOG[args.abilityId];
...
export function resolveHarthmereAbilityCast(args: {
  state: HarthmerePlayerProgressionState;
  abilityId: string;
  nowMs: number;
  isPvp?: boolean;
...
export function validateHarthmereLoadoutChange(args: {
  state: HarthmerePlayerProgressionState;
...
export function validateHarthmereRespec(args: {
  state: HarthmerePlayerProgressionState;
  inCombat?: boolean;
...
export function buildHarthmereCombatStatsFromProgression(state: HarthmerePlayerProgressionState): Record<string, number> {
  ...
  for (const itemId of Object.values(state.equipment)) {
    const item = itemId ? HARTHMERE_EQUIPMENT_CATALOG[itemId] : undefined;
    if (!item) continue;
...
export function resolveHarthmereContributionScore(input: {
  damage?: number;
  healing?: number;
  shielding?: number;
  threat?: number;
...
export function resolveHarthmereGroupRewardEligibility(args: {
  contributionScore: number;
...
export function resolveHarthmerePvpRewardEligibility(args: {
...
export function selectHarthmereLootDrops(args: {
  lootTableId: string;
  playerLevel: number;
  targetLevel: number;
  ...
  const table = HARTHMERE_LOOT_TABLES[args.lootTableId];
...
export function resolveHarthmereKillRewards(args: HarthmereRewardContext & {
  lootTableId: string;
  contributionEligible: boolean;
}): { xp: number; loot: ReturnType<typeof selectHarthmereLootDrops>; reasons: string[] } {
  const xp = enemyKillXpReward(args);
  const loot = selectHarthmereLootDrops({
...
export function normalizeHarthmerePvpStats(stats: Record<string, number>, mode: "open_world" | "arena" | "battleground", playerLevel: number): Record<string, number> {
...
export function resolveHarthmereNpcCombatProfile(npcId: string, playerCount = 1): Record<string, unknown> {
  const npc = HARTHMERE_NPC_COMBAT_PROFILES[npcId];
...
export function resolveHarthmereDeathPenalty(args: {
...
export function selectHarthmereDeathDropItems(args: {
  mode: "pve" | "pvp" | "duel" | "arena" | "battleground" | "hardcore_pvp" | "boss" | "server_issue";
  candidates: HarthmereDeathDropCandidate[];
  maxStacks?: number;
  ...
  const allowedCategories: HarthmereDeathDropCandidateCategory[] = ["trade_good", "gathered_resource", "material"];
...
export function validateHarthmereServerAuthorityEnvelope(args: {
  requestId: string;
...
export function auditHarthmereCompleteCombatCoverage(): {
  ok: boolean;
  classCount: number;
  skillCount: number;
  abilityCount: number;
  ...
  const requiredSkillCategories: HarthmereSkillCategory[] = ["combat","weapon","armor","magic","profession","gathering","crafting","social","exploration","survival","movement","stealth","leadership"];
  for (const category of requiredSkillCategories) {
    if (!Object.values(HARTHMERE_SKILL_CATALOG).some((skill) => skill.category === category)) missing.push(`skill_category:${category}`);
  }
  for (const classId of Object.keys(HARTHMERE_CLASS_CATALOG)) {
    const cls = HARTHMERE_CLASS_CATALOG[classId];
  ...
  return {
    ok: missing.length === 0,
    classCount: Object.keys(HARTHMERE_CLASS_CATALOG).length,
    skillCount: Object.keys(HARTHMERE_SKILL_CATALOG).length,
    abilityCount: Object.keys(HARTHMERE_ABILITY_CATALOG).length,
    equipmentCount: Object.keys(HARTHMERE_EQUIPMENT_CATALOG).length,
    lootTableCount: Object.keys(HARTHMERE_LOOT_TABLES).length,
    npcProfileCount: Object.keys(HARTHMERE_NPC_COMBAT_PROFILES).length,
    missing,
  };
}
