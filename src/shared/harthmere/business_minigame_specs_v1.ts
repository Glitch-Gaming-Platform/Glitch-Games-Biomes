...
export const HARTHMERE_BUSINESS_MINIGAME_SPECS_VERSION_V1 =
  "harthmere-business-minigame-specs-v1" as const;

export type HarthmereBusinessMiniGameDecisionValueV1 =
  | boolean
  | number
  | string;

export type HarthmereBusinessMiniGameDecisionV1 = Record<
  string,
  HarthmereBusinessMiniGameDecisionValueV1
>;

export interface HarthmereBusinessMiniGameUiElementV1 {
  elementId: string;
  label: string;
  description: string;
}

export interface HarthmereBusinessMiniGameCustomerTypeV1 {
  customerTypeId: string;
  label: string;
  requirements: string;
}

export interface HarthmereBusinessMiniGameDifficultyTierV1 {
  tier: "starter" | "shop" | "company" | "regional";
  rule: string;
}

export interface HarthmereBusinessMiniGameOfferRuleV1 {
  offerId: string;
  exact: HarthmereBusinessMiniGameDecisionV1;
  min?: Record<string, number>;
  max?: Record<string, number>;
  blocked?: HarthmereBusinessMiniGameDecisionV1;
  notes: readonly string[];
}

export interface HarthmereBusinessMiniGameEdgeFailureV1 {
  failureId: string;
  when: HarthmereBusinessMiniGameDecisionV1;
  warning: string;
  penalty: string;
}

export interface HarthmereBusinessMiniGameSpecV1 {
  specId: string;
  ...
  coreMechanic: string;
  uniqueTwist: string;
  uiElements: readonly HarthmereBusinessMiniGameUiElementV1[];
  customerTypes: readonly HarthmereBusinessMiniGameCustomerTypeV1[];
  difficultyScaling: readonly HarthmereBusinessMiniGameDifficultyTierV1[];
  edgeCases: readonly string[];
  winConditions: readonly string[];
  loseConditions: readonly string[];
  offerRules: readonly HarthmereBusinessMiniGameOfferRuleV1[];
  edgeFailureActions: readonly HarthmereBusinessMiniGameEdgeFailureV1[];
  interiorFixtureLabels: readonly [string, string, string, string];
}

export interface HarthmereBusinessMiniGameDecisionResultV1 {
  ok: boolean;
  specId: string;
  typeId: HarthmereEconomyBusinessTypeIdV1;
  offerId: string;
...
function customer(
  ...
  label: string,
  requirements: string
): HarthmereBusinessMiniGameCustomerTypeV1 {
  return { customerTypeId, label, requirements };
}

function tier(
  tierName: HarthmereBusinessMiniGameDifficultyTierV1["tier"],
  rule: string
): HarthmereBusinessMiniGameDifficultyTierV1 {
...
function rule(
  ...
  exact: HarthmereBusinessMiniGameDecisionV1,
  notes: readonly string[],
  limits: Pick<HarthmereBusinessMiniGameOfferRuleV1, "blocked" | "max" | "min"> = {}
...
function spec(
  input: Omit<HarthmereBusinessMiniGameSpecV1, "specId">
): HarthmereBusinessMiniGameSpecV1 {
  return {
    specId: `${input.typeId}:business_minigame:${HARTHMERE_BUSINESS_MINIGAME_SPECS_VERSION_V1}`,
    ...input,
  };
}

export const HARTHMERE_BUSINESS_MINIGAME_SPECS_V1: Record<
...
> = {
  ...
  repair_maintenance_person: spec({
    ...
    uiElements: [
      ...
      ui("object_category_buttons", "Object category buttons", "Fixture Fix, Furniture Patch, Urgent Call."),
...
export function getHarthmereBusinessMiniGameSpecV1(
  typeId: HarthmereEconomyBusinessTypeIdV1
) {
  return HARTHMERE_BUSINESS_MINIGAME_SPECS_V1[typeId];
}

function decisionMatchesV1(
  decision: HarthmereBusinessMiniGameDecisionV1,
  expected: HarthmereBusinessMiniGameDecisionV1
) {
...
export function createHarthmereBusinessMiniGameDecisionForOfferV1(
  typeId: HarthmereEconomyBusinessTypeIdV1,
  offerId: string
): HarthmereBusinessMiniGameDecisionV1 {
  const spec = getHarthmereBusinessMiniGameSpecV1(typeId);
  const offerRule = spec?.offerRules.find((candidate) => candidate.offerId === offerId);
  return offerRule ? { ...offerRule.exact } : {};
}

export function resolveHarthmereBusinessMiniGameDecisionV1(input: {
  typeId: HarthmereEconomyBusinessTypeIdV1;
  offerId: string;
  decision: HarthmereBusinessMiniGameDecisionV1;
}): HarthmereBusinessMiniGameDecisionResultV1 {
  const spec = getHarthmereBusinessMiniGameSpecV1(input.typeId);
  ...
  for (const failure of spec.edgeFailureActions) {
    if (decisionMatchesV1(input.decision, failure.when)) {
