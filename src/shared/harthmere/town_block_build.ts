...
export const HARTHMERE_TOWN_BLOCK_BUILD_VERSION = "harthmere-town-block-build" as const;

/** A wall block is 1 meter wide and ~storyHeight tall when stacked. */
export const HARTHMERE_BLOCK_TILE_METERS = 1.0 as const;

/** Tolerated drift between adjacent blocks. Beyond this is a "gap" (FAIL). */
export const HARTHMERE_BLOCK_MAX_GAP_METERS = 1.2 as const;

/** Default story height for ordinary buildings (per current service code). */
export const HARTHMERE_STORY_HEIGHT_DEFAULT = 2.7 as const;

/** Tall story for chapels / great halls (per current service code). */
export const HARTHMERE_STORY_HEIGHT_TALL = 3.05 as const;

/** Front-door clearance required by MMO Rules §56. */
export const HARTHMERE_FRONT_DOOR_CLEARANCE_METERS = 2.0 as const;

/** Public-building entrance clearance required by MMO Rules §56. */
export const HARTHMERE_PUBLIC_ENTRANCE_CLEARANCE_METERS = 3.0 as const;

/** Shop customer space required by MMO Rules §56. */
export const HARTHMERE_SHOP_CUSTOMER_SPACE_METERS = 4.0 as const;

/** Minimum free interior tile area to host an NPC stand-and-path slot. */
export const HARTHMERE_INTERIOR_FREE_TILES_MIN = 4 as const; // 2x2
...
export type HarthmereBlockBuildShellSpec = {
  ...
  floors: number;
  /** Story height; defaults to HARTHMERE_STORY_HEIGHT_DEFAULT. */
...
export type HarthmereBlockBuildRules = {
  version: typeof HARTHMERE_TOWN_BLOCK_BUILD_VERSION;
  tileMeters: number;
  maxGapMeters: number;
...
export const HARTHMERE_TOWN_BLOCK_BUILD_RULES: HarthmereBlockBuildRules = {
  version: HARTHMERE_TOWN_BLOCK_BUILD_VERSION,
  tileMeters: HARTHMERE_BLOCK_TILE_METERS,
  maxGapMeters: HARTHMERE_BLOCK_MAX_GAP_METERS,
  storyHeightDefault: HARTHMERE_STORY_HEIGHT_DEFAULT,
  storyHeightTall: HARTHMERE_STORY_HEIGHT_TALL,
  frontDoorClearance: HARTHMERE_FRONT_DOOR_CLEARANCE_METERS,
  publicEntranceClearance: HARTHMERE_PUBLIC_ENTRANCE_CLEARANCE_METERS,
  shopCustomerSpace: HARTHMERE_SHOP_CUSTOMER_SPACE_METERS,
  interiorFreeTilesMin: HARTHMERE_INTERIOR_FREE_TILES_MIN,
} as const;
...
export function harthmereGenerateStoryWallBlocks(
  ...
  const storyHeight = shell.storyHeightMeters ?? HARTHMERE_STORY_HEIGHT_DEFAULT;
  const floorBaseY = (floor - 1) * storyHeight;
  const tile = HARTHMERE_BLOCK_TILE_METERS;
...
export const HARTHMERE_BIBLE_REQUIRED_BUILDINGS = [
  // Town gate / road
...
export const HARTHMERE_BIBLE_REQUIRED_DUNGEON_ROOMS = [
...
/**
 * The renderer must call `createHarthmereContinuousBlockWalls(shell)` to
 ...
 */
export const HARTHMERE_RENDERER_BLOCK_WALL_ENTRY_NAME = "createHarthmereContinuousBlockWalls" as const;
