export interface HarthmereLocationVoiceMetadata {
  id: string;
  name: string;
  story: string;
  bounds: {
...
export const HARTHMERE_LOCATION_VOICE_METADATA: readonly HarthmereLocationVoiceMetadata[] =
  [
    {
      id: "grove_job_board",
...
export function harthmereLocationVoiceMetadataForPosition(
  position: readonly [number, number, number] | undefined
): HarthmereLocationVoiceMetadata | undefined {
  if (!position) {
    return undefined;
  }
  const [x, , z] = position;
  return HARTHMERE_LOCATION_VOICE_METADATA.find(
    (location) =>
