...
export const HARTHMERE_CARE_LOOPS_VERSION_V1 = "harthmere-care-loops-v1" as const;
...
export const HARTHMERE_CARE_DAILY_ACTIVITIES_V1: Record<string, {
...
export function normalizeHarthmereCareLoopStateV1(
  ...
  return {
    ...
    npcs: typeof parsed.npcs === "object" && parsed.npcs !== null ? parsed.npcs : {},
    projects: { ...defaults.projects, ...(typeof parsed.projects === "object" && parsed.projects !== null ? parsed.projects : {}) },
    collections: typeof parsed.collections === "object" && parsed.collections !== null ? parsed.collections : {},
    decorations: typeof parsed.decorations === "object" && parsed.decorations !== null ? parsed.decorations : {},
    townNeeds: { ...defaults.townNeeds, ...(typeof parsed.townNeeds === "object" && parsed.townNeeds !== null ? parsed.townNeeds : {}) },
    skills: typeof parsed.skills === "object" && parsed.skills !== null ? parsed.skills : {},
    seasonal: typeof parsed.seasonal === "object" && parsed.seasonal !== null ? parsed.seasonal : {},
  };
}
...
function result(
  ...
  return { care, warnings, touchedModels, itemDeltas, goldDelta, xpDelta, unlocked };
}

function addItems(items: Record<string, number>, deltas: Record<string, number>) {
  const next = { ...items };
...
function hasItems(inventory: Record<string, number>, needed: Record<string, number>) {
  return Object.entries(needed).every(([itemId, count]) => (inventory[itemId] ?? 0) >= count);
}

function negativeItems(needed: Record<string, number>) {
  return Object.fromEntries(Object.entries(needed).map(([itemId, count]) => [itemId, -count]));
}
...
function addSkillXp(care: HarthmereCareLoopStateV1, skillId: string, xp: number) {
...
function npcMemory(care: HarthmereCareLoopStateV1, npcId: string): HarthmereNpcCareMemoryV1 {
  return care.npcs[npcId] ?? {
    npcId,
    relationship: 0,
...
export function reduceHarthmereCareLoopV1(
  ...
  if (request.operation === "npc_talk") {
    if (!targetId) return result(care, ["care_rejected:missing_npc_id"], ["care_npc_rejection"]);
    const memory = npcMemory(care, targetId);
    if (memory.lastTalkDay === day) return result(care, ["care_rejected:npc_already_talked_today"], ["care_npc_rejection"]);
    const relationship = memory.relationship + 3;
    const unlocked = relationship >= 10 && !memory.unlockedDialogue.includes("trust_1")
      ? ["trust_1"]
      : [];
    care = {
      ...care,
    ...
    return result(care, [], ["care_npc_talk", "care_relationship"], {}, 1, 6, unlocked);
  }

  if (request.operation === "npc_gift") {
    if (!targetId) return result(care, ["care_rejected:missing_npc_id"], ["care_npc_rejection"]);
    if (!request.itemId) return result(care, ["care_rejected:missing_gift_item"], ["care_npc_rejection"]);
    if ((inventory[request.itemId] ?? 0) < 1) return result(care, ["care_rejected:missing_gift_inventory"], ["care_npc_rejection"]);
    const memory = npcMemory(care, targetId);
    if (memory.lastGiftDay === day) return result(care, ["care_rejected:npc_already_gifted_today"], ["care_npc_rejection"]);
    const liked = (HARTHMERE_CARE_NPC_PREFERENCES_V1[targetId] ?? []).includes(request.itemId);
    const relationship = memory.relationship + (liked ? 10 : 2);
    const unlocked = [
      ...(relationship >= 10 && !memory.unlockedDialogue.includes("trust_1") ? ["trust_1"] : []),
      ...(relationship >= 25 && !memory.unlockedDialogue.includes("personal_request") ? ["personal_request"] : []),
    ];
    care = {
      ...care,
      npcs: {
    ...
    return result(care, [], ["care_npc_gift", liked ? "care_npc_liked_gift" : "care_npc_neutral_gift"], { [request.itemId]: -1 }, 0, liked ? 10 : 3, unlocked);
  }

  if (request.operation === "restore_project") {
    const project = care.projects[targetId];
    if (!project) return result(care, ["care_rejected:unknown_project"], ["care_project_rejection"]);
    if (project.completedAtMs) return result(care, ["care_rejected:project_complete"], ["care_project_rejection"]);
    if (!hasItems(inventory, project.requiredItems)) return result(care, ["care_rejected:missing_project_materials"], ["care_project_rejection"]);
    const nextStage = project.stage + 1;
    const completed = nextStage >= project.maxStage;
    const unlocked = completed ? [`project:${project.projectId}:complete`] : [`project:${project.projectId}:stage_${nextStage}`];
    care = bumpNeed({
      ...care,
      projects: {
        ...care.projects,
    ...
    }, project.projectId.includes("food") ? "food" : "happiness", completed ? 12 : 5);
    return result(care, [], ["care_restoration", "care_town_state"], negativeItems(project.requiredItems), 0, completed ? 40 : 15, unlocked);
  }

  if (request.operation === "production_sale") {
    const itemId = request.itemId ?? "";
    const count = Math.max(1, Math.floor(request.count ?? 1));
    if ((inventory[itemId] ?? 0) < count) return result(care, ["care_rejected:missing_sale_inventory"], ["care_production_rejection"]);
    const isFood = Boolean(HARTHMERE_FOOD_DEFINITIONS_V1[itemId]) || /food|meal|bread|meat|carrot|trout|berries|ration/i.test(itemId);
    care = bumpNeed(addSkillXp(care, isFood ? "cooking" : "trading", count * 8), isFood ? "food" : "happiness", isFood ? count : 1);
    return result(care, [], ["care_production_profit", isFood ? "care_food_market" : "care_market"], { [itemId]: -count }, count * (isFood ? 5 : 3), count * 5);
  }

  if (request.operation === "collection_donation") {
    const itemId = request.itemId ?? "";
    if (!itemId) return result(care, ["care_rejected:missing_collection_item"], ["care_collection_rejection"]);
    if (care.collections[itemId]) return result(care, ["care_rejected:collection_duplicate"], ["care_collection_rejection"]);
    if ((inventory[itemId] ?? 0) < 1) return result(care, ["care_rejected:missing_collection_inventory"], ["care_collection_rejection"]);
    const category = HARTHMERE_SEED_DEFINITIONS_V1[itemId] ? "seed_catalog" : HARTHMERE_FOOD_DEFINITIONS_V1[itemId] ? "food_catalog" : "town_archive";
    care = {
      ...care,
      collections: { ...care.collections, [itemId]: { donatedAtMs: request.nowMs, category } },
    };
    return result(care, [], ["care_collection", category], { [itemId]: -1 }, 0, 12, [`collection:${category}:${itemId}`]);
  }

  if (request.operation === "decorate_space") {
    if (!targetId) return result(care, ["care_rejected:missing_decor_slot"], ["care_decor_rejection"]);
    if (!request.itemId) return result(care, ["care_rejected:missing_decor_item"], ["care_decor_rejection"]);
    if (care.decorations[targetId]) return result(care, ["care_rejected:decor_slot_occupied"], ["care_decor_rejection"]);
    if ((inventory[request.itemId] ?? 0) < 1) return result(care, ["care_rejected:missing_decor_inventory"], ["care_decor_rejection"]);
    care = bumpNeed({
      ...care,
      decorations: { ...care.decorations, [targetId]: { itemId: request.itemId, placedAtMs: request.nowMs } },
    }, "happiness", 4);
    return result(care, [], ["care_decor", "care_expression"], { [request.itemId]: -1 }, 0, 8, [`decor:${targetId}`]);
  }

  if (request.operation === "explore_forage") {
    const itemId = request.itemId ?? "wild_berries";
    if (!HARTHMERE_FOOD_DEFINITIONS_V1[itemId] && !HARTHMERE_SEED_DEFINITIONS_V1[itemId]) {
      return result(care, ["care_rejected:unknown_forage"], ["care_exploration_rejection"]);
    }
    care = addSkillXp(care, "foraging", 10);
    return result(care, [], ["care_explore_forage"], { [itemId]: 1 }, 0, 10, [`forage:${itemId}`]);
  }

  if (request.operation === "town_life_help") {
    const need = targetId || "happiness";
    care = bumpNeed(addSkillXp(care, "community", 10), need, 6);
    return result(care, [], ["care_town_life", `care_need:${need}`], {}, 4, 10, [`town_need:${need}`]);
  }

  if (request.operation === "skill_mastery") {
    const skillId = targetId || "farming";
    care = addSkillXp(care, skillId, Math.max(1, request.count ?? 12));
    return result(care, [], ["care_skill_mastery", `care_skill:${skillId}`], {}, 0, 0, care.skills[skillId]?.level >= 2 ? [`skill:${skillId}:level_${care.skills[skillId].level}`] : []);
  }
  ...
  if (request.operation === "seasonal_discovery") {
    ...
    if (!HARTHMERE_CARE_SEASONAL_DISCOVERIES_V1[season].includes(itemId)) {
      return result(care, ["care_rejected:item_not_in_season"], ["care_season_rejection"]);
    }
    const key = `${season}:${itemId}`;
    if (care.seasonal[key]) return result(care, ["care_rejected:seasonal_duplicate"], ["care_season_rejection"]);
    care = {
      ...care,
      seasonal: { ...care.seasonal, [key]: request.nowMs },
    };
    return result(care, [], ["care_seasonal", `care_season:${season}`], { [itemId]: 1 }, 0, 14, [`seasonal:${key}`]);
  }

  return result(care, ["care_rejected:unsupported_operation"], ["care_rejection"]);
}
