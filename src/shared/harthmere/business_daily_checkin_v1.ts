...
// caller supplies an integer day index (see harthmereDayIndexV1 on the client);
// time is never read here.

export const BUSINESS_CHECKIN_REWARD_GOLD_V1 = 500;
// Revenue cut for the first missed day.
export const BUSINESS_NEGLECT_BASE_PENALTY_V1 = 0.2;
// Extra cut added for each additional consecutive missed day (the acceleration).
export const BUSINESS_NEGLECT_ACCELERATION_V1 = 0.05;
...
export const BUSINESS_NEGLECT_FACTOR_FLOOR_V1 = -2.0;

export interface BusinessDailyCheckInStateV1 {
...
export function initBusinessDailyCheckInStateV1(): BusinessDailyCheckInStateV1 {
  return {
...
export function businessMissedDaysV1(
  state: Pick<BusinessDailyCheckInStateV1, "lastCheckInDay">,
  todayDay: number
): number {
  if (
...
function cumulativeNeglectPenaltyV1(d: number): number {
  if (d <= 0) {
    return 0;
  }
  return (
    BUSINESS_NEGLECT_BASE_PENALTY_V1 * d +
    (BUSINESS_NEGLECT_ACCELERATION_V1 * (d * (d - 1))) / 2
  );
}
...
// the cumulative penalty exceeds 1, floored at BUSINESS_NEGLECT_FACTOR_FLOOR_V1.
//   missed 0 -> 1.00, 1 -> 0.80, 2 -> 0.55, 3 -> 0.25, 4 -> -0.10, 5 -> -0.50, ...
export function businessNeglectRevenueFactorV1(missedDays: number): number {
  if (!Number.isFinite(missedDays) || missedDays <= 0) {
    return 1;
  }
  const factor = 1 - cumulativeNeglectPenaltyV1(Math.floor(missedDays));
  return Math.max(BUSINESS_NEGLECT_FACTOR_FLOOR_V1, factor);
}
...
export function businessNeglectRevenueLostV1(
  missedDays: number,
  ...
  for (let k = 1; k <= days; k += 1) {
    lost += (1 - businessNeglectRevenueFactorV1(k)) * baseDailyRevenue;
  }
  return lost;
}

export interface BusinessCheckInResultV1 {
  state: BusinessDailyCheckInStateV1;
...
export function processBusinessCheckInV1(
  state: BusinessDailyCheckInStateV1,
  todayDay: number,
  baseDailyRevenue: number
): BusinessCheckInResultV1 {
  ...
  const missedDays = businessMissedDaysV1(state, day);
  const revenueLostThisGap = businessNeglectRevenueLostV1(
    missedDays,
    baseDailyRevenue
  );
  const continued = state.lastCheckInDay !== undefined && missedDays === 0;
  const streak = continued ? state.currentStreak + 1 : 1;
  const next: BusinessDailyCheckInStateV1 = {
    lastCheckInDay: day,
    currentStreak: streak,
    longestStreak: Math.max(state.longestStreak, streak),
    totalGoldFromCheckIns:
      state.totalGoldFromCheckIns + BUSINESS_CHECKIN_REWARD_GOLD_V1,
  ...
  return {
    state: next,
    checkedIn: true,
    goldGranted: BUSINESS_CHECKIN_REWARD_GOLD_V1,
    revenueLostThisGap,
    missedDays,
    streak,
  };
...
export interface BusinessCheckInStatusV1 {
...
export function businessCheckInStatusV1(
  state: BusinessDailyCheckInStateV1,
  todayDay: number,
  baseDailyRevenue: number
  ...
  const revenueLostIfSkipToday =
    businessNeglectRevenueLostV1(missedDays + 1, baseDailyRevenue) -
    businessNeglectRevenueLostV1(missedDays, baseDailyRevenue);
  return {
    checkedInToday,
    missedDays,
    currentRevenueFactor: businessNeglectRevenueFactorV1(missedDays),
