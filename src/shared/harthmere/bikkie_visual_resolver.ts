...
import type { HarthmereItemObjectMetadata } from "./mmo_inventory_authority";

export const HARTHMERE_BIKKIE_VISUAL_RESOLVER_VERSION =
...
export interface HarthmereBikkieVisualResolverInput {
  id?: string;
  visualId?: string;
...
export interface HarthmereResolvedBikkieVisual {
  version: typeof HARTHMERE_BIKKIE_VISUAL_RESOLVER_VERSION;
  visualId: string;
  source: HarthmereResolvedBikkieVisualSource;
  label: string;
  ariaLabel: string;
  bikkieId?: BiomesId | string;
  kind?: string;
...
export function harthmereBikkieColorHex(
  ...
  const hash = hashString(`${text}:${fallbackText}`);
  return hslToHex(hash % 360, 48, 48);
}

export function harthmereInitialsGlyph(
...
function visualShapeForInput(
  input: HarthmereBikkieVisualResolverInput
): HarthmereResolvedBikkieVisualShape {
...
function suggestedShapeFor(shape: HarthmereResolvedBikkieVisualShape) {
  switch (shape) {
    case "block":
...
export function harthmereResolveBikkieVisual(
  input: HarthmereBikkieVisualResolverInput
): HarthmereResolvedBikkieVisual {
  const label =
    input.label ??
    input.displayName ??
    input.bikkieName ??
  ...
  const shape = visualShapeForInput(input);
  const colors = explicitColors.length
    ? explicitColors
    : fallbackColorsForInput(input, shape);
  ...
  return {
    ...
    role: input.role,
    shape,
    glyph: harthmereInitialsGlyph(label),
    colors,
    hexColors,
    primaryHex,
    accentHex,
