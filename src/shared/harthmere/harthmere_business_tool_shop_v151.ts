// HARTHMERE_BUSINESS_TOOL_SHOP_V151
//
...
// registers HARTHMERE_BUSINESS_TOOL_SHOP_NEW_TOOL_DEFS_V151 into its inventory item
...
import { harthmereBusinessOwnerMarkerIdV151 } from "@/shared/harthmere/business_owner_npc_seed_v1";
import type { HarthmereEconomyBusinessTypeIdV1 } from "@/shared/harthmere/mmo_economy_authority_v1";

export const HARTHMERE_BUSINESS_TOOL_SHOP_VERSION_V151 =
  "harthmere-business-tool-shop-v151" as const;

export interface HarthmereBusinessToolListingV151 {
...
const HARTHMERE_BUSINESS_TOOL_SHOP_V151: Readonly<
  Record<
...
export interface HarthmereBusinessNewToolDefV151 {
  itemId: string;
  name: string;
  subtype: string;
  icon: string;
...
export const HARTHMERE_BUSINESS_TOOL_SHOP_NEW_TOOL_DEFS_V151: readonly HarthmereBusinessNewToolDefV151[] =
  [
...
const HARTHMERE_BUSINESS_OWNER_NPC_ID_BY_TYPE_V151: Readonly<
  Record<string, string>
> = Object.fromEntries(
  HARTHMERE_BUSINESS_OUTPOSTS_V1.map((outpost: HarthmereBusinessOutpostV1) => [
    outpost.businessType,
    outpost.ownerNpcId,
  ])
);

export function harthmereBusinessToolForTypeV151(
  businessType: string | undefined
): HarthmereBusinessToolListingV151 | undefined {
  if (!businessType) {
    return undefined;
  }
  const entry =
  ...
  return {
    businessType: businessType as HarthmereEconomyBusinessTypeIdV1,
    ...entry,
  };
}

// Reverse lookup: which businessType sells this tool id (each tool is unique).
export function harthmereBusinessTypeSellingToolV151(
  toolItemId: string | undefined
): HarthmereEconomyBusinessTypeIdV1 | undefined {
  if (!toolItemId) {
    return undefined;
  }
  for (const [businessType, entry] of Object.entries(
    ...
    if (entry.toolItemId === toolItemId) {
      return businessType as HarthmereEconomyBusinessTypeIdV1;
    }
  }
  return undefined;
}
...
export function harthmereBusinessToolVendorMarkerIdV151(
  toolItemId: string | undefined
): string | undefined {
  const businessType = harthmereBusinessTypeSellingToolV151(toolItemId);
  if (!businessType) {
    return undefined;
  }
  const ownerNpcId = HARTHMERE_BUSINESS_OWNER_NPC_ID_BY_TYPE_V151[businessType];
  return ownerNpcId ? harthmereBusinessOwnerMarkerIdV151(ownerNpcId) : undefined;
}
...
export interface HarthmereBusinessToolPurchaseOutcomeV151 {
  ok: boolean;
  reason?: "no_tool" | "already_owned" | "insufficient_gold";
  goldAfter: number;
  listing?: HarthmereBusinessToolListingV151;
}

export function harthmereBusinessToolPurchaseOutcomeV151(input: {
  businessType: string | undefined;
  goldAvailable: number;
  alreadyOwned: boolean;
}): HarthmereBusinessToolPurchaseOutcomeV151 {
  const gold = Math.max(0, Math.floor(input.goldAvailable || 0));
  const listing = harthmereBusinessToolForTypeV151(input.businessType);
  if (!listing) {
...
export function harthmereBusinessToolListingsV151(): HarthmereBusinessToolListingV151[] {
  return Object.entries(HARTHMERE_BUSINESS_TOOL_SHOP_V151).map(
    ([businessType, entry]) => ({
      businessType: businessType as HarthmereEconomyBusinessTypeIdV1,
      ...entry,
    })
...
export function validateHarthmereBusinessToolShopV151(): string[] {
  const errors: string[] = [];
  const listings = harthmereBusinessToolListingsV151();
  // One listing per real outpost business type.
  const outpostTypes = new Set(
    HARTHMERE_BUSINESS_OUTPOSTS_V1.map((outpost) => outpost.businessType)
  );
  for (const type of outpostTypes) {
    if (!HARTHMERE_BUSINESS_TOOL_SHOP_V151[type]) {
  ...
  const newIds = HARTHMERE_BUSINESS_TOOL_SHOP_NEW_TOOL_DEFS_V151.map(
    (def) => def.itemId
  );
