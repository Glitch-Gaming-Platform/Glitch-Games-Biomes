...
export const HARTHMERE_DAILY_STREAK_BONUS_GOLD = 25;
export const HARTHMERE_DAILY_STREAK_BONUS_CAP_DAYS = 7;
export const HARTHMERE_DAILY_TASK_COUNT = 8;
...
export type HarthmereSeason = "spring" | "summer" | "autumn" | "winter";

export interface HarthmereNpcCareMemory {
  npcId: string;
  relationship: number;
  lastTalkDay?: number;
  lastGiftDay?: number;
...
export interface HarthmereCareProject {
  projectId: string;
  name: string;
  stage: number;
  maxStage: number;
...
export interface HarthmereCareLoopState {
  actorId: string;
  daily: {
    lastLoginDay?: number;
  ...
  npcs: Record<string, HarthmereNpcCareMemory>;
  projects: Record<string, HarthmereCareProject>;
...
export interface HarthmereCareLoopRequest {
  requestId: string;
  actorId: string;
  operation: HarthmereCareLoopKind;
  nowMs: number;
  targetId?: string;
  itemId?: string;
  count?: number;
...
export const HARTHMERE_CARE_NPC_PREFERENCES: Record<string, string[]> = {
...
export const HARTHMERE_CARE_DAILY_ACTIVITIES: Record<
  string,
  {
...
export function harthmereDailyTaskXpReward(input: {
  actorLevel?: number;
  taskCount?: number;
}) {
  const taskCount = Math.max(
    1,
    Math.trunc(input.taskCount ?? HARTHMERE_DAILY_TASK_COUNT)
  );
  const level = Math.max(1, Math.trunc(input.actorLevel ?? 1));
  const xpToNextLevel =
    HARTHMERE_SKILL_XP_PER_LEVEL * Math.max(1, Math.floor(Math.sqrt(level)));
  return Math.max(1, Math.ceil((xpToNextLevel * 0.5) / taskCount));
}

export interface HarthmereCareLoopClientSnapshot {
  actorId: string;
  day: number;
  streak: number;
...
export function createHarthmereCareLoopClientSnapshot(
  state: HarthmereCareLoopState,
  nowMs: number
): HarthmereCareLoopClientSnapshot {
  const day = harthmereCareDay(nowMs);
  const prefix = `${day}:`;
...
export const HARTHMERE_CARE_SEASONAL_DISCOVERIES: Record<
  HarthmereSeason,
  string[]
> = {
...
export function harthmereCareDay(nowMs: number): number {
  return Math.floor(nowMs / HARTHMERE_CARE_LOOP_DAY_MS);
}

export function defaultHarthmereCareLoopState(
  actorId: string,
  _nowMs: number
): HarthmereCareLoopState {
  return {
    actorId,
...
export function normalizeHarthmereCareLoopState(
  raw: unknown,
  actorId: string,
  nowMs: number
): HarthmereCareLoopState {
  const defaults = defaultHarthmereCareLoopState(actorId, nowMs);
...
function bumpNeed(care: HarthmereCareLoopState, need: string, delta: number) {
  return {
    ...care,
    townNeeds: {
      ...care.townNeeds,
...
function npcMemory(
  care: HarthmereCareLoopState,
  npcId: string
): HarthmereNpcCareMemory {
  return (
    care.npcs[npcId] ?? {
      npcId,
      relationship: 0,
      knownPreferences: HARTHMERE_CARE_NPC_PREFERENCES[npcId] ?? [],
      unlockedDialogue: [],
    }
  );
}

export function reduceHarthmereCareLoop(
  state: HarthmereCareLoopState,
  request: HarthmereCareLoopRequest
): HarthmereCareLoopResult {
  const day = harthmereCareDay(request.nowMs);
  const targetId = request.targetId ?? "";
  const inventory = request.inventory ?? {};
  let care = normalizeHarthmereCareLoopState(
    state,
    request.actorId,
    request.nowMs
  );
  ...
  if (request.operation === "daily_check_in") {
    ...
    const streak =
      previousDay === undefined
        ...
        ? care.daily.streak
        : 1;
    const reward = HARTHMERE_CARE_DAILY_ACTIVITIES[activity] ?? {};
    const xpReward =
      reward.xp ??
      harthmereDailyTaskXpReward({ actorLevel: request.actorLevel });
    ...
    const streakBonusGold =
      activity === "check_in"
        ? Math.min(
            Math.max(0, streak - 1),
            HARTHMERE_DAILY_STREAK_BONUS_CAP_DAYS - 1
  ...
  if (request.operation === "production_sale") {
    ...
    const isFood =
      Boolean(HARTHMERE_FOOD_DEFINITIONS[itemId]) ||
  ...
  if (request.operation === "collection_donation") {
    ...
    const category = HARTHMERE_SEED_DEFINITIONS[itemId]
      ? "seed_catalog"
  ...
  if (request.operation === "explore_forage") {
    ...
    if (
      !HARTHMERE_FOOD_DEFINITIONS[itemId] &&
      !HARTHMERE_SEED_DEFINITIONS[itemId]
    ) {
      return result(
        care,
  ...
  if (request.operation === "seasonal_discovery") {
    ...
    const itemId = request.itemId ?? "";
    if (!HARTHMERE_CARE_SEASONAL_DISCOVERIES[season].includes(itemId)) {
      return result(
        care,
...
export function applyHarthmereCareLoopInventoryDeltas(
  inventory: Record<string, number>,
  deltas: Record<string, number>
) {
