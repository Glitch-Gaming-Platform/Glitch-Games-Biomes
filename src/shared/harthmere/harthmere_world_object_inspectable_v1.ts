// HARTHMERE_WORLD_OBJECT_INSPECTABLE_V1
//
...
// full ECS entities (see harthmere_quest_object_markers_v145.ts). Because they
...
// talk-radius fallback (getNearbyNpcTalkInspectableOverlayV140): given the
...
import {
  ...
  type HarthmereObjectInteractionV1,
} from "@/shared/harthmere/object_interaction_semantics_v1";

export const HARTHMERE_WORLD_OBJECT_INSPECTABLE_VERSION_V1 =
  "harthmere-world-object-inspectable-v1" as const;

// Mirrors the NPC talk-radius tuning so objects and NPCs feel consistent.
export const HARTHMERE_WORLD_OBJECT_INSPECT_RADIUS_V1 = 6.5;
export const HARTHMERE_WORLD_OBJECT_INSPECT_CLOSE_RADIUS_V1 = 2.75;
export const HARTHMERE_WORLD_OBJECT_INSPECT_MIN_VIEW_DOT_V1 = 0.15;

export type HarthmereWorldObjectVec3V1 = readonly [number, number, number];

export interface HarthmereWorldObjectCandidateV1 {
  id: string;
  label: string;
  position: HarthmereWorldObjectVec3V1;
  entityDescription?: string;
}

export interface HarthmereWorldObjectInspectableV1 {
  id: string;
  label: string;
  entityDescription?: string;
  position: HarthmereWorldObjectVec3V1;
...
export interface SelectHarthmereWorldObjectInspectableInputV1 {
  playerPosition: HarthmereWorldObjectVec3V1;
  facingView: HarthmereWorldObjectVec3V1;
  candidates: readonly HarthmereWorldObjectCandidateV1[];
  radius?: number;
  closeRadius?: number;
  minViewDot?: number;
}
...
export function isHarthmereInspectableWorldObjectV1(input: {
  label?: string | null;
...
export function harthmereWorldObjectCandidateScoreV1(input: {
  playerPosition: HarthmereWorldObjectVec3V1;
  facingView: HarthmereWorldObjectVec3V1;
  objectPosition: HarthmereWorldObjectVec3V1;
  radius?: number;
  closeRadius?: number;
  minViewDot?: number;
}): number | undefined {
  const radius = input.radius ?? HARTHMERE_WORLD_OBJECT_INSPECT_RADIUS_V1;
  const closeRadius =
    input.closeRadius ?? HARTHMERE_WORLD_OBJECT_INSPECT_CLOSE_RADIUS_V1;
  const minViewDot =
    input.minViewDot ?? HARTHMERE_WORLD_OBJECT_INSPECT_MIN_VIEW_DOT_V1;
...
export function selectNearestHarthmereWorldObjectInspectableV1(
  input: SelectHarthmereWorldObjectInspectableInputV1
): HarthmereWorldObjectInspectableV1 | undefined {
  let best: HarthmereWorldObjectInspectableV1 | undefined;
  for (const candidate of input.candidates) {
    const labelInput = {
      label: candidate.label,
      entityDescription: candidate.entityDescription,
    };
    if (!isHarthmereInspectableWorldObjectV1(labelInput)) {
      continue;
    }
    const score = harthmereWorldObjectCandidateScoreV1({
    ...
    const interaction =
      harthmereObjectInteractionForLabelV1(labelInput) ?? {
        kind: "inspect",
        title: "Inspect",
        toastVerb: "Inspected",
      };
    best = {
      ...
      interaction,
      isContainer: isHarthmereContainerObjectLabelV1(labelInput),
      score,
    };
  }
  return best;
