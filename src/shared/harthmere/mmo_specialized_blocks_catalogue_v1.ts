// ---------------------------------------------------------------------------
// HARTHMERE_SPECIALIZED_BLOCKS_V1
//
...
export const HARTHMERE_NATURAL_BLOCK_ITEM_IDS_V1 = [
  // soils / loose ground
  "dirt",
  "grass",
  "soil",
...
const NATURAL_BLOCK_SET_V1: ReadonlySet<string> = new Set(
...
export function isHarthmereNaturalBlockV1(itemId: string): boolean {
  return NATURAL_BLOCK_SET_V1.has(itemId);
}
...
export const HARTHMERE_SPECIALIZED_BLOCK_STATIONS_V1 = {
...
export type HarthmereSpecializedBlockStationKeyV1 =
  keyof typeof HARTHMERE_SPECIALIZED_BLOCK_STATIONS_V1;

/** Block stations include the new ones plus the existing Workbench (wood). */
export type HarthmereBlockStationKeyV1 =
  | HarthmereSpecializedBlockStationKeyV1
  | "workbench";

/** Existing production Workbench station id (wood is worked here per the bible). */
const EXISTING_WORKBENCH_STATION_ID_V1 = "1534621126189448";

interface ResolvedBlockStationV1 {
  stationId: string;
  stationType: string;
  skillId: string;
  displayName: string;
...
function resolveBlockStationV1(
  key: HarthmereBlockStationKeyV1
): ResolvedBlockStationV1 {
  if (key === "workbench") {
    return {
      stationId: EXISTING_WORKBENCH_STATION_ID_V1,
      stationType: "general",
      skillId: "carpentry",
      displayName: "Workbench",
    };
  }
  const seed = SPECIALIZED_STATION_SEEDS_V1.find((s) => s.key === key)!;
  return {
...
interface SpecializedStationSeedV1 {
  key: HarthmereSpecializedBlockStationKeyV1;
  stationId: string;
  displayName: string;
  stationType: string;
  size: string;
...
const SPECIALIZED_STATION_SEEDS_V1: SpecializedStationSeedV1[] = [
  {
    key: "stonecutter",
  ...
  {
    key: "kiln",
    stationId: HARTHMERE_SPECIALIZED_BLOCK_STATIONS_V1.kiln,
    displayName: "Kiln",
    stationType: "kiln",
    size: "2x2x3",
  ...
  {
    key: "loom",
    stationId: HARTHMERE_SPECIALIZED_BLOCK_STATIONS_V1.loom,
    displayName: "Loom",
    stationType: "weaving",
    size: "4x1x3",
  ...
  {
    key: "alchemyBench",
    stationId: HARTHMERE_SPECIALIZED_BLOCK_STATIONS_V1.alchemyBench,
...
export type HarthmereBlockVendorTypeV1 =
  | "mason"
  | "carpenter"
  | "kiln_worker"
  | "forge_trader"
...
interface VendorBindingV1 {
  vendorId: string;
  /** Fraction of base value the vendor pays when buying a block back. */
  buyModifier: number;
}

const VENDOR_TYPE_BINDING_V1: Record<
  HarthmereBlockVendorTypeV1,
  VendorBindingV1
> = {
...
export type HarthmereBlockClassV1 =
  | "common_refined" // 3-6 gold
  | "decorative" // 6-12 gold
  | "industrial" // 12-25 gold
  | "rare"; // 30-80 gold

const BLOCK_CLASS_TIER_V1: Record<HarthmereBlockClassV1, number> = {
  common_refined: 1,
  decorative: 2,
  industrial: 4,
  rare: 5,
...
export interface HarthmereSpecializedBlockSpecV1 {
  itemId: string; // real bikkie terrain id
  displayName: string;
  station: HarthmereBlockStationKeyV1;
  vendorType: HarthmereBlockVendorTypeV1;
  blockClass: HarthmereBlockClassV1;
...
function buildStoneSpecs(): HarthmereSpecializedBlockSpecV1[] {
  const bases = [
    "cobblestone",
    "stone",
    "granite",
  ...
  const specs: HarthmereSpecializedBlockSpecV1[] = [];
...
const WOOD_SPECS_V1: HarthmereSpecializedBlockSpecV1[] = [
  ...([
    ["oak", 4],
    ["birch", 5],
    ["rubber", 6],
    ...
    ([wood, price]): HarthmereSpecializedBlockSpecV1 => ({
      itemId: `${wood}_lumber`,
...
const REINFORCED_WOOD_SPECS_V1: HarthmereSpecializedBlockSpecV1[] = [
  ["oak", 14],
  ["birch", 15],
  ["rubber", 16],
].map(
...
const CLAY_AND_GLASS_SPECS_V1: HarthmereSpecializedBlockSpecV1[] = [
  {
    itemId: "clay_brick",
...
const FABRIC_SPECS_V1: HarthmereSpecializedBlockSpecV1[] = [
  {
    itemId: "thatch",
    displayName: "Thatch",
    station: "loom",
    ...
    // HARTHMERE_FARM_CRAFT_BRIDGE_V1: inputs reference the actual FARMED crop ids
...
const METAL_SPECS_V1: HarthmereSpecializedBlockSpecV1[] = [
  {
    itemId: "copper",
...
const MAGIC_SPECS_V1: HarthmereSpecializedBlockSpecV1[] = [
  {
    itemId: "ice",
...
export const HARTHMERE_SPECIALIZED_BLOCK_SPECS_V1: HarthmereSpecializedBlockSpecV1[] =
  [
    ...buildStoneSpecs(),
    ...WOOD_SPECS_V1,
    ...REINFORCED_WOOD_SPECS_V1,
    ...CLAY_AND_GLASS_SPECS_V1,
    ...FABRIC_SPECS_V1,
    ...METAL_SPECS_V1,
    ...MAGIC_SPECS_V1,
  ];
...
const INPUT_MATERIAL_SEEDS_V1: InputMaterialSeedV1[] = [
  ...
  // REFINED_MATERIAL_RECIPES_V1) from gathered raw materials, AND buyable
  // (PURCHASABLE_INPUT_MATERIALS_V1). Craft is cheaper; buying is faster.
...
// HARTHMERE_REFINED_MATERIAL_RECIPES_V1: smelt/refine recipes so every refined
...
interface RefinedMaterialRecipeV1 {
  itemId: string;
  station: HarthmereSpecializedBlockStationKeyV1;
  inputs: Array<{ itemId: string; count: number }>;
  fuelInputs?: Array<{ itemId: string; count: number }>;
  output: number;
}
const REFINED_MATERIAL_RECIPES_V1: RefinedMaterialRecipeV1[] = [
...
export function refinedMaterialRecipeIdV1(itemId: string): string {
  return `harthmere_refine_${itemId}`;
}

// HARTHMERE_BLOCK_INPUT_OBTAINABLE_V1: processed inputs that aren't gathered or
...
const PURCHASABLE_INPUT_MATERIALS_V1: Array<{
  itemId: string;
  vendorId: string;
  buyPrice: number;
}> = [
...
export function specializedBlockRecipeIdV1(itemId: string): string {
  return `harthmere_block_${itemId}`;
}

function blockItemDefinitionV1(
  spec: HarthmereSpecializedBlockSpecV1
): HarthmereItemDefinitionV1 {
  return {
    itemId: spec.itemId,
    displayName: spec.displayName,
    description: `Specialized ${spec.materialFamily} building block. Craft at the ${
      resolveBlockStationV1(spec.station).displayName
    ...
    tradeable: true,
    category: "block",
    materialTier: BLOCK_CLASS_TIER_V1[spec.blockClass],
    objectMetadata: {
      objectKind: "material",
      physicalForm: "block",
...
function blockCraftingRecipeV1(
  spec: HarthmereSpecializedBlockSpecV1
): HarthmereCraftingRecipeV1 {
  const station = resolveBlockStationV1(spec.station);
  const tier = BLOCK_CLASS_TIER_V1[spec.blockClass];
  return {
    recipeId: specializedBlockRecipeIdV1(spec.itemId),
...
function refinedMaterialCraftingRecipeV1(
  mat: RefinedMaterialRecipeV1
): HarthmereCraftingRecipeV1 {
  const station = resolveBlockStationV1(mat.station);
  return {
    recipeId: refinedMaterialRecipeIdV1(mat.itemId),
...
function vendorEntryForBlockV1(
  spec: HarthmereSpecializedBlockSpecV1
): { vendorId: string; entry: HarthmereVendorEntryV1 } {
  const binding = VENDOR_TYPE_BINDING_V1[spec.vendorType];
  return {
...
function stationDefinitionV1(
  seed: SpecializedStationSeedV1
 ...
 */
export function assertHarthmereSpecializedBlockRulesV1(): void {
  for (const spec of HARTHMERE_SPECIALIZED_BLOCK_SPECS_V1) {
    if (isHarthmereNaturalBlockV1(spec.itemId)) {
      throw new Error(
...
export function ensureHarthmereSpecializedBlocksCatalogueV1(): void {
  ...
  for (const seed of SPECIALIZED_STATION_SEEDS_V1) {
    if (!getHarthmereCraftingStationV1(seed.stationId)) {
      registerHarthmereCraftingStationV1(stationDefinitionV1(seed));
    }
    if (!getHarthmereItemDefinitionV1(seed.stationId)) {
      registerHarthmereItemDefinitionV1(stationItemDefinitionV1(seed));
    }
    const buildRecipe = stationBuildRecipeV1(seed);
    if (!getHarthmereCraftingRecipeV1(buildRecipe.recipeId)) {
      registerHarthmereCraftingRecipeV1(buildRecipe);
    }
  }

  // 2. Input / intermediate materials (only if absent).
  for (const seed of INPUT_MATERIAL_SEEDS_V1) {
  ...
  for (const mat of REFINED_MATERIAL_RECIPES_V1) {
    const recipe = refinedMaterialCraftingRecipeV1(mat);
  ...
  for (const spec of HARTHMERE_SPECIALIZED_BLOCK_SPECS_V1) {
    if (!getHarthmereItemDefinitionV1(spec.itemId)) {
      registerHarthmereItemDefinitionV1(blockItemDefinitionV1(spec));
    }
    const recipe = blockCraftingRecipeV1(spec);
    if (!getHarthmereCraftingRecipeV1(recipe.recipeId)) {
      registerHarthmereCraftingRecipeV1(recipe);
    }
  }
  ...
  for (const spec of HARTHMERE_SPECIALIZED_BLOCK_SPECS_V1) {
    const { vendorId, entry } = vendorEntryForBlockV1(spec);
    if (!getHarthmereVendorEntryV1(vendorId, spec.itemId)) {
      registerHarthmereVendorEntryV1(entry);
    }
  }
  ...
  for (const mat of PURCHASABLE_INPUT_MATERIALS_V1) {
    if (!getHarthmereVendorEntryV1(mat.vendorId, mat.itemId)) {
      registerHarthmereVendorEntryV1({
...
export function harthmereSpecializedBlockRecipeIdsV1(): string[] {
  ensureHarthmereSpecializedBlocksCatalogueV1();
  return [
    ...SPECIALIZED_STATION_SEEDS_V1.map((s) => `harthmere_station_${s.key}`),
    ...HARTHMERE_SPECIALIZED_BLOCK_SPECS_V1.map((spec) =>
...
export function harthmereSpecializedBlockItemIdsV1(): string[] {
  return HARTHMERE_SPECIALIZED_BLOCK_SPECS_V1.map((spec) => spec.itemId);
}
