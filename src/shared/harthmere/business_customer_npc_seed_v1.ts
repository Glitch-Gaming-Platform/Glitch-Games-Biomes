...
// HARTHMERE_BUSINESS_CUSTOMER_NPC_SEED_V1
//
...
//
// Mirrors business_owner_npc_seed_v1.ts. Offset band 9701+ (owners 9601-9619;
// muckers 9451-9550; robots 9401+; Grove NPCs 9301+).

export const HARTHMERE_BUSINESS_CUSTOMER_NPC_SEED_VERSION_V1 =
  "harthmere-business-customer-npc-seed-v1" as const;

export const HARTHMERE_BUSINESS_CUSTOMER_NPC_ID_OFFSET_BASE_V1 = 9701;

export interface HarthmereBusinessCustomerNpcSeedV1 {
...
// in at the edges.
const HARTHMERE_BUSINESS_CUSTOMER_COPY_V1: Readonly<
  Record<string, readonly CustomerCopyV1[]>
> = {
...
function customerPositionForSiteV1(
  site: {
    groundY: number;
...
function slugifyV1(name: string): string {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "");
...
export const HARTHMERE_BUSINESS_CUSTOMER_NPC_SEEDS_V1: readonly HarthmereBusinessCustomerNpcSeedV1[] =
  (() => {
    const seeds: HarthmereBusinessCustomerNpcSeedV1[] = [];
    let offset = HARTHMERE_BUSINESS_CUSTOMER_NPC_ID_OFFSET_BASE_V1;
    for (const outpost of HARTHMERE_BUSINESS_OUTPOSTS_V1) {
      ...
      const copies = HARTHMERE_BUSINESS_CUSTOMER_COPY_V1[outpost.outpostId];
      if (!site || !copies) {
        continue;
      }
      copies.forEach((copy, index) => {
        ...
        seeds.push({
          customerNpcId: `customer_${outpost.outpostId}_${slugifyV1(copy.name)}`,
          ...
          position: customerPositionForSiteV1(site, index, copies.length),
...
export function harthmereBusinessCustomerNpcSeedIdsV1(): BiomesId[] {
  return HARTHMERE_BUSINESS_CUSTOMER_NPC_SEEDS_V1.map((seed) => seed.entityId);
}

const HARTHMERE_BUSINESS_CUSTOMER_ENTITY_ID_SET_V1 = new Set<number>(
  HARTHMERE_BUSINESS_CUSTOMER_NPC_SEEDS_V1.map((seed) => Number(seed.entityId))
);
...
// roof). See harthmere_entity_grounding_manifest_v1.
export function isHarthmereBusinessCustomerNpcEntityIdV1(
  id: BiomesId | number | undefined
): boolean {
  return (
    id !== undefined &&
    HARTHMERE_BUSINESS_CUSTOMER_ENTITY_ID_SET_V1.has(Number(id))
  );
}

export function validateHarthmereBusinessCustomerNpcSeedsV1(): string[] {
  const errors: string[] = [];
  ...
  for (const seed of HARTHMERE_BUSINESS_CUSTOMER_NPC_SEEDS_V1) {
    ...
    if (seed.idOffset < HARTHMERE_BUSINESS_CUSTOMER_NPC_ID_OFFSET_BASE_V1) {
