export const HARTHMERE_NPC_LORE_DIALOGUE_ENRICHMENT_VERSION_V1 =
  "harthmere-npc-lore-dialogue-enrichment-v1" as const;

export interface HarthmereNpcLoreLikeV1 {
  id: string;
  name?: string;
  displayName?: string;
  role?: string;
...
export function harthmereNpcLoreTextIsPlaceholderV1(text: unknown): boolean {
...
function npcName(npc: HarthmereNpcLoreLikeV1) {
  return npc.name ?? npc.displayName ?? npc.id;
}

function shortRole(npc: HarthmereNpcLoreLikeV1) {
  return String(npc.role ?? npc.category ?? npc.kind ?? "resident").replaceAll("_", " ");
}

export function buildHarthmereBackstoryV1(npc: HarthmereNpcLoreLikeV1): string {
...
export function buildBiomesEconomyBackstoryV1(npc: HarthmereNpcLoreLikeV1): string {
  const name = npcName(npc);
  const role = shortRole(npc);
...
export function buildNaturalNpcDialogueV1(
  npc: HarthmereNpcLoreLikeV1,
...
export function enrichNpcLoreDialogueV1<T extends HarthmereNpcLoreLikeV1>(
  npc: T,
  loreKind: "harthmere" | "biomes_economy",
): T {
  const backstory =
    loreKind === "harthmere"
      ? buildHarthmereBackstoryV1(npc)
      : buildBiomesEconomyBackstoryV1(npc);
  const dialogue = buildNaturalNpcDialogueV1(npc, loreKind);
  const next: any = { ...npc };
  if ("bibleBackstory" in next && harthmereNpcLoreTextIsPlaceholderV1(next.bibleBackstory)) {
    next.bibleBackstory = backstory;
  }
  if ("background" in next && harthmereNpcLoreTextIsPlaceholderV1(next.background)) {
    next.background = backstory;
  }
  if (!next.dialogue || Object.values(next.dialogue).some(harthmereNpcLoreTextIsPlaceholderV1)) {
    next.dialogue = { ...dialogue, ...(next.dialogue ?? {}) };
    for (const [key, value] of Object.entries(next.dialogue)) {
      if (harthmereNpcLoreTextIsPlaceholderV1(value)) {
        next.dialogue[key] = dialogue[key] ?? dialogue.greeting;
      }
    }
  }
  if (!next.line || harthmereNpcLoreTextIsPlaceholderV1(next.line)) {
