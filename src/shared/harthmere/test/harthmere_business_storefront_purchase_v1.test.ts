...
function run(
  ...
  return reduceHarthmereEconomyMutationV1(
    state,
    { requestId: `sf-${operation}-${seq}`, actorId: ACTOR, nowMs: NOW_MS, operation, ...payload },
    context
  );
}
...
function openBusiness(type = "food_service_restaurant") {
  ...
  let r = run(state, "register_business", { businessType: type as any, name: `${type} Shop` });
  assert.deepStrictEqual(r.warnings, []);
  const businessId = Object.keys(r.economy.businesses).find((id) => !before.has(id))!;
  assert.ok(businessId);
  r = run(r.economy, "issue_license", { businessId, licenseLevel: 1 });
  assert.deepStrictEqual(r.warnings, []);
  r = run(r.economy, "open_business", { businessId, propertyId: `property_${businessId}`, townId: "harthmere_grove" });
  assert.deepStrictEqual(r.warnings, []);
...
describe("Harthmere business storefront purchase (buy_storefront_good)", () => {
  ...
  it("sells a themed block: buyer gets the item + pays gold, no inventory needed", () => {
    ...
    const block = harthmereBusinessStorefrontGoodsForTypeV1("food_service_restaurant")!.blocks[0];
    const r = run(state, "buy_storefront_good", { businessId, itemId: block, count: 2 });
  ...
  it("sells a themed interior item too", () => {
    ...
    const interior = harthmereBusinessStorefrontGoodsForTypeV1("hospitality_inn_hotel_shelter")!.interior[0];
    const r = run(state, "buy_storefront_good", { businessId, itemId: interior, count: 1 });
    assert.deepStrictEqual(
      r.warnings.filter((w: string) => w.startsWith("economy_rejected")),
      []
    );
  ...
  it("rejects an item the business does not carry", () => {
    ...
    const r = run(state, "buy_storefront_good", { businessId, itemId: "neptunium", count: 1 });
    assert.ok(
  ...
  it("has unlimited / self-replenishing supply (a huge buy still succeeds)", () => {
    ...
    const block = harthmereBusinessStorefrontGoodsForTypeV1("general_trader")!.blocks[0];
    const r = run(state, "buy_storefront_good", { businessId, itemId: block, count: 999 }, ctx({ actorGold: 1_000_000 }));
    assert.deepStrictEqual(
      r.warnings.filter((w: string) => w.startsWith("economy_rejected")),
      []
    );
  ...
  it("rejects when the buyer cannot afford it", () => {
    const { state, businessId } = openBusiness("food_service_restaurant");
    const block = harthmereBusinessStorefrontGoodsForTypeV1("food_service_restaurant")!.blocks[0];
    const r = run(state, "buy_storefront_good", { businessId, itemId: block, count: 5 }, ctx({ actorGold: 0 }));
    assert.ok(
