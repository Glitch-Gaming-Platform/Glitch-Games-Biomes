...
import {
  HARTHMERE_BUSINESS_NPC_GROVE_COSMETIC_VERSION_V1,
  ...
  harthmereBusinessOutpostStaffRoleV1,
} from "../business_npc_cosmetics_v1";

const REQUIRED_CLOTHING_SLOTS = ["head", "torso", "legs", "feet", "belt"] as const;
const EXPECTED_OUTPOST_STAFF_PRESENTATION = {
  outpost_refinery_ashline: ["townsperson_dockhand", "farmer", "blacksmith_apron"],
  outpost_biome_repair_north: ["townsperson_dockhand", "farmer", "blacksmith_apron"],
  outpost_design_glassyard: ["townsperson_market", "merchant", "merchant_coat"],
  outpost_security_redoubt: ["townsperson_guard", "guard", "guard_tabard_armor"],
  ...
  outpost_tools_cinderlane: ["townsperson_dockhand", "farmer", "blacksmith_apron"],
  outpost_magic_moonstall: ["townsperson_clergy", "clergy", "scholar_robe"],
  outpost_exploration_westtrail: ["townsperson_hunter", "hunter", "hunter_jerkin"],
  outpost_property_keylot: ["townsperson_market", "merchant", "merchant_coat"],
  outpost_trader_brightcart: ["townsperson_market", "merchant", "merchant_coat"],
  outpost_hunter_ridgecooler: ["townsperson_hunter", "hunter", "hunter_jerkin"],
  outpost_clinic_greenlamp: ["townsperson_clergy", "clergy", "field_medic_coat"],
  outpost_teleport_returnstone: ["townsperson_courier", "merchant", "river_tunic"],
  outpost_sanitation_clearbarrel: ["townsperson_dockhand", "farmer", "blacksmith_apron"],
  outpost_repair_hingehall: ["townsperson_dockhand", "farmer", "blacksmith_apron"],
  ...
  outpost_hospitality_lanternrest: ["townsperson_market", "merchant", "merchant_coat"],
} as const;

function assertFullGroveCosmeticContract(
  label: string,
  appearance: ReturnType<typeof harthmereBusinessOutpostStaffAppearanceV1>,
  sourceKind: "staff" | "customer",
) {
  assert.equal(appearance.species, "human", `${label} should be a humanoid townsperson`);
  assert.ok(
    appearance.source?.includes(`${HARTHMERE_BUSINESS_NPC_GROVE_COSMETIC_VERSION_V1}:${sourceKind}`),
  ...
  for (const field of [
    ...
    assert.ok(String(field).length > 0, `${label} has an incomplete face/body cosmetic field`);
  }

  for (const slot of REQUIRED_CLOTHING_SLOTS) {
    const item = appearance.clothing[slot];
    assert.ok(item, `${label} should have Bikkie-style clothing in slot ${slot}`);
    assert.equal(item?.renderMode, "threejs", `${label} ${slot} should use the procedural clothing renderer`);
  }
}

describe("business_npc_cosmetics_v1", () => {
  it("creates business outpost staff as Grove-style procedural townspeople with full cosmetics", () => {
    ...
    for (const outpost of HARTHMERE_BUSINESS_OUTPOSTS_V1) {
      const asset = harthmereBusinessOutpostStaffAssetV1(outpost);
      assert.ok(
      ...
      const appearance = harthmereBusinessOutpostStaffAppearanceV1(outpost);
      const expected = EXPECTED_OUTPOST_STAFF_PRESENTATION[
        outpost.outpostId as keyof typeof EXPECTED_OUTPOST_STAFF_PRESENTATION
      ];
      assert.ok(expected, `${outpost.outpostId} must have an exact staff presentation contract`);
      assert.equal(asset, expected[0], `${outpost.outpostId} uses the wrong staff renderer family`);
      assert.equal(harthmereBusinessOutpostStaffRoleV1(outpost), expected[1], `${outpost.outpostId} uses the wrong staff role`);
      assert.equal(appearance.clothing.torso?.id, expected[2], `${outpost.outpostId} uses the wrong business outfit`);
      ...
      signatures.add(
        JSON.stringify({
          ...
          clothing: Object.fromEntries(
            Object.entries(appearance.clothing).map(([slot, item]) => [slot, item?.id]),
          ),
        }),
      );
    }
  ...
  it("can promote every customer-only NPC design into the same Grove cosmetic schema", () => {
    ...
    for (const npc of HARTHMERE_BUSINESS_CUSTOMER_NPCS_V1) {
      ...
      const appearance = harthmereBusinessCustomerCharacterAppearanceV1(npc);
    ...
    assert.equal(
      signatures.size,
      HARTHMERE_BUSINESS_CUSTOMER_NPCS_V1.length,
