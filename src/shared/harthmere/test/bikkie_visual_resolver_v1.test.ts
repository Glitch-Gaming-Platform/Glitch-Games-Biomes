...
import {
  harthmereBikkieColorHexV1,
  harthmereInitialsGlyphV1,
  harthmereResolveBikkieVisualV1,
} from "../bikkie_visual_resolver_v1";

describe("bikkie_visual_resolver_v1", () => {
  it("normalizes named Bikkie colors into renderable hex values", () => {
    assert.equal(harthmereBikkieColorHexV1("electric blue"), "#3f91c8");
    assert.equal(harthmereBikkieColorHexV1("oak brown"), "#7b5438");
    assert.match(harthmereBikkieColorHexV1("mystery aura"), /^#[0-9a-f]{6}$/);
  });
  ...
  it("derives stable short glyphs from player-facing labels", () => {
    assert.equal(harthmereInitialsGlyphV1("Dye-O-Matic"), "DO");
    assert.equal(harthmereInitialsGlyphV1("Power Cell"), "PC");
  });
});
