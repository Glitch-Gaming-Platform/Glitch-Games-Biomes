...
import {
  ...
  applyHarthmereCareLoopInventoryDeltas,
...
function mutate(
  ...
  return reduceHarthmereCareLoop(state, {
...
describe("mmo_care_loops", () => {
  it("supports daily routine check-ins without punishing missed days", () => {
    ...
    result = reduceHarthmereCareLoop(result.care, {
  ...
  it("preserves the daily streak through a backwards-clock blip instead of resetting it", () => {
    ...
    const care0 = defaultHarthmereCareLoopState(ACTOR, NOW);
    const day1 = reduceHarthmereCareLoop(care0, {
      requestId: "bc_d1", actorId: ACTOR, operation: "daily_check_in", targetId: "check_in", nowMs: NOW,
    });
    assert.equal(day1.care.daily.streak, 1);
    const day2 = reduceHarthmereCareLoop(day1.care, {
    ...
    const back1 = reduceHarthmereCareLoop(day2.care, {
      requestId: "bc_done", actorId: ACTOR, operation: "daily_task_completed", targetId: "garden", nowMs: NOW,
    });
    const back2 = reduceHarthmereCareLoop(back1.care, {
  ...
  it("pays an escalating daily check-in streak bonus that saturates at a one-week streak", () => {
    const DAY = HARTHMERE_CARE_LOOP_DAY_MS;
    const checkIn = (care: HarthmereCareLoopState, dayIndex: number) =>
      reduceHarthmereCareLoop(care, {
    ...
    let r = checkIn(defaultHarthmereCareLoopState(ACTOR, NOW), 0);
    assert.equal(r.care.daily.streak, 1);
    assert.equal(r.goldDelta, HARTHMERE_DAILY_TASK_MIN_GOLD);

    // Day 2 (streak 2): +1 step of streak bonus.
    r = checkIn(r.care, 1);
    assert.equal(r.care.daily.streak, 2);
    assert.equal(r.goldDelta, HARTHMERE_DAILY_TASK_MIN_GOLD + HARTHMERE_DAILY_STREAK_BONUS_GOLD);
    ...
    const maxBonus =
      (HARTHMERE_DAILY_STREAK_BONUS_CAP_DAYS - 1) * HARTHMERE_DAILY_STREAK_BONUS_GOLD;
    assert.ok(r.care.daily.streak >= HARTHMERE_DAILY_STREAK_BONUS_CAP_DAYS);
    assert.equal(
      r.goldDelta,
      HARTHMERE_DAILY_TASK_MIN_GOLD + maxBonus,
  ...
  it("supports daily check-in rewards and separate cozy tasks on the same day", () => {
    ...
    assert.equal(result.goldDelta, HARTHMERE_DAILY_TASK_MIN_GOLD);
    assert.equal(
      result.xpDelta,
  ...
  it("scales visible daily task XP so all tasks total half a level", () => {
    ...
    assert.equal(
      totalXp,
      harthmereDailyTaskXpReward({ actorLevel: 1 }) * tasks.length
    );
  ...
  it("rejects invalid NPC gift inputs without consuming inventory", () => {
    const state = defaultHarthmereCareLoopState(ACTOR, NOW);
  ...
  it("applies care loop inventory deltas without negative item counts", () => {
    const next = applyHarthmereCareLoopInventoryDeltas(
