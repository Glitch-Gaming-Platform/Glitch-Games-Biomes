...
describe("business_service_procedural_animations", () => {
  ...
  it("covers the full customer by cue matrix for base cases and edge-safe finite output", function () {
    ...
    assert.equal(combinations, HARTHMERE_BUSINESS_CUSTOMER_NPCS.length * cueIds.length);
  });
  ...
  it("renders a production visual audit page covering every customer design and every service cue", () => {
    ...
    assert.ok(html.includes(`data-customer-count="${HARTHMERE_BUSINESS_CUSTOMER_NPCS.length}"`));
    assert.ok(html.includes(`data-cue-count="${Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS).length}"`));
  ...
  it("passes the full procedural visual audit validator", () => {
    ...
    assert.equal(audit.cueCount, Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS).length);
  ...
  it("renders a live runtime audit page with moving coverage for every cue and customer design", () => {
    ...
    assert.equal(audit.ok, true, audit.warnings.slice(0, 10).join(", "));
    assert.equal(audit.customerCoverageCount, HARTHMERE_BUSINESS_CUSTOMER_NPCS.length);
    assert.equal(audit.cueCoverageCount, Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS).length);
