...
function craft(
  req: Partial<HarthmereInventoryMutationRequestV1>,
  snap: HarthmereInventorySnapshotV1,
...
describe("Harthmere crafting authority", () => {
  ...
  it("supports repair, salvage, upgrade, and enchant workflows as server-validated crafting", () => {
    ...
    for (const recipe of recipes) registerHarthmereCraftingRecipeV1(recipe);
  ...
  it("registers the Bikkie-backed production stations and lore recipes", () => {
    ...
    const stationIds = new Set(
      listHarthmereCraftingStationsV1().map((station) => station.stationId)
    );
    assert.ok(stationIds.has(HARTHMERE_CRAFTING_STATIONS_V1.workbench));
    assert.ok(stationIds.has(HARTHMERE_CRAFTING_STATIONS_V1.thermolite));
    assert.ok(getHarthmereCraftingRecipeV1("harthmere_bellbinders_voice"));
    assert.ok(getHarthmereCraftingRecipeV1("harthmere_blacksmith_iron_sword"));
  });

  it("registers Exotic Matter materials as crafting blocks with rich object metadata", () => {
    ensureHarthmereProductionCraftingCatalogueV1();
    for (const itemId of HARTHMERE_EXOTIC_MATTER_BLOCK_ITEM_IDS_V1) {
    ...
    const raw = getHarthmereItemDefinitionV1(
      HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.rawExoticMatter
    );
    const stable = getHarthmereItemDefinitionV1(
    ...
    const portalFuel = getHarthmereItemDefinitionV1(
      HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.portalFuel
    );
    const driveCore = getHarthmereItemDefinitionV1(
  ...
  it("registers and crafts home and business crafting stations and decoration items", () => {
    ensureHarthmereProductionCraftingCatalogueV1();
    const workbenchDef = getHarthmereItemDefinitionV1(
    ...
    const cabinetDef = getHarthmereItemDefinitionV1(
    ...
    const workbench = craft(
      ...
      snapshot({
        ...
        knownRecipes: [HARTHMERE_CRAFTING_STATION_RECIPE_IDS_V1.workbench],
      }),
      { carpentry: { level: 5 } }
    );
    assert.ok(workbench.ok, workbench.errors.join(", "));
    assert.strictEqual(
      workbench.itemDeltas[HARTHMERE_CRAFTING_STATIONS_V1.workbench],
      1
    );

    const thermoblasterRecipe = getHarthmereCraftingRecipeV1(
      HARTHMERE_CRAFTING_STATION_RECIPE_IDS_V1.thermoblaster
    );
    assert.strictEqual(
    ...
    const thermoblaster = craft(
      {
        ...
        toolItemIds: [HARTHMERE_CRAFTING_TOOLS_V1.slabber],
      },
      snapshot({
        items: { [HARTHMERE_CRAFTING_TOOLS_V1.slabber]: 1 },
        materialStorage: {
          iron_ingot: 6,
          crystal_shard: 2,
          arcane_dust: 1,
        },
        knownRecipes: [HARTHMERE_CRAFTING_STATION_RECIPE_IDS_V1.thermoblaster],
      }),
      { exotic_refining: { level: 5 } }
    );
    assert.ok(thermoblaster.ok, thermoblaster.errors.join(", "));
    assert.strictEqual(
      thermoblaster.itemDeltas[HARTHMERE_CRAFTING_STATIONS_V1.thermoblaster],
      1
    );
    ...
    const cabinet = craft(
      ...
      snapshot({
        items: { [HARTHMERE_CRAFTING_TOOLS_V1.slabber]: 1 },
        materialStorage: { wood_plank: 4, iron_ingot: 1 },
        knownRecipes: [HARTHMERE_HOME_DECORATION_RECIPE_IDS_V1.storageCabinet],
      }),
      { carpentry: { level: 5 } }
    );
    assert.ok(cabinet.ok, cabinet.errors.join(", "));
    assert.strictEqual(
      cabinet.itemDeltas[HARTHMERE_HOME_DECORATION_ITEM_IDS_V1.storageCabinet],
      1
    );
    assert.strictEqual(
  ...
  it("crafts antimatter blocks into raw and stabilized Exotic Matter", () => {
    ...
    const raw = craft(
      ...
      snapshot({
        items: { [HARTHMERE_CRAFTING_TOOLS_V1.bucket]: 1 },
        materialStorage: {
          [HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.antihydrogenBlock]: 1,
          [HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.antiheliumBlock]: 1,
          [HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.antiboronBlock]: 1,
          [HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.containmentFilter]: 2,
        },
        knownRecipes: [HARTHMERE_EXOTIC_MATTER_RECIPE_IDS_V1.rawExoticMatter],
      }),
      exoticSkills
    );
    assert.ok(raw.ok, raw.errors.join(", "));
    assert.strictEqual(
      raw.itemDeltas[HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.rawExoticMatter],
      1
    );
    assert.strictEqual(
    ...
    const stabilized = craft(
      ...
      snapshot({
        items: { [HARTHMERE_CRAFTING_TOOLS_V1.bucket]: 1 },
        materialStorage: {
          [HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.rawExoticMatter]: 2,
          [HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.stabilizingCrystal]: 1,
          [HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.coolant]: 1,
          [HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.containmentFilter]: 1,
        },
        knownRecipes: [
  ...
  it("registers downstream Exotic Matter fuels and cores as station-gated recipes", () => {
    ...
    const powerCell = craft(
      ...
      snapshot({
        items: { [HARTHMERE_CRAFTING_TOOLS_V1.slabber]: 1 },
        materialStorage: {
          [HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1.stabilizedExoticMatter]: 1,
          iron_ingot: 1,
          crystal_shard: 1,
        },
        knownRecipes: [HARTHMERE_EXOTIC_MATTER_RECIPE_IDS_V1.powerCell],
      }),
    ...
    assert.strictEqual(
      powerCell.craftingOutcome?.toolDurabilityCosts[
        HARTHMERE_CRAFTING_TOOLS_V1.slabber
      ],
      1
    );
  });
  ...
  it("requires the Bellbound Q5 forge sequence and unlocks Bell-Bronze Ingot", () => {
    ...
    const base = snapshot({
      items: {
        bell_bronze_ingot: 1,
        meteoric_trace: 1,
        [HARTHMERE_CRAFTING_TOOLS_V1.slabber]: 1,
      },
