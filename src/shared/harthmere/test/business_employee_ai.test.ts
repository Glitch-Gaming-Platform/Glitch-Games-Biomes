...
function employee(overrides: Partial<HarthmereEconomyEmployeeRecord> = {}): HarthmereEconomyEmployeeRecord {
  return {
...
describe("business_employee_ai", () => {
  it("creates per-business staff task flows for every service offer", () => {
    for (const [typeId, definition] of Object.entries(HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS)) {
      for (const offer of definition.offers) {
        const flow = createHarthmereBusinessEmployeeTaskFlow(typeId as any, offer.offerId);
  ...
  it("uses bespoke physical sub-actions for restaurant service instead of a generic task label", () => {
    const flow = createHarthmereBusinessEmployeeTaskFlow("food_service_restaurant", "serve_worker_meal");
  ...
  it("validates assignable staff tasks instead of accepting arbitrary strings", () => {
    assert.equal(normalizeHarthmereBusinessEmployeeTaskId("Front Counter"), "front_counter");
    assert.equal(normalizeHarthmereBusinessEmployeeTaskId("kitchen"), "production_station");
    assert.equal(validateHarthmereBusinessEmployeeAssignedTask("cleanup route")?.taskKind, "cleanup_route");
    assert.equal(validateHarthmereBusinessEmployeeAssignedTask("debugNope"), undefined);
    assert.equal(Object.keys(HARTHMERE_BUSINESS_EMPLOYEE_ASSIGNABLE_TASKS).length, 8);
  });
  ...
  it("models candidate market, interview scores, wage negotiation, and withdrawal edge cases", () => {
    ...
    const interviewed = interviewHarthmereBusinessEmployeeCandidate(candidates[0], "friendly");
    assert.equal(interviewed.status, "interviewed");
    assert.ok((interviewed.interviewScore ?? 0) > 0);
    const accepted = negotiateHarthmereBusinessEmployeeCandidate(interviewed, interviewed.wageAskGoldPerDay);
    assert.equal(accepted.accepted, true);
    assert.equal(accepted.candidate.status, "offer_made");
    let declined = negotiateHarthmereBusinessEmployeeCandidate(interviewed, 1).candidate;
    declined = negotiateHarthmereBusinessEmployeeCandidate(declined, 1).candidate;
  ...
  it("has visible AI behavior definitions for every automation role", () => {
    const roles = Object.keys(HARTHMERE_BUSINESS_EMPLOYEE_AUTOMATION_ROLE_BEHAVIORS) as HarthmereBusinessEmployeeAutomationRole[];
    ...
    for (const role of roles) {
      ...
      assert.equal(run.taskKind, HARTHMERE_BUSINESS_EMPLOYEE_AUTOMATION_ROLE_BEHAVIORS[role].taskKind);
      assert.notEqual(run.status, "failed");
      assert.ok(run.employeePath.length > 0);
    }
  });
  ...
  it("renders and validates the staff movement visual audit", () => {
    ...
    assert.equal(audit.businessCount, Object.keys(HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS).length);
