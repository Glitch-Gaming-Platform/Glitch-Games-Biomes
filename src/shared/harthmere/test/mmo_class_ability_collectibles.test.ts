...
function economyWithBusiness(typeId: HarthmereEconomyBusinessTypeId) {
...
describe("mmo_class_ability_collectibles", () => {
  it("defines every playable class without placeholder fallbacks", () => {
    assert.equal(Object.keys(HARTHMERE_CLASS_DEFINITIONS).length, 9);
    for (const cls of Object.values(HARTHMERE_CLASS_DEFINITIONS)) {
  ...
  it("creates exactly ten relevant business abilities for each economy business", () => {
    for (const typeId of Object.keys(HARTHMERE_ECONOMY_BUSINESS_TYPES)) {
      const abilities = Object.values(HARTHMERE_BUSINESS_ABILITY_DEFINITIONS)
      ...
      for (const ability of abilities) {
        ...
        assert.ok(ability.name.startsWith(HARTHMERE_ECONOMY_BUSINESS_TYPES[typeId as HarthmereEconomyBusinessTypeId].displayName));
      }
    }
  });

  it("gates business abilities by owned business type and skill", () => {
    const courierAbility = Object.values(HARTHMERE_BUSINESS_ABILITY_DEFINITIONS)
    ...
    assert.equal(canLearnHarthmereAbility({ classMagic, actorId: ACTOR, abilityId: courierAbility.id }).ok, false);
    assert.equal(canLearnHarthmereAbility({
      classMagic,
      actorId: ACTOR,
  ...
  it("seeds a first class choice with starting abilities and skills", () => {
    ...
    const result = applyHarthmereClassChoice(classMagic, "mage");
  ...
  it("accepts only specializations available to the current class", () => {
    ...
    const valid = applyHarthmereSpecializationChoice(classMagic, "protection");
    assert.equal(valid.ok, true);
    assert.equal(classMagic.specializationId, "protection");

    const invalid = applyHarthmereSpecializationChoice(classMagic, "pyromancer");
  ...
  it("clears stale specialization on authorized class change", () => {
    ...
    const result = applyHarthmereClassChoice(classMagic, "mage", { allowClassChange: true });
  ...
  it("cleans old class abilities and loadout when an authorized class change is applied", () => {
    ...
    const result = applyHarthmereClassChoice(classMagic, "bard", { allowClassChange: true });
  ...
  it("projects class, skills, abilities, loadout, and collections for the UI", () => {
    const abilityId = Object.keys(HARTHMERE_ABILITY_DEFINITIONS)[0];
    const snapshot = createHarthmereProgressionClientSnapshot({
      actorId: ACTOR,
      classMagic: {
        classId: "warrior",
    ...
    assert.ok(Object.keys(HARTHMERE_COLLECTIBLE_DEFINITIONS).length > SNAPSHOT_GROVE_NPCS.length);
  });
  ...
  it("projects skill XP as current-level progress instead of lifetime XP", () => {
    ...
    assert.equal(combat.nextLevel, HARTHMERE_SKILL_XP_PER_LEVEL);
  });
  ...
  it("keeps capped skills at a full but bounded progress bar", () => {
    ...
    assert.equal(characterLevel.xp, HARTHMERE_SKILL_XP_PER_LEVEL);
    assert.equal(characterLevel.nextLevel, HARTHMERE_SKILL_XP_PER_LEVEL);
  });

  it("keeps every Grove NPC backed by a real backstory", () => {
    const missing = SNAPSHOT_GROVE_NPCS
  ...
  it("lets every class learn all of its own (non-business) starting abilities from its starting skills", () => {
    for (const [classId, classDef] of Object.entries(HARTHMERE_CLASS_DEFINITIONS)) {
      ...
      const classMagic = { classId, skills } as Parameters<typeof canLearnHarthmereAbility>[0]["classMagic"];
      for (const abilityId of classDef.startingAbilities) {
        ...
        const res = canLearnHarthmereAbility({ classMagic, actorId: ACTOR, abilityId });
