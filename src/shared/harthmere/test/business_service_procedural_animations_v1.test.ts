...
import {
  buildHarthmereBusinessCustomerProceduralRigV1,
...
describe("business_service_procedural_animations_v1", () => {
  it("builds a unique voxel-style rig and color palette for every customer-only NPC design", () => {
    ...
    for (const npc of HARTHMERE_BUSINESS_CUSTOMER_NPCS_V1) {
      ...
      for (const field of [
        ...
        assert.ok(rig.coverageTags.some((tag) => tag.startsWith(`${field}:`)), `${npc.npcId} missing ${field} coverage`);
      }
      signatures.add([
        rig.palette.skin,
        rig.palette.hair,
    ...
    assert.equal(signatures.size, HARTHMERE_BUSINESS_CUSTOMER_NPCS_V1.length);
  });
  ...
  it("generates procedural service clips for every service cue without skeletons, root motion, or unsafe transforms", () => {
    ...
    for (const cueId of cueIds) {
      const clip = createHarthmereBusinessServiceProceduralClipV1({
        cueId,
        customerNpc: HARTHMERE_BUSINESS_CUSTOMER_NPCS_V1[0],
        sampleCount: 5,
      });
      const spec = HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS_V1[cueId];
      ...
      for (const frame of clip.frames) {
        ...
        assert.ok(frame.prop.bikkieId, `${cueId} should carry a Bikkie prop id`);
        assert.ok(frame.prop.graphicId?.includes(":"), `${cueId} should carry a business graphic id`);
        assert.ok(frame.prop.visual, `${cueId} should carry a resolved Bikkie visual`);
        assert.match(frame.prop.visual.primaryHex, HEX);
        assert.notEqual(frame.prop.visual.source, "metadata");
        for (const pose of [...Object.values(frame.owner), ...Object.values(frame.customer)]) {
  ...
  it("covers the full customer by cue matrix for base cases and edge-safe finite output", function () {
    ...
    for (const npc of HARTHMERE_BUSINESS_CUSTOMER_NPCS_V1) {
      for (const cueId of cueIds) {
        ...
        assert.deepEqual(clip.warnings, []);
        for (const frame of clip.frames) {
          const svg = renderHarthmereBusinessServiceFrameSvgV1(clip, frame);
    ...
    assert.equal(combinations, HARTHMERE_BUSINESS_CUSTOMER_NPCS_V1.length * cueIds.length);
  });
  ...
  it("handles missing cues with a visible fallback instead of crashing the service counter", () => {
    ...
    assert.ok(clip.warnings.includes("unknown_cue_fallback:procedural_missing_business_counter_cue"));
    assert.equal(clip.safety.noRootMotion, true);
    assert.equal(clip.frames[0].prop.source, "procedural_fallback");
    assert.match(clip.frames[0].prop.visual.primaryHex, HEX);
    const svg = renderHarthmereBusinessServiceFrameSvgV1(clip, clip.frames[0]);
    assert.ok(svg.includes("procedural_missing_business_counter_cue"));
    assert.ok(svg.includes('data-visual-source="galois_icon"') || svg.includes('data-visual-source="procedural_voxel"'));
    assert.equal(svg.includes("NaN"), false);
  });
  ...
  it("renders Bikkie prop metadata into service-frame SVGs", () => {
    ...
    assert.ok(svg.includes('data-galois-path="placeables/crafting_stations/oak_kitchen"'));
    assert.ok(svg.includes('data-icon-asset-path="icons/placeables/crafting_stations/oak_kitchen"'));
  });
  ...
  it("renders a production visual audit page covering every customer design and every service cue", () => {
    ...
    assert.ok(html.includes("Business Service Procedural Animation Visual Audit"));
    assert.ok(html.includes(`data-customer-count="${HARTHMERE_BUSINESS_CUSTOMER_NPCS_V1.length}"`));
    assert.ok(html.includes(`data-cue-count="${Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS_V1).length}"`));
  ...
  it("renders a live runtime audit page with moving coverage for every cue and customer design", () => {
    ...
    assert.equal(audit.ok, true, audit.warnings.slice(0, 10).join(", "));
    assert.equal(audit.customerCoverageCount, HARTHMERE_BUSINESS_CUSTOMER_NPCS_V1.length);
    assert.equal(audit.cueCoverageCount, Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS_V1).length);
    ...
    assert.ok(html.includes("__harthmereBusinessAnimationRuntimeAuditV1"));
    assert.ok(html.includes(`data-customer-count="${HARTHMERE_BUSINESS_CUSTOMER_NPCS_V1.length}"`));
    assert.ok(html.includes(`data-cue-count="${Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS_V1).length}"`));
    assert.equal(html.includes("NaN"), false);
    assert.equal(html.includes("undefined"), false);
