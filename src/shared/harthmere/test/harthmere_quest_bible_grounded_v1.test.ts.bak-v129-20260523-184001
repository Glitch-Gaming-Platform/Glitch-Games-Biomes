// HARTHMERE_QUEST_BIBLE_GROUNDED_TEST_V1
//
...
//   3. Every quest giverId resolves to a known v44/v45 NPC, or is one of
...
//      registered SNAPSHOT_FINAL_BIKKIE_REWARD_IDS_V77 reward symbol, or
...
// Quest givers that legitimately are not NPCs in v44/v45.
...
function allKnownNpcIds(): Set<string> {
  ...
  for (const npc of HARTHMERE_NAMED_NPCS_V44) set.add(npc.id);
  for (const npc of HARTHMERE_ALL_NPCS_V45) set.add(npc.id);
  return set;
}
...
function allKnownRewardSymbols(): Set<string> {
  ...
  for (const key of Object.keys(SNAPSHOT_FINAL_BIKKIE_REWARD_IDS_V77)) set.add(key);
...
// ---------------------------------------------------------------------------
export interface HarthmereQuestBibleGroundedReportV1 {
  ok: boolean;
  version: typeof HARTHMERE_QUEST_BIBLE_GROUNDED_TEST_VERSION_V1;
  totalQuests: number;
  failures: string[];
...
export function validateHarthmereQuestBibleGroundedV1(): HarthmereQuestBibleGroundedReportV1 {
  ...
  const allQuestIds = new Set(HARTHMERE_QUEST_CATALOG_V46.map((q) => q.id));
  ...
  for (const quest of HARTHMERE_QUEST_CATALOG_V46) {
    ...
    if (!npcIds.has(giverId)) {
      unresolvedGivers.push(`${quest.code}: giver '${giverId}' not in v44/v45`);
    }
  }
  ...
  for (const quest of HARTHMERE_QUEST_CATALOG_V46) {
    ...
    for (const prereqId of prereqIds) {
      const prereq = HARTHMERE_QUEST_CATALOG_V46.find((q) => q.id === prereqId);
  ...
  // build. As each id is fixed, remove it from KNOWN_MISALIGNED_GROVE_V1.
  const KNOWN_MISALIGNED_GROVE_V1: ReadonlySet<string> = new Set<string>([
  ...
  for (const quest of SNAPSHOT_GROVE_QUESTS_V75) {
    if (KNOWN_MISALIGNED_GROVE_V1.has(quest.id)) continue;
  ...
  return {
    ok,
    version: HARTHMERE_QUEST_BIBLE_GROUNDED_TEST_VERSION_V1,
    totalQuests: HARTHMERE_QUEST_CATALOG_V46.length,
    failures,
...
if (typeof (describe as any) === "function" && typeof (test as any) === "function") {
  (describe as any)("harthmere quest bible-grounded contract v1", () => {
    const report = validateHarthmereQuestBibleGroundedV1();

    (test as any)("catalog itself validates (v46 contract)", () => {
      const baseline = validateHarthmereQuestCatalogV46();
