...
import { BUILDING_SYSTEM_TERRAIN_BLOCKS } from "../building_system";
import {
  ...
  HARTHMERE_GROVE_BUSINESS_DESIGN_FURNITURE_SCAN,
  HARTHMERE_GROVE_BUSINESS_INTERIOR_REFERENCE_TEMPLATE_FIXTURES,
  HARTHMERE_GROVE_BUSINESS_PEOPLE_REFERENCE_COORDINATES,
  HARTHMERE_GROVE_BUSINESS_PEOPLE_SOURCE_SCAN,
  ...
  validateHarthmereGroveBusinessCoordinateReferenceRoles,
...
import { HARTHMERE_ECONOMY_BUSINESS_TYPES } from "../mmo_economy_authority";
...
describe("business_customer_simulator", () => {
  it("stores 65 unique customer-only NPCs away from the permanent map", () => {
    assert.equal(HARTHMERE_BUSINESS_CUSTOMER_NPCS.length, 65);
  ...
  it("separates report building references from Grove people/NPC reference coordinates", () => {
    const audit = validateHarthmereGroveBusinessCoordinateReferenceRoles();
    assert.equal(audit.ok, true, audit.errors.join(", "));
    assert.equal(audit.buildingReferenceCount, 8);
    assert.equal(audit.peopleReferenceCount, 6);
    assert.equal(
      HARTHMERE_GROVE_BUSINESS_DESIGN_FURNITURE_SCAN_COORDINATES.length,
      14
    );
    assert.equal(
    ...
    assert.equal(
      HARTHMERE_GROVE_BUSINESS_DESIGN_FURNITURE_SCAN.interiorCapturePolicy,
    ...
    assert.equal(
      HARTHMERE_GROVE_BUSINESS_DESIGN_FURNITURE_SCAN.constructionFindings
        .length,
      HARTHMERE_GROVE_BUSINESS_DESIGN_FURNITURE_SCAN_COORDINATES.length
    );
    assert.ok(
      HARTHMERE_GROVE_BUSINESS_DESIGN_FURNITURE_SCAN.constructionFindings.every(
        (finding) =>
          finding.hasBuildingOrThreshold && finding.constructedWith.length >= 3
      )
    );
    assert.ok(
      HARTHMERE_GROVE_BUSINESS_DESIGN_FURNITURE_SCAN.constructionFindings.some(
        (finding) => finding.patternId === "gray_office_lounge"
      )
    );
    assert.ok(
      HARTHMERE_GROVE_BUSINESS_DESIGN_FURNITURE_SCAN.reusableInteriorCues.some(
        (cue) => cue.includes("service counters")
      )
    );
    assert.equal(
      HARTHMERE_GROVE_BUSINESS_PEOPLE_REFERENCE_COORDINATES.length,
      6
    );
    assert.equal(
    ...
    assert.ok(
      HARTHMERE_GROVE_BUSINESS_PEOPLE_SOURCE_SCAN.findings.every((finding) =>
        finding.semanticUse.some((use) =>
          use.includes("not a business outpost site")
        )
      )
    );
    assert.equal(GROVE_ECONOMY_STARTER_NPCS.length, 6);
    assert.ok(
      GROVE_ECONOMY_STARTER_NPCS.every(
        (npc) =>
  ...
  it("defines a complete service mini-game for every production business type", () => {
    ...
    assert.deepEqual(
      Object.keys(HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS).sort(),
      businessTypes.sort()
    );
    assert.ok(
      HARTHMERE_BUSINESS_SCENARIO_MODIFIERS.length >= 30,
      "mini-game queues need at least 30 scenario modifiers for variety"
    );
    for (const typeId of businessTypes) {
      const definition =
        HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS[
          typeId as keyof typeof HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS
        ];
      assert.ok(
        definition.offers.length >= 3,
        `${typeId} needs multiple service choices`
      );
      assert.ok(
        definition.askTemplates.length >= 3,
        `${typeId} needs multiple customer asks`
      );
      ...
      assert.equal(
        definition.mechanicSpec,
        HARTHMERE_BUSINESS_MINIGAME_SPECS[
          typeId as keyof typeof HARTHMERE_BUSINESS_MINIGAME_SPECS
        ]
      );
      ...
      assert.equal(
        definition.navigation.movementPolicy,
        "walk_queue_counter_exit"
      );
      assert.deepEqual(definition.navigation.serviceFlow, [
        "enter",
        "join queue",
        "approach counter",
        "wait for service",
      ...
      assert.ok(
        definition.navigation.passableClearance.counterClearanceBlocks >= 2
      );
      assert.equal(
        definition.navigation.stuckRecovery.fallbackPolicy,
        "repath_then_sidestep_then_exit"
      );
      assert.ok(
        definition.navigation.stuckRecovery.fallbackExitAfterMs >= 10000
      );
      for (const ask of definition.askTemplates) {
        assert.ok(
          definition.offers.some(
            (offer) => offer.offerId === ask.desiredOfferId
          ),
          `${typeId} ask ${ask.askId} must match an offer`
        );
      }
    }
  });
  ...
  it("implements the documented mini-game rules for every business offer and edge case", () => {
    for (const [typeId, definition] of Object.entries(
      ...
      assert.equal(
        spec.offerRules.length,
        definition.offers.length,
        `${typeId} needs one mechanic rule per offer`
      );
      for (const offer of definition.offers) {
        ...
        assert.equal(
          ...
          `${typeId}:${
            offer.offerId
          } valid decision should pass ${result.warnings.join(", ")}`
        );
        const firstKey = Object.keys(decision)[0];
        assert.ok(
          firstKey,
          `${typeId}:${offer.offerId} should have an auditable decision field`
        );
        const invalid = { ...decision, [firstKey]: "__wrong__" };
        const failed = resolveHarthmereBusinessMiniGameDecision({
          typeId: typeId as any,
          offerId: offer.offerId,
          decision: invalid,
        });
        assert.equal(
          failed.ok,
          false,
          `${typeId}:${offer.offerId} wrong ${firstKey} should fail`
        );
      }
      for (const edgeCase of spec.edgeFailureActions) {
        ...
        assert.equal(
          failed.ok,
          false,
          `${typeId}:${edgeCase.failureId} should fail`
        );
        assert.ok(
          failed.warnings.includes(edgeCase.warning),
          `${typeId}:${edgeCase.failureId} should report ${edgeCase.warning}`
        );
      }
    }
  });
  ...
  it("assigns Bikkie graphics with metadata to every business service surface", () => {
    const businessTypes = Object.keys(HARTHMERE_ECONOMY_BUSINESS_TYPES);
    assert.deepEqual(
      Object.keys(HARTHMERE_BUSINESS_BIKKIE_GRAPHICS).sort(),
      businessTypes.sort()
    );
    const validation = validateHarthmereBusinessBikkieGraphics();
    assert.equal(validation.ok, true, JSON.stringify(validation));
    for (const typeId of businessTypes) {
      const graphics = getHarthmereBusinessBikkieGraphics(
        typeId as keyof typeof HARTHMERE_BUSINESS_BIKKIE_GRAPHICS
      );
      const definition =
        HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS[
          typeId as keyof typeof HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS
      ...
      assert.ok(
        graphics.length >= 3,
        `${typeId} should have several Bikkie graphics`
      );
      assert.deepEqual(definition.bikkieGraphics, graphics);
      assert.ok(
        graphics.some((graphic) => graphic.role === "primary_station"),
        `${typeId} needs a primary station`
      );
      for (const graphic of graphics) {
        assert.ok(
          Number(graphic.bikkieId) > 0,
          `${graphic.graphicId} needs a Bikkie id`
        );
        assert.ok(
          graphic.label.length >= 3,
          `${graphic.graphicId} needs a label`
        );
        assert.ok(
          graphic.description.length >= 10,
          `${graphic.graphicId} needs description metadata`
        );
        assert.ok(
          graphic.businessUse.length >= 10,
          `${graphic.graphicId} needs business-use metadata`
        );
        assert.ok(
          graphic.colors.length > 0,
          `${graphic.graphicId} needs color metadata`
        );
        assert.ok(
          graphic.visual,
          `${graphic.graphicId} needs a resolved visual`
        );
        ...
        if (graphic.galoisPath) {
          assert.equal(
            graphic.visual.iconAssetPath,
            `icons/${graphic.galoisPath}`
          );
        }
        if (graphic.kind === "crafting_station") {
          assert.ok(
            graphic.boxSize,
            `${graphic.graphicId} needs box size metadata`
          );
  ...
  it("maps the most relevant Bikkie graphics onto business recipes, stations, and service cues", () => {
    const namesFor = (
      typeId: keyof typeof HARTHMERE_BUSINESS_BIKKIE_GRAPHICS
    ) =>
      new Set(
        getHarthmereBusinessBikkieGraphics(typeId).map(
          (graphic) => graphic.bikkieName
        )
      );
    ...
    assert.ok(
      namesFor("custom_home_property_development").has("Builder's Wand")
    );
    assert.ok(namesFor("waste_sanitation_cleanup").has("Ye Olde Muck Buster"));
    assert.equal(
      getHarthmereBusinessBikkieGraphicForServiceCue(
        "procedural_plate_slide_counter"
      )?.bikkieName,
      "Kitchen"
    );
    assert.equal(
      getHarthmereBusinessBikkieGraphicForServiceCue(
        "procedural_parcel_weigh_tag"
      )?.bikkieName,
      "Parcel"
    );
    assert.equal(
      getHarthmereBusinessBikkieGraphicForServiceCue(
        "procedural_blueprint_unroll_point"
      )?.bikkieName,
      "Paper"
    );
  });
  ...
  it("keeps every service interaction on a procedural voxel-safe animation cue", () => {
    ...
    for (const definition of Object.values(
      ...
      for (const offer of definition.offers) {
        ...
        const spec = getHarthmereBusinessServiceAnimationCueSpec(
          offer.animationCue
        );
    ...
    assert.equal(
      cues.size,
      Object.values(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS).length
    );
  });

  it("cross-references every service ask item against the customer-service item catalog", () => {
    const validation = validateHarthmereBusinessServiceItemReferences();
    ...
    for (const definition of Object.values(
      ...
      for (const offer of definition.offers) {
        for (const itemId of Object.keys({
          ...
          const item = HARTHMERE_BUSINESS_SERVICE_ITEM_CATALOG[itemId];
  ...
  it("creates deterministic customer queues with real NPCs and matching asks", () => {
    const stats = defaultHarthmereBusinessCustomerStats("business_food");
    ...
    for (const ticket of result.queue) {
      assert.ok(
        HARTHMERE_BUSINESS_CUSTOMER_NPCS.some(
          (npc) => npc.npcId === ticket.npcId
        )
      );
  ...
  it("tracks cozy non-money service rewards without replacing money rewards", () => {
    ...
    const offer = definition.offers.find(
      (entry) => entry.offerId === "serve_worker_meal"
    )!;
    const ticket = createHarthmereBusinessCustomerQueue({
      businessId: "business_food",
      typeId: "food_service_restaurant",
      sessionId: "session_1",
      nowMs: 1000,
    ...
    const reward = createHarthmereBusinessCozyServiceReward({
      businessId: "business_food",
      typeId: "food_service_restaurant",
      npcId: ticket.npcId,
    ...
    applyHarthmereBusinessCozyServiceReward(stats, ticket.npcId, reward);
    assert.ok(reward.serviceXp > 0);
    assert.ok(reward.likeabilityDelta > 0);
    assert.ok(
      stats.serviceXp >= reward.serviceXp,
      JSON.stringify({
        statsServiceXp: stats.serviceXp,
        rewardServiceXp: reward.serviceXp,
      })
    );
    ...
    assert.ok(
      stats.thankYouNotes.some((note) => note.includes("your counter"))
    );
  ...
  it("places one spread-out non-Grove job business outpost per business type", () => {
    ...
    for (const outpost of HARTHMERE_BUSINESS_OUTPOSTS) {
      ...
      const expectedLocation =
        EXPECTED_OUTPOST_LOCATIONS[
          outpost.outpostId as keyof typeof EXPECTED_OUTPOST_LOCATIONS
        ];
      assert.ok(
        expectedLocation,
        `${outpost.outpostId} must have a fixed blueprint location`
      );
      assert.deepEqual(
    ...
    for (let i = 0; i < HARTHMERE_BUSINESS_OUTPOSTS.length; i += 1) {
      for (let j = i + 1; j < HARTHMERE_BUSINESS_OUTPOSTS.length; j += 1) {
        const a = HARTHMERE_BUSINESS_OUTPOSTS[i].position;
        const b = HARTHMERE_BUSINESS_OUTPOSTS[j].position;
        assert.ok(
          Math.hypot(a.x - b.x, a.z - b.z) >= 28,
          `${HARTHMERE_BUSINESS_OUTPOSTS[i].outpostId} is too close to ${HARTHMERE_BUSINESS_OUTPOSTS[j].outpostId}`
        );
      }
    }
  });
  ...
  it("cuts 19-business mini-game job pay to one quarter of authored values", () => {
    const restaurant =
      HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS.food_service_restaurant;
    assert.equal(HARTHMERE_BUSINESS_JOB_PAY_DIVISOR, 4);
    assert.equal(
      restaurant.offers.find((offer) => offer.offerId === "serve_worker_meal")
        ?.rewardGold,
      harthmereBusinessScaledJobPay(35)
    );
    assert.equal(
      restaurant.askTemplates.find((ask) => ask.askId === "hot_meal")
        ?.rewardGold,
      harthmereBusinessScaledJobPay(40)
    );
    assert.equal(
      HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS.medical_doctor.offers.find(
        (offer) => offer.offerId === "field_medkit_sale"
      )?.rewardGold,
      harthmereBusinessScaledJobPay(85)
    );
  });
  ...
  it("materializes every outpost as a server-owned procedural voxel building with passable customer paths", () => {
    ...
    for (const outpost of HARTHMERE_BUSINESS_OUTPOSTS) {
      ...
      assert.ok(
        record,
        `${outpost.outpostId} needs a procedural building record`
      );
      ...
      assert.equal(
        ...
        "building_system_materialization_plan"
      );
      assert.equal(
        record.materializationPlan.materializesSolidVoxelBuilding,
        true
      );
      assert.equal(
        record.origin.y,
        expectedGroundY,
        `${outpost.outpostId} origin must snap to its hilly terrain pad`
      );
      assert.equal(
        record.plot.groundY,
        expectedGroundY,
        `${outpost.outpostId} plot ground must match the pad`
      );
      assert.equal(
        record.materializationPlan.origin.y,
        expectedGroundY,
        `${outpost.outpostId} materialization plan must use the pad Y`
      );
      ...
      assert.ok(
        record.terrainGrounding.samples.some(
          (sample) =>
            sample.label === "front_door" && sample.y === expectedGroundY
        )
      );
      assert.equal(
      ...
      assert.ok(
        record.materializationPlan.safeZone,
        `${outpost.outpostId} must create a safe-zone record`
      );
      assert.equal(record.materializationPlan.safeZone?.safeFromMuck, true);
      assert.ok(
        record.materializationPlan.edits.some(
          (edit) => edit.label === "safe_ground"
        ),
        `${outpost.outpostId} must include safe-ground edits`
      );
      assert.ok(
        record.materializationPlan.edits.some(
          (edit) =>
      ...
      assert.deepEqual(
        record.visualReferenceCoordinates,
        HARTHMERE_GROVE_BUSINESS_BUILDING_REFERENCE_COORDINATES
      );
      ...
      assert.equal(
        HARTHMERE_GROVE_BUSINESS_BUILDING_SOURCE_SCAN.authoredPlacementFindings.filter(
          (finding) => finding.sourceKind === "authored_placement_cluster"
        ).length,
        3
      );
      assert.ok(
        record.buildingStyleKit.sourceAssetVocabulary.includes(
          "arch_wall_wood_door"
        ),
      ...
      assert.ok(
        record.buildingStyleKit.sourceFeatureTags.includes(
      ...
      assert.equal(
        record.buildingStyleKit.windowStyle,
        "large_framed_shop_glass"
      );
      assert.ok(
        record.buildingStyleKit.styleNotes.some(
          (note) => note.includes("Grove") || note.includes("grove")
        )
      );
      const exactPresentation =
        EXPECTED_OUTPOST_PRESENTATION[
          outpost.outpostId as keyof typeof EXPECTED_OUTPOST_PRESENTATION
        ];
      assert.ok(
        exactPresentation,
        `${outpost.outpostId} must have an exact guide presentation contract`
      );
      assert.equal(
        record.buildingStyleKit.referenceLanguage,
        exactPresentation[0],
        `${outpost.outpostId} uses the wrong Grove building language`
      );
      assert.equal(
        record.buildingStyleKit.signIcon,
        exactPresentation[1],
        `${outpost.outpostId} uses the wrong business sign icon`
      );
      assert.equal(
        record.buildingStyleKit.exteriorDressing,
        exactPresentation[2],
        `${outpost.outpostId} uses the wrong exterior dressing`
      );
      assert.ok(
        record.interiorFixtures.some(
          (fixture) => fixture.label === exactPresentation[3]
        ),
      ...
      const doorX =
        record.origin.x + Math.floor(record.blueprint.footprint.width / 2);
      ...
      const roofYs = new Set(
        editsByLabel("roof").map((edit) => edit.position[1])
      );
      assert.ok(
        roofYs.size >= 2 &&
          editsByLabel("roof").some(
            (edit) =>
              edit.position[0] < x0 ||
              edit.position[0] > x1 ||
              edit.position[2] < z0 ||
      ...
      assert.ok(
        editsByLabel("door_lock").some(
          (edit) => edit.position[2] <= record.origin.z
        ),
      ...
      assert.ok(
        staffWorkPoint.x > record.origin.x &&
          staffWorkPoint.x <
            record.origin.x + record.blueprint.footprint.width - 1 &&
          staffWorkPoint.z > record.origin.z &&
          staffWorkPoint.z <
            record.origin.z + record.blueprint.footprint.depth - 1,
      ...
      if (outpost.building.floors > 1) {
        assert.ok(
          editsByLabel("upgrade_addition").length >=
            (record.blueprint.footprint.width - 4) *
              (record.blueprint.footprint.depth - 6),
          `${outpost.outpostId} must materialize a real upper-floor deck for multi-floor business play`
        );
        assert.ok(
          editsByLabel("stair").some(
            (edit) => edit.position[1] > record.origin.y + 1
          ),
      ...
      assert.deepEqual(
        record.bikkieGraphics,
        getHarthmereBusinessBikkieGraphics(outpost.businessType)
      );
      assert.equal(record.primaryBikkieGraphic?.role, "primary_station");
      assert.ok(
        record.materializationPlan.inWorldMarkers?.some(
          (marker) =>
            marker.markerId === `${outpost.outpostId}:business-counter`
        )
      );
      assert.ok(
        record.materializationPlan.inWorldMarkers?.some(
          (marker) =>
            marker.markerId === `${outpost.outpostId}:customer-dashboard`
        )
      );
      assert.ok(
        record.materializationPlan.inWorldMarkers?.some(
          (marker) =>
            marker.markerId === `${outpost.outpostId}:dashboard-access-post`
        )
      );
      assert.ok(
        record.materializationPlan.inWorldMarkers?.some(
          (marker) =>
            marker.markerId === `${outpost.outpostId}:staff-npc` &&
            marker.kind === "npc_board"
        )
      );
      assert.ok(
        record.materializationPlan.inWorldMarkers?.some(
          (marker) => marker.markerId === `${outpost.outpostId}:customer-npc`
        )
      );
      assert.equal(
        record.dashboardAccessPoint.interaction,
      ...
      const dashboardKioskEdits = record.materializationPlan.edits.filter(
        (edit) =>
          edit.label === "business_marker" &&
          Math.abs(edit.position[0] - record.dashboardAccessPoint.position.x) <=
            1 &&
          edit.position[2] === record.dashboardAccessPoint.position.z &&
          edit.position[1] >= record.dashboardAccessPoint.position.y &&
          edit.position[1] <= record.dashboardAccessPoint.position.y + 2
      );
      assert.ok(
      ...
      assert.ok(
        record.dashboardAccessPoint.position.x >= record.origin.x + 2 &&
          record.dashboardAccessPoint.position.x <=
            record.origin.x + record.blueprint.footprint.width - 2,
      ...
      assert.ok(
        record.interiorFixtures.some(
          (fixture) =>
            fixture.role === "dashboard_access" && !fixture.blocksNavigation
        )
      );
      assert.ok(
        record.interiorFixtures.some(
          (fixture) =>
            fixture.role === "service_counter" && !fixture.blocksNavigation
        )
      );
      assert.ok(
        record.interiorFixtures.some(
      ...
      const fixtureIds = new Set(
        record.interiorFixtures.map((fixture) => fixture.fixtureId)
      );
      assert.equal(
        fixtureIds.size,
        record.interiorFixtures.length,
        `${outpost.outpostId} must not duplicate interior fixture IDs`
      );
      const businessSpecificFixtures = record.interiorFixtures.filter(
        (fixture) => fixture.businessSpecific
      );
      assert.ok(businessSpecificFixtures.length >= 5);
      assert.ok(
        businessSpecificFixtures.some(
          (fixture) =>
            fixture.role === "workstation" || fixture.role === "primary_station"
        ),
        `${outpost.outpostId} must include a business-specific work station`
      );
      assert.ok(
        businessSpecificFixtures.some(
          (fixture) => fixture.role === "stock_storage"
        ),
        `${outpost.outpostId} must include business-specific stock/storage decor`
      );
      assert.ok(
        businessSpecificFixtures.every(
          (fixture) =>
            fixture.label !== "Service worktable" ||
            !/restaurant|medical|weapons|refinery|portal|farming|courier|hospitality|security|waste|repair|design|property|exploration/.test(
              outpost.businessType
            )
        ),
      ...
      assert.ok(
        record.materializationPlan.inWorldMarkers?.some(
          (marker) =>
            marker.markerId.includes(":bikkie:") &&
            marker.label.includes(record.primaryBikkieGraphic!.label)
        )
      );
      const marker = HARTHMERE_BUSINESS_OUTPOST_MAP_MARKERS.find(
        (entry) => entry.outpostId === outpost.outpostId
      );
      assert.equal(
        marker?.primaryBikkieGraphic?.bikkieId,
        record.primaryBikkieGraphic?.bikkieId
      );
      assert.equal(
        marker?.primaryBikkieVisual?.visualId,
        record.primaryBikkieGraphic?.visual.visualId
      );
      ...
      assert.ok(
        record.materializationPlan.edits.some(
          (edit) =>
            ...
            edit.position[1] < record.origin.y - 1
        ),
      ...
      const audit = validateHarthmereBusinessOutpostPassability(record);
      assert.equal(
        audit.ok,
        true,
        `${outpost.outpostId} passability errors: ${audit.errors.join(", ")}`
      );
      assert.ok(audit.auditTags.includes("customer_path_clear"));
      const liveAudit =
        validateHarthmereBusinessOutpostLiveWorldNavigation(record);
      assert.equal(
        liveAudit.ok,
        true,
        `${
          outpost.outpostId
        } live navigation errors: ${liveAudit.unreachableRoutes.join(
          ", "
        )} ${liveAudit.unresolvedCollisions.join(", ")}`
      );
  ...
  it("keeps production-coordinate business pads clear of existing Grove reference buildings", () => {
    ...
    for (const outpost of HARTHMERE_BUSINESS_OUTPOSTS) {
      ...
      const nearestReferenceMeters = Math.min(
        ...HARTHMERE_GROVE_BUSINESS_BUILDING_REFERENCE_COORDINATES.map(
      ...
      assert.ok(
        ...
        `${
          outpost.outpostId
        } shifted world pad must not overlap or crowd existing Grove reference buildings; nearest=${nearestReferenceMeters.toFixed(
          1
        )}m`
      );
    }
  ...
  it("fails live-world navigation when a permanent player object seals the service counter", () => {
    ...
    assert.ok(
      audit.unreachableRoutes.some(
        (route) => route.includes("service") || route.includes("counter")
      )
    );
  });

  it("creates cleanup-plus-rebuild plans for production business outpost migrations", () => {
    const plans = createHarthmereBusinessOutpostRebuildMaterializationPlans();
    assert.equal(plans.length, HARTHMERE_BUSINESS_OUTPOSTS.length * 2);
    assert.ok(
      HARTHMERE_BUSINESS_OUTPOST_REBUILD_REVISION.includes("real-interior")
    );
    for (let index = 0; index < plans.length; index += 2) {
      ...
      assert.ok(
        ...
        `${cleanup.requestId} needs enough deletion edits to remove stale shells`
      );
      assert.ok(
        cleanup.edits.every(
          (edit) => edit.label === "demolition_cleanup" && edit.value === 0
        )
      );
      ...
      assert.equal(
        rebuild.edits.filter(
          (edit) =>
            ...
            edit.value === BUILDING_SYSTEM_TERRAIN_BLOCKS.woodCrate
        ).length,
        0,
  ...
  it("publishes all four minigame fixture labels as named interior fixtures for every outpost", () => {
    for (const outpost of HARTHMERE_BUSINESS_OUTPOSTS) {
      ...
      const spec =
        HARTHMERE_BUSINESS_MINIGAME_SPECS[
          outpost.businessType as keyof typeof HARTHMERE_BUSINESS_MINIGAME_SPECS
        ];
      ...
      const businessSpecific = record.interiorFixtures.filter(
        (f) => f.businessSpecific
      );
      const [primaryBoard, , stockSurface] = spec.interiorFixtureLabels;
      assert.ok(
        businessSpecific.some(
          (f) => f.label === primaryBoard && f.role === "workstation"
        ),
        `${outpost.outpostId} primary board fixture must be role "workstation"`
      );
      assert.ok(
        businessSpecific.some(
          (f) => f.label === stockSurface && f.role === "stock_storage"
        ),
    ...
    for (let index = 0; index < plans.length; index += 2) {
      ...
      for (const edit of rebuild.edits) {
        ...
        assert.ok(
          ...
          `${outpostId} cleanup must cover rebuild position ${edit.position.join(
            ":"
          )} (label: ${edit.label})`
        );
      }
  ...
  it("reports legacy outpost records with missing validation arrays instead of throwing", () => {
    ...
    const passability =
      validateHarthmereBusinessOutpostPassability(legacyRecord);
    assert.equal(passability.ok, false);
    assert.ok(
      passability.errors.includes("outpost_style_kit_missing_style_notes")
    );
    assert.ok(passability.errors.includes("outpost_missing_interior_fixtures"));
    assert.ok(
      passability.errors.includes("outpost_materialization_plan_missing_edits")
    );

    const navigation =
      validateHarthmereBusinessOutpostLiveWorldNavigation(legacyRecord);
  ...
  it("treats the inside of every business safe site as safe and far-away wilds as unsafe", () => {
    ...
    assert.equal(
      isPointInsideHarthmereBusinessSafeSite({ x: 332, z: -390 }),
      false,
  ...
  it("clears the full graded garden site before rebuild so no muck or hill remains", () => {
    ...
    for (let index = 0; index < plans.length; index += 2) {
      ...
      for (const edit of record.materializationPlan.edits) {
        ...
        assert.ok(
          cleanupKeys.has(edit.position.join(":")),
          `${outpostId} cleanup must cover graded edit ${edit.position.join(
            ":"
          )} (${edit.label})`
        );
      }
