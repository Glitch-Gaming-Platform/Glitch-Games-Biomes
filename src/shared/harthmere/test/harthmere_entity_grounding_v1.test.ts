...
describe("harthmere entity grounding: canStandAt", () => {
  it("stands on top of the surface with clear head room", () => {
    ...
    assert.equal(harthmereCanStandAtV1(FLAT_WILDS, 0, 54, 0), true);
    assert.equal(harthmereCanStandAtV1(FLAT_WILDS, 0, 53, 0), false); // inside solid
    assert.equal(harthmereCanStandAtV1(FLAT_WILDS, 0, 55, 0), false); // floating
  });
});
...
describe("harthmere entity grounding: findGroundFeetY", () => {
  it("flat wilds: feet land at surface+1 regardless of a wrong hint", () => {
    for (const hint of [54, 40, 90, 53, 100, 20]) {
      assert.equal(
        findHarthmereGroundFeetYV1(FLAT_WILDS, 10, -10, { hintY: hint }),
        54,
  ...
  it("EDGE: a muck mucker stamped at the wrong Grove Y=70 still lands on wilds ground (54)", () => {
    ...
    assert.equal(findHarthmereGroundFeetYV1(FLAT_WILDS, 512, -152, { hintY: 70 }), 54);
  });

  it("EDGE: a Grove NPC at Harthmere height lands on the raised courtyard (70)", () => {
    assert.equal(findHarthmereGroundFeetYV1(FLAT_GROVE, 486, -209, { hintY: 54 }), 70);
    assert.equal(findHarthmereGroundFeetYV1(FLAT_GROVE, 486, -209, { hintY: 70 }), 70);
  });
  ...
  it("HILLY: feet track the real surface across a steep slope (flat Y would float/bury)", () => {
    ...
    for (const x of [0, 5, 12, 25, 40]) {
      assert.equal(
        findHarthmereGroundFeetYV1(hill, x, 0, { hintY: 54 }),
        51 + x,
  ...
  it("un-buries a hint that sits inside the terrain", () => {
    ...
    assert.equal(findHarthmereGroundFeetYV1(tall, 0, 0, { hintY: 60 }), 81);
  });

  it("drops a hint floating high above the ground down onto it", () => {
    const ground = heightmapSampler(() => 30); // feet 31
    assert.equal(findHarthmereGroundFeetYV1(ground, 0, 0, { hintY: 95 }), 31);
  });
  ...
  it("returns undefined when no surface is within the scan budget (cliff)", () => {
    ...
    assert.equal(
      findHarthmereGroundFeetYV1(deep, 0, 0, { hintY: 200, maxScanDown: 20 }),
      undefined
    );
  });

  it("returns undefined for an ungenerated/empty column", () => {
    const empty: HarthmereSolidSamplerV1 = () => false;
    assert.equal(findHarthmereGroundFeetYV1(empty, 0, 0, { hintY: 60 }), undefined);
  });

  it("respects the default upward budget at the seam (Grove hint over wilds)", () => {
    // 70 -> 54 is 16 down, well within budget.
    assert.equal(findHarthmereGroundFeetYV1(FLAT_WILDS, 0, 0, { hintY: 70 }), 54);
    // And a deeply buried hint within the default up budget still climbs out.
    const surface = heightmapSampler(() => 100);
    const within = 100 + 1 - HARTHMERE_GROUND_SCAN_UP_DEFAULT_V1; // exactly reachable
    assert.equal(
      findHarthmereGroundFeetYV1(surface, 0, 0, { hintY: within }),
      101
    );
  });

  it("prefers the nearer surface and never floats on a flat hint", () => {
    assert.equal(findHarthmereGroundFeetYV1(FLAT_WILDS, 0, 0, { hintY: 54 }), 54);
  });
});
...
describe("harthmere entity grounding: CAVES", () => {
  ...
  const caveColumn: HarthmereSolidSamplerV1 = (_x, y) => {
  ...
  it("REGRESSION: requireOpenSky lands on the surface, never the cave floor", () => {
    ...
    assert.equal(
      findHarthmereGroundFeetYV1(caveColumn, 0, 0, { hintY: 42, requireOpenSky: true }),
      70
    );
    // Hint right at the cave floor.
    assert.equal(
      findHarthmereGroundFeetYV1(caveColumn, 0, 0, { hintY: 40, requireOpenSky: true }),
      70
    );
  });
  ...
  it("shows the bug requireOpenSky fixes: nearest-mode would pick the cave floor", () => {
    ...
    assert.equal(
      findHarthmereGroundFeetYV1(caveColumn, 0, 0, { hintY: 42, requireOpenSky: false }),
      40
    );
  });
  ...
  it("a floating hint above a cave still grounds on the surface in both modes", () => {
    ...
    assert.equal(findHarthmereGroundFeetYV1(caveColumn, 0, 0, { hintY: 80 }), 70);
    assert.equal(
      findHarthmereGroundFeetYV1(caveColumn, 0, 0, { hintY: 80, requireOpenSky: true }),
      70
    );
  });
});
...
describe("harthmere entity grounding: BUILDINGS (indoor floors)", () => {
  ...
  const buildingColumn: HarthmereSolidSamplerV1 = (_x, y) => {
  ...
  it("nearest-mode keeps an owner on the building FLOOR (not the roof)", () => {
    ...
    assert.equal(
      findHarthmereGroundFeetYV1(buildingColumn, 0, 0, { hintY: 54, requireOpenSky: false }),
      54
    );
  });
  ...
  it("DOCUMENTS: requireOpenSky would push an indoor entity onto the roof", () => {
    ...
    assert.equal(
      findHarthmereGroundFeetYV1(buildingColumn, 0, 0, { hintY: 54, requireOpenSky: true }),
      61
    );
  });
});
...
describe("harthmere entity grounding: WATER", () => {
  ...
  // (41..50); air above 50.
  const lake: HarthmereSolidSamplerV1 = (_x, y) => {
  ...
  it("rests an entity ON the water surface, not on the lake bed underwater", () => {
    ...
    // is never chosen.
    assert.equal(findHarthmereGroundFeetYV1(lake, 0, 0, { hintY: 54 }), 51);
    assert.equal(
      findHarthmereGroundFeetYV1(lake, 0, 0, { hintY: 54, requireOpenSky: true }),
      51
    );
  });

  it("a hint already at/below the water still surfaces to the water top", () => {
    assert.equal(findHarthmereGroundFeetYV1(lake, 0, 0, { hintY: 45 }), 51);
  });
});
...
describe("harthmere entity grounding: groundEntityPosition", () => {
  it("keeps X/Z and replaces Y with the surface feet", () => {
    assert.deepEqual(
      groundHarthmereEntityPositionV1(FLAT_WILDS, [123.5, 70, -45.25]),
      [123.5, 54, -45.25]
    );
  });
  ...
  it("applies a hover offset (e.g. floating quest markers)", () => {
    assert.deepEqual(
      groundHarthmereEntityPositionV1(FLAT_WILDS, [0, 70, 0], { hoverBlocks: 2 }),
      [0, 56, 0]
    );
  });
  ...
  it("falls back to the original Y when no ground is found (no teleporting)", () => {
    const empty: HarthmereSolidSamplerV1 = () => false;
    assert.deepEqual(
      groundHarthmereEntityPositionV1(empty, [7, 88, 9]),
      [7, 88, 9]
    );
  });
});
