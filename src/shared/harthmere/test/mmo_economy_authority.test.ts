...
import { HARTHMERE_BUSINESS_OUTPOST_PROCEDURAL_BUILDINGS } from "../business_customer_simulator";
import { normalizeHarthmereEconomyBusinessSystemsState } from "../mmo_economy_business_systems";
import type {
...
function createBusiness(
  state = defaultHarthmereProductionEconomyState(),
  type: HarthmereEconomyMutationRequest["businessType"] = "food_service_restaurant",
  name = "Grove Hearth",
) {
...
describe("mmo_economy_authority — catalog and lifecycle", () => {
  it("starts with no runtime businesses/contracts but exposes the full economy business catalog", () => {
    ...
    assert.strictEqual(Object.keys(HARTHMERE_ECONOMY_BUSINESS_TYPES).length, 19);
    assert.ok(HARTHMERE_ECONOMY_BUSINESS_TYPES.exotic_matter_refinery.startCostGold > 0);
    assert.ok(
      HARTHMERE_ECONOMY_BUSINESS_TYPES.exotic_matter_refinery.inputItemFamilies.includes(
        "antihydrogen_block"
      )
    );
    assert.ok(
      HARTHMERE_ECONOMY_BUSINESS_TYPES.exotic_matter_refinery.outputItemFamilies.includes(
        "alcubierre_drive_core"
      )
    );
    assert.ok(HARTHMERE_ECONOMY_BUSINESS_TYPES.courier.startCostGold > 0);
  });
...
describe("mmo_economy_authority — inventory, pricing, taxes, and sales", () => {
  ...
  it("records customer shop purchases, charges buyers, applies town tax, and uses supply-demand pricing", () => {
    ...
    const price = economyPriceForItem({
      state,
  ...
  it("rejects customer purchase edge cases without moving gold or stock", () => {
    ...
    const closed = JSON.parse(JSON.stringify(state)) as HarthmereProductionEconomyState;
...
describe("mmo_economy_authority — money-conservation edge cases (audit hardening)", () => {
  ...
  it("settles the defaulted loan and clears business debt when collateral is seized", () => {
    const setup = createBusiness(defaultHarthmereProductionEconomyState(), "general_trader", "Foreclosed Books");
    let state = licenseAndOpen(setup.state, setup.businessId, 1);
  ...
  it("settles outstanding loans from liquidation proceeds before crediting the owner", () => {
    const setup = createBusiness(defaultHarthmereProductionEconomyState(), "general_trader", "Liquidation Books");
    let state = licenseAndOpen(setup.state, setup.businessId, 1);
...
describe("mmo_economy_authority — logistics, market, town simulation, and NPC competition", () => {
  ...
  it("supports market sell orders and settlement between real businesses", () => {
    const seller = createBusiness(defaultHarthmereProductionEconomyState(), "general_trader", "Seller Shop");
  ...
  it("runs town demand ticks using stocked open businesses and explicit NPC competitors", () => {
    const setup = createBusiness(defaultHarthmereProductionEconomyState(), "food_service_restaurant", "Soup Loop");
    let state = licenseAndOpen(setup.state, setup.businessId, 1);
    state.businesses[setup.businessId].inventory.worker_meal = { itemId: "worker_meal", count: 5 };
...
describe("live_mode_backend — production economy integration", () => {
  it("persists economy mutations through request_economy_mutation and returns a client snapshot", () => {
    ...
    const snapshot = createHarthmereProductionEconomyClientSnapshotFromBackend(reduced.state) as any;
  ...
  it("builds production economy snapshots when Redis contains a legacy outpost record", () => {
    ...
    const snapshot = createHarthmereProductionEconomyClientSnapshotFromBackend(
      state
    ) as any;
  ...
  it("replaces stale persisted business outpost records with the canonical backend voxel plans", () => {
    const canonical = HARTHMERE_BUSINESS_OUTPOST_PROCEDURAL_BUILDINGS.outpost_restaurant_redpot;
    ...
    const normalized = normalizeHarthmereEconomyBusinessSystemsState({
      outpostBuildings: {
  ...
  it("moves real player inventory into business storage through the live reducer", () => {
    let state = defaultHarthmereLiveModeBackendState(ACTOR, NOW_MS);
...
describe("mmo_economy_authority — business-specific production systems", () => {
  function createOpenBusiness(
    ...
    const setup = createBusiness(defaultHarthmereProductionEconomyState(), type, name);
  ...
  it("enforces property development edge cases before rent, duplicate projects, or invalid progress can leak through", () => {
    const setup = createBusiness(defaultHarthmereProductionEconomyState(), "custom_home_property_development", "Permit Lock Builders");
    let state = setup.state;
...
describe("mmo_economy_authority — business banks, permissions, and balance guards", () => {
  it("creates business bank accounts, supports owner transfers, audit logs, and permission-scoped accountants", () => {
    ...
    result = reduceHarthmereEconomyMutation(state, req("transfer_business_to_personal_bank", { businessId: setup.businessId, amountGold: 100, actorId: "accountant_1" } as any), ctx({ actorGold: 0, businessPermissions: { [`${setup.businessId}:accountant_1`]: ["accountant"] } as any }) as any);
    assert.deepStrictEqual(result.warnings, []);
  ...
  it("lets the owner withdraw EARNED revenue (not just prior deposits) to personal gold", () => {
    ...
    // was flush.
    const setup = createBusiness(defaultHarthmereProductionEconomyState(), "food_service_restaurant", "Grove Hearth");
    let state = licenseAndOpen(setup.state, setup.businessId, 1);
    state.businesses[setup.businessId].inventory.worker_meal = { itemId: "worker_meal", count: 5 };
    state.regions.harthmere_grove_region.itemDemand.worker_meal = 100;
    state.regions.harthmere_grove_region.itemSupply.worker_meal = 10;
  ...
  it("supports guild and town business operation permissions without giving every actor full ownership", () => {
    ...
    assert.deepStrictEqual(result.warnings, []);
    state = result.economy;
    result = reduceHarthmereEconomyMutation(state, req("refresh_trader_inventory", { businessId, amountGold: 50, actorId: "guild_worker" } as any), ctx({ actorGold: 0 }) as any);
    ...
    result = reduceHarthmereEconomyMutation(state, req("refresh_trader_inventory", { businessId, amountGold: 50, actorId: "guild_worker" } as any), ctx({ actorGold: 0, actorGuildPermissions: { guild_econ: ["inventory_manager"] } as any }) as any);
    assert.deepStrictEqual(result.warnings, []);
    ...
    result = reduceHarthmereEconomyMutation(state, req("cleanup_contamination_site", { businessId: townBusinessId, siteId, actorId: "town_worker" } as any), ctx({ actorGold: 0, actorTownPermissions: { harthmere_grove: ["world_operator"] } as any }) as any);
    assert.deepStrictEqual(result.warnings, []);
  });

  it("handles rent, loan defaults, collateral seizure, bankruptcy liquidation, and balance sanity warnings", () => {
    const setup = createBusiness(defaultHarthmereProductionEconomyState(), "general_trader", "Risky Books");
    let state = licenseAndOpen(setup.state, setup.businessId, 1);
    state.businesses[setup.businessId].balanceGold = 50;
