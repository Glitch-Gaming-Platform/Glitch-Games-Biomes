...
// fell back to a flat authored Y. findHarthmereGroundFeetYWithStatus now
...
}): { isSolid: HarthmereSolidSampler; isLoaded: HarthmereLoadedSampler } {
  const isLoaded: HarthmereLoadedSampler = (x, _y, z) =>
    opts.surfaceY(x, z) !== undefined;
  const isSolid: HarthmereSolidSampler = (x, y, z) => {
...
describe("harthmere grounding tri-state (quest item visibility)", () => {
  it("grounds onto the real surface below the authored hint", () => {
    ...
    const r = findHarthmereGroundFeetYWithStatus(isSolid, isLoaded, 10, 10, {
  ...
  it("climbs out when the authored hint is buried inside terrain", () => {
    ...
    const r = findHarthmereGroundFeetYWithStatus(isSolid, isLoaded, 1, 1, {
  ...
  it("returns not-loaded (defer) instead of stamping the flat Y when terrain is unstreamed", () => {
    ...
    const r = findHarthmereGroundFeetYWithStatus(isSolid, isLoaded, 5, 5, {
      hintY: 53,
    });
  ...
  it("rejects a cave floor when requireOpenSky is set", () => {
    ...
    const isLoaded: HarthmereLoadedSampler = () => true;
    const isSolid: HarthmereSolidSampler = (_x, y) =>
      y < 53 || (y >= 56 && y <= 70); // ground below 53, ceiling slab 56..70
    const r = findHarthmereGroundFeetYWithStatus(isSolid, isLoaded, 0, 0, {
      hintY: 54,
      requireOpenSky: true,
      maxScanDown: 4,
      maxScanUp: 4,
  ...
  it("grounds (not not-loaded) even if far voxels are unloaded, as long as a real surface is found", () => {
    ...
    const isLoaded: HarthmereLoadedSampler = (_x, y) => y > 40; // deep voxels "unloaded"
    const isSolid: HarthmereSolidSampler = (_x, y) => y < 52;
    const r = findHarthmereGroundFeetYWithStatus(isSolid, isLoaded, 0, 0, {
      hintY: 55,
    });
    assert.strictEqual(r.status, "grounded");
  ...
  it("groundHarthmereEntityPosition keeps the original Y on no surface (no teleport)", () => {
    const { isSolid } = makeWorld({ surfaceY: () => -9999 });
    const pos = groundHarthmereEntityPosition(isSolid, [12, 53, -8], {
      maxScanDown: 4,
      maxScanUp: 4,
    });
