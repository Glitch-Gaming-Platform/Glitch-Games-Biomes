...
describe("business_customer_simulator_v1", () => {
  ...
  it("separates report building references from Grove people/NPC reference coordinates", () => {
    ...
    assert.equal(
      HARTHMERE_GROVE_BUSINESS_PEOPLE_REFERENCE_COORDINATES_V1.length,
      6
    );
    assert.equal(
    ...
    assert.ok(
      GROVE_ECONOMY_STARTER_NPCS_V1.every(
        (npc) =>
  ...
  it("defines a complete service mini-game for every production business type", () => {
    ...
    for (const typeId of businessTypes) {
      const definition =
        HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS_V1[
          typeId as keyof typeof HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS_V1
        ];
      assert.ok(
      ...
      assert.equal(
        ...
        HARTHMERE_BUSINESS_MINIGAME_SPECS_V1[
          typeId as keyof typeof HARTHMERE_BUSINESS_MINIGAME_SPECS_V1
        ]
      );
  ...
  it("assigns Bikkie graphics with metadata to every business service surface", () => {
    const businessTypes = Object.keys(HARTHMERE_ECONOMY_BUSINESS_TYPES_V1);
    assert.deepEqual(
      Object.keys(HARTHMERE_BUSINESS_BIKKIE_GRAPHICS_V1).sort(),
      businessTypes.sort()
    );
    const validation = validateHarthmereBusinessBikkieGraphicsV1();
    assert.equal(validation.ok, true, JSON.stringify(validation));
    for (const typeId of businessTypes) {
      const graphics = getHarthmereBusinessBikkieGraphicsV1(
        typeId as keyof typeof HARTHMERE_BUSINESS_BIKKIE_GRAPHICS_V1
      );
      const definition =
        HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS_V1[
          typeId as keyof typeof HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS_V1
  ...
  it("maps the most relevant Bikkie graphics onto business recipes, stations, and service cues", () => {
    const namesFor = (
      typeId: keyof typeof HARTHMERE_BUSINESS_BIKKIE_GRAPHICS_V1
    ) =>
      new Set(
        getHarthmereBusinessBikkieGraphicsV1(typeId).map(
  ...
  it("keeps every service interaction on a procedural voxel-safe animation cue", () => {
    ...
    for (const definition of Object.values(
      ...
      for (const offer of definition.offers) {
        ...
        const spec = getHarthmereBusinessServiceAnimationCueSpecV1(
          offer.animationCue
        );
    ...
    assert.equal(
      cues.size,
      Object.values(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS_V1).length
    );
  });
  ...
  it("places one spread-out non-Grove job business outpost per business type", () => {
    ...
    assert.deepEqual(
      HARTHMERE_BUSINESS_OUTPOSTS_V1.map(
    ...
    for (let i = 0; i < HARTHMERE_BUSINESS_OUTPOSTS_V1.length; i += 1) {
      for (let j = i + 1; j < HARTHMERE_BUSINESS_OUTPOSTS_V1.length; j += 1) {
        ...
        assert.ok(
          ...
          `${HARTHMERE_BUSINESS_OUTPOSTS_V1[i].outpostId} is too close to ${HARTHMERE_BUSINESS_OUTPOSTS_V1[j].outpostId}`
        );
      }
    }
  });
  ...
  it("materializes every outpost as a server-owned procedural voxel building with passable customer paths", () => {
    ...
    for (const outpost of HARTHMERE_BUSINESS_OUTPOSTS_V1) {
      ...
      assert.equal(
        HARTHMERE_GROVE_BUSINESS_BUILDING_SOURCE_SCAN_V1.scannedCoordinates
          .length,
        8
      );
      assert.equal(
      ...
      assert.deepEqual(
        ...
        getHarthmereBusinessBikkieGraphicsV1(outpost.businessType)
      );
      ...
      const liveAudit =
        validateHarthmereBusinessOutpostLiveWorldNavigationV1(record);
      assert.equal(
        liveAudit.ok,
        true,
        `${
  ...
  it("fails live-world navigation when a permanent player object seals the service counter", () => {
    ...
    const audit = validateHarthmereBusinessOutpostLiveWorldNavigationV1(
      record,
      {
        dynamicBlockers: [
          {
  ...
  it("creates cleanup-plus-rebuild plans for production business outpost migrations", () => {
    ...
    for (let index = 0; index < plans.length; index += 2) {
      ...
      assert.ok(
        cleanup.requestId.endsWith("_backend_cleanup_before_rebuild_v2")
      );
      ...
      const record =
        HARTHMERE_BUSINESS_OUTPOST_PROCEDURAL_BUILDINGS_V1[outpostId];
      assert.ok(
        record &&
          cleanup.edits.some(
            (edit) =>
  ...
  it("publishes all four minigame fixture labels as named interior fixtures for every outpost", () => {
    for (const outpost of HARTHMERE_BUSINESS_OUTPOSTS_V1) {
      ...
      const spec =
        HARTHMERE_BUSINESS_MINIGAME_SPECS_V1[
          outpost.businessType as keyof typeof HARTHMERE_BUSINESS_MINIGAME_SPECS_V1
        ];
  ...
  it("reports legacy outpost records with missing validation arrays instead of throwing", () => {
    ...
    const passability =
      validateHarthmereBusinessOutpostPassabilityV1(legacyRecord);
    ...
    const navigation =
      validateHarthmereBusinessOutpostLiveWorldNavigationV1(legacyRecord);
