...
describe("Harthmere NPC navigation guard current", () => {
  ...
  it("slides around a blocker instead of walking through the object in front", () => {
    const state = createHarthmereNpcNavigationState();
    const obstacle: HarthmereNpcNavigationObstacle = {
      id: "crate",
      label: "Town crate",
      cx: 1,
      cz: 0,
  ...
  it("holds last safe position and marks stuck after repeated fully blocked moves", () => {
    ...
    for (let i = 0; i < 12; i += 1) {
      result = resolveHarthmereNpcNavigationStep({
        mode: "combat_chase",
  ...
  it("keeps collision work bounded for dense nearby plants and props", () => {
    const state = createHarthmereNpcNavigationState();
    const obstacles = Array.from({ length: 120 }, (_, i): HarthmereNpcNavigationObstacle => ({
      id: `plant-${i}`,
      cx: 20 + i,
      cz: 20,
      halfX: 0.5,
