...
import { SNAPSHOT_GROVE_QUESTS, type SnapshotGroveTrigger } from "@/shared/harthmere/snapshot_grove_content";
import {
...
if (typeof (describe as any) === "function" && typeof (test as any) === "function") {
  (describe as any)("Snapshot Grove tutorial trigger contract current", () => {
    ...
    (test as any)("the current and current browser item-use events stay compatible", () => {
    ...
    (test as any)("wrong item families do not accidentally complete item-use objectives", () => {
      for (const row of itemUseRows()) {
        const family = snapshotGroveItemUseObjectiveKind(row.quest, row.objectiveIndex);
        (expect as any)(
          snapshotGroveItemUseEventMatchesObjective(
