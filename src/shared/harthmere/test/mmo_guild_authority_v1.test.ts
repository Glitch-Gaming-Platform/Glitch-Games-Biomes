...
import {
  ...
  validateHarthmereGuildIdentityV1,
...
function mutate(
  ...
  return reduceHarthmereGuildMutationV1(
    state,
    {
...
function createGuild(state = defaultHarthmereLiveModeGuildStateV1(), actorId = LEADER) {
...
function applyLive(
  ...
  return reduceHarthmereLiveModeBackendStateV1(state, env(actionKind, actorId, payload), NOW_MS);
}
...
before(function registerGuildTestItems() {
  ...
  const questSeal: HarthmereItemDefinitionV1 = {
  ...
  registerHarthmereItemDefinitionV1(ironOre);
  registerHarthmereItemDefinitionV1(questSeal);
});

describe("mmo_guild_authority_v1 — identity and creation", function () {
  it("rejects invalid, reserved, and malformed guild identity", function () {
    assert.ok(!validateHarthmereGuildIdentityV1({ name: "GM", tag: "G" }).ok);
    assert.ok(!validateHarthmereGuildIdentityV1({ name: "Biomes Admins", tag: "ADM" }).ok);
    assert.ok(!validateHarthmereGuildIdentityV1({ name: "Bad!!!Name", tag: "BAD" }).ok);
    assert.ok(validateHarthmereGuildIdentityV1({ name: "Iron Lanterns", tag: "IL" }).ok);
  });
...
describe("mmo_guild_authority_v1 — finder, applications, and invites", function () {
  it("exposes a guild finder entry and supports application accept/reject flow", function () {
    const { state, guildId } = createGuild();
    const finder = createHarthmereLiveModeGuildClientSnapshotV1(state, APPLICANT);
  ...
  it("supports direct invites, expiration, and target acceptance", function () {
    ...
    const expiredAccept = reduceHarthmereGuildMutationV1(expiredInviteResult.guild, {
...
describe("mmo_guild_authority_v1 — member management and ranks", function () {
...
describe("mmo_guild_authority_v1 — guild hall and chat", function () {
  it("links a completed guild hall to guild services", function () {
    const { state, guildId } = createGuild();
    const linked = linkHarthmereGuildHallPropertyV1({
      state,
      guildId,
      actorId: LEADER,
...
describe("mmo_guild_authority_v1 — disband edge cases", function () {
...
describe("live_mode_backend_v1 — guild integration", function () {
