...
import {
  computeHarthmereAbilityDamage,
  computeHarthmereAbilityHealing,
  computeHarthmereRespecCost,
...
function makeReq(
  ...
  return {
    ...
    nowMs: Date.now(),
    ...overrides,
  } as HarthmereCombatActionRequest;
}
...
describe("computeHarthmereAbilityHealing", () => {
  it("returns zero for a non-healing ability", () => {
    const ability = makeAbility({ baseHealing: 0, spellPowerScaling: 0 });
    const cls = makeClass();
    assert.strictEqual(computeHarthmereAbilityHealing(ability, cls, 10), 0);
  });
  ...
  it("scales healing with spell power", () => {
    ...
    const healL1 = computeHarthmereAbilityHealing(ability, cls, 1);
    const healL10 = computeHarthmereAbilityHealing(ability, cls, 10);
...
describe("Respec", () => {
  it("succeeds on first respec with sufficient gold", () => {
    ...
    const cost = computeHarthmereRespecCost(0);
  ...
  it("escalates respec cost with each respec", () => {
    const cost0 = computeHarthmereRespecCost(0);
    const cost1 = computeHarthmereRespecCost(1);
    const cost5 = computeHarthmereRespecCost(5);
  ...
  it("caps respec cost escalation at respecCount=10", () => {
    const cost10 = computeHarthmereRespecCost(10);
    const cost15 = computeHarthmereRespecCost(15);
  ...
  it("enforces respec cooldown after a recent respec", () => {
    const now = Date.now();
    const cost = computeHarthmereRespecCost(1);
    const ctx = makeCtx({
      respecCount: 1,
