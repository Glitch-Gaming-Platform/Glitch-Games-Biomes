...
import {
  HARTHMERE_BUSINESS_EMPLOYEE_ASSIGNABLE_TASKS_V1,
  HARTHMERE_BUSINESS_EMPLOYEE_AUTOMATION_ROLE_BEHAVIORS_V1,
  createHarthmereBusinessEmployeeTaskFlowV1,
  ...
  normalizeHarthmereBusinessEmployeeTaskIdV1,
  renderHarthmereBusinessEmployeeAiVisualAuditHtmlV1,
  simulateHarthmereBusinessEmployeeTaskRunV1,
  validateHarthmereBusinessEmployeeAiVisualAuditV1,
...
import type { HarthmereEconomyEmployeeRecordV1 } from "../mmo_economy_authority_v1";

const NOW_MS = 1_800_000_000_000;

function employee(overrides: Partial<HarthmereEconomyEmployeeRecordV1> = {}): HarthmereEconomyEmployeeRecordV1 {
  return {
...
describe("business_employee_ai_v1", () => {
  it("creates per-business staff task flows for every service offer", () => {
    for (const [typeId, definition] of Object.entries(HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS_V1)) {
      for (const offer of definition.offers) {
        const flow = createHarthmereBusinessEmployeeTaskFlowV1(typeId as any, offer.offerId);
        ...
        assert.ok(flow.steps.some((step) => step.stepId === "fetch_required_stock"));
        assert.ok(flow.steps.some((step) => step.stepId === "perform_service" && step.animationCue === offer.animationCue));
        assert.ok(flow.physicalActions.length >= 8);
        assert.deepEqual(flow.physicalActions.filter((action) => action.requiredBeforeComplete).length, flow.physicalActions.length);
        assert.ok(flow.physicalActions.some((action) => action.kind === "walk_to_node"));
        assert.ok(flow.physicalActions.some((action) => action.kind === "operate_station" && action.animationCue === offer.animationCue));
        assert.ok(flow.physicalActions.some((action) => action.kind === "serve_customer"));
        assert.ok(flow.physicalActions.some((action) => action.kind === "clean_station"));
      }
    }
  });

  it("uses bespoke physical sub-actions for restaurant service instead of a generic task label", () => {
    const flow = createHarthmereBusinessEmployeeTaskFlowV1("food_service_restaurant", "serve_worker_meal");
    assert.ok(flow.physicalActions.some((action) => action.objectRef === "oven" && action.label.includes("Open the oven")));
    assert.ok(flow.physicalActions.some((action) => action.objectRef === "plating pass" && action.label.includes("Cook, plate")));
    assert.ok(flow.physicalActions.some((action) => action.objectRef === "service plate" && action.label.includes("only after plating is complete")));
    assert.ok(flow.physicalActions.some((action) => action.objectRef === "plating pass" && action.kind === "clean_station"));
  });

  it("validates assignable staff tasks instead of accepting arbitrary strings", () => {
    assert.equal(normalizeHarthmereBusinessEmployeeTaskIdV1("Front Counter"), "front_counter");
    assert.equal(normalizeHarthmereBusinessEmployeeTaskIdV1("kitchen"), "production_station");
    assert.equal(validateHarthmereBusinessEmployeeAssignedTaskV1("cleanup route")?.taskKind, "cleanup_route");
    assert.equal(validateHarthmereBusinessEmployeeAssignedTaskV1("debugNope"), undefined);
    assert.equal(Object.keys(HARTHMERE_BUSINESS_EMPLOYEE_ASSIGNABLE_TASKS_V1).length, 8);
  });
  ...
  it("pathfinds staff and customers through counters, queue nodes, and service stations", () => {
    const run = simulateHarthmereBusinessEmployeeTaskRunV1({
      ...
      employee: employee({ assignedTask: "production_station", role: "Line Cook" }),
  ...
  it("uses procedural service animations for employee actions", () => {
    const run = simulateHarthmereBusinessEmployeeTaskRunV1({
      ...
      employee: employee({ role: "Clinic Aide", assignedTask: "quality_check" }),
      offerId: "urgent_treatment",
      nowMs: NOW_MS,
    });
    const spec = getHarthmereBusinessServiceAnimationCueSpecV1(run.animationCue);
    assert.ok(spec);
  ...
  it("recovers from blocked work objects by repathing or fallback pathing", () => {
    const run = simulateHarthmereBusinessEmployeeTaskRunV1({
      ...
      employee: employee({ role: "Fix-It Apprentice", assignedTask: "production_station" }),
      offerId: "fixture_fix",
  ...
  it("resolves customer plus employee collision with sidestep recovery", () => {
    const run = simulateHarthmereBusinessEmployeeTaskRunV1({
      ...
      businessId: "business_test",
      typeId: "courier",
      employee: employee({ role: "Dispatch Runner", assignedTask: "dispatch_runner" }),
  ...
  it("models candidate market, interview scores, wage negotiation, and withdrawal edge cases", () => {
    const candidates = generateHarthmereBusinessEmployeeCandidatesV1({
      businessId: "business_test",
      typeId: "food_service_restaurant",
      nowMs: NOW_MS,
      count: 3,
    ...
    const interviewed = interviewHarthmereBusinessEmployeeCandidateV1(candidates[0], "friendly");
    assert.equal(interviewed.status, "interviewed");
    assert.ok((interviewed.interviewScore ?? 0) > 0);
    const accepted = negotiateHarthmereBusinessEmployeeCandidateV1(interviewed, interviewed.wageAskGoldPerDay);
    assert.equal(accepted.accepted, true);
    assert.equal(accepted.candidate.status, "offer_made");
    let declined = negotiateHarthmereBusinessEmployeeCandidateV1(interviewed, 1).candidate;
    declined = negotiateHarthmereBusinessEmployeeCandidateV1(declined, 1).candidate;
    const withdrawn = negotiateHarthmereBusinessEmployeeCandidateV1(declined, 1);
  ...
  it("has visible AI behavior definitions for every automation role", () => {
    const roles = Object.keys(HARTHMERE_BUSINESS_EMPLOYEE_AUTOMATION_ROLE_BEHAVIORS_V1) as HarthmereBusinessEmployeeAutomationRoleV1[];
    ...
    for (const role of roles) {
      ...
      assert.equal(run.taskKind, HARTHMERE_BUSINESS_EMPLOYEE_AUTOMATION_ROLE_BEHAVIORS_V1[role].taskKind);
      assert.notEqual(run.status, "failed");
      assert.ok(run.employeePath.length > 0);
    }
  });
  ...
  it("renders and validates the staff movement visual audit", () => {
    const html = renderHarthmereBusinessEmployeeAiVisualAuditHtmlV1();
    const audit = validateHarthmereBusinessEmployeeAiVisualAuditV1();
    assert.equal(audit.ok, true, audit.warnings.slice(0, 10).join(", "));
    assert.equal(audit.businessCount, Object.keys(HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS_V1).length);
