...
import {
  ...
  HARTHMERE_NPC_MIN_RENDERABLE_VERTICES,
} from "../npc_visible_geometry_guard";
...
describe("harthmere npc visible geometry guard", () => {
  it("flags an empty scene as needing a visible fallback", () => {
    ...
    assert.equal(harthmereNpcSceneNeedsVisibleFallback(scene), true);
  ...
  it("flags a nullish scene as needing a visible fallback", () => {
    assert.equal(harthmereNpcSceneNeedsVisibleFallback(undefined), true);
    assert.equal(harthmereNpcSceneNeedsVisibleFallback(null), true);
  });

  it("does not require a fallback for a scene with real renderable geometry", () => {
    const scene = new THREE.Group();
    scene.add(meshWithVertices(36));
    assert.equal(harthmereNpcSceneNeedsVisibleFallback(scene), false);
    const stats = harthmereNpcVisibleGeometryStatsForScene(scene);
  ...
  it("requires a fallback when geometry is below the minimum vertex threshold", () => {
    const scene = new THREE.Group();
    scene.add(meshWithVertices(HARTHMERE_NPC_MIN_RENDERABLE_VERTICES - 1));
    assert.equal(harthmereNpcSceneNeedsVisibleFallback(scene), true);
  });
});
