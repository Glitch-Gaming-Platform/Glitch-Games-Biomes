...
import { harthmereMuckCreatureAssetKeyForLabel } from "../muck_creature_assets";
...
describe("Harthmere live entity AI audit matrix current", () => {
  ...
  it("keeps every production Mucker, Hexer, and robot sentinel in valid authored locations with renderable creature assets", () => {
    const errors = validateHarthmereLiveEntityProductionSeeds();
  ...
  it("selects server-validated AI intents for every combat archetype and blocks Muck aggression outside safe rules", () => {
    ...
    for (const archetypeId of Object.keys(HARTHMERE_COMBAT_AI_ARCHETYPES)) {
    ...
    const muckSeed = HARTHMERE_LIVE_ENTITY_MUCK_MONSTER_SEEDS[0];
    const aggro = evaluateMuckMonsterAggression({
    ...
    const townBlocked = evaluateMuckMonsterAggression({
  ...
  it("uses the same navigation guardrails for town NPCs, animals, and combat monsters so they do not float or bury", () => {
    ...
    for (const entry of cases) {
      const state = createHarthmereNpcNavigationState();
      const result = resolveHarthmereNpcNavigationStep({
        mode: entry.mode,
