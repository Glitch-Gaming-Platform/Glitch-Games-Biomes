...
import { HARTHMERE_BUSINESS_ABILITY_DEFINITIONS } from "../mmo_class_ability_collectibles";
import {
...
function addBusiness(state: HarthmereLiveModeBackendState, typeId: HarthmereEconomyBusinessTypeId) {
...
describe("live_mode progression backend", () => {
  ...
  it("rejects business abilities until the player owns the matching business", () => {
    const ability = Object.values(HARTHMERE_BUSINESS_ABILITY_DEFINITIONS)
    ...
    const learned = reduceHarthmereLiveModeBackendState(
      rejected.state,
      envelope("request_trainer_unlock", "trainer", { abilityId: ability.id }),
      NOW_MS,
    );
  ...
  it("rejects unknown classes, abilities, and collectibles", () => {
    const state = defaultHarthmereLiveModeBackendState(ACTOR, NOW_MS);
    const badClass = reduceHarthmereLiveModeBackendState(
      state,
    ...
    const badAbility = reduceHarthmereLiveModeBackendState(
      state,
    ...
    const badCollectible = reduceHarthmereLiveModeBackendState(
      state,
  ...
  it("discovers collectibles through the quest update pipeline", () => {
    ...
    const snapshot = createHarthmereProgressionClientSnapshotFromBackend(reduced.state);
  ...
  it("exposes live-mode quest state with progression snapshots", () => {
    ...
    const snapshot = createHarthmereProgressionClientSnapshotFromBackend(
      state
    ) as any;
