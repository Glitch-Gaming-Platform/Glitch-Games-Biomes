...
function homeConsolePosition() {
  ...
  const marker = createBuildingSystemHomeConsoleMarker({
    property: prop,
    plot: buildingSystemPlotById(prop.plotId)!,
    blueprint: buildingSystemBlueprintById(prop.blueprintId)!,
    nowMs: NOW,
  });
  return {
    x: marker.position[0],
...
describe("Harthmere live-mode home decoration backend", () => {
  ...
  it("places crafted stations as functional home workshop decorations", () => {
    ...
    state.inventory.items[HARTHMERE_CRAFTING_STATIONS.workbench] = 1;
  ...
  it("replays duplicate decoration requests without minting or consuming twice", () => {
    ...
    const first = reduceHarthmereLiveModeBackendState(state, request, NOW);
    const replay = reduceHarthmereLiveModeBackendState(
      first.state,
      request,
      NOW + 1
    );
  ...
  it("rejects client home decoration actions away from the home console", () => {
    ...
    const unverified = reduceHarthmereLiveModeBackendState(
      state,
    ...
    const tooFar = reduceHarthmereLiveModeBackendState(
      state,
