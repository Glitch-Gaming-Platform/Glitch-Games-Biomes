...
import {
  ...
  HARTHMERE_COOK_DURATION_MAX_MS_V1,
  HARTHMERE_COOK_DURATION_MIN_MS_V1,
  HARTHMERE_COOK_QUEUE_CAP_V1,
  HARTHMERE_COOK_SPOIL_MS_V1,
  HARTHMERE_COOK_STATION_JOBS_MAX_V1,
...
function baseState(
  ...
  return {
    ...defaultHarthmereFoodStaminaStateV1("player_cook", NOW),
    inventory: { ...inventory },
    cooking: {},
  };
}

const GRILLED_DURATION = scaleHarthmereCookDurationMsV1(
...
describe("harthmere cooking — duration scaler", () => {
  it("clamps every recipe into the 20s–120s band", () => {
    for (const recipe of Object.values(HARTHMERE_COOKING_RECIPES_V1)) {
      const ms = scaleHarthmereCookDurationMsV1(recipe.cookTimeMs, 1);
      assert.ok(
        ms >= HARTHMERE_COOK_DURATION_MIN_MS_V1 &&
          ms <= HARTHMERE_COOK_DURATION_MAX_MS_V1,
  ...
  it("maps the corpus min to 20s and max to 120s, and clamps beyond", () => {
    const times = Object.values(HARTHMERE_COOKING_RECIPES_V1).map(
      (r) => r.cookTimeMs,
    );
    const min = Math.min(...times);
    const max = Math.max(...times);
    assert.equal(
      scaleHarthmereCookDurationMsV1(min, 1),
      HARTHMERE_COOK_DURATION_MIN_MS_V1,
    );
    assert.equal(
      scaleHarthmereCookDurationMsV1(max, 1),
      HARTHMERE_COOK_DURATION_MAX_MS_V1,
    );
    assert.equal(
      scaleHarthmereCookDurationMsV1(min - 9999, 1),
      HARTHMERE_COOK_DURATION_MIN_MS_V1,
    );
    assert.equal(
      scaleHarthmereCookDurationMsV1(max + 9999, 1),
      HARTHMERE_COOK_DURATION_MAX_MS_V1,
    );
  });
  ...
  it("multiplies by batch count and is monotonic in cook time", () => {
    assert.equal(
      scaleHarthmereCookDurationMsV1(60_000, 3),
      scaleHarthmereCookDurationMsV1(60_000, 1) * 3,
    );
    assert.ok(
      scaleHarthmereCookDurationMsV1(30_000, 1) <=
        scaleHarthmereCookDurationMsV1(90_000, 1),
    );
  });
});
...
describe("harthmere cooking — enqueue", () => {
  it("reserves ingredients and starts the first job cooking", () => {
    const result = enqueueHarthmereCookV1(baseState({ raw_meat: 3 }), {
      stationId: "s1",
  ...
  it("rejects when ingredients are missing (incl. already-reserved)", () => {
    const empty = enqueueHarthmereCookV1(baseState({}), {
      stationId: "s1",
      recipeId: "grilled_meat",
      nowMs: NOW,
    });
    ...
    const noVeg = enqueueHarthmereCookV1(baseState({}), {
      stationId: "s1",
      stationKind: "cookpot",
    ...
    const first = enqueueHarthmereCookV1(baseState({ raw_meat: 1 }), {
      stationId: "s1",
      recipeId: "grilled_meat",
      nowMs: NOW,
    });
    const second = enqueueHarthmereCookV1(first.state, {
      stationId: "s1",
      recipeId: "grilled_meat",
      nowMs: NOW + 1,
    });
  ...
  it("rejects over-batch and station-kind mismatch", () => {
    const tooBig = enqueueHarthmereCookV1(baseState({ raw_meat: 99 }), {
      stationId: "s1",
      recipeId: "grilled_meat",
    ...
    const wrongStation = enqueueHarthmereCookV1(
  ...
  it("rejects once the station queue is full", () => {
    ...
    for (let i = 0; i < HARTHMERE_COOK_QUEUE_CAP_V1; i++) {
      const r = enqueueHarthmereCookV1(state, {
        stationId: "s1",
        recipeId: "grilled_meat",
        nowMs: NOW + i,
      });
    ...
    const overflow = enqueueHarthmereCookV1(state, {
      stationId: "s1",
      recipeId: "grilled_meat",
      nowMs: NOW + 100,
    });
  ...
  it("caps total jobs per station so uncollected dishes can't grow unbounded", () => {
    ...
    let state = baseState({ raw_meat: HARTHMERE_COOK_STATION_JOBS_MAX_V1 + 1 });
    for (let i = 0; i < HARTHMERE_COOK_STATION_JOBS_MAX_V1; i++) {
      const r = enqueueHarthmereCookV1(state, {
        stationId: "s1",
        recipeId: "grilled_meat",
        nowMs: NOW + i * step,
    ...
    const overflow = enqueueHarthmereCookV1(state, {
      stationId: "s1",
      recipeId: "grilled_meat",
      nowMs: NOW + HARTHMERE_COOK_STATION_JOBS_MAX_V1 * step,
    });
    assert.ok(overflow.warnings.includes("cooking_rejected:queue_full"));
  });

  it("chains a second job to start when the first finishes", () => {
    const first = enqueueHarthmereCookV1(baseState({ raw_meat: 2 }), {
      stationId: "s1",
      recipeId: "grilled_meat",
      nowMs: NOW,
    });
...
describe("harthmere cooking — tick", () => {
  it("promotes pending → cooking → ready and is reload-deterministic", () => {
    ...
    const enqueued = enqueueHarthmereCookV1(first.state, {
      stationId: "s1",
      recipeId: "grilled_meat",
      nowMs: NOW + 1,
    }).state.cooking;

    const midway = tickHarthmereCookingV1(enqueued, NOW + GRILLED_DURATION + 1);
    assert.equal(midway.s1.jobs[0].status, "ready");
    assert.equal(midway.s1.jobs[1].status, "cooking");

    const done = tickHarthmereCookingV1(enqueued, NOW + 2 * GRILLED_DURATION + 1);
    ...
    const afterReload = tickHarthmereCookingV1(
      reloaded,
  ...
  it("spoils an uncollected dish after the spoil window and prunes the station", () => {
    const cooking = enqueueHarthmereCookV1(baseState({ raw_meat: 1 }), {
      stationId: "s1",
      recipeId: "grilled_meat",
      nowMs: NOW,
    }).state.cooking;
  ...
  it("auto-cleans an orphaned station once every queued dish finishes and spoils", () => {
    ...
    state = enqueueHarthmereCookV1(state, {
      stationId: "s1",
      recipeId: "grilled_meat",
      nowMs: NOW,
    }).state;
    ...
    const wayLater = tickHarthmereCookingV1(
      state.cooking,
      NOW + 2 * GRILLED_DURATION + HARTHMERE_COOK_SPOIL_MS_V1 + 1,
    );
...
describe("harthmere cooking — collect", () => {
  it("rejects collecting before the job is ready", () => {
    const enq = enqueueHarthmereCookV1(baseState({ raw_meat: 1 }), {
      stationId: "s1",
      recipeId: "grilled_meat",
      nowMs: NOW,
    });
    const jobId = enq.state.cooking.s1.jobs[0].jobId;
    const early = collectHarthmereCookV1(enq.state, {
      stationId: "s1",
      jobId,
      nowMs: NOW + 1,
    });
  ...
  it("adds outputs, awards xp, and prunes the emptied station", () => {
    ...
    const jobId = enq.state.cooking.s1.jobs[0].jobId;
    const collected = collectHarthmereCookV1(enq.state, {
      stationId: "s1",
      jobId,
  ...
  it("blocks collection that would exceed the carry-weight limit", () => {
    ...
    const enq = enqueueHarthmereCookV1(
      baseState({ steel_sword: 5, raw_meat: 1 }),
      { stationId: "s1", recipeId: "grilled_meat", nowMs: NOW },
    );
    const jobId = enq.state.cooking.s1.jobs[0].jobId;
    const blocked = collectHarthmereCookV1(enq.state, {
      stationId: "s1",
      jobId,
      nowMs: NOW + GRILLED_DURATION,
    });
...
describe("harthmere cooking — cancel", () => {
  it("refunds a pending job and keeps the active one", () => {
    ...
    const cancelled = cancelHarthmereCookV1(second.state, {
      stationId: "s1",
      jobId: job2Id,
      nowMs: NOW + 2,
    });
  ...
  it("shifts later pending jobs earlier when a middle pending job is cancelled", () => {
    ...
    const cancelled = cancelHarthmereCookV1(state, {
      stationId: "s1",
      jobId: job2.jobId,
      nowMs: NOW + 2,
    });
  ...
  it("discards a ready dish without refunding (escape hatch), and prunes the station", () => {
    ...
    const discarded = cancelHarthmereCookV1(enq.state, {
      stationId: "s1",
      jobId,
      nowMs: NOW + GRILLED_DURATION,
    });
  ...
  it("cancelling a not-yet-ready job refunds and prunes", () => {
    ...
    const jobId = enq.state.cooking.s1.jobs[0].jobId;
    const pruned = cancelHarthmereCookV1(enq.state, {
      stationId: "s1",
      jobId,
      nowMs: NOW + 1,
    });
