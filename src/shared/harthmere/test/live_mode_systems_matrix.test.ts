...
import { HARTHMERE_AUCTION_LISTING_FEE_BASE_GOLD } from "../mmo_auction_authority";
import {
...
function registerMatrixItem(overrides: Partial<HarthmereItemDefinition>) {
  registerHarthmereItemDefinition({
...
describe("Harthmere live-mode MMO systems matrix", function () {
  ...
  it("covers guild creation and client snapshot boundaries", function () {
    ...
    const snapshot = createHarthmereLiveModeGuildClientSnapshot(
      next.guild,
      ACTOR
    );
  ...
  it("covers law and likeability state in the player status snapshot", function () {
    ...
    const status = createHarthmereLiveModePlayerStatusClientSnapshot(next);
