...
import {
  collectExpiredHarthmereAuctionListings,
  ...
  previewHarthmereAuctionFees,
  reduceHarthmereAuctionMutation,
  searchHarthmereAuctionListings,
  type HarthmereAuctionListing,
  type HarthmereAuctionMutationContext,
...
function makeSnap(overrides: Partial<HarthmereInventorySnapshot> = {}): HarthmereInventorySnapshot {
  return {
    actorId: "seller_1",
    gold: 1000,
    equipment: {},
...
function activeListing(overrides: Partial<HarthmereAuctionListing> = {}): HarthmereAuctionListing {
  return {
    listingId: "listing_1",
    sellerId: "seller_1",
    itemId: "iron_sword",
...
function makePostReq(overrides: Partial<HarthmereAuctionMutationRequest> = {}): HarthmereAuctionMutationRequest {
  return {
    requestId: "req_1",
    actorId: "seller_1",
    kind: "post_listing",
    ...
    nowMs: NOW,
    ...overrides,
  } as HarthmereAuctionMutationRequest;
}
...
describe("Auction post_listing", () => {
  it("succeeds and charges listing fee from seller gold", () => {
    ...
    const ctx: HarthmereAuctionMutationContext = { actorSnapshot: snap };
    const req = makePostReq();
    const result = reduceHarthmereAuctionMutation(req, ctx);
...
describe("Auction cancel_listing", () => {
  it("succeeds and returns escrow to inventory", () => {
    ...
    const ctx: HarthmereAuctionMutationContext = { actorSnapshot: snap, currentListing: listing };
    const req = { requestId: "r", actorId: "seller_1", kind: "cancel_listing", listingId: "listing_1", nowMs: NOW } as HarthmereAuctionMutationRequest;
    const result = reduceHarthmereAuctionMutation(req, ctx);
    assert.ok(result.ok, result.errors?.join(", "));
  ...
  it("fails if listing does not exist", () => {
    const snap = makeSnap();
    const ctx: HarthmereAuctionMutationContext = { actorSnapshot: snap, currentListing: undefined };
    const req = { requestId: "r", actorId: "seller_1", kind: "cancel_listing", listingId: "listing_1", nowMs: NOW } as HarthmereAuctionMutationRequest;
    const result = reduceHarthmereAuctionMutation(req, ctx);
    assert.ok(!result.ok);
  });
  ...
  it("fails if canceller is not the seller", () => {
    ...
    const listing = activeListing();
    const ctx: HarthmereAuctionMutationContext = { actorSnapshot: snap, currentListing: listing };
    const req = { requestId: "r", actorId: "other_player", kind: "cancel_listing", listingId: "listing_1", nowMs: NOW } as HarthmereAuctionMutationRequest;
    const result = reduceHarthmereAuctionMutation(req, ctx);
    assert.ok(!result.ok);
  });
...
describe("Auction buy_listing", () => {
  it("atomic swap: buyer gains item, seller gains gold minus tax", () => {
    ...
    const ctx: HarthmereAuctionMutationContext = {
      actorSnapshot: buyerSnap,
      buyerSnapshot: buyerSnap,
      currentListing: listing,
      buyerInventorySlots: 0,
    };
    const req = { requestId: "r", actorId: "buyer_1", kind: "buy_listing", listingId: "listing_1", nowMs: NOW } as HarthmereAuctionMutationRequest;
    const result = reduceHarthmereAuctionMutation(req, ctx);
    assert.ok(result.ok, result.errors?.join(", "));
  ...
  it("fails when buyer tries to purchase their own listing", () => {
    ...
    const req = { requestId: "r", actorId: "seller_1", kind: "buy_listing", listingId: "listing_1", nowMs: NOW } as HarthmereAuctionMutationRequest;
    const result = reduceHarthmereAuctionMutation(req, ctx);
    assert.ok(!result.ok);
...
describe("Auction expiry and recovery", () => {
  it("expire_listing transitions active listing to expired", () => {
    ...
    const ctx: HarthmereAuctionMutationContext = { actorSnapshot: snap, currentListing: listing };
    const req = { requestId: "r", actorId: "system", kind: "expire_listing", listingId: "listing_1", nowMs: NOW } as HarthmereAuctionMutationRequest;
    const result = reduceHarthmereAuctionMutation(req, ctx);
    assert.ok(result.ok, result.errors?.join(", "));
  ...
  it("recover_expired_escrow returns item to seller", () => {
    ...
    const ctx: HarthmereAuctionMutationContext = { actorSnapshot: snap, currentListing: listing };
    const req = { requestId: "r", actorId: "seller_1", kind: "recover_expired_escrow", listingId: "listing_1", nowMs: NOW } as HarthmereAuctionMutationRequest;
    const result = reduceHarthmereAuctionMutation(req, ctx);
    assert.ok(result.ok, result.errors?.join(", "));
  ...
  it("collectExpiredHarthmereAuctionListings returns only active+expired listings", () => {
    const listings: HarthmereAuctionListing[] = [
    ...
    const expired = collectExpiredHarthmereAuctionListings(listings, NOW);
...
describe("searchHarthmereAuctionListings", () => {
  const listings: HarthmereAuctionListing[] = [
  ...
  it("filters by itemId", () => {
    const res = searchHarthmereAuctionListings(listings, { itemId: "health_potion" }, NOW);
  ...
  it("activeOnly excludes sold and expired listings", () => {
    const res = searchHarthmereAuctionListings(listings, { activeOnly: true }, NOW);
    assert.ok(res.listings.every(r => r.status === "active"));
    assert.ok(!res.listings.some(r => r.listingId === "a4" || r.listingId === "a5"));
  });

  it("filters by price range", () => {
    const res = searchHarthmereAuctionListings(listings, { minUnitPrice: 100, maxUnitPrice: 200, activeOnly: true }, NOW);
    assert.ok(res.listings.every(r => r.unitPrice >= 100 && r.unitPrice <= 200));
  });

  it("filters by seller", () => {
    const res = searchHarthmereAuctionListings(listings, { sellerId: "seller_2" }, NOW);
    assert.ok(res.listings.every(r => r.sellerId === "seller_2"));
  });

  it("paginates results with offset and limit", () => {
    const allActive = searchHarthmereAuctionListings(listings, { activeOnly: true }, NOW);
    const page0 = searchHarthmereAuctionListings(listings, { activeOnly: true, offset: 0, limit: 1 }, NOW);
    const page1 = searchHarthmereAuctionListings(listings, { activeOnly: true, offset: 1, limit: 1 }, NOW);
  ...
  it("returns empty results for no match", () => {
    const res = searchHarthmereAuctionListings(listings, { itemId: "legendary_armor" }, NOW);
...
describe("previewHarthmereAuctionFees", () => {
  it("returns listing fee and estimated tax", () => {
    const preview = previewHarthmereAuctionFees(1000, 1);
  ...
  it("multi-count listing scales fees correctly", () => {
    const single = previewHarthmereAuctionFees(100, 1);
    const multi = previewHarthmereAuctionFees(100, 5);
  ...
  it("conserves items across the post -> expire -> recover lifecycle (no dupe)", () => {
    ...
    const posted = reduceHarthmereAuctionMutation(makePostReq(), { actorSnapshot: seller });
    assert.ok(posted.ok, posted.errors?.join(","));
    const listing = posted.listing!;
    const expiredInput = { ...listing, expiresAtMs: NOW - 1000 };
    const expired = reduceHarthmereAuctionMutation(
      { requestId: "r2", actorId: "system", kind: "expire_listing", listingId: listing.listingId, nowMs: NOW } as HarthmereAuctionMutationRequest,
      { actorSnapshot: seller, currentListing: expiredInput },
    );
    assert.ok(expired.ok, expired.errors?.join(","));
    const recovered = reduceHarthmereAuctionMutation(
      { requestId: "r3", actorId: "seller_1", kind: "recover_expired_escrow", listingId: listing.listingId, nowMs: NOW } as HarthmereAuctionMutationRequest,
