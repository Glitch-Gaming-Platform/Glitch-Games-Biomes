...
function lootItem(
  overrides: Partial<HarthmereInventoryLootItemDefinitionV1> = {}
...
describe("inventory system full stack audit", () => {
  it("validates every core backend mutation path and rejects protected item mistakes", () => {
    ...
    current = applyHarthmereInventoryMutationResultV1(current, buy);
    ...
    current = applyHarthmereInventoryMutationResultV1(current, equip);
    ...
    current = applyHarthmereInventoryMutationResultV1(current, unequip);
    assert.equal(current.items.audit_sword, 1);
    ...
    current = applyHarthmereInventoryMutationResultV1(current, craft);
    ...
    current = applyHarthmereInventoryMutationResultV1(current, bank);
    ...
    current = applyHarthmereInventoryMutationResultV1(current, withdraw);
    assert.equal(current.items.audit_ingot, 1);
  ...
  it("persists live reducer grants, admin authority checks, loot claims, overflow, and client inventory snapshots", () => {
    ...
    const clientSnapshot =
      require("../live_mode_backend_v1").createHarthmereInventoryLootClientSnapshotFromBackendV1(
        current
      );
    ...
    let lootState = createHarthmereEmptyInventoryLootStateV1();
    lootState.actors[ACTOR] = createHarthmereInventoryLootActorV1(ACTOR);
    const created = reduceHarthmereInventoryLootMutationV1(
      lootState,
      {
    ...
    const claimed = reduceHarthmereInventoryLootMutationV1(
      lootState,
      {
