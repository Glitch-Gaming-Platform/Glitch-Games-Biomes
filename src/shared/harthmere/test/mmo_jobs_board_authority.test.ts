...
function context(overrides: Partial<HarthmereJobsBoardMutationContext> = {}): HarthmereJobsBoardMutationContext {
  return {
    actorGold: 1000,
...
describe("mmo_jobs_board_authority — board location and empty state", () => {
  it("starts with the Grove jobs board but no dummy runtime jobs", () => {
    ...
    assert.equal(board.location.radius, HARTHMERE_JOBS_BOARD_INTERACTION_RADIUS);
  ...
  it("requires physical board interaction by marker or position", () => {
    ...
    assert.equal(isActorAtHarthmereJobsBoard(state, { actorPosition: { x: 501.99486179104775 + HARTHMERE_JOBS_BOARD_INTERACTION_RADIUS + 0.1, y: 70, z: -132.00350672753194 } }), false);
    assert.equal(isActorAtHarthmereJobsBoard(state, { actorPosition: { x: 900, y: 66, z: 900 } }), false);
  });
});
...
describe("mmo_jobs_board_authority — posting, accepting, quest todos, and completion", () => {
  ...
  it("rejects self-acceptance, double-acceptance, missing turn-in items, and late jobs", () => {
    const posted = mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", postPayload(), {}, "poster");
    const jobId = Object.keys(posted.jobsBoard.postings)[0];
...
describe("mmo_jobs_board_authority — issuers and abuse protections", () => {
  ...
  it("allows an authorized business to post jobs and escrows from business funds", () => {
    ...
    const result = mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", postPayload({ issuerKind: "business", businessId: "business_1" }), {
      economy,
      actorGold: 0,
  ...
  it("rejects invalid or unavailable escrow reward items and refunds open item rewards on cancel", () => {
    const unavailable = mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", postPayload({
      rewardItems: [{ itemId: "road_ration", count: 2 }],
    ...
    const invalid = mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", postPayload({
    ...
    const posted = mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", postPayload({
      rewardItems: [{ itemId: "road_ration", count: 2 }],
  ...
  it("covers every PDF business type with an executable jobs-board template", () => {
    ...
    const templateTypes = Array.from(new Set(HARTHMERE_JOBS_BOARD_BUSINESS_TEMPLATES.map((template) => template.businessType))).sort();
    ...
    for (const template of HARTHMERE_JOBS_BOARD_BUSINESS_TEMPLATES) {
      ...
      for (const req of template.requirements) {
        if (req.itemId) assert.ok(isKnownHarthmereJobsBoardExecutableItemId(req.itemId), `${template.templateId}:${req.itemId}`);
  ...
  it("lets each business template create an executable business-backed job when the business owns escrow", () => {
    for (const template of HARTHMERE_JOBS_BOARD_BUSINESS_TEMPLATES) {
      ...
      const result = mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", {
  ...
  it("rejects unauthorized business/guild/town/npc posting", () => {
    const economy = economyWithBusiness();
    assert.ok(mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", postPayload({ issuerKind: "business", businessId: "business_1" }), { economy, canManageBusinessJobs: () => false }, "not_owner").warnings.includes("jobs_board_rejected:business_job_permission_required"));
    assert.ok(mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", postPayload({ issuerKind: "guild", issuerId: "guild_1" }), { canManageGuildJobs: () => false }, "officer").warnings.includes("jobs_board_rejected:guild_job_permission_required"));
    assert.ok(mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", postPayload({ issuerKind: "town", issuerId: "harthmere_grove" }), { canManageTownJobs: () => false }, "clerk").warnings.includes("jobs_board_rejected:town_job_permission_required"));
    assert.ok(mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", postPayload({ issuerKind: "npc", issuerId: "npc_1" }), { allowNpcJobPosting: false }, "npc_admin").warnings.includes("jobs_board_rejected:npc_job_permission_required"));
  });
  ...
  it("enforces reward caps, cooldowns, active seeker limits, issuer limits, and suspicious text flags", () => {
    ...
    const first = reduceHarthmereJobsBoardMutation(state, { requestId: "first", actorId: "poster", nowMs: NOW, operation: "create_job_posting", boardId: HARTHMERE_JOBS_BOARD_DEFAULT_BOARD_ID, ...postPayload({ title: "Free gold exploit http://bad.example" }) } as any, context());
    const job = Object.values(first.jobsBoard.postings)[0];
    assert.deepEqual(job.abuseFlags, ["suspicious_text"]);
    const cooldown = reduceHarthmereJobsBoardMutation(first.jobsBoard, { requestId: "second", actorId: "poster", nowMs: NOW + 1, operation: "create_job_posting", boardId: HARTHMERE_JOBS_BOARD_DEFAULT_BOARD_ID, ...postPayload({ title: "Second" }) } as any, context());
    assert.ok(cooldown.warnings.includes("jobs_board_rejected:post_cooldown"));

    let limited = defaultHarthmereJobsBoardState(NOW);
    for (let i = 0; i < 12; i++) {
      const r = reduceHarthmereJobsBoardMutation(limited, { requestId: `p${i}`, actorId: "issuer", nowMs: NOW + i * 20_000, operation: "create_job_posting", boardId: HARTHMERE_JOBS_BOARD_DEFAULT_BOARD_ID, ...postPayload({ title: `Job ${i}` }) } as any, context({ actorGold: 100000 }));
      limited = r.jobsBoard;
    }
    assert.ok(reduceHarthmereJobsBoardMutation(limited, { requestId: "too_many", actorId: "issuer", nowMs: NOW + 999_999, operation: "create_job_posting", boardId: HARTHMERE_JOBS_BOARD_DEFAULT_BOARD_ID, ...postPayload({ title: "too many" }) } as any, context({ actorGold: 100000 })).warnings.includes("jobs_board_rejected:issuer_posting_limit"));
  });
  ...
  it("cancels open jobs with escrow refund and marks active cancellations as failed to prevent bait-and-switch abuse", () => {
    ...
    const posted2 = mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", postPayload(), {}, "poster2");
...
describe("mmo_jobs_board_authority — repair-tool completion gate (HARTHMERE_REPAIR_TOOL_COMPLETION)", () => {
  ...
  it("blocks completing a repair job when the repair tool was not used", () => {
    const posted = mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", repairPostPayload(), {}, "poster");
    const jobId = Object.keys(posted.jobsBoard.postings)[0];
    const accepted = mutate(posted.jobsBoard, "accept_job", { jobId }, {}, "seeker");
    const result = mutate(
      accepted.jobsBoard,
...
describe("mmo_jobs_board_authority — accept timer + failure (HARTHMERE_JOB_ACCEPT_TIMER)", () => {
  ...
  it("starts escort accepted timers in the 2-5 hour range and creates a companion", () => {
    const acceptNow = NOW;
    const posted = mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", escortPostPayload(), {}, "poster");
    const jobId = Object.keys(posted.jobsBoard.postings)[0];
  ...
  it("time-remaining label formats hours/minutes and Expired", () => {
    assert.equal(formatHarthmereJobTimeRemaining(1000 + 3 * 60 * 60 * 1000 + 12 * 60 * 1000, 1000), "3h 12m left");
    assert.equal(formatHarthmereJobTimeRemaining(1000 + 9 * 60 * 1000, 1000), "9m left");
...
describe("mmo_jobs_board_authority — delivery (HARTHMERE_DELIVERY)", () => {
  describe("harthmereDeliveryPlan", () => {
    it("returns undefined for non-delivery jobs", () => {
      assert.equal(
        harthmereDeliveryPlan({ kind: "repair", requirements: [{ itemId: "x" }] }),
        undefined
      );
    });
  ...
  it("blocks completing a person-delivery that was not handed to the recipient", () => {
    const posted = mutate(defaultHarthmereJobsBoardState(NOW), "create_job_posting", deliveryPostPayload(), {}, "poster");
    const jobId = Object.keys(posted.jobsBoard.postings)[0];
    const accepted = mutate(posted.jobsBoard, "accept_job", { jobId }, {}, "seeker");
    const result = mutate(
      accepted.jobsBoard,
