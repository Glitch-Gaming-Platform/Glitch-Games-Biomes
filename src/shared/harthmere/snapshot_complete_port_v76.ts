...
export const SNAPSHOT_COMPLETE_PORT_VERSION_V76 = "snapshot-complete-port-v76";
export const SNAPSHOT_CANONICAL_CHALLENGE_EXTRACTION_VERSION_V76 =
  "snapshot-canonical-nux-challenge-extraction-v76";
export const SNAPSHOT_SERVER_COMPLETION_STATE_VERSION_V76 =
  "snapshot-server-completion-state-contract-v76";
export const SNAPSHOT_MISSION_TEST_MATRIX_VERSION_V76 =
  "snapshot-mission-test-matrix-v76";
export const SNAPSHOT_GROVE_FOOT_CLEARANCE_AUDIT_VERSION_V76 =
  "snapshot-grove-foot-clearance-audit-v76";
export const SNAPSHOT_HARTHMERE_BIBLE_NPC_UPGRADE_VERSION_V76 =
  "snapshot-harthmere-bible-npc-upgrade-v76";

export const SNAPSHOT_GROVE_MAX_FEET_CLEARANCE_V76 = 0.25;
export const SNAPSHOT_GROVE_EXPECTED_FEET_Y_V76 = SNAPSHOT_GROVE_NPC_FEET_Y_V75;
export const SNAPSHOT_GROVE_EXPECTED_VISUAL_GROUND_Y_V76 = SNAPSHOT_GROVE_WORLD_GROUND_Y_V75;

export type SnapshotCanonicalMissionTriggerV76 =
  | "talk_npc"
  | "near_location"
  | "destroy"
  | "place_voxel"
...
export interface SnapshotCanonicalChallengeV76 {
  id: string;
  ...
  trigger: SnapshotCanonicalMissionTriggerV76;
  markerId: string;
...
export interface SnapshotStructuredRewardV76 {
  id: string;
  questId: string;
  xp: number;
  bling: number;
...
export interface SnapshotMissionTestCaseV76 {
  id: string;
  questId: string;
  title: string;
  stepIndex: number;
...
export interface SnapshotFootClearanceRecordV76 {
  npcId: string;
  displayName: string;
  entityId: BiomesId;
...
export interface SnapshotHarthmereNpcBibleProfileV76 {
  offset: number;
  id: string;
  displayName: string;
  district: string;
...
export const SNAPSHOT_OFFICIAL_NUX_CHALLENGES_V76: readonly SnapshotCanonicalChallengeV76[] = [
  {
...
function structuredCodexForQuestV76(quest: SnapshotGroveQuestV75): string[] {
  const codex: string[] = [];
...
export const SNAPSHOT_STRUCTURED_REWARDS_V76: readonly SnapshotStructuredRewardV76[] =
  SNAPSHOT_GROVE_QUESTS_V75.map((quest) => ({
    id: `reward_${quest.id}`,
    questId: quest.id,
    xp: questRewardXpV76(quest),
    bling: questRewardBlingV76(quest),
    items: structuredItemsForQuestV76(quest),
    recipes: quest.id === "toll_ledger_problem" ? ["recipe_road_repair_kit"] : [],
    codex: structuredCodexForQuestV76(quest),
...
export const SNAPSHOT_FISHING_WATER_CAMERA_SYSTEMS_V76 = {
  version: "snapshot-fishing-water-camera-runtime-v76",
  shutterCove: {
    ...
    cameraFallback: {
      ...
      localProofKey: "biomes.localDev.snapshotPhotoProofs.v76",
      backendFallbackLabel: "saved locally when social post backend is unavailable",
    },
  },
} as const;

export const SNAPSHOT_MUCK_PERSISTENCE_V76 = {
  version: "snapshot-persistent-muck-cleanup-v76",
  localStateKey: "biomes.localDev.snapshotClearedMuck.v76",
...
export const SNAPSHOT_AUDIO_CUES_V76 = {
...
export const SNAPSHOT_HARTHMERE_NPC_BIBLE_PROFILES_V76: readonly SnapshotHarthmereNpcBibleProfileV76[] = [
  {
    offset: 27,
...
export function snapshotHarthmereBibleProfileByOffsetV76(offset: number) {
  return SNAPSHOT_HARTHMERE_NPC_BIBLE_PROFILES_V76.find(
    (profile) => profile.offset === offset,
  );
}

export function snapshotHarthmereBibleLinesV76(offset: number): string[] {
  const profile = snapshotHarthmereBibleProfileByOffsetV76(offset);
  if (!profile) {
    return [];
  }
  return [
...
function triggerForGroveQuestV76(
  quest: SnapshotGroveQuestV75,
  stepIndex: number,
): SnapshotCanonicalMissionTriggerV76 {
...
function fallbackMarkerForQuestStepV76(quest: SnapshotGroveQuestV75, stepIndex: number) {
  const markerId = quest.markerIds[Math.min(stepIndex, quest.markerIds.length - 1)] ?? quest.markerIds[0] ?? "the_grove";
  return snapshotGroveLandmarkByIdV75(markerId) ?? SNAPSHOT_GROVE_LANDMARKS_V75[0];
}

export function snapshotMissionTestCasesV76(): SnapshotMissionTestCaseV76[] {
  const roadAhead = SNAPSHOT_OFFICIAL_NUX_CHALLENGES_V76.map((challenge, index): SnapshotMissionTestCaseV76 => {
    const marker = snapshotGroveLandmarkByIdV75(challenge.markerId) ?? SNAPSHOT_GROVE_LANDMARKS_V75[0];
    return {
      ...
      expectedStateAfter: index + 1 >= SNAPSHOT_OFFICIAL_NUX_CHALLENGES_V76.length ? "complete" : "active",
      expectedRewardIds: [...challenge.rewardIds],
      expectedInventoryItems: [...challenge.itemGrantIds],
      expectedAudioCue: challenge.audioCueIds[challenge.audioCueIds.length - 1] ?? SNAPSHOT_AUDIO_CUES_V76.stepComplete,
    };
  });

  const grove = SNAPSHOT_GROVE_QUESTS_V75.flatMap((quest) =>
    quest.objectives.map((objective, stepIndex): SnapshotMissionTestCaseV76 => {
      const marker = fallbackMarkerForQuestStepV76(quest, stepIndex);
      const reward = SNAPSHOT_STRUCTURED_REWARDS_V76.find((item) => item.questId === quest.id);
      return {
        ...
        stepIndex,
        objective,
        trigger: triggerForGroveQuestV76(quest, stepIndex),
        markerId: marker.id,
        expectedMarkerLabel: marker.label,
        expectedMarkerPosition: marker.position,
        expectedMarkerAppears: true,
        ...
        expectedAudioCue: reward?.audioCue ?? SNAPSHOT_AUDIO_CUES_V76.stepComplete,
      };
    }),
  );
...
export function snapshotGroveFootClearanceAuditV76(
  runtimePositions?: Record<string, Vec3 | undefined>,
): SnapshotFootClearanceRecordV76[] {
  return SNAPSHOT_GROVE_NPCS_V75.map((npc) => {
    const runtime = runtimePositions?.[npc.id];
    const feetY = runtime?.[1] ?? npc.authoredPosition[1];
    const clearance = Math.abs(feetY - SNAPSHOT_GROVE_EXPECTED_FEET_Y_V76);
    return {
      npcId: npc.id,
      displayName: npc.displayName,
      entityId: npc.seedServerNpc ? snapshotGroveNpcEntityIdV75(npc) : (8997551883502307 as BiomesId),
      expectedFeetY: SNAPSHOT_GROVE_EXPECTED_FEET_Y_V76,
      authoredFeetY: feetY,
      authoredClearance: Number(clearance.toFixed(3)),
      pass: clearance <= SNAPSHOT_GROVE_MAX_FEET_CLEARANCE_V76,
      note: clearance <= SNAPSHOT_GROVE_MAX_FEET_CLEARANCE_V76
...
export const SNAPSHOT_RAW_FLOATING_NPC_ASSET_PATTERNS_V76 = [
...
export function snapshotRemainingUnimplementedAfterV76() {
  return [
    ...
    "Bind actual audio files to the v76 cue ids when the final sound pack names are confirmed.",
