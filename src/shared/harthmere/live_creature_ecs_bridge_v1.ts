// HARTHMERE_LIVE_CREATURE_ECS_BRIDGE_V1
//
...
import {
  ...
  type HarthmereLiveCreatureRenderFamilyV1,
} from "@/shared/harthmere/live_creature_render_v1";

export type HarthmereLiveCreatureBridgeRecordV1 = {
  id: number;
  at: [number, number, number];
  yaw: number;
  family: HarthmereLiveCreatureRenderFamilyV1;
  asset: string;
  scale: number;
  label: string;
  species?: string;
...
export type HarthmereLiveCreatureEntityViewV1 = {
  id: number;
...
export function harthmereLiveCreatureFamilyForEntityV1(
  entity: HarthmereLiveCreatureEntityViewV1
): HarthmereLiveCreatureRenderFamilyV1 {
  return harthmereLiveCreatureRenderFamilyV1({
 ...
 */
export function isHarthmereLiveCreatureEntityV1(
  entity: HarthmereLiveCreatureEntityViewV1
): boolean {
...
export function harthmereLiveCreatureAssetForV1(
  family: HarthmereLiveCreatureRenderFamilyV1,
 ...
 */
export function harthmereLiveCreatureBridgeRecordV1(
  entity: HarthmereLiveCreatureEntityViewV1
): HarthmereLiveCreatureBridgeRecordV1 | undefined {
  if (!isHarthmereLiveCreatureEntityV1(entity)) {
    return undefined;
  }
  const pos = entity.position!.v as readonly [number, number, number];
  const family = harthmereLiveCreatureFamilyForEntityV1(entity);
  ...
  return {
    ...
    family,
    asset: harthmereLiveCreatureAssetForV1(family, species, entity.label?.text),
    scale: 1,
    label,
    species,
    hp: entity.health?.hp,
...
export type HarthmereLiveCreatureReconcileResultV1 = {
  toAdd: HarthmereLiveCreatureBridgeRecordV1[];
  toUpdate: HarthmereLiveCreatureBridgeRecordV1[];
  toRemove: number[];
};
 ...
 */
export function reconcileHarthmereLiveCreatureBridgeV1(
  renderedIds: ReadonlySet<number>,
  records: ReadonlyArray<HarthmereLiveCreatureBridgeRecordV1>
): HarthmereLiveCreatureReconcileResultV1 {
  const toAdd: HarthmereLiveCreatureBridgeRecordV1[] = [];
  const toUpdate: HarthmereLiveCreatureBridgeRecordV1[] = [];
...
export const HARTHMERE_LIVE_CREATURE_ECS_BRIDGE_KEY_V1 =
  "__harthmereLiveCreatureEcsBridgeV1";

type LiveCreatureBridgeWindow = typeof globalThis & {
  __harthmereLiveCreatureEcsBridgeV1?: {
    at: number;
    records: HarthmereLiveCreatureBridgeRecordV1[];
  };
};

export function publishHarthmereLiveCreatureBridgeV1(
  records: HarthmereLiveCreatureBridgeRecordV1[]
): void {
  if (typeof window === "undefined") {
    return;
  }
  (window as LiveCreatureBridgeWindow).__harthmereLiveCreatureEcsBridgeV1 = {
    at: Date.now(),
    records,
  };
}

export function readHarthmereLiveCreatureBridgeV1(): HarthmereLiveCreatureBridgeRecordV1[] {
  if (typeof window === "undefined") {
    return [];
  }
