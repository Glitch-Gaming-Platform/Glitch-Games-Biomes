import {
  ...
  type HarthmereBusinessCustomerAppearance,
  type HarthmereBusinessCustomerNpc,
  type HarthmereBusinessServiceAnimationCueSpec,
...
export const HARTHMERE_BUSINESS_SERVICE_PROCEDURAL_ANIMATION_VERSION =
  "harthmere-business-service-procedural-animation" as const;

export type HarthmereBusinessProceduralPart =
  | "head"
  | "hair"
  | "eyes"
  | "nose"
...
export interface HarthmereBusinessProceduralPalette {
  skin: string;
  hair: string;
  eyes: string;
  outfit: string;
...
export interface HarthmereBusinessProceduralRigPart {
  part: HarthmereBusinessProceduralPart;
  anchor: { x: number; y: number };
  size: { w: number; h: number };
  color: string;
  shape: "box" | "ellipse" | "hair" | "eyes" | "nose" | "prop";
  sourceFeature: keyof HarthmereBusinessCustomerAppearance | "derived";
}

export interface HarthmereBusinessProceduralCustomerRig {
  npcId: string;
  displayName: string;
  rendererFamily: "grove_townsperson_procedural";
  characterAppearance: HarthmereCharacterAppearance;
  palette: HarthmereBusinessProceduralPalette;
  bodyScale: { width: number; height: number; shoulder: number };
  postureLeanDeg: number;
  gaitEnergy: number;
  parts: Record<HarthmereBusinessProceduralPart, HarthmereBusinessProceduralRigPart>;
  coverageTags: string[];
}

export interface HarthmereBusinessProceduralPartPose {
  rotationDeg: number;
  scaleX: number;
  scaleY: number;
  visible: boolean;
...
export interface HarthmereBusinessProceduralFrame {
  timeMs: number;
  normalizedTime: number;
  owner: Record<HarthmereBusinessProceduralPart, HarthmereBusinessProceduralPartPose>;
  customer: Record<HarthmereBusinessProceduralPart, HarthmereBusinessProceduralPartPose>;
  prop: {
    label: string;
    color: string;
    ...
    bikkieId?: BiomesId;
    bikkieName?: string;
    bikkieKind?: HarthmereBusinessBikkieGraphic["kind"];
    galoisPath?: string;
...
export interface HarthmereBusinessProceduralClip {
  version: typeof HARTHMERE_BUSINESS_SERVICE_PROCEDURAL_ANIMATION_VERSION;
  cueId: string;
  family: HarthmereBusinessServiceAnimationCueSpec["family"];
  durationMs: number;
  customerRig: HarthmereBusinessProceduralCustomerRig;
  ownerRig: HarthmereBusinessProceduralCustomerRig;
  frames: HarthmereBusinessProceduralFrame[];
  safety: {
    procedural: true;
    voxelSafe: true;
    noRootMotion: true;
...
export interface HarthmereBusinessProceduralVisualAudit {
  ok: boolean;
...
export interface HarthmereBusinessProceduralRuntimeCombo {
  comboId: string;
...
export interface HarthmereBusinessProceduralRuntimeAudit {
  ok: boolean;
  comboCount: number;
...
function bodyWidthScale(appearance: HarthmereBusinessCustomerAppearance) {
...
function bodyHeightScale(appearance: HarthmereBusinessCustomerAppearance) {
...
function shoulderScale(appearance: HarthmereBusinessCustomerAppearance) {
...
function postureLeanDeg(posture: string) {
...
function gaitEnergy(gait: string) {
...
function rigPart(
  ...
  size: { w: number; h: number },
  color: string,
  shape: HarthmereBusinessProceduralRigPart["shape"],
  sourceFeature: HarthmereBusinessProceduralRigPart["sourceFeature"],
): HarthmereBusinessProceduralRigPart {
  return { part, anchor, size, color, shape, sourceFeature };
}

export function buildHarthmereBusinessCustomerProceduralRig(
  npc: HarthmereBusinessCustomerNpc,
): HarthmereBusinessProceduralCustomerRig {
  const appearance = npc.appearance;
  const characterAppearance = harthmereBusinessCustomerCharacterAppearance(npc);
  const width = bodyWidthScale(appearance);
  const height = bodyHeightScale(appearance);
  const shoulder = shoulderScale(appearance);
  const palette: HarthmereBusinessProceduralPalette = {
    skin: skinColorFromText(appearance.skinTone),
    hair: colorFromText(appearance.hairColor, 40),
    eyes: colorFromText(appearance.eyeColor, 120),
    outfit: colorFromText(appearance.outfit, 180),
    accent: colorFromText(`${appearance.accessory} ${appearance.voice}`, 260),
    accessory: colorFromText(appearance.accessory, 320),
    shadow: "#1b1f2a",
  };
  ...
  const parts = {
    head: rigPart("head", { x: 0, y: -58 * height }, { w: 24 * width, h: 25 * height }, palette.skin, "ellipse", "skinTone"),
    hair: rigPart("hair", { x: 0, y: -66 * height }, { w: 30 * width, h: 20 * height }, palette.hair, "hair", "hairStyle"),
    eyes: rigPart("eyes", { x: 0, y: -60 * height }, { w: 16 * width, h: 4 * height }, palette.eyes, "eyes", "eyeShape"),
    nose: rigPart("nose", { x: 0, y: -55 * height }, { w: 5 * width, h: 8 * height }, shadeHex(palette.skin, -24), "nose", "noseShape"),
    torso: rigPart("torso", { x: 0, y: -27 * height }, { w: torsoW, h: torsoH }, palette.outfit, "box", "outfit"),
    leftArm: rigPart("leftArm", { x: -torsoW * 0.62, y: -32 * height }, { w: 7 * width, h: armH }, palette.outfit, "box", "shoulderShape"),
    rightArm: rigPart("rightArm", { x: torsoW * 0.62, y: -32 * height }, { w: 7 * width, h: armH }, palette.outfit, "box", "shoulderShape"),
    leftHand: rigPart("leftHand", { x: -torsoW * 0.68, y: -8 * height }, { w: 8 * width, h: 8 * height }, palette.skin, "ellipse", "skinTone"),
    rightHand: rigPart("rightHand", { x: torsoW * 0.68, y: -8 * height }, { w: 8 * width, h: 8 * height }, palette.skin, "ellipse", "skinTone"),
    leftLeg: rigPart("leftLeg", { x: -7 * width, y: 14 * height }, { w: 8 * width, h: legH }, shadeHex(palette.outfit, -28), "box", "bodyBuild"),
    rightLeg: rigPart("rightLeg", { x: 7 * width, y: 14 * height }, { w: 8 * width, h: legH }, shadeHex(palette.outfit, -28), "box", "bodyBuild"),
    feet: rigPart("feet", { x: 0, y: 39 * height }, { w: 30 * width, h: 7 * height }, palette.shadow, "box", "gait"),
    accessory: rigPart("accessory", { x: 14 * width, y: -48 * height }, { w: 8 * width, h: 10 * height }, palette.accessory, "box", "accessory"),
    prop: rigPart("prop", { x: 0, y: -30 * height }, { w: 12, h: 12 }, palette.accent, "prop", "derived"),
  } satisfies Record<HarthmereBusinessProceduralPart, HarthmereBusinessProceduralRigPart>;
  return {
    npcId: npc.npcId,
    ...
    postureLeanDeg: postureLeanDeg(appearance.posture),
    gaitEnergy: gaitEnergy(appearance.gait),
    parts,
    coverageTags: [
...
const FALLBACK_OWNER_NPC: HarthmereBusinessCustomerNpc = {
  ...HARTHMERE_BUSINESS_CUSTOMER_NPCS[0],
  npcId: "business_owner_voxel_service_rig",
  displayName: "Business Owner",
  appearance: {
    ...HARTHMERE_BUSINESS_CUSTOMER_NPCS[0].appearance,
    outfit: "service counter apron",
    accessory: "owner ledger pin",
  },
};

function ownerNpcForCueSpec(
  spec: HarthmereBusinessServiceAnimationCueSpec,
): HarthmereBusinessCustomerNpc {
  const base = HARTHMERE_BUSINESS_CUSTOMER_NPCS[hashString(spec.cueId) % HARTHMERE_BUSINESS_CUSTOMER_NPCS.length] ?? FALLBACK_OWNER_NPC;
  const familyLook: Record<HarthmereBusinessServiceAnimationCueSpec["family"], { outfit: string; accessory: string; voice: string }> = {
...
function defaultPose(): HarthmereBusinessProceduralPartPose {
  return { rotationDeg: 0, scaleX: 1, scaleY: 1, visible: true };
}

function poseSet(): Record<HarthmereBusinessProceduralPart, HarthmereBusinessProceduralPartPose> {
  return {
    head: defaultPose(),
    hair: defaultPose(),
    eyes: defaultPose(),
    ...
    torso: defaultPose(),
    leftArm: defaultPose(),
    rightArm: defaultPose(),
    leftHand: defaultPose(),
    rightHand: defaultPose(),
    leftLeg: defaultPose(),
    rightLeg: defaultPose(),
    feet: defaultPose(),
    accessory: defaultPose(),
    prop: defaultPose(),
  };
}

function waveOscillation(t: number) {
...
function cueSpecOrFallback(cueId: string): { spec: HarthmereBusinessServiceAnimationCueSpec; warnings: string[] } {
  const existing = getHarthmereBusinessServiceAnimationCueSpec(cueId);
...
function propForCue(spec: HarthmereBusinessServiceAnimationCueSpec) {
...
function propColorForFamily(family: HarthmereBusinessServiceAnimationCueSpec["family"]) {
  switch (family) {
...
function frameForSpec(
  spec: HarthmereBusinessServiceAnimationCueSpec,
  timeMs: number,
): HarthmereBusinessProceduralFrame {
  const t = clamp01(timeMs / Math.max(1, spec.durationMs));
  const owner = poseSet();
  const customer = poseSet();
  const bikkieGraphic = getHarthmereBusinessBikkieGraphicForServiceCue(spec.cueId);
  const fallbackLabel = propForCue(spec);
  const visual =
    ...
    harthmereResolveBikkieVisual({
      ...
      label: fallbackLabel,
      kind: spec.family,
      colors: [propColorForFamily(spec.family)],
    });
  applyFamilyMotion(spec.family, t, owner, customer);
  ...
  const propY = spec.family === "tool_work" ? -40 + arc(t) * 34 : -35 + arc(t) * 12;
  return {
    timeMs,
    normalizedTime: t,
    owner,
    ...
    prop: {
      ...
      x: propX,
      y: propY,
      rotationDeg: spec.family === "dispatch" ? waveOscillation(t) * 45 : owner.rightArm.rotationDeg * 0.45,
      scale: 0.85 + arc(t) * 0.15,
      visible: true,
    },
  };
}

export function createHarthmereBusinessServiceProceduralClip(input: {
  cueId: string;
  customerNpc?: HarthmereBusinessCustomerNpc;
  ownerNpc?: HarthmereBusinessCustomerNpc;
  sampleCount?: number;
}): HarthmereBusinessProceduralClip {
  const { spec, warnings } = cueSpecOrFallback(input.cueId);
  ...
  const frames = Array.from({ length: sampleCount }, (_, index) => {
    ...
    return frameForSpec(spec, Math.round(spec.durationMs * t));
  });
  return {
    version: HARTHMERE_BUSINESS_SERVICE_PROCEDURAL_ANIMATION_VERSION,
    cueId: spec.cueId,
    family: spec.family,
    durationMs: spec.durationMs,
    customerRig: buildHarthmereBusinessCustomerProceduralRig(input.customerNpc ?? HARTHMERE_BUSINESS_CUSTOMER_NPCS[0]),
    ownerRig: buildHarthmereBusinessCustomerProceduralRig(input.ownerNpc ?? ownerNpcForCueSpec(spec)),
    frames,
    safety: {
      procedural: true,
      voxelSafe: true,
...
function renderPart(
  part: HarthmereBusinessProceduralRigPart,
  pose: HarthmereBusinessProceduralPartPose,
) {
...
function renderRigFrame(
  rig: HarthmereBusinessProceduralCustomerRig,
  framePose: Record<HarthmereBusinessProceduralPart, HarthmereBusinessProceduralPartPose>,
  x: number,
  y: number,
  mirror = false,
) {
  const scale = mirror ? "-1 1" : "1 1";
  const ordered: HarthmereBusinessProceduralPart[] = [
    "feet",
    "leftLeg",
    "rightLeg",
    "torso",
  ...
  return `<g transform="translate(${x} ${y}) scale(${scale}) rotate(${rig.postureLeanDeg})">${ordered.map((part) => renderPart(rig.parts[part], framePose[part])).join("")}</g>`;
}

function displayFamilyLabel(family: HarthmereBusinessServiceAnimationCueSpec["family"]) {
  return family.replace(/_/g, " ");
}

function renderBikkiePropVisualSvg(
  visual: HarthmereResolvedBikkieVisual,
  label: string
) {
  const primary = esc(visual.primaryHex);
  const accent = esc(visual.accentHex);
  const shadow = `<ellipse cx="0" cy="8" rx="12" ry="3.5" fill="#000000" opacity="0.22"/>`;
  const glyph = `<text x="0" y="4" fill="#ffffff" text-anchor="middle" font-family="Arial, sans-serif" font-size="5" font-weight="800">${esc(visual.glyph || label.slice(0, 3))}</text>`;
  switch (visual.shape) {
    case "block":
...
export function renderHarthmereBusinessServiceFrameSvg(
  clip: HarthmereBusinessProceduralClip,
  frame: HarthmereBusinessProceduralFrame,
  ...
  const label = showLabel
    ? `<text x="10" y="18" fill="#dfe8ff" font-family="Arial, sans-serif" font-size="10">${esc(clip.customerRig.displayName)}</text><text x="10" y="32" fill="#aeb9cf" font-family="Arial, sans-serif" font-size="9">${esc(displayFamilyLabel(clip.family))} ${Math.round(frame.normalizedTime * 100)}%</text>`
    : "";
  const owner = renderRigFrame(clip.ownerRig, frame.owner, 70, 98, false);
  const customer = renderRigFrame(clip.customerRig, frame.customer, 150, 98, true);
  const propDataAttrs = `data-prop-source="${esc(frame.prop.source)}" data-visual-source="${esc(frame.prop.visual.source)}" data-visual-kind="${esc(frame.prop.visual.shape)}" data-visual-id="${esc(frame.prop.visual.visualId)}"${frame.prop.bikkieId ? ` data-bikkie-id="${frame.prop.bikkieId}"` : ""}${frame.prop.graphicId ? ` data-graphic-id="${esc(frame.prop.graphicId)}"` : ""}${frame.prop.galoisPath ? ` data-galois-path="${esc(frame.prop.galoisPath)}"` : ""}${frame.prop.visual.iconAssetPath ? ` data-icon-asset-path="${esc(frame.prop.visual.iconAssetPath)}"` : ""}`;
  const prop = frame.prop.visible
    ? `<g ${propDataAttrs} transform="translate(${110 + frame.prop.x} ${74 + frame.prop.y * 0.18}) rotate(${frame.prop.rotationDeg.toFixed(2)}) scale(${frame.prop.scale.toFixed(2)})">${renderBikkiePropVisualSvg(frame.prop.visual, frame.prop.label)}</g>`
    : "";
  const progress = Math.max(0, Math.min(width - 24, (width - 24) * frame.normalizedTime));
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" data-cue-id="${esc(clip.cueId)}" data-npc-id="${esc(clip.customerRig.npcId)}"><rect width="${width}" height="${height}" fill="#101722"/><rect x="0" y="0" width="${width}" height="${height}" fill="#172235" opacity="0.42"/><rect x="8" y="118" width="${width - 16}" height="7" fill="#273447"/><rect x="76" y="82" width="68" height="18" rx="3" fill="#26364d" stroke="#516680" stroke-opacity="0.7"/><rect x="84" y="78" width="52" height="5" rx="2" fill="#364961"/><path d="M 80 86 L 140 86" stroke="#8aa0b8" stroke-width="2" stroke-linecap="round" opacity="0.65"/><circle cx="110" cy="68" r="24" fill="${frame.prop.color}" opacity="0.08"/><rect x="12" y="${height - 10}" width="${width - 24}" height="3" rx="1.5" fill="#263244"/><rect x="12" y="${height - 10}" width="${progress.toFixed(2)}" height="3" rx="1.5" fill="${frame.prop.color}"/>${owner}${customer}${prop}${label}</svg>`;
}

function renderCustomerCoverageAtlas() {
  const cellW = 184;
  const cellH = 132;
  const cols = 5;
  const rows = Math.ceil(HARTHMERE_BUSINESS_CUSTOMER_NPCS.length / cols);
  const cueIds = Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS);
  const cells = HARTHMERE_BUSINESS_CUSTOMER_NPCS.map((npc, index) => {
    const x = (index % cols) * cellW;
    const y = Math.floor(index / cols) * cellH;
    const cueId = cueIds[index % cueIds.length];
    const clip = createHarthmereBusinessServiceProceduralClip({ cueId, customerNpc: npc, sampleCount: 3 });
    const frame = clip.frames[1];
    const inner = renderHarthmereBusinessServiceFrameSvg(clip, frame, { width: cellW, height: cellH }).replace(/^<svg[^>]*>|<\/svg>$/g, "");
    return `<g transform="translate(${x} ${y})">${inner}</g>`;
  }).join("");
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${cellW * cols}" height="${cellH * rows}" viewBox="0 0 ${cellW * cols} ${cellH * rows}" data-atlas="customers" data-customer-count="${HARTHMERE_BUSINESS_CUSTOMER_NPCS.length}">${cells}</svg>`;
}

function renderCueCoverageAtlas() {
  const cues = Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS);
  const cellW = 184;
  const frameH = 132;
  const cellH = 154;
  const cols = 5;
  ...
  const cells = cues.map((cueId, index) => {
    const x = (index % cols) * cellW;
    const y = Math.floor(index / cols) * cellH;
    const npc = HARTHMERE_BUSINESS_CUSTOMER_NPCS[index % HARTHMERE_BUSINESS_CUSTOMER_NPCS.length];
    const clip = createHarthmereBusinessServiceProceduralClip({ cueId, customerNpc: npc, sampleCount: 5 });
    const frame = clip.frames[2];
    const inner = renderHarthmereBusinessServiceFrameSvg(clip, frame, { width: cellW, height: frameH }).replace(/^<svg[^>]*>|<\/svg>$/g, "");
    return `<g transform="translate(${x} ${y})"><rect width="${cellW}" height="${cellH}" fill="#111722"/>${inner}<rect x="0" y="${frameH}" width="${cellW}" height="${cellH - frameH}" fill="#0f1622"/><text x="10" y="${frameH + 15}" fill="#dfe8ff" font-family="Arial, sans-serif" font-size="8">${esc(cueId.replace(/^procedural_/, "").replace(/_/g, " ").slice(0, 32))}</text></g>`;
  }).join("");
...
export function renderHarthmereBusinessProceduralAnimationVisualAuditHtml() {
  const customerAtlas = renderCustomerCoverageAtlas();
  const cueAtlas = renderCueCoverageAtlas();
  return `<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><title>Business Service Procedural Animation Visual Audit</title><style>body{margin:0;background:#0c1018;color:#dfe8ff;font-family:Inter,Arial,sans-serif}main{padding:24px;display:grid;gap:18px}.hero{display:grid;gap:10px}.summary{display:flex;gap:10px;flex-wrap:wrap}.summary span{border:1px solid #33415a;border-radius:6px;background:#141d2b;padding:8px 10px;color:#dfe8ff}section{border:1px solid #33415a;border-radius:8px;padding:14px;background:#111722;box-shadow:0 14px 28px rgba(0,0,0,.22)}h1{margin:0;font-size:30px;letter-spacing:.01em}h2{margin:0 0 12px;font-size:20px}svg{display:block;max-width:100%;height:auto}p{margin:0;color:#aeb9cf;line-height:1.5}@media(max-width:720px){main{padding:12px}h1{font-size:24px}}</style></head><body data-version="${HARTHMERE_BUSINESS_SERVICE_PROCEDURAL_ANIMATION_VERSION}" data-customer-count="${HARTHMERE_BUSINESS_CUSTOMER_NPCS.length}" data-cue-count="${Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS).length}"><main><div class="hero"><h1>Business Service Procedural Animation Visual Audit</h1><p>Customer-only NPCs, owner service rigs, props, and cue timing are rendered from procedural voxel-safe animation data.</p><div class="summary"><span>${HARTHMERE_BUSINESS_CUSTOMER_NPCS.length} customer designs</span><span>${Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS).length} service cues</span><span>0 root motion</span><span>voxel safe</span></div></div><section><h2>Customer Design Coverage</h2>${customerAtlas}</section><section><h2>Service Cue Coverage</h2>${cueAtlas}</section></main></body></html>`;
}

function runtimeCombos() {
  const cueIds = Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS);
  const customers = HARTHMERE_BUSINESS_CUSTOMER_NPCS;
  return [
...
function frameMotionSignature(svg: string) {
  return svg
    .replace(/\sdata-[^=]+="[^"]*"/g, "")
...
export function buildHarthmereBusinessProceduralRuntimeCombos(
  sampleCount = 24,
): HarthmereBusinessProceduralRuntimeCombo[] {
  return runtimeCombos().map((entry) => {
    ...
    return {
      ...
      frameSvgs: clip.frames.map((frame) =>
        renderHarthmereBusinessServiceFrameSvg(clip, frame, {
          width: 220,
          height: 150,
        }),
      ),
...
export function renderHarthmereBusinessProceduralAnimationRuntimeAuditHtml() {
  const combos = buildHarthmereBusinessProceduralRuntimeCombos();
  const data = {
    version: HARTHMERE_BUSINESS_SERVICE_PROCEDURAL_ANIMATION_VERSION,
    customerCount: HARTHMERE_BUSINESS_CUSTOMER_NPCS.length,
    cueCount: Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS).length,
    combos,
  };
...
(() => {
  ...
  window.__harthmereBusinessAnimationRuntimeAudit = {
    data,
    state,
    select: setIndex,
...
export function validateHarthmereBusinessProceduralAnimationRuntimeAudit(): HarthmereBusinessProceduralRuntimeAudit {
  const cueIds = Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS);
  const warnings: string[] = [];
  const combos = buildHarthmereBusinessProceduralRuntimeCombos();
  let movingComboCount = 0;
  for (const combo of combos) {
    const signatures = new Set(combo.frameSvgs.map(frameMotionSignature));
  ...
  if (customerCoverage.size !== HARTHMERE_BUSINESS_CUSTOMER_NPCS.length) {
...
export function validateHarthmereBusinessProceduralAnimationVisualAudit(): HarthmereBusinessProceduralVisualAudit {
  const cueIds = Object.keys(HARTHMERE_BUSINESS_SERVICE_ANIMATION_CUE_SPECS);
  const warnings: string[] = [];
  ...
  return {
    ...
    renderedCustomerCells: HARTHMERE_BUSINESS_CUSTOMER_NPCS.length,
