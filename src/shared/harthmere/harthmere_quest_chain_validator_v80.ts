// HARTHMERE_QUEST_CHAIN_VALIDATOR_V80
//
// The bible quest catalog in `quest_compendium_v46.ts` stores quests with
...
export const HARTHMERE_QUEST_CHAIN_VALIDATOR_VERSION_V80 =
  "harthmere-quest-chain-validator-v80" as const;

export interface HarthmereQuestChainEdgeV80 {
  fromQuestId: string;
  toQuestId: string;
...
export interface HarthmereQuestChainV80 {
...
export function buildHarthmereQuestChainV80(): HarthmereQuestChainV80 {
  const warnings: string[] = [];
  const quests = HARTHMERE_QUEST_CATALOG_V46 as readonly any[];
...
export interface HarthmereQuestChainValidationV80 {
  ok: boolean;
  failures: string[];
  warnings: string[];
  totalQuests: number;
...
export function validateHarthmereQuestChainV80(opts?: {
  knownNpcIds?: ReadonlySet<string>;
}): HarthmereQuestChainValidationV80 {
  const failures: string[] = [];
  const knownNpcIds = opts?.knownNpcIds;
  const quests = HARTHMERE_QUEST_CATALOG_V46 as readonly any[];
  const catalogCheck = validateHarthmereQuestCatalogV46();
 ...
 */
export function nextSuggestedHarthmereQuestV80(input: {
