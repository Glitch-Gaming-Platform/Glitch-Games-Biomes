...
export interface HarthmereVendorStockLineV1 {
  itemId: string;
  quantity: number;
  price: number;
}

export interface HarthmereVendorProfileV1 {
  offset: number;
  vendorId: string;
  vendorName: string;
  name: string;
  vendorType: string;
  region: "harthmere";
  stocks: HarthmereVendorStockLineV1[];
  sells: Array<{ itemId: string; quantity: number }>;
  buys: HarthmereVendorCategoryV1[];
  buysCategories: HarthmereVendorCategoryV1[];
...
interface HarthmereVendorItemDefinitionSeedV1 {
  displayName: string;
  description?: string;
  category: HarthmereVendorCategoryV1;
  maxStackSize: number;
  baseValue: number;
...
export const HARTHMERE_VENDOR_CATALOG_V1: Record<
  number,
  HarthmereVendorProfileV1
> = {
  5: vendor({
...
const HARTHMERE_VENDOR_ITEM_DEFINITIONS_V1: Record<
  string,
  HarthmereVendorItemDefinitionSeedV1
> = {
  apple_tart: {
...
function itemDefinitionFromSeed(
  itemId: string,
  seed: HarthmereVendorItemDefinitionSeedV1
...
function unitBuyPrice(stock: HarthmereVendorStockLineV1) {
  return Math.max(1, Math.ceil(stock.price / Math.max(1, stock.quantity)));
}

function vendorBuysStockCategory(
  profile: HarthmereVendorProfileV1,
  stock: HarthmereVendorStockLineV1
) {
  const seedCategory = HARTHMERE_VENDOR_ITEM_DEFINITIONS_V1[stock.itemId]?.category;
  if (seedCategory) return profile.buys.includes(seedCategory);

  const def = getHarthmereItemDefinitionV1(stock.itemId);
  ...
  return category
    ? profile.buys.includes(category as HarthmereVendorCategoryV1)
    : false;
}

export function createHarthmereProductionVendorEntryV1(
  profile: HarthmereVendorProfileV1,
  stock: HarthmereVendorStockLineV1
): HarthmereVendorEntryV1 {
  const buyPrice = unitBuyPrice(stock);
  const seed = HARTHMERE_VENDOR_ITEM_DEFINITIONS_V1[stock.itemId];
  const baseValue =
    seed?.baseValue ??
    getHarthmereItemDefinitionV1(stock.itemId)?.baseValue ??
    buyPrice;
...
export function listHarthmereProductionVendorEntriesV1() {
  ensureHarthmereProductionVendorCatalogV1();
  return Object.values(HARTHMERE_VENDOR_CATALOG_V1).flatMap((profile) =>
    profile.stocks.map((stock) =>
      createHarthmereProductionVendorEntryV1(profile, stock)
    )
  );
}
...
export function ensureHarthmereProductionVendorCatalogV1() {
  if (registered) return;
  ensureHarthmereProductionCraftingCatalogueV1();
  registered = true;

  for (const [itemId, seed] of Object.entries(
    HARTHMERE_VENDOR_ITEM_DEFINITIONS_V1
  )) {
    if (!getHarthmereItemDefinitionV1(itemId)) {
      registerHarthmereItemDefinitionV1(itemDefinitionFromSeed(itemId, seed));
    }
  }

  for (const profile of Object.values(HARTHMERE_VENDOR_CATALOG_V1)) {
