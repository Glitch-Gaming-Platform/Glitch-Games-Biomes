...
export function collisionFromHarthmerePlacement(input: {
  ...
  if (asset === "cart" || asset === "cart_high" || asset === "trolley") {
    return { category: "soft", halfX: scaled(2.8, scale), halfZ: scaled(1.5, scale), padding: 0.72, blocksNpc: true, blocksPlayer: true, reason: "cart obstacle" };
  }
  if (asset === "fence" || asset === "fence_gate" || asset === "fence_broken") {
    return { category: "npcBlocker", halfX: scaled(2.8, scale), halfZ: scaled(0.42, scale), padding: 0.55, blocksNpc: true, blocksPlayer: false, reason: "low fence keeps NPCs in lanes" };
  }
  if (asset === "hedge" || asset === "hedge_large") {
    return { category: "npcBlocker", halfX: scaled(2.6, scale), halfZ: scaled(0.55, scale), padding: 0.55, blocksNpc: true, blocksPlayer: false, reason: "hedge lane boundary" };
  }
  if (/counter|table|bench|bed|cabinet|bookcase|shelf|workbench|anvil|dummy|cage|chest|crate|barrel|keg/i.test(label)) {
    return { category: "soft", halfX: scaled(1.35, scale), halfZ: scaled(1.0, scale), padding: 0.42, blocksNpc: true, blocksPlayer: false, reason: "furniture/large prop lane guidance" };
  }
