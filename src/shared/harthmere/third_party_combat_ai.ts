/*
 * Harthmere third-party combat AI integration current.
 *
...
export const HARTHMERE_THIRD_PARTY_COMBAT_AI_VERSION = "harthmere-third-party-combat-ai";

export const HARTHMERE_THIRD_PARTY_COMBAT_AI_STACK = [
  {
    id: "mistreevous",
    ...
    adapterExports: ["buildMistreevousCombatTreeSpec", "createBehaviorTreeAdapterPlan"],
  ...
  },
  {
    ...
    adapterExports: ["buildYukaSteeringPlan", "createSteeringAdapterPlan"],
  ...
  },
  {
    ...
    adapterExports: ["buildRecastNavigationRequest", "createNavigationAdapterPlan"],
  ...
  },
  {
    ...
    adapterExports: ["buildPythonCombatAISimulationSpec"],
  ...
  },
  {
    ...
    adapterExports: ["scoreHarthmereCombatAIAction"],
...
export const HARTHMERE_COMBAT_AI_ARCHETYPES = {
  basic_melee: {
    id: "basic_melee",
...
export type HarthmereCombatAIActionId = keyof typeof HARTHMERE_COMBAT_AI_ACTIONS;
export type HarthmereCombatAIArchetypeId = keyof typeof HARTHMERE_COMBAT_AI_ARCHETYPES;
export type HarthmereCombatAIArchetype = (typeof HARTHMERE_COMBAT_AI_ARCHETYPES)[HarthmereCombatAIArchetypeId];
type HarthmereCombatAIInput = Record<string, any>;

function normalizeCombatAIActionId(actionId: string): HarthmereCombatAIActionId {
  return (actionId in HARTHMERE_COMBAT_AI_ACTIONS ? actionId : "idle_watch") as HarthmereCombatAIActionId;
}

function normalizeCombatAIArchetypeId(archetypeId: string): HarthmereCombatAIArchetypeId {
  return (archetypeId in HARTHMERE_COMBAT_AI_ARCHETYPES ? archetypeId : "basic_melee") as HarthmereCombatAIArchetypeId;
}

function resolveCombatAIArchetype(archetypeInput?: string | HarthmereCombatAIArchetype | null): HarthmereCombatAIArchetype {
  if (typeof archetypeInput === "string") {
    return HARTHMERE_COMBAT_AI_ARCHETYPES[normalizeCombatAIArchetypeId(archetypeInput)];
  }
  return archetypeInput ?? HARTHMERE_COMBAT_AI_ARCHETYPES.basic_melee;
}
...
export function createHarthmereCombatAIBlackboard(input: HarthmereCombatAIInput = {}) {
  const src = input || {};
  return {
    actorId: src.actorId || "npc:unknown",
    archetypeId: normalizeCombatAIArchetypeId(String(src.archetypeId || "basic_melee")),
    ...
    healthPercent: clamp(src.healthPercent ?? 1, 0, 1),
    staminaPercent: clamp(src.staminaPercent ?? 1, 0, 1),
    manaPercent: clamp(src.manaPercent ?? 1, 0, 1),
    targetHealthPercent: clamp(src.targetHealthPercent ?? 1, 0, 1),
    allyHealthLowestPercent: clamp(src.allyHealthLowestPercent ?? 1, 0, 1),
...
export function cooldownReady(blackboard: ReturnType<typeof createHarthmereCombatAIBlackboard>, cooldownKey: string) {
...
export function evaluateHarthmereCombatAIFSM(blackboardInput: HarthmereCombatAIInput | ReturnType<typeof createHarthmereCombatAIBlackboard>) {
  const b = createHarthmereCombatAIBlackboard(blackboardInput as HarthmereCombatAIInput);
...
export function scoreHarthmereCombatAIAction(actionId: string, blackboardInput: HarthmereCombatAIInput | ReturnType<typeof createHarthmereCombatAIBlackboard>, archetypeInput?: string | HarthmereCombatAIArchetype) {
  const b = createHarthmereCombatAIBlackboard(blackboardInput as HarthmereCombatAIInput);
  const actionKey = normalizeCombatAIActionId(actionId);
  const action = HARTHMERE_COMBAT_AI_ACTIONS[actionKey];
  if (!action) return { actionId, score: -9999, legal: false, reasons: ["unknown_action"] };
  const archetype = archetypeInput ? resolveCombatAIArchetype(archetypeInput) : HARTHMERE_COMBAT_AI_ARCHETYPES[b.archetypeId];
  const reasons: string[] = [];
  const fsm = evaluateHarthmereCombatAIFSM(b);
  ...
  if (!cooldownReady(b, action.cooldownKey)) reasons.push("cooldown_not_ready");
...
export function chooseHarthmereCombatAIDecision(input: HarthmereCombatAIInput = {}) {
  const blackboard = createHarthmereCombatAIBlackboard(input);
  const archetype = HARTHMERE_COMBAT_AI_ARCHETYPES[blackboard.archetypeId] || HARTHMERE_COMBAT_AI_ARCHETYPES.basic_melee;
  const fsm = evaluateHarthmereCombatAIFSM(blackboard);
  ...
  const scored = candidates.map((actionId: string) => scoreHarthmereCombatAIAction(actionId, blackboard, archetype)).sort((a, b) => b.score - a.score);
  const selected = scored.find((s) => s.legal) || { actionId: "idle_watch", score: 0, legal: true, reasons: ["fallback_idle"] };
  const movementRequest = buildYukaSteeringPlan(blackboard, archetype, selected.actionId);
  const navmeshRequest = buildRecastNavigationRequest(blackboard, archetype, selected.actionId);
  return {
    version: HARTHMERE_THIRD_PARTY_COMBAT_AI_VERSION,
    ...
    behaviorTreeNode: inferBehaviorTreeNode(selected.actionId),
    utilityScores: scored,
    movementRequest,
    navmeshRequest,
    serverActionRequest: buildServerActionRequest(blackboard, selected.actionId),
    auditTags: ["third_party_ai_intent_only", "server_authority_required", `archetype:${archetype.id}`, `action:${selected.actionId}`],
  };
}

export function inferBehaviorTreeNode(actionId: string) {
...
export function buildServerActionRequest(blackboard: ReturnType<typeof createHarthmereCombatAIBlackboard>, actionId: string) {
  const actionKey = normalizeCombatAIActionId(actionId);
  const action = HARTHMERE_COMBAT_AI_ACTIONS[actionKey] || HARTHMERE_COMBAT_AI_ACTIONS.idle_watch;
  return {
...
export function buildMistreevousCombatTreeSpec(archetypeId: string) {
  const archetype = HARTHMERE_COMBAT_AI_ARCHETYPES[normalizeCombatAIArchetypeId(archetypeId)] || HARTHMERE_COMBAT_AI_ARCHETYPES.basic_melee;
  return {
...
export function buildYukaSteeringPlan(blackboardInput: HarthmereCombatAIInput | ReturnType<typeof createHarthmereCombatAIBlackboard>, archetypeInput: string | HarthmereCombatAIArchetype | undefined, selectedActionId: string) {
  const b = createHarthmereCombatAIBlackboard(blackboardInput as HarthmereCombatAIInput);
  const archetype = archetypeInput ? resolveCombatAIArchetype(archetypeInput) : HARTHMERE_COMBAT_AI_ARCHETYPES[b.archetypeId];
...
export function buildRecastNavigationRequest(blackboardInput: HarthmereCombatAIInput | ReturnType<typeof createHarthmereCombatAIBlackboard>, archetypeInput: string | HarthmereCombatAIArchetype | undefined, selectedActionId: string) {
  const b = createHarthmereCombatAIBlackboard(blackboardInput as HarthmereCombatAIInput);
  const archetype = archetypeInput ? resolveCombatAIArchetype(archetypeInput) : HARTHMERE_COMBAT_AI_ARCHETYPES[b.archetypeId];
  ...
  return {
    ...
    selectedActionId,
    navmeshId: "harthmere-runtime-navmesh",
    from: b.position,
...
export function buildPythonCombatAISimulationSpec(archetypeId: string) {
  const archetype = HARTHMERE_COMBAT_AI_ARCHETYPES[normalizeCombatAIArchetypeId(archetypeId)] || HARTHMERE_COMBAT_AI_ARCHETYPES.basic_melee;
  return {
    archetypeId: archetype.id,
...
export function createBehaviorTreeAdapterPlan(providerId: string = "mistreevous") {
  return {
    ...
    deterministicFallback: "buildMistreevousCombatTreeSpec + chooseHarthmereCombatAIDecision",
  };
}

export function createSteeringAdapterPlan() {
  return {
    providerId: "yuka",
    ...
    deterministicFallback: "buildYukaSteeringPlan",
  };
}

export function createNavigationAdapterPlan() {
  return {
    ...
    deterministicFallback: "buildRecastNavigationRequest",
  };
}

export function validateThirdPartyAIDependencyPlan(packageJson: Record<string, any> = {}, pythonRequirementsText: string = "") {
...
export function validateHarthmereCombatAIReadiness() {
  const roles = HARTHMERE_THIRD_PARTY_COMBAT_AI_STACK.map((x) => x.role);
  const archetypes = Object.values(HARTHMERE_COMBAT_AI_ARCHETYPES);
  const errors = [];
  ...
  const stackText = JSON.stringify(HARTHMERE_THIRD_PARTY_COMBAT_AI_STACK).toLowerCase();
  ...
  return {
    ok: errors.length === 0,
    errors,
    providerCount: HARTHMERE_THIRD_PARTY_COMBAT_AI_STACK.length,
