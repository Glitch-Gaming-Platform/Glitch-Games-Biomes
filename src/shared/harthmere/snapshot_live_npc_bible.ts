...
export const SNAPSHOT_LIVE_NPC_BIBLE_VERSION =
  "snapshot-live-npc-bible" as const;
export const SNAPSHOT_LIVE_NPC_BIBLE_NO_NEW_NPCS = true as const;
export const SNAPSHOT_LIVE_NPC_BIBLE_EXCLUDES_HARTHMERE = true as const;

export type SnapshotLiveNpcHomeArea =
  | "the_grove"
  | "genesis_crossroads"
  | "old_grove_road"
  | "lovely_locks"
...
export type SnapshotLiveNpcLoreSource =
  | "live_snapshot_label"
  | "screenshot_visible_snapshot_label";

export interface SnapshotLiveNpcLore {
  id: string;
  displayName: string;
  source: SnapshotLiveNpcLoreSource;
  homeArea: SnapshotLiveNpcHomeArea;
  role: string;
...
export const SNAPSHOT_LIVE_NPC_EXCLUDED_HARTHMERE_NAMES = [
  "Sergeant Bram Holt",
  "Mara",
  "Osric",
  "Elowen",
...
export const SNAPSHOT_LIVE_NPC_LORE: readonly SnapshotLiveNpcLore[] = [
  {
    id: "allix",
    displayName: "Allix",
...
export const SNAPSHOT_LIVE_NPC_LORE_LABELS = SNAPSHOT_LIVE_NPC_LORE.map(
  (npc) => npc.displayName,
) as readonly string[];

export function snapshotNormalizeLiveNpcName(value: string | undefined) {
  return (value ?? "")
    .replace(/<[^>]*>/g, " ")
    .replace(/[^a-z0-9]+/gi, " ")
    .trim()
...
export function snapshotLiveNpcLoreByName(name: string | undefined) {
  const normalized = snapshotNormalizeLiveNpcName(name);
  if (!normalized) return undefined;
  return SNAPSHOT_LIVE_NPC_LORE.find(
    (npc) => snapshotNormalizeLiveNpcName(npc.displayName) === normalized,
  );
}

export function snapshotLiveNpcLoreFromText(text: string | undefined) {
  const haystack = ` ${snapshotNormalizeLiveNpcName(text)} `;
  if (!haystack.trim()) return undefined;
  return SNAPSHOT_LIVE_NPC_LORE.find((npc) => {
    const needle = ` ${snapshotNormalizeLiveNpcName(npc.displayName)} `;
    return haystack.includes(needle);
  });
}

export function snapshotIsExcludedHarthmereNpcLore(text: string | undefined) {
  const haystack = ` ${snapshotNormalizeLiveNpcName(text)} `;
  if (!haystack.trim()) return false;
  return SNAPSHOT_LIVE_NPC_EXCLUDED_HARTHMERE_NAMES.some((name) => {
    const needle = ` ${snapshotNormalizeLiveNpcName(name)} `;
    return haystack.includes(needle);
  });
}

export function snapshotLiveNpcLoreForDialog(input: {
  ...
  const text = [input.label, input.entityDescriptionText, input.defaultDialog]
    .filter(Boolean)
    .join(" ");
  if (snapshotIsExcludedHarthmereNpcLore(text)) return undefined;
  return snapshotLiveNpcLoreFromText(text) ?? snapshotLiveNpcLoreByName(input.label);
}

export function snapshotLiveNpcBibleAudit() {
  return {
    version: SNAPSHOT_LIVE_NPC_BIBLE_VERSION,
    noNewNpcs: SNAPSHOT_LIVE_NPC_BIBLE_NO_NEW_NPCS,
    excludesHarthmere: SNAPSHOT_LIVE_NPC_BIBLE_EXCLUDES_HARTHMERE,
    count: SNAPSHOT_LIVE_NPC_LORE.length,
    labels: SNAPSHOT_LIVE_NPC_LORE_LABELS,
  };
}
