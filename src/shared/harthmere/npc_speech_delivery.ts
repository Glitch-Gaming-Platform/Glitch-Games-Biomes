...
export type HarthmereSpeechDeliveryTone =
  | "warm_practical"
  | "brisk_road"
  | "dry_precise"
...
export interface HarthmereSpeechDeliveryProfile {
  tone: HarthmereSpeechDeliveryTone;
  rateDelta: number;
  pitchDelta: number;
...
export interface HarthmereResolvedSpeechDelivery
  extends HarthmereSpeechDeliveryProfile {
  actorKey?: string;
}

const BASE_DELIVERIES: Record<
  HarthmereSpeechDeliveryTone,
  HarthmereSpeechDeliveryProfile
> = {
  warm_practical: {
    tone: "warm_practical",
    rateDelta: 0,
...
const EXACT_TONE_BY_ID: Readonly<
  Record<string, HarthmereSpeechDeliveryTone>
> = {
...
function actorIdFromKey(actorKey: string | undefined) {
  const parts = actorKey?.split(":").filter(Boolean) ?? [];
  return parts.length >= 2 ? parts[1] : undefined;
}

function toneFromActorText(text: string): HarthmereSpeechDeliveryTone {
...
export function harthmereSpeechDeliveryForActor(input: {
  actorKey?: string;
  text?: string;
}): HarthmereResolvedSpeechDelivery {
  const actorId = actorIdFromKey(input.actorKey);
  const exactTone = actorId ? EXACT_TONE_BY_ID[actorId] : undefined;
  const text = `${input.actorKey ?? ""} ${input.text ?? ""}`.toLowerCase();
  const base = BASE_DELIVERIES[exactTone ?? toneFromActorText(text)];
  return {
    ...base,
...
export function harthmereSpeechDeliveryToneForActorForTest(input: {
  actorKey?: string;
  text?: string;
}) {
  return harthmereSpeechDeliveryForActor(input).tone;
}

export function harthmereSpeechDeliveryPromptBrief(input: {
  actorKey?: string;
  text?: string;
}) {
  const delivery = harthmereSpeechDeliveryForActor(input);
  const guidanceByTone: Record<HarthmereSpeechDeliveryTone, string> = {
    warm_practical:
