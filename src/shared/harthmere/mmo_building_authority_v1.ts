...
export const MMO_BUILDING_AUTHORITY_VERSION_V1 = "mmo-building-authority-v1";

// ---------------------------------------------------------------------------
// Terrain types
// ---------------------------------------------------------------------------

export type HarthmereTerrainTypeV1 =
  | "grass"
  | "dirt"
  | "stone"
  | "sand"
...
const UNBUILDABLE_TERRAIN: ReadonlySet<HarthmereTerrainTypeV1> = new Set([
  "water",
  "lava",
  "road",
  "bridge",
...
export type HarthmereStructureTypeV1 =
  | "small_house"
  | "medium_house"
  | "large_house"
  | "shop"
...
export interface HarthmereStructureDefinitionV1 {
  ...
  allowedTerrainTypes: HarthmereTerrainTypeV1[];
  /** Maximum height above ground in voxels */
  maxHeightAboveGround: number;
  /** Required plot type */
  requiredPlotType?: HarthmerePlotTypeV1;
...
export type HarthmerePlotTypeV1 =
  | "residential"
  | "commercial"
  | "crafting"
  | "farm"
...
export interface HarthmerePlotDefinitionV1 {
  plotId: string;
  ownerId: string;
...
export interface HarthmereBuildingPlacementContextV1 {
  ...
  plot?: HarthmerePlotDefinitionV1;
}
...
export interface HarthmereBuildingPlacementRequestV1 {
  requestId: string;
  actorId: string;
  structureTypeId: HarthmereStructureTypeV1;
...
export interface HarthmereBuildingPlacementResultV1 {
  ok: boolean;
  requestId: string;
  actorId: string;
  structureTypeId: HarthmereStructureTypeV1;
...
const _structureRegistry = new Map<HarthmereStructureTypeV1, HarthmereStructureDefinitionV1>();

export function registerHarthmereStructureDefinitionV1(
  def: HarthmereStructureDefinitionV1
) {
  _structureRegistry.set(def.structureTypeId, def);
}

export function getHarthmereStructureDefinitionV1(
  typeId: HarthmereStructureTypeV1
): HarthmereStructureDefinitionV1 | undefined {
...
(function seedBuiltinStructures() {
  const defaults: HarthmereStructureDefinitionV1[] = [
    {
...
export function validateHarthmereBuildingPlacementV1(
  req: HarthmereBuildingPlacementRequestV1,
  ctx: HarthmereBuildingPlacementContextV1
): HarthmereBuildingPlacementResultV1 {
  const errors: string[] = [];
  const warnings: string[] = [];

  const def = getHarthmereStructureDefinitionV1(req.structureTypeId);
  if (!def) {
    return {
      ok: false,
...
export interface HarthmereBuildingDemolitionRequestV1 {
  requestId: string;
  actorId: string;
  structureId: string;
  plotId?: string;
...
export interface HarthmereBuildingDemolitionResultV1 {
  ok: boolean;
  requestId: string;
  errors: string[];
  warnings: string[];
...
export interface HarthmereBuildingDemolitionContextV1 {
...
export function validateHarthmereBuildingDemolitionV1(
  req: HarthmereBuildingDemolitionRequestV1,
  ctx: HarthmereBuildingDemolitionContextV1
): HarthmereBuildingDemolitionResultV1 {
  const errors: string[] = [];
  const warnings: string[] = [];
...
export interface HarthmerePlotClaimRequestV1 {
  requestId: string;
  actorId: string;
  plotId: string;
  nowMs: number;
...
export interface HarthmerePlotClaimResultV1 {
  ok: boolean;
  requestId: string;
  errors: string[];
  warnings: string[];
...
export interface HarthmerePlotClaimContextV1 {
  plot: HarthmerePlotDefinitionV1 | undefined;
...
export function validateHarthmerePlotClaimV1(
  req: HarthmerePlotClaimRequestV1,
  ctx: HarthmerePlotClaimContextV1
): HarthmerePlotClaimResultV1 {
  const errors: string[] = [];
  const warnings: string[] = [];
