// HARTHMERE_LIVE_CREATURE_RESPAWN_REGISTRY_V1
//
...
export class HarthmereLiveCreatureRespawnRegistryV1<TId = number> {
  ...
  isSuppressed(id: TId, nowMs: number): boolean {
    ...
    if (harthmereLiveCreatureShouldRespawnV1({ nowMs, respawnAtMs })) {
...
let sharedRegistry: HarthmereLiveCreatureRespawnRegistryV1 | undefined;

export function harthmereSharedLiveCreatureRespawnRegistryV1(): HarthmereLiveCreatureRespawnRegistryV1 {
  if (!sharedRegistry) {
    sharedRegistry = new HarthmereLiveCreatureRespawnRegistryV1();
  }
