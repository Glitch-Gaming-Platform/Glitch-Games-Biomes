...
export interface HarthmereEconomyBusinessSystemsState {
  ...
  permissions: Record<string, Record<string, HarthmereEconomyBusinessPermission[]>>;
  ...
  outpostBuildings: Record<string, HarthmereBusinessOutpostProceduralBuildingRecord>;
...
type BusinessSystemsContext = HarthmereEconomyMutationContext & {
  ...
  actorBankAccountPermissions?: Record<string, HarthmereEconomyBusinessPermission[]>;
};

function defaultBusinessSystemsState(nowMs = 0): HarthmereEconomyBusinessSystemsState {
  void nowMs;
  return {
...
function cloneState(state: HarthmereProductionEconomyState): BusinessSystemsEconomyState {
  const cloned = JSON.parse(JSON.stringify(state ?? {})) as BusinessSystemsEconomyState;
  cloned.businessSystems = normalizeHarthmereEconomyBusinessSystemsState(cloned.businessSystems);
  return cloned;
}
...
function applyItem(inventory: HarthmereEconomyInventoryRecord, itemId: string, delta: number, extras: Partial<{ expiresAtMs: number; condition: number; contaminated: boolean }> = {}) {
...
function businessIsType(b: HarthmereEconomyBusinessRecord, typeId: HarthmereEconomyBusinessTypeId) {
...
function addNeed(state: BusinessSystemsEconomyState, businessRecord: HarthmereEconomyBusinessRecord, need: string, delta: number, nowMs: number) {
...
function audit(systems: HarthmereEconomyBusinessSystemsState, account: HarthmereEconomyBusinessBankAccount | undefined, request: HarthmereEconomyMutationRequest, kind: string, values: Partial<HarthmereEconomyBusinessAuditEntry>) {
...
function accountForBusiness(systems: HarthmereEconomyBusinessSystemsState, businessId: string) {
  return Object.values(systems.bankAccounts).find((account) => account.businessId === businessId && account.status === "active");
}

function createBankAccount(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function transferPersonalToBusinessBank(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, goldDelta: { value: number }, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "accountant");
  if (!b) return;
  const systems = state.businessSystems!;
...
function transferBusinessToPersonalBank(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, goldDelta: { value: number }, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "accountant");
  if (!b) return;
  const systems = state.businessSystems!;
  const account = accountForBusiness(systems, b.businessId);
...
function grantBusinessPermission(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  ...
  const permissions = [...new Set(requestedPermissions)].filter(Boolean) as HarthmereEconomyBusinessPermission[];
...
function adjustBusinessFunds(b: HarthmereEconomyBusinessRecord, amount: number) {
  b.balanceGold = Math.max(0, b.balanceGold + Math.trunc(amount));
}

function requireInventory(b: HarthmereEconomyBusinessRecord, warnings: string[], touched: Set<string>, items: Record<string, number>) {
...
function consumeInventory(b: HarthmereEconomyBusinessRecord, items: Record<string, number>) {
  for (const [itemId, count] of Object.entries(items)) applyItem(b.inventory, itemId, -count);
}

function produceInventory(b: HarthmereEconomyBusinessRecord, items: Record<string, number>, extras: Partial<{ expiresAtMs: number; condition: number; contaminated: boolean }> = {}) {
...
function ensurePublicNpcBusinessQueueServiceStock(
  b: HarthmereEconomyBusinessRecord,
  definition: NonNullable<ReturnType<typeof getHarthmereBusinessMiniGameDefinition>>,
...
function statsForCustomerBusiness(systems: HarthmereEconomyBusinessSystemsState, businessId: string) {
  systems.customerStats[businessId] = normalizeHarthmereBusinessCustomerStats(
    systems.customerStats[businessId] ?? defaultHarthmereBusinessCustomerStats(businessId),
    businessId,
  );
  return systems.customerStats[businessId];
}

const HARTHMERE_BUSINESS_AUTOMATION_ROLES = new Set<HarthmereEconomyBusinessAutomationRole>([
  "front_counter",
  "branch_manager",
  "courier_dispatch",
  "purchasing_manager",
...
function isBusinessAutomationRole(role: string): role is HarthmereEconomyBusinessAutomationRole {
  return HARTHMERE_BUSINESS_AUTOMATION_ROLES.has(role as HarthmereEconomyBusinessAutomationRole);
}
...
function openBusinessBranch(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "owner_admin");
  if (!b) return;
  ...
  const passabilityAudit = validateHarthmereBusinessOutpostPassability(outpostBuilding);
  ...
  const liveNavigationAudit = validateHarthmereBusinessOutpostLiveWorldNavigation(outpostBuilding);
...
function assignBusinessAutomation(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "employee_manager");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const systems = state.businessSystems!;
  const role = str(request.role, "");
  if (!isBusinessAutomationRole(role)) return reject(warnings, touched, "economy_rejected:invalid_business_automation_role");
...
function assignBusinessBranchManager(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "employee_manager");
  if (!b) return;
  const systems = state.businessSystems!;
  const branchId = str((request as any).branchId, "");
  ...
  employee.assignedTask = "branch_manager" satisfies HarthmereBusinessEmployeeAssignableTaskId;
...
function routeBusinessBranchStock(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function scheduleBusinessBranchStaff(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "employee_manager");
  if (!b) return;
  const systems = state.businessSystems!;
  const branchId = str((request as any).branchId, "");
...
function runBusinessEmpireDay(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "accountant");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const systems = state.businessSystems!;
  ...
  for (const branch of branches) {
    const elapsedDays = Math.max(1, Math.floor((request.nowMs - branch.lastSettlementAtMs) / HARTHMERE_BUSINESS_CUSTOMER_DAY_MS));
...
function closeBusinessBranch(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "owner_admin");
  if (!b) return;
  const systems = state.businessSystems!;
  const branchId = str((request as any).branchId, "");
...
function refreshBusinessEmployeeCandidates(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "employee_manager");
  if (!b) return;
  const systems = state.businessSystems!;
...
function interviewBusinessEmployeeCandidate(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "employee_manager");
  if (!b) return;
  const systems = state.businessSystems!;
  ...
  const style = str(request.interviewStyle, "friendly") as HarthmereBusinessEmployeeInterviewStyle;
  systems.employeeCandidates[candidateId] = interviewHarthmereBusinessEmployeeCandidate(candidate, style);
  touched.add("economy_business_employee_candidate");
  shared.add(systemsSharedKey("employee_candidate", candidateId));
}

function negotiateBusinessEmployeeCandidate(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "employee_manager");
  if (!b) return;
  const systems = state.businessSystems!;
  const candidateId = str(request.candidateId, "");
...
function hireBusinessEmployeeCandidate(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "employee_manager");
  if (!b) return;
  const systems = state.businessSystems!;
  const candidateId = str(request.candidateId, "");
...
function promoteBusinessEmployee(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "employee_manager");
  if (!b) return;
  const employeeId = str(request.employeeId, "");
  const employee = employeeId ? state.employees[employeeId] : undefined;
  ...
  const task = validateHarthmereBusinessEmployeeAssignedTask(str(request.assignedTask, employee.assignedTask ?? ""));
...
function runBusinessEmployeeTask(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "employee_manager");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const systems = state.businessSystems!;
  ...
  const employee = employeeId ? state.employees[employeeId] : undefined;
  if (!employee || employee.businessId !== b.businessId) return reject(warnings, touched, "economy_rejected:employee_not_found");
  const task = validateHarthmereBusinessEmployeeAssignedTask(str(request.assignedTask, employee.assignedTask ?? "front_counter"));
...
function runBusinessEmployeeMoraleTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "employee_manager");
  if (!b) return;
  ...
  for (const employeeId of [...b.employees]) {
    ...
    const unpaidDays = Math.max(0, Math.floor((request.nowMs - employee.lastPaidAtMs) / HARTHMERE_BUSINESS_CUSTOMER_DAY_MS) - 1);
    ...
    if (employee.morale < 25) {
      employee.assignedTask = "rest_required" satisfies HarthmereBusinessEmployeeAssignableTaskId;
...
function expireImpatientCustomerTickets(
  ...
  while (session.status === "active") {
    ...
    const elapsedPatience = Math.max(0, Math.floor((nowMs - ticket.arrivedAtMs) / HARTHMERE_BUSINESS_CUSTOMER_PATIENCE_STEP_MS));
...
function startBusinessCustomerSession(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = business(state, request.businessId);
  ...
  const restockedPublicServiceBoard =
    ensurePublicNpcBusinessQueueServiceStock(b, definition, queueResult.queue);
...
function serveBusinessCustomer(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, goldDelta: { value: number }, itemDeltas: Record<string, number>, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = business(state, request.businessId);
  if (!b) return reject(warnings, touched, "economy_rejected:business_not_found");
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const systems = state.businessSystems!;
  ...
  const definition = getHarthmereBusinessMiniGameDefinition(b.typeId);
  const currentTicketId = session.currentTicketId ?? activeHarthmereBusinessCustomerTicket(session)?.ticketId;
...
function runExoticRefinery(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function certifyPortalFuel(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "world_operator", "exotic_matter_refinery");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
...
function createContamination(state: BusinessSystemsEconomyState, b: HarthmereEconomyBusinessRecord, request: HarthmereEconomyMutationRequest, kind: string, severity: number, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const systems = state.businessSystems!;
...
function runBiomeDecayTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const systems = state.businessSystems!;
...
function performBiomeMaintenance(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function linkBusinessProperty(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function installBiomeDesign(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function createSecurityThreat(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function resolveSecurityThreat(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function buildPortalEndpoint(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function runPortalTransit(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "route_manager", "portal_transit_company");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
...
function plantCropNode(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function runCropGrowthTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const systems = state.businessSystems!;
  const days = Math.max(1, int(request.days, 1));
  let changed = 0;
...
function harvestCropNode(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "world_operator", "biome_farming_rare_foods");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
...
function runSpoilageTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  let spoiled = 0;
...
function repairDurableItem(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function upgradeDurableItem(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "world_operator", "weapons_tools");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
...
function registerDurableItem(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, _warnings: string[], touched: Set<string>, shared: Set<string>) {
  const systems = state.businessSystems!;
...
function craftMagicGood(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function installWard(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "world_operator", "magic_goods");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
...
function runUnstableMagicTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  let expired = 0;
...
function removeAnomaly(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "world_operator", "magic_goods");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
...
function discoverRoute(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function runMapAgingTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  let aged = 0;
...
function leadExpedition(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function startPropertyProject(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function advancePropertyProject(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "world_operator", "custom_home_property_development");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const property = state.businessSystems!.propertyIntegrations[str(request.propertyId, "")];
...
function refreshTraderInventory(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function performRegionalArbitrage(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function ensureAnimalPopulation(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest) {
  const systems = state.businessSystems!;
...
function huntWildlife(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function runWildlifeTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  let changed = 0;
...
function registerPatient(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function treatPatient(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function buildTeleportPad(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function issueTeleportAccessKey(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "route_manager", "teleport_owner");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
...
function useTeleportPad(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const pad = state.businessSystems!.teleportPads[str(request.padId, "")];
...
function runTeleportDestabilizationTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  let changed = 0;
...
function accumulateWaste(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function cleanupContaminationSite(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function runOutbreakPreventionTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  let activeRisk = 0;
...
function repairFixture(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function setRestaurantMenu(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function serveRestaurantDay(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function createDelivery(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function completeDelivery(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "contract_manager", "courier");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
...
function runDeliveryDeadlineTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  let expired = 0;
...
function createHospitalityState(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function runHospitalityDay(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function cleanHospitalityRooms(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "world_operator", "hospitality_inn_hotel_shelter");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const hosp = Object.values(state.businessSystems!.hospitality).find((entry) => entry.businessId === b.businessId);
...
function createShelterContract(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "contract_manager", "hospitality_inn_hotel_shelter");
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const hosp = Object.values(state.businessSystems!.hospitality).find((entry) => entry.businessId === b.businessId);
...
function runRentTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const systems = state.businessSystems!;
...
function runLoanDefaultTick(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  let defaults = 0;
...
function seizeLoanCollateral(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
...
function liquidateBankruptBusiness(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, context: BusinessSystemsContext, warnings: string[], touched: Set<string>, shared: Set<string>) {
  const b = requireBusiness(state, request, context, warnings, touched, "owner_admin");
  if (!b) return;
...
export function validateHarthmereEconomyBalance(state: HarthmereProductionEconomyState): string[] {
  ...
  const systems = normalizeHarthmereEconomyBusinessSystemsState((state as BusinessSystemsEconomyState).businessSystems);
  const serviceCatalogValidation = validateHarthmereBusinessServiceItemReferences();
  ...
  for (const outpostBuilding of Object.values(systems.outpostBuildings)) {
    ...
    const liveAudit = validateHarthmereBusinessOutpostLiveWorldNavigation(outpostBuilding);
...
function runBalanceValidation(state: BusinessSystemsEconomyState, request: HarthmereEconomyMutationRequest, _context: BusinessSystemsContext, warnings: string[], touched: Set<string>) {
