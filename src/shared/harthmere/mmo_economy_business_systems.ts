/*
 * mmo_economy_business_systems.ts
 *
...
import {
  applyHarthmereBusinessCozyServiceReward,
  activeHarthmereBusinessCustomerTicket,
  createHarthmereBusinessCozyServiceReward,
  ...
  harthmereBusinessCustomerTierForStats,
  ...
  validateHarthmereBusinessOutpostPassability,
  validateHarthmereBusinessServiceItemReferences,
...
import {
  ...
  type HarthmereBusinessEmployeeAssignableTaskId,
  type HarthmereBusinessEmployeeAutomationRole,
  type HarthmereBusinessEmployeeCandidate,
  type HarthmereBusinessEmployeeInterviewStyle,
  type HarthmereBusinessEmployeeTaskRun,
} from "./business_employee_ai";

export const HARTHMERE_ECONOMY_BUSINESS_SYSTEMS_VERSION =
  "harthmere-economy-business-systems" as const;

export type HarthmereEconomyBusinessPermission =
  | "employee_manager"
  | "accountant"
  | "inventory_manager"
  | "contract_manager"
...
export interface HarthmereEconomyBusinessBankAccount {
  accountId: string;
  businessId: string;
  ownerKind: string;
  ownerId: string;
  ...
  createdAtMs: number;
  audit: HarthmereEconomyBusinessAuditEntry[];
}

export interface HarthmereEconomyBusinessAuditEntry {
  auditId: string;
  atMs: number;
  actorId: string;
  businessId?: string;
...
export interface HarthmereEconomyPropertyIntegration {
  propertyId: string;
  businessId?: string;
  ownerKind?: string;
  ownerId?: string;
...
export interface HarthmereEconomyBiomeAnchorState {
  anchorId: string;
  propertyId: string;
  condition: number;
...
export interface HarthmereEconomyThreatState {
  threatId: string;
  townId: string;
  kind: string;
  severity: number;
...
export interface HarthmereEconomyPortalEndpoint {
  endpointId: string;
  businessId: string;
  originTownId: string;
...
export interface HarthmereEconomyTeleportPad {
  padId: string;
  businessId: string;
  locationId: string;
...
export interface HarthmereEconomyCropNode {
  cropId: string;
  businessId: string;
  townId: string;
  cropItemId: string;
...
export interface HarthmereEconomyAnimalPopulation {
  populationId: string;
  townId: string;
  species: string;
  count: number;
...
export interface HarthmereEconomyContaminationSite {
  siteId: string;
  townId: string;
  severity: number;
  kind: string;
...
export interface HarthmereEconomyPatientState {
  patientId: string;
  actorId?: string;
  townId: string;
...
export interface HarthmereEconomyDurableItemState {
...
export interface HarthmereEconomyExplorationRoute {
  routeId: string;
  businessId: string;
  originTownId: string;
...
export interface HarthmereEconomyDeliveryState {
  deliveryId: string;
...
export interface HarthmereEconomyHospitalityState {
...
export interface HarthmereEconomyBusinessServiceQuest {
  questId: string;
  contractId: string;
  businessId: string;
...
export type HarthmereEconomyBusinessAutomationRole =
  | "front_counter"
  | "branch_manager"
  | "courier_dispatch"
  | "purchasing_manager"
...
export interface HarthmereEconomyBusinessBranch {
  branchId: string;
...
export interface HarthmereEconomyBusinessBranchDashboard {
  dashboardId: string;
  branchId: string;
  parentBusinessId: string;
  atMs: number;
...
export interface HarthmereEconomyBusinessAutomation {
  automationId: string;
  businessId: string;
  branchId?: string;
  role: HarthmereEconomyBusinessAutomationRole;
  level: number;
...
export interface HarthmereEconomyBusinessSystemsState {
  version: typeof HARTHMERE_ECONOMY_BUSINESS_SYSTEMS_VERSION;
  permissions: Record<
    string,
    Record<string, HarthmereEconomyBusinessPermission[]>
  >;
  bankAccounts: Record<string, HarthmereEconomyBusinessBankAccount>;
  propertyIntegrations: Record<string, HarthmereEconomyPropertyIntegration>;
  biomeAnchors: Record<string, HarthmereEconomyBiomeAnchorState>;
  threats: Record<string, HarthmereEconomyThreatState>;
  portalEndpoints: Record<string, HarthmereEconomyPortalEndpoint>;
  teleportPads: Record<string, HarthmereEconomyTeleportPad>;
  cropNodes: Record<string, HarthmereEconomyCropNode>;
  animalPopulations: Record<string, HarthmereEconomyAnimalPopulation>;
  contaminationSites: Record<string, HarthmereEconomyContaminationSite>;
  patients: Record<string, HarthmereEconomyPatientState>;
  durableItems: Record<string, HarthmereEconomyDurableItemState>;
  explorationRoutes: Record<string, HarthmereEconomyExplorationRoute>;
  deliveries: Record<string, HarthmereEconomyDeliveryState>;
  hospitality: Record<string, HarthmereEconomyHospitalityState>;
  menuByBusiness: Record<string, string[]>;
  unstableMagicItems: Record<
    string,
    {
      businessId: string;
      itemId: string;
  ...
  >;
  serviceQuests: Record<string, HarthmereEconomyBusinessServiceQuest>;
  ...
  >;
  empireBranches: Record<string, HarthmereEconomyBusinessBranch>;
  branchDashboards: Record<string, HarthmereEconomyBusinessBranchDashboard>;
  automationAssignments: Record<string, HarthmereEconomyBusinessAutomation>;
...
export interface HarthmereEconomyBusinessSpecificMutationResult {
  handled: boolean;
...
type BusinessSystemsEconomyState = HarthmereProductionEconomyState & {
  businessSystems?: HarthmereEconomyBusinessSystemsState;
};

type BusinessSystemsContext = HarthmereEconomyMutationContext & {
  businessPermissions?: Record<string, HarthmereEconomyBusinessPermission[]>;
  actorGuildPermissions?: Record<string, HarthmereEconomyBusinessPermission[]>;
  actorTownPermissions?: Record<string, HarthmereEconomyBusinessPermission[]>;
  actorBankAccountPermissions?: Record<
    string,
    HarthmereEconomyBusinessPermission[]
  >;
};

function defaultBusinessSystemsState(
  nowMs = 0
): HarthmereEconomyBusinessSystemsState {
  void nowMs;
  return {
    version: HARTHMERE_ECONOMY_BUSINESS_SYSTEMS_VERSION,
    permissions: {},
    bankAccounts: {},
    ...
    customerSessions: {},
    customerStats: {},
    outpostBuildings: { ...HARTHMERE_BUSINESS_OUTPOST_PROCEDURAL_BUILDINGS },
    empireBranches: {},
    branchDashboards: {},
...
export function normalizeHarthmereEconomyBusinessSystemsState(
  raw: unknown
): HarthmereEconomyBusinessSystemsState {
  const defaults = defaultBusinessSystemsState();
  const value = (
    raw && typeof raw === "object" ? raw : {}
  ) as Partial<HarthmereEconomyBusinessSystemsState>;
  return {
    ...defaults,
    ...value,
    version: HARTHMERE_ECONOMY_BUSINESS_SYSTEMS_VERSION,
    ...
    customerStats: Object.fromEntries(
      Object.entries(
        ((value as any).customerStats ?? {}) as Record<string, unknown>
    ...
    balanceReports: Array.isArray(value.balanceReports)
      ? value.balanceReports.slice(-50)
      : [],
  };
}
...
): BusinessSystemsEconomyState {
  const cloned = JSON.parse(
    JSON.stringify(state ?? {})
  ) as BusinessSystemsEconomyState;
  cloned.businessSystems = normalizeHarthmereEconomyBusinessSystemsState(
    cloned.businessSystems
  );
  return cloned;
}
...
function itemCount(inventory: HarthmereEconomyInventoryRecord, itemId: string) {
...
function systemsSharedKey(kind: string, id: string) {
  return `harthmere:live_mode:current:economy_business_system:${kind}:${id}`;
}
...
function businessIsType(
  b: HarthmereEconomyBusinessRecord,
...
function canManageBusinessSpecific(
  ...
  permission: HarthmereEconomyBusinessPermission
) {
  if (b.ownerKind === "player" && b.ownerId === request.actorId) return true;
  if (b.ownerKind === "npc" && context.allowNpcAdministration === true)
    return true;
  if (b.ownerKind === "guild") {
    if (context.canManageGuildBusiness?.(b.ownerId) === true) return true;
    if ((context.actorGuildPermissions?.[b.ownerId] ?? []).includes(permission))
      return true;
  }
  if (b.ownerKind === "town") {
    if (context.canManageTownBusiness?.(b.ownerId) === true) return true;
    if ((context.actorTownPermissions?.[b.ownerId] ?? []).includes(permission))
      return true;
  }
  const direct =
    context.businessPermissions?.[`${b.businessId}:${request.actorId}`] ?? [];
...
function requireBusiness(
  ...
  if (typeId && !businessIsType(b, typeId)) {
    reject(
      warnings,
      touched,
      `economy_rejected:business_type_required:${typeId}`
    );
    return undefined;
  }
  if (!canManageBusinessSpecific(b, request, context, permission)) {
    reject(
      warnings,
      touched,
      `economy_rejected:business_permission_required:${permission}`
    );
    return undefined;
  }
  return b;
  ...
  if (!state.towns[townId]) {
    const needs = Object.fromEntries(
      [
        ...
        need,
        { value: 65, demandWeight: 1, lastUpdatedAtMs: nowMs },
      ])
    );
    state.towns[townId] = {
      townId,
      ...
      serviceCoverage: Object.fromEntries(
        Object.keys(needs).map((need) => [need, 0])
      ) as any,
      lastTickAtMs: nowMs,
    };
  }
  if (!state.regions[regionId].towns.includes(townId))
    state.regions[regionId].towns.push(townId);
  return state.towns[townId] as any;
}

function addNeed(
  state: BusinessSystemsEconomyState,
  businessRecord: HarthmereEconomyBusinessRecord,
  need: string,
  delta: number,
  nowMs: number
) {
  const townId =
    businessRecord.townId ??
    str((businessRecord as any).townId, "harthmere_grove");
  ...
  town.needs[need] = {
    ...current,
    value: clamp(current.value + delta, 0, 100, current.value),
    lastUpdatedAtMs: nowMs,
  };
  town.cleanlinessRating =
    town.needs.sanitation?.value ?? town.cleanlinessRating;
  town.safetyRating = town.needs.safety?.value ?? town.safetyRating;
  town.happiness = Math.round(
    ((town.needs.food?.value ?? 65) +
      (town.needs.housing?.value ?? 65) +
      (town.needs.health?.value ?? 65) +
      (town.needs.safety?.value ?? 65) +
      (town.needs.tourism?.value ?? 65)) /
      5
  );
  return town;
}
...
function audit(
  ...
  account: HarthmereEconomyBusinessBankAccount | undefined,
  request: HarthmereEconomyMutationRequest,
  kind: string,
  values: Partial<HarthmereEconomyBusinessAuditEntry>
) {
  const entry: HarthmereEconomyBusinessAuditEntry = {
...
function accountForBusiness(
  systems: HarthmereEconomyBusinessSystemsState,
  businessId: string
) {
  return Object.values(systems.bankAccounts).find(
    (account) =>
      account.businessId === businessId && account.status === "active"
  );
}
...
function createBankAccount(
  ...
  if (accountForBusiness(systems, b.businessId))
    return reject(
      warnings,
      touched,
      "economy_rejected:business_bank_account_already_exists"
    );
  ...
  audit(
    ...
    { businessId: b.businessId, after: 0 }
  );
  touched.add("economy_business_bank_account");
  shared.add(businessSharedKey(b.businessId));
}

function transferPersonalToBusinessBank(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  goldDelta: { value: number },
  warnings: string[],
  touched: Set<string>,
  shared: Set<string>
) {
  ...
  if (!account)
    return reject(
      warnings,
      touched,
      "economy_rejected:business_bank_account_required"
    );
  ...
  if (amount <= 0)
    return reject(
      warnings,
      touched,
      "economy_rejected:invalid_bank_transfer_amount"
    );
  if ((context.actorGold ?? 0) < amount)
    return reject(
      warnings,
      touched,
      "economy_rejected:insufficient_personal_gold_for_transfer"
    );
  const before = account.balanceGold;
  account.balanceGold += amount;
  b.balanceGold += amount;
  goldDelta.value -= amount;
  audit(systems, account, request, "personal_to_business_bank", {
    businessId: b.businessId,
    amountGold: amount,
    before,
    after: account.balanceGold,
  });
  touched.add("economy_business_bank_account");
  shared.add(businessSharedKey(b.businessId));
}

function transferBusinessToPersonalBank(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  goldDelta: { value: number },
  ...
  if (b.balanceGold < amount)
    return reject(
      warnings,
      touched,
      "economy_rejected:business_bank_funds_insufficient"
    );
  const before = account.balanceGold;
  b.balanceGold -= amount;
  account.balanceGold = Math.max(0, account.balanceGold - amount);
  goldDelta.value += amount;
  audit(systems, account, request, "business_to_personal_bank", {
    businessId: b.businessId,
    amountGold: -amount,
    before,
    after: account.balanceGold,
...
function grantBusinessPermission(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  const permissions = [...new Set(requestedPermissions)].filter(
    Boolean
  ) as HarthmereEconomyBusinessPermission[];
  if (
    !targetActorId ||
    permissions.length === 0 ||
    permissions.some((permission) => !valid.includes(permission))
  )
    return reject(
      warnings,
      touched,
      "economy_rejected:invalid_business_permission_grant"
    );
  const systems = state.businessSystems!;
  systems.permissions[b.businessId] ??= {};
  systems.permissions[b.businessId][targetActorId] ??= [];
  for (const permission of permissions) {
    if (!systems.permissions[b.businessId][targetActorId].includes(permission))
      systems.permissions[b.businessId][targetActorId].push(permission);
  }
  touched.add("economy_business_permissions");
  shared.add(businessSharedKey(b.businessId));
}

function adjustBusinessFunds(
  b: HarthmereEconomyBusinessRecord,
  amount: number
) {
  ...
  for (const [itemId, count] of Object.entries(items)) {
    if (itemCount(b.inventory, itemId) < count) {
      reject(
        warnings,
        touched,
        `economy_rejected:business_item_required:${itemId}`
      );
      return false;
    }
  }
...
function produceInventory(
  ...
  for (const [itemId, count] of Object.entries(items))
    applyItem(b.inventory, itemId, count, extras);
}

function isPublicNpcServiceBusiness(
  b: HarthmereEconomyBusinessRecord
): boolean {
  return b.ownerKind === "npc" || b.ownerKind === "town";
}

function ensurePublicNpcBusinessServiceStock(
  b: HarthmereEconomyBusinessRecord,
  requiredItems: Record<string, number>
): boolean {
  if (!isPublicNpcServiceBusiness(b)) return false;
  let changed = false;
...
function ensurePublicNpcBusinessQueueServiceStock(
  b: HarthmereEconomyBusinessRecord,
  definition: NonNullable<
    ReturnType<typeof getHarthmereBusinessMiniGameDefinition>
  >,
  queue: HarthmereBusinessCustomerSession["queue"]
): boolean {
  ...
  for (const ticket of queue) {
    ...
    for (const [itemId, rawCount] of Object.entries(
      ...
      const count = Math.max(0, Math.trunc(Number(rawCount) || 0));
      if (count > 0)
        requiredItems[itemId] = (requiredItems[itemId] ?? 0) + count;
    }
  }
  return ensurePublicNpcBusinessServiceStock(b, requiredItems);
}

const HARTHMERE_BUSINESS_CUSTOMER_SESSION_MS = 2 * 60 * 60 * 1000;
const HARTHMERE_BUSINESS_CUSTOMER_DAY_MS = 24 * 60 * 60 * 1000;
const HARTHMERE_BUSINESS_CUSTOMER_PATIENCE_STEP_MS = 1000;

function customerDay(nowMs: number) {
  return Math.floor(nowMs / HARTHMERE_BUSINESS_CUSTOMER_DAY_MS);
}

function statsForCustomerBusiness(
  systems: HarthmereEconomyBusinessSystemsState,
  businessId: string
) {
  systems.customerStats[businessId] = normalizeHarthmereBusinessCustomerStats(
    systems.customerStats[businessId] ??
...
const HARTHMERE_BUSINESS_AUTOMATION_ROLES =
  new Set<HarthmereEconomyBusinessAutomationRole>([
    "front_counter",
    "branch_manager",
    "courier_dispatch",
    "purchasing_manager",
...
function isBusinessAutomationRole(
  role: string
): role is HarthmereEconomyBusinessAutomationRole {
  return HARTHMERE_BUSINESS_AUTOMATION_ROLES.has(
    role as HarthmereEconomyBusinessAutomationRole
  );
}
...
function activeBranchForBusiness(
  ...
  return branch &&
    branch.parentBusinessId === businessId &&
    branch.status === "active"
    ? branch
    : undefined;
}

function branchWarehouseUnits(branch: HarthmereEconomyBusinessBranch) {
  return Object.values(branch.warehouseInventory ?? {}).reduce(
    (sum, count) => sum + Math.max(0, Math.trunc(count)),
    0
  );
}

function createBranchDashboard(
  branch: HarthmereEconomyBusinessBranch,
  dailyProfitGold: number,
  nowMs: number
): HarthmereEconomyBusinessBranchDashboard {
  const stockUnits = branchWarehouseUnits(branch);
  const staffCoverage = Math.min(
    1,
    branch.scheduledStaffIds.length / Math.max(1, branch.staffSlots)
  );
  const alerts: string[] = [];
...
function openBusinessBranch(
  ...
  if (!outpostBuilding)
    return reject(
      warnings,
      touched,
      "economy_rejected:business_outpost_building_required"
    );
  const passabilityAudit =
    validateHarthmereBusinessOutpostPassability(outpostBuilding);
  if (!passabilityAudit.ok) {
    return reject(
      warnings,
      touched,
      `economy_rejected:business_outpost_passability_failed:${passabilityAudit.errors[0] ?? "unknown"}`
    );
  }
  const liveNavigationAudit =
    validateHarthmereBusinessOutpostLiveWorldNavigation(outpostBuilding);
  if (!liveNavigationAudit.ok) {
    return reject(
      warnings,
      touched,
      `economy_rejected:business_outpost_live_navigation_failed:${liveNavigationAudit.unreachableRoutes[0] ?? liveNavigationAudit.unresolvedCollisions[0] ?? "unknown"}`
    );
  }
  ...
  if (existing)
    return reject(
      warnings,
      touched,
      "economy_rejected:business_branch_outpost_already_claimed"
    );
  const baseOpenCostGold = 600 + tier * 150;
  const openCostGold = Math.max(
    baseOpenCostGold,
    int(request.amountGold, baseOpenCostGold)
  );
  if (b.balanceGold < openCostGold)
    return reject(
      warnings,
      touched,
      "economy_rejected:business_branch_funds_insufficient"
    );
  const branchId =
    str((request as any).branchId, "") ||
    `business_branch_${systems.nextBranchNumber++}`;
  if (systems.empireBranches[branchId])
    return reject(
      warnings,
      touched,
      "economy_rejected:business_branch_id_exists"
    );
  ...
  systems.empireBranches[branchId] = {
    ...
    townId:
      outpostBuilding.plot.area === "harthmere"
        ? "harthmere_town"
        : "harthmere_grove",
    regionId: b.regionId,
    status: "active",
    ...
    dailyUpkeepGold: Math.max(
      35,
      Math.round(b.upkeepGoldPerDay * 0.55) + tier * 12
    ),
    ...
    branchNotes: [
      "Branch opened with an empty warehouse and no regional manager assigned.",
    ],
  };
  b.serviceRadius += 1;
  ...
  (state as any).__buildingMaterializationPlans.push(
    outpostBuilding.materializationPlan
  );
}

function assignBusinessAutomation(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!isBusinessAutomationRole(role))
    return reject(
      warnings,
      touched,
      "economy_rejected:invalid_business_automation_role"
    );
  ...
  if (
    branchId &&
    (!branch ||
      branch.parentBusinessId !== b.businessId ||
      branch.status !== "active")
  ) {
    return reject(
      warnings,
      touched,
      "economy_rejected:active_business_branch_required"
    );
  }
  if (
    branch &&
    automationsForBranch(systems, branch.branchId).length >=
      branch.automationSlots
  ) {
    return reject(
      warnings,
      touched,
  ...
  if (employeeId && (!employee || employee.businessId !== b.businessId)) {
    return reject(
      warnings,
      touched,
      "economy_rejected:business_employee_required_for_automation"
    );
  }
  const level = Math.max(
    1,
    Math.min(5, int(request.skill, employee?.skill ?? 1))
  );
  const automationId =
    str((request as any).automationId, "") ||
    `business_automation_${systems.nextAutomationNumber++}`;
  if (systems.automationAssignments[automationId])
    return reject(
      warnings,
      touched,
      "economy_rejected:business_automation_id_exists"
    );
  systems.automationAssignments[automationId] = {
    ...
    serviceCapacityBonus:
      role === "front_counter" || role === "branch_manager"
        ? 1 + level
        : Math.max(1, Math.floor(level / 2)),
    passiveProfitGoldPerDay:
...
function assignBusinessBranchManager(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  const employeeId = str(request.employeeId, "");
  const employee = employeeId ? state.employees[employeeId] : undefined;
  if (!employee || employee.businessId !== b.businessId)
    return reject(
      warnings,
      touched,
      "economy_rejected:business_employee_required_for_branch_manager"
    );
  if (employee.skill < 3)
    return reject(
      warnings,
      touched,
      "economy_rejected:branch_manager_skill_too_low"
    );
  branch.regionalManagerEmployeeId = employee.employeeId;
  branch.regionalDemandMultiplier = Number(
    Math.min(
      1.5,
      (branch.regionalDemandMultiplier || 1) + 0.08 + employee.skill * 0.01
    ).toFixed(2)
  );
  branch.competitorPressure = Math.max(0, branch.competitorPressure - 3);
  branch.branchNotes = [
    ...(branch.branchNotes ?? []),
    `${employee.role} assigned as regional manager.`,
  ].slice(-20);
  employee.assignedTask =
    "branch_manager" satisfies HarthmereBusinessEmployeeAssignableTaskId;
...
function routeBusinessBranchStock(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!itemId)
    return reject(
      warnings,
      touched,
      "economy_rejected:branch_stock_item_required"
    );
  if (itemCount(b.inventory, itemId) < count)
    return reject(
      warnings,
  ...
  if (branchWarehouseUnits(branch) + count > branch.warehouseSlots)
    return reject(warnings, touched, "economy_rejected:branch_warehouse_full");
  consumeInventory(b, { [itemId]: count });
  branch.warehouseInventory = branch.warehouseInventory ?? {};
  branch.warehouseInventory[itemId] =
    (branch.warehouseInventory[itemId] ?? 0) + count;
  branch.branchNotes = [
    ...(branch.branchNotes ?? []),
    `Routed ${count} ${itemId.replace(/_/g, " ")} to the branch warehouse.`,
  ].slice(-20);
...
function scheduleBusinessBranchStaff(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!employeeIds.length)
    return reject(warnings, touched, "economy_rejected:branch_staff_required");
  if (employeeIds.length > branch.staffSlots)
    return reject(
      warnings,
      touched,
  ...
  for (const employeeId of employeeIds) {
    ...
    if (!employee || employee.businessId !== b.businessId)
      return reject(
        warnings,
        touched,
        "economy_rejected:business_employee_required_for_branch_schedule"
      );
    if (employee.injuredUntilMs && employee.injuredUntilMs > request.nowMs)
      return reject(
        warnings,
        touched,
        "economy_rejected:branch_staff_unavailable"
      );
  }
  branch.scheduledStaffIds = employeeIds;
  branch.branchNotes = [
    ...(branch.branchNotes ?? []),
    `Scheduled ${employeeIds.length} staff for the next branch day.`,
  ].slice(-20);
...
function runBusinessEmpireDay(
  ...
  const branches = Object.values(systems.empireBranches).filter(
    (branch) =>
      branch.parentBusinessId === b.businessId && branch.status === "active"
  );
  if (branches.length === 0)
    return reject(
      warnings,
      touched,
      "economy_rejected:business_branch_required_for_empire_settlement"
    );
  ...
  for (const branch of branches) {
    const elapsedDays = Math.max(
      1,
      Math.floor(
        (request.nowMs - branch.lastSettlementAtMs) /
          HARTHMERE_BUSINESS_CUSTOMER_DAY_MS
      )
    );
    ...
    const activeAutomationProfit = branchAutomations.reduce(
      (sum, automation) => sum + automation.passiveProfitGoldPerDay,
      0
    );
    const activeAutomationUpkeep = branchAutomations.reduce(
      (sum, automation) => sum + automation.dailyUpkeepGold,
      0
    );
    const capacityBonus = branchAutomations.reduce(
      (sum, automation) => sum + automation.serviceCapacityBonus,
      0
    );
    const stockUnits = branchWarehouseUnits(branch);
    ...
    const staffCoverage = Math.min(
      1,
      (branch.scheduledStaffIds?.length ?? 0) / Math.max(1, branch.staffSlots)
    );
    const managerBonus = branch.regionalManagerEmployeeId ? 30 : 0;
    const demandMultiplier = Math.max(
      0.65,
      Math.min(1.6, branch.regionalDemandMultiplier || 1)
    );
    const competitorPenalty = Math.max(0, branch.competitorPressure || 0) * 3;
    const dayProfit = Math.round(
      (branch.dailyRevenueGold +
        activeAutomationProfit +
        (branch.queueCapacityBonus + capacityBonus) * 5 +
        stockBonus +
        managerBonus) *
        demandMultiplier *
    ...
    totalCapacity +=
      Math.max(0, branch.queueCapacityBonus + capacityBonus) * days;
    branch.lifetimeProfitGold += branchProfit;
    branch.lastSettlementAtMs = request.nowMs;
    branch.lastDashboardAtMs = request.nowMs;
    branch.competitorPressure = Math.max(
      0,
      Math.min(
        25,
        (branch.competitorPressure ?? 0) +
          (branchProfit > 0 ? -1 : 2) -
          (branch.regionalManagerEmployeeId ? 1 : 0)
      )
    );
    branch.regionalDemandMultiplier = Number(
      Math.max(
        ...
        Math.min(
          1.6,
          (branch.regionalDemandMultiplier ?? 1) +
            (branchProfit > 0 ? 0.01 : -0.03)
        )
      ).toFixed(2)
    );
    const firstStockItem = Object.keys(branch.warehouseInventory ?? {}).find(
      (itemId) => (branch.warehouseInventory[itemId] ?? 0) > 0
    );
    if (firstStockItem) {
      branch.warehouseInventory[firstStockItem] = Math.max(
        0,
        (branch.warehouseInventory[firstStockItem] ?? 0) -
          Math.min(days, branch.warehouseInventory[firstStockItem] ?? 0)
      );
      if (branch.warehouseInventory[firstStockItem] <= 0)
        delete branch.warehouseInventory[firstStockItem];
    }
    const dashboard = createBranchDashboard(branch, dayProfit, request.nowMs);
    systems.branchDashboards[branch.branchId] = dashboard;
    branch.branchNotes = [
      ...(branch.branchNotes ?? []),
      `Daily branch report: ${dayProfit} gold, ${dashboard.stockUnits} stock units, ${Math.round(dashboard.staffCoverage * 100)}% staff coverage.`,
    ].slice(-20);
    for (const automation of branchAutomations)
      automation.lastRunAtMs = request.nowMs;
    shared.add(systemsSharedKey("empire_branch", branch.branchId));
    shared.add(systemsSharedKey("branch_dashboard", branch.branchId));
  }
  const standaloneAutomations = Object.values(
    systems.automationAssignments
  ).filter(
    (automation) =>
      automation.businessId === b.businessId &&
      automation.active &&
      !automation.branchId
  );
  const standaloneProfit = standaloneAutomations.reduce(
    (sum, automation) =>
      sum + automation.passiveProfitGoldPerDay - automation.dailyUpkeepGold,
    0
  );
  totalProfit += standaloneProfit;
  for (const automation of standaloneAutomations)
    automation.lastRunAtMs = request.nowMs;
  ...
  stats.currentTier = harthmereBusinessCustomerTierForStats(stats);
...
function closeBusinessBranch(
  ...
  const branchId = str((request as any).branchId, "");
  const branch = systems.empireBranches[branchId];
  if (!branch || branch.parentBusinessId !== b.businessId)
    return reject(
      warnings,
      touched,
      "economy_rejected:business_branch_not_found"
    );
  ...
  const saleValue = Math.max(
    50,
    Math.floor(
      (branch.dailyRevenueGold + branch.queueCapacityBonus * 10) * 0.4
    ) + stockRefund
  );
  ...
  branch.branchNotes = [
    ...(branch.branchNotes ?? []),
    `Branch closed and assets recovered for ${saleValue} gold.`,
  ].slice(-20);
...
function requestBlockedCells(request: HarthmereEconomyMutationRequest) {
    ...
    return [
      {
        x: Math.trunc(x),
        y: Math.trunc(y),
        reason: str(cell?.reason, "blocked"),
      },
    ];
  });
}

function refreshBusinessEmployeeCandidates(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  const generated = generateHarthmereBusinessEmployeeCandidates({
    businessId: b.businessId,
    typeId: b.typeId,
    nowMs: request.nowMs,
...
function requireEmployeeCandidate(
  ...
  if (!candidate || candidate.businessId !== businessId) {
    reject(
      warnings,
      touched,
      "economy_rejected:business_employee_candidate_not_found"
    );
    return undefined;
  }
  if (candidate.expiresAtMs <= request.nowMs && candidate.status !== "hired") {
    ...
    reject(
      warnings,
      touched,
      "economy_rejected:business_employee_candidate_expired"
    );
    return undefined;
  }
  return candidate;
...
function interviewBusinessEmployeeCandidate(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  const candidate = requireEmployeeCandidate(
    systems,
    b.businessId,
    candidateId,
    request,
  ...
  if (candidate.status === "hired" || candidate.status === "withdrawn") {
    return reject(
      warnings,
      touched,
      "economy_rejected:business_employee_candidate_unavailable"
    );
  }
  const style = str(
    request.interviewStyle,
    "friendly"
  ) as HarthmereBusinessEmployeeInterviewStyle;
  systems.employeeCandidates[candidateId] =
    interviewHarthmereBusinessEmployeeCandidate(candidate, style);
  touched.add("economy_business_employee_candidate");
  shared.add(systemsSharedKey("employee_candidate", candidateId));
}

function negotiateBusinessEmployeeCandidate(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (
    ...
    return reject(
      warnings,
      touched,
      "economy_rejected:business_employee_candidate_not_negotiable"
    );
  }
  const result = negotiateHarthmereBusinessEmployeeCandidate(
    candidate,
    int(request.wageGoldPerDay, candidate.wageAskGoldPerDay),
    context.actorSkillLevels?.persuasion ?? 1
  );
...
function hireBusinessEmployeeCandidate(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (candidate.status !== "offer_made" || !candidate.acceptedWageGoldPerDay) {
    return reject(
      warnings,
      touched,
      "economy_rejected:business_employee_candidate_offer_required"
    );
  }
  if (b.employees.length >= 30)
    return reject(
      warnings,
      touched,
  ...
  const employeeId =
    str(request.employeeId, "") ||
    `econ_employee_${state.nextEmployeeNumber++}`;
  if (state.employees[employeeId])
    return reject(
      warnings,
      touched,
      "economy_rejected:employee_already_exists"
    );
  state.employees[employeeId] = {
    ...
    morale: Math.max(
      50,
      Math.min(85, 45 + Math.floor((candidate.interviewScore ?? 50) / 3))
    ),
    loyalty: Math.max(
      40,
      Math.min(80, 35 + (candidate.status === "offer_made" ? 15 : 0))
    ),
...
function promoteBusinessEmployee(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  const employee = employeeId ? state.employees[employeeId] : undefined;
  if (!employee || employee.businessId !== b.businessId)
    return reject(warnings, touched, "economy_rejected:employee_not_found");
  const cost = Math.max(40, employee.skill * 35);
  if (b.balanceGold < cost)
    return reject(
      warnings,
      touched,
      "economy_rejected:business_employee_promotion_unfunded"
    );
  const task = validateHarthmereBusinessEmployeeAssignedTask(
    str(request.assignedTask, employee.assignedTask ?? "")
  );
  if (request.assignedTask && !task)
    return reject(
      warnings,
      touched,
      "economy_rejected:invalid_business_employee_task"
    );
...
function runBusinessEmployeeTask(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  const employeeId =
    requestedEmployeeId || b.employees.find((id) => state.employees[id]) || "";
  const employee = employeeId ? state.employees[employeeId] : undefined;
  if (!employee || employee.businessId !== b.businessId)
    return reject(warnings, touched, "economy_rejected:employee_not_found");
  const task = validateHarthmereBusinessEmployeeAssignedTask(
    str(request.assignedTask, employee.assignedTask ?? "front_counter")
  );
  if (!task)
    return reject(
      warnings,
  ...
  const role = str(request.role, "");
  const automationRole = isBusinessAutomationRole(role)
    ? (role as HarthmereBusinessEmployeeAutomationRole)
    : undefined;
  const taskRunId =
    str(request.taskRunId, "") ||
    `business_employee_task_${systems.nextEmployeeTaskRunNumber++}`;
  if (systems.employeeTaskRuns[taskRunId])
    return reject(
      warnings,
      touched,
      "economy_rejected:business_employee_task_id_exists"
    );
  ...
  const allRuns = Object.values(systems.employeeTaskRuns).sort(
    (a, bRun) => a.createdAtMs - bRun.createdAtMs
  );
  ...
  if (run.status === "completed") {
    ...
    b.customerSatisfaction = clamp(
      b.customerSatisfaction + 1,
      0,
      100,
      b.customerSatisfaction
    );
  } else if (run.status === "recovered") {
    b.flags.employee_stuck_recovery_used =
      run.pathAudit.sidestepCount > 0 ||
      run.pathAudit.repathCount > 0 ||
      run.pathAudit.fallbackExitUsed;
  } else {
    employee.loyalty = clamp(employee.loyalty - 2, 0, 100, employee.loyalty);
    b.customerSatisfaction = clamp(
      b.customerSatisfaction - 3,
      0,
      100,
      b.customerSatisfaction
    );
    b.reputation = Math.max(0, b.reputation - 1);
    if (run.failureReason === "employee_morale_too_low")
      b.flags.employee_morale_failure = true;
  }
  for (const warning of run.warnings)
    warnings.push(`economy_warning:${warning}`);
...
function runBusinessEmployeeMoraleTick(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  for (const employeeId of [...b.employees]) {
    ...
    const unpaidDays = Math.max(
      0,
      Math.floor(
        (request.nowMs - employee.lastPaidAtMs) /
          HARTHMERE_BUSINESS_CUSTOMER_DAY_MS
      ) - 1
    );
    employee.morale = clamp(
      employee.morale - days - unpaidDays * 4,
      0,
      100,
      employee.morale
    );
    if (employee.morale < 12 && employee.loyalty < 25) {
      ...
      b.wageGoldPerDay = Math.max(
        0,
        b.wageGoldPerDay - employee.wageGoldPerDay
      );
    ...
    if (employee.morale < 25) {
      employee.assignedTask =
        "rest_required" satisfies HarthmereBusinessEmployeeAssignableTaskId;
      b.customerSatisfaction = clamp(
        b.customerSatisfaction - 2,
        0,
        100,
        b.customerSatisfaction
      );
    ...
    if (employee.loyalty < 15 && b.balanceGold > 0) {
      ...
      warnings.push(
        `economy_warning:employee_theft_risk_loss:${employeeId}:${loss}`
      );
    }
  }
  touched.add("economy_employee");
...
function advanceCustomerSession(
  session: HarthmereBusinessCustomerSession,
  nowMs?: number
) {
  ...
  if (typeof nowMs === "number") {
    ...
    next.patienceStartedAtMs = undefined;
...
function expireImpatientCustomerTickets(
  ...
  while (session.status === "active") {
    const ticket = activeHarthmereBusinessCustomerTicket(session);
    if (!ticket) {
      advanceCustomerSession(session, nowMs);
      break;
    }
    if (nativeCustomerReady) {
      // Native readiness is exact-session, exact-ticket, and position-bounded.
      // Persist it before starting patience so the countdown and service UI
      // can never disagree about whether this customer reached the counter.
      ticket.spatialPhase = "serving";
      ticket.reaction = "neutral";
    }
    if (ticket.patienceStartedAtMs === undefined) {
      if (!nativeCustomerReady) break;
      ticket.patienceStartedAtMs = nowMs;
      ticket.patienceRemaining = ticket.patience;
      break;
    }
    const elapsedPatience = Math.max(
      0,
      Math.floor(
        (nowMs - ticket.patienceStartedAtMs) /
          HARTHMERE_BUSINESS_CUSTOMER_PATIENCE_STEP_MS
      )
    );
    ...
    session.streak = 0;
    session.satisfaction = clamp(
      session.satisfaction - 10,
      0,
      100,
      session.satisfaction
    );
    stats.totalFailed += 1;
    business.customerSatisfaction = clamp(
    ...
    const npc = findHarthmereBusinessCustomerNpc(ticket.npcId);
    session.notes.push(
      `${npc?.displayName ?? "A customer"} left after waiting too long.`
    );
...
function startBusinessCustomerSession(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  const b = business(state, request.businessId);
  if (!b)
    return reject(warnings, touched, "economy_rejected:business_not_found");
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  ...
  if (!definition)
    return reject(
      warnings,
      touched,
      `economy_rejected:business_customer_minigame_missing:${b.typeId}`
    );
  const systems = state.businessSystems!;
  ...
  const requestedCount = Math.max(
    ...
    Math.min(
      HARTHMERE_BUSINESS_CUSTOMERS_PER_SHIFT,
      int(request.count, HARTHMERE_BUSINESS_CUSTOMERS_PER_SHIFT)
    )
  );
  const requestedSessionId = str(request.sessionId, "");
  const sessionId =
    requestedSessionId ||
    `business_customer_session_${systems.nextCustomerSessionNumber++}`;
  if (systems.customerSessions[sessionId])
    return reject(
      warnings,
      touched,
      "economy_rejected:business_customer_session_id_exists"
    );
  const queueResult = createHarthmereBusinessCustomerQueue({
    businessId: b.businessId,
    typeId: b.typeId,
    sessionId,
    actorId: request.actorId,
  ...
  const restockedPublicServiceBoard = ensurePublicNpcBusinessQueueServiceStock(
    b,
    definition,
    queueResult.queue
  );
  const firstTicket = queueResult.queue[0];
  for (const ticket of queueResult.queue) {
    ticket.patienceRemaining = ticket.patience;
  }
...
function serveBusinessCustomer(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  goldDelta: { value: number },
  ...
  const stats = statsForCustomerBusiness(systems, b.businessId);
  const leftTicketIds = expireImpatientCustomerTickets(
    session,
    b,
    stats,
    request.nowMs,
  ...
  if (ticketId && currentTicketId && ticketId !== currentTicketId)
    return reject(
      warnings,
      touched,
  ...
  if (!ticket || ticket.status !== "waiting")
    return reject(
      warnings,
      touched,
      "economy_rejected:waiting_business_customer_not_found"
    );
  if (
  ...
  if (!offer)
    return reject(
      warnings,
      touched,
      "economy_rejected:business_customer_offer_not_found"
    );
  for (const itemId of Object.keys({
    ...offer.requiredItems,
    ...(offer.producedItems ?? {}),
  })) {
    if (!getHarthmereBusinessServiceItemDefinition(itemId)) {
      return reject(
        warnings,
        touched,
        `economy_rejected:business_customer_item_not_in_catalog:${itemId}`
      );
    }
  }
  ...
  if (!matched) {
    ...
    b.reputation = Math.max(0, b.reputation - 1);
    session.notes.push(
      `${npc?.displayName ?? "A customer"} left unhappy after receiving the wrong service.`
    );
  ...
  const minigameAction =
    ...
    !Array.isArray(request.minigameAction)
      ? (request.minigameAction as HarthmereBusinessMiniGameDecision)
      : undefined;
  if (request.minigameAction !== undefined && !minigameAction) {
    return reject(
      warnings,
      touched,
      "economy_rejected:business_customer_minigame_action_invalid"
    );
  }
  if (minigameAction) {
    const minigameResult = resolveHarthmereBusinessMiniGameDecision({
      typeId: b.typeId,
      offerId,
    ...
    if (!minigameResult.ok) {
      ...
      b.reputation = Math.max(0, b.reputation - 1);
      warnings.push(
        `economy_business_customer_minigame_rule_failed:${minigameResult.failedRules[0] ?? "unknown"}`
      );
      session.notes.push(
    ...
    session.notes.push(
      `${definition.mechanicSpec.gameTitle}: ${minigameResult.passedRules.length} rules verified.`
    );
  }

  if (ensurePublicNpcBusinessServiceStock(b, offer.requiredItems)) {
  ...
  session.satisfaction = clamp(
    session.satisfaction + offer.satisfactionDelta,
    0,
    100,
    session.satisfaction
  );
  ...
  stats.currentTier = harthmereBusinessCustomerTierForStats(stats);
  const cozyReward = createHarthmereBusinessCozyServiceReward({
    businessId: b.businessId,
    typeId: b.typeId,
    npcId: ticket.npcId,
  ...
  applyHarthmereBusinessCozyServiceReward(stats, ticket.npcId, cozyReward);
  // Hand collectible drops to the player inventory so earned items appear in the HUD.
  if (cozyReward.collectibleId)
    recordDelta(itemDeltas, cozyReward.collectibleId, 1);
  b.customerSatisfaction = clamp(
    b.customerSatisfaction + offer.satisfactionDelta,
    0,
    100,
    b.customerSatisfaction
  );
  b.reputation += ticket.reputationDelta;
  b.flags.customer_service_daily_bonus_claimed =
    dailyBonus > 0 || b.flags.customer_service_daily_bonus_claimed === true;
  session.notes.push(
    `${npc?.displayName ?? "Customer"}: ${offer.interactionVerb} complete for ${rewardGold} gold.`
  );
  session.notes.push(
    `Cozy reward: +${cozyReward.serviceXp} service XP, +${cozyReward.likeabilityDelta} likeability.`
  );
  if (cozyReward.collectibleId)
    session.notes.push(
      `Collectible earned: ${cozyReward.collectibleId.replace(/_/g, " ")}.`
    );
  if (cozyReward.decorationUnlockId)
    session.notes.push(
      `Decor unlocked: ${cozyReward.decorationUnlockId.replace(/_/g, " ")}.`
    );
  if (cozyReward.badgeId)
    session.notes.push(
      `Badge earned: ${cozyReward.badgeId.replace(/_/g, " ")}.`
    );
  const completed = advanceCustomerSession(session, request.nowMs);
  if (completed) b.flags.customer_service_shift_completed = true;
  touched.add("economy_business_customer_session");
...
function runExoticRefinery(
  ...
  if (b.licenseLevel < 2)
    return reject(
      warnings,
      touched,
      "economy_rejected:hazardous_refinery_license_required"
    );
  if (
    !requireInventory(b, warnings, touched, {
      raw_exotic_matter: 2,
      stabilizing_crystal: 1,
      coolant: 1,
...
function certifyPortalFuel(
  ...
  if (b.licenseLevel < 2)
    return reject(
      warnings,
      touched,
      "economy_rejected:fuel_certification_license_required"
    );
...
function createContamination(
  ...
  systems.contaminationSites[siteId] = {
    siteId,
    townId,
    kind,
    severity: clamp(severity, 1, 10, 1),
    outbreakRisk: clamp(severity * 10, 0, 100, 10),
    status: "active",
    createdAtMs: request.nowMs,
  };
  ...
  reject(
    warnings,
    touched,
    "economy_warning:containment_failure_created_contamination"
  );
  shared.add(systemsSharedKey("contamination", siteId));
}

function runBiomeDecayTick(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  _context: BusinessSystemsContext,
  warnings: string[],
  touched: Set<string>,
  shared: Set<string>
) {
  ...
  for (const property of Object.values(systems.propertyIntegrations)) {
    ...
    const anchorId =
      Object.values(systems.biomeAnchors).find(
        (anchor) => anchor.propertyId === property.propertyId
      )?.anchorId ?? `econ_anchor_${systems.nextAnchorNumber++}`;
    ...
    anchor.condition = clamp(
      anchor.condition - days * 2,
      0,
      100,
      anchor.condition
    );
    anchor.climateStability = clamp(
      anchor.climateStability - days * 1.5,
      0,
      100,
      anchor.climateStability
    );
    anchor.timelineLeak = clamp(
      anchor.timelineLeak + days,
      0,
      100,
      anchor.timelineLeak
    );
  ...
  if (processed === 0)
    reject(warnings, touched, "economy_warning:no_biome_properties_to_decay");
  touched.add("economy_biome_decay");
}

function performBiomeMaintenance(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  const anchor = Object.values(state.businessSystems!.biomeAnchors).find(
    (a) => a.propertyId === propertyId
  );
  if (!propertyId || !anchor)
    return reject(warnings, touched, "economy_rejected:biome_anchor_not_found");
  if (
    !requireInventory(b, warnings, touched, {
      repair_kit: 1,
      stabilized_exotic_matter: 1,
  ...
  anchor.climateStability = clamp(
    anchor.climateStability + 30,
    0,
    100,
    anchor.climateStability
  );
  anchor.timelineLeak = clamp(
    anchor.timelineLeak - 25,
    0,
    100,
    anchor.timelineLeak
  );
  ...
  if (property)
    property.condition = Math.max(property.condition, anchor.condition);
...
function linkBusinessProperty(
  ...
  const propertyId = str(
    request.propertyId,
    `econ_property_${systems.nextPropertyNumber++}`
  );
  systems.propertyIntegrations[propertyId] = {
    ...
    valueGold: int(
      request.propertyValueGold,
      Math.max(500, b.balanceGold + 500)
    ),
    ...
    permits: Array.isArray(request.permits)
      ? request.permits.map(String)
      : [b.licenseClass],
...
function installBiomeDesign(
  ...
  if (!property)
    return reject(
      warnings,
      touched,
      "economy_rejected:property_not_found_for_design_install"
    );
  if (
    !requireInventory(b, warnings, touched, {
      decor_pack: 1,
...
function createSecurityThreat(
  ...
  if (
    !context.allowNpcAdministration &&
    !(context as any).allowWorldAdministration
  )
    return reject(
      warnings,
      touched,
      "economy_rejected:world_admin_required_for_threat_creation"
    );
  const systems = state.businessSystems!;
  ...
  const town = ensureTown(
    state,
    systems.threats[threatId].townId,
    str(request.regionId, "harthmere_grove_region"),
    request.nowMs
  );
  town.needs.safety.value = clamp(
    town.needs.safety.value - systems.threats[threatId].severity * 3,
    0,
    100,
...
function resolveSecurityThreat(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!threat || threat.status !== "active")
    return reject(
      warnings,
      touched,
      "economy_rejected:active_threat_not_found"
    );
  const hasGear = Object.keys(b.inventory).some((item) =>
    /weapon|sword|armor|trap|ration/i.test(item)
  );
  if (!hasGear)
    return reject(
      warnings,
      touched,
      "economy_rejected:security_contract_requires_gear"
    );
...
function buildPortalEndpoint(
  ...
  if (b.licenseLevel < 3)
    return reject(
      warnings,
      touched,
      "economy_rejected:portal_transit_license_required"
    );
  if (
    !requireInventory(b, warnings, touched, {
      anchor_core: 1,
      destination_crystal: 1,
      certified_portal_fuel: 2,
    })
  )
    return;
  consumeInventory(b, {
...
function runPortalTransit(
  ...
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const endpoint =
    state.businessSystems!.portalEndpoints[str(request.endpointId, "")];
  if (!endpoint || endpoint.businessId !== b.businessId || !endpoint.active)
    return reject(
      warnings,
      touched,
      "economy_rejected:active_portal_endpoint_not_found"
    );
  ...
  if (endpoint.fuelUnits < fuelNeeded)
    return reject(
      warnings,
      touched,
      "economy_rejected:portal_fuel_insufficient"
    );
  endpoint.fuelUnits -= fuelNeeded;
  endpoint.uptime = clamp(
    endpoint.uptime - fuelNeeded,
    0,
    100,
    endpoint.uptime
  );
  const fare =
    passengers * int(request.passengerFeeGold, 12) +
    cargoUnits * int(request.cargoFeeGold, 5);
...
function runCropGrowthTick(
  ...
  for (const crop of Object.values(systems.cropNodes)) {
    ...
    crop.growth = clamp(
      crop.growth + days * (climateMatch ? 35 : 12),
      0,
      100,
      crop.growth
    );
    crop.health = clamp(
      crop.health - days * (climateMatch ? 1 : 8),
      0,
      100,
      crop.health
    );
  ...
  if (changed === 0)
    reject(warnings, touched, "economy_warning:no_crops_to_grow");
  touched.add("economy_farming");
}

function harvestCropNode(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!crop || crop.businessId !== b.businessId || crop.harvested)
    return reject(
      warnings,
      touched,
      "economy_rejected:harvestable_crop_not_found"
    );
  if (crop.growth < 100 || crop.spoiledAtMs)
    return reject(
      warnings,
      touched,
      "economy_rejected:crop_not_ready_or_spoiled"
    );
  crop.harvested = true;
  const count = Math.max(1, Math.round(crop.health / 25));
  produceInventory(
    b,
    { [crop.cropItemId]: count },
    { expiresAtMs: request.nowMs + 5 * 24 * 60 * 60 * 1000 }
  );
...
function runSpoilageTick(
  ...
  if (spoiled === 0)
    reject(warnings, touched, "economy_warning:no_spoilage_found");
  touched.add("economy_spoilage");
}

function repairDurableItem(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!item)
    return reject(warnings, touched, "economy_rejected:durable_item_not_found");
  if (
    !requireInventory(b, warnings, touched, { repair_tool: 1, iron_ingot: 1 })
  )
    return;
...
function upgradeDurableItem(
  ...
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const item =
    state.businessSystems!.durableItems[str(request.durableItemId, "")];
  if (!item)
    return reject(warnings, touched, "economy_rejected:durable_item_not_found");
  if (item.restricted && b.licenseLevel < 2)
    return reject(
      warnings,
      touched,
      "economy_rejected:restricted_weapon_permit_required"
    );
  if (
    !requireInventory(b, warnings, touched, {
      upgrade_crystal: 1,
...
function craftMagicGood(
  ...
  if (b.licenseLevel < 2)
    return reject(
      warnings,
      touched,
      "economy_rejected:magic_goods_license_required"
    );
  if (
    !requireInventory(b, warnings, touched, {
      stabilized_exotic_matter: 1,
  ...
  produceInventory(
    b,
    { [itemId]: 1 },
    { expiresAtMs: request.nowMs + 3 * 24 * 60 * 60 * 1000 }
  );
  const magicId = `econ_magic_${state.businessSystems!.nextMagicNumber++}`;
  state.businessSystems!.unstableMagicItems[magicId] = {
    businessId: b.businessId,
    itemId,
    expiresAtMs: request.nowMs + 3 * 24 * 60 * 60 * 1000,
    stability: clamp(request.stability, 0, 100, 70),
  };
  touched.add("economy_magic_goods");
  shared.add(businessSharedKey(b.businessId));
}

function installWard(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const property =
    state.businessSystems!.propertyIntegrations[str(request.propertyId, "")];
  if (!property)
    return reject(
      warnings,
      touched,
      "economy_rejected:ward_property_not_found"
    );
  if (!requireInventory(b, warnings, touched, { protective_ward_charm: 1 }))
    return;
  consumeInventory(b, { protective_ward_charm: 1 });
  property.condition = clamp(
...
function runUnstableMagicTick(
  ...
  let expired = 0;
  for (const [magicId, magic] of Object.entries(
    state.businessSystems!.unstableMagicItems
  )) {
    magic.stability = clamp(
      magic.stability - int(request.days, 1) * 20,
      0,
      100,
      magic.stability
    );
    if (magic.expiresAtMs <= request.nowMs || magic.stability <= 0) {
      ...
      if (b)
        applyItem(
          b.inventory,
          magic.itemId,
          -itemCount(b.inventory, magic.itemId)
        );
  ...
  if (expired === 0)
    reject(warnings, touched, "economy_warning:no_unstable_magic_expired");
  touched.add("economy_magic_goods");
}
...
function removeAnomaly(
  ...
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const site =
    state.businessSystems!.contaminationSites[str(request.siteId, "")];
  if (!site || site.status !== "active")
    return reject(
      warnings,
      touched,
      "economy_rejected:active_anomaly_not_found"
    );
...
function discoverRoute(
  ...
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  if (!requireInventory(b, warnings, touched, { field_kit: 1, ration_pack: 1 }))
    return;
  consumeInventory(b, { ration_pack: 1 });
  const systems = state.businessSystems!;
  const routeId = `econ_exploration_route_${systems.nextRouteNumber++}`;
  systems.explorationRoutes[routeId] = {
    routeId,
    businessId: b.businessId,
    originTownId: str(request.originTownId, b.townId ?? "harthmere_grove"),
    destinationId: str(request.destinationId, "unknown_ruins"),
    safetyRating: clamp(request.safetyRating, 0, 100, 60),
    mapFreshness: 100,
    discoveredAtMs: request.nowMs,
    lastSurveyedAtMs: request.nowMs,
  };
...
function runMapAgingTick(
  ...
  for (const route of Object.values(state.businessSystems!.explorationRoutes)) {
    route.mapFreshness = clamp(
      route.mapFreshness - days * 7,
      0,
      100,
      route.mapFreshness
    );
...
function leadExpedition(
  ...
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const route =
    state.businessSystems!.explorationRoutes[str(request.routeId, "")];
  if (!route || route.businessId !== b.businessId)
    return reject(
      warnings,
      touched,
      "economy_rejected:exploration_route_not_found"
    );
  if (route.mapFreshness < 35)
    return reject(
      warnings,
      touched,
      "economy_rejected:map_too_stale_for_expedition"
    );
  const success =
    route.safetyRating + route.mapFreshness >=
    clamp(request.difficulty, 1, 200, 100);
  if (!success) {
    route.safetyRating = clamp(
    ...
    return reject(
      warnings,
      touched,
      "economy_warning:expedition_failed_route_became_riskier"
    );
  }
...
function startPropertyProject(
  ...
  if (existing && existing.businessId !== b.businessId)
    return reject(
      warnings,
      touched,
      "economy_rejected:property_already_claimed"
    );
  if (existing?.constructionComplete)
    return reject(
      warnings,
      touched,
      "economy_rejected:property_already_completed"
    );
  if (existing && !existing.constructionComplete)
    return reject(
      warnings,
      touched,
      "economy_rejected:property_project_already_active"
    );
  const projectInputs = {
    wood_plank: 4,
    stone_block: 4,
  ...
  const permits = new Set(
    Array.isArray(request.permits) ? request.permits.map(String) : []
  );
  ...
  systems.propertyIntegrations[propertyId] = {
    ...
    ownerKind: b.ownerKind,
    ownerId: b.ownerId,
    buildingType: str(request.buildingType, "shop"),
    valueGold: int(request.propertyValueGold, 1000),
    condition: 50,
    cleanliness: 50,
    beauty: 35,
    rentGoldPerDay: int(request.rentGoldPerDay, 25),
...
function advancePropertyProject(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  const property =
    state.businessSystems!.propertyIntegrations[str(request.propertyId, "")];
  if (!property || property.businessId !== b.businessId)
    return reject(
      warnings,
      touched,
      "economy_rejected:property_project_not_found"
    );
  if (property.constructionComplete)
    return reject(
      warnings,
      touched,
      "economy_rejected:property_project_already_complete"
    );
  const requestedProgress =
    request.progress === undefined ? 35 : Number(request.progress);
  if (!Number.isFinite(requestedProgress) || requestedProgress <= 0)
    return reject(
      warnings,
      touched,
      "economy_rejected:invalid_property_project_progress"
    );
  if (
    !requireInventory(b, warnings, touched, { wood_plank: 2, stone_block: 2 })
  )
    return;
  consumeInventory(b, { wood_plank: 2, stone_block: 2 });
  property.constructionStage = clamp(
    property.constructionStage + Math.trunc(requestedProgress),
    0,
    100,
  ...
  if (property.constructionComplete)
    addNeed(state, b, "housing", 10, request.nowMs);
  touched.add("economy_property_development");
  shared.add(systemsSharedKey("property", property.propertyId));
}

function refreshTraderInventory(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (b.balanceGold < budget)
    return reject(
      warnings,
      touched,
      "economy_rejected:trader_refresh_funds_insufficient"
    );
  ...
  for (const item of [
    ...
    region.itemSupply[item] = (region.itemSupply[item] ?? 0) + 1;
  touched.add("economy_general_trader");
  shared.add(businessSharedKey(b.businessId));
}

function performRegionalArbitrage(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (itemCount(b.inventory, itemId) < count)
    return reject(
      warnings,
      touched,
      "economy_rejected:arbitrage_inventory_insufficient"
    );
  const originPrice = clamp(request.originUnitPriceGold, 1, 10000, 5);
  const destinationPrice = clamp(
    request.destinationUnitPriceGold,
    1,
    10000,
    10
  );
  if (destinationPrice <= originPrice)
    return reject(
      warnings,
      touched,
      "economy_rejected:arbitrage_not_profitable"
    );
  applyItem(b.inventory, itemId, -count);
...
function ensureAnimalPopulation(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest
) {
  const systems = state.businessSystems!;
  const populationId = str(
    request.populationId,
    `econ_animal_population_${systems.nextAnimalPopulationNumber++}`
  );
  systems.animalPopulations[populationId] ??= {
    populationId,
    townId: str(request.townId, "harthmere_grove"),
    species: str(request.species, "muckernut"),
    count: int(request.populationCount, 20),
    protected: request.protected === true,
  };
...
function huntWildlife(
  ...
  const count = int(request.count, 1);
  if (population.protected && b.licenseLevel < 2)
    return reject(
      warnings,
      touched,
      "economy_rejected:protected_species_permit_required"
    );
  if (population.count < count || population.count - count < 3)
    return reject(
      warnings,
      touched,
      "economy_rejected:wildlife_population_too_low"
    );
  population.count -= count;
  if (population.count < 8)
    population.overhuntedUntilMs = request.nowMs + 7 * 24 * 60 * 60 * 1000;
...
function runWildlifeTick(
  ...
  for (const population of Object.values(
    state.businessSystems!.animalPopulations
  )) {
    if (
      population.overhuntedUntilMs &&
      population.overhuntedUntilMs > request.nowMs
    ) {
      changed++;
      continue;
    }
    population.count = Math.min(
      80,
      population.count + int(request.days, 1) * 2
    );
    changed++;
    shared.add(systemsSharedKey("animal_population", population.populationId));
  }
  if (changed === 0)
    reject(
      warnings,
      touched,
      "economy_warning:no_wildlife_populations_to_tick"
    );
  touched.add("economy_hunter");
}
...
function registerPatient(
  ...
  if (
    ...
    return reject(
      warnings,
      touched,
      "economy_rejected:world_admin_required_for_patient_creation"
    );
  const systems = state.businessSystems!;
  const patientId = `econ_patient_${systems.nextPatientNumber++}`;
  systems.patients[patientId] = {
    patientId,
    actorId: str(request.patientActorId, ""),
    townId: str(request.townId, "harthmere_grove"),
    conditionKind: str(request.conditionKind, "injury"),
    severity: clamp(request.severity, 1, 10, 3),
    status: "sick",
    createdAtMs: request.nowMs,
  };
...
function treatPatient(
  ...
  if (b.licenseLevel < 2)
    return reject(
      warnings,
      touched,
      "economy_rejected:medical_license_required"
    );
  const patient = state.businessSystems!.patients[str(request.patientId, "")];
  if (!patient || patient.status !== "sick")
    return reject(warnings, touched, "economy_rejected:sick_patient_not_found");
  if (!requireInventory(b, warnings, touched, { field_medkit: 1, medicine: 1 }))
    return;
...
function buildTeleportPad(
  ...
  if (b.licenseLevel < 2)
    return reject(
      warnings,
      touched,
      "economy_rejected:teleport_license_required"
    );
  if (
    !requireInventory(b, warnings, touched, {
      teleport_fuel: 2,
  ...
  systems.teleportPads[padId] = {
    padId,
    businessId: b.businessId,
    locationId: str(request.locationId, b.townId ?? "harthmere_grove"),
    linkedDestinationId: str(request.destinationId, "home"),
    fuelUnits: 2,
    stability: 100,
    accessKeys: {},
    usesToday: 0,
...
function issueTeleportAccessKey(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!pad || pad.businessId !== b.businessId)
    return reject(warnings, touched, "economy_rejected:teleport_pad_not_found");
  const keyId = `teleport_key_${Object.keys(pad.accessKeys).length + 1}`;
  pad.accessKeys[keyId] = {
    actorId: str(request.targetActorId, request.actorId),
    expiresAtMs: Number(
      request.expiresAtMs ?? request.nowMs + 7 * 24 * 60 * 60 * 1000
    ),
  };
  touched.add("economy_teleport");
...
function useTeleportPad(
  ...
  if (!b || b.status !== "open")
    return reject(warnings, touched, "economy_rejected:business_not_open");
  const hasAccess = Object.values(pad.accessKeys).some(
    (key) => key.actorId === request.actorId && key.expiresAtMs > request.nowMs
  );
  if (!hasAccess)
    return reject(
      warnings,
      touched,
      "economy_rejected:teleport_access_key_required"
    );
  if (pad.fuelUnits <= 0 || pad.stability < 25)
    return reject(
      warnings,
      touched,
      "economy_rejected:teleport_pad_unstable_or_unfueled"
    );
  pad.fuelUnits -= 1;
  pad.usesToday += 1;
...
function runTeleportDestabilizationTick(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  _context: BusinessSystemsContext,
  warnings: string[],
  ...
  for (const pad of Object.values(state.businessSystems!.teleportPads)) {
    pad.stability = clamp(
      pad.stability - int(request.days, 1) * (2 + pad.usesToday),
      0,
      100,
      pad.stability
    );
  ...
  if (changed === 0)
    reject(warnings, touched, "economy_warning:no_teleport_pads_to_tick");
  touched.add("economy_teleport");
}

function accumulateWaste(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (
    ...
    return reject(
      warnings,
      touched,
      "economy_rejected:world_admin_required_for_waste_accumulation"
    );
  const systems = state.businessSystems!;
  const siteId = `econ_contamination_${systems.nextContaminationNumber++}`;
  ...
  systems.contaminationSites[siteId] = {
    ...
    townId: str(request.townId, "harthmere_grove"),
    severity,
    kind: str(request.kind, "waste_overflow"),
    outbreakRisk: severity * 8,
    status: "active",
    createdAtMs: request.nowMs,
  };
  const town = ensureTown(
    state,
    systems.contaminationSites[siteId].townId,
    str(request.regionId, "harthmere_grove_region"),
    request.nowMs
  );
  town.needs.sanitation.value = clamp(
    town.needs.sanitation.value - severity * 5,
    0,
    100,
    town.needs.sanitation.value
  );
  touched.add("economy_sanitation");
  shared.add(systemsSharedKey("contamination", siteId));
}

function cleanupContaminationSite(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  const site =
    state.businessSystems!.contaminationSites[str(request.siteId, "")];
  if (!site || site.status === "clean")
    return reject(
      warnings,
      touched,
      "economy_rejected:contamination_site_not_found"
    );
  if (
    !requireInventory(b, warnings, touched, {
      cleaning_reagent: 1,
...
function runOutbreakPreventionTick(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  _context: BusinessSystemsContext,
  warnings: string[],
  ...
  let activeRisk = 0;
  for (const site of Object.values(state.businessSystems!.contaminationSites))
    if (site.status === "active") activeRisk += site.outbreakRisk;
  if (activeRisk <= 0)
    return reject(
      warnings,
      touched,
      "economy_warning:no_outbreak_risk_present"
    );
  for (const town of Object.values(state.towns) as any[]) {
    town.needs.health.value = clamp(
      town.needs.health.value - Math.ceil(activeRisk / 50),
      0,
      100,
...
function repairFixture(
  ...
  if (!property && !durable)
    return reject(
      warnings,
      touched,
      "economy_rejected:repair_target_not_found"
    );
  if (
    !requireInventory(b, warnings, touched, {
      nails: 1,
  ...
  if (durable)
    durable.condition = clamp(
      durable.condition + 30,
      0,
      100,
      durable.condition
    );
...
function setRestaurantMenu(
  ...
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const menu = Array.isArray(request.menuItems)
    ? request.menuItems.map(String).slice(0, 12)
    : [];
  if (menu.length === 0)
    return reject(
      warnings,
      touched,
      "economy_rejected:restaurant_menu_required"
    );
...
function serveRestaurantDay(
  ...
  if (sanitation < 35)
    return reject(
      warnings,
      touched,
      "economy_rejected:restaurant_sanitation_too_low"
    );
  let mealsServed = 0;
  ...
  if (mealsServed === 0)
    return reject(
      warnings,
      touched,
      "economy_rejected:restaurant_out_of_menu_stock"
    );
  adjustBusinessFunds(b, mealsServed * int(request.unitPriceGold, 10));
  b.customerSatisfaction = clamp(
    b.customerSatisfaction + mealsServed + (sanitation > 70 ? 2 : -2),
    0,
    100,
    b.customerSatisfaction
  );
...
function createDelivery(
  ...
  const itemId = str(request.itemId, "");
  const count = int(request.count, 1);
  if (!itemId || itemCount(b.inventory, itemId) < count)
    return reject(
      warnings,
      touched,
      "economy_rejected:courier_delivery_inventory_required"
    );
  applyItem(b.inventory, itemId, -count);
  const systems = state.businessSystems!;
  const deliveryId = `econ_delivery_${systems.nextDeliveryNumber++}`;
  systems.deliveries[deliveryId] = {
    deliveryId,
    courierBusinessId: b.businessId,
    fromBusinessId: str(request.fromBusinessId, ""),
    toBusinessId: str(request.toBusinessId, ""),
    itemId,
    count,
    escrowGold: int(request.rewardGold, 30),
    deadlineAtMs: Number(
      request.deadlineAtMs ?? request.nowMs + 24 * 60 * 60 * 1000
    ),
    condition: clamp(request.condition, 0, 100, 100),
    status: "active",
  };
...
function completeDelivery(
  ...
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const delivery =
    state.businessSystems!.deliveries[str(request.deliveryId, "")];
  if (
    !delivery ||
    delivery.courierBusinessId !== b.businessId ||
    delivery.status !== "active"
  )
    return reject(
      warnings,
      touched,
      "economy_rejected:active_delivery_not_found"
    );
  if (delivery.deadlineAtMs < request.nowMs) {
    ...
    return reject(
      warnings,
      touched,
      "economy_rejected:delivery_deadline_missed"
    );
  }
  delivery.condition = clamp(
    delivery.condition - int(request.damage, 0),
    0,
    100,
    delivery.condition
  );
  ...
  if (delivery.status === "delivered")
    adjustBusinessFunds(b, delivery.escrowGold);
...
function runDeliveryDeadlineTick(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  _context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (expired === 0)
    reject(warnings, touched, "economy_warning:no_deliveries_expired");
  touched.add("economy_courier");
}

function createHospitalityState(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  systems.hospitality[hospitalityId] = {
    hospitalityId,
    businessId: b.businessId,
    rooms: int(request.rooms, 4),
    occupiedRooms: 0,
    cleanliness: clamp(request.cleanliness, 0, 100, 70),
    safety: clamp(request.safetyRating, 0, 100, 70),
    shelterBeds: int(request.shelterBeds, 0),
    refugeeContractActive: false,
  };
  touched.add("economy_hospitality");
  shared.add(systemsSharedKey("hospitality", hospitalityId));
}

function runHospitalityDay(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!b) return;
  if (!requireOpenBusinessStatus(b, warnings, touched)) return;
  const hosp = Object.values(state.businessSystems!.hospitality).find(
    (entry) => entry.businessId === b.businessId
  );
  if (!hosp)
    return reject(
      warnings,
      touched,
      "economy_rejected:hospitality_state_required"
    );
  if (hosp.cleanliness < 35 || hosp.safety < 35)
    return reject(
      warnings,
      touched,
      "economy_rejected:hospitality_cleanliness_or_safety_too_low"
    );
  ...
  const foodConsumed = Math.min(
    hosp.occupiedRooms,
    itemCount(b.inventory, "worker_meal")
  );
  if (foodConsumed < Math.ceil(hosp.occupiedRooms / 2))
    return reject(
      warnings,
      touched,
      "economy_rejected:hospitality_food_supply_required"
    );
  applyItem(b.inventory, "worker_meal", -foodConsumed);
  adjustBusinessFunds(b, hosp.occupiedRooms * int(request.roomRateGold, 35));
  hosp.cleanliness = clamp(
    hosp.cleanliness - hosp.occupiedRooms * 2,
    0,
    100,
    hosp.cleanliness
  );
...
function cleanHospitalityRooms(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (
    !requireInventory(b, warnings, touched, { linen: 1, cleaning_reagent: 1 })
  )
    return;
...
function createShelterContract(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!hosp || hosp.shelterBeds <= 0)
    return reject(warnings, touched, "economy_rejected:shelter_beds_required");
...
function runRentTick(
  ...
  for (const property of Object.values(systems.propertyIntegrations)) {
    if (
      !property.businessId ||
      property.rentGoldPerDay <= 0 ||
      !property.constructionComplete
    )
      continue;
    ...
    if (b.balanceGold < rent) {
      ...
      reject(
        warnings,
        touched,
        "economy_warning:business_suspended_for_unpaid_rent"
      );
    } else {
      b.balanceGold -= rent;
  ...
  if (processed === 0)
    reject(warnings, touched, "economy_warning:no_business_rent_due");
  touched.add("economy_property_rent");
}

function runLoanDefaultTick(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  _context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (defaults === 0)
    reject(warnings, touched, "economy_warning:no_defaulted_business_loans");
...
function seizeLoanCollateral(
  ...
  if (
    !context.allowNpcAdministration &&
    !(context as any).allowBankAdministration
  )
    return reject(
      warnings,
      touched,
      "economy_rejected:bank_admin_required_for_collateral_seizure"
    );
  const loan = request.loanId
    ? (state.loans as any)[request.loanId]
    : undefined;
  if (!loan || loan.status !== "defaulted")
    return reject(
      warnings,
      touched,
      "economy_rejected:defaulted_loan_required_for_collateral_seizure"
    );
  const systems = state.businessSystems!;
  const property = Object.values(systems.propertyIntegrations).find(
    (p) =>
      p.businessId === loan.businessId || p.collateralLoanId === loan.loanId
  );
  if (!property)
    return reject(
      warnings,
      touched,
      "economy_rejected:loan_collateral_not_found"
    );
...
function liquidateBankruptBusiness(
  state: BusinessSystemsEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: BusinessSystemsContext,
  warnings: string[],
  ...
  if (!b) return;
  if (b.status !== "bankrupt" && b.status !== "suspended")
    return reject(
      warnings,
      touched,
      "economy_rejected:business_not_eligible_for_liquidation"
    );
  const liquidationValue =
    Object.values(b.inventory).reduce(
      (sum, stack) => sum + stack.count * 2,
      0
    ) + Math.round(b.balanceGold * 0.5);
  b.inventory = {};
  ...
  b.debtGold = (Object.values(state.loans) as any[])
    .filter(
      (loan) => loan.businessId === b.businessId && loan.status !== "paid"
    )
...
export function validateHarthmereEconomyBalance(
  state: HarthmereProductionEconomyState
): string[] {
  const warnings: string[] = [];
  for (const type of Object.values((state as any).businessTypes ?? {}))
    void type;
  for (const business of Object.values(state.businesses)) {
    if (business.salesTaxRate > 0.18)
      warnings.push(`balance:tax_rate_too_high:${business.businessId}`);
    if (
      business.upkeepGoldPerDay >
      Math.max(250, business.balanceGold + business.reputation * 10)
    )
      warnings.push(`balance:upkeep_pressure_extreme:${business.businessId}`);
    if (business.balanceGold < 0)
      warnings.push(`balance:negative_business_balance:${business.businessId}`);
    for (const stack of Object.values(business.inventory)) {
      if (stack.count < 0)
        warnings.push(
          `balance:negative_inventory:${business.businessId}:${stack.itemId}`
        );
      if (stack.count > 100000)
        warnings.push(
          `balance:inventory_duplication_risk:${business.businessId}:${stack.itemId}`
        );
    }
  }
  for (const contract of Object.values(state.contracts)) {
    if (contract.rewardGold > 50000)
      warnings.push(`balance:contract_payout_too_high:${contract.contractId}`);
    if (contract.rewardGold < 0 || contract.escrowGold < 0)
      warnings.push(`balance:negative_contract_money:${contract.contractId}`);
  }
  for (const loan of Object.values(state.loans) as any[]) {
    const b = state.businesses[loan.businessId];
    if (
      b &&
      loan.principalOriginal > Math.max(1000, b.balanceGold * 20 + 10000)
    )
      warnings.push(`balance:loan_cap_too_high:${loan.loanId}`);
  }
  const systems = normalizeHarthmereEconomyBusinessSystemsState(
    (state as BusinessSystemsEconomyState).businessSystems
  );
  const serviceCatalogValidation =
    validateHarthmereBusinessServiceItemReferences();
  for (const itemId of serviceCatalogValidation.missingRequiredItems)
    warnings.push(
      `balance:business_customer_missing_required_item_catalog:${itemId}`
    );
  for (const itemId of serviceCatalogValidation.missingProducedItems)
    warnings.push(
      `balance:business_customer_missing_produced_item_catalog:${itemId}`
    );
  const openMarketOrders = Object.values(state.marketOrders).filter(
    (order) => order.status === "open"
  ).length;
  if (
    openMarketOrders > Math.max(200, Object.keys(state.businesses).length * 100)
  )
    warnings.push("balance:npc_or_market_order_flood_risk");
  for (const town of Object.values(state.towns) as any[]) {
    for (const [needId, need] of Object.entries(town.needs ?? {}) as any[]) {
      if (need.value < 0 || need.value > 100)
        warnings.push(
          `balance:town_need_out_of_bounds:${town.townId}:${needId}`
        );
    }
  }
  for (const account of Object.values(systems.bankAccounts)) {
    if (account.balanceGold < 0)
      warnings.push(`balance:negative_bank_account:${account.accountId}`);
  }
  for (const outpostBuilding of Object.values(systems.outpostBuildings)) {
    const audit = validateHarthmereBusinessOutpostPassability(outpostBuilding);
    for (const error of audit.errors)
      warnings.push(
        `balance:outpost_passability:${outpostBuilding.outpostId}:${error}`
      );
    const liveAudit =
      validateHarthmereBusinessOutpostLiveWorldNavigation(outpostBuilding);
    for (const route of liveAudit.unreachableRoutes)
      warnings.push(
        `balance:outpost_live_navigation_unreachable:${outpostBuilding.outpostId}:${route}`
      );
    for (const collision of liveAudit.unresolvedCollisions)
      warnings.push(
        `balance:outpost_live_navigation_collision:${outpostBuilding.outpostId}:${collision}`
      );
  }
  for (const branch of Object.values(systems.empireBranches)) {
    if (!systems.outpostBuildings[branch.outpostId])
      warnings.push(
        `balance:branch_missing_outpost_building:${branch.branchId}`
      );
    if (branch.dailyRevenueGold < 0 || branch.dailyUpkeepGold < 0)
      warnings.push(`balance:branch_money_negative:${branch.branchId}`);
    if (
      branch.status === "active" &&
      !state.businesses[branch.parentBusinessId]
    )
      warnings.push(
        `balance:branch_missing_parent_business:${branch.branchId}`
      );
    if (branchWarehouseUnits(branch) > branch.warehouseSlots)
      warnings.push(
        `balance:branch_warehouse_over_capacity:${branch.branchId}`
      );
    if ((branch.scheduledStaffIds ?? []).length > branch.staffSlots)
      warnings.push(`balance:branch_staff_over_capacity:${branch.branchId}`);
    if (
      branch.regionalManagerEmployeeId &&
      !state.employees[branch.regionalManagerEmployeeId]
    )
      warnings.push(
        `balance:branch_manager_missing_employee:${branch.branchId}`
      );
    if (branch.competitorPressure < 0 || branch.competitorPressure > 25)
      warnings.push(
        `balance:branch_competitor_pressure_out_of_bounds:${branch.branchId}`
      );
  }
  for (const automation of Object.values(systems.automationAssignments)) {
    if (!state.businesses[automation.businessId])
      warnings.push(
        `balance:automation_missing_business:${automation.automationId}`
      );
    if (automation.branchId && !systems.empireBranches[automation.branchId])
      warnings.push(
        `balance:automation_missing_branch:${automation.automationId}`
      );
    if (
      automation.dailyUpkeepGold < 0 ||
      automation.passiveProfitGoldPerDay < 0
    )
      warnings.push(
        `balance:automation_money_negative:${automation.automationId}`
      );
  }
  return warnings;
}
...
function runBalanceValidation(
  ...
  const report = validateHarthmereEconomyBalance(state);
...
export function reduceHarthmereEconomyBusinessSpecificMutation(
  state: HarthmereProductionEconomyState,
  request: HarthmereEconomyMutationRequest,
  context: HarthmereEconomyMutationContext
): HarthmereEconomyBusinessSpecificMutationResult {
  ...
  switch (request.operation) {
    case "grant_business_permission":
      ...
      break;
    case "create_business_bank_account":
      createBankAccount(next, request, typedContext, warnings, touched, shared);
      break;
    case "transfer_personal_to_business_bank":
      ...
      break;
    case "transfer_business_to_personal_bank":
      ...
      break;
    case "link_business_property":
      linkBusinessProperty(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "run_business_rent_tick":
      runRentTick(next, request, typedContext, warnings, touched, shared);
      break;
    case "run_loan_default_tick":
      runLoanDefaultTick(
        next,
        request,
        typedContext,
    ...
    case "seize_loan_collateral":
      ...
      break;
    case "liquidate_bankrupt_business":
      liquidateBankruptBusiness(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "run_exotic_refinery_cycle":
      runExoticRefinery(next, request, typedContext, warnings, touched, shared);
      break;
    case "certify_portal_fuel":
      certifyPortalFuel(next, request, typedContext, warnings, touched, shared);
      break;
    case "run_biome_decay_tick":
      runBiomeDecayTick(next, request, typedContext, warnings, touched, shared);
      break;
    case "perform_biome_maintenance":
      performBiomeMaintenance(
        next,
        request,
        typedContext,
        warnings,
    ...
    case "build_portal_endpoint":
      ...
      break;
    case "run_portal_transit":
      runPortalTransit(next, request, typedContext, warnings, touched, shared);
      break;
    case "plant_crop_node":
      plantCropNode(next, request, typedContext, warnings, touched, shared);
      break;
    case "run_crop_growth_tick":
      runCropGrowthTick(next, request, typedContext, warnings, touched, shared);
      break;
    case "harvest_crop_node":
      harvestCropNode(next, request, typedContext, warnings, touched, shared);
      break;
    case "run_spoilage_tick":
      runSpoilageTick(next, request, typedContext, warnings, touched, shared);
      break;
    case "register_durable_item":
      registerDurableItem(
        next,
        request,
        typedContext,
      ...
      break;
    case "repair_durable_item":
      repairDurableItem(next, request, typedContext, warnings, touched, shared);
      break;
    case "upgrade_durable_item":
      upgradeDurableItem(
        next,
        request,
        typedContext,
      ...
      break;
    case "craft_magic_good":
      craftMagicGood(next, request, typedContext, warnings, touched, shared);
      break;
    case "install_ward":
      installWard(next, request, typedContext, warnings, touched, shared);
      break;
    case "remove_anomaly":
      removeAnomaly(next, request, typedContext, warnings, touched, shared);
      break;
    case "run_unstable_magic_tick":
      runUnstableMagicTick(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "discover_exploration_route":
      discoverRoute(next, request, typedContext, warnings, touched, shared);
      break;
    case "run_map_aging_tick":
      runMapAgingTick(next, request, typedContext, warnings, touched, shared);
      break;
    case "lead_expedition":
      leadExpedition(next, request, typedContext, warnings, touched, shared);
      break;
    case "start_property_project":
      startPropertyProject(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "advance_property_project":
      advancePropertyProject(
        next,
        request,
        typedContext,
        warnings,
    ...
    case "refresh_trader_inventory":
      ...
      break;
    case "perform_regional_arbitrage":
      performRegionalArbitrage(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "hunt_wildlife":
      huntWildlife(next, request, typedContext, warnings, touched, shared);
      break;
    case "run_wildlife_tick":
      runWildlifeTick(next, request, typedContext, warnings, touched, shared);
      break;
    case "register_patient":
      registerPatient(next, request, typedContext, warnings, touched, shared);
      break;
    case "treat_patient":
      treatPatient(next, request, typedContext, warnings, touched, shared);
      break;
    case "build_teleport_pad":
      buildTeleportPad(next, request, typedContext, warnings, touched, shared);
      break;
    case "issue_teleport_access_key":
      issueTeleportAccessKey(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "use_teleport_pad":
      useTeleportPad(next, request, typedContext, warnings, touched, shared);
      break;
    case "run_teleport_destabilization_tick":
      runTeleportDestabilizationTick(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "accumulate_waste":
      accumulateWaste(next, request, typedContext, warnings, touched, shared);
      break;
    case "cleanup_contamination_site":
      cleanupContaminationSite(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "run_outbreak_prevention_tick":
      runOutbreakPreventionTick(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "repair_fixture":
      repairFixture(next, request, typedContext, warnings, touched, shared);
      break;
    case "set_restaurant_menu":
      setRestaurantMenu(next, request, typedContext, warnings, touched, shared);
      break;
    case "serve_restaurant_day":
      serveRestaurantDay(
        next,
        request,
        typedContext,
    ...
    case "create_delivery":
      createDelivery(next, request, typedContext, warnings, touched, shared);
      break;
    case "complete_delivery":
      completeDelivery(next, request, typedContext, warnings, touched, shared);
      break;
    case "run_delivery_deadline_tick":
      runDeliveryDeadlineTick(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "create_hospitality_state":
      createHospitalityState(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "run_hospitality_day":
      runHospitalityDay(next, request, typedContext, warnings, touched, shared);
      break;
    case "clean_hospitality_rooms":
      cleanHospitalityRooms(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "create_shelter_contract":
      createShelterContract(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "start_business_customer_session":
      startBusinessCustomerSession(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "serve_business_customer":
      serveBusinessCustomer(
        next,
        request,
        typedContext,
        goldDelta,
      ...
      break;
    case "tick_business_customer_session":
      tickBusinessCustomerSession(next, request, warnings, touched, shared);
      break;
    case "end_business_customer_session":
      endBusinessCustomerSession(next, request, warnings, touched, shared);
      break;
    case "open_business_branch":
      openBusinessBranch(
        next,
        request,
        typedContext,
      ...
      break;
    case "assign_business_automation":
      assignBusinessAutomation(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "assign_business_branch_manager":
      assignBusinessBranchManager(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "route_business_branch_stock":
      routeBusinessBranchStock(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "schedule_business_branch_staff":
      scheduleBusinessBranchStaff(
        next,
        request,
        typedContext,
        warnings,
    ...
    case "close_business_branch":
      ...
      break;
    case "run_business_empire_day":
      runBusinessEmpireDay(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "refresh_business_employee_candidates":
      refreshBusinessEmployeeCandidates(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "interview_business_employee_candidate":
      interviewBusinessEmployeeCandidate(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "negotiate_business_employee_candidate":
      negotiateBusinessEmployeeCandidate(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "hire_business_employee_candidate":
      hireBusinessEmployeeCandidate(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "promote_business_employee":
      promoteBusinessEmployee(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "run_business_employee_task":
      runBusinessEmployeeTask(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "run_business_employee_morale_tick":
      runBusinessEmployeeMoraleTick(
        next,
        request,
        typedContext,
        warnings,
      ...
      break;
    case "validate_economy_balance":
      runBalanceValidation(next, request, typedContext, warnings, touched);
      break;
  }

  // Kept for future personal-item delta business modules. No-op in current because
  // business-specific systems operate against business/world state.
  for (const [itemId, delta] of Object.entries(itemDeltas))
    recordDelta(itemDeltas, itemId, delta);

  const buildingMaterializationPlans = Array.isArray(
