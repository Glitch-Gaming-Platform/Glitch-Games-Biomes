...
export const HARTHMERE_COMBAT_SYSTEM_VERSION = "harthmere-combat-system-core";

export const HARTHMERE_COMBAT_PIPELINE = [
...
export const HARTHMERE_REQUIRED_COMBAT_FEEDBACK = [
  "miss",
  "dodge",
  "block",
  "parry",
...
export type HarthmereInventoryDropPolicy =
  | "none"
...
export interface HarthmereCombatAbility {
  id: string;
  name: string;
  ...
  statusEffects?: HarthmereStatusEffect[];
}

export interface HarthmereStatusEffect {
  id: string;
  label: string;
...
export interface HarthmereCombatRequest {
  requestId: string;
  idempotencyKey: string;
  serverNowMs: number;
  attacker: HarthmereCombatStats;
  target: HarthmereCombatStats;
  ability: HarthmereCombatAbility;
...
export interface HarthmereValidationResult {
  ok: boolean;
...
export interface HarthmereDamageResult {
  baseDamage: number;
  rawDamage: number;
  reduction: number;
  absorbDamage: number;
...
export interface HarthmereContribution {
  playerId: string;
  damage: number;
  healing: number;
  shielding: number;
...
export interface HarthmereThreatEntry {
  targetId: string;
  threat: number;
...
export interface HarthmereRespawnPoint {
  id: string;
  label: string;
  position: HarthmereVec3;
  faction?: HarthmereFaction;
...
export interface HarthmereCombatResolution {
  ok: boolean;
  result: HarthmereHitResult;
  attacker: HarthmereCombatStats;
  target: HarthmereCombatStats;
  damage: HarthmereDamageResult;
  death?: HarthmereDeathRecord;
  contribution: HarthmereContribution;
  threat: HarthmereThreatEntry;
  rewards: HarthmereRewardDecision;
  auditLog: string[];
  feedback: string;
}

export interface HarthmereRewardDecision {
  xpEligible: boolean;
  ...
  inventoryDropPolicy: HarthmereInventoryDropPolicy;
  pvpRewardMultiplier: number;
  suppressionReasons: string[];
}

export const HARTHMERE_DEFAULT_ABILITIES: Record<string, HarthmereCombatAbility> = {
  basic_strike: {
    id: "basic_strike",
    name: "Basic Strike",
    type: "melee",
...
export function deriveHarthmereCombatStats(input: {
  ...
  name: string;
  level: number;
  entityKind: HarthmereCombatStats["entityKind"];
  faction: HarthmereFaction;
  ...
  rank?: HarthmereCombatStats["rank"];
  position?: HarthmereVec3;
...
export function isHarthmerePvpAttackLegal(request: HarthmereCombatRequest) {
...
export function validateHarthmereCombatRequest(request: HarthmereCombatRequest): HarthmereValidationResult {
...
export function physicalDamageReduction(attacker: HarthmereCombatStats, defender: HarthmereCombatStats) {
...
export function magicalDamageReduction(attacker: HarthmereCombatStats, defender: HarthmereCombatStats, damageType: HarthmereDamageType) {
  const typedResistance = defender.resistances?.[damageType] ?? 0;
  const defensiveValue = defender.magicResistance + typedResistance;
  return clampHarthmereCombatNumber(defensiveValue / (defensiveValue + attacker.level * 100), 0, 0.75);
}

export function damageReductionForType(attacker: HarthmereCombatStats, defender: HarthmereCombatStats, damageType: HarthmereDamageType) {
...
export function calculateHarthmereDamage(input: {
  ...
  rng?: () => number;
}): HarthmereDamageResult {
  const zero: HarthmereDamageResult = { baseDamage: 0, rawDamage: 0, reduction: 0, absorbDamage: 0, mitigatedDamage: 0, finalDamage: 0 };
...
export function resolveHarthmereDeathState(input: {
  ...
  hardcore?: boolean;
  creditPlayerIds?: string[];
}): HarthmereDeathRecord | undefined {
...
export function updateHarthmereThreat(input: { attacker: HarthmereCombatStats; target: HarthmereCombatStats; ability: HarthmereCombatAbility; damage: number; healing?: number; shielding?: number; nowMs: number }): HarthmereThreatEntry {
...
export function updateHarthmereContribution(input: { playerId: string; damage?: number; healing?: number; shielding?: number; objective?: number; revive?: boolean; crowdControl?: boolean; interrupt?: boolean; tanking?: number; support?: number; nowMs: number; inRange: boolean; active: boolean }): HarthmereContribution {
  return {
...
export function calculateHarthmereCombatRewards(input: { attacker: HarthmereCombatStats; target: HarthmereCombatStats; contribution: HarthmereContribution; relationship: HarthmereCombatRelationship; repeatedKillCount?: number; pvp?: boolean; hardcorePvp?: boolean; afk?: boolean; lowLevelProtection?: boolean }): HarthmereRewardDecision {
  ...
  const inventoryDropPolicy: HarthmereInventoryDropPolicy =
...
export function resolveHarthmereCombatAction(request: HarthmereCombatRequest, rng: () => number = Math.random): HarthmereCombatResolution {
  ...
  let target: HarthmereCombatStats = { ...request.target, hp: nextHp, combatState: duelNonLethalFinish ? "idle" : nextHp <= 0 ? "dead" : "in_combat" };
...
export function canReviveHarthmereCombatTarget(input: { reviver: HarthmereCombatStats; target: HarthmereCombatStats; distanceMeters: number; lineOfSight: boolean; zoneAllowsRevive: boolean; pvpRulesAllowRevive: boolean; bossRulesAllowRevive: boolean }) {
  const reasons: string[] = [];
  ...
  if (input.distanceMeters > HARTHMERE_DEFAULT_ABILITIES.revive.range + 0.35) reasons.push("revive_out_of_range");
...
export function reviveHarthmereCombatTarget(input: { target: HarthmereCombatStats; reviveType: "basic" | "healer" | "high_level" | "shrine" | "self" }) {
...
export function validateHarthmereRespawnPoint(point: HarthmereRespawnPoint) {
  const reasons: string[] = [];
...
export function respawnHarthmereCombatant(input: { combatant: HarthmereCombatStats; point: HarthmereRespawnPoint; nowMs: number }) {
...
export function validateHarthmereLeash(input: { npc: HarthmereCombatStats; currentPosition: HarthmereVec3 }) {
...
export const HARTHMERE_COMBAT_EDGE_CASES = [
