...
export const HARTHMERE_RECIPE_BOOKS_VERSION =
  "harthmere-recipe-books" as const;

export interface HarthmereRecipeBookDefinition {
  itemId: string;
  displayName: string;
  description: string;
...
export const HARTHMERE_RECIPE_BOOKS = [
  {
...
] as const satisfies readonly HarthmereRecipeBookDefinition[];

export type HarthmereRecipeBookId =
  (typeof HARTHMERE_RECIPE_BOOKS)[number]["itemId"];

const recipeBookByItemId = new Map(
  HARTHMERE_RECIPE_BOOKS.map((book) => [book.itemId, book])
);

export function harthmereRecipeBookForItem(
  itemId: string | undefined
): HarthmereRecipeBookDefinition | undefined {
  if (!itemId) return undefined;
  return recipeBookByItemId.get(itemId as HarthmereRecipeBookId);
}

export function harthmereRecipeBookForBusinessType(
  businessType: string | undefined
): HarthmereRecipeBookDefinition | undefined {
  if (!businessType) return undefined;
  return HARTHMERE_RECIPE_BOOKS.find(
    (book) => book.businessType === businessType
  );
}

export function harthmereRecipeBookItemIds(): string[] {
  return HARTHMERE_RECIPE_BOOKS.map((book) => book.itemId);
}

export function harthmereRecipeBookLearnableRecipeIds(
  itemId: string | undefined,
  knownRecipes: readonly string[] = []
): string[] {
  const book = harthmereRecipeBookForItem(itemId);
  if (!book) return [];
  const known = new Set(knownRecipes);
  return book.recipeIds.filter((recipeId) => !known.has(recipeId));
}

function recipeBookItemDefinition(
  book: HarthmereRecipeBookDefinition
...
export function ensureHarthmereRecipeBookItems(): void {
  if (registered) return;
  registered = true;
  for (const book of HARTHMERE_RECIPE_BOOKS) {
    if (!getHarthmereItemDefinition(book.itemId)) {
      registerHarthmereItemDefinition(recipeBookItemDefinition(book));
    }
  }
}

export function validateHarthmereRecipeBooks(): string[] {
  const errors: string[] = [];
  ...
  for (const book of HARTHMERE_RECIPE_BOOKS) {
    ...
    for (const recipeId of book.recipeIds) {
      ...
      if (!getHarthmereCraftingRecipe(recipeId)) {
