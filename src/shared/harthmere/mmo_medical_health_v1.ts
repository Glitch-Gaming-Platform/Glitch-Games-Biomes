export const HARTHMERE_MEDICAL_HEALTH_VERSION_V1 =
  "harthmere-medical-health-v1" as const;

export interface HarthmereMedicalItemDefinitionV1 {
  itemId: string;
  displayName: string;
...
export interface HarthmereDoctorServiceSnapshotV1 {
  businessId: string;
  type: "medical_doctor";
  licenseLevel: number;
...
export interface HarthmereMedicalHealthStateV1 {
  stateVersion?: typeof HARTHMERE_MEDICAL_HEALTH_VERSION_V1;
  actorId: string;
  health: number;
  maxHealth: number;
  gold: number;
  ...
  doctorBusinesses: Record<string, HarthmereDoctorServiceSnapshotV1>;
}

export interface HarthmereMedicalHealthResultV1 {
  state: HarthmereMedicalHealthStateV1;
  warnings: string[];
...
export const HARTHMERE_MEDICAL_ITEM_DEFINITIONS_V1: Record<string, HarthmereMedicalItemDefinitionV1> = {
  bandage: {
    itemId: "bandage",
    displayName: "Bandage",
    healthRestore: 12,
...
const DOCTOR_TREATMENT_REQUIRED_ITEMS_V1: Record<string, number> = {
  field_medkit: 1,
  medicine: 1,
};

export function defaultHarthmereMedicalHealthStateV1(
  actorId: string,
): HarthmereMedicalHealthStateV1 {
  return {
    stateVersion: HARTHMERE_MEDICAL_HEALTH_VERSION_V1,
    actorId,
    health: 100,
    maxHealth: 100,
    gold: 0,
...
function result(
  ...
  touchedDoctorBusinessIds: string[] = [],
): HarthmereMedicalHealthResultV1 {
  return {
    state,
    warnings,
    inventoryDeltas,
...
function maxHealthFor(state: HarthmereMedicalHealthStateV1) {
  return Math.max(1, Number.isFinite(state.maxHealth) ? Math.trunc(state.maxHealth) : 100);
}

function currentHealthFor(state: HarthmereMedicalHealthStateV1) {
...
function isOnCooldown(
  state: HarthmereMedicalHealthStateV1,
  def: HarthmereMedicalItemDefinitionV1,
  nowMs: number,
) {
...
export function receiveHarthmereDoctorTreatmentV1(
  state: HarthmereMedicalHealthStateV1,
  ...
  for (const [itemId, count] of Object.entries(DOCTOR_TREATMENT_REQUIRED_ITEMS_V1)) {
  ...
  const nextDoctor: HarthmereDoctorServiceSnapshotV1 = {
    ...doctor,
