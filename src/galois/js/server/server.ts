...
function existingPythonCandidate(path: string | undefined): string | undefined {
  if (!path) {
    return undefined;
  }
  try {
...
function localVenvPythonCandidates(): string[] {
  // The asset builder imports Python packages such as pygltflib. During local
  // boot, the Node asset server used to run plain `python`, which can resolve
  // outside the repo venv even when `./b` itself is using `.venv/bin/python`.
  // Prefer the repo venv explicitly so asset workers and boot checks use the
...
function resolveAssetPythonBuildCommand(execDir: string): string {
  ...
  for (const candidate of localVenvPythonCandidates()) {
    const existing = existingPythonCandidate(candidate);
    if (existing) {
      return `${shellQuotePath(existing)} py/assets/build.py`;
    }
  }

  log.warn(
    `Falling back to shell python for Galois assets from ${execDir}. ` +
      "Set BIOMES_ASSET_PYTHON or create .venv if imports like pygltflib fail."
  );
  return "python py/assets/build.py";
}
...
export class BatchAssetServer implements AssetServer {
  ...
  constructor(
    ...
    const resolvedBuildCommand =
      buildCommand ?? resolveAssetPythonBuildCommand(execDir);
