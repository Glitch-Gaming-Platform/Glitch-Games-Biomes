...
export type NpcSpawnInfo = {
  ...
  /**
   * Radius of deterministic home-position jitter. Ambient packs default to
   * four metres so identical anchors do not stack; authored quest NPCs set
   * this to zero because their interaction post is part of the quest contract.
   */
  spawnPositionJitterRadius?: number;
  spawnEvent?:
    | undefined
    | {
        id: BiomesId;
...
export function npcEntity(
  ...
  return {
    ...
    npc_metadata: NpcMetadata.create({
      ...
      // Deterministic jitter so multiple NPCs spawning at the same anchor
      // (e.g. a district hub) spread instead of stacking on each other.
      spawn_position: ((): Vec3 => {
        const base = npc.spawnEvent?.position ?? npc.position;
        const seed =
          (typeof npc.id === "number" ? Number(npc.id) : 0) || 1;
        const hashA = Math.sin(seed * 12.9898) * 43758.5453;
        const hashB = Math.sin(seed * 78.233) * 22578.1459;
        const radius = Math.max(0, npc.spawnPositionJitterRadius ?? 4);
        const jx = (hashA - Math.floor(hashA) - 0.5) * radius * 2;
        const jz = (hashB - Math.floor(hashB) - 0.5) * radius * 2;
...
const getRandomizedScaleFactor = ({
  ...
  // Keep NPC size variation uniform. Per-axis random scaling made characters
  // appear squashed/stretched and caused the collision capsule to drift away
  // from the rendered body.
  return [overallScale, overallScale, overallScale];
};
