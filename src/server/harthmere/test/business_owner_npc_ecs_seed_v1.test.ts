...
import { buildHarthmereBusinessOwnerNpcSeedChangesV1 } from "@/server/harthmere/business_owner_npc_ecs_seed_v1";
...
describe("business owner NPC ECS seed builder", () => {
  it("creates one fully-formed NPC entity per business owner", () => {
    const changes = buildHarthmereBusinessOwnerNpcSeedChangesV1({
      tick: 1,
      nowSeconds: 1000,
    });
    assert.equal(changes.length, HARTHMERE_BUSINESS_OWNER_NPC_SEEDS_V1.length);
    ...
    for (const change of changes) {
      ...
      const seed = HARTHMERE_BUSINESS_OWNER_NPC_SEEDS_V1.find(
  ...
  it("emits updates instead of creates for already-seeded owners", () => {
    const existingIds = new Set<BiomesId>([
      HARTHMERE_BUSINESS_OWNER_NPC_SEEDS_V1[0].entityId,
    ]);
    const changes = buildHarthmereBusinessOwnerNpcSeedChangesV1({
      tick: 2,
      nowSeconds: 1000,
