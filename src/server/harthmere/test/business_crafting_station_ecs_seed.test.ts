...
import { HARTHMERE_BUSINESS_CRAFTING_STATION_SEEDS } from "@/shared/harthmere/business_crafting_station_seed";
import type { BiomesId } from "@/shared/ids";

describe("business crafting station ECS seed builder", () => {
  it("creates one placeable crafting-station entity per business", () => {
    const changes = buildHarthmereBusinessCraftingStationSeedChanges({
      tick: 1,
      nowSeconds: 1000,
    });
    assert.equal(
    ...
    for (const change of changes) {
      ...
      const seed = HARTHMERE_BUSINESS_CRAFTING_STATION_SEEDS.find(
      ...
      const owner = HARTHMERE_BUSINESS_OWNER_NPC_SEEDS.find(
  ...
  it("emits updates instead of creates for already-seeded stations", () => {
    const existingIds = new Set<BiomesId>([
      HARTHMERE_BUSINESS_CRAFTING_STATION_SEEDS[0].entityId,
    ]);
    const changes = buildHarthmereBusinessCraftingStationSeedChanges({
      tick: 2,
      nowSeconds: 1000,
  ...
  it("exposes the same ids as the shared seed and proposes matching changes", () => {
    const ids = harthmereBusinessCraftingStationSeedEntityIds().map(Number);
    assert.deepEqual(
      ids,
    ...
    const proposed = buildHarthmereBusinessCraftingStationSeedProposedChanges({
      nowSeconds: 1000,
    });
