...
const LOCAL_DEV_WILDS_EDGE_RESCUE_BOUNDS = {
  minX: -224,
  maxX: 1216,
  minZ: -960,
  maxZ: 480,
...
function localDevWildsEdgeRescuePosition(
  ...
  let safeX = position[0];
  let safeZ = position[2];
  if (position[0] <= minX) {
    safeX = minX + LOCAL_DEV_WILDS_EDGE_RESCUE_BUFFER;
  } else if (position[0] >= maxX) {
    safeX = maxX - LOCAL_DEV_WILDS_EDGE_RESCUE_BUFFER;
  }
  if (position[2] <= minZ) {
    safeZ = minZ + LOCAL_DEV_WILDS_EDGE_RESCUE_BUFFER;
