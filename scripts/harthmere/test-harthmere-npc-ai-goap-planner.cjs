#!/usr/bin/env node
const { createHarness, aiSource, planGoal } = require("./harthmere-npc-ai-test-lib.cjs");
const h = createHarness("Harthmere NPC AI GOAP tests current"); const src=aiSource(h); for (const token of ["planHarthmereNpcGoal","preconditions","effects","cost"]) h.ok(src.includes(token), `GOAP includes ${token}`);
