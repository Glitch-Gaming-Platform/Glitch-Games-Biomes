...
check(combat.includes('HARTHMERE_ECS_NPC_RETALIATION_BRIDGE'), 'combat bridge version constant exists');
check(combat.includes('__harthmereEcsNpcCombatActorPositions'), 'combat reads ECS NPC bridge window registry');
check(combat.includes('source: "ecs_npc_overlay_bridge"') || overlays.includes('source: "ecs_npc_overlay_bridge"'), 'ECS NPC bridge source marker exists');
...
check(overlays.includes('HARTHMERE_ECS_NPC_RETALIATION_BRIDGE'), 'overlay bridge version constant exists');
check(overlays.includes('publishHarthmereEcsNpcCombatActorSnapshot'), 'overlay publishes ECS NPC combat snapshots');
...
check(overlays.includes('publishHarthmereEcsNpcCombatActorSnapshot({});'), 'overlay clears stale ECS bridge when NPC overlays are disabled');
...
console.log('\nAll current ECS NPC retaliation bridge checks passed.');
