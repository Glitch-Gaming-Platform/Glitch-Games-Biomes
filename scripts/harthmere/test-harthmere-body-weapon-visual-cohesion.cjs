...
check(
  ...
  /getHarthmereWeaponSyncedEmoteWeights[\s\S]*layers:\s*\{[\s\S]*arms:\s*"apply"[\s\S]*notArms:\s*"noApply"/.test(player),
...
check(
  ...
  /HARTHMERE_FULL_BODY_POSE_LAYER_RULES[\s\S]*rangedAim:\s*\{\s*arms:\s*"apply",\s*notArms:\s*"noApply"/.test(player) &&
...
check(
  ...
  /harthmereBodyWeaponVisualCohesion/.test(assets) && /oversizedManualTranslationPrevented/.test(assets),
);
