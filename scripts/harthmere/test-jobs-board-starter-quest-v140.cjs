...
const files = {
  ...
  live: "src/shared/harthmere/live_mode_backend_v1.ts",
};
...
console.log("== Harthmere Jobs Board starter quest regression v140 ==");
...
check("defines stable Read the Jobs Board quest id", includes(quests, 'HARTHMERE_READ_JOBS_BOARD_QUEST_ID_V140 =\n  "read-the-jobs-board"'));
check("defines user-facing Read the Jobs Board title", includes(quests, 'HARTHMERE_READ_JOBS_BOARD_TITLE_V140 =\n  "Read the Jobs Board"'));
check("defines a dedicated synthetic jobs board target offset", includes(quests, "HARTHMERE_JOBS_BOARD_TARGET_OFFSET_V140 = 140_041"));
check("Read the Jobs Board quest is in QUESTS", includes(quests, "id: HARTHMERE_READ_JOBS_BOARD_QUEST_ID_V140"));
check("Read the Jobs Board quest appears before Mira so it can be one of the first assigned quests", indexBefore(quests, "id: HARTHMERE_READ_JOBS_BOARD_QUEST_ID_V140", "id: BUILDING_SYSTEM_MIRA_INTRO_QUEST_V1.questId"));
...
check("Read the Jobs Board quest objective points at the jobs board", includes(quests, 'objective: "Read the Harthmere Grove Jobs Board."'));
...
check("jobs board quest target uses the real board location", includes(quests, "pos: [482, 66, -198]"));
check("jobs board quest target is visibly labeled", includes(quests, 'label: "Harthmere Grove Jobs Board"'));
...
check("autostart list includes only the jobs board starter quest", includes(quests, "HARTHMERE_AUTOSTART_QUEST_IDS_V140") && includes(quests, "HARTHMERE_READ_JOBS_BOARD_QUEST_ID_V140,"));
check("starter state function assigns autostart quests as active", includes(quests, "createHarthmereStarterQuestStateV140") && includes(quests, "Object.fromEntries"));
...
check("writeHarthmereQuestState persists normalized state", includes(quests, "const normalized = normalizeHarthmereQuestStateV140(state)") && includes(quests, "JSON.stringify(normalized)"));

// Completion base/edge cases: reading the board completes the quest exactly once
// and does not crash SSR/non-browser paths.
check("exports jobs board read completion helper", includes(quests, "export function completeHarthmereJobsBoardReadQuestV140"));
...
check("completion helper removes the quest from active state", includes(quests, "delete next.active[HARTHMERE_READ_JOBS_BOARD_QUEST_ID_V140]"));
check("completion helper records the quest as completed", includes(quests, "HARTHMERE_READ_JOBS_BOARD_QUEST_ID_V140,") && includes(quests, "completed: ["));
...
check("local-dev quest reset now reassigns starter quests", includes(quests, "const resetState = createHarthmereStarterQuestStateV140()"));
...
check("jobs board panel imports the completion helper", includes(panel, "completeHarthmereJobsBoardReadQuestV140"));
check("jobs board panel resolves board before completion", panel.indexOf("const board = snapshot.boards[boardId]") >= 0 && panel.indexOf("const board = snapshot.boards[boardId]") < panel.lastIndexOf("completeHarthmereJobsBoardReadQuestV140"));
check("jobs board panel does not complete quest for an invalid board", includes(panel, "if (!board) return;"));
check("jobs board panel completes the quest when opened", includes(panel, 'completeHarthmereJobsBoardReadQuestV140("jobs_board_panel_opened")'));
...
check("live backend exports jobs board starter quest id", includes(live, "HARTHMERE_READ_JOBS_BOARD_QUEST_ID_V140"));
check("live backend exports jobs board starter step id", includes(live, "HARTHMERE_READ_JOBS_BOARD_STEP_ID_V140"));
check("live backend assigns jobs board quest by default", includes(live, "[HARTHMERE_READ_JOBS_BOARD_QUEST_ID_V140]:"));
check("live backend quest starts at zero progress", includes(live, "stepId: HARTHMERE_READ_JOBS_BOARD_STEP_ID_V140,\n          progress: 0"));
check("live backend keeps Mira starter quest active too", includes(live, "[BUILDING_SYSTEM_MIRA_INTRO_QUEST_V1.questId]:"));
check("jobs board starter is before Mira in live defaults", indexBefore(live, "[HARTHMERE_READ_JOBS_BOARD_QUEST_ID_V140]:", "[BUILDING_SYSTEM_MIRA_INTRO_QUEST_V1.questId]:"));
