...
check("runtime exposes clothing key marker", runtime.includes("harthmereForceProceduralTownspersonClothingKeys") || runtime.includes("harthmereForceProceduralTownspersonClothingKeys"));
check("runtime exposes outfit color marker", runtime.includes("harthmereRuntimeOutfitColor"));
check("runtime exposes weapon slot marker", runtime.includes("harthmereProceduralWeaponSlot"));
check("runtime exposes face shape marker through named current meshes", runtime.includes("face-shape-wide-jaw") && runtime.includes("face-shape-narrow-chin"));
...
(() => {
  ...
  const walk = (obj) => {
    if (!obj) return;
    if (obj.userData?.harthmereAppearance || obj.userData?.harthmereForceProceduralTownspersonClothing) {
      roots.push(obj);
    }
  ...
  return roots.map((root) => {
    ...
    return {
      ...
      clothingKeys: root.userData?.harthmereForceProceduralTownspersonClothingKeys,
      outfitColor: root.userData?.harthmereRuntimeOutfitColor,
...
const docPath = path.join(root, "scripts/harthmere/harthmere-npc-visual-runtime-audit.js.txt");
