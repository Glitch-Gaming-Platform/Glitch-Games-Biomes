...
const {
  ...
  repo,
  "src/shared/harthmere/business_customer_simulator_v1"
));
const {
  ...
  repo,
  "src/client/game/renderers/local_dev/harthmere_business_outpost_buildings_v1"
));
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  <g font-family="Inter, ui-sans-serif, system-ui, sans-serif" fill="#f0f7ef">
    ...
    <text x="72" y="116" font-size="18" fill="#bbd0c1">${escapeXml(record.businessType)} · ${escapeXml(HARTHMERE_BUSINESS_OUTPOST_BUILDING_RENDER_VERSION_V1)}</text>
...
async function main() {
  ...
  const root = createHarthmereBusinessOutpostBuildingMeshV1(record);
  ...
  const svgPath = path.join(outDir, `${slug}-guide-mesh-snapshot-v1.svg`);
  const pngPath = path.join(outDir, `${slug}-guide-mesh-snapshot-v1.png`);
    ...
    JSON.stringify(
      {
        ...
        renderVersion: HARTHMERE_BUSINESS_OUTPOST_BUILDING_RENDER_VERSION_V1,
