#!/usr/bin/env node
"use strict";
/* HARTHMERE_INTERIOR_EXPANSION_REGRESSION_FIXES_TEST */
const fs = require("fs");
...
console.log("== Harthmere interior expansion regression fixes tests current ==");
...
check("dungeon console teleport helper exists", /HARTHMERE_DUNGEON_CONSOLE_TELEPORT_USES_LIVE_PLAYER_HOOK/.test(src) && /teleportToDungeonTestTarget/.test(src), "Missing strict live teleport helper marker/function");
check("dungeon console global alias exists", /__harthmereDungeonTest/.test(src), "Missing __harthmereDungeonTest global alias");
check("dungeon console helper exposes documented dungeon target names", /bellwardHalls[\s\S]*firstChoir[\s\S]*oldHarth[\s\S]*bellbinderTomb[\s\S]*bellwardChamber[\s\S]*wyrmsBed/.test(src), "Missing documented dungeon targets");
check("dungeon console helper stores localStorage fallback request", /biomes\.localDev\.harthmere\.teleportTarget/.test(src) && /window\.localStorage\.setItem/.test(src), "Missing localStorage fallback request");
