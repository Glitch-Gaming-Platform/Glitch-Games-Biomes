...
ok(s.includes('SNAPSHOT_RUNTIME_BRIDGE_REPAIR'), 'runtime bridge repair current marker is present');
ok(s.includes('SNAPSHOT_RUNTIME_BRIDGE'), 'old runtime bridge current marker remains for compatibility checks');
ok(s.includes('class RedisServer(object):'), 'RedisServer class exists');
const classIdx = s.indexOf('class RedisServer(object):');
const bridgeIdx = s.indexOf('SNAPSHOT_RUNTIME_BRIDGE_REPAIR');
...
console.log('snapshot runtime bridge repair current check passed');
