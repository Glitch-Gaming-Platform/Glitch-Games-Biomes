#!/usr/bin/env node
// HARTHMERE_ATTACKED_NPC_RETALIATION
...
//
// current changes vs current:
//   - Keeps the current condition check: retaliation only blocks when damageable is
//     absent entirely OR attackable is explicitly false.
//   - Tightens the deep-merge test: snapshotLegacyNpcType must not inherit
...
console.log("== Harthmere attacked NPC retaliation current ==");
...
// ── current condition: block only when damageable absent OR explicitly false ──────
...
check(
  ...
  "fallback blocks retaliation only when damageable is absent or explicitly false (current fix)"
);
...
// ── snapshotLegacyNpcType deep-merges damageable ──────────────────────────
...
check(
  bikkie.includes("SNAPSHOT_LEGACY_NPC_BEHAVIOR_DEEP_MERGE"),
  "snapshotLegacyNpcType is versioned for deep-merge/default-attackable fix"
);
check(
  bikkie.includes("candidateDamageable") &&
    bikkie.includes("mergedDamageable") &&
    bikkie.includes("damageable: mergedDamageable"),
  "snapshotLegacyNpcType deep-merges damageable instead of shallow-overriding it"
);
check(
