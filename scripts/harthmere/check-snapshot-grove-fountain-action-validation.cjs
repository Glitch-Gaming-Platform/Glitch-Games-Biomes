...
ok(runtime.includes("SNAPSHOT_GROVE_CONTEXTUAL_PRACTICE_TRIGGERS"), "contextual HUD/world practice trigger set exists");
...
failIf(/onPerformed:\s*\(\)\s*=>\s*advanceSnapshotGroveQuest/.test(runtime), "active Grove/Fountain steps cannot advance from NPC dialogue actions");
...
ok(
  /currentTriggerForQuest\(\s*quest\s*,\s*state\.activeObjectiveIndex\s*\)/.test(runtime) &&
...
ok(runtime.includes("expectedOpenTabForObjective") && runtime.includes('(event as any).tab === expectedTab'), "open_tab steps validate the intended HUD tab when known");
ok(
...
ok(
  runtime.includes("disabled={!practiceIsInRange}") &&
    (runtime.includes("Move closer to practice") || /Walk to .* first/.test(runtime)),
...
if (failures) {
  console.error(`current Grove/Fountain action validation check failed: ${failures} failure(s)`);
  process.exit(1);
}
