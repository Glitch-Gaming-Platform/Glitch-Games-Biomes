...
check("current version marker exists", src.includes("HARTHMERE_SERVICE_BUILDING_BLOCK_REBUILD_VERSION"));
check("block-built service generator exists", src.includes("createHarthmereBlockBuiltServiceBuilding"));
for (const name of required) {
  check(
    `${name} uses block-built current generator`,
    src.includes(`createHarthmereBlockBuiltServiceBuilding({ name: "${name}"`),
  );
}
