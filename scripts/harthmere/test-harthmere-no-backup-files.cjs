...
console.log("== Harthmere no backup files regression current ==");
...
const oldManifestPaths = [
  "scripts/harthmere/backup-cleanup-deleted-files.txt",
  "scripts/harthmere/backup-cleanup-deleted-files.txt",
];
...
if (fs.existsSync(suitePath)) {
  ...
  if (suite.includes("test-harthmere-no-backup-files.cjs") && !suite.includes("test-harthmere-no-backup-files.cjs")) {
    pass("town placement suite includes current no-backup regression and no stale current entry");
  } else {
    fail("town placement suite includes current no-backup regression and no stale current entry");
  }
} else {
