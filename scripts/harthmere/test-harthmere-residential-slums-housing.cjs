...
const { loadTown, normalizeDistrict, makeReporter } = require("./harthmere-town-rule-test-utils.cjs");

const root = process.argv[2] ? path.resolve(process.argv[2]) : process.cwd();
const reporter = makeReporter("Harthmere residential/slums vertical housing current", root);
const town = loadTown(root);
const manifestPath = path.join(root, "src/shared/harthmere/resident_housing.ts");
...
const residentialBuildings = parseBuildings("HARTHMERE_RESIDENTIAL_HOUSE_BUILDINGS");
const slumBuildings = parseBuildings("HARTHMERE_SLUM_STACK_BUILDINGS");
...
reporter.check("renderer creates current residential/slum placements", /function createHarthmereResidentHousingPlacements/.test(assets) && /\.\.\.createHarthmereResidentHousingPlacements\(\)/.test(assets));
reporter.check("every vertical building generates story-to-story stairs", (/for \(let floor = 1; floor < building\.floors; floor \+= 1\)/.test(assets) && /stair floor \$\{floor\} to \$\{floor \+ 1\} climbable story access clear landing/.test(assets)) || (/createHarthmereBlockStairRun/.test(assets) && /block stair riser/.test(assets) && /npc travel tread/.test(assets)));
reporter.check("each floor gets a landing/accessibility marker", /floor \$\{floor \+ 1\} landing clear accessibility anchor visual only/.test(assets) || /hard landing deck block player and NPC accessible/.test(assets) || /clear center stone floor accessibility marker/.test(assets));
...
reporter.check("town placement suite includes the current housing/accessibility regression test", suite.includes("test-harthmere-residential-slums-housing.cjs"));
