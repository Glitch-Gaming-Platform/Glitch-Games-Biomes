...
assert(
  "combat v193 version constant exists",
  includes(combat, "HARTHMERE_NPC_CHASE_REGEN_WANDER_V193")
);
assert(
  "combat v193 motion event exists",
  includes(combat, "HARTHMERE_NPC_MOTION_EVENT_V193")
);
assert(
  "combat v196 chase/fight logic marker exists",
  includes(combat, "HARTHMERE_NPC_CHASE_FIGHT_LOGIC_V196")
);
assert(
  "combat imports the 28m line-of-sight range contract",
  includes(combat, "HARTHMERE_LOCAL_COMBAT_LINE_OF_SIGHT_RANGE_V1")
);
assert(
  "combat emits chase motion when NPC is pursuing player",
  includes(combat, "emitHarthmereNpcCombatPressureMotionV196") &&
...
assert(
  ...
  includes(combat, "maybeEmitHarthmereNpcRetaliationPressureV196") &&
...
assert(
  ...
  regex(
    combat,
    /emitHarthmereVoxelNpcMotionV193\([\s\S]*?offset,[\s\S]*?npc,[\s\S]*?"chase",[\s\S]*?"windup_face_player"/
  )
);
assert(
...
assert(
  ...
  regex(
    combat,
    /const sightRange = HARTHMERE_LOCAL_COMBAT_LINE_OF_SIGHT_RANGE_V1;[\s\S]*?const chaseReach = profile\.keepFighting[\s\S]*?\? sightRange/
  )
);
assert(
...
assert(
  ...
  includes(combat, "__harthmereVoxelNpcMotionV193")
);
assert(
  "combat dispatches v193 motion custom event",
  includes(combat, "new CustomEvent(HARTHMERE_NPC_MOTION_EVENT_V193")
);
assert(
  "combat reads voxel motion actor positions for hit/range checks",
  includes(combat, "__harthmereVoxelNpcMotionActorPositionsV193")
);
assert(
  "combat has health recharge tick helper",
  includes(combat, "tickHarthmereNpcHealthRegenV193")
);
assert(
...
assert(
  ...
  includes(combat, "__harthmereNpcHealthRegenLogV193")
);
assert(
...
assert(
  "npc resource v193 version constant exists",
  includes(npcs, "HARTHMERE_NPC_CHASE_REGEN_WANDER_V193")
);
assert(
  "npc resource has motion override helper",
  includes(npcs, "getHarthmereVoxelNpcMotionOverrideV193")
);
assert(
  "npc resource consumes combat chase motion global",
  includes(npcs, "__harthmereVoxelNpcMotionV193")
);
assert(
...
assert(
  ...
  includes(npcs, "__harthmereVoxelNpcAmbientWanderEnabledV193")
);
assert(
  "npc resource publishes visible motion position to combat registry",
  includes(npcs, "publishHarthmereVoxelNpcMotionActorPositionV193")
);
assert(
  "npc resource writes voxel motion actor registry",
  includes(npcs, "__harthmereVoxelNpcMotionActorPositionsV193")
);
assert(
  "npc resource applies motion position before interpolation retarget",
  regex(
    npcs,
    /position = harthmereVoxelNpcMotionV193\.position;[\s\S]*?this\.interpolationNeedRetarget = true;/
  )
);
assert(
  ...
  regex(
    npcs,
    /orientation = harthmereVoxelNpcMotionV193\.orientation;[\s\S]*?this\.orientation = \[\.\.\.orientation\]/
  )
);
assert(
  "npc resource does not move dead NPCs with v193 motion",
  includes(npcs, "!motionOverrides && entity.health.hp > 0")
);

assert(
  "renderer has no v190 ECS snapshot fallback method reference",
  !includes(renderer, "findCombatLifeByEcsNpcSnapshotV190")
);
assert(
  "renderer does not use v190 Harthmere actor fallback constant",
  !includes(renderer, "HARTHMERE_RETALIATION_VISIBLE_FEEDBACK_V190")
);
