...
const artifactsDir = path.resolve(
  process.env.HARTHMERE_GROVE_NPC_VISUAL_ARTIFACTS_DIR ||
    path.join(repo, "artifacts", "snapshot-grove-npc-visuals")
);
...
const {
  SNAPSHOT_GROVE_NPCS,
  SNAPSHOT_GROVE_NPC_ROUTE_PROFILES,
...
function routeCorridorDistance(npcId, position) {
  const profile = SNAPSHOT_GROVE_NPC_ROUTE_PROFILES[npcId];
...
function buildNpcTargets() {
  return SNAPSHOT_GROVE_NPCS.filter((npc) => npc.seedServerNpc).map((npc) => ({
    ...
    entityId: String(snapshotGroveNpcEntityId(npc)),
    expectedPosition: snapshotGroveGroundedPosition(npc.authoredPosition),
    authoredPosition: npc.authoredPosition,
    routeProfile: Boolean(SNAPSHOT_GROVE_NPC_ROUTE_PROFILES[npc.id]),
  }));
}
...
async function main() {
  console.log("== Snapshot Grove NPC visual audit ==");
  ...
  check(targets.length === SNAPSHOT_GROVE_NPCS.length, "every Grove NPC is seeded for visual/live testing", `${targets.length}/${SNAPSHOT_GROVE_NPCS.length}`);
