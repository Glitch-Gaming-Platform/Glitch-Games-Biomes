...
console.log("== Harthmere animation/world interaction tests current ==");
console.log(`Root: ${root}\n`);

check("renderer has current object effect range contract", /HARTHMERE_OBJECT_EFFECT_RANGE_VERSION/.test(assets));
check("renderer has current resource hit telegraph contract", /HARTHMERE_RESOURCE_HIT_TELEGRAPH_VERSION/.test(assets));
check("renderer has current weapon hand-tracking contract", /HARTHMERE_WEAPON_HAND_TRACKING_VERSION/.test(assets));
...
check("full suite includes animation/world interaction current test", /test-harthmere-animation-world-interaction\.cjs/.test(suite));
