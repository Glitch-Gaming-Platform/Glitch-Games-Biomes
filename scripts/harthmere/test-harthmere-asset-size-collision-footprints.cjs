...
console.log("== Harthmere measured asset-size collision footprint tests current ==");
...
const generatedPath = path.join(root, "src/shared/harthmere/uploaded_asset_dimensions.ts");
const manifestPath = path.join(root, "public/assets/harthmere/manifest/harthmere-uploaded-asset-dimensions.json");
const registry = fs.readFileSync(registryPath, "utf8");
const assets = fs.readFileSync(assetsPath, "utf8");
const generated = fs.readFileSync(generatedPath, "utf8");
const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
check("town registry imports measured uploaded asset dimensions", /uploaded_asset_dimensions/.test(registry) && /harthmereUploadedAssetCollisionFootprint/.test(registry));
check("measured collision footprint is consulted before generic prop family fallbacks", registry.indexOf("harthmereUploadedAssetCollisionFootprint(asset, scale)") > -1 && registry.indexOf("harthmereUploadedAssetCollisionFootprint(asset, scale)") < registry.indexOf("/counter|table|bench|bed|cabinet|bookcase"));
check("bespoke wall/building collision rules still run before measured fallback", registry.indexOf("asset.startsWith(\"arch_wall_\")") < registry.indexOf("harthmereUploadedAssetCollisionFootprint(asset, scale)") && registry.indexOf("BUILDING_BODY_ASSET_RE.test(asset)") < registry.indexOf("harthmereUploadedAssetCollisionFootprint(asset, scale)"));
...
check("browser debug exposes asset dimensions and measured collision footprints", /__harthmereAssetDimensions/.test(assets) && /collisionFootprint/.test(assets));
