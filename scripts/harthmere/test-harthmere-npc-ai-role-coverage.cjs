#!/usr/bin/env node
const { createHarness, aiSource, npcBehaviorSource, npcSystemSource } = require("./harthmere-npc-ai-test-lib.cjs");
const h = createHarness("Harthmere NPC AI role coverage tests current"); const src=aiSource(h), behavior=npcBehaviorSource(h), npc=npcSystemSource(h);
