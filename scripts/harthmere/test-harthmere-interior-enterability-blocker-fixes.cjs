...
const v31Path = path.join(root, "scripts/harthmere/test-harthmere-all-interior-building-enterability.cjs");
const designPath = path.join(root, "scripts/harthmere/test-harthmere-town-placement-building-design.cjs");
...
console.log("== Harthmere interior enterability blocker fixes current ==");
console.log(`Root: ${root}`);
console.log("");

check("current marker exists", assets.includes("HARTHMERE_INTERIOR_ENTERABILITY_FIX_VERSION"));
...
check("old current-blocking exact placements are gone", oldBlockers.every((needle) => !assets.includes(needle)), oldBlockers.filter((needle) => assets.includes(needle)));
...
check("current explicit clear-lane placements are present", expectedClearanceLabels.every((label) => assets.includes(label)), expectedClearanceLabels.filter((label) => !assets.includes(label)));

runNode("current all-interior enterability regression passes after current fixes", v31Path);
runNode("town placement and NPC anchor regression still passes after moving blockers", designPath);

check("town placement suite includes current regression", suite.includes("test-harthmere-interior-enterability-blocker-fixes.cjs"));
