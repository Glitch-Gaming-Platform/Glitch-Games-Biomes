...
check("shared hand policy switches sword/main weapon to left", () => assert(/HARTHMERE_COMBAT_ANIMATION_HAND_POLICY[\s\S]{0,260}mainWeaponHand:\s*"left"[\s\S]{0,220}shieldHand:\s*"right"/.test(shared), "missing left main-hand policy"));
...
check("NPC weapon fallback anchor is left hand", () => assert(/ensureHarthmereNpcPolishedWeaponAnchor[\s\S]{0,700}harthmere-anchor-left-hand/.test(renderer) && /attachHarthmereNpcWeaponVisual[\s\S]{0,3200}(?:harthmere-anchor-left-hand|npcMainHandAnchorName)[\s\S]{0,520}anchor\.add\(visual\)/.test(renderer), "NPC main weapon not on left anchor"));
