...
ok(
  /export type SnapshotGroveQuestPrerequisite\s*=/.test(shared),
  "Shared content declares SnapshotGroveQuestPrerequisite union",
);
ok(
  /export type SnapshotGroveQuestCategory\s*=/.test(shared),
  "Shared content declares SnapshotGroveQuestCategory union",
);
ok(
  /unlockedBy\?:\s*SnapshotGroveQuestPrerequisite/.test(shared),
  "SnapshotGroveQuest now has an optional unlockedBy field",
);
ok(
  /category\?:\s*SnapshotGroveQuestCategory/.test(shared),
  "SnapshotGroveQuest now has an optional category field",
);
...
// 5. The runtime exposes isSnapshotGroveQuestUnlocked and uses it in the
//    available-for-npc filter so locked quests are never offered.
ok(
  /export function isSnapshotGroveQuestUnlocked\s*\(/.test(runtime),
  "Runtime exposes isSnapshotGroveQuestUnlocked",
);
ok(
  /function countCompletedFountainLessons\s*\(/.test(runtime),
  "Runtime defines countCompletedFountainLessons",
);
ok(
  /isSnapshotGroveQuestUnlocked\(quest,\s*state\)/.test(runtime),
  "availableQuestsForNpc filters by isSnapshotGroveQuestUnlocked",
);
ok(
  /snapshotGroveQuestCategoryRank/.test(runtime),
...
ok(
  ...
  "Runtime version constant records the current graduation-chain update",
);
...
if (failures) {
  console.error(`current Grove graduation chain check failed: ${failures} failure(s)`);
  process.exit(1);
}
console.log("current Grove graduation chain check passed");
