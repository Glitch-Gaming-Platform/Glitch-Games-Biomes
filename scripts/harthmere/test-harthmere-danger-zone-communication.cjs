...
const { loadTown, makeReporter, normalizeDistrict } = require("./harthmere-town-rule-test-utils.cjs");
const root = process.argv[2] ? path.resolve(process.argv[2]) : process.cwd();
const report = makeReporter("Harthmere danger-zone communication tests current", root);
