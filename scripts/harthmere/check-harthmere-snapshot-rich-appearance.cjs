...
ok("snapshot rich appearance marker exists", playerMesh.includes("SNAPSHOT_RICH_NPC_APPEARANCE"));
...
ok("NPC renderer prefers rich snapshot mesh before fallback", npcs.includes("SNAPSHOT_RICH_NPC_APPEARANCE makeNpcMesh") && npcs.includes("await makeSnapshotPlayerLikeAppearanceMesh(deps, id)"));
...
console.log("current snapshot rich NPC appearance check passed");
