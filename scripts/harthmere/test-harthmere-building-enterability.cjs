#!/usr/bin/env node
"use strict";
/* HARTHMERE_BUILDING_ENTERABILITY_TEST
 *
...
const clearChecks = [
  ["HARTHMERE_FRONT_DOOR_CLEARANCE_METERS", 2.0, "front door 2m clear (MMO §56)"],
  ["HARTHMERE_PUBLIC_ENTRANCE_CLEARANCE_METERS", 3.0, "public entrance 3m clear (MMO §56)"],
  ["HARTHMERE_SHOP_CUSTOMER_SPACE_METERS", 4.0, "shop customer 4m clear (MMO §56)"],
];
...
// 2. current must accept openings and skip blocks within them
...
if (/opening|openings|skip|continue/i.test(v44Body)) {
  pass("current wall function processes openings (skips blocks within door/window range)");
} else {
  fail("current wall function processes openings", "expected openings/skip/continue inside current body");
}
...
const interiorMatch = src.match(/function\s+createHarthmereServiceInteriorBuildout[\s\S]*?\n\}/);
...
if (/HARTHMERE_(?:SERVICE_BLOCK_STAIR|RESIDENT_BLOCK_STAIR)_MAX_RISE[03]\s*=\s*0\.42/.test(src)) {
  pass("stair max rise is 0.42m (player/NPC accessible)");
} else {
  fail("stair max rise is 0.42m", "expected max rise constant = 0.42");
}
if (/HARTHMERE_(?:SERVICE_BLOCK_STAIR|RESIDENT_BLOCK_STAIR)_MIN_TREAD[03]\s*=\s*0\.7[0-9]/.test(src)) {
  pass("stair min tread is ≥0.7m (player/NPC accessible)");
} else {
  fail("stair min tread is ≥0.7m", "expected min tread constant ≥0.70");
}

console.log(ok ? "\nRESULT: PASS enterability current" : "\nRESULT: FAIL enterability current");
process.exit(ok ? 0 : 1);
