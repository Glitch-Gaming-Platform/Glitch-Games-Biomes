...
check("stale current base role helper removed", !faces.includes("defaultHarthmereClothingForRoleBase"));
check("stale current merge helper removed", !faces.includes("mergeHarthmereLicensedClothingDefaults"));
check("stale current merge helper removed", !faces.includes("mergeHarthmereLicensedClothingDefaults"));

check("NPC walk/run animation version exists", npcs.includes("HARTHMERE_NPC_WALK_RUN_ANIMATION_VERSION"));
check("local dev NPC creates animation clips", npcs.includes("makeLocalDevVoxelNpcAnimationClips"));
...
check("mixed NPC records animation load check", npcs.includes("recordHarthmereNpcAnimationLoadCheck(three"));
check("NPC tick records animation execution check", npcs.includes("recordHarthmereNpcAnimationExecutionCheck("));
