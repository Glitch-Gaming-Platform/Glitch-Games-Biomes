...
console.log("== Harthmere live animation scenario browser tests current ==");
...
async function loadPlaywright() {
  ...
  for (const name of candidates) {
    try {
      return leasePlaywright(require(name), { baseUrl: url });
    } catch (error) {
      // try next
    }
  }
...
(async () => {
  ...
  try {
    ...
    const result = await page.evaluate(() => {
      const w = window;
      const api = w.__harthmereAnimationScenarioRegression || w.__harthmereLiveAnimationScenarioRegression;
      if (!api) {
        return { ok: false, error: "missing __harthmereAnimationScenarioRegression" };
      }
    ...
    assert("browser exposes current animation scenario API", result.ok, result.error);
    assert("current version is correct", result.version === "harthmere-live-animation-scenario-regression", result.version);
