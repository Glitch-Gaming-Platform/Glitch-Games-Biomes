...
console.log('== Harthmere live-mode backend safety cap current ==');

check(fs.existsSync(backendPath), 'live_mode_backend.ts exists');
check(backend.includes('HARTHMERE_LIVE_MODE_BACKEND_SAFETY_CAP'), 'current safety cap constant exists');
check(backend.includes('clampLiveModeMutationDelta'), 'current mutation delta clamp helper exists');
...
check(/function\s+recordDelta\s*\([^)]*\)\s*{[\s\S]*?clampLiveModeMutationDelta\(delta\)/.test(backend), 'recordDelta uses the current clamp helper before applying deltas');
