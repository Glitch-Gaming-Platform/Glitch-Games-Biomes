...
const groveEconomy = read('src/shared/harthmere/grove_economy_starter.ts');
const snapshotContent = read('src/shared/harthmere/snapshot_grove_content.ts');
...
console.log('== Harthmere shim startup + BiomesUI inventory icon regression current ==');

// Startup regression: snapshot_grove_content appends the economy data, so
// grove_economy_starter must not value-import back from it during CommonJS
// module evaluation. That was the source of the shim ReferenceError.
assert(/\.\.\.\(GROVE_ECONOMY_STARTER_NPCS|\.\.\.GROVE_ECONOMY_STARTER_NPCS/.test(snapshotContent), 'snapshot Grove content still appends economy NPC data');
assert(snapshotContent.includes('...GROVE_ECONOMY_STARTER_LANDMARKS'), 'snapshot Grove content still appends economy landmark data');
assert(snapshotContent.includes('...GROVE_ECONOMY_STARTER_QUESTS'), 'snapshot Grove content still appends economy quest data');
assert(!/import\s*\{[\s\S]*?\}\s*from\s*["']@\/shared\/harthmere\/snapshot_grove_content["']/.test(groveEconomy), 'grove economy starter has no runtime value import from snapshot_grove_content');
assert(/import\s+type\s*\{[\s\S]*SnapshotGroveQuest[\s\S]*\}\s*from\s*["']@\/shared\/harthmere\/snapshot_grove_content["']/.test(groveEconomy), 'grove economy starter keeps only erased type imports from snapshot_grove_content');
assert(!groveEconomy.includes('SNAPSHOT_GROVE_NPC_FEET_Y'), 'grove economy starter no longer reads SNAPSHOT_GROVE_NPC_FEET_Y during module initialization');
assert(!groveEconomy.includes('snapshotGroveFountainPosition'), 'grove economy starter no longer calls snapshotGroveFountainPosition during module initialization');
assert(groveEconomy.includes('GROVE_ECONOMY_AUTHORED_NPC_FEET_Y'), 'grove economy starter has local authored feet-Y constant');
assert(groveEconomy.includes('function groveEconomyFountainPosition'), 'grove economy starter has local fountain position helper');
