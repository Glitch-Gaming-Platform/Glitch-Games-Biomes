...
const files = {
  ...
  wildLoops: "src/shared/harthmere/wilds_gameplay_loops.ts",
...
const report = {
  version: "harthmere-bridge-wilds-implementation-audit",
...
report.bridge = {
  status: [
    /HARTHMERE_WALKABLE_BRIDGE_VERSION/.test(renderer),
    /HARTHMERE_WALKABLE_BRIDGE_COLLISION/.test(registry),
    /Old Bridge walkable bridge deck/i.test(renderer),
    /bridge parapet/i.test(renderer),
    /bronze disc/i.test(renderer),
    /old_bridge_crack_interaction_space/.test(read(files.mainQuestSpaces)),
  ].every(Boolean) ? "IMPLEMENTED" : "MISSING_OR_PARTIAL",
  checks: {
    helper: /createHarthmereOldBridgeWalkableParapets/.test(renderer),
    collisionException: /HARTHMERE_WALKABLE_BRIDGE_COLLISION/.test(registry),
    ...
    q1SpaceStillExists: /old_bridge_crack_interaction_space/.test(read(files.mainQuestSpaces)),
    bronzeDisc: /bronze disc set into Old Bridge cobbles/i.test(renderer),
    line: lineOf(renderer, "HARTHMERE_WALKABLE_BRIDGE Old Bridge"),
  },
};
...
const systems = [
  ["Resource ownership and theft law", /Resource Ownership and Theft Law|classifyHarthmereResourceOwnership|owned goods require permission/i.test(allText)],
  ["Overharvesting consequences", /overharvest|resourcePressure|forest_backlash/i.test(allText)],
  ["Public world events", /Public World Events|resolveHarthmereWildsPublicEvent|contribution required/i.test(allText)],
...
const jsonRel = "public/assets/harthmere/manifest/harthmere-bridge-wilds-audit.json";
const mdRel = "docs/harthmere/HARTHMERE_BRIDGE_WILDS_IMPLEMENTATION_AUDIT.md";
