...
const content = read('src/shared/harthmere/snapshot_grove_content_v75.ts');
...
ok(runtime.includes('snapshotGroveNpcIdForDialogLabelV103'), 'Grove dialog exports a real label/profile resolver for talkability');
...
ok(dialog.includes('snapshotGroveNpcIdForDialogLabelV103'), 'Talk prompt visibility uses the Grove tutorial resolver before live lore fallback');
...
if (failures) {
  console.error(`v103 Grove fountain tutorial talkability check failed: ${failures}`);
  process.exit(1);
}
console.log('v103 Grove fountain tutorial talkability check passed');
