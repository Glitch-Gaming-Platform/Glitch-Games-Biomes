...
function loadModule(root) {
  const modulePath = path.join(root, "src/shared/harthmere/third_party_combat_ai.ts");
  assert(fs.existsSync(modulePath), "third_party_combat_ai.ts exists");
  ...
  try {
    ...
    console.error("FAIL TypeScript package is required to load third_party_combat_ai.ts in tests");
...
function assertContracts(root) {
  ...
  assert(api.HARTHMERE_THIRD_PARTY_COMBAT_AI_VERSION === "harthmere-third-party-combat-ai", "version marker exists");
  const stack = api.HARTHMERE_THIRD_PARTY_COMBAT_AI_STACK;
  ...
  const readiness = api.validateHarthmereCombatAIReadiness();
...
function assertBehaviorTreeAndFSM(root) {
  ...
  const tree = api.buildMistreevousCombatTreeSpec("shield_guard");
  ...
  const dead = api.evaluateHarthmereCombatAIFSM({ actorCombatState: "dead", healthPercent: 0 });
  assert(!dead.canAct && dead.recommendedMode === "dead", "FSM blocks dead actors");
  const stunned = api.evaluateHarthmereCombatAIFSM({ actorCombatState: "stunned", healthPercent: 0.8 });
  assert(!stunned.canAct && stunned.recommendedMode === "incapacitated", "FSM blocks stunned actors");
  const flee = api.evaluateHarthmereCombatAIFSM({ actorCombatState: "in_combat", healthPercent: 0.12, targetId: "player:1", navmeshHasRetreat: true });
  assert(flee.canAct && flee.recommendedMode === "flee", "FSM recommends flee at low health");
  const revive = api.evaluateHarthmereCombatAIFSM({ actorCombatState: "in_combat", healthPercent: 0.9, downedAllyNearby: true, allyHealthLowestPercent: 0, manaPercent: 0.8 });
  assert(revive.recommendedMode === "revive", "FSM prioritizes downed ally revive");
}

function assertUtilityMovement(root) {
  const { api } = loadModule(root);
  const archer = api.chooseHarthmereCombatAIDecision({ archetypeId: "archer_kiter", actorId: "npc:archer", targetId: "player:1", distanceToTarget: 2, healthPercent: 0.9, lineOfSight: true, legalTargets: ["player:1"], nowMs: 1000 });
  ...
  const melee = api.chooseHarthmereCombatAIDecision({ archetypeId: "basic_melee", actorId: "npc:melee", targetId: "player:1", distanceToTarget: 20, healthPercent: 0.8, lineOfSight: true, legalTargets: ["player:1"], nowMs: 1000 });
  ...
  const guard = api.chooseHarthmereCombatAIDecision({ archetypeId: "shield_guard", actorId: "npc:guard", targetId: "player:1", distanceToTarget: 2, healthPercent: 0.45, allyHealthLowestPercent: 1, targetIsCasting: true, legalTargets: ["player:1"], nowMs: 1000 });
  assert(["shield_bash", "taunt", "block"].includes(guard.selectedActionId), `shield guard uses control/defense (${guard.selectedActionId})`);

  const healer = api.chooseHarthmereCombatAIDecision({ archetypeId: "healer_support", actorId: "npc:healer", targetId: "ally:1", distanceToTarget: 4, downedAllyNearby: true, allyHealthLowestPercent: 0, manaPercent: 0.8, legalTargets: ["ally:1"], nowMs: 1000 });
  assert(healer.selectedActionId === "revive_ally", "healer support revives downed ally");
}

function assertAdapterAvailability(root) {
  const { api } = loadModule(root);
  const emptyPlan = api.validateThirdPartyAIDependencyPlan({ dependencies: {} }, "");
  ...
  const fullPlan = api.validateThirdPartyAIDependencyPlan({ dependencies: { mistreevous: "^4", yuka: "^0", "recast-navigation": "^0" } }, "py_trees==2.4.0\ntransitions>=0.9\n");
  ...
  assert(api.createBehaviorTreeAdapterPlan("mistreevous").forbiddenMethods.includes("apply_damage"), "behavior-tree adapter forbids direct damage");
  assert(api.createSteeringAdapterPlan().forbiddenMethods.includes("teleport_actor"), "steering adapter forbids teleport authority");
  assert(api.createNavigationAdapterPlan().requiredMethods.includes("find_path"), "navigation adapter requires pathfinding method");
}

function assertNavigationPerception(root) {
  const { api } = loadModule(root);
  const b = api.createHarthmereCombatAIBlackboard({ archetypeId: "pack_wolf", targetId: "player:1", position: { x: 1, y: 0, z: 2 }, targetPosition: { x: 8, y: 0, z: 9 }, distanceToTarget: 9 });
  const steering = api.buildYukaSteeringPlan(b, "pack_wolf", "flank");
  ...
  const nav = api.buildRecastNavigationRequest(b, "pack_wolf", "flank");
  ...
  const fleeNav = api.buildRecastNavigationRequest({ archetypeId: "civilian_flee", targetId: "wolf", safeAnchorId: "guard_yard", distanceToTarget: 3, retreatPosition: { x: 0, y: 0, z: 20 } }, "civilian_flee", "flee_to_help");
...
function assertEndToEnd(root) {
  ...
  for (const scenario of scenarios) {
    const decision = api.chooseHarthmereCombatAIDecision(scenario.input);
...
function assertProductionHardening(root) {
  ...
  for (let i = 0; i < 300; i += 1) {
    const archetypeId = Object.keys(api.HARTHMERE_COMBAT_AI_ARCHETYPES)[i % Object.keys(api.HARTHMERE_COMBAT_AI_ARCHETYPES).length];
    const decisionA = api.chooseHarthmereCombatAIDecision({ archetypeId, actorId: `npc:${i}`, targetId: "player:1", legalTargets: ["player:1"], distanceToTarget: (i % 35) + 1, healthPercent: ((i % 100) + 1) / 100, manaPercent: 0.8, staminaPercent: 0.8, lineOfSight: i % 7 !== 0, deterministicSeed: 42, nowMs: 10000 });
    const decisionB = api.chooseHarthmereCombatAIDecision({ archetypeId, actorId: `npc:${i}`, targetId: "player:1", legalTargets: ["player:1"], distanceToTarget: (i % 35) + 1, healthPercent: ((i % 100) + 1) / 100, manaPercent: 0.8, staminaPercent: 0.8, lineOfSight: i % 7 !== 0, deterministicSeed: 42, nowMs: 10000 });
    assert(decisionA.selectedActionId === decisionB.selectedActionId, `deterministic decision ${i}`);
    assert(api.HARTHMERE_COMBAT_AI_ACTIONS[decisionA.selectedActionId], `decision ${i} selects known action ${decisionA.selectedActionId}`);
