...
assert(all.length === named.length + remaining.length, "combined current all count matches current + remaining");
assert(remaining.some((npc) => npc.id === "tessen_hark"), "Tessen Hark added beyond current");
assert(remaining.some((npc) => npc.id === "yenna_holt"), "Yenna Holt added beyond current");
...
console.log("RESULT: PASS npc total coverage current");
