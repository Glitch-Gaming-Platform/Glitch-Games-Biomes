...
ok("web main has no explicit Glitch proxy invalid-asset branch", !src.main.includes("GLITCH_PLAYER_MESH_PROXY") && !has(/isGlitchRuntimeForWeb\s*\(\)[\s\S]{0,120}assetServerMode\s*===\s*["']proxy["'][\s\S]{0,260}InvalidAssetExportServer/m, src.main));
...
ok("NPC non-player path falls back to visible voxel mesh instead of name-only invisibility", src.npcResources.includes("HARTHMERE_NPC_GALOIS_VISIBLE_FALLBACK") && has(/catch\s*\(error\)[\s\S]{0,700}makeLocalDevVoxelNpcGltf\(deps, id\)/m, src.npcResources));
