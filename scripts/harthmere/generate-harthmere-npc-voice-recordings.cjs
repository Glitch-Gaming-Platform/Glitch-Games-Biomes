...
function usage() {
  ...
  AZURE_SPEECH_KEY=... AZURE_SPEECH_REGION=eastus2 node scripts/harthmere/generate-harthmere-npc-voice-recordings.cjs --limit=25
...
async function main() {
  ...
  console.log(
    `Harthmere Azure voice recordings ${HARTHMERE_NPC_VOICE_CATALOG_VERSION}: ${rows.length} planned`
  );
  ...
  const manifest = {
    version: HARTHMERE_NPC_VOICE_CATALOG_VERSION,
