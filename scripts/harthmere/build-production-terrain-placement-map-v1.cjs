...
} = require("../../src/shared/harthmere/quest_compendium_v46");
const {
  HARTHMERE_MAIN_QUEST_SPACES_V47,
} = require("../../src/shared/harthmere/main_quest_spaces_v47");
const {
  shiftHarthmereAuthoredPositionToWorldV71,
} = require("../../src/shared/harthmere/coordinate_transform_v71");
const {
...
} = require("../../src/shared/harthmere/business_owner_npc_seed_v1");
const {
  HARTHMERE_BUSINESS_CUSTOMER_NPC_SEEDS_V1,
} = require("../../src/shared/harthmere/business_customer_npc_seed_v1");
const {
...
} = require("../../src/shared/harthmere/exotic_matter_caves_v1");
const {
  muckMonsterAreaForPositionV1,
} = require("../../src/shared/harthmere/muck_monster_aggression_ai_v1");

const VERSION = "harthmere-production-terrain-placement-map-v1";
...
function parseArgs(argv) {
  const args = {
    ...
    artifactOut: path.join(
      process.cwd(),
      "artifacts/harthmere-production-placement-map-v1/placement-map.json"
    ),
    tsOut: path.join(
      process.cwd(),
...
function usage() {
  console.log(`Usage:
  node scripts/harthmere/build-production-terrain-placement-map-v1.cjs --write
...
Options:
  ...
  --margin=96
  --y0=-64 --y1=160
  --artifact-out=artifacts/harthmere-production-placement-map-v1/placement-map.json
  --ts-out=src/shared/harthmere/generated/production_terrain_placement_map_v1.ts`);
}
...
function collectAuthoredPlacements() {
  ...
  for (const quest of HARTHMERE_QUEST_CATALOG_V46) {
    if (quest.location?.waypoint) {
      add({
        ...
        worldPosition: shiftHarthmereAuthoredPositionToWorldV71(
  ...
  for (const space of HARTHMERE_MAIN_QUEST_SPACES_V47) {
    for (const which of ["entry", "exit"]) {
      ...
      add({
        ...
        worldPosition: shiftHarthmereAuthoredPositionToWorldV71(authored),
  ...
  for (const marker of harthmereJobsBoardQuestMarkerPositionsV1()) {
    add({
  ...
  for (const marker of harthmereExoticMatterDepositQuestMarkersV1()) {
    add({
...
function buildOutdoorSpawnPoints(columns, maxPoints) {
  ...
  for (const column of columns.values()) {
    ...
    const area = muckMonsterAreaForPositionV1(position, 1.5);
    candidates.push({
...
function writeGeneratedTs(file, map) {
  ...
  const text = `// Auto-generated by scripts/harthmere/build-production-terrain-placement-map-v1.cjs.
// Do not edit by hand.

import type { HarthmereProductionTerrainPlacementMapV1 } from "@/shared/harthmere/production_terrain_placement_map_v1";

export const HARTHMERE_PRODUCTION_TERRAIN_PLACEMENT_MAP_GENERATED_AT_V1 =
  ${JSON.stringify(map.generatedAtIso)} as const;

export const HARTHMERE_PRODUCTION_TERRAIN_PLACEMENT_MAP_V1 = ${json} as const satisfies HarthmereProductionTerrainPlacementMapV1;
`;
