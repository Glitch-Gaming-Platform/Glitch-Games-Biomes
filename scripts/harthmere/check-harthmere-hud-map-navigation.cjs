...
if (legacySystemsPanelRetired) {
  ...
  ok(/SYSTEM_MENU_SELECTABLE_QUERY/.test(hud), "Systems menu still has a single selectable-query for every tab");
  ok(/data-harthmere-system-menu="true"/.test(hud), "black Systems menu root remains test-tagged");
  ok(/data-harthmere-systems-auto-focus="true"/.test(hud), "black Systems menu opts into current autofocus");
  ok(/findHarthmereSystemsMenuInitialFocus/.test(hud), "black Systems menu has a deterministic initial-focus helper");
  ok(/data-harthmere-auto-focus/.test(hud) && /data-harthmere-tutorial-item-highlight/.test(hud), "autofocus prefers quest/item affordances before generic buttons");
  ok(/requestAnimationFrame/.test(hud) && /focusHarthmereSystemMenuElement/.test(hud), "menu focuses and scrolls a real visible element after render");
  ...
  ok(/isTextEntryElement\(event\.target\)/.test(hud), "keyboard handler still refuses to steal arrows/Return from text inputs");
...
ok(/HARTHMERE_INVENTORY_TUTORIAL_ITEM_HIGHLIGHT/.test(inventory), "inventory still has the tutorial item highlight system");
ok(/data-harthmere-auto-focus=\{highlighted/.test(inventory), "highlighted backpack rows become autofocus targets");
ok(/data-harthmere-primary-action=\s*\{\s*highlighted/.test(inventory), "highlighted item primary actions can receive focus first");
...
ok(/SnapshotGroveMiniMapQuestMarkers/.test(minimap), "minimap still renders independent Snapshot Grove quest/item pins");
ok(/data-snapshot-grove-minimap-item/.test(minimap), "minimap item/pickup pins remain testable");
ok(/data-snapshot-grove-minimap-active/.test(minimap), "minimap active objective pin remains testable");
ok(/groveQuestMarkerRows/.test(quests), "center map still builds per-step active quest marker rows");
ok(/data-snapshot-grove-center-map-item/.test(quests), "center map item pins remain testable");
ok(/data-snapshot-grove-center-map-item-list/.test(quests), "center map keeps active lesson item-stop list support");
...
ok(questBlocks.length >= 25, "current quest authoring test parses all Grove quests");
...
if (failures) {
  console.error(`current HUD/map/navigation check failed: ${failures} failure(s)`);
  process.exit(1);
}
