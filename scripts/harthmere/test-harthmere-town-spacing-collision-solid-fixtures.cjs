...
console.log("== Harthmere town spacing, solid collision, and fixture support tests current ==");
...
check(
  ...
  (/createBuildingShell\(\{ name: "Black Anvil Smithy"[\s\S]{0,260}rot: Math\.PI \/ 2/.test(assets) || /createHarthmereBlockBuiltServiceBuilding\(\{ name: "Black Anvil Smithy"[\s\S]{0,260}rot: Math\.PI \/ 2/.test(assets)) &&
...
check(
  ...
  suite.includes("test-harthmere-town-spacing-collision-solid-fixtures.cjs"),
);
