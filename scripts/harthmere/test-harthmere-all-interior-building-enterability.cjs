...
function parseBuildingShells() {
  ...
  for (const call of parseLiteralCalls(assetsSrc, "createHarthmereBlockBuiltServiceBuilding")) {
...
console.log("== Harthmere all interior building enterability tests current ==");
console.log(`Root: ${root}`);
console.log(`Parsed building shells: ${buildingShells.length}; blockers: ${hardBlockers.length}; actors: ${actors.length}`);
console.log("");

check("current test has enough authored building shells to be meaningful", buildingShells.length >= 14, buildingShells.map((s) => s.name));
...
check(
  ...
  suiteSrc.includes("test-harthmere-all-interior-building-enterability.cjs"),
);
