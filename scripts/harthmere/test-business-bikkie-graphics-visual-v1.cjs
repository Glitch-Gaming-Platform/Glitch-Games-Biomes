...
} = require("../../src/shared/harthmere/mmo_farming_food_stamina_v1");
const {
...
} = require("../../src/shared/harthmere/bikkie_visual_resolver_v1");
...
const htmlPath = path.join(outDir, "business-bikkie-graphics-visual-audit-v1.html");
const desktopPng = path.join(outDir, "business-bikkie-graphics-visual-audit-v1-desktop.png");
const mobilePng = path.join(outDir, "business-bikkie-graphics-visual-audit-v1-mobile.png");
const summaryPath = path.join(outDir, "business-bikkie-graphics-visual-audit-v1.json");
...
function servicePropCards() {
  ...
  for (const definition of Object.values(HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS_V1)) {
    for (const offer of definition.offers) {
      ...
      const svg = renderHarthmereBusinessServiceFrameSvgV1(clip, frame, { width: 220, height: 150 });
      rows.push(`
...
function foodCatalogVisualCards() {
  return Object.entries(HARTHMERE_BIKKIE_ITEM_METADATA_BY_ID_V1)
...
function exoticMatterVisualCards() {
  ensureHarthmereProductionCraftingCatalogueV1();
  const ids = Object.values(HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1);
  const blockIds = new Set(HARTHMERE_EXOTIC_MATTER_BLOCK_ITEM_IDS_V1);
  return ids
    .map((itemId) => {
      const item = getHarthmereItemDefinitionV1(itemId);
      const visual = harthmereResolveBikkieVisualV1({
        id: itemId,
...
function renderHtml() {
  const entries = Object.entries(HARTHMERE_BUSINESS_BIKKIE_GRAPHICS_V1);
  const fixtureCount = entries.reduce((sum, [, graphics]) => sum + graphics.length, 0);
  const foodCatalogCount = Object.keys(HARTHMERE_BIKKIE_ITEM_METADATA_BY_ID_V1).length;
  const exoticCount = Object.keys(HARTHMERE_EXOTIC_MATTER_ITEM_IDS_V1).length;
  const mapMarkerCount = HARTHMERE_BUSINESS_OUTPOST_MAP_MARKERS_V1.length;
  return `<!doctype html>
<html>
<head>
  <meta charset="utf-8"/>
  ...
  <main>
    <section class="hero">
      ...
      <div class="summary">
        ...
        <span>${Object.values(HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS_V1).flatMap((definition) => definition.offers).length} service prop frames</span>
...
(async () => {
  console.log("== Harthmere business Bikkie graphics Playwright visual audit v1 ==");
  console.log(`Root: ${root}`);
  console.log(`Output: ${outDir}`);
  fs.mkdirSync(outDir, { recursive: true });

  const metadataAudit = validateHarthmereBusinessBikkieGraphicsV1();
  ...
  const expectedBusinessCount = Object.keys(HARTHMERE_BUSINESS_BIKKIE_GRAPHICS_V1).length;
  const expectedFixtureCount = Object.values(HARTHMERE_BUSINESS_BIKKIE_GRAPHICS_V1).reduce((sum, graphics) => sum + graphics.length, 0);
  const expectedPropCount = Object.values(HARTHMERE_BUSINESS_MINIGAME_DEFINITIONS_V1).flatMap((definition) => definition.offers).length;
