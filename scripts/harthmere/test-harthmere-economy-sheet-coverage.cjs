...
const {
  ...
  read,
  checkFactory,
} = require("./harthmere-economy-optimization-test-lib.cjs");

const root = process.argv[2] || process.cwd();
const { check, finish } = checkFactory();

console.log("== test-harthmere-economy-sheet-coverage ==");
...
const src = exists(root, "src/client/components/challenges/LocalDevHarthmereEconomyOptimizationSystem.tsx")
  ? economySource(root)
  : "";
const suite = exists(root, "scripts/harthmere/test-harthmere-town-placement-suite.cjs")
  ? read(root, "scripts/harthmere/test-harthmere-town-placement-suite.cjs")
  : "";
...
check("full suite includes sheet coverage test", suite.includes("test-harthmere-economy-sheet-coverage.cjs"));
