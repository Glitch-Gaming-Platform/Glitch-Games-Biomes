...
const installerPath = path.join(root, "scripts/harthmere/install-harthmere-ai-deps.sh");
...
console.log("== Harthmere NPC AI dependency install command tests current ==");
...
check(
  ...
  installer.includes("test-harthmere-npc-ai-adapter-runtime-safety.cjs") ||
    installer.includes("test-harthmere-npm-peer-mass-audit.cjs")
);
check(
  "installer preserves third-party runtime availability test",
  installer.includes("test-harthmere-npc-ai-third-party-runtime-availability.cjs")
);

const suite = fs.existsSync(suitePath) ? fs.readFileSync(suitePath, "utf8") : "";
check("full suite includes dependency install command test", suite.includes("test-harthmere-npc-ai-dependency-install-command.cjs"));
