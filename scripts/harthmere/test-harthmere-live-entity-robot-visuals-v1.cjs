...
const artifactsDir = path.resolve(
  process.env.HARTHMERE_LIVE_ENTITY_VISUAL_ARTIFACTS_DIR ||
    path.join(repo, "artifacts", "harthmere-live-entity-robot-visuals-v1")
);
  ...
  return await page.evaluate((target) => {
    ...
    if (typeof liveDebug?.teleportTo === "function") {
      const result = liveDebug.teleportTo({
        ...
        source: "harthmere-live-entity-robot-visuals-v1",
      });
...
async function terrainColumnForArea(page, area) {
  return await page.evaluate((target) => {
    const diagnostics = window.__snapshotDiagnosticsV78;
...
async function main() {
  console.log("== Harthmere live entity robot visual smoke v1 ==");
  ...
  check(
    validateLiveEntityRobotProtectionAreasV1().length === 0,
  ...
  try {
    ...
    for (const area of LIVE_ENTITY_ROBOT_PROTECTION_AREAS_V1) {
      ...
      check(
        probe.robotActors.length >= LIVE_ENTITY_ROBOT_PROTECTION_AREAS_V1.length,
