...
check("NPC clothing uses material recolor, not only spatial shell", runtime.includes("setHarthmereRuntimeNpcMeshColor") && runtime.includes("townsperson-body"));
check("NPC clothing recolor is called from procedural townsperson path", runtime.includes("addHarthmereRuntimeNpcClothingMaterialRecolor(root, appearance, body, palette, torsoY, shoulderY);"));
