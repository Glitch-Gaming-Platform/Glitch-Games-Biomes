...
console.log('== Harthmere live world chat current ==');
...
const deploy = fs.existsSync(path.join(root, 'scripts/glitch/deploy-production-local-redis-smoke.sh'))
...
ok(runtime.includes('Harthmere world chat current sends'), 'HUD Chat panel documents real live chat delivery');
...
if (deploy) {
  ok(deploy.includes('test-harthmere-world-chat-live.cjs'), 'production deploy guardrails include live world chat test');
}
