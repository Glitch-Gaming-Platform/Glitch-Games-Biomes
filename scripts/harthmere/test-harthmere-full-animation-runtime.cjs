...
const paths = {
  contract: path.join(root, "src/shared/harthmere/animation_runtime_contracts.ts"),
  ...
  suite: path.join(root, "scripts/harthmere/test-harthmere-town-placement-suite.cjs"),
};
...
console.log("== Harthmere full animation runtime tests current ==");
...
check("shared contract exposes full animation runtime version", files.contract.includes("HARTHMERE_FULL_ANIMATION_RUNTIME_VERSION"));
...
check("player system exposes full body animation current marker", files.player.includes("HARTHMERE_FULL_BODY_ANIMATION_RUNTIME_VERSION"));
...
const helperStart = files.player.indexOf("function getHarthmereWeaponSyncedEmoteWeights");
const helperEnd = files.player.indexOf("function getHarthmereStableAnimationVelocity");
const helperBody = helperStart >= 0 && helperEnd > helperStart ? files.player.slice(helperStart, helperEnd) : "";
check("weapon-synced body helper no longer recursively calls itself", !helperBody.includes("const weaponSyncedWeights = getHarthmereWeaponSyncedEmoteWeights"), "Self-recursion here would cause runtime stack overflow and jitter.");
...
check("getEmoteBasedWeights applies weapon-synced body weights once", emoteBody.includes("getHarthmereWeaponSyncedEmoteWeights"));
check("body animation still preserves lower-body locomotion for upper-body actions", hasAll(files.player, ["notArms: \"ifIdle\"", "HARTHMERE_FULL_BODY_POSE_LAYER_RULES"]));
check("HUD installs comprehensive animation runtime bridge", hasAll(files.hud, ["HARTHMERE_FULL_ANIMATION_RUNTIME_BRIDGE_VERSION", "useHarthmereComprehensiveAnimationRuntimeBridge();"]));
check("HUD bridge exposes browser debug API", hasAll(files.hud, ["__harthmereAnimationRuntime", "record:", "request:", "snapshot:"]));
...
check("HUD bridge listens for animation request events", hasAll(files.hud, ["biomes:harthmere-animation-request", "addEventListener(HARTHMERE_FULL_ANIMATION_REQUEST_EVENT"]));
check("NPC system exposes full animation runtime marker", files.npcs.includes("HARTHMERE_NPC_FULL_ANIMATION_RUNTIME_VERSION"));
check("NPC system covers creature/social/boss state families", hasAll(files.npcs, ["HARTHMERE_NPC_CREATURE_ANIMATION_STATES", "HARTHMERE_NPC_SOCIAL_ANIMATION_STATES", "HARTHMERE_NPC_BOSS_ANIMATION_STATES"]));
...
check("renderer exposes full animation runtime debug marker", files.renderer.includes("HARTHMERE_RENDERER_FULL_ANIMATION_RUNTIME_VERSION"));
check("renderer exposes pose catalog for all families", hasAll(files.renderer, families) && files.renderer.includes("HARTHMERE_RENDERER_ANIMATION_DEBUG_POSES"));
