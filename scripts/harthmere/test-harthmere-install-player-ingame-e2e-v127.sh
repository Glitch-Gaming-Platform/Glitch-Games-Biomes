...
ARTIFACTS_DIR="${E2E_ARTIFACTS_DIR:-/tmp/harthmere-playboot-e2e-v127}"
...
echo 'Preflight 0: validate v127 patch files'
node "$REPO/scripts/harthmere/validate-harthmere-install-id-flow-v127.cjs" "$REPO"

echo 'Preflight 1: unit tests'
node "$REPO/scripts/harthmere/test-harthmere-install-id-flow-unit-v127.cjs" "$REPO"

echo 'Preflight 2: web responds'
curl -fsS "$BASE_URL/api/bikkie" >/tmp/harthmere-e2e-bikkie.json

echo 'Preflight 3: install autoLogin and auth check using isolated cookie jar'
...
echo 'Browser E2E: install_id to player-in-game (v127 checkpointed)'
E2E_ARTIFACTS_DIR="$ARTIFACTS_DIR" \
INSTALL_ID="$INSTALL_ID" \
BASE_URL="$BASE_URL" \
node "$REPO/scripts/harthmere/test-harthmere-install-player-ingame-e2e-v127.cjs" "$REPO" \
  --base-url "$BASE_URL" \
