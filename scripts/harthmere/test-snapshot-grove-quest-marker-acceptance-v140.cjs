...
const contentPath = path.join(root, 'src/shared/harthmere/snapshot_grove_content_v75.ts');
...
ok(
  ...
  deploy.includes('test-snapshot-grove-quest-marker-acceptance-v140.cjs'),
);
ok(
  'stale v182 WakeUpScreen visual coverage guardrail is removed from production deploy smoke',
  !deploy.includes('test-harthmere-character-builder-supported-voxel-features-v182.cjs'),
);
