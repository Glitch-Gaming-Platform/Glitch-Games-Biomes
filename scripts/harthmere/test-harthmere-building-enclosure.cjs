...
check("service buildings have stone theme function", src.includes("harthmereServiceStoneTheme"));
...
check("resident/slum housing no longer picks wood wall blocks", !/function harthmereResidentWallBlockAsset[\s\S]*?arch_wall_wood[\s\S]*?function createHarthmereResidentFloorDeckBlocks/.test(src));
check("resident/slum housing wall function returns stone/ore", /function harthmereResidentWallBlockAsset[\s\S]*mine_stone_01[\s\S]*arch_wall_stone[\s\S]*function createHarthmereResidentFloorDeckBlocks/.test(src));
check("resident/slum housing theme uses stone wall", /function harthmereHousingTheme[\s\S]*wall:\s*"arch_wall_stone"[\s\S]*door:\s*"arch_wall_door"/.test(src));
