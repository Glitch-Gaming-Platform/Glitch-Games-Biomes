...
console.log("== Harthmere residential/slums block-built accessibility current ==");
...
check(
  "current keeps prior fixes and adds block-built resident housing version marker",
  /HARTHMERE_RESIDENT_HOUSING_BLOCK_BUILD_VERSION/.test(manifest) && /HARTHMERE_RESIDENT_HOUSING_RENDERER_VERSION/.test(assets) && /HARTHMERE_RESIDENT_HOUSING_STONE_SHELL_VERSION/.test(manifest) && /HARTHMERE_RESIDENT_HOUSING_RENDERER_VERSION/.test(assets)
);
check(
  "slum and residential buildings are generated from solid stone/ore blocks, not floating-only props",
  /createHarthmereResidentWallBlocks/.test(assets) &&
    /arch_wall_stone/.test(assets) &&
    (/solid stone\/ore house shell/.test(assets) || /BUILDING_VOXEL_MESHES whole voxel building mesh/.test(assets)) &&
...
check(
  ...
  /function createHarthmereResidentHousingPlacements\(\)[\s\S]*createHarthmereResidentStoryFrame\(building\)[\s\S]*createHarthmereResidentRoomDecorPlacements\(building\)/.test(assets) &&
    !/function createHarthmereResidentHousingPlacements\(\)[\s\S]*createBuildingShell\(\{[\s\S]*name: building\.name/.test(assets)
);
check(
  "each floor has real floor deck support before room decor is placed",
  /createHarthmereResidentFloorDeckBlocks/.test(assets) &&
    /solid stone ceiling and floor slab enclosing the story shell/.test(assets) &&
    /placed on block floor deck/.test(assets)
);
const maxRise = numberAfter(assets, /HARTHMERE_RESIDENT_BLOCK_STAIR_MAX_RISE = ([0-9.]+)/);
const minTread = numberAfter(assets, /HARTHMERE_RESIDENT_BLOCK_STAIR_MIN_TREAD = ([0-9.]+)/);
check(
  ...
  Number.isFinite(maxRise) && maxRise <= 0.45 &&
    ...
    /createHarthmereBlockStairRun/.test(assets) &&
    (/solid stone block stair riser/.test(assets) || /BUILDING_VOXEL_MESHES exterior voxel stair mesh/.test(assets)) &&
...
check(
  ...
  /test-harthmere-residential-slums-block-build\.cjs/.test(suite)
);
