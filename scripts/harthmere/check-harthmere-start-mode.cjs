...
ok(text.includes('HARTHMERE_START_MODE_GUARD'), 'current Harthmere start mode guard marker exists');
...
ok(text.includes('shouldMoveExistingSnapshotPlayerToHarthmereStart'), 'existing Grove-positioned player relocation helper exists');
ok(text.includes('BIOMES_KEEP_EXISTING_PLAYER_POSITION'), 'developer escape hatch exists');
ok(text.includes('isInsideAuthoredSnapshotGroveStartArea(position)'), 'existing-player relocation only targets authored Grove start area');
ok(text.includes('moving to Harthmere'), 'server log explains existing player relocation');

if (!process.exitCode) {
  console.log('\nHarthmere current start-mode checks passed.');
}
