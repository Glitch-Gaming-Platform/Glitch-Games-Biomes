...
report.check("player animation system maps attack1 to Attack", /attack1\s*:\s*\{\s*fileAnimationName\s*:\s*["']Attack["']/.test(playerAnimations), "Expected attack1 -> Attack");
report.check("player animation system maps attack2 to HeavyAttack with Attack2 fallback", /attack2\s*:\s*\{\s*fileAnimationName\s*:\s*["']HeavyAttack["'][\s\S]{0,140}backupFileAnimationNames\s*:\s*\[\s*["']Attack2["']\s*\]/.test(playerAnimations), "Expected attack2 -> HeavyAttack, fallback Attack2");
