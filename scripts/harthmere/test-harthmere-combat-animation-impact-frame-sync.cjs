...
check("impact window is locked to readable 12-15 frame range", () => {
  assert(/HARTHMERE_COMBAT_ANIMATION_IMPACT_FRAME_WINDOW\s*=\s*\[12,\s*15\]/.test(shared), "impact window should be [12, 15]");
});
...
finish("combat animation impact-frame sync current");
