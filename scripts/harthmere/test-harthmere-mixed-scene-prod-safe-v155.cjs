#!/usr/bin/env node
// HARTHMERE_MIXED_SCENE_PROD_SAFE_V155
...
console.log('== Harthmere mixed scene production-safe v155 ==');

const scenes = fs.readFileSync(scenesPath, 'utf8');
const playerMesh = fs.readFileSync(playerMeshPath, 'utf8');

ok(scenes.includes('HARTHMERE_MIXED_SCENE_TYPE_PROD_SAFE_FALLBACK_V155'), 'v155 fallback marker exists');
...
ok(scenes.includes('chooseMixedSceneFallbackV155'), 'mixed fallback decision is centralized');
ok(scenes.includes('isBasePassCoercedPlayerRoot'), 'player-only base fallback is guarded by explicit marker');
ok(scenes.includes('harthmere-player-avatar-base-pass-materials-v153'), 'fallback recognizes the player material coercion marker');
