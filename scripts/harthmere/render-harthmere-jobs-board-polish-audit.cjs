...
const htmlPath = path.join(outDir, "grove-jobs-board-polish-audit.html");
...
<body data-harthmere-jobs-board-visual-audit="current">
  ...
  <script>
    ...
    window.__harthmereJobsBoardPolishAudit = {
...
function main() {
  ...
  if (audit.polishVersion !== HARTHMERE_JOBS_BOARD_PROCEDURAL_POLISH_VERSION) {
    throw new Error("Jobs Board mesh did not carry the production polish version");
  }
  if (audit.markerVersion !== HARTHMERE_JOBS_BOARD_PROCEDURAL_MARKER_VERSION) {
