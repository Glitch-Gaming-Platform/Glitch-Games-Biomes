...
ok(bridge.includes("snapshot-road-ahead-full-chain"), "current full chain marker exists");
for (const token of [
...
ok(bridge.includes("shouldEventCompleteStep"), "mission chain has trigger matcher");
...
ok(bridge.includes("recordSnapshotMissionReward"), "visible reward log is wired");
ok(bridge.includes("arrivalRadius"), "location marker arrival completion is wired");
ok(hud.includes("SnapshotMissionRuntimeController"), "runtime controller is mounted in HUD");
