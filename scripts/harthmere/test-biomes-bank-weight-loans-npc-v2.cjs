...
const groveContent = read('src/shared/harthmere/snapshot_grove_content_v75.ts');
const groveRuntime = read('src/client/components/challenges/LocalDevSnapshotGroveBibleRuntime.tsx');
const reducerTests = read('src/shared/harthmere/test/live_mode_backend_v1.test.ts');
...
contains(groveRuntime, 'groveBankerProgressiveQuestionActionsV1', 'Grove banker has progressive Q&A actions');
...
contains(backend, 'HARTHMERE_CARRY_WEIGHT_LIMIT_V1', 'Backend has server carry-weight limit');
contains(backend, 'harthmereInventoryCarryWeightV1', 'Backend computes inventory carry weight');
contains(backend, 'wouldExceedCarryWeightV1', 'Backend checks incoming weight');
contains(backend, 'wouldDirectInventoryPayloadExceedCarryWeightV1', 'Backend checks direct inventory item-delta weight');
contains(backend, 'wouldCraftExceedCarryWeightV1', 'Backend checks post-craft weight');
contains(backend, 'pushCarryWeightRejectionV1(warnings, touchedModels, "bank_withdraw")', 'Backend rejects overweight bank withdraws');
contains(backend, 'pushCarryWeightRejectionV1(warnings, touchedModels, "vendor")', 'Backend rejects overweight vendor buys');
contains(backend, 'pushCarryWeightRejectionV1(warnings, touchedModels, "auction_settle")', 'Backend rejects overweight auction buys');
contains(backend, 'pushCarryWeightRejectionV1(warnings, touchedModels, "mail_claim")', 'Backend rejects overweight mail claims');
contains(backend, 'pushCarryWeightRejectionV1(warnings, touchedModels, "crafting")', 'Backend rejects overweight crafting output');
contains(backend, 'applyHarthmereBankLoanConsequencesV1', 'Backend applies loan consequences');
contains(backend, 'bank_loan_defaulted', 'Backend logs loan defaults');
contains(backend, 'HARTHMERE_BANK_CREDIT_HOLD_FLAG_V1', 'Backend enforces bank credit hold');
...
contains(bankRoute, 'applyHarthmereBankLoanConsequencesV1', 'Bank state GET applies overdue loan consequences');

contains(reducerTests, 'banking v2 carry weight enforcement', 'Reducer tests cover carry weight enforcement');
...
console.log('All Biomes banking weight/loan/NPC v2 checks passed.');
