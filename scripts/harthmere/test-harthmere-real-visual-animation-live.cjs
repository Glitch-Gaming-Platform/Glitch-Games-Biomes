...
(async () => {
  ...
  assert("drawn sword is closer to left hand than right hand", drawn.leftHandDistanceMeters < drawn.rightHandDistanceMeters, drawn);
  assert("drawn sword respects left-hand distance budget", drawn.leftHandDistanceMeters <= 0.14, drawn);
  assert("main hand side score is on visual left", drawn.mainHandSideScore > 0, drawn);
