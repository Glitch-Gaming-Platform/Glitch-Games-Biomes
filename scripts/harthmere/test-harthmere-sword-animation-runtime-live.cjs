...
const report = makeReporter("Harthmere live sword animation runtime tests current", root);
...
async function main() {
  ...
  try {
    ...
    await page.evaluateOnNewDocument(() => {
      ...
      window.__harthmereSwordRuntimeLog = [];
      const push = (type, detail) => {
        window.__harthmereSwordRuntimeLog.unshift({ at: Date.now(), type, detail });
        window.__harthmereSwordRuntimeLog.length = Math.min(window.__harthmereSwordRuntimeLog.length, 200);
      };
    ...
    const afterDraw = await page.evaluate(() => {
      ...
      const log = window.__harthmereSwordRuntimeLog || [];
    ...
    const directionResult = await page.evaluate(async () => {
      ...
      window.__harthmereForwardArcRuntime = { position: [10, 53.05, 20], forward: [1, 0], bodyForward: [1, 0], yaw: Math.PI / 2, at: Date.now(), source: "sword_runtime_east" };
      ...
      const east = snapshot();
      window.__harthmereForwardArcRuntime = { position: [10, 53.05, 20], forward: [0, 1], bodyForward: [0, 1], yaw: 0, at: Date.now(), source: "sword_runtime_north" };
      window.dispatchEvent(new CustomEvent("biomes:harthmere-player-sword-visual", { detail: { action: "draw", drawn: true, itemId: "iron_longsword", at: Date.now() } }));
      await new Promise((resolve) => setTimeout(resolve, 300));
