...
const artifactsDir = path.resolve(
  process.env.HARTHMERE_QUEST_MARKER_VISUAL_ARTIFACTS_DIR ||
    path.join(repo, "artifacts", "harthmere-quest-marker-visuals-v1")
);
...
} = require("../../src/shared/harthmere/jobs_board_business_templates_v146");
const {
  LIVE_ENTITY_HELPER_QUEST_TARGET_MARKERS_V1,
} = require("../../src/shared/harthmere/live_entity_helper_quests_v1");
const {
...
} = require("../../src/shared/harthmere/jobs_board_quest_marker_positions_v1");
...
function landmarkForId(markerId) {
  return SNAPSHOT_GROVE_LANDMARKS_V75.find((landmark) => landmark.id === markerId);
}

function seedJobsBoard(boardId, ticks = 50) {
  let state = defaultHarthmereJobsBoardStateV1(1_800_000_000_000);
  for (let i = 0; i < ticks; i += 1) {
    state = reduceHarthmereJobsBoardMutationV1(
      state,
      {
...
function buildAuditSets() {
  const objectiveIds = Array.from(
    new Set(SNAPSHOT_GROVE_QUESTS_V75.flatMap((quest) => quest.markerIds))
  );
  ...
  const groveSeed = seedJobsBoard(HARTHMERE_JOBS_BOARD_DEFAULT_BOARD_ID_V1);
  const harthmereSeed = seedJobsBoard(HARTHMERE_JOBS_BOARD_HARTHMERE_BOARD_ID_V141);
  const seededJobs = [
  ...
  const monsterHuntMarkerIds = seededJobs
    ...
    .map((job) => job.mapMarkerId)
    .filter(Boolean);
  const businessTemplateMarkerIds = HARTHMERE_JOBS_BOARD_BUSINESS_TEMPLATES_V146.map(
    (template) => template.mapMarkerId
  ).filter(Boolean);
  const helperMarkerIds = LIVE_ENTITY_HELPER_QUEST_TARGET_MARKERS_V1.map(
    (marker) => marker.id
  );
  return {
...
async function activateHelperMarkers(page) {
  await page.evaluate(() => {
    ...
    window.localStorage.setItem(
      `biomes.localDev.liveEntityHelperQuests.v1.user.${scope}`,
      JSON.stringify({
        active: {
          "visual:exotic": {
    ...
    window.dispatchEvent(new Event("biomes:live-entity-helper-quest-v1"));
  });
}
...
async function collectMarkerDebug(page) {
  return page.evaluate(() => {
    ...
    const questDebug = window.__harthmereQuestObjectMarkerDebugV145;
    const boardDebug = window.__harthmereJobsBoardMarkerDebugV144;
    let questMarkers = [];
    let boards = [];
    try {
...
async function waitForMarkerRuntime(page, expectedMarkerId) {
  ...
  while (Date.now() < deadline) {
    ...
    const ready =
      ...
      lastProbe.questMarkers.length >= HARTHMERE_QUEST_OBJECT_MARKERS_V145.length &&
      lastProbe.boards.length >= HARTHMERE_JOBS_BOARD_MARKER_LOCATIONS_V144.length &&
...
function expectedPositionForVisualId(markerId) {
  const dedicatedBoard = HARTHMERE_JOBS_BOARD_MARKER_LOCATIONS_V144.find(
    (board) =>
      board.id === markerId ||
  ...
  return harthmereJobsBoardQuestMarkerPositionForIdV1(markerId)?.position;
}
...
async function main() {
  console.log("== Harthmere quest marker visual audit v1 ==");
  ...
  const unresolved = unresolvedHarthmereJobsBoardQuestMarkerIdsV1(
  ...
  const renderedIds = new Set(HARTHMERE_QUEST_OBJECT_MARKERS_V145.map((marker) => marker.id));
  ...
  for (const board of [
    HARTHMERE_JOBS_BOARD_LOCATIONS_V1[HARTHMERE_JOBS_BOARD_DEFAULT_BOARD_ID_V1],
    HARTHMERE_JOBS_BOARD_LOCATIONS_V1[HARTHMERE_JOBS_BOARD_HARTHMERE_BOARD_ID_V141],
  ]) {
    const rendered = HARTHMERE_JOBS_BOARD_MARKER_LOCATIONS_V144.find(
