...
let makeReporter;
try {
  ({ makeReporter } = require("./harthmere-town-rule-test-utils.cjs"));
} catch {
...
const report = makeReporter("Harthmere creature/social/death/weapon-handtracking animation tests current", root);
...
const contract = read("src/shared/harthmere/animation_visual_regression_contracts.ts");
const suite = read("scripts/harthmere/test-harthmere-town-placement-suite.cjs");

report.check(
  "current shared contract exists for creatures/social/death/handtracking",
  /HARTHMERE_CREATURE_SOCIAL_DEATH_HANDTRACKING_VERSION/.test(contract),
  "Expected shared current animation visual regression contract.",
);
report.check(
  "current contract covers weapon hand tracking max grip distance",
  /HARTHMERE_WEAPON_HAND_TRACKING_CONTRACT[\s\S]*maxGripDistanceMeters:\s*0\.22/.test(contract),
  "Weapon/hand tests need a concrete max distance, not just an event hook.",
);
report.check(
  "current contract covers creature and animal states",
  /HARTHMERE_CREATURE_ANIMAL_ANIMATION_CONTRACT[\s\S]*idle[\s\S]*walk[\s\S]*run[\s\S]*attack[\s\S]*hit[\s\S]*death[\s\S]*flee[\s\S]*turnInPlace[\s\S]*pathVelocitySync/.test(contract),
  "Creature/animal coverage must include locomotion, combat, death, flee, turn smoothing, and velocity sync.",
);
report.check(
  "current contract covers social/work NPC states",
  /HARTHMERE_SOCIAL_WORK_NPC_ANIMATION_CONTRACT[\s\S]*vendorIdle[\s\S]*talkGesture[\s\S]*questGesture[\s\S]*sit[\s\S]*eat[\s\S]*drink[\s\S]*sleep[\s\S]*workLoop[\s\S]*smithWork[\s\S]*cookWork[\s\S]*dockWork[\s\S]*healerWork[\s\S]*guardPatrolIdle[\s\S]*crowdEmote/.test(contract),
  "Social/work NPC animations need explicit states tied to town schedules and roles.",
);
report.check(
  "current contract prevents instant death disappearance",
  /HARTHMERE_DEATH_RESPAWN_CINEMATIC_CONTRACT[\s\S]*corpseHoldScale:\s*0\.84[\s\S]*npcCorpseShouldRemainVisible:\s*true[\s\S]*playerControlsLockDuringDeath:\s*true/.test(contract),
...
report.check(
  ...
  /HARTHMERE_NPC_DEATH_CORPSE_HOLD_SCALE\s*=\s*0\.84/.test(npcs) &&
    /HARTHMERE_NPC_DEATH_ANIMATION_DURATION_SECS\s*=\s*(?:1\.25|1\.8)/.test(npcs) &&
...
report.check(
  ...
  /this\.mixedMesh\.three\.visible\s*=\s*true/.test(npcs) && /harthmereDeathRespawnCinematic/.test(npcs),
...
report.check(
  ...
  /HARTHMERE_NPC_BODY_LOCOMOTION_DEADZONE_SPEED\s*=\s*0\.06/.test(npcs) && /HARTHMERE_NPC_BODY_MAX_BLEND_DT\s*=\s*1\s*\/\s*24/.test(npcs),
  "Creature locomotion must keep the current velocity deadzone and dt cap.",
);

report.check(
  "player runtime still prefers aligned action clips after current",
...
report.check(
  ...
  !/\nconst HARTHMERE_BODY_WEAPON_VISUAL_COHESION_VERSION\s*=\s*\n\s*"harthmere-body-weapon-visual-cohesion";\s*\n\s*private ensureHarthmerePlayerSword/.test(assets),
...
report.check(
  ...
  /HARTHMERE_WEAPON_HAND_TRACKING_VERSION/.test(assets) &&
    /harthmereWeaponHandTrackingCurrentAnchor/.test(assets) &&
...
report.check(
  ...
  /HARTHMERE_WEAPON_HAND_GRIP_MAX_DISTANCE\s*=\s*0\.22/.test(assets) && /harthmerePlayerWeaponGripDistanceLast/.test(assets) && /harthmerePlayerWeaponHandTrackingSamples/.test(assets),
...
report.check(
  ...
  /actor\.object\.visible\s*=\s*true/.test(assets) && /harthmereDeathRespawnCinematic/.test(assets),
  "Combat actors should hold a visible death pose instead of disappearing.",
);
report.check(
  "full suite includes current animation test",
  /test-harthmere-creature-social-death-handtracking\.cjs/.test(suite),
