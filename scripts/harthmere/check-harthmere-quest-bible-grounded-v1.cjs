#!/usr/bin/env node
// check-harthmere-quest-bible-grounded-v1.cjs
//
...
// (harthmere_quest_bible_grounded_v1.test.ts) checks structurally.
...
const catalogFile = read('src/shared/harthmere/quest_compendium_v46.ts');

// Extract the JSON catalog literal.
const m = catalogFile.match(/HARTHMERE_QUEST_CATALOG_V46_JSON\s*=\s*`(\[[\s\S]*?\])`;/);
...
// 4. Givers resolve.
const v44 = read('src/shared/harthmere/npc_compendium_v44.ts');
const v45 = read('src/shared/harthmere/npc_compendium_v45.ts');
...
const KNOWN_MISALIGNED_GROVE_V1 = new Set([
...
const groveQuestBlock = groveContent.match(/SNAPSHOT_GROVE_QUESTS_V75[\s\S]*?\];/);
...
if (groveQuestBlock) {
  ...
  for (let i = 0; i < objectivesArrays.length; i++) {
    ...
    if (KNOWN_MISALIGNED_GROVE_V1.has(qid)) continue;
  ...
  ok(mismatches === 0, `Grove quest objectives/triggers/markerIds arrays align per quest (${mismatches} new mismatches, ${KNOWN_MISALIGNED_GROVE_V1.size} grandfathered)`);
}
