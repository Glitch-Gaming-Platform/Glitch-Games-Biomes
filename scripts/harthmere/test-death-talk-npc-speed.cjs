...
assert(
  death.includes("HARTHMERE_DEATH_SCREEN_VERSION"),
  "Death screen current marker missing."
);
assert(
  death.includes("HarthmereDeathScreenOverlay"),
...
assert(
  death.includes('data-harthmere-death-respawn-grove="true"'),
...
assert(
  death.includes("HARTHMERE_GROVE_RESPAWN_TELEPORT_TARGET"),
...
assert(
  death.includes('id === "the_grove"') &&
    death.includes("respawnHarthmerePlayerAtGrove()"),
...
assert(
  esc.includes("harthmereDeathScreenActive"),
  "ESC menu death guard missing."
);
assert(
  esc.includes('data-harthmere-esc-hidden-for-death-screen="true"'),
...
assert(
  overlays.includes("HARTHMERE_NPC_TALK_INSPECT_RADIUS"),
...
assert(
  grove.includes("SNAPSHOT_GROVE_NPC_ROUTE_SPEED_MULTIPLIER"),
  "Grove route speed multiplier marker missing."
);
assert(
  grove.includes("SNAPSHOT_GROVE_NPC_ROUTE_SPEED_MULTIPLIER = 1.35"),
...
console.log("PASS death-talk-npc-speed");
