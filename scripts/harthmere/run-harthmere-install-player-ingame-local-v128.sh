...
echo 'Step 1/7: validate v127 source patch (must be in place)'
  ...
  if [ -f "$envfile" ]; then
    if grep -E '^\s*NEXT_PUBLIC_GLITCH_SYNC_BASE_URL\s*=' "$envfile" >/dev/null 2>&1; then
      ...
      if echo "$VALUE" | grep -Eiq 'azurecontainerapps|biomes\.gg|prod'; then
        echo "  NOTE: $envfile NEXT_PUBLIC_GLITCH_SYNC_BASE_URL looks remote. v127 runtime override neutralizes it."
      fi
    fi
  fi
done

echo 'Step 3/7: v127 unit tests'
...
echo 'Step 7/7: run v128 browser E2E (headless WebGL graceful)'
INSTALL_ID="$INSTALL_ID" \
BASE_URL="http://127.0.0.1:3017" \
DOCKER_CONTAINER="biomes-local" \
HEADLESS="$HEADLESS" \
STRICT_RENDER="$STRICT_RENDER" \
bash scripts/harthmere/test-harthmere-install-player-ingame-e2e-v128.sh "$REPO"
