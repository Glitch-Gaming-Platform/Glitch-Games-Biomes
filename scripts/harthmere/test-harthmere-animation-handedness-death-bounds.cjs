...
const contract = read("src/shared/harthmere/animation_handedness_death_bounds_contracts.ts");
...
ok("shared current contract exists", contract.includes("HARTHMERE_ANIMATION_HANDEDNESS_DEATH_BOUNDS_VERSION"));
ok("full suite includes current handedness/death/bounds test", suite.includes("test-harthmere-animation-handedness-death-bounds.cjs"));

ok("main hand visual contract says visible attacking hand is left", contract.includes('primaryAttackVisualSide: "left"'));
ok("main-hand fallback anchor is on the visual attack side", contract.includes('primaryFallbackAnchor: "harthmere-anchor-left-hand"'));
ok("shield/offhand fallback anchor is opposite main-hand", contract.includes('shieldFallbackAnchor: "harthmere-anchor-right-hand"'));
ok("renderer melee fallback now uses visible attack-hand anchor", assets.includes('activeWeaponProfile === "shield" ? "harthmere-anchor-right-hand" : "harthmere-anchor-left-hand"'));
ok("renderer melee bone lookup follows visible attack hand", assets.includes('? ["righthand"') && assets.includes(': ["lefthand"'));
ok("renderer writes handedness debug payload on weapon", assets.includes("harthmereHandednessDeathBounds"));
ok("renderer debug bridge exposes current handedness probe", assets.includes("handednessProbe"));
...
ok("renderer exposes death all-actor probe", assets.includes("deathAllActorsProbe"));
ok("renderer exposes death bounds probe", assets.includes("deathBoundsProbe"));
ok("renderer exposes world effect visibility probe", assets.includes("worldEffectVisibilityProbe"));
