...
const placementSuitePath = path.join(root, "scripts/harthmere/test-harthmere-town-placement-suite.cjs");
...
console.log("== Harthmere runtime navigation, collision, and service reachability tests current ==");
...
check("runtime collision export still exposes current obstacle profiles", assetsSrc.includes("HARTHMERE_COLLISION_PROFILE_OBSTACLE_EXPORT") && assetsSrc.includes("harthmerePlayerCollisionObstacleShapeForPlacement"));
...
check("full placement suite includes this runtime navigation checker", fs.existsSync(placementSuitePath) && read(placementSuitePath).includes("test-harthmere-runtime-navigation-collision.cjs"));
