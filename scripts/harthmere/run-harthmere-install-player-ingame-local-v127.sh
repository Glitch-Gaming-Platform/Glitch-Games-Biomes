...
echo 'Step 1/7: validate v127 test files'
node scripts/harthmere/validate-harthmere-install-id-flow-v127.cjs "$REPO"

echo 'Step 2/7: env_preflight - check .env / .env.local for stale NEXT_PUBLIC_GLITCH_SYNC_BASE_URL'
STALE_ENV_FOUND=0
for envfile in .env .env.local .env.production .env.production.local; do
  if [ -f "$envfile" ]; then
    if grep -E '^\s*NEXT_PUBLIC_GLITCH_SYNC_BASE_URL\s*=' "$envfile" >/dev/null 2>&1; then
      VALUE="$(grep -E '^\s*NEXT_PUBLIC_GLITCH_SYNC_BASE_URL\s*=' "$envfile" | sed 's/^[^=]*=//')"
      printf '  %s: NEXT_PUBLIC_GLITCH_SYNC_BASE_URL=%s\n' "$envfile" "$VALUE"
      if echo "$VALUE" | grep -Eiq 'azurecontainerapps|biomes\.gg|prod'; then
        STALE_ENV_FOUND=1
        echo "  WARNING: $envfile NEXT_PUBLIC_GLITCH_SYNC_BASE_URL looks remote." >&2
      fi
    fi
  fi
done
if [ "$STALE_ENV_FOUND" = "1" ]; then
  echo '  v127 runtime override will reject the remote URL and use local fallback, but consider clearing .env.local for cleanliness.' >&2
fi

echo 'Step 3/7: unit tests (pre-build)'
if [ -d node_modules/typescript ]; then
  node scripts/harthmere/test-harthmere-install-id-flow-unit-v127.cjs "$REPO"
else
  echo '  SKIP: node_modules/typescript not installed; will re-run unit tests inside the container check.'
fi

echo 'Step 4/7: clean production build (full .next wipe to avoid stale bundles)'
rm -rf .next node_modules/.cache/webpack dist
mkdir -p dist
...
echo 'Step 4b/7: post-build sanity - scan built chunks for any remote sync host residue'
# This is informational. v127's runtime resolver guarantees the local host is
# used at runtime even if a remote URL is baked in, but if a remote URL is
# baked in we want a heads-up so the human can fix .env.local.
if find .next -name "*.js" -print0 2>/dev/null | xargs -0 grep -l 'azurecontainerapps' 2>/dev/null | head -1 >/dev/null; then
  echo '  NOTE: built chunks reference azurecontainerapps. v127 runtime override will still force local sync.'
fi

echo 'Step 5/7: docker build'
docker buildx build \
...
echo 'Step 6/7: start redis and local production container'
...
echo 'Step 6b/7: wait for web server'
...
echo 'Step 7/7: run full browser E2E until player is in-game'
INSTALL_ID="$INSTALL_ID" \
BASE_URL="http://127.0.0.1:3017" \
DOCKER_CONTAINER="biomes-local" \
bash scripts/harthmere/test-harthmere-install-player-ingame-e2e-v127.sh "$REPO"
