#!/usr/bin/env node
// SNAPSHOT_GROVE_PRODUCTION_DIALOGUE:
...
ok(
  /SYSTEM_TAB_HIGHLIGHTS/.test(hud),
  "HUD defines SYSTEM_TAB_HIGHLIGHTS production highlight copy per tab",
);
ok(
...
ok(
  ...
  "Grove runtime version constant records the current production-dialogue polish (or current successor)",
);
...
if (failures) {
  console.error(`current Grove production dialogue check failed: ${failures} failure(s)`);
  process.exit(1);
}
console.log("current Grove production dialogue check passed");
