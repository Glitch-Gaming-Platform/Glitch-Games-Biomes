...
check("terrain seed loop calls dungeon air carve", /!harthmereV6ShouldCarveDungeonAirBlockAt\((?:worldX|authoredWorldX), worldY, (?:worldZ|authoredWorldZ)\)/.test(main));
...
check("runtime filter is called before floating/performance prep", /serverVoxelFiltered = filterHarthmereServerVoxelOwnedStructuralPlacementsV6[45]\((?:placements|shiftedPlacements)\)[\s\S]*filterHarthmereUnsupportedFloatingBlockPlacementsV3\(serverVoxelFiltered\.placements\)/.test(assets));
check("NPC collision cache also ignores server-voxel-owned structural GLBs", /serverVoxelCollisionPlacements = filterHarthmereServerVoxelOwnedStructuralPlacementsV6[45]\(PLACEMENTS\)\.placements[\s\S]*serverVoxelCollisionPlacements\.map/.test(assets));
