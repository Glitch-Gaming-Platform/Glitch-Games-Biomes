...
} = require(path.join(repoRoot, "src/shared/harthmere/business_customer_simulator_v1"));
const {
...
const HTML_PATH = path.join(OUT_DIR, "business-outpost-visual-audit-v1.html");
const SUMMARY_PATH = path.join(OUT_DIR, "business-outpost-visual-audit-v1.json");
...
function card(outpost) {
  ...
  const appearance = makeHarthmereNpcAppearanceConfig({
    ...
    source: "harthmere-business-outpost-visual-audit-v1",
  });
...
const summary = HARTHMERE_BUSINESS_OUTPOSTS_V1.map((outpost) => {
  const record = HARTHMERE_BUSINESS_OUTPOST_PROCEDURAL_BUILDINGS_V1[outpost.outpostId];
  return {
...
<body>
  ...
  <main class="grid">
    ${HARTHMERE_BUSINESS_OUTPOSTS_V1.map(card).join("\n")}
  </main>
</body>
</html>`;
