#!/usr/bin/env node
"use strict";
/* HARTHMERE_BUILDING_BIBLE_COVERAGE_TEST
 *
 * Asserts that every building named in the Harthmere story bible is placed
 * in the renderer via createHarthmereBlockBuiltServiceBuilding or the
 * current housing equivalent. Reads the required manifest from the shared
...
for (const r of required) {
  ...
  // Match either service or housing wrapper. Both end up using current walls.
  const re = new RegExp(
    "(?:createHarthmereBlockBuiltServiceBuilding|createHarthmereBlockBuiltHousingPlacements)" +
...
console.log(ok ? "\nRESULT: PASS bible building coverage current" : "\nRESULT: FAIL bible building coverage current");
process.exit(ok ? 0 : 1);
