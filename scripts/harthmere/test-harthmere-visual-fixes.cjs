...
console.log("== Harthmere visual fixes current ==");
...
check(
  ...
  /HARTHMERE_PROCEDURAL_ANIMAL_GAIT_VERSION/.test(text) && /function applyAnimalLegSwing\(/.test(text)
);
check(
...
check(
  "weapons resolve to left-hand anchors and shield to right-hand",
  /activeWeaponProfile === "shield"\s*\? \["righthand"/.test(text) && /: \["lefthand"/.test(text) && /activeWeaponProfile === "shield" \? "harthmere-anchor-right-hand" : "harthmere-anchor-left-hand"/.test(text)
);
check(
  "weapon orientation includes outward blade offsets",
  /weaponGripPitchOffset/.test(text) && /weaponGripYawOffset/.test(text) && /bladeForwardMode: "outward"/.test(text)
);
check(
  "large body clothing helper exists",
  /HARTHMERE_LARGE_BODY_CLOTHING_VISIBILITY_VERSION/.test(text) && /function ensureHarthmereLargeBodyClothingVisibility\(/.test(text)
);
check(
...
check(
  ...
  /ensureHarthmereLargeBodyClothingVisibility\(root, appearance, body\);/.test(text)
);
check(
  "full suite includes current visual fixes test",
  fs.existsSync(suite) && /test-harthmere-visual-fixes\.cjs/.test(fs.readFileSync(suite, "utf8"))
);
console.log();
