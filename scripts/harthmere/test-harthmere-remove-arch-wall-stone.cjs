...
ok(src.includes("HARTHMERE_REMOVE_ARCH_WALL_STONE_RUNTIME_VERSION"), "current runtime removal marker exists");
ok(src.includes('return placement.asset === "arch_wall_stone";'), "arch_wall_stone is removed at runtime only");
ok(src.includes("placementsWithoutRemovedAssets"), "runtime placements filter out removed assets before performance filtering");
