#!/usr/bin/env node
"use strict";
/* HARTHMERE_BIBLE_IMPLEMENTATION_AUDIT
 ...
 * Writes:
 * - docs/harthmere/HARTHMERE_BIBLE_IMPLEMENTATION_AUDIT.md
 * - public/assets/harthmere/manifest/harthmere-bible-implementation-audit.json
 */
const fs = require("fs");
...
const paths = {
  ...
  blockContract: "src/shared/harthmere/town_block_build.ts",
  housing: "src/shared/harthmere/resident_housing.ts",
  questCatalog: "src/shared/harthmere/bible/bible_quest_catalog.ts",
  questRuntime: "src/shared/harthmere/bible_quest_live_authority.ts",
  questSpaces: "src/shared/harthmere/main_quest_spaces.ts",
  boss: "src/shared/harthmere/thaedryn_boss.ts",
  wilds: "src/shared/harthmere/wilds_gameplay_loops.ts",
  ...
  measuredDimensions: "src/shared/harthmere/uploaded_asset_dimensions.ts",
};
...
const report = {
  version: "harthmere-bible-implementation-audit",
  root,
...
function buildingImplemented(r) {
  ...
  const strictRe = new RegExp(`(?:createHarthmereBlockBuiltServiceBuilding|createHarthmereBlockBuiltHousingPlacements)[\\s\\S]{0,80}name:\\s*"${escapeRe(name)}"`, "i");
...
report.housing = {
  ...
  roomDecorEvidence: /HARTHMERE_RESIDENTIAL_ROOM_DECOR[\s\S]*HARTHMERE_SLUM_ROOM_DECOR/.test(housing),
};
...
report.npc = {
  namedCount: (npc44.match(/^\s*"implementationStatus"\s*:/gm) || []).length,
  remainingCount: (npc45.match(/^\s*"implementationStatus"\s*:/gm) || []).length,
...
if (/Yenna/.test(npc44) && /Mira Holt|mira_holt/.test(npc45)) report.designWarnings.push("Bram Holt child-name continuity issue: current references sick Yenna, while current adds Mira Holt. Normalize this before final narrative lock.");
...
if (!report.quests.runtimeFilesExist) report.designWarnings.push("Quest catalog exists but one or more current runtime/space/boss/wilds files are missing.");
...
if (!exists(paths.measuredDimensions)) {
  productionManualGaps.push("current measured asset-dimension registry not found in this checkout. Run the current asset-size patch before relying on collision-size audit.");
}
...
const md = [];
md.push(`# Harthmere Bible Implementation Audit current`);
md.push("");
...
md.push("");
md.push(`Residential buildings: **${report.housing.residentialBuildings}**. Estimated residential room capacity from current pattern: **${report.housing.residentialCapacity ?? "unknown"}**.`);
...
md.push("");
md.push(`Named NPC compendium current count: **${report.npc.namedCount}**. Remaining/ambient/wildlife/etc. current count: **${report.npc.remainingCount}**. Total NPC records with implementation status: **${report.npc.totalCount}**.`);
...
const reportPath = abs("docs/harthmere/HARTHMERE_BIBLE_IMPLEMENTATION_AUDIT.md");
const jsonPath = abs("public/assets/harthmere/manifest/harthmere-bible-implementation-audit.json");
fs.mkdirSync(path.dirname(reportPath), { recursive: true });
fs.mkdirSync(path.dirname(jsonPath), { recursive: true });
fs.writeFileSync(reportPath, md.join("\n"));
fs.writeFileSync(jsonPath, JSON.stringify(report, null, 2));

console.log("== Harthmere Bible implementation audit current ==");
