...
console.log("== Harthmere live animation scenario regression tests current ==");
console.log(`Root: ${root}\n`);

const contract = read("src/shared/harthmere/animation_live_scenario_contracts.ts");
...
ok("shared current scenario contract exists", contract.includes("HARTHMERE_LIVE_ANIMATION_SCENARIO_REGRESSION_VERSION"));
ok("renderer exposes current debug bridge", assets.includes("__harthmereAnimationScenarioRegression"));
ok("renderer aliases current debug bridge for live tests", assets.includes("__harthmereLiveAnimationScenarioRegression"));
ok("full suite includes live animation scenario current test", suite.includes("test-harthmere-live-animation-scenario-regression.cjs"));

ok("current contract samples frame 0 / 8 / 15 / 22 / 30 across the swing", allPresent(contract, ["[0, 8, 15, 22, 30]"]));
ok("current grip budget remains tight enough for blocky body scale", numberFrom(contract, /GRIP_BUDGET_METERS\s*=\s*([0-9.]+)/) <= 0.22);
...
ok("renderer current snapshot probes weapon hand tracking", assets.includes("weaponHandTracking"));
ok("renderer current snapshot probes resource hit telegraph state", assets.includes("resourceHitTelegraphState"));
ok("renderer current snapshot probes object/effect range", assets.includes("objectEffectRangeAudit"));
ok("renderer current snapshot probes creature animation audits", assets.includes("creatureAnimationAudit"));
ok("renderer current snapshot probes social/work animation audits", assets.includes("socialWorkAnimationAudit"));
ok("renderer current snapshot probes death/respawn audits", assets.includes("deathRespawnCinematicAudit"));

ok("current API exposes two-handed weapon probe", assets.includes("twoHandedProbe"));
ok("current API exposes locomotion/action probe", assets.includes("locomotionActionProbe"));
ok("current API exposes NPC interruption probe", assets.includes("npcInterruptionProbe"));
ok("current API exposes player death/respawn probe", assets.includes("playerDeathRespawnProbe"));
ok("current API exposes resource hit visibility probe", assets.includes("resourceHitVisibilityProbe"));
ok("current API exposes multiplayer animation probe", assets.includes("multiplayerAnimationProbe"));
ok("current API exposes animation performance probe", assets.includes("performanceProbe"));
ok("current API exposes location/effect probe", assets.includes("locationEffectProbe"));
