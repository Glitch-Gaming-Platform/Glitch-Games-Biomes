...
const canonical = read("src/shared/harthmere/snapshot_complete_port_v76.ts");
const overlay = read(
...
const objectSemantics = read(
  "src/shared/harthmere/object_interaction_semantics_v1.ts"
);
const helperQuests = read(
  "src/shared/harthmere/live_entity_helper_quests_v1.ts"
);
const ecsBridge = read("src/shared/harthmere/live_entity_ecs_bridge_v1.ts");
...
ok(
  bridge.includes('const SNAPSHOT_MISSION_TITLE_V73 = "Road Ahead"') &&
    bridge.includes(
...
ok(
  canonical.includes("SNAPSHOT_OFFICIAL_NUX_CHALLENGES_V76"),
  "canonical v76 Road Ahead chain is still present"
);
...
ok(
  overlay.includes("openHarthmereObjectContainerV1") &&
    overlay.includes("isHarthmereContainerObjectLabelV1") &&
...
ok(
  objectInteractions.includes("HARTHMERE_JOBS_BOARD_OPEN_EVENT_V141") &&
    objectInteractions.includes(
      'dispatchHarthmereHudActionEventV96("crafting")'
    ) &&
    objectInteractions.includes("harthmereReadableObjectTextForLabelV1"),
  "client object interactions route jobs boards, craft/cook stations, and readable text through real UI paths"
);
ok(
  dialogueSemantics.includes("isHarthmereNonLivingObjectLabelV1") &&
    helperQuests.includes("isHarthmereNonLivingObjectLabelV1") &&
    ecsBridge.includes("isHarthmereNonLivingObjectLabelV1"),
