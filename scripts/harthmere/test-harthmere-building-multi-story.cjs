#!/usr/bin/env node
"use strict";
/* HARTHMERE_BUILDING_MULTI_STORY_TEST
 *
 ...
 *   - emits per-story wall ring (current called per floor)
...
const svcBlockMatch = src.match(/function\s+createHarthmereBlockBuiltServiceBuilding[\s\S]*?\n\}/);
const svcBody = svcBlockMatch ? svcBlockMatch[0] : "";
if (svcBody && /for\s*\(\s*let\s+floor\s*=\s*1\s*;[^)]*floors[^)]*\)/.test(svcBody)) {
  pass("service buildings iterate per-floor in createHarthmereBlockBuiltServiceBuilding");
} else {
  fail("service buildings iterate per-floor in createHarthmereBlockBuiltServiceBuilding",
       "expected `for (let floor = 1; floor <= floors; ...)` loop");
}

// 2. Per-story stair run for service buildings
if (/createHarthmereServiceBlockStairRun\s*\(/.test(svcBody)) {
  pass("service buildings emit per-story block stair runs");
} else {
  fail("service buildings emit per-story block stair runs",
       "expected createHarthmereServiceBlockStairRun call");
}

// 3. Per-story loop in resident housing
const hsBlockMatch = src.match(/function\s+createHarthmereBlockBuiltHousingPlacements[\s\S]*?\n\}/);
const hsBody = hsBlockMatch ? hsBlockMatch[0] : "";
if (hsBody && /for\s*\(\s*let\s+floor\s*=\s*1\s*;[^)]*floors[^)]*\)/.test(hsBody)) {
  pass("housing buildings iterate per-floor in createHarthmereBlockBuiltHousingPlacements");
} else {
  fail("housing buildings iterate per-floor in createHarthmereBlockBuiltHousingPlacements",
       "expected `for (let floor = 1; floor <= floors; ...)` loop");
}

// 4. Per-story stair run for housing
if (/createHarthmereBlockStairRun\s*\(/.test(hsBody)) {
  pass("housing buildings emit per-story block stair runs");
} else {
  fail("housing buildings emit per-story block stair runs",
       "expected createHarthmereBlockStairRun call");
}
...
for (const name of requiredTwoStory) {
  const re = new RegExp(
    "createHarthmereBlockBuiltServiceBuilding\\([^)]*name:\\s*\"" +
      name.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&") +
      "\"[^)]*floors:\\s*2",
    "i",
  );
  if (re.test(src)) {
    ...
    fail(`multi-story building present with floors:2 — ${name}`,
         "expected createHarthmereBlockBuiltServiceBuilding({ ... name: \"" + name + "\" ... floors: 2 })");
  }
}
...
console.log(ok ? "\nRESULT: PASS multi-story current" : "\nRESULT: FAIL multi-story current");
process.exit(ok ? 0 : 1);
