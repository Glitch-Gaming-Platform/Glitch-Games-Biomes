...
for (const npc of remaining) {
  ...
  assert(!signatures.has(npc.face.signature), `${npc.id} visual signature unique across current+current`);
...
console.log("RESULT: PASS npc remaining face weapon stats inventory clothing current");
