...
const { loadTown, makeReporter, isSolidUploadedAsset, collisionBlocksPlayerLikeRegistry } = require("./harthmere-town-rule-test-utils.cjs");
const root = process.argv[2] ? path.resolve(process.argv[2]) : process.cwd();
const report = makeReporter("Harthmere uploaded asset solid collision tests current", root);
