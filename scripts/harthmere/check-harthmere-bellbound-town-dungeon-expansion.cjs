#!/usr/bin/env node
"use strict";
/* HARTHMERE_BELLBOUND_TOWN_DUNGEON_EXPANSION_TEST */
const fs = require("fs");
...
console.log("== Harthmere Bellbound town/building/dungeon expansion tests current ==");
...
const marker = "// HARTHMERE_BELLBOUND_TOWN_DUNGEON_EXPANSION";
const markerIndex = src.indexOf(marker);
if (markerIndex >= 0) {
  pass("Bellbound town/dungeon expansion marker exists");
} else {
  fail("Bellbound town/dungeon expansion marker exists", "Missing HARTHMERE_BELLBOUND_TOWN_DUNGEON_EXPANSION");
}
