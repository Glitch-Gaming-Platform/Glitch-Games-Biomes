...
report.check("moving NPC LOD follows current object position so walkers do not disappear away from spawn", /isHarthmereRuntimeLifePlacement\(instance\.placement\)[\s\S]{0,160}instance\.object\.position\.x[\s\S]{0,220}instance\.object\.position\.z/.test(assetsSrc));
