...
ok(bridge.includes("snapshot-road-ahead-full-chain"), "Road Ahead full-chain runtime is present");
ok(bridge.includes("shouldEventCompleteStep"), "Road Ahead validates gameplay events before advancing");
...
ok(bridge.includes("hasRequiredClothing"), "Road Ahead gear step checks worn top and bottoms");
...
ok(bridge.includes("pinSnapshotMissionTarget") && bridge.includes("mapManager.addNavigationAid"), "Road Ahead pins the active step marker");
ok(bridge.includes("autoremoveWhenNear: true"), "Road Ahead active marker disappears when the player reaches it");
ok(bridge.includes("mapManager.removeNavigationAid?.(SNAPSHOT_MISSION_NAV_AID_ID)"), "Road Ahead removes the marker after completion");
ok(hud.includes("SnapshotMissionRuntimeController"), "Road Ahead runtime controller is mounted in the HUD");
...
if (failures) {
  console.error(`current Road Ahead marker/action validation check failed: ${failures} failure(s)`);
  process.exit(1);
}
console.log("current Road Ahead marker/action validation check passed");
