...
const fs = require("fs");
const { loadTown, makeReporter } = require("./harthmere-town-rule-test-utils.cjs");
const root = process.argv[2] ? path.resolve(process.argv[2]) : process.cwd();
const report = makeReporter("Harthmere visual/readability audit tests current", root);
const town = loadTown(root);
const visualFiles = ["src/shared/harthmere/visual_readability.ts", "src/client/game/renderers/local_dev/harthmere_visual_readability.ts", "scripts/harthmere/check-harthmere-town-visual-readability.cjs"].map((p) => path.join(root, p));
