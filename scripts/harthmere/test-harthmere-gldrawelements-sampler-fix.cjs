#!/usr/bin/env node
// HARTHMERE_GLDRAWELEMENTS_SAMPLER_FIX
...
//   current  — playerMeshUrlForId routes the player mesh through
//            /api/assets/player_mesh.glb so the skeleton/boneTexture is
//            always fully initialized by GLTFLoader before first render.
//   current  — animationTrackCanBindToMesh() filters out every KeyframeTrack
...
//   1. current pruning exists in AnimationSystem.newState
...
console.log("== Harthmere glDrawElements sampler fix current ==");
...
// ── 1. current animation track pruning is in AnimationSystem.newState ────────────
check(
  animSystem.includes("HARTHMERE_STATIC_FALLBACK_ANIMATION_TARGET_PRUNING"),
  "current marker present in animation_system.ts"
);
check(
  ...
  "current: collectAnimationTargetNodeNames + animationTrackCanBindToMesh called in newState"
);
check(
  animSystem.includes("anim.tracks = anim.tracks.filter(") &&
    animSystem.includes("animationTrackCanBindToMesh"),
  "current: tracks are filtered with animationTrackCanBindToMesh before clipAction"
);
...
check(
  ...
  "current: zero-track clips return undefined instead of calling clipAction"
);

// ── 2. NPC system routes through the same AnimationSystem.newState ────────────
check(
  npcs.includes("npcSystem.newState("),
  "NPC meshes call npcSystem.newState (inherits current track pruning)"
);
