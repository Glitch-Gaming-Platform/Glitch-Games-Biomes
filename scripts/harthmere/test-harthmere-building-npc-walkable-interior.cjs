#!/usr/bin/env node
"use strict";
/* HARTHMERE_BUILDING_NPC_WALKABLE_INTERIOR_TEST
 *
 ...
 *   1. References the current contract for HARTHMERE_INTERIOR_FREE_TILES_MIN
 *      (2x2 minimum interior tile area).
 *   2. The accessibility marker stones are emitted in createHarthmere
 *      ResidentRoomDecorPlacements at room centres.
...
if (/HARTHMERE_INTERIOR_FREE_TILES_MIN\s*=\s*4\b/.test(contract)) {
  pass("contract declares minimum 2x2 (4-tile) free interior area for NPC stand+path");
} else {
  fail("contract declares minimum 2x2 free interior area", "expected HARTHMERE_INTERIOR_FREE_TILES_MIN = 4");
}
...
//    in createHarthmereServiceInteriorBuildout should have at most 3
...
console.log(ok ? "\nRESULT: PASS npc walkable interior" : "\nRESULT: FAIL npc walkable interior");
process.exit(ok ? 0 : 1);
