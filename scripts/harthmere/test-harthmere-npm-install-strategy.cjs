...
const installer = path.join(root, 'scripts/harthmere/install-harthmere-ai-deps.sh');
const audit = path.join(root, 'scripts/harthmere/run-harthmere-npm-peer-audit.sh');
let ok = true;
function check(label, condition) { if (condition) console.log(`OK ${label}`); else { console.log(`FAIL ${label}`); ok = false; } }
console.log('== Harthmere npm install strategy tests current ==');
...
check('installer runs mass peer audit tests before install', /test-harthmere-npm-peer-mass-audit\.cjs/.test(text));
