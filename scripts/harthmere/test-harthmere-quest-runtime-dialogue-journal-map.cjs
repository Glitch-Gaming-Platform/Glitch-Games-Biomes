...
const bridgePath = path.join(root, "src/client/components/challenges/LocalDevHarthmereQuestRuntime.tsx");
const bridge = fs.existsSync(bridgePath) ? fs.readFileSync(bridgePath, "utf8") : "";
check("dialogue links exported", runtime.includes("HARTHMERE_QUEST_DIALOGUE_LINKS"));
...
for (const fn of ["getHarthmereDialogueQuestOffers", "getHarthmereQuestJournalEntry", "getHarthmereQuestMapHint"]) check(`exports ${fn}`, runtime.includes(`function ${fn}`));
...
check("local dev quest bridge exists", bridge.includes("HARTHMERE_LOCAL_DEV_QUEST_RUNTIME_BRIDGE_VERSION"));
...
console.log("RESULT: PASS quest runtime dialogue journal map current");
