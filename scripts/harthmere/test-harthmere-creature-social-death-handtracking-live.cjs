...
const report = makeReporter("Harthmere live creature/social/death/weapon-handtracking animation tests current", root);
...
const settleMs = Number(process.env.HARTHMERE_ANIMATION_SETTLE_MS || 750);
...
async function main() {
  ...
  try {
    ...
    report.fail("puppeteer is installed", "Install repo browser-test dependencies first; live current test uses Puppeteer.");
    report.finish();
    return;
  }
...
main().catch((error) => {
  report.fail("live current animation test completed", error?.stack || String(error));
  report.finish();
});
