...
const h = createHarness("Harthmere production vendor catalog current");

const clientCatalog = vendorSource(h);
const productionCatalog = h.read("src/shared/harthmere/harthmere_vendor_catalog.ts");
const liveBackend = h.read("src/shared/harthmere/live_mode_backend.ts");
...
const seedBlock = productionCatalog.slice(
  productionCatalog.indexOf("const HARTHMERE_VENDOR_ITEM_DEFINITIONS"),
...
h.ok(
  productionCatalog.includes("registerHarthmereVendorEntry"),
  "production catalog registers vendor entries"
);
h.ok(
  productionCatalog.includes("registerHarthmereItemDefinition"),
  "production catalog registers missing item definitions"
);
h.ok(
  productionCatalog.includes("ensureHarthmereProductionCraftingCatalogue"),
  "production catalog preserves crafting catalogue definitions first"
);
h.ok(
  liveBackend.includes("ensureHarthmereProductionVendorCatalog"),
