...
function envelope(actionKind, payload = {}, actorId = "player-building-v5") {
...
function apply(state, payload, now = 2000, actorId = "player-building-v5") {
...
console.log("== Building System production in-world v5 ==");
...
check(catalogSource.includes("BUILDING_SYSTEM_BUSINESS_TYPES_V1"), "business economy catalogue exists");
...
check(catalogSource.includes("createBuildingSystemDemolitionMaterializationPlanV1") && catalogSource.includes("demolition_cleanup"), "shared catalogue creates demolition voxel cleanup plans");
check(catalogSource.includes("createBuildingSystemStorageContainerV1") && catalogSource.includes("createBuildingSystemDoorLockV1"), "shared catalogue creates physical storage containers and door locks");
check(catalogSource.includes("createBuildingSystemPlacementPreviewV1") && catalogSource.includes("ghostFootprint"), "shared catalogue creates blueprint ghost preview contracts");
check(catalogSource.includes("createBuildingSystemRepairDamageMaterializationPlanV1") && catalogSource.includes("createBuildingSystemRepairRestoreMaterializationPlanV1"), "shared catalogue maps repair decay/restoration to visible voxel edits");
check(catalogSource.includes("createBuildingSystemUpgradeMaterializationPlanV1") && catalogSource.includes("upgrade_addition"), "shared catalogue maps tier upgrades to physical voxel additions");
check(backendSource.includes("createBuildingSystemMiraMapMarkerV1") && adapterSource.includes("mira_grove_land_steward"), "Mira/Miranda is injected as a map marker");
...
check(BUILDING_SYSTEM_BUSINESS_TYPES_V1.length === 19, "business catalogue implements all 19 PDF business types");
const traderDef = buildingSystemBusinessTypeByIdV1("general_trader");
...
const restaurantDef = buildingSystemBusinessTypeByIdV1("food_service_restaurant");
...
let state = defaultHarthmereLiveModeBackendStateV1("player-building-v5", 1000);
...
check(buildingSystemCanUseStorageContainerV1({ property, container: storage, actorId: "player-building-v5" }) === true, "owner can use in-world storage container");
check(buildingSystemCanUseStorageContainerV1({ property, container: storage, actorId: "stranger" }) === false, "stranger cannot use private in-world storage container");
check(buildingSystemCanOpenDoorLockV1({ property, lock: door, actorId: "player-building-v5" }) === true, "owner can open physical door lock");
...
const preview = createBuildingSystemPlacementPreviewV1({ plot, blueprint, owned: true });
...
let shop = defaultHarthmereLiveModeBackendStateV1("player-building-v5", 1000);
...
const parsed = parseHarthmereLiveModeBackendStateV1(JSON.stringify(transfer.state), "player-building-v5", 9000);
