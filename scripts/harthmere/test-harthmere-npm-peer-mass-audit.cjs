...
console.log('== Harthmere npm peer mass audit current ==');
console.log(`Root: ${root}\n`);
const p = pkg();
check('package.json exists', fs.existsSync(pkgPath));
check('React and React DOM are on 19 line', major(dep(p, 'react')) === 19 && major(dep(p, 'react-dom')) === 19);
check('unused ReactGrid peer blocker is removed', !dep(p, '@silevis/reactgrid'));
check('Emoji Mart uses current package line', major(dep(p, 'emoji-mart')) >= 5);
check('@emoji-mart/data is declared', !!dep(p, '@emoji-mart/data'));
check('@emoji-mart/react React-18 peer blocker is removed', !dep(p, '@emoji-mart/react'));
check('React Leaflet uses the React 19 package line', major(dep(p, 'react-leaflet')) >= 5);
...
check('@kubernetes/client-node stays on the verified Node 24-compatible pre-1 line', /^\^?0\.22\.3/.test(dep(p, '@kubernetes/client-node')));
