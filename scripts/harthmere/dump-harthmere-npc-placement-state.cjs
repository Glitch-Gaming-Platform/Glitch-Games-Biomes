...
const offsetX = Number.parseInt(process.env.BIOMES_HARTHMERE_EXTRA_TOWN_OFFSET_X || '1600', 10);
...
const dump = { version: 'dump-harthmere-npc-placement-state', generatedAt: new Date().toISOString(), mode: { BIOMES_ENABLE_HARTHMERE_EXTRA_TOWN: process.env.BIOMES_ENABLE_HARTHMERE_EXTRA_TOWN || '', BIOMES_FORCE_LOCAL_DEV_TOWN: process.env.BIOMES_FORCE_LOCAL_DEV_TOWN || '', shouldAppear: enabled || forced, expectedCoordinateSet: enabled && !forced ? 'shifted' : forced ? 'authored-legacy' : 'none-snapshot-only', offsetX, offsetZ }, counts: { npcs: npcs.length }, npcs };
const outDir = path.join(repo, 'harthmere-debug-dumps');
fs.mkdirSync(outDir, { recursive: true });
const out = path.join(outDir, `harthmere-npc-placement-state.${Date.now()}.json`);
