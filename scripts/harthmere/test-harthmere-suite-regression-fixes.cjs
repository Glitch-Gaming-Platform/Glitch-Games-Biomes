...
check(
  "town placement suite includes current regression guard",
  suite.includes("test-harthmere-suite-regression-fixes.cjs"),
);

run("builder option expression matrix passes after startGame fix", "test-harthmere-builder-option-expression-matrix.cjs");
run("player/NPC expression parity passes after face-cache-key fix", "test-harthmere-player-npc-expression-parity.cjs");
run("gathering remainder scenario still passes", "test-harthmere-gathering-remainder.cjs");
run("building remainder scenario still passes", "test-harthmere-building-remainder.cjs");
