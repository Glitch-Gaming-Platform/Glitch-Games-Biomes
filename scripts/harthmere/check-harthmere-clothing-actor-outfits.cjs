...
check("current marker exists", facesText.includes("HARTHMERE_CLOTHING_ACTOR_OUTFITS"));
check("current merge helper exists", facesText.includes("mergeHarthmereLicensedClothingDefaults"));
check("current wrapper is active", facesText.includes("return mergeHarthmereLicensedClothingDefaults(role as any, species as any, body as any, baseClothing);"));
check("current wrapper no longer active", !facesText.includes("return mergeHarthmereLicensedClothingDefaults(role as any, baseClothing);"));
check("getById imported", facesText.includes("getHarthmereLicensedClothingById"));
check("female peasant outfit defined", facesText.includes("HARTHMERE_QUATERNIUS_FEMALE_PEASANT_OUTFIT"));
check("male peasant outfit defined", facesText.includes("HARTHMERE_QUATERNIUS_MALE_PEASANT_OUTFIT"));
check("female ranger outfit defined", facesText.includes("HARTHMERE_QUATERNIUS_FEMALE_RANGER_OUTFIT"));
check("male ranger outfit defined", facesText.includes("HARTHMERE_QUATERNIUS_MALE_RANGER_OUTFIT"));
