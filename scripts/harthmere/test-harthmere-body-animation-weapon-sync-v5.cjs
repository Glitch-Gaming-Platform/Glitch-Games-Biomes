...
check("attack1 uses explicit body timeScale contract", /attack1:\s*\{\s*fileAnimationName:\s*"Attack",\s*timeScale:\s*HARTHMERE_BODY_ATTACK_TIME_SCALE_V5\.attack1/.test(player));
check("attack2 uses HeavyAttack with Attack2 fallback and body timeScale", /attack2:\s*\{\s*fileAnimationName:\s*"HeavyAttack",\s*backupFileAnimationNames:\s*\["Attack2"\],\s*timeScale:\s*HARTHMERE_BODY_ATTACK_TIME_SCALE_V5\.attack2/.test(player));
