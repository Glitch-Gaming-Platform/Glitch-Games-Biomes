#!/usr/bin/env node
/*
 * HARTHMERE_INSTALL_PLAYER_INGAME_E2E_V128
 *
 * v128 = v127 + headless WebGL handling.
 *
 * v127 already proves the install_id login flow end-to-end in headless
 ...
 * the game. v128 keeps that strict, while making the headless rendering
...
const artifactsDir = path.resolve(
  process.env.E2E_ARTIFACTS_DIR || "/tmp/harthmere-playboot-e2e-v128"
);
...
const launchUrl =
  ...
  `&glitch_auto_play=1&e2e=v128&t=${Date.now()}`;
...
(async () => {
  ...
  try {
    ...
    page.on("console", (msg) => {
      ...
      // v127 install_id checkpoint markers
