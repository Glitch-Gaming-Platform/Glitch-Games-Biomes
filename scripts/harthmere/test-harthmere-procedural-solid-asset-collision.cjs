#!/usr/bin/env node
/* HARTHMERE_PROCEDURAL_SOLID_ASSET_COLLISION */
const fs = require("fs");
...
console.log("== Harthmere procedural/spawned solid asset collision tests current ==");
...
if (/HARTHMERE_PROCEDURAL_SOLID_ASSET_COLLISION|proceduralSolidAssetCollision|spawnedSolidCollision/i.test(all)) {
  pass("procedural/spawned solid collision contract marker exists");
} else {
  fail("procedural/spawned solid collision contract marker exists", "Add HARTHMERE_PROCEDURAL_SOLID_ASSET_COLLISION and route spawned props through the same town collision registry.");
}
