...
console.log();
check("current variation manifest exists", exists("src/shared/harthmere/attack_variation_polish.ts"));
const manifest = exists("src/shared/harthmere/attack_variation_polish.ts") ? read("src/shared/harthmere/attack_variation_polish.ts") : "";
...
check("deterministic round-robin cycle exists", /advanceHarthmereAttackVariationIndex/.test(manifest) && /% variants.length/.test(manifest));
check("player animations import current manifest", /attack_variation_polish/.test(playerAnimations));
check("player animations expose current selection helper", /getHarthmereAttackVariationForAction/.test(playerAnimations));
...
check("renderer validates main-hand anchoring to left hand", /mainHandExpected:\s*"left"/.test(renderer) && /actualHandAnchor/.test(renderer) && /mainHandDistanceMeters/.test(renderer));
check("renderer requires left-hand proximity budget", /mainHandDistanceBudgetMeters:\s*0\.14/.test(renderer));
check("full suite includes current variation polish test", /test-harthmere-attack-variation-polish\.cjs/.test(suite));
console.log();
