...
ok(src.includes('SNAPSHOT_PLAYER_ANIMATION_COMPAT_VERSION'), "snapshot player animation compatibility marker is present");
ok(src.includes('"snapshot-player-animation-compat"'), "snapshot player animation compatibility version string is present");
ok(src.includes('function getResolvedPlayerAnimationClipName('), "resolved clip-name helper is present");
ok(src.includes('function hasResolvedHarthmereWeaponBodyClip('), "Harthmere clip-detection helper is present");
...
const weaponFn = bodyOfFunction("getHarthmereWeaponSyncedEmoteWeights");
...
ok(weaponFn.includes('const hasHarthmereWeaponClip = hasResolvedHarthmereWeaponBodyClip'), "weapon synced emote resolver checks actual resolved clip");
...
ok(weaponFn.includes('harthmereVariationEmoteType as any') || weaponFn.includes('harthmereVariationEmoteType as AnimationName'), "Harthmere branch still uses variation action when Harthmere clips exist");
...
if (failed) {
  console.error("snapshot player animation profile current check failed");
  process.exit(1);
}
console.log("snapshot player animation profile current check passed");
