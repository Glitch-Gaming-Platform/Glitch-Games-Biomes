...
const src = fs.readFileSync(path.join(root, "src/shared/harthmere/thaedryn_boss.ts"), "utf8");
const phases = readJsonExport(src, "HARTHMERE_THAEDRYN_PHASES_JSON");
const paths = readJsonExport(src, "HARTHMERE_THAEDRYN_PATHS_JSON");
...
for (const fn of ["createThaedrynBossState", "applyThaedrynBossEvent", "resolveThaedrynPath", "completeThaedrynBoss", "validateThaedrynBossContract"]) check(`exports ${fn}`, src.includes(`function ${fn}`));
...
console.log("RESULT: PASS Thaedryn boss current");
