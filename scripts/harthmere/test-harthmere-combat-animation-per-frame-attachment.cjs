...
check("production contract requires every-frame weapon samples", () => {
  ...
  assert(/Array\.from\([\s\S]{0,140}HARTHMERE_COMBAT_ANIMATION_FRAME_COUNT/.test(shared), "does not enumerate all 30 frames");
});
...
check("renderer exposes hand and tip/hilt runtime snapshots", () => {
  assert(renderer.includes("getHarthmereWeaponHandTrackingSnapshot"), "missing hand tracking snapshot helper");
  assert(renderer.includes("recordHarthmereWeaponTipHiltDirection"), "missing tip/hilt recorder");
  assert(renderer.includes("productionVisualSnapshot"), "missing browser visual snapshot");
});
finish("combat animation per-frame attachment current");
