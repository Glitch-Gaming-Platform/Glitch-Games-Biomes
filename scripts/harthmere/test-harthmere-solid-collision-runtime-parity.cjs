...
const report = makeReporter("Harthmere solid collision runtime parity tests current", root);
const town = loadTown(root);

report.check(
  "production registry has uploaded-solid player collision version marker",
  town.registrySrc.includes("HARTHMERE_SOLID_UPLOADED_ASSET_COLLISION_VERSION"),
...
report.check(
  ...
  town.assetsSrc.includes("HARTHMERE_SOLID_UPLOADED_ASSET_PLAYER_COLLISION"),
