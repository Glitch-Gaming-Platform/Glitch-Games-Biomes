...
function readModule(root) {
  const file = path.join(root, "src/shared/harthmere/complete_combat_progression.ts");
...
function assertCatalogs(root) {
  ...
  const classes = keyCount(text, "HARTHMERE_CLASS_CATALOG");
  ...
  const skills = keyCount(text, "HARTHMERE_SKILL_CATALOG");
...
function assertAbilityEquipment(root) {
  const text = readModule(root);
  const abilities = keyCount(text, "HARTHMERE_ABILITY_CATALOG");
  ...
  const equipment = keyCount(text, "HARTHMERE_EQUIPMENT_CATALOG");
  ...
  includesAll(text, "equipment validators", ["validateHarthmereEquipmentChange","broken_weapon_cannot_attack_normally","two_handed_weapon_disables_offhand","equipment_changes_blocked_in_combat","player_changes_gear_mid_cast","repairCostCopper","applyHarthmereDurabilityLoss"]);
}

function assertLevelSkillLoot(root) {
  const text = readModule(root);
  includesAll(text, "level/xp functions", ["xpRequiredForHarthmereLevel","levelDifferenceXpModifier","questXpReward","enemyKillXpReward","resolveHarthmereLevelGain"]);
  includesAll(text, "skill progression functions", ["validateHarthmereSkillProgressAction","applyHarthmereSkillProgress","grey_content_no_skill_progress","trivial_action_zero_progress","training_dummy_after_daily_cap"]);
  const loot = keyCount(text, "HARTHMERE_LOOT_TABLES");
  ...
  includesAll(text, "loot/drop rules", ["personalLoot","uniquePerKill","overflowPolicy","selectHarthmereLootDrops","resolveHarthmereKillRewards","boss_loot_not_awarded_on_wipe","inventory_full_reward_routed_to_overflow_recovery","low_level_enemy_no_currency_or_power_loot"]);
...
function assertServerPvpDeath(root) {
  const text = readModule(root);
  includesAll(text, "server authority", ["validateHarthmereServerAuthorityEnvelope","client_authoritative_claim_rejected","stale_entity_version_rejected","duplicate_request_id_rejected","server_authoritative_validation","combat_audit_logs"]);
  includesAll(text, "ability server validation", ["validateHarthmereAbilityUse","ability_hits_through_wall","ability_double_casts_from_lag","player_uses_ability_without_required_weapon","player_uses_illegal_ability_in_town"]);
  includesAll(text, "PvP/group/death rules", ["resolveHarthmerePvpRewardEligibility","resolveHarthmereGroupRewardEligibility","low_level_grief_reward_suppressed","repeated_kill_farming_suppressed","no_meaningful_pvp_contribution","win_trading_suspected","spawn_camping","raid_kick_after_contribution_keeps_eligibility","resolveHarthmereDeathPenalty","normal_pvp_death_no_inventory_destroy","drop_only_unbound_trade_goods_and_gathered_resources","bound_quest_spellbook_mount_pet_cosmetic_keyring_protected","unfair_death_no_harsh_penalty"]);
  const npcs = keyCount(text, "HARTHMERE_NPC_COMBAT_PROFILES");
...
function assertRuntimeIntegration(root) {
  const text = readModule(root);
  const combat = path.join(root, "src/shared/harthmere/combat_system.ts");
  assert(fs.existsSync(combat), "combat_system shared engine still exists");
  assert(text.includes("buildHarthmereCombatStatsFromProgression"), "progression can build combat stats for combat engine");
  assert(text.includes("resolveHarthmereNpcCombatProfile"), "NPC profile resolver exists for runtime AI");
  assert(text.includes("normalizeHarthmerePvpStats"), "PvP bracket/stat normalization exists");
  assert(text.includes("HARTHMERE_COMPLETE_TDD_TESTS"), "test manifest exists");
  const tests = ["test-harthmere-complete-progression-catalogs.cjs","test-harthmere-complete-abilities-equipment.cjs","test-harthmere-complete-level-skill-loot.cjs","test-harthmere-complete-server-pvp-death.cjs","test-harthmere-complete-runtime-integration.cjs"];
  for (const test of tests) assert(fs.existsSync(path.join(root, "scripts/harthmere", test)), `${test} exists`);
  const suite = path.join(root, "scripts/harthmere/test-harthmere-town-placement-suite.cjs");
