#!/usr/bin/env node
/*
 * HARTHMERE_INSTALL_PLAYER_INGAME_E2E_V124
...
const artifactsDir = path.resolve(process.env.E2E_ARTIFACTS_DIR || "/tmp/harthmere-playboot-e2e-v124");
...
const launchUrl = `${baseUrl}/at?install_id=${encodeURIComponent(installId)}&glitch_install_id=${encodeURIComponent(installId)}&game_install_id=${encodeURIComponent(installId)}&glitch_auto_play=1&e2e=v124&t=${Date.now()}`;
