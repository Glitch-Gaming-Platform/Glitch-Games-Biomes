#!/usr/bin/env node
// HARTHMERE_PLAYER_AVATAR_VISIBLE_BASE_PASS
...
console.log("== Harthmere player avatar visible base-pass current ==");
...
check(
  ...
  "current marker documents the invisible-avatar fix"
);
check(
...
check(
  ...
  "mixed scene fallback was hardened for production in current"
);
check(
