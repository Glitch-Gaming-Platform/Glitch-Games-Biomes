...
ok("retaliation diagnostics version marker exists", combat.includes("HARTHMERE_RETALIATION_DIAGNOSTICS_V183"));
...
ok("sync diagnosis performs contact-proven attack", combat.includes("function diagnoseHarthmereRetaliation(") && combat.includes("contactSource: \"retaliation_diagnostics_v183\""));
ok("async diagnosis follows windup/recovery windows", combat.includes("async function diagnoseHarthmereRetaliationAsync") && combat.includes("retaliation_diagnostics_v183_after_windup") && combat.includes("retaliation_diagnostics_v183_after_recovery"));
...
if (deploy) {
  ok("deploy guardrails include retaliation diagnostics test", deploy.includes("test-harthmere-retaliation-diagnostics-v183.cjs"));
}
