#!/usr/bin/env node
/* SNAPSHOT_BIKKIE_BISCUIT_DECODE_VERSION_V77
 * SNAPSHOT_BIKKIE_BISCUIT_DECODE_V77
 ...
 * readable state_machines.ts NUX chain v76 already extracted.
 */
const fs = require('fs');
const path = require('path');

const root = process.argv[2] ? path.resolve(process.argv[2]) : process.cwd();
const out = process.argv[3] ? path.resolve(process.argv[3]) : path.join(root, 'tmp/snapshot-bikkie-challenge-decode-v77.json');
...
const result = {
  version: 'snapshot-bikkie-biscuit-decode-v77',
  root,
