...
const marker = "HARTHMERE_WALKABLE_BRIDGE_COLLISION";
console.log("== Harthmere town registry bridge label TDZ test current ==");
...
check("current guard marker exists", /HARTHMERE_BRIDGE_LABEL_TDZ_FIX/.test(collisionBlock));

// Do not invoke the older current test from here. Some local suites append nested
...
const report = {
  version: "harthmere-town-registry-bridge-label-tdz",
...
const outRel = "public/assets/harthmere/manifest/harthmere-town-registry-bridge-label-tdz.json";
...
if (process.exitCode) {
  console.error("\nRESULT: FAIL Harthmere town registry bridge label TDZ current");
  process.exit(process.exitCode);
}
console.log("\nRESULT: PASS Harthmere town registry bridge label TDZ current");
