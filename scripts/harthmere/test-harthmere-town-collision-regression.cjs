...
console.log("== Harthmere town collision regression tests current ==");
...
ok(
  ...
  /HARTHMERE_SOLID_UPLOADED_ASSET_PLAYER_BLOCKER_FAMILIES/.test(registry) && /banner flag sign window shrine monument pole standard mast temple blocksPlayer: true/.test(registry),
...
ok(
  ...
  /__harthmerePlayerCollisionObstacles/.test(assets) && /__harthmereTownCollisionObstacles/.test(assets) && /harthmereDebugCollisionObstacles/.test(assets),
