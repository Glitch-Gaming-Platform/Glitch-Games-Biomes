#!/usr/bin/env node
"use strict";
/* HARTHMERE_BUILDING_PERIMETER_CLOSURE_TEST
 *
...
const v44Match = src.match(/function\s+createHarthmereContinuousBlockWalls\s*\(([\s\S]*?)\n\}/);
if (!v44Match) {
  fail("current wall function exists", "createHarthmereContinuousBlockWalls not found");
  console.log("\nRESULT: FAIL");
  process.exit(1);
}
pass("current wall function exists");
...
if (missingFaces.length === 0) {
  pass("current emits all four wall faces (north/south/east/west)");
} else {
  fail("current emits all four wall faces", `missing: ${missingFaces.join(", ")}`);
}

// 2. Corner placement must not be skipped — must NOT be inside any opening
//    pattern. We check that the current function does not short-circuit corners
//    before checking openings.
if (/corner|cornerColumn|halfX|halfZ/i.test(v44Body)) {
  pass("current references corner / halfX / halfZ positions");
} else {
  fail("current references corner / halfX / halfZ positions",
...
if (/columns?\s*=\s*[^;]*\+\s*1/.test(v44Body) ||
  ...
  pass("current loop includes both endpoints of each face (corners present)");
} else {
  fail("current loop includes both endpoints of each face (corners present)",
       "expected `+ 1` column count or `<= halfX/halfZ` bound");
}

// 4. The current function must be called once per story (multi-story closure).
...
if (/createHarthmereContinuousBlockWalls\s*\(/.test(shellBody)) {
  pass("createBuildingShell calls current walls (per-story closure when called per floor)");
} else {
  fail("createBuildingShell calls current walls", "expected current call inside createBuildingShell");
}

// 5. Block stair runs must still exist (do not break multi-story access fix)
if (/createHarthmereServiceBlockStairRun|createHarthmereBlockStairRun/.test(src)) {
  pass("multi-story stair runs preserved (current housing and current service)");
} else {
  fail("multi-story stair runs preserved (current housing and current service)",
       "expected createHarthmereServiceBlockStairRun and createHarthmereBlockStairRun to remain");
}

console.log(ok ? "\nRESULT: PASS perimeter closure current" : "\nRESULT: FAIL perimeter closure current");
process.exit(ok ? 0 : 1);
