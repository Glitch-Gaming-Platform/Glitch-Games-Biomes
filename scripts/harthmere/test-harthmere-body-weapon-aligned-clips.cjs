...
ok('shared current body/weapon animation sync manifest exists', fs.existsSync(contractPath));
...
ok('weapon body attacks remain upper-body-only', /getHarthmereWeaponSyncedEmoteWeights[\s\S]*layers:\s*\{[\s\S]*arms:\s*"apply"[\s\S]*notArms:\s*"noApply"/s.test(pa));
