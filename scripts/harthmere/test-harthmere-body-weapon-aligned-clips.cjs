...
const playerAnimationsPath = path.join(
  root,
...
function decodeBuffer(gltf) {
  ...
  if (!uri.startsWith("data:"))
    throw new Error("expected embedded data URI buffer");
...
function accessorFloats(gltf, buffer, idx) {
  ...
  const out = [];
  for (let i = 0; i < acc.count * comps; i++)
    out.push(buffer.readFloatLE(byteOffset + i * 4));
...
function animationByName(gltf, name) {
  return (gltf.animations || []).find((a) => a.name === name);
}
function targetName(gltf, ch) {
  return gltf.nodes[ch.target.node]?.name;
}

console.log("== Harthmere body/weapon aligned clip tests current ==");
console.log("Root:", root, "\n");

const variantFiles = fs.existsSync(variantDir)
  ? fs
      .readdirSync(variantDir)
      .filter(
        (n) =>
          /^harthmere_player_.*\.gltf$/.test(n) &&
          !n.includes("manifest") &&
          !n.includes("action_animation")
      )
      .sort()
  : [];

ok(
  "all 36 body color/size variants are present",
  variantFiles.length === 36,
  `Found ${variantFiles.length}`
);
for (const body of BODY_TYPES) {
  for (const color of COLORS) {
    ok(
      `body variant exists for ${body}/${color}`,
      variantFiles.includes(`harthmere_player_${body}_${color}.gltf`)
    );
  }
}
...
for (const file of variantFiles) {
  ...
  for (const name of REQUIRED) {
    ...
    if (!extras[VERSION] || extras.frameCount !== 30 || extras.fps !== 30)
      allHaveMetadata = false;
    const nodeTargets = new Set(
      anim.channels.map((ch) => `${targetName(gltf, ch)}:${ch.target.path}`)
    );
    for (const must of [
      "Hips:translation",
      "Chest:rotation",
    ...
    if (/Basic|Heavy|Release|Tool|Bash/.test(name)) {
      ...
      if (!(
        Number.isInteger(impactFrame) &&
        impactFrame > 0 &&
        impactFrame < 29
      ))
        impactMetadataGood = false;
    }
  ...
  if (
    names.has("Attack") &&
    names.has("HeavyAttack") &&
    !names.has("HarthmereBodyWeaponBasic_Aligned_30")
  )
    noOnlyLegacyAttack = false;
}
...
ok(
  "aligned clips carry production metadata/version/fps/frameCount",
  allHaveMetadata
);
ok(
  "aligned clips target right/left arms, hands, stable chest, and locked hips",
  allHaveCoreTargets
);
ok(
  "aligned clips lock root/hips translation to prevent body drift",
  rootLocked
);
ok(
  "aligned clips keep chest rotation subtle to avoid full-body tumble",
  chestStable
);
ok(
  "impact-frame metadata exists for strike/release/tool/bash clips",
  impactMetadataGood
);
ok(
  "legacy Attack/HeavyAttack are no longer the only body attack clips",
  noOnlyLegacyAttack
);

const manifestText = readText(contractPath);
ok(
  "shared current body/weapon animation sync manifest exists",
  fs.existsSync(contractPath)
);
ok(
  "shared manifest declares all aligned clips",
  REQUIRED.every((name) => manifestText.includes(name))
);
ok(
  "shared manifest accounts for all body size/color variants",
  BODY_TYPES.every((b) => manifestText.includes(b)) &&
    COLORS.every((c) => manifestText.includes(c))
);
ok(
...
ok(
  "player animation system prefers aligned basic attack clip",
  /attack1:\s*\{[^}]*HarthmereBodyWeaponBasic_Aligned_30/s.test(pa)
);
ok(
  "player animation system prefers aligned heavy attack clip",
  /attack2:\s*\{[^}]*HarthmereBodyWeaponHeavy_Aligned_30/s.test(pa)
);
ok(
  "player animation system keeps legacy attack fallbacks for non-Harthmere bodies",
  /backupFileAnimationNames:\s*\[[^\]]*Attack/s.test(pa) &&
    /backupFileAnimationNames:\s*\[[^\]]*HeavyAttack/s.test(pa)
);
ok(
...
ok(
  ...
  /magicCast:\s*\{[^}]*HarthmereBodyMagicCast_Aligned_30/s.test(pa) &&
    /magicChannel:\s*\{[^}]*HarthmereBodyMagicChannel_Aligned_30/s.test(pa)
);
ok(
  "shield body actions prefer aligned block/bash clips",
  /shieldBlock:\s*\{[^}]*HarthmereBodyWeaponBlock_Aligned_30/s.test(pa) &&
    /shieldBash:\s*\{[^}]*HarthmereBodyShieldBash_Aligned_30/s.test(pa)
);
ok(
  "tool/build/repair actions prefer aligned tool clips",
  /mineImpact:\s*\{[^}]*HarthmereBodyToolUse_Aligned_30/s.test(pa) &&
    /repairImpact:\s*\{[^}]*HarthmereBodyToolUse_Aligned_30/s.test(pa) &&
    /buildPlace:\s*\{[^}]*HarthmereBodyToolUse_Aligned_30/s.test(pa)
);
ok(
  "item use/eat/drink have aligned item animation coverage",
  /eat:\s*\{[^}]*HarthmereBodyItemUse_Aligned_30/s.test(pa) &&
    /drink:\s*\{[^}]*HarthmereBodyItemUse_Aligned_30/s.test(pa)
);
ok(
  "weapon body attacks preserve authored footwork while stationary",
  /getHarthmereWeaponSyncedEmoteWeights[\s\S]*layers:\s*\{[\s\S]*arms:\s*"apply"[\s\S]*notArms:\s*"ifIdle"/s.test(
    pa
  )
);
ok(
  "body animation sync has dt cap and deadzone to reduce jitter",
  /HARTHMERE_BODY_MAX_BLEND_DT/.test(pa) &&
    /HARTHMERE_BODY_LOCOMOTION_DEADZONE_SPEED/.test(pa)
);

const hud = readText(hudPath);
ok(
  "HUD debug desired body animation names reference aligned attack clips",
  !hud ||
    /HarthmereBodyWeaponBasic_Aligned_30/.test(hud) ||
    /desiredFileAnimationName/.test(hud)
);
