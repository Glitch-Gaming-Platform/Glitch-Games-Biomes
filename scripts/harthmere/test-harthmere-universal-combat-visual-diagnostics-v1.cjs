...
} = require("../../src/shared/harthmere/combat_visual_diagnostics_v1");
const {
...
} = require("../../src/shared/harthmere/mmo_crafting_catalogue_v1");
const {
  getHarthmereItemDefinitionV1,
} = require("../../src/shared/harthmere/mmo_inventory_authority_v1");

const root = process.argv[2] ? path.resolve(process.argv[2]) : process.cwd();
const report = makeReporter("Harthmere universal combat visual diagnostics v1", root);
const rawUrl =
...
const artifactsDir = path.resolve(
  process.env.HARTHMERE_UNIVERSAL_COMBAT_ARTIFACTS_DIR ||
    path.join(root, ".codex-artifacts", "harthmere-universal-combat-visual-diagnostics-v1")
);
...
function runDiagnosis(label, input) {
  const result = evaluateHarthmereCombatVisualDiagnosisV1(input);
...
async function waitForRuntimeReady(page) {
  await page.waitForFunction(
    () => {
      ...
      const actorCount = Object.keys(window.__harthmereVoxelNpcMotionActorPositionsV193 || {}).length;
...
async function runtimeSnapshot(page) {
  return await page.evaluate(() => {
    ...
    const actors = window.__harthmereVoxelNpcMotionActorPositionsV193 || {};
    const rendererActors =
      typeof window.__harthmereRendererDebug?.actors === "function"
        ? window.__harthmereRendererDebug.actors()
        : [];
    return {
      ...
      animationAudit: window.__harthmereVoxelNpcAnimationAuditV195 || {},
      animationAuditLog: (window.__harthmereVoxelNpcAnimationAuditLogV195 || []).slice(0, 80),
      retaliationLog: (window.__harthmereVoxelNpcRetaliationAnimationLogV191 || []).slice(0, 40),
      retaliationReadLog: (window.__harthmereVoxelNpcRetaliationAnimationReadLogV191 || []).slice(0, 40),
      attackAnimationLog: (window.__harthmereUniversalCombatAttackAnimationLogV1 || []).slice(0, 80),
      combatEffectLog: (window.__harthmereUniversalCombatEffectLogV1 || []).slice(0, 80),
      animationRuntimeLog:
        Array.isArray(window.__harthmereAnimationRuntimeV6?.log)
          ? window.__harthmereAnimationRuntimeV6.log.slice(0, 80)
          : typeof window.__harthmereAnimationRuntimeV6?.log === "function"
          ? window.__harthmereAnimationRuntimeV6.log().slice?.(0, 80) ?? []
          : [],
    };
  });
...
async function installRuntimeListeners(page) {
  await page.evaluateOnNewDocument(() => {
    ...
    window.__harthmereUniversalCombatAttackAnimationLogV1 = [];
    window.__harthmereUniversalCombatEffectLogV1 = [];
    window.__harthmereUniversalCombatWeaponVisualLogV1 = [];
    ...
    window.addEventListener("biomes:harthmere-attack-animation", (event) =>
      push("__harthmereUniversalCombatAttackAnimationLogV1", event.detail)
    );
    window.addEventListener("biomes:harthmere-combat-effect", (event) =>
      push("__harthmereUniversalCombatEffectLogV1", event.detail)
    );
    window.addEventListener("biomes:harthmere-player-sword-visual", (event) =>
      push("__harthmereUniversalCombatWeaponVisualLogV1", event.detail)
    );
  });
}
...
async function playerUnarmedLiveNpcScenario(page) {
  const result = await page.evaluate(async () => {
    ...
    for (let index = 0; index < (window.localStorage?.length ?? 0); index += 1) {
      ...
      if (key?.includes("harthmere.inventoryState.v1")) {
    ...
    const chooseTargetActor = () => {
      ...
      return Object.entries(window.__harthmereVoxelNpcMotionActorPositionsV193 || {})
    ...
    if (!Number.isFinite(offset) || !actor?.pos) {
      const fallback = Object.entries(window.__harthmereVoxelNpcMotionActorPositionsV193 || {})
    ...
    if (actor?.pos) {
      ...
      if (Number.isFinite(x) && Number.isFinite(z)) {
        ...
        window.__harthmereLivePlayerDebug?.teleportTo?.({
          ...
          source: "harthmere-universal-combat-visual-diagnostics-v1",
        });
    ...
    for (let poll = 0; poll < 18; poll += 1) {
      ...
      const auditNow = window.__harthmereVoxelNpcAnimationAuditV195?.[String(offset)] ?? null;
      const auditLogNow = window.__harthmereVoxelNpcAnimationAuditLogV195 || [];
      const readLogNow = window.__harthmereVoxelNpcRetaliationAnimationReadLogV191 || [];
      observedBodyAttack =
    ...
    const audit = window.__harthmereVoxelNpcAnimationAuditV195?.[String(offset)] ?? null;
    const auditLog = (window.__harthmereVoxelNpcAnimationAuditLogV195 || []).filter(
      (entry) => String(entry?.id) === String(offset)
    );
    const retaliationLog = (window.__harthmereVoxelNpcRetaliationAnimationLogV191 || []).filter(
      (entry) => String(entry?.entityId ?? entry?.attackerOffset) === String(offset)
    );
    const retaliationReadLog = (window.__harthmereVoxelNpcRetaliationAnimationReadLogV191 || []).filter(
...
async function playerKeyedEmptyHandAttackCouplingScenario(page) {
  const result = await page.evaluate(async () => {
    ...
    for (const key of storageKeys) {
      if (
        key.includes("harthmere.inventoryState.v1") ||
        key.includes("harthmere.multiplayerCombatState.v1")
      ) {
        window.localStorage.removeItem(key);
      }
    }
    ...
    const targetAfter = Number(after?.npcs?.[String(offset)]?.hp);
    const playerAfter = Number(after?.player?.hp);
    const attackAnimations = (window.__harthmereUniversalCombatAttackAnimationLogV1 || [])
      .filter((entry) => Number(entry?.at ?? 0) >= startedAt - 10);
    const effects = (window.__harthmereUniversalCombatEffectLogV1 || [])
      .filter((entry) => Number(entry?.at ?? 0) >= startedAt - 10);
    const weaponVisuals = (window.__harthmereUniversalCombatWeaponVisualLogV1 || [])
      .filter((entry) => Number(entry?.at ?? 0) >= startedAt - 10);
...
async function forceMultipleNativeBodyAnimations(page, label, filterRegex) {
  const result = await page.evaluate(async ({ sourceLabel, pattern }) => {
    ...
    const buildCandidates = () => {
      ...
      const auditMap = window.__harthmereVoxelNpcAnimationAuditV195 || {};
      return Object.entries(window.__harthmereVoxelNpcMotionActorPositionsV193 || {})
        .map(([id, actor]) => {
    ...
    if (actors.length === 0 && sourceLabel === "visible-live") {
      actors.push(
        ...Object.entries(window.__harthmereVoxelNpcMotionActorPositionsV193 || {})
          .map(([id, actor]) => ({ id, ...(actor || {}) }))
          .slice(0, 3)
      );
    }
    window.__harthmereVoxelNpcRetaliationAnimationV191 = {
      ...(window.__harthmereVoxelNpcRetaliationAnimationV191 || {}),
    };
    const startedAt = Date.now();
    const collectHits = () => {
      const auditLog = window.__harthmereVoxelNpcAnimationAuditLogV195 || [];
      const latestAuditMap = window.__harthmereVoxelNpcAnimationAuditV195 || {};
      const readLog = window.__harthmereVoxelNpcRetaliationAnimationReadLogV191 || [];
      return actors
        .map((actor) => {
    ...
    for (let tick = 0; tick < 18; tick += 1) {
      for (const actor of actors) {
        window.__harthmereVoxelNpcRetaliationAnimationV191[String(actor.id)] = {
          at: Date.now(),
          animation: "attack1",
...
async function teleportAndScan(page, anchor, pattern) {
  return await page.evaluate(async ({ target, source }) => {
    ...
    const nativeActors = Object.entries(window.__harthmereVoxelNpcMotionActorPositionsV193 || {})
      .map(([id, actor]) => ({ id, ...(actor || {}) }))
      .filter((actor) => regex.test(String(actor.label || "")));
...
async function forceRuntimeNonNpcBodyAnimations(page, familyLabel, pattern, minimumCount = 1) {
  const result = await page.evaluate(async ({ label, source, minimum }) => {
    ...
    const beforeLog = Array.isArray(window.__harthmereNonNpcCombatAnimationAuditLogV1)
      ? window.__harthmereNonNpcCombatAnimationAuditLogV1.length
      : 0;
    const matches = debug?.forcePulseByPattern?.(source, "attack") || [];
    await new Promise((resolve) => setTimeout(resolve, 220));
    const audit = Object.values(window.__harthmereNonNpcCombatAnimationAuditV1 || {})
      .filter((entry) => {
        const text = `${entry.label || ""} ${entry.asset || ""} ${entry.district || ""}`;
        return new RegExp(source, "i").test(text);
      });
    const afterLog = Array.isArray(window.__harthmereNonNpcCombatAnimationAuditLogV1)
      ? window.__harthmereNonNpcCombatAnimationAuditLogV1.length
      : 0;
    return {
      label,
...
async function projectileAndToolDiagnostics(page) {
  ...
  const projectileResult = await page.evaluate(async () => {
    const readRuntimeLog = () =>
      Array.isArray(window.__harthmereAnimationRuntimeV6?.log)
        ? window.__harthmereAnimationRuntimeV6.log
        : typeof window.__harthmereAnimationRuntimeV6?.log === "function"
        ? window.__harthmereAnimationRuntimeV6.log()
        : [];
    const beforeLog = readRuntimeLog().length;
    const requestResult = window.__harthmereAnimationRuntimeV6?.request?.({
      family: "ranged",
