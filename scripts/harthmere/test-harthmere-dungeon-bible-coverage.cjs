#!/usr/bin/env node
"use strict";
/* HARTHMERE_DUNGEON_BIBLE_COVERAGE_TEST
 *
...
const spacesPath = path.join(root, "src/shared/harthmere/main_quest_spaces.ts");
...
for (const r of required) {
  if (fuzzyContains(r.name)) {
    ...
    fail(`bible dungeon room placed: ${r.name} (${r.quest})`, `not in renderer or main_quest_spaces`);
    missing.push(r.name);
  }
}
...
console.log(ok ? "\nRESULT: PASS dungeon bible coverage" : "\nRESULT: FAIL dungeon bible coverage");
process.exit(ok ? 0 : 1);
