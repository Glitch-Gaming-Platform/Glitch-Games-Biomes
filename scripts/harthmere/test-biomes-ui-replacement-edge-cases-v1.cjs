...
function contains(text, needle, msg) { assert(text.includes(needle), msg || `contains ${needle}`); }
function notContains(text, needle, msg) { assert(!text.includes(needle), msg || `does not contain ${needle}`); }
...
const shortcuts = read("src/client/components/biomes_ui/shortcuts/BiomesShortcuts.ts");
...
const cueBar = read("src/client/components/biomes_ui/tutorial/BiomesUITutorialCueBar.tsx");

notContains(biomesUI, 'import "./theme/biomes_ui.css"', "BiomesUI does not import global CSS directly");
notContains(biomesUI, "import './theme/biomes_ui.css'", "BiomesUI does not import global CSS directly, single-quote form");
contains(biomesUI, "installBiomesUITheme", "BiomesUI installs runtime theme instead of global CSS import");
contains(theme, 'document.createElement("style")', "theme injects a style tag");
contains(theme, "BIOMES_UI_THEME_ID", "theme exposes stable id constant for idempotency tests");
contains(theme, "document.getElementById(BIOMES_UI_THEME_ID)", "theme injection is idempotent by id");

contains(flags, "biomes_ui_replace_legacy", "replacement flag reads/writes localStorage key");
contains(flags, "biomes-ui-flags-changed", "replacement flag dispatches change event");
contains(flags, "NEXT_PUBLIC_BIOMES_UI_REPLACE_LEGACY", "replacement flag supports NEXT_PUBLIC env");
contains(flags, "useBiomesUIReplaceLegacyFlag", "replacement flag has React hook");

assert(/forceEnabled\??\s*:\s*boolean/.test(mount) || /forceEnabled\s*=\s*false/.test(mount), "BiomesUIMount exposes forceEnabled prop");
contains(mount, "useBiomesUILiveAdapters", "BiomesUIMount uses live adapters instead of placeholder data");
contains(mount, "hotbar={live.hotbar}", "BiomesUIMount passes live hotbar into BiomesUI");
contains(mount, "adapters={live.adapters}", "BiomesUIMount passes adapters into BiomesUI");
contains(mount, "step={live.tutorialStep}", "TutorialDirector receives live tutorial step");
notContains(mount, "PLACEHOLDER_HOTBAR", "BiomesUIMount no longer declares placeholder hotbar data");
notContains(mount, "Bare Hands", "BiomesUIMount no longer uses sample item labels");
notContains(mount, "Singularity Block", "BiomesUIMount no longer uses sample block labels");

contains(adapters, "useClientContext", "live adapters read ClientContext");
contains(adapters, 'reactResources.use("/ecs/c/inventory", userId)', "live adapters read ECS inventory");
...
contains(quest, "useBiomesUIReplaceLegacyFlag", "QuestAndMinimapHUD reads replacement flag");
contains(quest, "<BiomesUIMount forceEnabled={replaceLegacy}", "QuestAndMinimapHUD force-mounts new UI in replacement mode");
contains(quest, "<HarthmereUnifiedHUD hideLegacyVisuals={replaceLegacy}", "QuestAndMinimapHUD hides only legacy visuals, not controllers");
assert(count(quest, /<MiniMapHUD\s*\/>/g) >= 2, "QuestAndMinimapHUD keeps MiniMapHUD in normal and replacement branches");
assert(/replaceLegacy[\s\S]{0,600}<MiniMapHUD\s*\/>/.test(quest), "replacement branch still renders the live MiniMapHUD");
contains(quest, 'RulesetToggleable name="minimap"', "MiniMapHUD remains behind minimap ruleset toggle");
...
contains(unified, "if (hideLegacyVisuals)", "HarthmereUnifiedHUD has replacement-mode visual hiding branch");
contains(unified, "if (hideLegacyVisuals) {\n        // Replacement mode keeps all Harthmere runtime hooks/controllers alive", "legacy HUD hotkey handler yields to BiomesUI in replacement mode");
...
for (const name of controllerNames) {
  contains(unified, `<${name} />`, `runtime controller remains mounted: ${name}`);
}

const hiddenBranchMatch = unified.match(/if \(hideLegacyVisuals\) \{[\s\S]*?return \([\s\S]*?<>[\s\S]*?\{runtimeControllers\}[\s\S]*?<HarthmereVendorTradePanel \/>[\s\S]*?<\/>([\s\S]*?)\);[\s\S]*?\}/);
assert(Boolean(hiddenBranchMatch), "hidden legacy branch still returns runtimeControllers and critical vendor panel");
notContains(hiddenBranchMatch ? hiddenBranchMatch[0] : "", "<CompactStatusCluster", "hidden legacy branch does not render old status cluster");
notContains(hiddenBranchMatch ? hiddenBranchMatch[0] : "", "<SnapshotMissionMapHUD", "hidden legacy branch does not render old mission map HUD");
notContains(hiddenBranchMatch ? hiddenBranchMatch[0] : "", "<SnapshotGroveMapHUD", "hidden legacy branch does not render old Grove map HUD");
notContains(hiddenBranchMatch ? hiddenBranchMatch[0] : "", "<SnapshotCombatMapHUD", "hidden legacy branch does not render old combat map HUD");

assert(!/this\.findCombatLifeByEcsNpcSnapshotV190\s*\(/.test(assets), "harthmere_assets no longer directly calls missing findCombatLifeByEcsNpcSnapshotV190");

contains(shortcuts, "if (e.repeat) return", "tab shortcuts ignore repeated keydown");
...
contains(cueBar, "UI_IDS.CAMERA_BUTTON", "tutorial cue bar exposes camera highlight target");
contains(cueBar, "UI_IDS.RECIPE_LIST", "tutorial cue bar exposes recipe list highlight target");
contains(cueBar, "UI_IDS.RECIPE_MUCK_BUSTER", "tutorial cue bar exposes muck buster recipe highlight target");
...
console.log(`Biomes UI replacement edge-case contract: ${passes.length} passed, ${failures.length} failed.`);
if (failures.length) {
  for (const f of failures) console.error("FAIL", f);
  process.exit(1);
}
