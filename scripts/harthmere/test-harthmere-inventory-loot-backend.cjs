...
const rel = "src/shared/harthmere/mmo_inventory_loot_authority.ts";
const srcPath = path.join(root, rel);
if (!fs.existsSync(srcPath)) {
  console.error(`Missing ${rel}`);
  process.exit(1);
}
const tmp = path.join(os.tmpdir(), `mmo_inventory_loot_authority.${process.pid}.cjs`);
...
function mut(req) {
  const r = reduceHarthmereInventoryLootMutation(state, { requestId: `req_${Math.random()}`, nowMs: now, actorId: "p1", ...req }, ctx);
...
console.log("== Harthmere inventory/loot backend current ==");
...
state.guilds.guild_1 = createHarthmereInventoryLootGuild("guild_1", ["p1", "p2"], { lootRule: "guild_project" });
...
const firstRoll = rollHarthmereInventoryLootTable(lootTables.mucker_common, ctx, 123, []);
...
const laterRoll = rollHarthmereInventoryLootTable(lootTables.mucker_common, ctx, 123, ["mucker_intro_key"]);
...
const snapshot = createHarthmereInventoryLootClientSnapshot(state, "p1");
...
console.log("");
console.log("All inventory/loot backend current tests passed.");
