...
function readLiveModule(root) {
  const file = path.join(root, "src/shared/harthmere/live_mode_readiness.ts");
...
function assertLiveModeContracts(root) {
  ...
  includesAll(text, "version/contracts", [
    "HARTHMERE_LIVE_MODE_READINESS_VERSION",
    "HarthmereLiveModeAuthorityEnvelope",
    "HarthmereLiveModePipelineDefinition",
...
function assertPersistenceIdempotency(root) {
  ...
  includesAll(text, "persistence/idempotency", [
    ...
    "validateHarthmereLiveModeIdempotencyReplay",
    "idempotencyKey",
    "requiredLocks",
    "readModels",
    "writeModels",
...
function assertUiEvents(root) {
  ...
  includesAll(text, "death recap UI", [
    "buildHarthmereDeathRecapUiPayload",
    "killerName",
    "damageSummary",
    "availableRespawns",
    "reviveAvailable",
...
function assertEndToEndScenarios(root) {
  ...
  includesAll(text, "end-to-end scenario", [
    "simulateHarthmereLiveModeEndToEndScenario",
    "req-live-attack-001",
    "req-live-death-001",
  ...
  includesAll(text, "forced/wave respawn", [
    "resolveHarthmereWaveRespawnSlot",
    "nextRespawnAtMs",
    "wave_respawn",
  ]);
...
function assertProductionHardening(root) {
  const text = readLiveModule(root);
  const combat = readOptional(root, "src/shared/harthmere/combat_system.ts");
  const progression = readOptional(root, "src/shared/harthmere/complete_combat_progression.ts");
  const contracts = readOptional(root, "src/client/components/challenges/LocalDevHarthmereServerAuthorityContracts.tsx");

  assert(combat.includes("resolveHarthmereCombatAction"), "combat_system is installed before live-mode readiness");
  assert(progression.includes("HARTHMERE_COMPLETE_COMBAT_PROGRESSION_VERSION"), "complete combat/progression module is installed before live-mode readiness");
