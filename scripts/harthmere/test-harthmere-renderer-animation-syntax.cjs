...
console.log("== Harthmere renderer animation syntax current test ==");

check(
  "current syntax fix marker exists",
  assets.includes("HARTHMERE_RENDERER_ANIMATION_SYNTAX_FIX_VERSION"),
);
...
check(
  ...
  /harthmereRoutePosition\(instance\.wander\.route, progress\)/.test(assets),
);
...
check(
  "full placement suite includes current syntax test",
  suite.includes("test-harthmere-renderer-animation-syntax.cjs"),
);
