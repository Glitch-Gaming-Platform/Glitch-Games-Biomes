...
failIf(/onPerformed:\s*\(\)\s*=>\s*advanceSnapshotGroveQuest/.test(grove), "Snapshot Grove/Fountain cannot complete active steps by NPC dialogue button");
...
ok(grove.includes("pinSnapshotGroveLandmark") && (grove.includes("autoremoveWhenNear: true") || grove.includes("autoremoveWhenNear: false")), "Snapshot Grove/Fountain markers show and stay visible until the quest step controls them");
...
ok(road.includes("mapManager.removeNavigationAid?.(SNAPSHOT_MISSION_NAV_AID_ID)"), "Snapshot Road Ahead clears map markers on completion");
ok(road.includes("distance <= (step.arrivalRadius ?? 8)"), "Snapshot Road Ahead marker arrival is validated by distance");

if (failures) {
  console.error(`current onboarding quest_step_validation gate check failed: ${failures} failure(s)`);
  process.exit(1);
}
