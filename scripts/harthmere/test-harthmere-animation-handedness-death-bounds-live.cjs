...
console.log("== Harthmere animation handedness/death/bounds live tests current ==");
console.log(`Root: ${root}`);
console.log(`URL: ${url || "<HARTHMERE_E2E_URL not set>"}\n`);

if (!url) {
  console.log("INFO live current probe skipped because HARTHMERE_E2E_URL is not set");
...
(async () => {
  ...
  if (!pw?.chromium) {
    console.log("INFO live current probe skipped because Playwright is not installed");
    console.log("RESULT: PASS");
    return;
  }
  ...
  try {
    ...
    const result = await page.evaluate(() => {
      const api = window.__harthmereAnimationHandednessDeathBounds;
      const debug = window.__harthmereRendererDebug;
      return {
        ok: !!api,
        version: api?.version,
        handedness: api?.handednessProbe?.() ?? debug?.handednessProbe?.(),
        deathAll: api?.deathAllActorsProbe?.() ?? debug?.deathAllActorsProbe?.(),
        deathBounds: api?.deathBoundsProbe?.() ?? debug?.deathBoundsProbe?.(),
        worldEffects: api?.worldEffectVisibilityProbe?.() ?? debug?.worldEffectVisibilityProbe?.(),
      };
    });
    ...
    assert("browser exposes current API", result.ok, "missing __harthmereAnimationHandednessDeathBounds");
    assert("current version matches", result.version === "harthmere-animation-handedness-death-bounds", result.version);
    ...
    await browser.close();
    console.log("FAIL live current probe threw");
