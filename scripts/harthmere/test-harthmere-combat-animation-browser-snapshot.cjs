...
check("renderer debug bridge publishes production visual snapshot", () => {
  ...
  assert(renderer.includes("harthmereWeaponTipHiltDirection"), "missing hilt/tip data in runtime");
  assert(renderer.includes("harthmereWeaponHandTracking"), "missing hand tracking data in runtime");
});
finish("combat animation browser-snapshot current");
