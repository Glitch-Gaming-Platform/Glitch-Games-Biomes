#!/usr/bin/env node
/*
 * HARTHMERE_INSTALL_ID_FLOW_V127 validator
 * Confirms the v127 source modifications, unit tests, E2E, and runner are in
 * place and consistent. Run with: node validate-harthmere-install-id-flow-v127.cjs /path/to/repo
 */
const fs = require("fs");
...
const unitTest = read("scripts/harthmere/test-harthmere-install-id-flow-unit-v127.cjs");
const e2eCjs = read("scripts/harthmere/test-harthmere-install-player-ingame-e2e-v127.cjs");
const e2eSh = read("scripts/harthmere/test-harthmere-install-player-ingame-e2e-v127.sh");
const runSh = read("scripts/harthmere/run-harthmere-install-player-ingame-local-v127.sh");

// client_config.ts: v127 sync URL resolver
ok(/HARTHMERE_RUNTIME_SYNC_BASE_URL_V127/.test(clientConfig),
   "client_config.ts has v127 runtime sync base url marker");
...
ok(/HARTHMERE_SYNC_URL_RESOLVED_V127/.test(clientConfig),
   "client_config.ts logs HARTHMERE_SYNC_URL_RESOLVED_V127 marker");
ok(/installIdInUrl/.test(clientConfig),
   "client_config.ts derives installIdInUrl from window.location.search");

// bootstrap: v127 checkpoint markers
ok(/HARTHMERE_INSTALL_ID_FOUND_V127/.test(bootstrap),
   "bootstrap emits HARTHMERE_INSTALL_ID_FOUND_V127");
ok(/HARTHMERE_INITIAL_AUTH_CHECK_V127/.test(bootstrap),
   "bootstrap emits HARTHMERE_INITIAL_AUTH_CHECK_V127");
ok(/HARTHMERE_AUTO_LOGIN_REQUEST_V127/.test(bootstrap),
   "bootstrap emits HARTHMERE_AUTO_LOGIN_REQUEST_V127");
ok(/HARTHMERE_AUTO_LOGIN_RESPONSE_V127/.test(bootstrap),
   "bootstrap emits HARTHMERE_AUTO_LOGIN_RESPONSE_V127");
ok(/HARTHMERE_POST_LOGIN_AUTH_CHECK_V127/.test(bootstrap),
   "bootstrap emits HARTHMERE_POST_LOGIN_AUTH_CHECK_V127");
ok(/HARTHMERE_PRE_RELOAD_V127/.test(bootstrap),
   "bootstrap emits HARTHMERE_PRE_RELOAD_V127");
ok(/HARTHMERE_ALREADY_AUTHED_V127/.test(bootstrap),
   "bootstrap emits HARTHMERE_ALREADY_AUTHED_V127");
ok(/HARTHMERE_AUTH_COOKIE_MISSING_V127/.test(bootstrap),
   "bootstrap emits HARTHMERE_AUTH_COOKIE_MISSING_V127");
ok(/GLITCH_INSTALL_BOOTSTRAP_AUTO_LOGIN_V115/.test(bootstrap),
   "bootstrap preserves legacy v115 marker for compatibility");
...
ok(/initialAuthed\)[\s\S]*?HARTHMERE_ALREADY_AUTHED_V127/.test(bootstrap),
   "bootstrap skips the autoLogin reload when already authed");

// v126 changes must still be present (we built on top of them)
ok(/HARTHMERE_CLIENT_CONTEXT_RENDER_UNBLOCK_V126/.test(loadProgress),
   "load_progress.ts still has v126 render-unblock marker");
ok(/HARTHMERE_GAME_MOUNT_CONTEXT_BEFORE_RENDER_READY_V126/.test(game),
   "Game.tsx still has v126 early canvas mount marker");

// Unit test
ok(/HARTHMERE_INSTALL_ID_FLOW_UNIT_V127/.test(unitTest),
   "unit test has v127 marker");
...
// E2E JS test
ok(/HARTHMERE_INSTALL_PLAYER_INGAME_E2E_V127/.test(e2eCjs),
   "v127 browser E2E marker exists");
ok(/HARTHMERE_INSTALL_ID_FOUND_V127/.test(e2eCjs) &&
   /HARTHMERE_AUTO_LOGIN_RESPONSE_V127/.test(e2eCjs) &&
   /HARTHMERE_POST_LOGIN_AUTH_CHECK_V127/.test(e2eCjs) &&
   /HARTHMERE_SYNC_URL_RESOLVED_V127/.test(e2eCjs),
   "E2E test tracks every v127 checkpoint marker");
...
ok(/test-harthmere-install-player-ingame-e2e-v127\.sh/.test(runSh),
   "runner ends with v127 E2E test");
...
console.log("Harthmere install-id full-flow v127 validation passed.");
