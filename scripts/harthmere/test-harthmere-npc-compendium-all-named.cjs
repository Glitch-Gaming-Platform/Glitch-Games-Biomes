...
const compendiumPath = path.join(root, "src/shared/harthmere/npc_compendium.ts");
const text = fs.readFileSync(compendiumPath, "utf8");
const match = text.match(/export const HARTHMERE_NAMED_NPCS = (\[[\s\S]*?\]) as const;/);
if (!match) throw new Error("Could not parse HARTHMERE_NAMED_NPCS JSON literal");
...
for (const npc of npcs) {
  assert(npc.implementationStatus === "implemented", `${npc.id} marked implemented`);
  assert(npc.name && npc.district && npc.home, `${npc.id} has name, district, and home`);
}
if (process.exitCode) process.exit(process.exitCode);
console.log("RESULT: PASS npc compendium all named current");
