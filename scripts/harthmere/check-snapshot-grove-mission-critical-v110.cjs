...
ok(/snapshotGroveStepNavAidIdV107\(stepIndex\)/.test(runtime), "each visible marker row can pin its own per-step nav aid");
ok(/snapshotGroveStepNavAidIdV107\(objectiveIndex\)/.test(runtime), "active pin button refreshes the current step's nav aid id");
failIf(/position:\s*\[[0-9][^\n]*SNAPSHOT_GROVE_MARKER_Y_V75/.test(shared), "no Grove static landmark uses the old non-live marker Y directly");
ok(/snapshotGroveMarkerPositionV75\(\[/.test(shared), "static landmarks are normalized through the live-marker position helper");
...
ok(questBlocks.length >= 25, "v110 test parses the full Grove quest list, not just one tutorial");
...
if (failures) {
  console.error(`v110 mission-critical check failed: ${failures} failure(s)`);
  process.exit(1);
}
console.log("v110 mission-critical check passed");
