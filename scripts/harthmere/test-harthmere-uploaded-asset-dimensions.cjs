...
console.log("== Harthmere uploaded asset dimensions tests current ==");
const tsPath = path.join(root, "src/shared/harthmere/uploaded_asset_dimensions.ts");
const jsonPath = path.join(root, "public/assets/harthmere/manifest/harthmere-uploaded-asset-dimensions.json");
...
check("version marker is current", ts.includes("harthmere-uploaded-asset-dimensions") && manifest.version === "harthmere-uploaded-asset-dimensions");
check("registry exposes lookup and collision-footprint helpers", /harthmereUploadedAssetDimensionForKey/.test(ts) && /harthmereUploadedAssetCollisionFootprint/.test(ts));
...
check("image metadata is captured when image files exist", typeof manifest.imageCount === "number" && manifest.imageCount >= 0 && /HARTHMERE_UPLOADED_IMAGE_METADATA/.test(ts));
