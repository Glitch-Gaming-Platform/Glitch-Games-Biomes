...
check("current outfit selector active", faces.includes("mergeHarthmereLicensedClothingDefaults"));
check("NPC current marker exists", npcs.includes("HARTHMERE_NPC_LICENSED_CLOTHING_MODELS"));
check("NPC queues licensed modelUrl clothing", npcs.includes("queueLocalDevNpcLicensedClothingModel(root, String(slot), item, body)"));
check("NPC uses loadGltf for clothing models", npcs.includes("loadGltf(modelUrl)"));
check("NPC keeps current proxy fallback", npcs.includes("harthmere-npc-clothing-torso"));
check("NPC records loaded licensed clothing metadata", npcs.includes("harthmereNpcLicensedClothingModelsLoaded"));
check("runtime current marker exists", assets.includes("HARTHMERE_RUNTIME_LICENSED_CLOTHING_MODELS"));
check("runtime imports/uses loadGltf", assets.includes("loadGltf(modelUrl)"));
check("runtime queues modelUrl clothing", assets.includes("queueHarthmereRuntimeLicensedClothingModels(root, appearance.clothing as any, body)"));
check("runtime keeps current proxy fallback", assets.includes("townsperson-clothing-torso"));
