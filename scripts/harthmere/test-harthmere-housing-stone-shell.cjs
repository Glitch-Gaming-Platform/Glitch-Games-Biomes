...
console.log("== Harthmere housing stone story-shells current ==");
...
check("current version marker exists", /HARTHMERE_RESIDENT_HOUSING_STONE_SHELL_VERSION/.test(manifest) && /HARTHMERE_RESIDENT_HOUSING_RENDERER_VERSION/.test(assets));
...
check("town placement suite includes current regression coverage", /test-harthmere-housing-stone-shell\.cjs/.test(suite));
if (failures > 0) {
