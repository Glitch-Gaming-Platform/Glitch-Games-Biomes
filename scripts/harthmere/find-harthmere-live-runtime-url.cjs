#!/usr/bin/env node
"use strict";
/* HARTHMERE_FIND_LIVE_RUNTIME_URL */
const { resolvePlaywright, collectRuntimeProbe, runtimeLooksLoaded, formatProbe } = require("./harthmere-live-runtime-probe.cjs");
...
(async () => {
  console.log("== Harthmere live runtime URL finder current ==");
  ...
  if (!found) {
    ...
    console.log(`  HARTHMERE_E2E_URL="<that exact URL>" node scripts/harthmere/test-harthmere-live-browser-regression-suite.cjs "${root}"`);
    process.exit(1);
  }
