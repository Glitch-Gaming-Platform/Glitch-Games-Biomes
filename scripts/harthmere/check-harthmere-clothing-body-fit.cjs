...
check("player current runtime version exists", /harthmere-modular-clothing-runtime-body-fit/.test(player));
...
check("player procedural clothing is body-fitted", /harthmere-threejs-clothing-body-fit/.test(player));
...
check("ECS NPC clothing runtime bumped to current", /harthmere-modular-clothing-runtime-body-fit/.test(npcs));
check("ECS NPC stores clothing fit metrics", /harthmereClothingFitMetrics = body/.test(npcs));
check("runtime clothing runtime bumped to current", /harthmere-modular-clothing-runtime-body-fit/.test(assets));
check("runtime stores clothing fit metrics", /harthmereClothingFitMetrics = body/.test(assets));
check("runtime NPC cosmetics bumped to body-fit version", /harthmere-unique-npc-cosmetics-body-fit-clothing/.test(assets));
