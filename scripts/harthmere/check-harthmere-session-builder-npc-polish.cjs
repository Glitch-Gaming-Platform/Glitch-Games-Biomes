...
ok('runtime NPC cosmetics bumped to current', /harthmere-unique-npc-cosmetics-distinct-crowd/.test(assets));
ok('runtime NPC unique shoulder cloak exists', /npc-unique-shoulder-cloak/.test(assets));
ok('runtime NPC unique hair streak exists', /npc-unique-hair-streak/.test(assets));
ok('runtime NPC unique bandolier exists', /npc-unique-bandolier/.test(assets));
ok('runtime NPC unique bedroll exists', /npc-unique-bedroll/.test(assets));
ok('runtime NPC stores distinct seed', /distinctSeed: seed/.test(assets));

ok('ECS NPC current helper exists', /function addLocalDevNpcUniqueEnhancementDetails/.test(npcs));
ok('ECS NPC helper is called', /addLocalDevNpcUniqueEnhancementDetails\(root, id, label, palette, body, appearance\);/.test(npcs));
ok('ECS NPC unique shoulder cloak exists', /harthmere-npc-unique-shoulder-cloak/.test(npcs));
ok('ECS NPC role-specific details exist', /harthmere-npc-unique-guard-medal/.test(npcs) && /harthmere-npc-unique-red-sash-knot/.test(npcs));
ok('ECS NPC stores current visual version', /harthmereNpcUniqueVisualVersion = "harthmere-unique-npc-cosmetics-distinct-crowd"/.test(npcs));
