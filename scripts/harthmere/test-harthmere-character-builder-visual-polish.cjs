...
check("builder keeps release layout marker for existing suite", wake.includes('data-harthmere-builder-layout="release-polish-clothing"'));
check("builder keeps legacy current UI marker while current preview is active", wake.includes('data-harthmere-builder-ui-version="full-body-readable-chips"') && wake.includes('svg-full-body-readable'));
...
check("full body preview is explicitly marked with non-overlapping SVG data", wake.includes('data-harthmere-builder-preview-svg="current"') && !wake.includes("Full body visible"));
...
check("CSS hardens selected/unselected visual states", css.includes("Harthmere character builder current release readability patch") && css.includes('[data-harthmere-choice-chip-selected="true"]') && css.includes('[data-harthmere-clothing-chip-selected="true"]'));
