...
const report = makeReporter(
  "Harthmere sword animation polish tests current",
  root
);
...
const updateSword = sliceBetween(
  renderer,
...
const manualSwing = sliceBetween(
  ...
  "private applyHarthmerePlayerSwordManualSwing"
);
...
const combatEffect = sliceBetween(
  renderer,
  "private readonly onCombatEffect",
  "private faceCombatActorToward"
);
const pulse = sliceBetween(
  renderer,
  "private applyCombatPulse",
  "private async loadAll"
);
const registerCombatLife = sliceBetween(
  renderer,
  "private registerCombatLife",
  "private findCombatLifeByOffset"
);
...
check(
  ...
  has(
    renderer,
    /resolveHarthmerePlayerBoneAnchor[\s\S]{0,1800}record\.isBone/i
  ) &&
    has(updateSword, /boneHandAnchor/) &&
    has(updateSword, /boneSheatheAnchor/) &&
    has(updateSword, /boneSheatheAnchor\s*\?\?/) &&
    has(updateSword, /lefthand|left_hand|mixamorigLeftHand/i),
...
check(
  ...
  has(debugBridge, /anchorMode[\s\S]{0,240}harthmereAttachmentMode/) &&
    has(
      updateSword,
      /harthmereAttachmentMode\s*=\s*boneHandAnchor|harthmereAttachmentMode[\s\S]{0,80}bone/
    ),
...
check(
  ...
  has(
    multiplayer,
    /HARTHMERE_SWORD_ATTACK_TIMINGS\s*=\s*HARTHMERE_PLAYER_ATTACK_TIMINGS/
  ) &&
    has(
      multiplayer,
      /HARTHMERE_SWORD_ATTACK_TIMINGS\.heavy[\s\S]{0,120}HARTHMERE_SWORD_ATTACK_TIMINGS\.basic/
    ),
...
check(
  ...
  has(visualEmitter, /harthmereSwordAttackTiming\(attack\)/) &&
    has(visualEmitter, /windupMs|impactMs|recoveryMs/),
...
check(
  ...
  has(
    keyedAttack,
    /resolveHarthmereSwordImpactFrame[\s\S]{0,180}performHarthmereForwardArcAttack\(attack\)/
  ) &&
    has(
      keyedAttack,
      /window\.setTimeout\(resolveHarthmereSwordImpactFrame,\s*timing\.impactMs\)/
    ),
...
check(
  ...
  has(multiplayer, /__harthmereSwordImpactTimingDebugLog/) &&
    has(keyedAttack, /phase:\s*["']scheduled["']/) &&
    has(keyedAttack, /phase:\s*["']impact["']/),
...
check(
  ...
  has(
    renderer,
    /ensureHarthmerePlayerSwordTrail[\s\S]{0,900}TorusGeometry[\s\S]{0,400}transparent:\s*true[\s\S]{0,240}DoubleSide/
  ) ||
    has(
      renderer,
      /harthmere-player-sword-slash-trail[\s\S]{0,900}transparent:\s*true/
    ),
...
check(
  ...
  has(renderer, /spawnHarthmerePlayerSwordTrail\(["']basic["']/) &&
    has(renderer, /spawnHarthmerePlayerSwordTrail\(["']heavy["']/) &&
    has(renderer, /attack === ["']heavy["'] \? 230 : 155/),
...
check(
  ...
  has(
    renderer,
    /updateHarthmerePlayerSwordTrail[\s\S]{0,700}remaining <= 0[\s\S]{0,500}opacity\s*=\s*0/
  ) &&
    has(updateSword, /updateHarthmerePlayerSwordTrail\(performance\.now\(\)\)/),
...
check(
  ...
  has(
    updateSword,
    /curveLift[\s\S]{0,220}Math\.sin\(t \* Math\.PI\)[\s\S]{0,500}sword\.position\.y \+= curveLift/
  ),
...
check(
  ...
  has(
    updateSword,
    /wristTwist[\s\S]{0,500}sword\.rotateX\(wristTwist\)[\s\S]{0,160}sword\.rotateZ\(wristTwist/
  ),
...
check(
  ...
  has(combatEffect, /renderer\.hit_stop\.impact/) &&
    has(
      combatEffect,
      /harthmereHitStopUntil\s*=\s*performance\.now\(\) \+ 65/
    ) &&
    has(
      combatEffect,
      /harthmereAttackerRecoveryUntil\s*=\s*performance\.now\(\) \+ 180/
    ),
...
check(
  ...
  has(
    pulse,
    /nowMs < this\.harthmereHitStopUntil[\s\S]{0,180}pulse\.kind === ["']hit["'][\s\S]{0,120}pulse\.kind === ["']block["']/
  ),
...
check(
  ...
  has(
    pulse,
    /recoveryWave[\s\S]{0,180}harthmereAttackerRecoveryUntil[\s\S]{0,260}actor\.object\.position\.y/
  ),
...
check(
  ...
  has(
    renderer,
    /ensureHarthmereBlockContactFeedback[\s\S]{0,900}harthmere-block-contact-spark/
  ) &&
    has(
      combatEffect,
      /targetKind === ["']block["'][\s\S]{0,180}triggerHarthmereBlockContactFeedback/
    ),
...
check(
  ...
  has(
    renderer,
    /harthmereNpcWeaponVisuals\s*=\s*new Map<\s*THREE\.Object3D,\s*THREE\.Group\s*>/
  ),
  "NPC held weapons need lifecycle tracking."
);
check(
  "NPC combat actors attach a visible main-hand weapon to left-hand anchor",
  has(
    renderer,
    /attachHarthmereNpcWeaponVisual[\s\S]{0,4200}harthmere-anchor-left-hand[\s\S]{0,900}anchor\.add\(visual\)/
  ) &&
    has(
      registerCombatLife,
      /this\.combatLifeInstances\.push\(actor\)[\s\S]{0,160}this\.attachHarthmereNpcWeaponVisual\(actor\)/
    ),
...
check(
  ...
  has(renderer, /kind === ["']attack["'][\s\S]{0,500}Attack/) &&
    has(actionCsv, /Death/) &&
    has(actionCsv, /HitReact/),
...
check(
  ...
  has(
    debugBridge,
    /swordVisualRegressionPose[\s\S]{0,900}sheathed[\s\S]{0,600}basic_slash[\s\S]{0,600}heavy_slash[\s\S]{0,600}block[\s\S]{0,600}npc_attack/
  ),
...
check(
  ...
  has(
    debugBridge,
    /setSwordFacing[\s\S]{0,500}north[\s\S]{0,160}east[\s\S]{0,160}south[\s\S]{0,160}west[\s\S]{0,500}bodyForward/
  ),
...
check(
  ...
  has(
    debugBridge,
    /trailVisible[\s\S]{0,300}blockFeedbackVisible[\s\S]{0,300}npcWeaponVisualCount[\s\S]{0,300}lastImpactAt/
  ),
...
for (const clip of [
  ...
  check(
    `equipment manifest still includes ${clip}`,
    equipmentManifest.includes(clip),
    `Missing generated sword clip ${clip}`
  );
}
