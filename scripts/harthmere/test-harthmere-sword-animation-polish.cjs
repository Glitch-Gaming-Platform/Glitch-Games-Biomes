...
const report = makeReporter("Harthmere sword animation polish tests current", root);
...
check(
  ...
  has(multiplayer, /HARTHMERE_SWORD_ATTACK_TIMINGS[\s\S]{0,300}basic[\s\S]{0,120}impactMs[\s\S]{0,240}heavy[\s\S]{0,120}impactMs/),
...
check(
  ...
  has(renderer, /attachHarthmereNpcWeaponVisual[\s\S]{0,1200}harthmere-anchor-left-hand[\s\S]{0,500}anchor\.add\(visual\)/) &&
