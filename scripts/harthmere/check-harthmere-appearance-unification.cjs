...
ok(/HARTHMERE_BODY_VERSION\s*=\s*2/.test(shared), "shared has body schema version current");
...
ok(/playerBody\.user\./.test(shared), "player body storage key uses canonical namespace");
ok(/PLAYER_BODY_LEGACY_KEY_PREFIXES[\s\S]*playerBody\.user\./.test(shared), "player body legacy migration remains supported");
