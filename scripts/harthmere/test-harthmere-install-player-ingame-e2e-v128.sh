...
ARTIFACTS_DIR="${E2E_ARTIFACTS_DIR:-/tmp/harthmere-playboot-e2e-v128}"
...
echo 'Preflight 0: v127 source validator (install_id flow patch must be in place)'
node "$REPO/scripts/harthmere/validate-harthmere-install-id-flow-v127.cjs" "$REPO"

echo 'Preflight 1: v127 unit tests'
...
echo 'Browser E2E v128: install_id to player-in-game (headless WebGL graceful)'
E2E_ARTIFACTS_DIR="$ARTIFACTS_DIR" \
INSTALL_ID="$INSTALL_ID" \
BASE_URL="$BASE_URL" \
HEADLESS="${HEADLESS:-1}" \
STRICT_RENDER="${STRICT_RENDER:-0}" \
node "$REPO/scripts/harthmere/test-harthmere-install-player-ingame-e2e-v128.cjs" "$REPO" \
  --base-url "$BASE_URL" \
...
echo "  HEADLESS=0 bash scripts/harthmere/test-harthmere-install-player-ingame-e2e-v128.sh $REPO"
