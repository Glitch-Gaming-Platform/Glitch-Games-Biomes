...
const contract = blockFrom(runtime, "HARTHMERE_RUNTIME_APPEARANCE_OPTION_EXPRESSION_CONTRACT", 12000);
const metrics = bodyOf(runtime, "function harthmereRuntimeBodyMetrics(");
const faceMetrics = bodyOf(runtime, "function harthmereRuntimeFaceShapeMetrics(");
...
check("current option-expression contract exists", contract.length > 0);
check("body metrics function references current option-expression contract",
  metrics.includes("HARTHMERE_RUNTIME_APPEARANCE_OPTION_EXPRESSION_CONTRACT") ||
  metrics.includes("optionExpressionContract"));
check("face-shape metrics function references current face contract",
  faceMetrics.includes("HARTHMERE_RUNTIME_APPEARANCE_OPTION_EXPRESSION_CONTRACT.faceShape") ||
  faceMetrics.includes("faceShapeExpressionContract"));
...
for (const [field, constName] of Object.entries(optionSources)) {
  ...
  for (const option of options) {
    check(`current contract/runtime handles ${field}:${option}`,
...
check("body expression derives neck/head expression from torsoHeight",
  bodyExpression.includes("expressionHeadHeight") && bodyExpression.includes("body.torsoHeight"));
