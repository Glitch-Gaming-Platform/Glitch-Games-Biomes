...
// SNAPSHOT_LIVE_NPC_LORE_CHECK_V79
const fs = require("fs");
...
const shared = read("src/shared/harthmere/snapshot_live_npc_bible_v79.ts");
const runtime = read("src/client/components/challenges/LocalDevSnapshotLiveNpcLoreRuntimeV79.tsx");
const dialog = read("src/client/components/challenges/TalkToNPCDefaultDialog.tsx");
const doc = read("docs/harthmere/snapshot_grove_live_npc_bible_v79.md");

ok(shared.includes("SNAPSHOT_LIVE_NPC_BIBLE_VERSION_V79"), "shared live NPC bible registry exists");
ok(shared.includes("SNAPSHOT_LIVE_NPC_BIBLE_NO_NEW_NPCS_V79 = true"), "registry explicitly does not spawn new NPCs");
ok(shared.includes("SNAPSHOT_LIVE_NPC_BIBLE_EXCLUDES_HARTHMERE_V79 = true"), "registry explicitly excludes Harthmere");
ok(runtime.includes("SNAPSHOT_LIVE_NPC_LORE_RUNTIME_VERSION_V79"), "client live NPC lore runtime exists");
ok(dialog.includes("useSnapshotLiveNpcLoreDialogV79"), "default NPC dialog imports live snapshot lore hook");
...
for (const name of harthmereNames) {
  const bodyWithoutExcludedList = shared.replace(/SNAPSHOT_LIVE_NPC_EXCLUDED_HARTHMERE_NAMES_V79[\s\S]*?\] as const;/, "");
...
ok(runtime.includes("__snapshotLiveNpcBibleV79"), "debug helper exposes live NPC bible audit");

if (process.exitCode) {
  console.error("v79 live snapshot NPC lore check failed");
  process.exit(process.exitCode);
}
console.log("v79 live snapshot NPC lore check passed");
