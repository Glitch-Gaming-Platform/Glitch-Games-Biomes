#!/usr/bin/env node
/*
 * Optional live browser smoke for Building System v5.
...
(async () => {
  ...
  try {
    ...
    const hasCollisionProbe = await page.evaluate(() => {
      const w = window;
      return Boolean(
        w.__harthmereBuildingSmokeV5?.floorWalkable &&
          w.__harthmereBuildingSmokeV5?.wallsCollide &&
          w.__harthmereBuildingSmokeV5?.roofStandable &&
          w.__harthmereBuildingSmokeV5?.stairsNavigable
      );
    });
    if (!hasCollisionProbe) {
      console.log("WARN collision probe not exposed at window.__harthmereBuildingSmokeV5; UI/map smoke passed, physics probe skipped.");
    }
    console.log("RESULT: PASS");
  } finally {
    await browser.close();
