...
check("quest runtime current version exported", /HARTHMERE_QUEST_RUNTIME_VERSION\s*=\s*47/.test(src));
for (const fn of ["createHarthmereQuestRuntimeContext", "acceptHarthmereQuest", "advanceHarthmereQuestObjective", "completeHarthmereQuest", "validateHarthmereQuestRuntimeEvent"]) {
...
check("imports current quest catalog", src.includes("HARTHMERE_QUEST_CATALOG"));
check("accept uses current activation validation", src.includes("validateHarthmereQuestActivation"));
...
console.log("RESULT: PASS quest runtime execution current");
