#!/usr/bin/env node
"use strict";
/* HARTHMERE_BIBLE_IMPLEMENTATION_AUDIT_TEST */
const fs = require("fs");
...
console.log("== Harthmere bible implementation audit test current ==");
console.log(`Root: ${root}`);
const auditScript = path.join(root, "scripts/harthmere/audit-harthmere-bible-implementation.cjs");
...
const reportPath = path.join(root, "docs/harthmere/HARTHMERE_BIBLE_IMPLEMENTATION_AUDIT.md");
const jsonPath = path.join(root, "public/assets/harthmere/manifest/harthmere-bible-implementation-audit.json");
...
if (fs.existsSync(jsonPath)) {
  ...
  check(data.npc.totalCount >= 180, "npc catalog has broad current/current coverage", data.npc.totalCount);
...
console.log(ok ? "\nRESULT: PASS bible implementation audit current" : "\nRESULT: FAIL bible implementation audit current");
process.exit(ok ? 0 : 1);
