...
check(renderer.includes("private async loadHarthmerePlayerSwordGltf"), "renderer has real sword GLTF loader method");
check(renderer.includes("this.gltfLoader.loadAsync(entry.assetUrl)"), "renderer loads real sword GLTF");
check(renderer.includes("private normalizeHarthmerePlayerSwordGltfScale"), "renderer normalizes imported sword scale");
check(renderer.includes("private playHarthmerePlayerSwordClip"), "renderer can play sword animation clips");
...
check(renderer.includes('playHarthmerePlayerSwordAnimationForCurrentState("event")'), "renderer plays sword clip on state events");
check(hud.includes('const itemId = "iron_longsword"') || hud.includes('itemId: "iron_longsword"'), "HUD pins sword visual to longsword for current renderer");
