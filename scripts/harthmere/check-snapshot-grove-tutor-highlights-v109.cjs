...
// Verifies the four v109 deliverables on top of v108:
...
//      sentence bug visible in the pre-v109 screenshots.
//   2. The runtime broadcasts a bottom-bar nav-label highlight via the
//      SNAPSHOT_GROVE_TUTOR_HIGHLIGHT_EVENT_V109 event, and the HUD listens
//      and renders a pulsing ring + bouncing down-arrow on the matching
//      NavSlot.
//   3. A new "Chat" NavSlot exists, an openSnapshotGroveTutorChatPanelV109()
//      helper exists, the SnapshotGroveTutorChatPanelV109 component renders
...
ok(
  ...
  "npcQuestDialogueCopyV100 does NOT concatenate <text> tags directly (forces use of {break})",
);
ok(
  /dialogText: `<text>\$\{line\}<\/text>\{break\}\$\{questCopy\}`/.test(runtime),
  "Bark line and quest copy are joined with {break} in useSnapshotGroveNpcDialogV75",
);
ok(
...
ok(
  /export const SNAPSHOT_GROVE_TUTOR_HIGHLIGHT_EVENT_V109\s*=/.test(runtime),
  "Runtime exports SNAPSHOT_GROVE_TUTOR_HIGHLIGHT_EVENT_V109 event constant",
);
ok(
  /export function snapshotGroveTutorNavLabelsForHighlightsV109\(/.test(runtime),
  "Runtime exports snapshotGroveTutorNavLabelsForHighlightsV109 (chip -> NavSlot label mapping)",
);
ok(
  /function broadcastSnapshotGroveTutorHudLabelsV109\(/.test(runtime),
  "Runtime defines broadcastSnapshotGroveTutorHudLabelsV109 helper",
);
...
ok(
  /broadcastSnapshotGroveTutorHudLabelsV109\(\[\]\)/.test(runtime),
...
ok(
  ...
  "expectedOpenTabForObjectiveV106 recognizes 'chat' objective text and routes to chat tab",
);
ok(
  /highlights\.add\("CHAT"\)/.test(runtime),
  "groveHudHighlightsForTriggerV106 adds CHAT chip for chat/whisper/channel objectives",
);

// 4. HUD changes — pulse + arrow keyframes, useTutorHighlightedNavLabelsV109
//    hook, Chat NavSlot, ensureSnapshotGroveTutorHighlightStylesV109 mount.
ok(
  /snapshotGroveTutorPulseV109/.test(hud),
  "HUD defines a snapshotGroveTutorPulseV109 keyframe for the pulsing ring",
);
ok(
  /snapshotGroveTutorArrowV109/.test(hud),
  "HUD defines a snapshotGroveTutorArrowV109 keyframe for the bouncing down-arrow",
);
ok(
  /ensureSnapshotGroveTutorHighlightStylesV109/.test(hud),
  "HUD injects the keyframe styles via ensureSnapshotGroveTutorHighlightStylesV109",
);
ok(
  /function useTutorHighlightedNavLabelsV109\(/.test(hud),
  "HUD defines useTutorHighlightedNavLabelsV109 hook",
);
ok(
  /"biomes:snapshot-grove-tutor-hud-highlights-v109"/.test(hud),
  "HUD listens for the v109 highlight broadcast event",
);
ok(
...
ok(
  /label="Chat"[\s\S]{0,200}openSnapshotGroveTutorChatPanelV109\(\)/.test(hud),
  "New Chat NavSlot exists and calls openSnapshotGroveTutorChatPanelV109 on click",
);
ok(
...
ok(
  /export const SnapshotGroveTutorChatPanelV109/.test(runtime),
  "Runtime exports SnapshotGroveTutorChatPanelV109 component",
);
ok(
  /export function openSnapshotGroveTutorChatPanelV109\(\)/.test(runtime),
  "Runtime exports openSnapshotGroveTutorChatPanelV109 helper",
);
...
ok(
  /SnapshotGroveTutorChatPanelV109,/.test(hud),
  "HUD imports SnapshotGroveTutorChatPanelV109",
);
ok(
  /openSnapshotGroveTutorChatPanelV109,/.test(hud),
  "HUD imports openSnapshotGroveTutorChatPanelV109",
);
ok(
  /<SnapshotGroveTutorChatPanelV109 \/>/.test(hud),
  "HUD mounts <SnapshotGroveTutorChatPanelV109 /> inside the HUD tree",
);
...
ok(
  /data-snapshot-grove-nav-arrow-keys-v109="true"/.test(hud),
...
if (failures) {
  console.error(`v109 tutor highlights check failed: ${failures} failure(s)`);
  process.exit(1);
}
console.log("v109 tutor highlights check passed");
