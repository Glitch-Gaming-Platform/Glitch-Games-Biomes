...
/**
 * Harthmere NPC clothing + animation invariant tests current.
 *
 ...
 *   or as a wrapper around makeHarthmereNpcAppearanceConfigBase.
 * - The exported NPC appearance function must guarantee final non-animal
 *   clothing through harthmereEnsureProductMinecraftClothingSet(...).
...
console.log("== Harthmere NPC clothing + animation invariant tests current ==");
...
const npcAppearanceBase = extractFunction(faces, "makeHarthmereNpcAppearanceConfigBase");
...
const clothingGuarantee = extractFunction(faces, "harthmereEnsureProductMinecraftClothingSet");

section("Clothing catalog and assignment tests");

ok(
  "current product polish catalog is active",
...
ok(
  ...
  clothingGuarantee.includes("harthmereEnsureProductMinecraftClothingSet")
);

ok(
  "generated NPC appearance calls complete clothing guarantee",
  npcAppearanceWrapper.includes("harthmereEnsureProductMinecraftClothingSet") ||
    npcAppearanceBase.includes("harthmereEnsureProductMinecraftClothingSet")
);

ok(
  "generated NPC appearance preserves unique clothing generation",
  npcAppearanceWrapper.includes("makeHarthmereNpcAppearanceConfigBase") ||
...
ok("ECS/local-dev NPC renderer queues optional GLTF modelUrl clothing", npcs.includes("queueLocalDevNpcLicensedClothingModel"));
...
ok("runtime/ambient NPC path queues optional GLTF modelUrl clothing", assets.includes("queueHarthmereRuntimeLicensedClothingModels"));
...
const animationFactory = extractFunction(npcs, "makeLocalDevVoxelNpcAnimationClips");
const gltfFactory = extractFunction(npcs, "makeLocalDevVoxelNpcGltf");
const executionRecorder = extractFunction(npcs, "recordHarthmereNpcAnimationExecutionCheck");

ok("NPC animation clip factory exists", animationFactory.includes("makeLocalDevVoxelNpcAnimationClips"));
...
ok(
  ...
  (
    gltfFactory.includes("animations: makeLocalDevVoxelNpcAnimationClips()") ||
    (
      gltfFactory.includes("makeLocalDevVoxelNpcAnimationClips") &&
      (
    ...
    (
      gltfFactory.includes("const animations = makeLocalDevVoxelNpcAnimationClips()") &&
      /\banimations\s*,/.test(gltfFactory)
    ) ||
    (
      npcs.includes("makeLocalDevVoxelNpcAnimationClips") &&
      (
...
ok(
  ...
  !defaultRoleClothing.includes("mergeHarthmereLicensedClothingDefaults") &&
    !defaultRoleClothing.includes("mergeHarthmereLicensedClothingDefaults")
);
