...
// The current installer failed when invoked from ~/Downloads because process.cwd()
...
ok(text.includes("GLITCH_SNAPSHOT_BUFF_TYPE_COMPAT"), "buff type compatibility marker is present");
...
ok(text.includes("GLITCH_SNAPSHOT_BUFF_TYPE_COMPAT unknown buff type fallback"), "fallback warning includes searchable marker");
ok(text.includes("type = maybeBuffType"), "itemBuffDescription uses soft buff type lookup");

if (process.exitCode) {
  process.exit(process.exitCode);
}
console.log("snapshot buff type compatibility current check passed");
