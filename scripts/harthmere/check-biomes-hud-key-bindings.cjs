#!/usr/bin/env node
/* BIOMES_HUD_KEY_BINDINGS_STATIC_CHECK
...
const bindings = read("src/shared/harthmere/harthmere_hud_key_bindings.ts");
const hud = read("src/client/components/challenges/HarthmereUnifiedHUD.tsx");
const hudState = read("src/shared/harthmere/harthmere_hud_state.ts");
...
if (/export function reduceHarthmereHudStateForAction/.test(hudState) && /export \{ reduceHarthmereHudStateForAction \}/.test(hud)) ok("HUD action reducer is exported for unit testing"); else fail("HUD action reducer export missing");
...
if (/hudMapRegionForPlayerPosition/.test(quests) && /The Grove field map/.test(quests)) ok("quest map adapts to the player's current region"); else fail("quest map still does not adapt to The Grove vs Harthmere");
...
if (/harthmereResourceIconForItem/.test(inventory) && /🪵|⛏️|🌿|🦴/.test(inventory)) ok("inventory resource icons are item-specific"); else fail("inventory material icons still look generic");
if (/harthmereGatheringResourceIcon/.test(gathering)) ok("gathering storage shows resource-specific icons"); else fail("gathering storage still lacks resource icons");
