...
assert(bridge.includes('GLITCH_INSTALL_HEARTBEAT_INTERVAL_MS_V143 = 60_000'), 'client uses the required 60-second Glitch install heartbeat cadence');
...
assert(!deploy.includes('test-harthmere-character-builder-supported-voxel-features-v182.cjs'), 'deploy guardrails no longer run stale v182 WakeUpScreen visual coverage test');

if (process.exitCode) {
  process.exit(process.exitCode);
}
console.log('PASS glitch-guest-identity-funnel-v143');
