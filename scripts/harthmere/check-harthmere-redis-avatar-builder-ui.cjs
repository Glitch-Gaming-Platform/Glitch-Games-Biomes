...
check('builder layout marked current balanced preview', /data-harthmere-builder-layout="balanced-preview"/.test(wake));
check('builder width reduced from current huge layout', /w-\[min\(82rem,96vw\)\]/.test(wake) && !/w-\[min\(106rem,98vw\)\]/.test(wake));
...
check('current scoped CSS exists', /Harthmere character builder current balanced preview polish/.test(css));
check('current CSS constrains avatar viewer max height', /max-height:\s*23rem/.test(css));
check('current CSS has small-height media guard', /@media \(max-height: 760px\)/.test(css));
