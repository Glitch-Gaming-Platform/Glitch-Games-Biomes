...
// HARTHMERE_LIVE_PROD_LOCAL_ASSET_PARITY_V164
...
if (!origin) {
  console.error("usage: node scripts/harthmere/test-harthmere-live-prod-local-asset-parity-v164.cjs https://your-origin");
  process.exit(2);
}
...
function request(url, depth = 0) {
  ...
  return new Promise((resolve, reject) => {
    const req = client.get(url, {
      timeout: 45000,
      headers: {
        "User-Agent": "harthmere-live-prod-local-asset-parity-v164",
