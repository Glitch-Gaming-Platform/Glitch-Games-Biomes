...
// HARTHMERE_NPC_RENDER_ASSET_COVERAGE_V164
...
ok("NPC resource has visible fallback for failed generated player-like mesh", /HARTHMERE_NPC_RENDER_PARITY_V164[\s\S]*falling back to visible voxel NPC/m.test(npcResources));
ok("NPC resource has visible fallback for failed galois mesh", /HARTHMERE_NPC_GALOIS_VISIBLE_FALLBACK_V164[\s\S]*makeLocalDevVoxelNpcGltf\(deps, id\)/m.test(npcResources));
