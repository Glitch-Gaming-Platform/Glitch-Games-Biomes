...
const versions =
  JSON.parse(fs.readFileSync(assetVersionsPath, "utf8")).paths || {};
...
if (missing.length) {
  console.error(
    `FAIL missing ${missing.length} static bucket assets referenced by asset_versions.json`
  );
  console.error(
    "Hydrate or export the current generated assets in this exact build workspace before creating a production image."
  );
  process.exit(1);
}

console.log(
  "OK every asset_versions.json path exists in public/buckets/biomes-static"
);
