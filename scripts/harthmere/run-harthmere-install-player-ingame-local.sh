...
echo 'Step 1/7: validate current source patch (must be in place)'
node scripts/harthmere/validate-harthmere-install-id-flow.cjs "$REPO"
  ...
  if [ -f "$envfile" ]; then
    if grep -E '^\s*NEXT_PUBLIC_GLITCH_SYNC_BASE_URL\s*=' "$envfile" >/dev/null 2>&1; then
      ...
      if echo "$VALUE" | grep -Eiq 'azurecontainerapps|biomes\.gg|prod'; then
        echo "  NOTE: $envfile NEXT_PUBLIC_GLITCH_SYNC_BASE_URL looks remote. current runtime override neutralizes it."
      fi
    fi
  fi
done

echo 'Step 3/7: current unit tests'
if [ -d node_modules/typescript ]; then
  node scripts/harthmere/test-harthmere-install-id-flow-unit.cjs "$REPO"
else
  echo '  SKIP: node_modules/typescript not installed.'
fi

if [ "$SKIP_REBUILD" = "0" ]; then
  echo 'Step 4/7: clean production build (full .next wipe)'
  ...
  docker run -d \
    ...
    --entrypoint /bin/bash \
    "$IMAGE_TAG" \
    -lc './scripts/glitch/run-glitch-local-game-stack.sh'
else
  echo 'Step 4-6/7: SKIP_REBUILD=1 - reusing existing biomes-local container.'
fi
...
echo 'Step 7/7: run current browser E2E (headless WebGL graceful)'
INSTALL_ID="$INSTALL_ID" \
BASE_URL="http://127.0.0.1:3017" \
DOCKER_CONTAINER="biomes-local" \
HEADLESS="$HEADLESS" \
STRICT_RENDER="$STRICT_RENDER" \
bash scripts/harthmere/test-harthmere-install-player-ingame-e2e.sh "$REPO"
