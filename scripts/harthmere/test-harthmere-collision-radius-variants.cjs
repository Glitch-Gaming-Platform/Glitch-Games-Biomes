#!/usr/bin/env node
"use strict";
/* HARTHMERE_COLLISION_RADIUS_VARIANTS_STRICT_RUNTIME */
const fs = require("fs");
...
console.log("== Harthmere collision radius variant tests current ==");
...
if (/HARTHMERE_COLLISION_RADIUS_VARIANTS|collision-radius-variants/i.test(player + mount + routes)) {
  pass("collision radius variant contract marker exists");
} else {
  fail("collision radius variant contract marker exists", "Add HARTHMERE_COLLISION_RADIUS_VARIANTS or equivalent radius variant definitions.");
}
