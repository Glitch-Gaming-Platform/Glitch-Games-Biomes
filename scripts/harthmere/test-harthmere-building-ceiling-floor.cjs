#!/usr/bin/env node
"use strict";
/* HARTHMERE_BUILDING_CEILING_FLOOR_TEST
 *
...
if (src.includes("solid stone/ore ground floor slab")) {
  ...
  fail("service buildings declare ground floor slab",
       "expected `solid stone/ore ground floor slab` label in createHarthmereServiceFloorDeckBlocks");
}
...
// 5. current must NOT replace the slab emit — the slab functions must still be
//    called by the service / housing wrappers.
if (/createHarthmereServiceFloorDeckBlocks\s*\(/.test(src)) {
  pass("service buildings still emit floor/ceiling slabs (current deck blocks)");
} else {
  fail("service buildings still emit floor/ceiling slabs (current deck blocks)",
       "expected createHarthmereServiceFloorDeckBlocks call");
}
if (/createHarthmereResidentFloorDeckBlocks\s*\(/.test(src)) {
  pass("housing buildings still emit floor/ceiling slabs (current deck blocks)");
} else {
  fail("housing buildings still emit floor/ceiling slabs (current deck blocks)",
       "expected createHarthmereResidentFloorDeckBlocks call");
}

console.log(ok ? "\nRESULT: PASS ceiling/floor current" : "\nRESULT: FAIL ceiling/floor current");
process.exit(ok ? 0 : 1);
