...
const report = {
  version: "harthmere-floating-runtime-assets",
  glbDefinitions: glbDefinitions.length,
  structuralOrMapGlbDefinitions: structuralKeys.length,
  note: "Definitions are intentionally left in the file. current removes their placements in snapshot-built runtime mode; it does not delete assets.",
  structuralKeys,
};
const out = path.join(outDir, `harthmere-floating-runtime-assets.${Date.now()}.json`);
