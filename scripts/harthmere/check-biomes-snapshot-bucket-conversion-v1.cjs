...
console.log("== Biomes snapshot bucket conversion v1 ==");

const bucketFiles = walk(bucketsRoot).map((file) => file.slice(root.length + 1));
const harthmereFiles = walk(harthmereRoot).map((file) => file.slice(root.length + 1));
...
check(harthmereFiles.length >= 7000, "Harthmere runtime asset file count is present");
...
check((extCounts.obj || 0) >= 500, "Harthmere OBJ assets are present");
check((extCounts.fbx || 0) >= 250, "Harthmere FBX assets are present");
check((extCounts.png || 0) >= 1500, "Harthmere PNG assets are present");
