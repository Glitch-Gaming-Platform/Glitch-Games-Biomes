...
//
// Terrain shards are ECS entities indexed by voxelShard(box.v0) -> entityId, so
...
const {
  findHarthmereGroundFeetYByCanStandV1,
  HARTHMERE_GROUND_SCAN_DOWN_DEFAULT_V1,
  HARTHMERE_GROUND_SCAN_UP_DEFAULT_V1,
} = require("../../src/shared/harthmere/harthmere_entity_grounding_v1");
const {
...
function grounderFeetY(solidity, x, z, hintY) {
  ...
  return findHarthmereGroundFeetYByCanStandV1((feetY) => canStand(feetY), { hintY });
}
...
function summarize(label, rows) {
  ...
  console.log(JSON.stringify({
    ...
    scanBudget: { down: HARTHMERE_GROUND_SCAN_DOWN_DEFAULT_V1, up: HARTHMERE_GROUND_SCAN_UP_DEFAULT_V1 },
  }, null, 2));
...
async function main() {
  ...
  const muckers = harthmereGroundedMuckMonsterSeedsInTerritoryV1().map((s) => ({
    tag: s.seedId,
    position: s.position,
    areaId: muckMonsterAreaForPositionV1(s.position, 1.5)?.id ?? "none",
  }));
  const owners = HARTHMERE_BUSINESS_OWNER_NPC_SEEDS_V1.map((s) => ({ tag: s.ownerNpcId, position: s.position }));
