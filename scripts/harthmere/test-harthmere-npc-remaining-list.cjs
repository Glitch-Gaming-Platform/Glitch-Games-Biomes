...
assert(named.length >= 44, `current named NPCs remain present, found ${named.length}`);
assert(remaining.length === required.length, `expected ${required.length} remaining current NPCs, found ${remaining.length}`);
...
assert(new Set(allIds).size === allIds.length, "current+current NPC ids are globally unique");
if (process.exitCode) process.exit(process.exitCode);
console.log("RESULT: PASS npc remaining list current");
