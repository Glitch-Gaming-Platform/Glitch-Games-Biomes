...
esbuild.buildSync({
  entryPoints: [
    path.join(
      root,
      "scripts/harthmere/browser/boss-animation-visual-audit.ts"
    ),
  ],
  bundle: true,
  outfile: bundlePath,
...
<html lang="en">
  <head>
    ...
    <title>Harthmere boss animation visual audit</title>
  </head>
...
server.listen(port, "127.0.0.1", () => {
  process.stdout.write(
    `Harthmere boss animation visual audit: http://127.0.0.1:${port}/\n`
  );
});
