...
function expandUiIdsExpression(name, rawArg) {
  ...
  const stringArg = arg && ((arg.match(/^"([^"]+)"$/) || arg.match(/^'([^']+)'$/) || [])[1]);
  const numArg = arg && (/^\d+$/.test(arg) ? Number(arg) : undefined);

  switch (name) {
    case "HOTBAR_SLOT": return numArg ? [`hotbar.slot_${numArg}`] : ["hotbar.slot_"];
    case "MAP_MARKER": return stringArg ? [`map.marker.${stringArg}`] : ["map.marker."];
    case "ABILITY_SLOT": return numArg ? [`abilities.slot_${numArg}`] : ["abilities.slot_"];
    case "SKILL_ROW": return stringArg ? [`skills.row.${stringArg}`] : ["skills.row."];
    case "CLASS_CARD": return stringArg ? [`classes.card.${stringArg}`] : ["classes.card."];
    case "LAND_PLOT": return stringArg ? [`land.plot.${stringArg}`] : ["land.plot."];
    case "LOOT_ENTRY": return stringArg ? [`loot.entry.${stringArg}`] : ["loot.entry."];
    case "GUILD_RANK": return stringArg ? [`guilds.rank.${stringArg}`] : ["guilds.rank."];
    case "BANKING_VAULT_SLOT": return numArg ? [`banking.vault.slot_${numArg}`] : ["banking.vault.slot_"];
    default: return [];
  }
}
...
function hasMountedTargetFor(id) {
  ...
  return dynamicPrefixes.some((prefix) => id.startsWith(prefix) && allUiText.includes(prefix));
}

for (const id of cueIds) {
  assert(hasMountedTargetFor(id), `tutorial cue has mounted highlight target or dynamic renderer: ${id}`);
}

notContains(mount, "PLACEHOLDER_HOTBAR", "BiomesUIMount does not rely on placeholder hotbar data in replacement mode");
notContains(mount, "Bare Hands", "BiomesUIMount hotbar is not sample Bare Hands data");
notContains(mount, "Singularity Block", "BiomesUIMount hotbar is not sample Singularity Block data");
notContains(mount, "<TutorialDirector step={null}", "TutorialDirector receives live mission step, not null");
contains(mount, "adapters={live.adapters}", "BiomesUIMount passes real adapters into BiomesUI");
contains(mount, "hotbar={live.hotbar}", "BiomesUIMount hotbar is connected through live hook");
contains(mount, "useBiomesUILiveAdapters", "BiomesUIMount delegates state wiring to live adapter hook");

contains(liveAdapters, "useClientContext", "live adapter hook reads game ClientContext");
contains(liveAdapters, 'reactResources.use("/ecs/c/inventory", userId)', "live adapter hook subscribes to ECS inventory");
contains(liveAdapters, 'reactResources.use("/hotbar/index")', "live adapter hook subscribes to hotbar index");
contains(liveAdapters, "InventoryChangeSelectionEvent", "live hotbar selection updates real selected item path");
...
contains(liveAdapters, "__snapshotGrove", "map/tutorial adapter reads live Snapshot Grove state when available");
...
contains(questHud, "<MiniMapHUD />", "replacement mode keeps live minimap path available");
...
for (const id of requiredCueIds) {
  assert(hasMountedTargetFor(id), `required tutorial/highlight target is implemented: ${id}`);
}

console.log(`Biomes UI full replacement readiness: ${passes.length} passed, ${failures.length} failed.`);
if (failures.length) {
