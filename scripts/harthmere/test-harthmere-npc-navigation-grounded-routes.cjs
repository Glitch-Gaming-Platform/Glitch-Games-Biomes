...
const {
  loadTown,
  makeReporter,
} = require("./harthmere-town-rule-test-utils.cjs");

const root = process.argv[2] ? path.resolve(process.argv[2]) : process.cwd();
const report = makeReporter("Harthmere NPC grounded swept route navigation current", root);
...
const rendererAnchors = extractRouteAnchors(assetsSrc, "HARTHMERE_NPC_ROUTE_ANCHORS");
const sharedAnchors = extractRouteAnchors(routesSrc, "HARTHMERE_TOWN_ROUTE_ANCHORS");
...
report.check("renderer declares current NPC navigation fix", assetsSrc.includes("HARTHMERE_NPC_NAVIGATION_FIX_VERSION") && assetsSrc.includes("harthmere-npc-grounded-swept-route-navigation"));
...
report.check("collision lookup remains grid-hashed for performance", assetsSrc.includes("HARTHMERE_OBSTACLE_GRID_CELL_METERS") && assetsSrc.includes("harthmereNpcObstacleGrid") && hardObstacles.length < 700, `hard obstacle count ${hardObstacles.length}`);
report.check("existing unreachable/route tests are wired into production deploy guardrails", deploySrc.includes("test-harthmere-npc-route-graph.cjs") && deploySrc.includes("test-harthmere-runtime-navigation-collision.cjs") && deploySrc.includes("test-harthmere-npc-navigation-grounded-routes.cjs"));
