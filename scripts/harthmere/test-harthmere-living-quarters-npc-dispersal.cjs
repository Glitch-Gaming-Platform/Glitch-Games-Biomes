...
const housingPath = path.join(root, "src/shared/harthmere/resident_housing.ts");
...
console.log("== Harthmere living quarters + NPC dispersal current test ==");

check("housing current version marker exists", housing.includes("HARTHMERE_RESIDENT_HOUSING_LIVING_QUARTERS_REBUILD_VERSION"));
check("renderer current living-quarter rebuild marker exists", assets.includes("HARTHMERE_LIVING_QUARTERS_REBUILD_RENDERER_VERSION"));
check("resident story frame delegates to current block shell", (/return createHarthmereLivingQuarterBlockShell\(building\);/.test(assets) || /BUILDING_VOXEL_MESHES whole voxel building mesh/.test(assets)));
check("living quarters use resource/chippable stone block assets", /HARTHMERE_LIVING_QUARTERS_STRUCTURAL_BLOCKS = \["mine_stone_01", "mine_stone_02", "arch_wall_stone"\]/.test(assets));
...
check("home assignment summary still maps actors to home ids", /assignHarthmereResidentHome/.test(housing) && /homeId: `\$\{room\.building\.id\}_f\$\{room\.floor\}_r\$\{room\.room\}`/.test(housing));

check("route graph current exists", routes.includes("HARTHMERE_TOWN_ROUTE_GRAPH_VERSION"));
check("route schedule current exists", schedules.includes("HARTHMERE_TOWN_NPC_ROUTE_SCHEDULE_VERSION"));
check("renderer has npc route distribution marker", assets.includes("HARTHMERE_NPC_ROUTE_DISTRIBUTION_VERSION"));
check("town actors get route wander, not static piling", /makeHarthmereTownActorRouteWander/.test(assets) && /routeLabel: routeKey/.test(assets));
...
check("renderer repositions initial actor placements to route anchors", /applyHarthmereNpcRouteDistribution/.test(assets) && /placementWithHarthmereRuntimeAt\(placement, \[x, placement\.at\[1\], z\]\)/.test(assets));
check("loadAll uses current cleaned/distributed runtime placements", /prepareHarthmereRuntimePlacements\(RUNTIME_PLACEMENTS\)/.test(assets));
check("debug window exposes npc distribution report", /__harthmereNpcDistributionReport/.test(assets));
check("draw loop follows route interpolation", /harthmereRoutePosition\(instance\.wander\.route, progress\)/.test(assets));

const anchorBlock = assets.match(/const HARTHMERE_NPC_ROUTE_ANCHORS = \{([\s\S]*?)\n\} as const satisfies/);
...
check("full placement suite includes current test", suite.includes("test-harthmere-living-quarters-npc-dispersal.cjs"));
