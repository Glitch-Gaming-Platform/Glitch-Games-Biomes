...
const js = read("scripts/harthmere/test-harthmere-install-player-ingame-e2e.cjs");
const sh = read("scripts/harthmere/test-harthmere-install-player-ingame-e2e.sh");
const run = read("scripts/harthmere/run-harthmere-install-player-ingame-local.sh");
const all = `${load}\n${game}\n${js}\n${sh}\n${run}`;

ok(/HARTHMERE_CLIENT_CONTEXT_RENDER_UNBLOCK/.test(load), "ClientLoader has current render-unblock marker");
...
ok(/HARTHMERE_GAME_MOUNT_CONTEXT_BEFORE_RENDER_READY/.test(game), "Game has current early canvas mount marker");
...
ok(/HARTHMERE_INSTALL_PLAYER_INGAME_E2E/.test(js), "current browser E2E marker exists");
...
ok(/test-harthmere-install-player-ingame-e2e\.sh/.test(run), "full local runner ends with full E2E test");
console.log("Harthmere client-context render unblock + player-in-game E2E current validation passed");
