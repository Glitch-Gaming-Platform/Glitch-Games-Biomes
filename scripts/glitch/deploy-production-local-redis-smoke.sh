...
# Authored Harthmere water (the Brell, fountain, trough, and mill race) lives
# in shard_water rather than shard_seed. Existing production shards therefore
# need this reviewed partial-component migration whenever terrain maintenance
# runs; it does not clear shard_diff or any other mutable overlay.
...
run_azure_world_sync_job() {
  ...
  az containerapp job create \
    ...
    --env-vars \
      ...
      HARTHMERE_SKIP_AUTHORED_WATER_MATERIALIZE="${HARTHMERE_SKIP_AUTHORED_WATER_MATERIALIZE:-0}" \
...
push_and_deploy() {
  ...
  update_args+=(
    --set-env-vars
      ...
      GLITCH_TITLE_ID="$GLITCH_TITLE_ID"
      # Force a fresh Azure revision when resuming an already-pushed image.
      # Reusing an old revision also reuses its historical restart counters.
