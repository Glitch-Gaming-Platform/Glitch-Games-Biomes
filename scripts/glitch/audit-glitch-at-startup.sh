...
grep -q "GLITCH_LOCAL_SYNC_BASE_URL" src/client/game/client_config.ts \
...
grep -q "DISCOVERY_KIND" scripts/glitch/run-glitch-local-game-stack.sh \
  && echo "OK runner sets discovery kind" \
  || { echo "FAIL missing discovery kind"; fail=1; }

grep -q "NEXT_PUBLIC_GLITCH_SYNC_BASE_URL" scripts/glitch/run-glitch-local-game-stack.sh \
  && echo "OK runner sets browser sync base" \
  || { echo "FAIL missing browser sync base"; fail=1; }

if grep -q 'start_service "bikkie"' scripts/glitch/run-glitch-local-game-stack.sh; then
