...
export const HARTHMERE_BODY_VERSION = 2 as const;
export const HARTHMERE_APPEARANCE_VERSION = 1 as const;
...
export type HarthmereVoxelBodyConfig = {
  version: typeof HARTHMERE_BODY_VERSION;
...
export type HarthmereCharacterRole =
  ...
  | "wildlife"
  | "undead";
export type HarthmereForwardAxis = "plusZ" | "minusZ";

export type HarthmereEquipmentSlot =
  | "mainHand"
  | "offHand"
  | "head"
  | "back"
  | "hip"
  | "accessory";
export type HarthmereCharacterEquipment = Partial<
...
export type HarthmereClothingItem = {
  ...
  fitScale?: number;
  threeJsVariant?: string;
  hidesBodyZones?: string[];
  materialOverrides?: Record<string, string>;
  palette?: HarthmereOutfitColor | string;
  faction?: string;
  rarity?: "common" | "rare" | "epic" | "legendary";
};

export type HarthmereCharacterClothing = Partial<
  Record<HarthmereClothingSlot, HarthmereClothingItem>
>;

export type HarthmereAttachmentAnchorKey =
  | "head"
  | "neck"
  | "rightHand"
  | "leftHand"
  | "hip"
  | "back";
export type HarthmereCharacterAttachmentAnchors = Record<
  HarthmereAttachmentAnchorKey,
  string[]
>;

export type HarthmereCharacterAppearance = {
  version: typeof HARTHMERE_APPEARANCE_VERSION;
...
export type HarthmereCharacterAppearanceInput = Partial<
  Omit<HarthmereCharacterAppearance, "version" | "face" | "body" | "anchors">
> & {
...
// debug/audit tools all agree on the exact target property for every selection.
export const HARTHMERE_APPEARANCE_BUILDER_FACE_FIELDS = [
  "genderIdentity",
  "pronouns",
  "customPronouns",
  "skinTone",
...
export const HARTHMERE_APPEARANCE_BUILDER_BODY_FIELDS = [
  "bodyType",
  "bodyHeight",
  "shoulderWidth",
  "armLength",
...
export const HARTHMERE_APPEARANCE_BUILDER_FIELDS = [
  ...HARTHMERE_APPEARANCE_BUILDER_FACE_FIELDS,
  ...HARTHMERE_APPEARANCE_BUILDER_BODY_FIELDS,
] as const;

export type HarthmereAppearanceBuilderFaceField =
  (typeof HARTHMERE_APPEARANCE_BUILDER_FACE_FIELDS)[number];
export type HarthmereAppearanceBuilderBodyField =
  (typeof HARTHMERE_APPEARANCE_BUILDER_BODY_FIELDS)[number];
export type HarthmereAppearanceBuilderField =
  (typeof HARTHMERE_APPEARANCE_BUILDER_FIELDS)[number];

const HARTHMERE_APPEARANCE_BUILDER_FIELD_ALIASES: Record<
  string,
  HarthmereAppearanceBuilderField
> = {
...
function canonicalizeHarthmereBuilderAlias(value: string) {
  return value
    .trim()
...
export function canonicalizeHarthmereAppearanceBuilderField(
  ...
  return HARTHMERE_APPEARANCE_BUILDER_FIELD_ALIASES[
    canonicalizeHarthmereBuilderAlias(field)
  ];
}
...
export function applyHarthmereAppearanceBuilderSelection(input: {
  face: Partial<HarthmereVoxelFaceConfig> | undefined;
  body: Partial<HarthmereVoxelBodyConfig> | undefined;
  field: HarthmereAppearanceBuilderField | string;
  value: string;
}): {
  face: HarthmereVoxelFaceConfig;
  body: HarthmereVoxelBodyConfig;
  ...
  if (!canonicalField) {
    return { face, body, applied: false };
  }

  if (isHarthmereAppearanceBuilderFaceField(canonicalField)) {
    const nextFace = normalizeHarthmereFaceConfig({
      ...face,
      [canonicalField]: input.value,
    });
    // Gender has a dependent default pronoun. Only auto-change pronouns when the
    // user changed gender itself; direct Pronouns/Custom Pronouns choices must
    // not be overwritten by this mapping layer.
    if (canonicalField === "genderIdentity") {
      nextFace.pronouns = defaultPronounsForGender(nextFace.genderIdentity);
    }
    return {
      face: nextFace,
      body,
  ...
  const nextBody = normalizeHarthmereBodyConfig({
    ...body,
    [canonicalField]: input.value,
  });
  return {
...
export function applyHarthmereAppearanceBuilderSelectionToAppearance(input: {
  appearance: HarthmereCharacterAppearanceInput | HarthmereCharacterAppearance;
  field: HarthmereAppearanceBuilderField | string;
  value: string;
}): HarthmereCharacterAppearance {
  const appearance = normalizeHarthmereCharacterAppearance(input.appearance);
  ...
  // This helper is intentionally appearance-level, not player-specific. Use it
  // for player previews, saved player configs, generated NPCs, and debug tools
  // whenever a selection should update the same canonical face/body field.
...
export function auditHarthmereAppearanceBuilderFieldMappings() {
  const defaults = normalizeHarthmereCharacterAppearance({
    species: "human",
    role: "player",
    face: DEFAULT_HARTHMERE_PLAYER_FACE,
    body: DEFAULT_HARTHMERE_PLAYER_BODY,
  });
  return HARTHMERE_APPEARANCE_BUILDER_FIELDS.map((field) => {
    const options = isHarthmereAppearanceBuilderFaceField(field)
      ? HARTHMERE_APPEARANCE_BUILDER_FACE_FIELDS
      : HARTHMERE_APPEARANCE_BUILDER_BODY_FIELDS;
...
// These anchor names are intentionally broad. Harthmere assets come from
...
export function normalizeHarthmereFaceConfig(
  ...
  const merged = {
    ...
    ...legacy,
    // Older audit/debug tools and early builder patches used `cheeks`. The
...
export function normalizeHarthmereBodyConfig(
  ...
  const merged = {
    ...
    ...legacy,
    // Early docs/debugging called this field `height`; the actual renderer uses
...
const HARTHMERE_CHARACTER_ROLES: readonly HarthmereCharacterRole[] = [
  "player",
  "civilian",
  "guard",
  "merchant",
...
const HARTHMERE_FORWARD_AXES: readonly HarthmereForwardAxis[] = [
  "plusZ",
  "minusZ",
] as const;
...
function normalizeHarthmereAnchors(
  ...
  const fallback =
    species === "animal"
      ? HARTHMERE_DEFAULT_ANIMAL_ANCHORS
...
function normalizeHarthmereClothing(
  ...
  for (const slot of HARTHMERE_CLOTHING_SLOTS) {
    const item = normalizeHarthmereClothingItem(slot, clothing?.[slot]);
    if (item) {
      result[slot] = item;
    }
  }
...
export function harthmereThreeJsClothingItem(
  ...
  if (!base) {
    const fallbackSlot = overrides.slot ?? "torso";
  ...
  return {
    ...
    slot: overrides.slot ?? base.slot,
    materialOverrides: {
      ...(base.materialOverrides ?? {}),
      ...(overrides.materialOverrides ?? {}),
    },
    hidesBodyZones: overrides.hidesBodyZones ?? base.hidesBodyZones,
  };
}
...
export function defaultHarthmereClothingForRole(
  ...
  if (body.bodyType === "athletic") {
    common.hands = harthmereThreeJsClothingItem("fingerless_gloves");
  }
  if (role === "guard") {
    return {
      ...common,
...
function harthmereNpcPaletteClothingIds(
  ...
  return {
    torso: `${palette}_tunic`,
    legs: `${palette}_trousers`,
  };
}
...
/**
 * Build one deterministic clothing set per NPC instead of only per role.
 *
 * The catalog remains Three.js-first because those items fit the customized
 * body metrics. GLB clothing stays available through modelUrl/renderMode for
 * manually selected production assets, but generated town NPCs should not
...
export function makeHarthmereNpcUniqueClothingSet(
  ...
  if (input.role === "farmer") {
    return {
      ...common,
      ...(pick([true, false, true] as const, seed, 8)
        ? { head: harthmereThreeJsClothingItem("straw_hat") }
        : {}),
      torso: harthmereThreeJsClothingItem(
  ...
  if (input.role === "merchant") {
    return {
      ...common,
      ...(pick([true, false] as const, seed, 8)
  ...
  if (input.role === "bandit" || input.role === "hostile") {
    return {
      ...common,
      torso: harthmereThreeJsClothingItem(
...
export function normalizeHarthmereCharacterAppearance(
  ...
  return {
    version: HARTHMERE_APPEARANCE_VERSION,
    species,
    role,
    face,
    body,
...
export function inferHarthmereCharacterRole(input: {
  name?: string;
  roleHint?: string;
  species?: HarthmereCharacterSpecies;
}): HarthmereCharacterRole {
  const species = input.species ?? inferHarthmereCharacterSpecies(input);
  const text = `${input.name ?? ""} ${input.roleHint ?? ""}`.toLowerCase();
  if (species === "animal") {
    return "wildlife";
  }
  ...
  if (/chapel|father|sister|choir|clergy|temple/.test(text)) {
    return "clergy";
  }
  if (/hunter|trail guide|ranger/.test(text)) {
    return "hunter";
  }
  return "civilian";
}
...
function defaultHarthmereEquipmentForRole(
  ...
  // can draw lightweight voxel proxies now and later swap in GLTF attachments
  // without changing the shared appearance schema.
  if (role === "player") {
    return { mainHand: "iron_longsword", hip: "iron_longsword_scabbard" };
  }
  if (role === "guard") {
    return {
  ...
  if (role === "bandit" || role === "hostile") {
    return {
      ...
      head: "bandit_mask",
      hip: "dagger_sheath",
      accessory: "bandit_belt",
    };
  }
  if (role === "hunter") {
    return { mainHand: "bow", back: "quiver", hip: "hunting_knife" };
  }
  if (role === "farmer") {
    return { mainHand: "work_axe", head: "straw_hat", accessory: "tool_belt" };
  }
  if (role === "merchant") {
    return { back: "merchant_satchel", accessory: "ledger_pouch" };
  }
  if (role === "clergy") {
    return { mainHand: "staff", accessory: "clergy_sash" };
  }
  if (role === "undead") {
    return { accessory: "torn_cloth" };
  }
  if (species === "animal") {
    return {};
  }
...
/**
 * Product-level Minecraft polish pass.
 *
 * Guarantees every non-animal actor has a complete body-fitted clothing set.
 * This is intentionally Three.js-first: rounded voxel silhouettes look better
...
function harthmereEnsureProductMinecraftClothingSetV20(
  ...
  base.torso ??= harthmereThreeJsClothingItem(
    /smith|forge|anvil|blacksmith/.test(roleText)
      ? "blacksmith_apron"
      : /inn|tavern|kettle|server|cook|baker/.test(roleText)
      ? "innkeeper_vest"
  ...
  if (!base.head) {
    const headChoice = /guard|watch|sentry|militia/.test(roleText)
      ? pick(["guard_helmet", "militia_halfhelm"] as const, seed, 37)
      : /farmer|field|stable/.test(roleText)
      ? "straw_hat"
      : /hunter|ranger|trail/.test(roleText)
      ? "hunter_cap"
  ...
  if (!base.back) {
    const backChoice = /merchant|vendor|banker/.test(roleText)
      ? "merchant_satchel"
  ...
  if (!base.weapon) {
    const weaponChoice = /guard|watch|sentry/.test(roleText)
      ? "sword_1handed"
      : /hunter|ranger/.test(roleText)
      ? "bow"
      : /bandit|outlaw|thief|smuggler/.test(roleText)
      ? "dagger"
      : /smith|forge|anvil/.test(roleText)
      ? "tool_hammer"
      : undefined;
    if (weaponChoice) {
      base.weapon = harthmereThreeJsClothingItem(weaponChoice);
    }
  }
  ...
  for (const slot of Object.keys(base) as HarthmereClothingSlot[]) {
...
function makeHarthmereNpcAppearanceConfigBaseV21d(input: {
  ...
  return normalizeHarthmereCharacterAppearance({
    ...
    face,
    body,
    forwardAxis: input.forwardAxis ?? "minusZ",
    equipment: {
...
export function parseHarthmereAppearanceMarker(
  ...
  const start = value.indexOf(HARTHMERE_APPEARANCE_MARKER);
  if (start < 0) {
    return undefined;
  }
  const payloadStart = start + HARTHMERE_APPEARANCE_MARKER.length;
  const end = value.indexOf(HARTHMERE_APPEARANCE_MARKER_END, payloadStart);
  if (end < 0) {
    return undefined;
  }
  try {
...
export function withHarthmereBodyAndFaceMarkers(
  ...
  // truth for face, body, forward axis, equipment, and anchor metadata.
  const appearance = normalizeHarthmereCharacterAppearance({
    species: "human",
    role: "civilian",
    face,
    body,
    forwardAxis: "minusZ",
    source: "markers:face-body-compat",
  });
...
function dispatchHarthmereAppearanceStorageEvent(
  ...
  // without snapshotting all localStorage on every click.
...
export function clearHarthmereOtherCustomizationSessionsForUser(
  ...
  const result: {
    ...
    currentOwnerKey: String(userId ?? ""),
    keptKeys: [],
    removedKeys: [],
  };

  if (!isBrowserStorageAvailable() || !isRealHarthmereUserKey(userId)) {
    return result;
  }
...
export function saveHarthmerePlayerFaceConfig(
  ...
  const key = harthmerePlayerFaceStorageKey(userId);
  const normalized = normalizeHarthmereFaceConfig(face);
  if (!writeHarthmereJsonIfChanged(key, normalized)) {
    return;
  }
  dispatchHarthmereAppearanceStorageEvent("biomes:harthmere-face-changed", {
    key,
...
export function migrateHarthmereAnonymousCustomizationToUser(
  ...
  if (!window.localStorage.getItem(userBodyKey)) {
    const anonymousBody = anonymousBodyKeys
...
export function loadHarthmerePlayerBodyConfig(
  ...
  const [currentKey, ...legacyKeys] = harthmerePlayerBodyStorageKeys(userId);
  for (const key of [currentKey, ...legacyKeys]) {
    ...
    try {
      ...
      // Continue through legacy keys before falling back to the default body.
    }
  }
  return DEFAULT_HARTHMERE_PLAYER_BODY;
}
...
export function saveHarthmerePlayerBodyConfig(
  ...
  const key = harthmerePlayerBodyStorageKey(userId);
  const normalized = normalizeHarthmereBodyConfig(body);
  if (!writeHarthmereJsonIfChanged(key, normalized)) {
    return;
  }
  dispatchHarthmereAppearanceStorageEvent("biomes:harthmere-body-changed", {
    key,
    userId: String(userId ?? ""),
    body: normalized,
  });
...
export {
  ...
  HARTHMERE_CLOTHING_ASSET_LICENSES,
  HARTHMERE_LICENSED_CLOTHING_ASSETS,
  HARTHMERE_ROLE_LICENSED_CLOTHING_DEFAULTS,
  getHarthmereLicensedClothingById,
  getHarthmereLicensedClothingForRole,
  mergeHarthmereLicensedClothingForRole,
  getHarthmereClothingCreditLinesForItemIds,
} from "./harthmere_clothing_asset_manifest";
