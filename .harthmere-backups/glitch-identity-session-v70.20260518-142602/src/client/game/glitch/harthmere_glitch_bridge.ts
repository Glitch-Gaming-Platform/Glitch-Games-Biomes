...
function progressionPayloadFromSnapshot(snapshot: HarthmereGlitchSnapshot, playtimeDeltaSeconds: number) {
  ...
  return {
    ...
    metadata: {
      source: "harthmere-glitch-bridge-v68",
...
class HarthmereGlitchBridgeController {
  ...
  private async validateInstall() {
    try {
      const response = await requestGlitch<any>("validate", {
      ...
      if (response?.disabled) {
        writeStatus({
          mode: "local",
          valid: false,
          lastValidationError: response.reason ?? "server_not_configured",
          playtimeSeconds: this.currentPlaytimeSeconds(),
        });
        return;
      }
      ...
      writeStatus({
        ...
        lastValidationError: this.valid ? undefined : response?.reason ?? "INVALID_INSTALL",
        playtimeSeconds: this.currentPlaytimeSeconds(),
      });
    } catch (error: any) {
      this.valid = false;
  ...
  async saveNow(reason = "manual") {
    if (this.stopped || !this.valid || !this.config.installId) return;
    ...
    const response = await requestGlitch<any>("storeSave", {
      ...
      platform: "web",
      game_version: "harthmere-glitch-v68",
    });
  ...
  async submitProgression(reason = "manual") {
    ...
    await requestGlitch<any>("submitProgression", {
      title_id: this.config.titleId,
      install_id: this.config.installId,
      idempotency_key: `${this.config.installId}:${reason}:${now}`,
      payload,
      trust_level: "client",
      platform: "web",
    });
...
export function useHarthmereGlitchBridge(gameReady: boolean) {
