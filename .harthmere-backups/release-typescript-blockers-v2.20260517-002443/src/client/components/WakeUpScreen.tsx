...
const HarthmereFaceOptionRow = <T extends string>({
  ...
  return (
    <div
      className="group rounded-2xl border border-white/12 bg-white/[0.045] p-3 shadow-[0_14px_34px_rgba(0,0,0,0.18)] ring-1 ring-white/[0.025] transition hover:border-amber-200/30 hover:bg-white/[0.07]"
    ...
    >
      <div className="mb-1.5 flex items-center justify-between gap-2">
        ...
        <div className="max-w-[10rem] truncate rounded-full border border-amber-200/15 bg-amber-200/10 px-2 py-0.5 text-[0.68rem] font-black text-amber-100/95">
        ...
        {options.map((option) => {
          ...
          return (
            <button
              key={option}
              type="button"
              // Use canonical schema field names here. Earlier audits derived a
              // slug from the visible label (for example `cheeks`/`height`),
              // which did not always match the stored schema
              // (`cheekStyle`/`bodyHeight`). These attributes are now the source
              // of truth for field-by-field builder tests.
              data-harthmere-builder-field={field}
              data-harthmere-builder-value={option}
              data-harthmere-builder-selected={selected ? "true" : "false"}
              aria-pressed={selected}
              className={`rounded-xl border px-3 py-2 text-[0.72rem] font-black leading-tight transition focus:outline-none focus:ring-2 focus:ring-amber-200/60 ${
                selected
                  ? "border-amber-200 bg-gradient-to-br from-amber-200/32 to-orange-300/14 text-white shadow-[0_0_22px_rgba(251,191,36,0.22)]"
                  : "border-white/10 bg-black/24 text-white/76 hover:border-amber-100/28 hover:bg-white/10 hover:text-white"
              }`}
...
const HarthmereVoxelBodyPreview: React.FunctionComponent<{
  ...
  const hasCape = Boolean(clothing?.back) && /cape|cloak|shroud/i.test(clothing.back.id);
        ...
        {options.map((item) => {
          ...
          return (
            <button
              ...
              className={`rounded-xl border px-3 py-2 text-[0.72rem] font-black leading-tight transition focus:outline-none focus:ring-2 focus:ring-amber-200/60 ${
                selected
                  ? "border-emerald-200 bg-emerald-200/18 text-white shadow-[0_0_18px_rgba(110,231,183,0.16)]"
                  : "border-white/10 bg-black/28 text-white/72 hover:border-white/30 hover:bg-white/10 hover:text-white"
              }`}
      ...
      >
        <div data-harthmere-builder-layout="v21-release-polish-clothing" className="grid max-h-[calc(100vh-6.25rem)] min-h-[min(40rem,calc(100vh-6.25rem))] w-full grid-cols-1 gap-5 overflow-hidden text-left lg:grid-cols-[minmax(22rem,30rem)_minmax(0,1fr)]">
          <aside className="relative flex min-h-0 flex-col gap-4 overflow-hidden rounded-[2rem] border border-amber-200/20 bg-[radial-gradient(circle_at_50%_0%,rgba(251,191,36,0.18),rgba(15,23,42,0.94)_34%,rgba(2,6,23,0.96)_100%)] p-4 shadow-[0_24px_70px_rgba(0,0,0,0.42)] lg:row-span-2">
            ...
            <div className="preview-container relative min-h-[20rem] flex-1 overflow-hidden rounded-[1.75rem] border border-white/12 bg-[radial-gradient(circle_at_50%_18%,rgba(251,191,36,0.22),rgba(76,29,149,0.2)_38%,rgba(2,6,23,0.82)_100%)] p-3 shadow-inner lg:min-h-[24rem]">
              <CharacterPreview
