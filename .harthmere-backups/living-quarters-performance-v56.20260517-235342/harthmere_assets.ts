...
function prepareHarthmereRuntimePlacementsV3(
  ...
  const floating = filterHarthmereUnsupportedFloatingBlockPlacementsV3(placements);
  ...
  return { placements: runtimePlacements, removedFloating: floating.removed, removedForPerformance };
}
...
function createHarthmereResidentStoryFrameV38(
  ...
  // V49 supersedes V48: continuous 1 m solid voxel stone walls (no walk-
  // through gaps), per-room balcony doors on every upper floor, stacked
  // solid-block voxel stair, exterior south balcony with stone railing,
  // interior partition walls, walkable floor and ceiling slabs. Existing
  // building IDs and room indexes preserved so NPC home assignments stay
  // attached.
  return createHarthmereLivingQuarterVoxelShellV49(building);
}
