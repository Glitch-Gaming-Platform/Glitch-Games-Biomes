...
ok(
  runtime.includes("animate-pulse") &&
    (runtime.includes("Watch the blinking HUD item") ||
      ...
      /panel is|panels are/.test(runtime)),
  "HUD highlights/blinks the next required player control (v106 or v107 naturalized callout)",
);
ok(
  runtime.includes("disabled={!practiceIsInRange}") &&
    (runtime.includes("Move closer to practice") || /Walk to .* first/.test(runtime)),
  "contextual practice controls require the player to be near the marked location (v106 or v107 wording)",
);
