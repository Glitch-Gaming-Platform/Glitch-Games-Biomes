# Harthmere Azure Voice and Speech V1
...
```text
src/shared/harthmere/npc_voice_profiles_v1.ts
src/shared/harthmere/npc_voice_catalog_v1.ts
```

Conversation tone and influence mapping for the current NPC cast lives in:

```text
docs/harthmere/HARTHMERE_NPC_CONVERSATION_TONE_MAP_V1.md
```

The runtime delivery rules live in:

```text
src/shared/harthmere/npc_speech_delivery_v1.ts
```
...
```text
azure-speech-v1|voice=en-US-LunaNeural|style=conversation|styleDegree=0.95|rate=-3%|pitch=+1%|volume=default|break=150|actor=...
```
...
node scripts/harthmere/generate-harthmere-npc-voice-recordings-v1.cjs --limit=25
```

The script writes MP3s and a manifest under:

```text
public/harthmere/voices/generated/v1/
```
