...
node scripts/harthmere/check-snapshot-glitch-merge-all.cjs
```
...
```bash
node scripts/harthmere/dump-harthmere-npc-placement-state.cjs
BIOMES_ENABLE_HARTHMERE_EXTRA_TOWN=1 node scripts/harthmere/dump-harthmere-npc-placement-state.cjs
node scripts/harthmere/dump-snapshot-quest-mission-state.cjs
node scripts/harthmere/dump-snapshot-pre-mission-integration-state.cjs
```
