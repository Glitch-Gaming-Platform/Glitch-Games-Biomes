...
- Voices are full volume within 4 world units, fade to silence at 24 units,
  and the WebRTC peer is closed after players move more than 40 units apart.
...
3. Keep the returned participant `voice_token` only in client memory.
...
For reliable production connections across restrictive NAT/firewall setups,
...
When this value is absent, the server publishes public STUN fallbacks in the
room connection configuration. TURN is still recommended for production.
...
profiles and move the players within 24 units. Toggle the minimap microphone or
press F8 in one profile and confirm the second profile hears audio, then move
beyond 24 units and confirm it fades to silence.
