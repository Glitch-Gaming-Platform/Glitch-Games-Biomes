...
bash scripts/harthmere/install-harthmere-ai-deps.sh \
  /Users/devindixon/Development/biomes-game
```
...
3. Add the test to `scripts/harthmere/test-harthmere-town-placement-suite.cjs`.
4. Keep local-dev state keys canonical and migrate old keys through explicit legacy readers.
...
```bash
node -c scripts/harthmere/test-harthmere-town-placement-suite.cjs
```

Then inspect the first 80 lines:

```bash
sed -n '1,80p' scripts/harthmere/test-harthmere-town-placement-suite.cjs
```
...
```bash
bash scripts/harthmere/run-harthmere-npm-peer-audit.sh \
  /Users/devindixon/Development/biomes-game
```
