# Biomes HUD UI Screen Review V96

This patch reviews the May 21 HUD screenshots and turns the visible problems into testable HUD contracts.
...
- `biomes-rules/snapshot_grove_harthmere_lore_bible_v1.pdf`: Grove/Harthmere objective state must stay consistent across HUD, map, journal, and NPC dialogue.
...
Fixes:
- Added `harthmere_hud_key_bindings_v96.ts` as the central key contract.
...
Fixes:
- Added `harthmereResourceIconForItemV96()` so crafting materials get item-specific icons.
- Added matching gathering-storage icons through `harthmereGatheringResourceIconV96()`.
...
Fixes retained from V95 and protected in V96:
...
- `src/shared/harthmere/harthmere_hud_key_bindings_v96.ts`
- `src/shared/harthmere/test/harthmere_hud_key_bindings_v96.test.ts`
- `scripts/harthmere/check-biomes-hud-key-bindings-v96.cjs`
- `docs/harthmere/BIOMES_HUD_UI_SCREEN_REVIEW_V96.md`

## Validation commands

```bash
node scripts/harthmere/check-biomes-hud-key-bindings-v96.cjs .
```

Optional, when repo dependencies are installed:

```bash
npx mocha src/shared/harthmere/test/harthmere_hud_key_bindings_v96.test.ts
```
