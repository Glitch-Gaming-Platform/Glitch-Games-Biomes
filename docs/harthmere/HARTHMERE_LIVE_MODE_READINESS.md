...
This document tracks the live-mode-ready contracts and tests for Harthmere combat and progression. It does not claim that the production multiplayer backend exists yet. Instead, it defines the authoritative envelopes, required action pipelines, persistence mutation plans, idempotency rules, UI event outbox, audit logs, and end-to-end simulation scenarios that the eventual live server must call.

## What is now live-mode ready

- Attacks and ability casts must enter `validateHarthmereLiveModeAuthorityEnvelope` before mutation.
